...
//---Bad Run
    int  AddBadRun(unsigned int badrunb,unsigned int badrune=0);
    int  AddBadRun(unsigned int badrun[],int nrun);
...
int QRunManager::SetFDir(const char *fdir,const char *ntxt){
  ...
  if(fdir){
    qfdir=fdir;
    if(qfdir.size()>=2){//remove '' or ""
      if((qfdir.front()=='\''&&qfdir.back()=='\'')||(qfdir.front()=='\"'&&qfdir.back()=='\"')){
        qfdir=qfdir.substr(1,qfdir.size()-2);
      }
    }
  }
  qntxt=ntxt;
...
int QRunManager::AddBadRun(unsigned int badrunb,unsigned int badrune){
  int isnew=1;
  if(badrunb==badrune||badrune==0){//single run
    vector<unsigned int>::iterator iter=std::find(qbadrunlist.begin(),qbadrunlist.end(),badrunb);
    if(iter==qbadrunlist.end())qbadrunlist.push_back(badrunb);//New bad run
    else isnew=0;//already has
  }
  else {//run range
...
///----
int QRunManager::AddBadRun(unsigned int badrun[],int nrun){
  int nadd=0;
  for(int irun=0;irun<nrun;irun++){
    unsigned int run=badrun[irun];
    nadd+=AddBadRun(run);
  }
  return nadd;
}
...
int QRunManager::AddBadRun(vector<unsigned int > &badrunlist){
  ...
  for(unsigned int irun=0;irun<badrunlist.size();irun++){
    unsigned int run=badrunlist[irun];
    nadd+=AddBadRun(run);
  }
  return nadd;
}
...
//--previous stat
  if(optforce==0&&run==qbadrunn.first)return qbadrunn.second; //YC: withoug forcing the calculate IsBadRun, and if it's the current run (qbadrunn.first), simply returns the previously stored IsBadRun result (qbadrunn.second)
  qbadrunn.first=run;
//---
  qbadrunn.second=0;//goodrun by default 
  //YC: two kinds of bad run, continues range of bad run (qbadrunbe) or concrete bad runs (qbadrunlist). Treat them diffrently
  for(unsigned int i=0;i<qbadrunbe.size();i++){
...
int  QRunManager::ProcessFiles(string fn,unsigned int run){
   if(qprocessfile.find(fn)==qprocessfile.end())qprocessfile[fn]=run;
   return 0;
}
...
//     unsigned bp=fname.rfind("/");
     ...
     fname=indir+"/"+fname;
     if(endadd>0){
       if     (fonde!=string::npos)fname="root://eosams.cern.ch/"+fname;
...
int QRunManager::LoadRunList(unsigned int time[2],vector<unsigned int > &runlist){
  ...
  for(int tn=time[0];tn<time[1];tn++){
     ...
     if((tmp->tm_year!=tpry||tmp->tm_mon!=tprm||tmp->tm_mday!=tprd)){
        ...
        if(qntxt!=0)sprintf(nfile1,"%s/%sRunA_%s-24H.cvs",qfdir.c_str(),qntxt,nfile);
        else        sprintf(nfile1,"%s/RunA_%s-24H.cvs",qfdir.c_str(),nfile);
...
//   cout<<"grun="<<run<<" optinsert="<<optinsert<<endl;
   if(qfdir.size()==0)return 0;//not find
...
void QRunManager::GetNewRunList(vector<unsigned int >& runlist,vector<unsigned int >& runlistn){
  for(vector<unsigned int > ::iterator it=runlist.begin();it!=runlist.end();it++){
     if(FindRun(*it,0)==0){runlistn.push_back(*it);cout<<"listnew run="<<(*it)<<endl;}//Not Exist
     else                 {cout<<"listold run="<<(*it)<<endl;}
  }
}
