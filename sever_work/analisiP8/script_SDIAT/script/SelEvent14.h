...
// int iFVTight=2;
int iFVTight=0; //-- have no pattern cut
...
public:
	...
	//----list of bad run
	#ifdef USEQTOOL
	int SelectGoodRun(int opt=0); ///opt=100 use the photon trigger
	// int SelectGoodRun(int opt=100); ///opt=100 use the photon trigger
	#endif //USEQTOOL
...
bool SelEvent::Select_Beta() {
	...
	// return tof_betah>((GetTkInZ()<=2)?0.3:0.4); // if L1Z > 2 InnerZ<=2, mistake
...
bool SelEvent::Select_TrTrack(const int igeom, const int chisOpt, const int ialgo, const int ipgc) const
{
	...
	bool l1innerl9HitChiY = (chisOpt%1000>=100)?(GetChis(2, 1, ialgo, ipgc)<10):true;
	// bool l1innerl9HitChiY = (chisOpt%1000>=100)?((GetChis(2, 1, ialgo, ipgc)<10)&&((chisOpt%100>=10)?(GetL19ChisY(ialgo, ipgc)<10):true)):true;//l1Innerl9										//l1Innerl9
...
bool SelEvent::Select_ExtQ(const int ich, int igeom, const int iVer, const float fTight, const bool bL1QUCut)
{
	...
	//2025.09.06 v4
	bool bl1QXYDiff=true;
	double l1qxn=GetTkLQNew(0,0,iVer);
	double l1qyn=GetTkLQNew(0,1,iVer);
	if(ich>=9) bl1QXYDiff=fabs(l1qxn-l1qyn)/(l1qxn+l1qyn)<0.2;
	bool bl9QXYDiff=true;
	double l9qxn=GetTkLQNew(8,0,iVer);
	double l9qyn=GetTkLQNew(8,1,iVer);
	if(ich>=9) bl9QXYDiff=fabs(l9qxn-l9qyn)/(l9qxn+l9qyn)<0.2;

	bool bExtQ[4];
	bExtQ[0] = true;
	bExtQ[1] = bl1Q && bl1Qs && bl1QXYDiff;
	bExtQ[2] = bl1Q && bl1Qs && bl9Q && bl1QXYDiff && bl9QXYDiff;
	bExtQ[3] = bl9Q && bl9QXYDiff;
	return bExtQ[igeom];
}
