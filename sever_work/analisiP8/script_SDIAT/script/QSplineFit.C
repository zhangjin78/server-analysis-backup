...
class QSplineFit{
 ...
 public:
  ...
  int  FillFlux(TH1 *hflux,double scaleindex=0);
...
//---Add Flux Extrapolate 
  if(_sfitmode&ExtrapolateFlux){
    ...
    if(_verbose>=1){
       cout<<"xv="<<x[0]<<" log(xv)="<<xv<<" index="<<kx<<endl;
    } 
...
TF1 *QSplineFit::GetFun(double xmin,double xmax){
  ...
  for(int ino=0;ino<qnnodes;ino++){
    ...
    else if(qh) qfun->SetParameter(ino,qh->Interpolate(qxnodes[ino]));//Histo Second Order
...
TFitResultPtr QSplineFit::DoFit(Option_t *option ,Option_t *goption, Double_t xxmin, Double_t xxmax){
  if(qgr){
    ...
    string opt(option),gopt(goption);
...
int QSplineFit::FillFlux(TH1 *hflux,double scaleindex){
//---
 for(int ib=1;ib<=hflux->GetNbinsX();ib++){
   double xv=hflux->GetXaxis()->GetBinCenterLog(ib);
   double yv=GetFun()->Eval(xv)*pow(xv,scaleindex);
