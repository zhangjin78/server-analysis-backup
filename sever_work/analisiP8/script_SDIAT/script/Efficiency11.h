...
ofstream fout("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/testoutput/ChargeAboveL1Qplus2_MC.txt");
...
#endif //AMSOFT
{
	...
	for (it=viQSel.begin(); it!=viQSel.end(); it++)
	{
		...
		for (int isamt=-1; isamt<iNSam; isamt++)
		{
			...
			bTofQEffSam = ev.Select_InnerQ(*it, QVer, dTight) && ev.Select_ExtQ(*it, 2, QVer, dTight); //L9 cut //2025.09.16
			if(span == 1){
				bTofQEffSam = bTofQEffSam && ev.Select_TofQ(*it, 3, QVer); // Add LTOF cut //2025.09.16
			}
			else if(span == 2){
				bTofQEffSam = bTofQEffSam && ev.Select_TofQ(*it, 1, QVer);
			}
				...
				for (int ieff=0; ieff<3; ieff++) //2023.02.22: ieff=2 for tighten L1PUEff sample for primary
				{
					...
					for (int irig=0; irig<NRig; irig++) //2022.10.03
					{
						...
						for (int isam=0; isam<3; isam++){
							...
							if((isam == 1) && (ieff == 0)){
								for(int isel = 1; isel <= 4; isel++){
									histn = Form("unbiasedL1QvsRigHZBin_r%d_q%d_%s%d_Sel%d", ir, *it, sEff.c_str(), isam, isel);
									histn += sRig; //2022.10.03
									histn += sName; //2022.10.16
									hman1.Add(new TH2F(histn.c_str(), histn.c_str(), getNbinPub(14), getBinsPub(14), 300, 0, 30));
								}
							...
							if(isam == 1){
								for(int isel = 1; isel <= 6; isel++){
									histn = Form("L1QvsRigHZBin_r%d_q%d_%s%d_Sel%d", ir, *it, sEff.c_str(), isam, isel);
									histn += sRig; //2022.10.03
									histn += sName; //2022.10.16
									hman1.Add(new TH2F(histn.c_str(), histn.c_str(), getNbinPub(14), getBinsPub(14), 300, 0, 30));
								}

								// histn = Form("L1InnerNormChisqvsRigHZBin_r%d_q%d_%s%d_wXYhit", ir, *it, sEff.c_str(), isam);
								// histn += sRig; //2022.10.03
								// histn += sName; //2022.10.16
								// hman1.Add(new TH2F(histn.c_str(), histn.c_str(), getNbinPub(14), getBinsPub(14), 1000, 0, 100));

								// histn = Form("L1InnerNormChisqvsRigHZBin_r%d_q%d_%s%d_wXYhitChisqL1Inner", ir, *it, sEff.c_str(), isam);
								// histn += sRig; //2022.10.03
								// histn += sName; //2022.10.16
								// hman1.Add(new TH2F(histn.c_str(), histn.c_str(), getNbinPub(14), getBinsPub(14), 1000, 0, 100));

								// histn = Form("L1InnerNormChisqvsRigHZBin_r%d_q%d_%s%d_wXYhitChisqL1InnerL1Charge", ir, *it, sEff.c_str(), isam);
								// histn += sRig; //2022.10.03
								// histn += sName; //2022.10.16
								// hman1.Add(new TH2F(histn.c_str(), histn.c_str(), getNbinPub(14), getBinsPub(14), 1000, 0, 100));
								...
								// 	histn = Form("L1InnerNormChisqvsRigHZBin_r%d_q%d_%s%d_wXYhitChisqL1InnerL1ChargeChisqL1", ir, *it, sEff.c_str(), isam);
								// 	histn += sRig; //2022.10.03
								// 	histn += sName; //2022.10.16
								// 	hman1.Add(new TH2F(histn.c_str(), histn.c_str(), getNbinPub(14), getBinsPub(14), 1000, 0, 100));
								// }
...
#endif //AMSOFT
{
	...
	for (it=viQSel.begin(); it!=viQSel.end(); it++)
	{
		...
		double l1qx = ev.GetUBExtQTrkCor(0, 0, QVer);
		double l1qy = ev.GetUBExtQTrkCor(0, 1, QVer);
		bool bL1Qxydiff = ( abs( (l1qx-l1qy) / (l1qx+l1qy) ) ) < 0.2;
		bL1BZSel = bL1BZSel && bL1Qxydiff;
		...
		double tl1qx = ev.GetTkLQNew(0, 0, QVer);
		double tl1qy = ev.GetTkLQNew(0, 1, QVer);
		bool bTL1Qxydiff = ( abs( (tl1qx-tl1qy) / (tl1qx+tl1qy) ) ) < 0.2;
		bL1PUSel = bL1PUSel && bTL1Qxydiff;
		//2022.10.16
		if (ir==1)
		{
		...
		for (int isamt=-1; isamt<iNSam; isamt++)
		{
			...
			if (bL1Sam)
			{
				...
				for (int igr=1; igr>=0; igr--)
				{
					...
					if (igr==1)
					{
						...
						if(!ev.isreal){
							...
							if(ev.GetTkLQNew(0, 2, QVer) > *it + 2){
								...
								histn += sRig; //2022.10.03
								histn += sName; //2022.10.16
								if(ev.PrimaryUpToTrLayer(1)) GetHist(hman1, histn.c_str())->Fill(dRigStore, weight);

								fout << "run:" << ev.run << "\t event:" <<ev.event << endl;
							}
					...
					// }
					//--2025.09.16
					if( (igr==1) && ev.GetUBExtQTrkCor(0, 0, QVer)>0 && ev.GetUBExtQTrkCor(0, 1, QVer)>0){
						histn = Form("unbiasedL1QvsRigHZBin_r%d_q%d_%s%d_Sel1", ir, *it, sEff.c_str(), 1);
						histn += sRig; //2022.10.03
						histn += sName; //2022.10.16
						GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetUBExtQTrkCor(0, 2, QVer), weight);
						if((ev.GetUBExtQTrkCor(0, 2, QVer)>ev.GetQSelRange(0, *it, 4, QVer))){
							histn = Form("unbiasedL1QvsRigHZBin_r%d_q%d_%s%d_Sel2", ir, *it, sEff.c_str(), 1);
							histn += sRig; //2022.10.03
							histn += sName; //2022.10.16
							GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetUBExtQTrkCor(0, 2, QVer), weight);
							if(((ev.GetUBExtQStatusTrkCor(0,QVer) & 0x10013D)==0)){
								histn = Form("unbiasedL1QvsRigHZBin_r%d_q%d_%s%d_Sel3", ir, *it, sEff.c_str(), 1);
								histn += sRig; //2022.10.03
								histn += sName; //2022.10.16
								GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetUBExtQTrkCor(0, 2, QVer), weight);
								if(bL1Qxydiff){
									histn = Form("unbiasedL1QvsRigHZBin_r%d_q%d_%s%d_Sel4", ir, *it, sEff.c_str(), 1);
									histn += sRig; //2022.10.03
									histn += sName; //2022.10.16
									GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetUBExtQTrkCor(0, 2, QVer), weight);
								}
				...
				for (int itightsam=0; itightsam<2; itightsam++)
				{
					...
					if (!(bL1BZSel && ev.GetUBExtQTrkCor(0, 2, QVer)<ev.GetQSelRange(1, *it, 4, QVer))) continue; //with unbiased L1Q upper limit 
					for (int igr=1; igr>=0; igr--)
					{
						if (igr==0 && ev.isreal) continue;
						double dRigStore = (igr==0)?ev.mmom/ev.mch:ev.GetRigidity(iRigSpan, irig);
						...
						bool bL1PUSel_wXYHit_2 = bL1PUSel_wXYHit && ev.GetChis(1, 1, irig) < 10;
						bool bL1PUSel_wXYHit_3 = bL1PUSel_wXYHit_2 && (ev.GetTkLQNew(0, 2, QVer)>ev.GetQSelRange(0, *it, 4, QVer)) && ((ev.tk_qls[0]&0x10013D)==0);
						bool bL1PUSel_wXYHit_4 = bL1PUSel_wXYHit_3 && ev.GetL1ChisY(irig)<10;
						...
						if(igr == 1){
							if(ev.HasTkLHitXY(0)==2){
								histn = Form("L1QvsRigHZBin_r%d_q%d_%s%d_Sel1", ir, *it, sEff.c_str(), 1);
								histn += sRig; //2022.10.03
								histn += sName; //2022.10.16
								GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetTkLQNew(0, 2, QVer), weight);
								if((ev.tk_qls[0]&0x10013D)==0){
									histn = Form("L1QvsRigHZBin_r%d_q%d_%s%d_Sel2", ir, *it, sEff.c_str(), 1);
									histn += sRig; //2022.10.03
									histn += sName; //2022.10.16
									GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetTkLQNew(0, 2, QVer), weight);
									if((ev.GetTkLQNew(0, 2, QVer)>ev.GetQSelRange(0, *it, 4, QVer))){
										histn = Form("L1QvsRigHZBin_r%d_q%d_%s%d_Sel3", ir, *it, sEff.c_str(), 1);
										histn += sRig; //2022.10.03
										histn += sName; //2022.10.16
										GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetTkLQNew(0, 2, QVer), weight);
										if(ev.GetChis(1, 1, irig) < 10){
											histn = Form("L1QvsRigHZBin_r%d_q%d_%s%d_Sel4", ir, *it, sEff.c_str(), 1);
											histn += sRig; //2022.10.03
											histn += sName; //2022.10.16
											GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetTkLQNew(0, 2, QVer), weight);
											if(bTL1Qxydiff){
												histn = Form("L1QvsRigHZBin_r%d_q%d_%s%d_Sel5", ir, *it, sEff.c_str(), 1);
												histn += sRig; //2022.10.03
												histn += sName; //2022.10.16
												GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetTkLQNew(0, 2, QVer), weight);
												if(ev.GetL1ChisY(irig)<10){
													histn = Form("L1QvsRigHZBin_r%d_q%d_%s%d_Sel6", ir, *it, sEff.c_str(), 1);
													histn += sRig; //2022.10.03
													histn += sName; //2022.10.16
													GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetTkLQNew(0, 2, QVer), weight);
												}
							...
							// if(bL1PUSel_wXYHit){
							// 	histn = Form("L1InnerNormChisqvsRigHZBin_r%d_q%d_%s%d_wXYhit", ir, *it, sEff.c_str(), 1);
							// 	histn += sRig; //2022.10.03
							// 	histn += sName; //2022.10.16
							// 	GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetChis(1,1,irig), weight);
							// }
							// if(bL1PUSel_wXYHit_2){
							// 	histn = Form("L1InnerNormChisqvsRigHZBin_r%d_q%d_%s%d_wXYhitChisqL1Inner", ir, *it, sEff.c_str(), 1);
							// 	histn += sRig; //2022.10.03
							// 	histn += sName; //2022.10.16
							// 	GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetChis(1,1,irig), weight);
							// }
							// if(bL1PUSel_wXYHit_3){
							// 	histn = Form("L1InnerNormChisqvsRigHZBin_r%d_q%d_%s%d_wXYhitChisqL1InnerL1Charge", ir, *it, sEff.c_str(), 1);
							// 	histn += sRig; //2022.10.03
							// 	histn += sName; //2022.10.16
							// 	GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetChis(1,1,irig), weight);
							// }
							// if(ir == 1 && bL1PUSel_wXYHit_4){
							// 	histn = Form("L1InnerNormChisqvsRigHZBin_r%d_q%d_%s%d_wXYhitChisqL1InnerL1ChargeChisqL1", ir, *it, sEff.c_str(), 1);
							// 	histn += sRig; //2022.10.03
							// 	histn += sName; //2022.10.16
							// 	GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetChis(1,1,irig), weight);
							// }
		...
		for (int isamt=-1; isamt<iNSam; isamt++)
		{
			...
			for (int ir=1; ir<3; ir++)
			{
				for (int irig=0; irig<NRig; irig++)
				{
					...
					//--Check diffenerce between MC and data //2024.07.06
					if(isamt == -1){
						histn = Form("L1QvsInnerQ_r%d_q%d_%s_2p40T2p67", ir, *it, sEff.c_str());
						histn += sRig; //2023.03.21
						hman1.Add(new TH2F(histn.c_str(), histn.c_str(), 350, 0, 35, 350, 0, 35));
						histn = Form("L1QvsInnerQ_r%d_q%d_%s_4p02T4p43", ir, *it, sEff.c_str());
						histn += sRig; //2023.03.21
						hman1.Add(new TH2F(histn.c_str(), histn.c_str(), 350, 0, 35, 350, 0, 35));
						histn = Form("UbL1QvsInnerQ_r%d_q%d_%s_2p40T2p67", ir, *it, sEff.c_str());
						histn += sRig; //2023.03.21
						hman1.Add(new TH2F(histn.c_str(), histn.c_str(), 350, 0, 35, 350, 0, 35));
						histn = Form("UbL1QvsInnerQ_r%d_q%d_%s_4p02T4p43", ir, *it, sEff.c_str());
						histn += sRig; //2023.03.21
						hman1.Add(new TH2F(histn.c_str(), histn.c_str(), 350, 0, 35, 350, 0, 35));
					}
					...
					for (int isam=0; isam<2; isam++){
						...
						histn = Form("InnerQvsRigHZBin_r%d_q%d_%s%d_noReweight", ir, *it, sEff.c_str(), isam);
						histn += sRig; //2023.03.21
						hman1.Add(new TH2F(histn.c_str(), histn.c_str(), getNbinPub(14), getBinsPub(14), 350, 0, 35));
...
#endif //AMSOFT
{
	...
	for (it=viQSel.begin(); it!=viQSel.end(); it++)
	{
		...
		// bTkQEffSel = ev.Select_SubDetQ(*it, 0, QVer); // 2024.07.23
		float dInnerQ= ev.GetSubDetQ(0, QVer);
		// bTkQEffSel = (dInnerQ > *it-0.3) && (dInnerQ < *it+0.3); // 2024.09.12 for Range[-0.3,0.3]
		bTkQEffSel = (dInnerQ > *it-0.5) && (dInnerQ < *it+0.5); // 2025.09.16 for Range[-0.5,0.5]
		...
		for (int isamt=-1; isamt<iNSam; isamt++)
		{
			...
			/// tof Q cut // 2025.09.16
			bTkQEffSam = bTkQEffSam && ev.GetTOFUDQ(0) > (*it-dTight*0.5) && ev.GetTOFUDQ(0) < (*it+dTight*0.5);
			bTkQEffSam = bTkQEffSam && ev.GetTOFUDQ(1) > (*it-dTight*0.5) && ev.GetTOFUDQ(1) < (*it+dTight*1.5);
			bTkQEffSam = bTkQEffSam && (ev.HasTkLHitXY(0)==2);
			double l1qx = ev.GetUBExtQTrkCor(0, 0, QVer);
			double l1qy = ev.GetUBExtQTrkCor(0, 1, QVer);
			bool bL1Qxydiff = ( abs( (l1qx-l1qy) / (l1qx+l1qy) ) ) < 0.2;
			bTkQEffSam = bTkQEffSam && bL1Qxydiff;
			...
			if (bTkQEffSam && ev.Select_TkLQStatus(0))
			{
				for (int igr=1; igr>=0; igr--)
				{
					...
					if (igr==1)
					{
						...
						/////2024.07.06
						if(isamt == -1){
							histn = Form("L1QvsInnerQ_r%d_q%d_%s_2p40T2p67", span, *it, sEff.c_str());
							histn += sRig;
							if(dRigStore > 2.4 && dRigStore < 2.67) GetHist2d(hman1, histn.c_str())->Fill(ev.GetSubDetQ(0, QVer), ev.GetTkLQNew(0,2,QVer), weight);
							histn = Form("L1QvsInnerQ_r%d_q%d_%s_4p02T4p43", span, *it, sEff.c_str());
							histn += sRig;
							if(dRigStore > 4.02 && dRigStore < 4.43) GetHist2d(hman1, histn.c_str())->Fill(ev.GetSubDetQ(0, QVer), ev.GetTkLQNew(0,2,QVer), weight);
							
							histn = Form("UbL1QvsInnerQ_r%d_q%d_%s_2p40T2p67", span, *it, sEff.c_str());
							histn += sRig;
							if(dRigStore > 2.4 && dRigStore < 2.67) GetHist2d(hman1, histn.c_str())->Fill(ev.GetSubDetQ(0, QVer), ev.GetUBExtQTrkCor(0,2,QVer), weight);
							histn = Form("UbL1QvsInnerQ_r%d_q%d_%s_4p02T4p43", span, *it, sEff.c_str());
							histn += sRig;
							if(dRigStore > 4.02 && dRigStore < 4.43) GetHist2d(hman1, histn.c_str())->Fill(ev.GetSubDetQ(0, QVer), ev.GetUBExtQTrkCor(0,2,QVer), weight);
						}
						...
						histn = Form("InnerQvsRigHZBin_r%d_q%d_%s%d_noReweight", span, *it, sEff.c_str(), 0);
						histn += sRig;
						GetHist2d(hman1, histn.c_str())->Fill(dRigStore, ev.GetSubDetQ(0, QVer));
						...
						// if(dRigStore > 200.) cout << "check Eff TkQ ; \trigidity: " << dRigStore << " \t weight: " << weight << endl; 
						#endif //SELCHECK
					}
