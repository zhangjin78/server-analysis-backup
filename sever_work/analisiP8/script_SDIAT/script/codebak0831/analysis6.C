...
int analysis(TChain &ch, string odir, string ofile, long int nrun=-1, const int iGeoSafety=1)
{
	...
	// cout << __LINE__ << "\ttotal entries: " << nrun2 << endl;
	...
	// (int ievent=2500000; ievent<nrun2; ievent++)
	{
		...
		{
			RunT_EX[1] = ev->time[0];
			if (RunT_EX[1] > RunT_EX[0]) //store expo-time for later time
			{
				if (ev->Select_Run() && ev->Select_RTI() && ev->Select_RTIdL1L9())
				{
					for (it=viQSel.begin(); it!=viQSel.end(); it++)
					{
						// for (int id=0; id<4; id++)
						for (int id=1; id<2; id++)
						{
							...
							TH1D *h1expot = static_cast<TH1D*>(GetHist(hman1, histn.c_str()));
							...
							TH1D *h4expot = static_cast<TH1D*>(GetHist(hman1, histn.c_str()));
					...
					for (it=viQSel.begin(); it!=viQSel.end(); it++)
					{
						for (int id=1; id<2; id++){
							...
							for(int igeo=12;igeo<=12;igeo++)
							{
								...
								double ucutr= double(igeo)/10. * ev->mcutoffi[id][1];//IGRF Cutoff  Recommended
