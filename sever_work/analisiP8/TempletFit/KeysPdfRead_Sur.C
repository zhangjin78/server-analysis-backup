				...
				const int ispan=1)
{
	if(bLog){
		// #undef LINEARY
		// cout << "Using Log Scale_____" << endl;
	}
	//----draw setting
	gStyle->SetOptStat("");
	...
	// TFile fL1Q(Form("%s/amsd69nChenYa_B1130_SurvivalProbability_L1RangePM03.root", sDataDir.c_str()), "READ");
	...
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_CutHe_Z7TZ18_rho2p0/LQTemp_rig%d.root", sTempDir.c_str(), iRigTemp), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_CutHe_Z24TZ28_rho2p0/LQTemp_rig%d.root", sTempDir.c_str(), iRigTemp), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_CutHe_Z10TZ16_rho2p0/LQTemp_rig%d.root", sTempDir.c_str(), iRigTemp), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_Z3TZ18_rho2p0/LQTemp_rig%d.root", sTempDir.c_str(), iRigTemp), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_TransRoot_Clockwise_rho2p0/LQTemp_rig%d.root", sTempDir.c_str(), iRigTemp), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_TransRoot_Anticlockwise_rho2p0/LQTemp_rig%d.root", sTempDir.c_str(), iRigTemp), "READ");
	...
	for (int ir=ispan; ir<ispan+1; ir++){
		...
		//----output directory
		string outdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/OutputFile/CutHe/FreeShift/AllL1PDF";
		if(bOddL2) outdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/OutputFile/CutHe/FreeShift/L1L2PDF";
		// outdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/OutputFile/CutHe/FreeShift/L1L2PDF/L1Range";
		// if(bReShift) outdir += "_ReShift";
		...
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			...
			// TH1F *hShiftFit = (TH1F*)fShift.Get(Form("Shift/Shift_r1_q%d", Charge+ivar));
			// if(icharge % 2 == 0){
			// 	rrvshift[ivar]->setVal(hShiftFit->GetBinContent(iSelRig+1));
			// 	rrvshift[ivar]->setConstant(kTRUE);
			// }
		} //ivar
		...
		double dIntValPdf_L1Range[iNVar][3], dIntSumPdf_L1Range[3];
	...
	// if(!bLog) cout << KeysPdfRead_Sur(iSelRig, run_charge, QVer, NVar0, true) << endl;
	return 0;
}//end of KeysPdfRead
