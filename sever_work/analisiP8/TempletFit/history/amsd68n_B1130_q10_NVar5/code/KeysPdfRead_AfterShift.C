...
int KeysPdfRead_AfterShift( int iSelRig=20,
				...
				const bool bBadLadder=false)
{
	...
	for (int ir=ispan; ir<ispan+1; ir++){
		...
		//----output directory
		string sDraw = Form("%s/TempletFit/r%d_xy%d_NVar%d_rebin%d_q%d", sTotalDir.c_str(), ir, ixy, iNVar, NRebin, run_charge);
		...
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			...
			if (bshift)
			{
				TH1F *hShiftFit = (TH1F*)fShift.Get(Form("Shift_r1_q%d", Charge+ivar));
		...
		for (int ivar=0; ivar<iNVar-1; ivar++)
		{
			histn = Form("n%d", ivar);
			double dIni, dL, dU;
			if (Charge+ivar==run_charge) dIni=0.600, dL=0.5, dU=1.0;
			else dIni=0.10, dL=0.00, dU=0.5;
		...
		// double dFitL=Charge, dFitU=Charge+(iNVar-1);
		...
		rpQFrame->GetXaxis()->SetRangeUser(run_charge-2,run_charge+2);
		#ifdef NOPULL
		...
		rpPullFrame->GetXaxis()->SetRangeUser(run_charge-2,run_charge+2);
