...
void runplotKeysPdf_test(const int run_charge=14, const int SelRig=20)
{
	...
	///----- output file
	string sDrawL1QTempFit = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit_test/result");
	...
	double dDrawL = run_charge - 2;
	double dDrawU = run_charge + 2;

	double dAxisL = run_charge - 1.4;
	double dAxisU = run_charge + 1.4;

	double dFitL = run_charge - 0.33;
	double dFitU = run_charge + 0.33;

	// if(run_charge %2 == 1 && run_charge > 7) dEvU=2*1e3;
	// if(run_charge == 16) NRebin = 10;
	// if(run_charge < 10){
	// 	dFitL = run_charge - 0.4;
	...
	rfrResult = rkpLQ->fitTo(*htempalte, RooFit::Save(), RooFit::SumCoefRange("Fit"), RooFit::Range("Fit"), RooFit::PrintLevel(0));
	// rfrResult = rkpLQ->fitTo(*htempalte, RooFit::Save(), RooFit::Range("Fit"), RooFit::PrintLevel(0));
	...
	double binw_fit = rpQFrame->getFitRangeBinW();
	double events_fit = rpQFrame->getFitRangeNEvt();
	double events_fit2 = rpQFrame->getFitRangeNEvt(dFitL, dFitU);
	double max_fit = rpQFrame->GetMaximum();
	double min_fit = rpQFrame->GetMinimum();
	double nbins_fit = rpQFrame->GetNbinsX();

	printf("\tfit information\n");
	printf("\tbin_width:%f\n", binw_fit);
	printf("\tevents in fit range:%f\n", events_fit);
	printf("\tevents in fit range:%f\n", events_fit2);
	printf("\tmax in fit range:%f\n", max_fit);
	printf("\tmin in fit range:%f\n", min_fit);
	printf("\tN bins:%f\n", nbins_fit);

	RooHist *rhPull = rpQFrame->pullHist();
	// RooHist *rhPull = rpQFrame->residHist();
	...
	// lLegend.DrawLatexNDC(.15, .55, Form("#color[%d]{Fit Events:%.0f}", kBlack, rpQFrame->getFitRangeNEvt()));
	lLegend.DrawLatexNDC(.15, .55, Form("#color[%d]{Q = %d}", kBlack, run_charge));
	lLegend.DrawLatexNDC(.65, .85, Form("#color[%d]{Rig%d[%.2f GV, %.2f GV]}", kBlack, SelRig, dLRig, dURig));
	...
	cCanv->Draw();
	cCanv->SaveAs(Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit_test/L1QTemp/q%d_rig%d.png", run_charge, SelRig));
	// cCanv->SaveAs(Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit/L1QTemp/q%d_rig%d.root", run_charge, SelRig));
