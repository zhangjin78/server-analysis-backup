...
void singleFit(const int run_charge=15, const int SelRig=20){
	...
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_MC_CutHe_rho1p0/LQTemp_rig%d.root", ftmp.c_str(), SelRig), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_MC_rho1p0/LQTemp_rig%d.root", ftmp.c_str(), SelRig), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_rrvQ_rho1p0/LQTemp_rig%d.root", ftmp.c_str(), SelRig), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_TransRootCutHe_rho2p0/LQTemp_rig%d.root", ftmp.c_str(), SelRig), "READ");
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_TransRoot/LQTemp_rig%d.root", ftmp.c_str(), SelRig), "READ");
	TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_CutHe%s/LQTemp_rig%d.root", ftmp.c_str(), sRho.c_str(),  SelRig), "READ");
	...
	// TFile fL1Q(Form("%s/InputFile/MC/amsd68nMCl1_B1130_KeysPDF_SURSEL_MC_Z12T16.root", ftmp.c_str()), "READ"); // MC
	// TFile fL1Q(Form("%s/InputFile/amsd69nChenYa_B1130_KeysPDF_SURSEL_asSurProbSe_Z10T18.root", ftmp.c_str()), "READ");
	string rootdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L1Qtemp/Plot/RMS/TransRoot";
	// TFile fL1Q(Form("%s/MC_CutHe_KeysPdf_Z12ToZ16.root", rootdir.c_str()), "READ");
	TFile fL1Q(Form("%s/CutHe_KeysPdf_Z12ToZ16.root", rootdir.c_str()), "READ");
	// TFile fL1Q(Form("%s/InerpolationRoot_KeysPdf_Z12ToZ16_CutHe.root", rootdir.c_str()), "READ");

    ///----- output file
    string sDrawDir = Form("%s/Plot/Pic/CutHe/runQ%d/Rig%d", ftmp.c_str(),run_charge,SelRig);
    // string sDrawDir = Form("%s/Plot/Pic/FitMCpdf_CutHe/DataTempFit/runQ%d/Rig%d", ftmp.c_str(),run_charge,SelRig);
    cout << "Create Pic dir " << system(Form("mkdir -pv %s", sDrawDir.c_str())) << endl;
	string sRootDir = Form("%s/Plot/Root/CutHe/TempShift/L%dTemp", ftmp.c_str(), iTempLayer);
	// string sRootDir = Form("%s/Plot/Root/FitMCpdf_CutHe/DataTempFit/L%dTemp", ftmp.c_str(), iTempLayer);
