...
void diffSelFit(const int run_charge=14, const int SelRig=20, const int isel = 4){
	...
	// TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_AddSel1T4_NoShift_rho2p0/LQTemp_rig%d.root", ftmp.c_str(), SelRig), "READ");
	TFile fQTemp(Form("%s/OutputFile/UBL1Q/LQTemp_AddSel1T4_rho2p0/LQTemp_rig%d.root", ftmp.c_str(), SelRig), "READ");
	...
	//--Charge Temp
	int ir = 1;
	TFile fTotalFile(Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L1Qtemp/InputFile/NewL1PDF_AddSel1T4/amsd69nChenYa_B1130_KeysPDF_SURSEL_NewL1PDF_AddSel1T4_Z12T16.root"), "READ");
	...
	for(int ie = 0; ie < nentries; ie++){
		...
		// if(bChargeCut) hL1QTemp->Fill(tk_exqln[0][0][2]);
	}
	...
	histn = Form("%s/PDFAddSel1T4/FreeShift/%s", sDrawDir.c_str(), sPdf.c_str());
	// histn = Form("%s/BelowL1BK/FreeShift/%s", sDrawDir.c_str(), sPdf.c_str());
