...
const int nRebin = 5;
const bool bCutHe  = true; // UBL1Q > 3
const bool bTranRoot  = false;
...
void CaltStdDev(TH1 *h1, double &stddev, double &stddeverr);
...
void plotDeltaQ(){
    infile[0] = Form("%s/amsd69nChenYa_B1130_KeysPDF_SURSEL_asSurProbSe_Z10T18.root", rootdir.c_str());
    infile[1] = Form("%s/TransRoot/InerpolationRoot_KeysPdf_Z12ToZ16_CutHe.root", wdir.c_str());
    infile[2] = Form("%s/TransRoot/OldVersion/InerpolationRoot_KeysPdf_Z12ToZ16.root", wdir.c_str());
...
void GassFitTH1(TH1* h1, const int run_charge, int SelRig, double par[2], double parerr[2], int itri){
    ...
    string sPic = Form("%s/Pic/deltaQvsRig/runQ%d/GaussFit", wdir.c_str(), run_charge);
    if(bCutHe) sPic += "_CutHe";
    if(bTranRoot) sPic += "_TransRoot";
    string sSam = Form("/sam%d", itri);
    sPic = Form("%s%s",sPic.c_str(), sSam.c_str());
...
void CaltStdDev(TH1 *h1, double &stddev, double &stddeverr){
    ...
    double sumbin = 0, sumcon;
    int nsum = 0;
    for(int i = 0; i < nbins; i++){
        if(h1->GetBinContent(i+1) == 0) continue;
        sumbin += h1->GetBinContent(i+1);
        sumcon += h1->GetBinContent(i+1) * h1->GetBinCenter(i+1);
        nsum ++;
    }
    double mean = sumcon / sumbin;
    double var = 0, verr = 0;
    for(int i = 0; i < nbins; i++){
        if(h1->GetBinContent(i+1) == 0) continue;
        var += pow((h1->GetBinCenter(i+1) - mean),2) * h1->GetBinContent(i+1);
    }
    stddev = sqrt(var/sumbin);
    verr = sqrt(verr);
    stddeverr = h1->GetStdDevError();
}
...
//             // hrms = htemp->GetStdDev();
//             // hrmse = htemp->GetStdDevError();
//             CaltStdDev(htemp, hrms, hrmse);
//             if(itri == 0){
//                 // hrms = hrms / run_charge;
//                 // hrmse = hrmse / run_charge;
//             }
//             else{
//                 GassFitTH1(htemp, run_charge, iselrig, hrms, hrmse);
//             }
//             gr[itri]->SetPoint(iselrig, hbk->GetBinCenter(iselrig+1), hrms);
//             gr[itri]->SetPointError(iselrig, 0, hrmse);
//             if(maxen < hrms) maxen = hrms;
//         }
//     }
...
//     double tFoffsety = 1.2;
//     string htitle = Form("Data, L1Q=%d, L%d Template", run_charge, iTempLayer);
...
//     hbk->GetYaxis()->SetRangeUser(0., 1.5);
//     if(!bCutHe) hbk->GetYaxis()->SetRangeUser(0., maxen*1.5);
...
//     legend->AddEntry(gr[0], "StdDev","p");
//     // legend->AddEntry(gr[0], "StdDev/Q","p");
//     legend->AddEntry(gr[1], "GaussFit","p");
//     legend->Draw("");
//     for(int itri = 0; itri < 2; itri++){
...
//     string sPic = Form("%s/Pic/deltaQvsRig/runQ%d", wdir.c_str(), run_charge);
//     system(Form("mkdir -pv %s", sPic.c_str()));
//     string sCut = "";
//     if(bCutHe) sCut += "_CutHe";
//     if(bTranRoot) sCut += "_TransRoot";
//     c1->SaveAs(Form("%s/runQ%d_L%dtemp%s.png", sPic.c_str(), run_charge, iTempLayer, sCut.c_str()));
// }
...
//             histn = Form("hTemplate_q%d_rig%d_L%d_%d_Z", ich, SelRig, iTempLayer, itri);
//             TH1F *htemp = new TH1F(histn.c_str(), histn.c_str(), 3500,0,35);
//             htemp->Rebin(nRebin);
//             long int nentries = L1QTem->GetEntries();
//             double l1q;
...
//             double hrms = htemp->GetRMS(); /// same as StdDev
//             double hrmse = htemp->GetRMSError();
//             // cout << "old:" << hrms << "\t" << hrmse << endl;
//             // hrms = htemp->GetStdDev();
//             // hrmse = htemp->GetStdDevError();
//             CaltStdDev(htemp, hrms, hrmse);
//             // cout << hrms << "\t" << hrmse << endl;
//             if(itri == 0){
//                 // hrms = hrms / ich;
//                 // hrmse = hrmse / ich;
//             }
//             else{
//                 GassFitTH1(htemp, ich, SelRig, hrms, hrmse);
//             }
//             int ip = ich - sCharge;
//             gr[itri]->SetPoint(ip, ich, hrms);
//             gr[itri]->SetPointError(ip, 0, hrmse);
//             if(maxen < hrms) maxen = hrms;
//         }
//     }
...
//     double tFoffsetx = 1.2;
//     double tFoffsety = 1.2;
//     string htitle = Form("Data, [%.2f, %.2f]GV, L%d Template", L1QTF_BinsAllQ2[SelRig],L1QTF_BinsAllQ2[SelRig+1], iTempLayer);
//     hbk->SetTitle(htitle.c_str());
...
//     string sPic = Form("%s/Pic/deltaQvsZ/Protype", wdir.c_str());
//     if(bCutHe) sPic += "_CutHe";
//     if(bTranRoot) sPic += "_TransRoot";
//     system(Form("mkdir -pv %s", sPic.c_str()));
...
void TempDeltaQvsRig(const int run_charge = 14){
    ...
    double tFoffsetx = 1.2;
    double tFoffsety = 1.2;
    string htitle = Form("Data, L1Q=%d, L%d Template, GaussFit", run_charge, iTempLayer);
    hbk->SetTitle(htitle.c_str());
    hbk->GetXaxis()->SetTitle("Rigidity [GV]");
    hbk->GetXaxis()->SetMoreLogLabels(true);
    hbk->GetXaxis()->SetTitleSize(tFsize);
    ...
    hbk->GetYaxis()->SetTitle("Sigma");
    hbk->GetXaxis()->SetRangeUser(2, 3000);
    hbk->GetYaxis()->SetRangeUser(0., 1.0);
    hbk->GetYaxis()->SetTitleSize(tFsize);
    hbk->GetYaxis()->SetLabelSize(tFsize2);
    hbk->GetYaxis()->SetTitleOffset(tFoffsety);
    hbk->GetYaxis()->CenterTitle(true);
    ...
    auto legend = new TLegend(0.45, 0.63, 0.88, 0.78);
    legend->SetFillColor(0);
    legend->SetLineColor(0);
    legend->SetTextSize(0.04);
    // legend->SetFillStyle(0);
    legend->AddEntry(gr[0], "Default","p");
    legend->AddEntry(gr[1], "Template Migration","p");
    legend->Draw("");
    for(int itri = 0; itri < 2; itri++){
        gr[itri]->SetMarkerStyle(20);
        gr[itri]->SetMarkerColor(ncolor[itri]);
    ...
    string sPic = Form("%s/Pic/deltaQvsRig/runQ%d/CompareMigration", wdir.c_str(), run_charge);
    system(Form("mkdir -pv %s", sPic.c_str()));
    string sCut = "";
    if(bCutHe) sCut += "_CutHe";
...
void TempDeltaQvsZ(const int SelRig){
    ...
    double tFoffsetx = 1.2;
    double tFoffsety = 1.2;
    string htitle = Form("Data, [%.2f, %.2f]GV, L%d Template, GaussFit", L1QTF_BinsAllQ2[SelRig],L1QTF_BinsAllQ2[SelRig+1], iTempLayer);
    hbk->SetTitle(htitle.c_str());
    hbk->GetXaxis()->SetTitle("Charge");
    hbk->GetXaxis()->SetTitleSize(tFsize);
    hbk->GetXaxis()->SetLabelSize(tFsize2);
    ...
    auto legend = new TLegend(0.24, 0.7, 0.75, 0.88);
    legend->SetFillColor(0);
    legend->SetLineColor(0);
    legend->SetTextSize(0.04);
    // legend->SetFillStyle(0);
    ...
    string sPic = Form("%s/Pic/deltaQvsZ/CompareMigration/Sigma", wdir.c_str());
    if(bCutHe) sPic += "_CutHe";
    system(Form("mkdir -pv %s", sPic.c_str()));
    c1->SaveAs(Form("%s/Sigma_L%dtemp_Rig%d.png", sPic.c_str(), iTempLayer, SelRig));
}
...
void TempMeanvsRig(const int run_charge = 14){
    ...
    auto legend = new TLegend(0.45, 0.14, 0.88, 0.4);
    legend->SetFillColor(0);
    legend->SetLineColor(0);
    legend->SetTextSize(0.04);
    // legend->SetFillStyle(0);
...
void TempMeanvsZ(const int SelRig){
    ...
    auto legend = new TLegend(0.15, 0.73, 0.65, 0.88);
    legend->SetFillColor(0);
    legend->SetLineColor(0);
    legend->SetTextSize(0.04);
    // legend->SetFillStyle(0);
    ...
    string sPic = Form("%s/Pic/deltaQvsZ/CompareMigration/Mean", wdir.c_str());
    if(bCutHe) sPic += "_CutHe";
    system(Form("mkdir -pv %s", sPic.c_str()));
    c1->SaveAs(Form("%s/Mean_L%dtemp_Rig%d.png", sPic.c_str(), iTempLayer, SelRig));
}