...
void transRootCutHe(){
    ...
    string inRoot = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L1Qtemp/InputFile/MC/amsd68nMCl1_B1130_KeysPDF_SURSEL_MC_Z12T16.root";
    TFile *infile = new TFile(inRoot.c_str());

    string outRoot = Form("%s/TransRoot", wdir.c_str());
    system(Form("mkdir -pv %s", outRoot.c_str()));
    histn = Form("%s/MC_CutHe_KeysPdf_Z%dToZ%d.root", outRoot.c_str(), sCharge, eCharge);
