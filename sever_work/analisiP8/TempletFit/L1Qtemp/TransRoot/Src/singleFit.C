...
void singleFit(const int run_charge=15, const int SelRig=20){
	...
	if(!bClock) histn = Form("%s/LQTemp_TransRoot_Anticlockwise%s/LQTemp_rig%d.root", ftmp.c_str(), sRho.c_str(),  SelRig);
    ...
    ///----- output file
	histn = "Clockwise";
	if(!bClock) histn = "Anticlockwise";
	histn += Form("_rho%dp0",rho);
