string wdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L1Qtemp/TransRoot";
...
void transRoot(){
    ...
    string outRoot = Form("%s/OutRoot", wdir.c_str());
    if(bClock) outRoot += "/Clockwise";
    else outRoot += "/Anticlockwise";
    system(Form("mkdir -pv %s", outRoot.c_str()));
    histn = Form("%s/InerpolationRoot_Z%dToZ%d.root", outRoot.c_str(), sCharge, eCharge);
            ...
            for(int irig = 0; irig < L1QTF_NbinAllQ2; irig++){
                ...
                for(int ie = 0; ie < nentries; ie++){
                    ...
                    if(ich % 2 == 0){
                        ...
                        if(bClock) outL1Q = (inL1Q - meanL1Q) * ratio_ich + (meanL1Q + 1); /// new version: mean shift = 1
                        else outL1Q = (inL1Q - meanL1Q) * ratio_ich + (meanL1Q - 1);
                    }
                    outTree->Fill();
                }
