...
void runplotKeysPdf_T4(const int run_charge=13, const int SelRig=20)
{
	...
	TFile fshift(Form("%s/L2TempFitL1QTemp_shift_T3.root", fTSH.c_str()), "READ");
	TH1F *hT3shift = (TH1F *)fshift.Get(Form("Shift/Shift_r1_q%d", run_charge+1));
	double evenQs = hT3shift->GetBinContent(SelRig+1);
	TFile fL1Q(Form("%s/total_result_L1QTemp.root", ftmp.c_str()), "READ");

	///----- output file
	string sDrawL1QTempFit = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit/result/T4");
...
double sumfitEv = 0;
	...
	cCanv->Draw();
	string sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit/L1QTemp/T4/pic";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
	cCanv->SaveAs(Form("%s/q%d_rig%d.png", sPicdir.c_str(), run_charge, SelRig));
	sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit/L1QTemp/T4/root";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
