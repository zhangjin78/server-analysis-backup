...
void runplotKeysPdf_T3(const int run_charge=12, const int SelRig=20)
{
	if(run_charge % 2 == 1) return;  /// only for even charge 
	if(run_charge > 17 || run_charge < 7){
		cout << "only fit the charge [8,16]" << endl;
		return;
	}
	//------check Rigidity
	...
	cCanv->Draw();
	string sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit/L1QTemp/T3/pic";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
	cCanv->SaveAs(Form("%s/q%d_rig%d.png", sPicdir.c_str(), run_charge, SelRig));
	sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit/L1QTemp/T3/root";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
