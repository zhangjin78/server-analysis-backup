				...
				const bool bBadLadder=false)
{
	...
	double dAxisL = Charge - 2;
	double dAxisU = Charge + iNVar;

	//------fit
	// double dFitL=Charge-0.5, dFitU=Charge+(iNVar-1)+0.5;
	// double dFitL=Charge+0.2, dFitU=Charge+(iNVar-1)-0.2;
	double dFitL=Charge - 0.65;
	double dFitU=run_charge + 2.8;
	...
	for (int ir=ispan; ir<ispan+1; ir++){
		...
		//----output directory
		string sOutput = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit";
		string sDraw = Form("%s/result/r%d_xy%d_NVar%d_rebin%d_q%d", sOutput.c_str(), ir, ixy, iNVar, NRebin, run_charge);
		...
		for(int ivar = 0; ivar < iNVar; ivar ++){
			...
			tTempFit->Branch(Form("Impurity_q%d", icharge), &fImp[ivar], Form("Impurity_q%d/D", icharge));
			tTempFit->Branch(Form("Impurity_Error_q%d", icharge), &fImpError[ivar][0], Form("Impurity_Error_q%d/D", icharge));
		}
