				...
				const bool bBadLadder=false)
{
	...
	// TFile fL1Q(Form("%s/amsd69nChenYa_B1130_SurvivalProbability_L1Qdistribution_DiffSel3.root", sTotalDir.c_str()), "READ"); //q14
	...
	for (int ir=ispan; ir<ispan+1; ir++){
		...
		//----output directory
		string outdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/OutputFile";
		...
		int isel = 0;
		string csel = "";
		if(isel != 0) csel = Form("_Sel%d", isel);
		histn = Form("L1L2Sur4/q%d/%s/period1/tofqcut0/L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period1_tofqcut0%s", 
										run_charge, sSpan[ir].c_str(), ir, run_charge, csel.c_str());
		...
		for (int ivar=0; ivar<iNVar-1; ivar++)
		{
			histn = Form("n%d", ivar);
			double dIni, dL, dU;
			if (Charge+ivar==run_charge) dIni=0.700, dL=0.5, dU=1.0;
			else dIni=0.020, dL=0.00, dU=0.5;
			...
			if(icharge == 12) dIni=0.4, dL=0.2, dU=0.6;
			if(icharge == 13) dIni=0.001, dL=0.0, dU=0.1;
			if(icharge == 14) dIni=0.4, dL=0.2, dU=0.6;
			if(icharge == 15) dIni=0.001, dL=0.0, dU=0.1;
			if(icharge == 16) dIni=0.1, dL=0.0, dU=0.4;
		...
		rpQFrame->GetXaxis()->SetRangeUser(dFitL-1,dFitU+1);
		#ifdef NOPULL
		...
		lLegend.DrawLatexNDC(.68, .90, Form("#color[%d]{[%.2f GV, %.2f GV]}", kBlack, dLRig, dURig));
		...
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			...
			if (Charge+ivar==run_charge) lLegend.DrawLatexNDC(.12, .85-ivar*0.05, Form("#color[%d]{Q%d}: %.3f%%", iColor[ivar], Charge+ivar, dFrac*rarInt[ivar]->getVal()/rarIntAll->getVal()*100.));
