...
string wdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1_OddCh";

void runplotKeysPdf_T2(const int run_charge=13, const int SelRig=20)
{
	if(run_charge % 2 == 0) return;  /// only for odd charge 
	if(run_charge > 18){
		cout << "only fit the charge < 19" << endl;
		return;
	}
	//------check Rigidity
	...
	TFile fQTemplate(Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L1Qtemp/InputFile/amsd69nChenYa_B1130_KeysPDF_SURSEL_asSurProbSe_Z10T18.root"), "READ");

	///----- output file
	string sDrawL1QTempFit = Form("%s/result/T2", wdir.c_str());
	...
	//--TTree
	string histn = Form("Charge_Calibration1/q%d/L1Inner/L1Q_KeysPdf/L1Q_r1_q%d_sel1_rig%d", run_charge, run_charge, SelRig);
	if (SelRig==-1) histn = Form("Charge_Calibration1/q%d/L1Inner/L1Q_KeysPdf/L1Q_r1_q%d_sel1_allrig", run_charge, run_charge);
	TTree *tLQTemp = static_cast<TTree*>(fQTemplate.Get(histn.c_str()));

	//---Hist
	TH1F *hL1Temp = new TH1F(Form("q%d_rig%d", run_charge, SelRig), Form("q%d_rig%d", run_charge, SelRig), 3500, 0, 35);
	TH1F *hL1Samp = (TH1F*)fQTemplate.Get(Form("Charge_Calibration1/q%d/L1Inner/L1QTemp/rig%d/l1/L1Q_r1_q%d_xy2_L1QTemp2.0sigma_sel1_rig%d_Tk2nd0", run_charge, SelRig, run_charge, SelRig));
	int NRebin = 5;
	...
	// if(run_charge == 11) nVar = 2, bcharge = run_charge;
	const int iNVar = nVar;
	...
	double L1QXY = 0;
	tLQTemp->SetBranchAddress("UBL1QXY", &L1QXY);
	long int nentries = tLQTemp->GetEntries();
	for(int ie = 0; ie < nentries; ie++){
		tLQTemp->GetEntry(ie);
		hL1Temp->Fill(L1QXY);
	}
	hL1Temp->Rebin(NRebin);
	double ymax = hL1Temp->GetMaximum();
	hL1Samp->Rebin(NRebin);
	// double ymax = hL1Samp->GetBinContent(hL1Samp->FindBin(run_charge));
	dEvU = ymax*1.5; 
	htempalte = new RooDataHist(Form("template_q%d", run_charge), Form("template_q%d", run_charge), *rrvQXY, hL1Temp);
	// htempalte = new RooDataHist(Form("template_q%d", run_charge), Form("template_q%d", run_charge), *rrvQXY, hL1Samp);
	...
	double nEvents = hL1Samp->Integral(1, hL1Samp->GetNbinsX());
	for (int ivar=0; ivar<iNVar; ivar++)
	{
	...
	double chis_fit = rpQFrame->chiSquare(nullptr, nullptr, iNVar); // Fit para 1
	// RooAbsReal *rhchis = rapSumpdf->createChi2(*htempalte, RooFit::Range("Fit"));

	///Pull Hist
	RooHist *rhPull = rpQFrame->pullHist(nullptr, nullptr, false);
	// int bins = hL1Samp->FindBin(dFitU) - hL1Samp->FindBin(dFitL) + 1;
	...
	lLegend.DrawLatexNDC(.15, .80, Form("#color[%d]{Rebin: %d}", kBlack, NRebin));
	...
	lLegend.DrawLatexNDC(.15, .70, Form("#color[%d]{Fit Range Events:%.0f}", kBlack, rpQFrame->getFitRangeNEvt(dFitL, dFitU)));
	lLegend.DrawLatexNDC(.15, .65, Form("#color[%d]{Fit Events:%.0f}", kRed, sumfitEv));
	...
	cCanv->Draw();
	string sPicdir = Form("%s/FittingPic/T2/pic", wdir.c_str());
	system(Form("mkdir -pv %s", sPicdir.c_str()));
	cCanv->SaveAs(Form("%s/q%d_rig%d.png", sPicdir.c_str(), run_charge, SelRig));
	// sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/FittingPic/T2/root";
	// system(Form("mkdir -pv %s", sPicdir.c_str()));
