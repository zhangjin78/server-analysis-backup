...
void runplotKeysPdf_T1(const int run_charge=18, const int SelRig=6)
{
	...
	///----- output file
	string sDrawL1QTempFit = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/result/T1");
	...
	cCanv->Draw();
	string sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/FittingPic/T1/pic";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
	cCanv->SaveAs(Form("%s/q%d_rig%d.png", sPicdir.c_str(), run_charge, SelRig));
	sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/FittingPic/T1/root";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
