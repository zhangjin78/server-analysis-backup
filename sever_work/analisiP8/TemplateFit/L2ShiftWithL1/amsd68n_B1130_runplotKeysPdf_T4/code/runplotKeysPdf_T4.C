...
void runplotKeysPdf_T4(const int run_charge=11, const int SelRig=20)
{
	...
	TFile fshiftT3(Form("%s/L2TempFitL1QTemp_shift_T3.root", fTSH.c_str()), "READ");
	TFile fL1Q(Form("%s/total_result_0301.root", ftmp.c_str()), "READ");

	///----- output file
	string sDrawL1QTempFit = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/result/T4");
	...
	if(run_charge == 15) NRebin=10;
	if(run_charge <= 9) dFitL = run_charge - 1.2;
	...
	for(int ivar = 0; ivar < iNVar; ivar++){
		...
		if(bcharge + ivar != run_charge){
			if((bcharge+ivar) % 2 == 0) hTshift[ivar] = (TH1F *)fshiftT3.Get(Form("Shift/Shift_r1_q%d", bcharge+ivar));
			else hTshift[ivar] = (TH1F *)fshiftT2.Get(Form("Shift/Shift_r1_q%d", bcharge+ivar));
			evenQs[ivar] = hTshift[ivar]->GetBinContent(SelRig+1);
...
double sumfitEv = 0;
	...
	cCanv->Draw();
	string sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/FittingPic/T4/pic";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
	cCanv->SaveAs(Form("%s/q%d_rig%d.png", sPicdir.c_str(), run_charge, SelRig));
	sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/FittingPic/T4/root";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
