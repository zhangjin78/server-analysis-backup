...
void runplotKeysPdf_T3(const int run_charge=18, const int SelRig=10)
{
	if(run_charge % 2 == 1) return;  /// only for even charge 
	if(run_charge > 18 || run_charge < 11){
		cout << "only fit the charge [11,18]" << endl;
		return;
	}
	//------check Rigidity
	...
	//----input result
	string ftmp = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L1Qtemp/Template");
	TFile fQTemplate(Form("%s/total_result_0301.root", ftmp.c_str()), "READ");
	TFile fQTemp(Form("%s/LQTemp/LQTemp_rig%d.root", ftmp.c_str(), SelRig), "READ");

	string fTSH = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/result";
	TFile fshiftT1(Form("%s/L2TempFitL1QTemp_shift_T1.root", fTSH.c_str()), "READ");
	TFile fshiftT2(Form("%s/L2TempFitL1QTemp_shift_T2.root", fTSH.c_str()), "READ");
	TFile fL1Q(Form("%s/total_result_0301.root", ftmp.c_str()), "READ");

	///----- output file
	string sDrawL1QTempFit = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L2Qtemp_L1QTempFit/result/T3");
	...
	double dDrawL = run_charge - 3;
	double dDrawU = run_charge + 3;
	double dFitL = run_charge - 1.4;
	double dFitU = run_charge + 2.3;
	double dAxisL = run_charge - 2.2;
	double dAxisU = run_charge + 2.8;

	// if(run_charge %2 == 1 && run_charge > 7) dEvU=2*1e3;
	// if(run_charge %2 == 1 && run_charge > 9) NRebin=10;
	// if(run_charge = 9 && SelRig < 2) NRebin=10;
	if(run_charge >= 14 && run_charge < 16){
		dFitL = run_charge - 1.0;
		dFitU = run_charge + 1.0;
	}
	...
	for(int ivar = 0; ivar < iNVar; ivar++){
		...
		// if(run_charge == 16 && bcharge + ivar == 17) continue;
		if((bcharge+ivar) % 2 == 0) hT2shift[ivar] = (TH1F *)fshiftT1.Get(Form("Shift/Shift_r1_q%d", bcharge+ivar));
		else hT2shift[ivar] = (TH1F *)fshiftT2.Get(Form("Shift/Shift_r1_q%d", bcharge+ivar));
		evenQs[ivar] = hT2shift[ivar]->GetBinContent(SelRig+1);

		rrvshift[ivar]->setVal(evenQs[ivar]);
	...
	rfrResult = rapSumpdf->fitTo(*htempalte, RooFit::Save(), RooFit::SumCoefRange("Fit"), RooFit::Range("Fit"), RooFit::PrintLevel(1));
	...
	int runID = run_charge - bcharge;
	double shift_v = rrvshift[runID]->getVal();
	double shift_e = rrvshift[runID]->getError();
	...
	lLegend.DrawLatexNDC(.65, .75, Form("#color[%d]{shift: %.2e #pm %.2e}", iColor[runID], shift_v, shift_e));
	...
	cCanv->Draw();
	string sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/FittingPic/T3/pic";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
	cCanv->SaveAs(Form("%s/q%d_rig%d.png", sPicdir.c_str(), run_charge, SelRig));
	sPicdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/FittingPic/T3/root";
	system(Form("mkdir -pv %s", sPicdir.c_str()));
