...
// const int iNBL1Fit=L1QTF_NbinSec_JZ_S;
// double *dBL1Fit=L1QTF_BinsSec_JZ_S;
...
int KeysPdfRead_Sur( int iSelRig=10,
				...
				const int ispan=1)
{
	...
	// const string sTempDir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/makePDF/OutputFile/UBL1Q_Front";
	// const string sTempDir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/makePDF/OutputFile/UBL1Q";
	...
	string sTempName = Form("LQTemp_Rebin_Z7TZ18_NDKeys_NoShift_rho%dp0_sigma%dp0_Z1516rZ14", nd_rho, nd_sigma);
	...
	for (int ir=ispan; ir<ispan+1; ir++){
		...
		//----output directory
		string outdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/OutputFile";
		// string sDraw = Form("%s/%s/r%d_xy%d_NVar%d_rebin%d_q%d", outdir.c_str(), sTempName.c_str(), ir, ixy, iNVar, NRebin, run_charge);
		string sDraw = Form("%s/%s/r%d_xy%d_NVar%d_rebin%d_q%d_L1QRebin_MigrationR14", outdir.c_str(), sTempName.c_str(), ir, ixy, iNVar, NRebin, run_charge);
		...
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			...
			if(icharge > 20){
				dIni = 0.5*rNum[ivar], dL = 0.001 * rNum[ivar], dU = 2.5 * rNum[ivar];
			}
		...
		for (int ivar=0; ivar<iNVar; ivar++) ///even charge
		{
			...
			histn = Form("L%dQ_r%d_q%d_xy%d_sel%d_rig%d_KeysPdf", ilTemp, iSelSpan, Charge+ivar, ixy, iSelTemp, iRigTemp);
			rapSumpdf->plotOn(rpQFrame, RooFit::Components(histn.c_str()), RooFit::LineWidth(3), RooFit::FillStyle(fillStyle), RooFit::FillColor(iColor[ivar]), RooFit::LineColor(iColor[ivar]), RooFit::Range("draw"), RooFit::DrawOption("FL"), RooFit::VLines()); //2020.09.30: without drawing the line
		...
		rpQFrame->GetXaxis()->SetRangeUser(run_charge-3,run_charge+2.5);
		#ifdef NOPULL
		...
		rpQFrame->GetYaxis()->SetTitleSize(0.07);
		rpQFrame->GetYaxis()->SetTitleOffset(0.7);
		...
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			...
			if(Charge+ivar < run_charge -2) continue;
			...
			lLegend.DrawLatexNDC(.65, .80-ivar*0.05, Form("#color[%d]{Z=%d:%.2f%%}", iColor[ivar], Charge+ivar, dFrac*100.));
