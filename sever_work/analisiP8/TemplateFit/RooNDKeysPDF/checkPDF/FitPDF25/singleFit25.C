...
void singleFit25( const int run_charge=25, 
				const int SelRig=8,
	...
	histn = Form("%s/OutputFile/UBL1Q_RebinRho/%s/LQTemp_rig%d.root", sTotalDir.c_str(), sTempName.c_str(),  SelRig);
	if(bShiftL2 && run_charge >= 24) histn = Form("%s/OutputFile/UBL1Q_RebinRho_ShiftL2/%s/LQTemp_rig%d.root", sTotalDir.c_str(), sTempName.c_str(),  SelRig);
    ...
    ///----- output file
    string sDrawDir = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/checkPDF/FitPDF25/OutputFile/RebinRoot_TotalL1/%s/runQ%d/L%d", sTempName.c_str(),run_charge,iTempLayer);
	if(run_charge >= 24 && bShiftL2) sDrawDir = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/checkPDF/FitPDF25/OutputFile/RebinRoot_TotalL1_ShiftL2/%s/runQ%d/L%d", sTempName.c_str(),run_charge,iTempLayer);
	...
	for (int ivar=0; ivar<iNVar; ivar++){
		// int ilTemp = iTempLayer;
		int ilTemp = 2;
		if(run_charge == 25) ilTemp = 2;
		histn = Form("L%dQ_r1_q%d_xy2_sel1_rig%d_KeysPdf", ilTemp, run_charge+ivar, SelRig);
	...
	for(int ivar=0; ivar<iNVar; ivar++){
		rNum[ivar] = 0;
		double x12 = 0.5;
		if((run_charge+ivar)%2 == 1) x12 = 0.4;
		if((run_charge+ivar) == 26) x12 = 0.7;
		int xb1 = hL1QTemp->FindBin((run_charge+ivar)-x12);
		int xb2 = hL1QTemp->FindBin((run_charge+ivar)+x12);
		for(int ix = xb1; ix < xb2; ix++) rNum[ivar] += hL1QTemp->GetBinContent(ix);
		cout << "\t" << ivar+run_charge << "\t" << rNum[ivar] << endl;
	}
	RooRealVar *rrvn[iNVar];
	RooArgList ralLQ, raln;
	for (int ivar=0; ivar<iNVar; ivar++){
		histn = Form("nparticle_q%d", run_charge+ivar);
		double dIni = 0.96*rNum[ivar], dL = 0. * rNum[ivar], dU = 1.7 * rNum[ivar];
		if(ivar == 0) dU = 2.5 * rNum[ivar];
	...
	for (int ivar=0; ivar<iNVar; ivar++)
	{
		cout << "\t\t q_" << run_charge+ivar << " \t" << rrvn[ivar]->getVal() / rNum[ivar] << endl;
	} //ivar
	...
	rpPullFrame->GetXaxis()->SetLabelSize(0.09);
	...
	rpPullFrame->GetYaxis()->SetLabelSize(0.09);
