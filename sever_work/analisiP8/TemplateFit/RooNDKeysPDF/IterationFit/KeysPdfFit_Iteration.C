...
// const int L1QTF_NbinSec_JZ = 36;
// double L1QTF_BinsSec_JZ[L1QTF_NbinSec_JZ+1] = {
// 0.80,1.33,1.92,2.15,2.67,3.29,4.02,4.88,
...
// const int L1QTF_NbinSec_JZ_S = 21;
// double L1QTF_BinsSec_JZ_S[L1QTF_NbinSec_JZ_S+1] = {
// 0.80,1.33,1.92,2.15,2.67,3.29,4.02,4.88,
// 5.37,5.90,6.47,7.09,8.48,10.1,14.1,
// 18.0,22.8,31.1,45.1,93.0,
// 330.,3000
// };

const int L1QTF_NbinSec_JZ_Fe = 23;
double L1QTF_BinsSec_JZ_Fe[L1QTF_NbinSec_JZ_Fe+1] = {
0.80,1.33,1.92,2.15,2.67,3.29,4.02,4.88,
5.37,5.90,6.47,7.09,7.76,8.48,9.26,10.1,12.0,14.1,
18.0,22.8,31.1,108.,259.,3000
};
...
const int iNBL1Fit=L1QTF_NbinSec_JZ_Fe;
double *dBL1Fit=L1QTF_BinsSec_JZ_Fe;
...
////-- 2024.12.19 add dir for iteration
const string dir_FluxRatio = "/afs/cern.ch/work/j/jzhang2/AMS/plot/SurvivalProb/FluxCalculation/Input/FluxFit/FluxRatio";
const string dir_FitResult = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/OutputFile";
const string fitfile_Z15r14 = Form("%s/LQTemp_Rebin_Z7TZ18_NDKeys_NoShift_rho2p0_sigma3p0_Z15r14/r1_xy2_NVar5_rebin2_q16_L1QRebin_MigrationR14"
									"/NDKeys_NoShift_rho2p0_sigma3p0_r1_xy2_NVar5_rebin2_q16_L1QTemp_L1QRebin__MigrationR14.root", dir_FitResult.c_str());
const string fitfile_Z15r16 = Form("%s/LQTemp_Rebin_Z7TZ18_NDKeys_NoShift_rho2p0_sigma3p0_Z15r16/r1_xy2_NVar5_rebin2_q16_L1QRebin_MigrationR16"
									"/NDKeys_NoShift_rho2p0_sigma3p0_r1_xy2_NVar5_rebin2_q16_L1QTemp_L1QRebin__MigrationR16.root", dir_FitResult.c_str());

int KeysPdfFit_Iteration( int iSelRig=10,
				const int run_charge=16, 
				const int QVer=1,
				const int NVar0=-1,
				bool bLog = false,
				const int ixy=2,
				...
				const int ispan=1)
{
	//--- Just test for only Z = 16
	if(run_charge != 16) return 0;
	...
	cout << " *** Fit bin=" << iSelRig << " temp bin=" << iRigTemp << " *** " << endl;
	...
	// dCutL = run_charge - 0.3; dCutU = run_charge + 0.3;
	...
	string sDataDir = "/afs/cern.ch/work/j/jzhang2/AMS/plot/SurvivalProb/RootFile/Data";
	TFile fL1Q(Form("%s/amsd69nChenYa_B1130_SurvivalProbability_Z10TZ16Z26.root", sDataDir.c_str()), "READ");

	int nd_rho = 2, nd_sigma = 3;
	// string sTempName = Form("LQTemp_CutHe_Z7TZ18_NDKeys_NoShift_rho%dp0_sigma%dp0", nd_rho, nd_sigma);
	// if(run_charge == 26) sTempName = Form("LQTemp_CutHe_Z24TZ28_NDKeys_NoShift_rho%dp0_sigma%dp0", nd_rho, nd_sigma);
	string sTempDir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/makePDF/OutputFile/UBL1Q_RebinRho_Migration";
	// string sTempDir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/makePDF/OutputFile/UBL1Q_RebinRho";
	if(bShiftL2 && run_charge == 26) sTempDir += "_ShiftL2";
	string sTempName = Form("LQTemp_Rebin_Z7TZ18_NDKeys_NoShift_rho%dp0_sigma%dp0_Z15r16", nd_rho, nd_sigma);
	// string sTempName = Form("LQTemp_Rebin_Z7TZ18_NDKeys_NoShift_rho%dp0_sigma%dp0", nd_rho, nd_sigma);
	if(run_charge == 26) sTempName = Form("LQTemp_Rebin_Z24TZ28_NDKeys_NoShift_rho%dp0_sigma%dp0", nd_rho, nd_sigma);
	TFile fQTemp(Form("%s/%s/LQTemp_rig%d.root", sTempDir.c_str(), sTempName.c_str(), iRigTemp), "READ");
	...
	for (int ir=ispan; ir<ispan+1; ir++){
		...
		//----output directory
		string outdir = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/RooNDKeysPDF/IterationFit/OutputFile";
		// string sDraw = Form("%s/%s/r%d_xy%d_NVar%d_rebin%d_q%d", outdir.c_str(), sTempName.c_str(), ir, ixy, iNVar, NRebin, run_charge);
		string sDraw = Form("%s/%s/r%d_xy%d_NVar%d_rebin%d_q%d_L1QRebin_MigrationR16", outdir.c_str(), sTempName.c_str(), ir, ixy, iNVar, NRebin, run_charge);
		// string sDraw = Form("%s/%s/r%d_xy%d_NVar%d_rebin%d_q%d_L1QRebin", outdir.c_str(), sTempName.c_str(), ir, ixy, iNVar, NRebin, run_charge);
		//--L1QTempFit
		string sDrawL1QTempFit = Form("%s/L1QTempFit_L%dQTemp", sDraw.c_str(), il);
		if(bShiftL2 && run_charge == 26) sDrawL1QTempFit = Form("%s/L1QTempFit_L%dQTemp_ShiftL2", sDraw.c_str(), il);
		...
		////-- 2024.12.19 Add temp fit result
		TFile *inFitRes = new TFile(fitfile_Z15r16.c_str());
		TTree *tInTempFit = (TTree*)inFitRes->Get(Form("Tree_q%d/TemplateFit_q%d_Nvar%d_L1temp", run_charge, run_charge, iNVar));
		int selrig;
		double fit_para[iNVar], fit_err[iNVar];
		tInTempFit->SetBranchAddress("RigidityBin", &selrig);
		for (int ivar=0; ivar<iNVar; ivar++){
			int  icharge = Charge + ivar;
			tInTempFit->SetBranchAddress(Form("Fitting_Parameter_q%d", icharge), &fit_para[ivar]);
        	tInTempFit->SetBranchAddress(Form("Fitting_ParameterError_q%d", icharge), &fit_err[ivar]);
		}
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			int  icharge = Charge + ivar;
			...
			for(int ibin = 0; ibin < tInTempFit->GetEntries(); ibin++){
				tInTempFit->GetEntry(ibin);
				if(iSelRig != selrig) continue;
				if(icharge % 2 == 0){
					dIni = 1.0*fit_para[ivar], dL = fit_para[ivar] - 2.0*fit_err[ivar], dU = fit_para[ivar] + 3.0*fit_err[ivar];
				}
				else{
					string ratioName;
					if(icharge == 15) ratioName = "Psratio68_20230304P8_B1236602RAMCKY11COMBUNFOLDTOINNB_totalQYAN.root";
					else if(icharge == 17) ratioName = "clsratio68_20230304P8_B1236602RAMCKY11COMBUNFOLDTOINNB_totalQYAN.root";
					else return 0; //// no other flux ratio
					histn = Form("%s/%s", dir_FluxRatio.c_str(), ratioName.c_str());
    				TFile *inFluxRatio = new TFile(histn.c_str());

					string grName;
					if(icharge == 15) grName = "PSRatioAhgr";
					else if(icharge == 17) grName = "ClSRatioAhgr";
					TGraphErrors* grFluxRmit = (TGraphErrors*)inFluxRatio->Get(grName.c_str());
					double xx  = CalAbs(hL1QFit.GetBinLowEdge(selrig+1), hL1QFit.GetBinLowEdge(selrig+2));
					double y_ratio = grFluxRmit->Eval(xx);
					int varS = 16 - Charge;
					dIni = y_ratio*fit_para[varS], dL = 0.7*dIni, dU = 1.2*dIni;
				}
			}
		...
		//----retrieve RooNDKeysPdf for each charge
		RooNDKeysPdf *rkpLQ[iNVar];
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			int iSelSpan=ir;
			...
			if(Charge+ivar == 25) ilTemp = 2;
			if(Charge+ivar == 27 || Charge+ivar == 28) ilTemp = 1;
			
			histn = Form("L%dQ_r%d_q%d_xy%d_sel%d_rig%d_KeysPdf", ilTemp, iSelSpan, Charge+ivar, ixy, iSelTemp, iRigTemp);
			
			rkpLQ[ivar] = static_cast<RooNDKeysPdf*>(rwsLQTemp->pdf(histn.c_str()));
		} //ivar
		...
		for (int ivar=0; ivar<iNVar; ivar++) ///even charge
		{
			...
			histn = Form("L%dQ_r%d_q%d_xy%d_sel%d_rig%d_KeysPdf", ilTemp, iSelSpan, Charge+ivar, ixy, iSelTemp, iRigTemp);
			rapSumpdf->plotOn(rpQFrame, RooFit::Components(histn.c_str()), RooFit::LineWidth(3), RooFit::FillStyle(fillStyle), RooFit::FillColor(iColor[ivar]), RooFit::LineColor(iColor[ivar]), RooFit::Range("draw"), RooFit::DrawOption("L"), RooFit::VLines()); //2020.09.30: without drawing the line
		...
		lLegend.DrawLatexNDC(.20, .90, Form("#color[%d]{Fit Range[%.2f, %.2f]}", kBlack, dFitL, dFitU));
		lLegend.DrawLatexNDC(.20, .85, Form("#color[%d]{Cut Range[%.2f, %.2f]}", kRed, dCutL, dCutU));
		...
		// lLegend.DrawLatexNDC(.65, .85, Form("#color[%d]{Chi2/ndf: %.0f/%d}", kBlack, chis_fit, ndf));
		for (int ivar=0; ivar<iNVar; ivar++){
