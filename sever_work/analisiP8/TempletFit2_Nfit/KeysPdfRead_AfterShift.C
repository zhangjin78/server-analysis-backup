...
int KeysPdfRead_AfterShift( int iSelRig=10,
				...
				const bool bBadLadder=false)
{
	...
	if(iNVar == 7) Charge = run_charge - 4;
	...
	if(iNVar == 10){
		dCutL = run_charge - 0.5; dCutU = run_charge + 0.5;
		dDrawL = Charge - 3; dDrawU = Charge + iNVar + 3;
		dAxisL = Charge - 1; dAxisU = Charge + iNVar + 1;
		dFitL = Charge - 0.5; dFitU = Charge + iNVar + 0.8;
	}
	...
	const string sTempDir=Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit/L1Qtemp/Template");
	TFile fTotalFile(Form("%s/total_result_0301.root", sTempDir.c_str()), "READ");
	// TFile fQTemp(Form("%s/LQTemp/LQTemp_rig%d.root", sTempDir.c_str(), iRigTemp), "READ");
	TFile fQTemp(Form("%s/LQTempAll/LQTemp_rig%d.root", sTempDir.c_str(), iRigTemp), "READ");

	////---- input shift root
	TFile fShift(Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TemplateFit/L2ShiftWithL1/result/combinded/L2TempFitL1QTemp_shiftT3T4.root"), "READ");
	bool bshift = true;
	...
	for (int ir=ispan; ir<ispan+1; ir++){
		...
		string sOutput = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/TempletFit2_Nfit";
		string sDraw = Form("%s/result/r%d_xy%d_NVar%d_rebin%d_q%d%s", sOutput.c_str(), ir, ixy, iNVar, NRebin, run_charge, sTighten.c_str());
		...
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			histn = Form("n%d_q%d", ivar, Charge+ivar);
			double dIni, dL, dU;
			dL=0, dU=0.4*events_hL1Q; dIni = 0.2*dU;  /// for 1st pdf
			int icharge = Charge + ivar;
			if(icharge % 2 == 1 && icharge > 7) dU=0.2*events_hL1Q, dIni=0.1*dU;
			// if((icharge % 2 == 0) && ivar > 0) dU=0.8*dU, dIni=0.4*dU;
			if(icharge == 14) dU=0.75*events_hL1Q, dIni=0.6*dU;
			// if(icharge == 15) dU=5000, dIni=0.2*dU;
			// if(icharge == 16) dU=0.4*events_hL1Q, dIni=0.1*dU;
			// else if(icharge == 16) dU=0.2*dU, dIni=0.4*dU;
			// else dU=0.1*dU, dIni=0.4*dU;
		...
		// rfrResult = rapSumpdf->fitTo(*rdhL1Q, RooFit::Save(0), RooFit::SumCoefRange("Fit"), RooFit::Range("Fit"), RooFit::PrintLevel(1));
		rfrResult = rapSumpdf->fitTo(*rdhL1Q, RooFit::Save(0), RooFit::Minimizer("Minuit", "minimize"), RooFit::SumCoefRange("Fit"), RooFit::Range("Fit"), RooFit::PrintLevel(1));
		// rfrResult = rapSumpdf->fitTo(*rdhL1Q, RooFit::Save(0), RooFit::Minimizer("GSLMultiMin", "steepestdescent"), RooFit::SumCoefRange("Fit"), RooFit::Range("Fit"), RooFit::PrintLevel(1));
		...
		double fscale = (sumrrvn*rarIntAll->getVal()) / events_cut;
		cout << fscale << endl;
		// rapSumpdf->plotOn(rpQFrame, RooFit::LineColor(kRed), RooFit::Range("draw"), RooFit::DrawOption("L"), RooFit::Normalization(fscale));
		rapSumpdf->plotOn(rpQFrame, RooFit::LineColor(kRed), RooFit::Range("draw"), RooFit::DrawOption("L"));
		// rapSumpdf->paramOn(rpQFrame);
		...
		//// compare the sunm Events in Cut Range
		// xBinL = hQDisD->FindBin(dCutL);
		// xBinU = hQDisD->FindBin(dCutU);
		// double events_hist = 0;
		// for(int ix = xBinL; ix < xBinU; ix++){
		// 	events_hist += hQDisD->GetBinContent(ix);
		// }
		// cout << endl;
		// cout << "\t" << events_hist << "\t" << events_cut << "\t" << sumrrvn*rarIntAll->getVal() << endl;
		// cout << endl;
		...
		for (int ivar=0; ivar<iNVar; ivar++)
		{
			...
			hShift->SetEntries(1);
			cout << "\t q_" << Charge+ivar << " \t" << rrvshift[ivar]->getVal() << " \t" << rrvn[ivar]->getVal() / events_fit << endl;
		} //ivar
