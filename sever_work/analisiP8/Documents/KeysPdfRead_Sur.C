...
#ifdef SDIAT
string sWDirOnline = "/home/ams/jzhang/AMS/Flux/Sulfur/plot/SurvivalProbability"; //XXX
#endif

int KeysPdfRead_Sur( int iSelRig=20,
				const int run_charge=12, 
				const int QVer=1,
				const int NVar0=-1,
				const int ixy=2,
				const int il=2,
				...
				const bool bBadLadder=false)
{
	...
	else if (run_charge>=9)
	{
		...
		dDrawL = run_charge - 5; dDrawU = run_charge + 4;
	...
	const string sTotalDir = Form("%s/root/ISS", sWDirOnline.c_str());
	TFile fL1Q(Form("%s/P8_SurvivalProbability_test2.root", sTotalDir.c_str()), "READ"); ///q9,10,11,12,13,14

	const string sTempDir=Form("%s/root/CHARGECAL_NEWTRQ", sWDirOnline.c_str());
	...
	for (int ir=ispan; ir<ispan+1; ir++){
		...
		//----output directory
		string sDraw = Form("%s/L1L2Sur/L1BK/TempletFit/r%d_xy%d_NVar%d_rebin%d_q%d", sTotalDir.c_str(), ir, ixy, iNVar, NRebin, run_charge);
