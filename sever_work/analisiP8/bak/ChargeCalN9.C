...
#ifdef SURSEL
double UBL1QX=0., UBL1QY=0., UBL1QXY=0.;
#endif
#endif //KEYSPDF
...
#endif //AMSSOFT
{
	...
	for (it=viQSel.begin(); it!=viQSel.end(); it++)
	{
		if (!ev.isreal) break; //only store for ISS
		#ifdef REDUCESIZELQTEMP
		for (int ir=1; ir<2; ir++)
		#else
		for (int ir=1; ir<3; ir++)
							...
							for (int isel=0; isel<MSel; isel++)
							{
								...
								if (il==0)
								{
									...
									for(int ivar = 0; ivar < 15; ivar++){
										tKeysPdf->Branch(Form("L1_%s", sVarL1QSel[ivar].c_str()), &dvarL1Q[ivar], Form("L1_%s/D", sVarL1QSel[ivar].c_str()));
									}
									#endif //L1QTRDVAR
...
#endif //AMSSOFT
{
	...
	#endif //REDUCESIZELQTEMP
	{
		for (int ialgo=0; ialgo<NRig; ialgo++)
		{
			...
			// if (!(ev.Select_TrTrack(iReferSpan, 111, ialgo) && bTkgeom[ir] && bRig[ir][ialgo] && ((ev.tk_qls[0] & 0x10013D)==0))) continue; //2023.04.06
			if (!(bTrack[ir][ialgo] && bTkgeom[ir] && bRig[ir][ialgo] && ((ev.tk_qls[0] & 0x10013D)==0))) continue; //2023.04.06
			#endif
			#endif
			string sRig=sRigName[ialgo];
				...
				for (int isigma=0; isigma<nSigma; isigma++)
				{
					...
					for (int itype=0; itype<MType; itype++)
					{
						...
						if (bTypeSel)
						{
							...
							#ifdef L1QTRDVAR
							for(int ivar = 0; ivar < 15; ivar++){
								dvarL1Q[ivar] = ev.GetL1QDisSelVar(ivar, *it);
							}
							#endif //L1QTRDVAR
							#ifdef SURSEL
