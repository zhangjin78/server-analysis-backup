...
//***Check the layer charge resolution 2023.11.03
// #define CHARGERES
...
int analysis(TChain &ch, string odir, string ofile, long int nrun=-1, const int iGeoSafety=1)
{
			...
			for (int ir=1; ir<2; ir++)
			{
				...
				for (int iperiod=0; iperiod<2; iperiod++)
				{
					...
					for (int itofqcut=0; itofqcut<4; itofqcut++)
					{
						...
						hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), i1DBin/10, iQDL, iQDU, i1DBin/10, iQDL, iQDU));
	...
	for (int iQVer=QVeri; iQVer<nQVer; iQVer++){
		if (iQVer%3<ChargeVersion || iQVer%3>ChargeVersion+nQVerStore-1) continue;
		outfile.mkdir(Form("ChargeRes%d", iQVer));

	} // iQver
	cout << "Finish booking histogram for Charge Resolution in different layer." << endl;
	#endif ///CHARGERES
					...
					#endif //L1L2SURNEW
					{
						...
						for (it=viQSel.begin(); it!=viQSel.end(); it++)
						{
							...
							for (int iQVer=QVeri; iQVer<nQVer; iQVer++)
							{
								...
								bool bL1PU = ev->GetChis(1, 1) < 10 && (ev->HasTkLHitXY(0)==2) && ((ev->tk_qls[0]&0x10013D)==0);
								// bool bL1PU = (ev->tk_lid[0] == ev->tk_exlid[0]) && (ev->tk_cad[0] == ev->tk_excad[0]);
								// bool bL1PU = (ev->tk_lid[0] == ev->tk_exlid[0]);

								// if(!bL1PU) continue; // 2023.04.12
								...
								for (int iperiod=0; iperiod<2; iperiod++)
								{
									...
									for (int itofqcut=0; itofqcut<4; itofqcut++)
									{
										...
										//---for L1Q distribution 2024.04.07
										if((ev->GetUBExtQBetaCor(0, 0, iQVer)>0 && ev->GetUBExtQBetaCor(0, 1, iQVer)>0) && (ev->tk_l1qvs > 1.5)){
											histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_BetaCor_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetUBExtQBetaCor(0, 2, iQVer), ww[ir]);
										}
										...
										for (int isel=-1; isel<iNTighten; isel++)
										{
											...
											// if (isel==-1) bL1QSel = (dL1Q>*it-1.5) && (dL1Q<*it-0.5);  // 2024.03.14 [Z-1.5,Z-0.5] range1
											// if (isel==-1) bL1QSel = (dL1Q>*it+0.5) && (dL1Q<*it+1.5);  // 2024.03.14 [Z+0.5,Z+1.5] range2
											// if (isel==-1) bL1QSel = (dL1Q>*it+1.5) && (dL1Q<*it+2.5);  // 2024.03.14 [Z+1.5,Z+2.5] range3
