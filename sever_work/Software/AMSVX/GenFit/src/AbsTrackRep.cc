...
AbsTrackRep::AbsTrackRep() :
  pdgCode_(0), pdgcharge_(0), pdgmass_(0), propDir_(0), debugLvl_(0)
{
  ;
}

AbsTrackRep::AbsTrackRep(int pdgCode, char propDir) :
  pdgCode_(pdgCode), pdgcharge_(0), pdgmass_(0), propDir_(propDir), debugLvl_(0)
{
  ;
}


AbsTrackRep::AbsTrackRep(double mass, double charge, char propDir) :
  pdgCode_(0), pdgcharge_(charge), pdgmass_(mass), propDir_(propDir), debugLvl_(0)
{
  ;
}
...
AbsTrackRep::AbsTrackRep(const AbsTrackRep& rep) :
  TObject(rep), pdgCode_(rep.pdgCode_), pdgcharge_(rep.pdgcharge_), pdgmass_(rep.pdgmass_), propDir_(rep.propDir_), debugLvl_(rep.debugLvl_)
{
  ;
}
...
double AbsTrackRep::getPDGCharge() const {
  if(pdgcharge_!=0)return  pdgcharge_;
...
double AbsTrackRep::getPDGMass() const {
  if(pdgmass_!=0)return pdgmass_;
...
bool AbsTrackRep::switchPDGSign() {
  if(pdgcharge_!=0){pdgcharge_*=-1;return true;}
  TParticlePDG* particle = TDatabasePDG::Instance()->GetParticle(-pdgCode_);
  if(particle != nullptr) {
    pdgCode_ *= -1;
    return true;
