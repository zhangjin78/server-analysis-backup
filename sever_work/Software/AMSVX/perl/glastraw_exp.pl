...
sub refreshStatus{
 $clastupdate=localtime();
    ...
    $lastupdate = time();
    my @servers=( "ams-soc-monitor", "pcamsf4" );
    my $iServer = 0;
    while (time() - $lastupdate <= 600) {
        my $conntime = localtime();
        print "$conntime: connecting $servers[$iServer] ...\n";
        $output = `timeout 30 ssh amslocal\@$servers[$iServer] "cd /afs/cern.ch/ams/Offline/vdev/perl/; ./rdlastraw.perl -ok @ARGV" 2>/tmp/err_ssh`;
        chomp $output;
        if  ($output eq "") {
            sleep 5;
            $iServer += 1;
            $iServer = $iServer % scalar @servers;
        }
        else {
            last;
        }
    ...
    if ($output eq "") {
        my $pingmsg = `ping -c 3 $servers[$iServer] | grep transmitted | awk '{print \$4}'`;
        chomp($pingmsg);
        if ($pingmsg eq '0') {
            $statusLabel->configure(-background => "red");
            $status = "Cannot reach $servers[$iServer]";
