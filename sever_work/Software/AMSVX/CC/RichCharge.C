...
#ifdef __PPC64
#pragma omp threadprivate(RichPMTCalib::_header)
#endif
   RichPMTCalib* RichPMTCalib::getHead(){return _header;};
...
bool RichPMTCalib::init_checkRichPmtTemperatures = true, RichPMTCalib::initok_checkRichPmtTemperatures = false, RichPMTCalib::stop_checkRichPmtTemperatures = false;
bool RichPMTCalib::init_getRichPmtTemperatures   = true, RichPMTCalib::initok_getRichPmtTemperatures   = false, RichPMTCalib::stop_getRichPmtTemperatures   = false;
bool RichPMTCalib::init_getRichBrickTemperatures = true, RichPMTCalib::initok_getRichBrickTemperatures = false, RichPMTCalib::stop_getRichBrickTemperatures = false;
bool RichPMTCalib::init_CheckRichRun             = true, RichPMTCalib::initok_CheckRichRun             = false, RichPMTCalib::stop_CheckRichRun             = false;
bool RichPMTCalib::init_ReadPmtDB                = true, RichPMTCalib::initok_ReadPmtDB                = false, RichPMTCalib::stop_ReadPmtDB                = false;

vector<int>   RichPMTCalib::v_pmt_dts, RichPMTCalib::v_utime;
vector<float> RichPMTCalib::v_temp[48];

vector<int>   RichPMTCalib::v_pmt_rep;
vector<string> RichPMTCalib::v_dts_el, RichPMTCalib::v_dts_nn;
multimap<float,int> RichPMTCalib::m_pmt_d2_dts[NPMT];

vector<int>   RichPMTCalib::v_pmt_rep_brick;
vector<string> RichPMTCalib::v_dts_el_brick, RichPMTCalib::v_dts_nn_brick;
multimap<float,int> RichPMTCalib::m_pmt_d2_dts_brick[NPMT];

map<int,string> RichPMTCalib::m_tag[NTAG];
vector<unsigned short> RichPMTCalib::w_pmt_stat, RichPMTCalib::w_pmt_volt;
vector<string> RichPMTCalib::v_brick_name[NBRICK];
vector<int> RichPMTCalib::v_brick_busa[NBRICK], RichPMTCalib::v_brick_busb[NBRICK], RichPMTCalib::v_brick_fgin[NBRICK];
vector<unsigned short> RichPMTCalib::v_pmt_stat_prev, RichPMTCalib::v_pmt_volt_prev;

void setCurrentDir(TString dir) {
  if(dir=="")
    dir=Form("%sRichDefaultPMTCalib",AMSDATADIR.amsdatabase);//the directory has been changed to v6.00
...
bool RichPMTCalib::initPMTs() {
  ...
  // Clear
  v_pmt_ecor.assign(long(NPMT), 1);
  v_pmt_gcor.assign(long(NPMT), 1);
  v_pmt_gmcor.assign(long(NPMT), 1);
  v_pmt_bcor.assign(long(NPMT), 1);
  v_pmt_temp_ref.assign(NPMT, pmtRefTemperature);
  v_pmt_ecor_dflt.assign(long(NPMT), 1);
  v_pmt_gcor_dflt.assign(long(NPMT), 1);
  v_pmt_gmcor_dflt.assign(long(NPMT), 1);
...
bool RichPMTCalib::checkRichPmtTemperatures() {
  ...
  static int last_rec = 0, prev_rec = 0;
#pragma omp threadprivate(last_rec,prev_rec)
  ...
  while (stop_checkRichPmtTemperatures);

  if (init_checkRichPmtTemperatures) {

    stop_checkRichPmtTemperatures = true;

    init_checkRichPmtTemperatures = false;

    v_pmt_temp.assign(long(NPMT), 25); // Default PMT Temperatures
    ...
    v_dts_g.assign(long(NDTS), 0);
    v_dts_x.assign(long(NDTS), 0);
    v_dts_y.assign(long(NDTS), 0);
    int ndts = 0;
    ...
    v_pmt_b.assign(long(NPMT), 0);
    v_pmt_g.assign(long(NPMT), 0);
    v_pmt_x.assign(long(NPMT), 0);
    v_pmt_y.assign(long(NPMT), 0);
    int npmt = 0;
    ...
    initok_checkRichPmtTemperatures = true;

    stop_checkRichPmtTemperatures = false;
  }


  if (!initok_checkRichPmtTemperatures) return checkRichPmtTemperatures;
...
#pragma omp threadprivate(last_run,last_time,skip_run,last_stat)
  ...
  while (stop_getRichPmtTemperatures);

  if (init_getRichPmtTemperatures) {

    stop_getRichPmtTemperatures = true;

    init_getRichPmtTemperatures = false;

    v_pmt_temp.assign(NPMT, pmtRefTemperature); // Default PMT Temperatures
    v_pmt_rep.assign(long(NPMT), 0);
    ...
    initok_getRichPmtTemperatures = true;

    stop_getRichPmtTemperatures = false;
  }

#ifndef __ROOTSHAREDLIBRARY__
  if (!initok_getRichPmtTemperatures) return RichPmtTemperatures;
  RichPmtTemperatures = true;
#else
  if (!initok_getRichPmtTemperatures || !AMSSetupR::gethead()) return RichPmtTemperatures;
  RichPmtTemperatures = true;
...
#pragma omp threadprivate(last_run,last_time,skip_run,last_stat)
  ...
  while (stop_getRichBrickTemperatures);

  if (init_getRichBrickTemperatures) {

    stop_getRichBrickTemperatures = true;

    init_getRichBrickTemperatures = false;

    v_brick_temp.assign(NPMT, brickRefTemperature); // Default HV Brick Temperatures
    v_pmt_rep_brick.assign(long(NPMT), 0);
    ...
    v_dts_b.assign(long(NDTS), 0);
    v_dts_el_brick.assign(NDTS, "");
    v_dts_nn_brick.assign(NDTS, "");
    int ndts = 0;
	...
	ndts++;
	v_dts_b[dts] = brick;
      	v_dts_el_brick[dts] = element;
	v_dts_nn_brick[dts] = node;
	//cout << " DTS " << dts << " BRICK " << v_dts_b[dts]
	//     << " " << v_dts_el_brick[dts] << " " << v_dts_nn_brick[dts] << endl;
      }
      else {
    ...
    // PMT-DST Map
    for (pmt=0; pmt<NPMT; pmt++) {
      m_pmt_d2_dts_brick[pmt].clear();
      for (dts=0; dts<NDTS; dts++) {
	if (v_dts_b[dts] == v_pmt_b[pmt])
	  m_pmt_d2_dts_brick[pmt].insert( pair<float,int>(1,dts) );
      }
      if (!m_pmt_d2_dts_brick[pmt].size()) {
	cout << "RichPMTCalib::getRichBrickTemperatures: Error in PMT-DST Mapping : PMT " << pmt << endl;
	return RichBrickTemperatures;
      }
      //for (it=m_pmt_d2_dts_brick[pmt].begin(); it!=m_pmt_d2_dts_brick[pmt].end(); it++)
      //cout << "PMT " << pmt << " d2 " << it->first << " dts " << it->second << endl;

    }

    initok_getRichBrickTemperatures = true;

    stop_getRichBrickTemperatures = false;
  }

#ifndef __ROOTSHAREDLIBRARY__
  if (!initok_getRichBrickTemperatures) return RichBrickTemperatures;
  RichBrickTemperatures = true;
#else
  if (!initok_getRichBrickTemperatures || !AMSSetupR::gethead()) return RichBrickTemperatures;
  RichBrickTemperatures = true;
  ...
  if (int(AMSEventR::Head()->fHeader.Run)!=last_run ||
    ...
    for (int dts=0; dts<NDTS; dts++) {
      rc = AMSSetupR::gethead()->fSlowControl.GetData(
						      v_dts_el_brick[dts].c_str(),
						      AMSEventR::Head()->fHeader.Time[0],
						      0,
						      value,
						      method,
						      v_dts_nn_brick[dts].c_str()
						      );
      if (!rc && value[0]<127.) v_dts_temp[dts] = value[0]; // protection against crazy DB values
      else if (rc==1) {
  ...
  for (int pmt=0; pmt<NPMT; pmt++) {
    ...
    if (v_brick_temp[pmt]==HUGE_VALF) {
      if (DEBUG)
	if (v_pmt_rep_brick[pmt]++ < MAX_REP) 
	  cout << "RichPMTCalib::getRichBrickTemperatures: "
	       << " Run "   << AMSEventR::Head()->fHeader.Run 
	       << " Event " << AMSEventR::Head()->fHeader.Event 
	       << " NoValidTempFoundForPMT " << pmt 
	       << " (Reported/Max " << v_pmt_rep_brick[pmt] << "/" << MAX_REP << ")" << endl;
...
#pragma omp threadprivate(run_prev,richRunTag_prev)

  while(stop_CheckRichRun);

  if (init_CheckRichRun) {

    stop_CheckRichRun = true;

    init_CheckRichRun = false;

    w_pmt_stat.assign(long(NPMT), richRunDflt << 8 | richPmtDflt);
    w_pmt_volt.assign(long(NPMT), richVltDflt);
  ...
  if (run == run_prev) {
    ...
    for (brick=0; brick<NBRICK; brick++) {
      ...
      v_brick_busa[brick].assign(NSIDE, -1l);
      v_brick_busb[brick].assign(NSIDE, -1l);
      v_brick_fgin[brick].assign(NSIDE, -1l);
    }
    int nbside = 0;
  ...
  if (!initok_CheckRichRun) return richRunTag;
...
bool RichPMTCalib::ReadPmtDB() {
  ...
  while(stop_ReadPmtDB);

  if (init_ReadPmtDB) {

    stop_ReadPmtDB = true;

    init_ReadPmtDB = false;

    v_pmt_rdr.assign(long(NPMT), 0);
    v_pmt_brick.assign(long(NPMT), 0);
    v_pmt_hvchn.assign(long(NPMT), 0);
    v_pmt_vnom.assign(long(NPMT), 0); 
    ...
    initok_ReadPmtDB = true;

    stop_ReadPmtDB = false;
  }

  if (initok_ReadPmtDB) readPmtDB = true;
...
bool RichChargeUniformityCorrection::Init(TString file){
  ...
  if(file==""){
    // Get default
    file=Form("%sRichChargeUniformityCorrection.root",AMSDATADIR.amsdatadir);//the directory has been changed to v6.00
  }
...
cerr<<" RichChargeUniformityCorrection-E-UnableToReadFile "<<file<<endl;
}
     f.Close();
