//  $Id: server.C,v 1.245 2018/06/18 12:33:35 aegorov2 Exp $
//
#include <stdlib.h>
#include "server.h"
#include <fstream>
...
{
	AMSServer::OrbitVars e;
	e._orb = new CORBA::ORB(orb);
	e._poa = new PortableServer::POA(child_poa);
	e._mgr=e._poa->the_POAManager();
	e._mgr->activate();
	cout <<"  Enter Listening loop ... thread #"<<omp_get_thread_num()<<endl;
	while(true) {
...
#pragma omp single nowait
{
	...
	e._mgr=e._poa->the_POAManager();
	e._mgr->activate();
    cout <<"  Enter SystemCheck loop ... thread #"<<omp_get_thread_num()<<endl;
#endif
    while(true){
      try{
...
if(iface)_pid.Interface = (const char *)iface;
   ...
   cout <<"amsdatabase  "<<AMSDBc::amsdatabase<<endl;
...
#ifdef __x86_64__ && _IS_CORBA64_
    // indicate initial cwd
    char *icwd = new char[PATH_MAX];
    char *ocwd;
    if ((ocwd=getcwd(icwd,PATH_MAX-1)) || (ocwd=getenv("PWD")))
      cout<<"AMSServer::AMSServer-I- working directory "<<ocwd<<endl;
    else
      cout<<"AMSServer::AMSServer-W- can't get working directory"<<endl;
    delete icwd;
    // change cwd
	const char *pcwd=getenv("AMSServerCWD");
	const char *ptmp=getenv("TMPDIR");
	const char *iorip = hasArg(argc2,argv2,"--ORBIIOPIPSock=");
    if (iorip && !pcwd) pcwd =  ptmp;
    if (pcwd)
    	if (pathExists(pcwd)) {
    		chdir(pcwd);
    		cout<<"AMSServer::AMSServer-I- working directory changed to "<<pcwd<<endl;
    	}
    	else
    		cout<<"AMSServer::AMSServer-I- can not change working directory to "<<pcwd<<endl;
    if (iorip) remove(iorip);
#endif
    ...
    try {
         orb = CORBA_ORB_init(&argc2,argv2,"orbit-local-non-threaded-orb",&ev);
         poa = (PortableServer_POA)CORBA_ORB_resolve_initial_references(orb,"RootPOA",&ev);
         mgr = PortableServer_POA__get_the_POAManager(poa,&ev);

         int n_policies = 1;
 	     if (hasArg(argc,argv,"--ORBIIOPIPSock="))
 	    	n_policies++;
         policies = CORBA_PolicyList__alloc();
         policies->_maximum = n_policies;
         policies->_length  = n_policies;
         policies->_buffer  = CORBA_PolicyList_allocbuf(n_policies);
         CORBA_sequence_set_release (policies, CORBA_TRUE);
         policies->_buffer[0] = (CORBA_Policy)PortableServer_POA_create_id_uniqueness_policy(poa,PortableServer_MULTIPLE_ID,&ev);
         if (n_policies > 1)
        	 policies->_buffer[1]=(CORBA_Policy)PortableServer_POA_create_lifespan_policy(poa,PortableServer_TRANSIENT,&ev);
	 ...
	 if(fbin){
		  while(!fbin.eof() && fbin.good()){
				...
				*/
				...
				// tmp
				//
				   cout << "AMSServer::AMSServer-I- tmp init started, "<<tmpbuf<<endl;
				   ...
				   cout<<"AMSServer::AMSServer-I- IORS "<<str<<endl;
...
void AMSServer::Listening(int sleeptime) {
    ...
    if(AMSServer::Singleton()->getSleepTime()<20000){
        cout<<"AMSServer::Listening-I- entering listening "<<"  "<<AMSServer::Singleton()->getSleepTime()<<" "<<sleeptime<<"  "<<ntry<<endl;
    }
    // i don't understand the meaning of ntry>1 (done by vc)
    ntry=1;
#ifndef _OPENMP
...
//        else ((*i).second)._orb->perform_work();
       	  if (((*i).second)._orb->work_pending())
              ((*i).second)._orb->perform_work();
       }
    }
    if(AMSServer::Singleton()->getSleepTime()<20000){
        cout <<"AMSServer::Listening-I- exiting listening "<<"  "<<AMSServer::Singleton()->getSleepTime()<<" "<<sleeptime<<endl;
    }
#else
#pragma omp critical
    for(MOI i=_orbmap.begin();i!=_orbmap.end();++i) {
...
#endif
    time_t tdur = tstart-time(0);
    if (tdur>150)
    	cout<<"AMSServer::Listening-W- slow listening loop: "<<tdur<<" sec"<<endl;
}
...
/* Here run Start,Stop,Kill,Check Clients:
		  Server_impl::CheckClients
		  Client_impl::CheckClients
		Producer_impl::CheckClients
		Producer_impl::CheckRuns
		  Server_impl::StartClients
		  Client_impl::StartClients
		Producer_impl::StartClients
		  Server_impl::KillClients
		  Client_impl::KillClients
		Producer_impl::KillClients
*/
    int count=AMSServer::Singleton()->MT()?-1:1;
for(AMSServerI * pcur=_pser; pcur; pcur=(pcur->down())?pcur->down():pcur->next()){
  ...
  // here and below vc condition was: if(check<1000)
  if(check<2)cout << "  list ok "<<endl;
  if(!server)pcur->StartClients(_pid);
  if(check<2)cout << "  start ok "<<endl;
  if(fforce)Listening(count);
  if(check<2)cout << "  list ok "<<endl;
  if(!server)pcur->CheckClients(_pid);
  if(check<2)cout << "  check ok "<<endl;
  pcur->CheckRuns();
  if(check<2)cout << "  check runs ok "<<endl;
  if(fforce)Listening(count);
  if(check<2)cout << "  list ok "<<endl;
  if(!server)pcur->KillClients(_pid);
  if(check<2)cout << "  kill ok "<<endl;
#else
#pragma omp critical
  if(!server)pcur->StartClients(_pid);
  if(check<2)cout << "  start ok "<<endl;
...
#pragma omp critical
  ...
  static time_t kinit=0;
  if (time(0)-kinit > 60) {
	kinit = time(0);
	long cred=0;
	char *pcred = getenv("AMSServCred");
	if (pcred) cred = atol(pcred);
	if (cred == 2) {
		system("kinit -R");
		...
		char fout[256];
		sprintf(fout,"/tmp/server.krb5.v%d",_v5);
		AString cmd="klist ";
		cmd+=" >& ";
		cmd+=fout;
		int i=system((const char*)cmd);
		if(i==0 ){
			...
			cerr<<"AMSServer::SystemCheck-E- "<<(const char*)cmd<<" failed"<<endl;
	}
  }
...
_ExitInProgress=true;
  Server_impl* pser=dynamic_cast<Server_impl*>(_pser);
  DPS::Client::ARS_var arf;
  int length=pser?pser->getARS(_pid,arf):0;
...
#else
    DPS::Server_ptr _pvar = reinterpret_cast<DPS::Server_ptr>(cnarrow<DPS::Server>::giop((_orbmap.find((const char*)(arf[i].Interface))->second)._orb,arf[i].IOR));
#endif
...
void AMSServerI::_UpdateACT(const DPS::Client::CID & cid, DPS::Client::ClientStatus st, bool forced) {
	{  time_t tt;
	   time(&tt);
	...
	for(ACLI j=_acl.begin();j!=_acl.end();++j){
		...
		if (forced || acv->Status != st || acv->id.threads != cid.threads || acv->id.threads_change != cid.threads_change) {
			acv->LastUpdate=tt;
			acv->Status=st;
...
void AMSServerI::PropagateAC(DPS::Client::ActiveClient & ac,DPS::Client::RecordChange rc,DPS::Client::AccessType type, uinteger uid){
  int lenHost = safe_strlen(ac.id.HostName,48);
  int lenInterf = safe_strlen(ac.id.Interface,32);
  if(lenHost<0 || lenInterf<0) {
	  cerr<<"AMSServerI::PropagateAC-E- **sendac not send "<< lenHost <<" "<<lenInterf<<endl;
	  return;
  }
  cout<<"AMSServerI::PropagateAC-I- rc="<<AMSClient::RC2string(rc)<<" uid="<<ac.id.uid<<
        " Status="<<AMSClient::CS2string(ac.Status)<<" exitStatus="<<AMSClient::CSE2string(ac.id.Status)<<
        " threads="<<ac.id.threads<<" host="<<ac.id.HostName<<" interf="<<ac.id.Interface<<endl;
  PropagateACDB(ac,rc);
//         cout <<" entering Producer_impl::propagateac"<<endl;
  ...
  DPS::Client::ARS_var arf;
  int length=_pser->getARS(cid,arf,type,uid);
  if(length==0){
      _parent->EMessage(AMSClient::print(ac,"PropagateAC-getARS returns 0"));
  }
//  cout <<"propagateAC - getars returns "<<length<<endl;
  ...
  try{
    ...
    DPS::Server_var _pvar=DPS::Server::_narrow(obj);
#else
    DPS::Server_ptr _pvar = reinterpret_cast<DPS::Server_ptr>(cnarrow<DPS::Server>::giop(_defaultorb,arf[i].IOR));
#endif
...
/*
     ...
     if(li!=_pser->getacl().end()){
      ...
      _parent->EMessage(AMSClient::print(ac," PropagateRunExceptionClientNotFound"));
     }
*/
   }
  } 
...
void AMSServerI::PropagateAH(const DPS::Client::CID & pid,DPS::Client::ActiveHost & ah,DPS::Client::RecordChange rc,DPS::Client::AccessType type, uinteger uid){
  int lenHost = safe_strlen(ah.HostName,48);
  int lenInterf = safe_strlen(ah.Interface,32);
  if(lenHost<=0 || lenInterf<=0) {
	  cerr<<"AMSServerI::PropagateAH-E- **sendah not send "<< lenHost <<" "<<lenInterf<<endl;
	  return;
  }
  lenHost = safe_strlen(pid.HostName,48);
  if(lenHost<=0) {
	  cerr<<"AMSServerI::PropagateAH-E- **sendcid not send "<< lenHost <<" "<<lenInterf<<endl;
	  return;
  }
  cout<<"AMSServerI::PropagateAH-I- rc="<<AMSClient::RC2string(rc)<<" uid="<<pid.uid<<" status="<<AMSClient::HS2string(ah.Status)<<
		  " host="<<ah.HostName<<" interf="<<ah.Interface<<" running="<<ah.ClientsRunning<<" allowed="<<ah.ClientsAllowed<<endl;
  ...
  int length=_pser->getARS(cid,arf,type,uid);
  if(length==0){
      _parent->EMessage(AMSClient::print(ah,"PropagateAC-getARS returns 0"));
  }
//  cout <<"propagateAC - getars returns "<<length<<endl;
  time_t tt;
  time(&tt);
  ...
  try {
     ...
     cerr<<"AMSServerI::PropagateAH-E- oops corba exc found for "<<i<<" "<<ctime(&tt)<<endl;
   }
  } 
}
...
void AMSServerI::_UpdateHosts(){
    for(AHLI i=_ahl.begin(); i!=_ahl.end(); ++i){
       int runcl=0;
  	   int ahHost_len = safe_strlen((const char *)((*i)->HostName),48);
  	   if (ahHost_len<=0) {
  		   cout<<"AMSServerI::_UpdateHosts-E- wrong ah.HostName, len "<<ahHost_len<<endl;
  		   continue;
  	   }
       for (ACLI j=_acl.begin(); j!=_acl.end(); ++j) {
    	 int aclHost_len = safe_strlen((const char *)(((*j)->id).HostName),48);
    	 if (aclHost_len<=0) {
    		 cout<<"AMSServerI::_UpdateHosts-E- uid "<<(*j)->id.uid<<", wrong ac.HostName, len "<<aclHost_len<<endl;
    		 continue;
    	 }
       ...
       if(runcl!=(*i)->ClientsRunning){
         _parent->EMessage(AMSClient::print(*i,"ResettingHost "));
...
char *a2="41754110923@mail2sms.cern.ch";
...
#else
      DPS::DBServer_ptr dvar = reinterpret_cast<DPS::DBServer_ptr>(cnarrow<DPS::DBServer>::giop(_defaultorb,((*li)->ars)[i].IOR));
#endif
...
//    (*li)->LastUpdate=tt;
    ...
    if(_parent->Debug())_parent->EMessage(AMSClient::print(acv,"Server_impl::CheckClients-W- Client TIMEOUT"));
    PropagateAC(acv,DPS::Client::Update,DPS::Client::AnyButSelf,acv->id.uid);
 }
 }
}
...
//cout <<" entering Server_impl::sendId"<<endl;
    int lenHost = safe_strlen(cid.HostName,48);
    int lenInterf = safe_strlen(cid.Interface,32);
    if(lenHost<=0 || lenInterf<0) {
    	cerr<<"Server_impl::sendId-E- wrong cid "<<cid.uid<<" "<<lenHost<<" "<<lenInterf<<endl;
        return false;
    }
   	cout<<"Server_impl::sendId-I- uid="<<cid.uid<<" threads="<<cid.threads<<
   		" host="<<cid.HostName<<" interf="<<cid.Interface<<endl;
      cid.Mips=Mips;
...
//cout <<" entering Server_impl::getNC"<<endl;
 ...
 if(_parent->DBServerExists()){
     ...
     DPS::Client::ARS_var arf;
     int length=_pser->getARS(pid,arf,DPS::Client::Any,0,1);
     for(unsigned int i=0;i<length;i++){
      try{
#ifndef NARROW_ONCE
       CORBA::Object_var obj=_defaultorb->string_to_object(arf[i].IOR);
...
#else
       DPS::DBServer_ptr _pvar = reinterpret_cast<DPS::DBServer_ptr>(cnarrow<DPS::DBServer>::giop(_defaultorb,arf[i].IOR));
#endif
    ...
    if(pcur->getType()==cid.Type){
      ...
      switch (rc){
       ...
       if(li==pcur->getacl().end())
	    _parent->EMessage(AMSClient::print(cid,"Client not found for deleting"));
       else{
        if(_parent->Debug())_parent->IMessage(AMSClient::print(ac,"Server_impl::sendAC-I-Deleting Client "));
...
//cout <<" entering Server_impl::sendAH "<<ah.HostName<<endl;
  ...
  for (AMSServerI* pcur=this;pcur;pcur=pcur->next()?pcur->next():pcur->down()){
    if(pcur->getType()==cid.Type){
      ...
      switch (rc){
       case DPS::Client::Update:
       if(li==pcur->getahl().end())_parent->EMessage(AMSClient::print(ah,"Server_impl::sendAH-W- Host not found for editing"));
       else{
/* see EMessage below
        cout <<" Server_impl::SendAH-I- "<<cid.Type<<" "<<ah.HostName<<" "<<" "<<ah.Interface<<
        		" "<<(*li)->ClientsRunning<<" "<<(*li)->ClientsAllowed<<endl;
*/
        _parent->EMessage(AMSClient::print(ah,"Server_impl::sendAH-I- update "));
...
//cout <<" exiting Server_impl::sendAH"<<endl;
  ...
  for (AMSServerI* pcur=this;pcur;pcur=pcur->next()?pcur->next():pcur->down()){
    if(pcur->getType()==cid.Type){
      ...
      switch (rc){
       case DPS::Client::Update:
       if(li==pcur->getnhl().end())_parent->EMessage(AMSClient::print(cid,"Server_impl::sendNH-W- Host not found for editing"));
       else{
    	_parent->EMessage(AMSClient::print(ah,"Server_impl::sendNH-I- update"));
...
return length;
  ...
  void Producer_impl::pingp()throw (CORBA::SystemException){
	  cout<<"Producer_impl::pingp-I- done"<<endl;
  }
...
/* this piece of code has been inserted by demand of Baosong  on debuging,
 * we don't need it in production, but let's keep it available
 *
   string flog = string("/tmp/ping.")+host;
   string cmd = string("ping -c 1 -w 9 ")+host+" >& "+flog;
   stringstream mess;
   int retries = 1;

    int result = system(cmd.c_str());
	fstream fs(flog.c_str(),fstream::in);
	if (fs.is_open()) {
		mess<<"Server_impl::pingHost-I- "<<host<<" result="<<result<<endl;
		while (fs.good()) {
			string s;
			std::getline(fs,s);
			mess<<s<<endl;
...
//Get producer_var and read producer specifics
    ...
    cid.Interface= (const char *) " ";
    DPS::Client::ARS_var arf;
    int length=_pser->getARS(cid,arf);
...
#else
    _pvar = reinterpret_cast<DPS::Producer_ptr>(cnarrow<DPS::Producer>::giop(_defaultorb,arf[i].IOR));
#endif
...
for(AMSServerI * pcur=getServer(); pcur; pcur=(pcur->down())?pcur->down():pcur->next()){
 ...
 bool suc=false;
 if(_parent->DBServerExists() && getHostInfoSDB(_parent->getcid(),ahlv)){
     if (safe_strlen((*ahlv).HostName,48)>0 && safe_strlen((*ahlv).Interface,32)>=0)
    	 suc=true;
     else
         cerr<<"  Producer_impl::StartClients-E- suspicious ah.HostName, skipped"<<endl;
 }
 else if(!_parent->DBServerExists()){
 _ahl.sort(Less());
 ...
 for(AHLI i=_ahl.begin();i!=_ahl.end();++i){
  if((*i)->Status!=DPS::Server::NoResponse && (*i)->Status!=DPS::Server::InActive){
      ...
      else cerr<<"  Producer_impl::StartClients-E- suspicious host, skipped"<<endl;
    }
   }
  }
 }
  ...
  CORBA::String_var _refstring;
   ...
   if(_refmap.find((const char *)((ahlv)->Interface))->second){
    _refstring=_refmap.find((const char *)((ahlv)->Interface))->second;
     ...
     _parent->EMessage(AMSClient::print(ahlv, " Producer_impl::StartClients-W- Could not find refstring for the host "));
     _refstring=_refmap.find((const char *)("default"))->second;
...
if(0 || (reinfo->CounterFail>minr && reinfo->History==DPS::Producer::Failed)){
        singlethread=true;
}
    ...
    if(cli==_ncl.end())cli=_ncl.begin();
     ...
     ac.id.Interface=(ahlv)->Interface;
        ...
        if (dstinfo->DieHard) {
          ...
          if (ahlv->Status == DPS::Client::InActive) {
              // [ae] leave transition state, let node working
        	  // controversial solution, since new EventStatusTable write algorithm
        	  ahlv->Status = DPS::Client::OK;
        	  HostClientFailed(ahlv);
              DPS::Client::CID cid=_parent->getcid();
              cid.Type=getType();
              ...
              // originally by [vc], run not found, aborting client
        	  // node keeps staying InActive
        	  HostClientFailed(ahlv);
    ...
    bool condor=false;
    if(strstr((const char*)submit,"condor"))condor=true;
    if(condor)cout<<"StartClients-I-CondorEnvDetected "<<endl; 
//    else cout<<"StartClients-I-NOCONDOR !!! "<<(const char*)submit<<endl;
    if(condor){
         submit+=" hostname=";
         submit+=(const char*)((ahlv)->HostName);
         submit+=" exec=";	
    }
	 int pos_script = strlen((const char*)submit);
       AString fnam="";
     if(_parent->IsMC()){
    ...
    int pose_script = strlen((const char*)submit);
	// script path from command line
	string submit_script = string((const char *)submit).substr(pos_script,pose_script-pos_script);
	// trim spaces to the right side
	char *pch = submit_script.c_str();
	for (int i=submit_script.length()-1; i>=0; i--)
	  if (pch[i] == ' ') pch[i] = '\0';
	  else break;
    if(condor)submit+=" args=\\\"";   	
    submit+=" -";
    submit+=(const char*) _refstring;
    submit+=" -U";
    submit+=uid;
    if(_parent->MT())submit+=" -M";
    if(_parent->Debug())submit+=" -D1";
    int corfac=(long)200000/((ahlv)->Clock>0?(ahlv)->Clock:1000);
    char cuid[80];
    ...
    if(condor){
          submit+="\\\" ";   	
          submit+=getenv("AMSProdDir");
          submit+="/prod/submit.condor ";
    }
    if((*cli)->LogInTheEnd){
     submit+=" ";
     submit+=(const char*)((*cli)->LogPath);  
    ...
    int pos = -1;
    int pose = -1;
    string s_submit=(const char*)submit;
    int posb1=s_submit.find("bsub");
    if ((posb1>=0 || condor) && (pos=s_submit.find(pat))>=0)
    	pose=s_submit.find(" ",pos+strlen(pat)+1);
    ...
    if(_parent->Debug())_parent->IMessage(submit);
{
      if(pos>=0){
       ...
       AString badd=(!condor)?" 1>& /afs/cern.ch/ams/local/bsubs/":" 1>&/afs/cern.ch/ams/local/csubs/";
       
       badd+=uid;
       if(!(*cli)->LogInTheEnd){
         submit +=(const char*)badd;
       }
       cout<<"AMSProducer::StartClients-I- bsub DetectedTimeoutchanged "<<ac.TimeOut<<" "<<submit<<endl;
      }
}
    submit+=" &";
	
    // submit string is ready
    s_submit=(const char*)submit;
    // read job script
    bool ftxt_ok;
    string sMISC;
    string sAMSFSCRIPT;
    ...
    if(ftxt) {
		...
		while(ftxt.good() && !ftxt.eof()){
			ftxt.getline(line,255);
			line[255] = "\0";
			if(strstr(line,"MISC "))
				sMISC = line;
			else if(strstr(line,"AMSFSCRIPT")) {
...
//         read script put max cpu number for given host
        if(ftxt_ok){
          if (sMISC.length() > 1) {
            ...
            if(poss>=0){
              char *pch=strtok(cline.c_str()+poss+3," ");
              if(pch){
                int tr=atol(pch);
                ...
                if(tr>32)tr=32;
                char cmaxn[33];
                sprintf(cmaxn," -n %u ",tr);
                threads=tr;
                if(pos>=0){
                   cout<<"AMSProducer::StartClients-I-threadChangedFrom "<<s_submit.substr(pos,pose-pos)<<endl;
                   s_submit.replace(pos,pose-pos,cmaxn);
                   submit=s_submit.c_str();
                   cout<<"AMSProducer::StartClients-I-threadChangedTo "<<cmaxn<<endl;
                }
              }
            }
          }
    ...
    if(singlethread){
      ...
      if(pos>=0) {
         cout<<"AMSProducer::StartClients-I-threadChangedFrom "<<s_submit.substr(pos,pose-pos)<<endl;
         s_submit.replace(pos,pose-pos," -n 1 ");
         submit=s_submit.c_str();
         cout<<"AMSProducer::StartClients-I-threadChangedTo "<<" -n 1 "<<endl;
      }
    }
	if (pos>=0 && sAMSFSCRIPT.length()>1 && sAMSFSCRIPT.find("=")>=0) {
		const char *pch = sAMSFSCRIPT.c_str() + sAMSFSCRIPT.find("=") + 1;
		if (strcmp(pch,submit_script.c_str())) {
			s_submit.replace(s_submit.find(submit_script),submit_script.size(),pch);
			cout<<"AMSProducer::StartClients-I-scriptChangedFrom " << submit_script << endl;
			submit = s_submit.c_str();
			cout<<"AMSProducer::StartClients-I-scriptChangedTo " << pch << endl;
		}
	}

             // -n nthreads  --> n=
    if(condor && (pos=s_submit.find(pat))>=0){
         s_submit.replace(pos,strlen(pat)," n=");
 	cout<<"AMSProducer::StartClients-I-CondorReplace "<<s_submit<<endl;
    }
        
    submit=s_submit.c_str();
    cout<<"AMSProducer::StartClients-I-submit "<<submit<<endl;
    int out=system((const char*)submit);
    ...
    cout <<"   AMSProducer::StartClients-I-InitialthreadsSet "<<threads<<endl;
    PropagateAC(acv,DPS::Client::Create);
//     ((ahlv)->ClientsRunning)++;
    }
    else{
...
for(ACLI li=_acl.begin();li!=_acl.end();++li){
 ...
 if((*li)->Status!=DPS::Client::Killed && ((*li)->LastUpdate+(*li)->TimeOut<tt && ((*li)->Status!=DPS::Client::Submitted || (*li)->LastUpdate+100<tt))){
   ...
   if(acv->Status==DPS::Client::Submitted){
    ...
    NCLI cli=find_if(_ncl.begin(),_ncl.end(),NCL_find((const char *)(acv->id).HostName));
    if(cli==_ncl.end()) cli=_ncl.begin();
    if (strstr((*cli)->LogPath,"condor"))
    	is_condor = true;
    ...
    char uid[80];
     sprintf(uid,"%u",(*li)->id.uid);
      ...
      if(kstart>=0 && kend>=0 && kstart<kend && isdigit(line[kstart+1])){
        ...
        cout <<"Producer_impl::CheckClients-W- Check Client jid found jid="<<jid<<", uid="<<uid<<endl;
        acv->id.pid=jid;
        char tmp[1024];
        ...
        cout <<"Producer_impl::CheckClients-I- "<<tmp<<endl;
        int suc=system(tmp);
        if(suc){
...
// check csubs
    ...
    const char add[]="/afs/cern.ch/ams/local/csubs/";
    AString badd=add;
    char uid[80];
     sprintf(uid,"%u",(*li)->id.uid);
      badd+=uid;
      ...
      fbin.open((const char*)badd);
         int kstart=-1;
         int kend=-1;
      while(fbin && fbin.good() && !fbin.eof()){
        char line[1255]="";
        fbin.getline(line,1254);
        if(strstr(line,"submitted to cluster")){
          char *pch=strtok(line," .");
          for(int l=0;l<5;l++)if(pch)pch=strtok(NULL," .");
            if(pch)jid=atol(pch);
          break;
        }
      ...
      if(jid>0){
        ...
        acv->id.pid=jid;
        char tmp[1024];
        sprintf(tmp,"%s%s.csub",add,uid);
        unlink(tmp);
        sprintf(tmp,"/afs/cern.ch/ams/local/bin/timeout --signal 9 60 ssh %s /afs/cern.ch/ams/local/bin/timeout --signal 9 120  condor_q %u 1>& %s%s.csub",(const char*)(acv->id).HostName,jid,add,uid);
        cout <<"Producer_impl::CheckClients-I- "<<tmp<<endl;
        int suc=system(tmp);
        if(suc){
            cerr<<" unable to "<<tmp<<endl;
...
//         cout <<" line "<<line<<endl;
         ...
         if(!found)cerr<<" unable to find CMD: in "<<tmp<<" "<<jid<<endl;         
        }
      else{
         cerr <<" unable to open "<<tmp<<endl;
      }
      ...
      unlink(tmp);
       ...
       cerr <<"unable to find jid in "<<badd<<endl;
     }
} //!!!!!
     ...
     acv->Status=DPS::Client::TimeOut;
{
  ...
  }
  {
   ...
   if(_parent->Debug())_parent->EMessage(AMSClient::print(acv,"Producer_impl::CheckClients-W- Client TIMEOUT"));
   PropagateAC(acv,DPS::Client::Update);
 }
}
...
//cout <<" entering Producer_impl::sendId"<<endl;
    ...
    int lenInterf = safe_strlen(cid.Interface,32);
    if(lenHost<=0 || lenInterf<0) {
    	cerr<<"Producer_impl::sendId-E- wrong cid "<<cid.uid<<" "<<lenHost<<" "<<lenInterf<<endl;
        return false;
    }
    cout<<"Producer_impl::sendId-I- uid="<<cid.uid<<" pid="<<cid.pid<<" ppid="<<cid.ppid<<
    	" threads="<<cid.threads<<" host="<<cid.HostName<<" interf="<<cid.Interface<<" timeout="<<timeout<<endl;
     cid.Mips=Mips;
     for(ACLI j=_acl.begin();j!=_acl.end();++j){
      if(((*j)->id).uid==cid.uid && (*j)->Status ==DPS::Client::Submitted){
       ((*j)->id).pid=cid.pid;
       ((*j)->id).ppid=cid.ppid;
// CID::coid field added, the line below was initially commented by vc
//       ((*j)->id).coid=cid.coid;
       ((*j)->id).Mips=Mips;
       ((*j)->id).threads=cid.threads;
       ((*j)->id).threads_change=cid.threads_change;
...
//     ac.id.coid=cid.coid;
     ...
     for(AHLI i=_ahl.begin();i!=_ahl.end();++i) {
    	int lenHost = safe_strlen((*i)->HostName,48);
    	int lenInterf = safe_strlen(cid.Interface,32);
    	if(lenHost<=0 || lenInterf<0) {
        	cerr<<"Producer_impl::sendId-E- wrong ah found "<<lenHost<<" "<<lenInterf<<endl;
    		continue;
    	}
        if(!strcmp((const char *)(*i)->HostName, (const char *)(ac.id).HostName)){
          (*i)->Clock=ac.id.Mips;
          _parent->IMessage(AMSClient::print (*i,"Producer_impl::sendId-W- Setting Clock to "));
          _parent->IMessage(AMSClient::print (ac,"Producer_impl::sendId-W- Setting Clock to "));
          PropagateAH(cid,(*i),DPS::Client::Update);
          break;
        }
     }
...
_UpdateACT(cid,DPS::Client::Active,false);
	 Server_impl* _pser=dynamic_cast<Server_impl*>(getServer());
...
string mess="Producer_impl::Exiting-I- ";
if (message)
	mess = mess+"message="+message+", ";
_parent->IMessage(AMSClient::print(cid,mess.c_str()));
Server_impl* _pser=dynamic_cast<Server_impl*>(getServer()); 
_pser->MonInfo(AMSClient::print(cid,mess.c_str()),DPS::Client::Info);
// find and remove client
for( ACLI li=_acl.begin();li!=_acl.end();++li){
 if (cid.uid==((*li)->id).uid){
   (*li)->id.Status=status;
   DPS::Client::ActiveClient_var acv=*li;
   if(acv->id.Status==DPS::Client::CInAbort) {
	   RLI li=find_if(_rl.begin(),_rl.end(),REInfo_EqsClient(cid));
       if(li!=_rl.end() && (*li)->Status==DPS::Producer::Processing) {
         DPS::Producer::RunEvInfo_var rv=*li;
      	 multimap<long,long> dsts;
      	 for(DSTLI li=_dst.begin();li!=_dst.end();++li){
      	 	DPS::Producer::DST &dst = li->second;
      	 	if(dst.Run==rv->Run && dst.Status==DPS::Producer::Validated)
      	 		dsts.insert(pair<long,long>(dst.FirstEvent,dst.LastEvent));
      	 }
      	 long range=0, skiped=0;
      	 for (multimap<long,long>::iterator it=dsts.begin(); it!=dsts.end(); ++it)
      	 	if (it->first>range) range += it->second-it->first+1;
      	 	  else skiped++;
      	 if (!skiped && range==rv->LastEvent-rv->FirstEvent+1) {
      	 	 rv->Status=DPS::Producer::Finished;
      	 	 rv->cuid=0;
      	 	 rv->cinfo.HostName=cid.HostName;
      	     _parent->IMessage(AMSClient::print(rv,"Producer_impl::Exiting-W- DST(s) are Validated, set run Status::Finished here"));
             ...
             stringstream s;
             s<<"Producer_impl::Exiting-W- not all events ("<<range<<") processed: ";
      	     _parent->IMessage(AMSClient::print(rv,s.str().c_str()));
      	     vector<DPS::Producer::DST_var> dstvs;
          	 for(DSTLI li=_dst.begin();li!=_dst.end();++li){
          	 	DPS::Producer::DST &dst = li->second;
          	 	if(dst.Run==rv->Run && dst.Status==DPS::Producer::Validated) {
          	 		dst.Status=DPS::Producer::Failure;
              		DPS::Producer::DST_var dstv=new DPS::Producer::DST(dst);
              		dstvs.push_back(dstv);
          	 	}
          	 }
          	 for (unsigned i=0; i<dstvs.size(); i++)
          		 PropagateDST(dstvs[i],DPS::Client::Update);
     	     rv->cinfo.EventsProcessed=0;
      	     rv->cinfo.LastEventProcessed=0;
      	     PropagateRun(rv,DPS::Client::Update);
      	 }
       }
   }
   ...
   if(li!=_rl.end()) {
     if((*li)->Status == DPS::Producer::Processing){
       ...
       if(rv->History != DPS::Producer::Failed){
         ...
         if(acv->id.Status!=DPS::Client::SInExit)
           rv->CounterFail++;
         if(LastTry(rv))
           rv->History=DPS::Producer::Failed;
       }
       else
         rv->Status=DPS::Producer::Failed;
       rv->cuid=0;
       //rv->pid=0;
       _parent->EMessage(AMSClient::print(rv,"Producer_impl::Exiting-I- run Failed "));
       Server_impl* _pser=dynamic_cast<Server_impl*>(getServer());
       _pser->MonInfo(AMSClient::print(rv,"Producer_impl::Exiting-I- run Failed: "),DPS::Client::Error);
       PropagateRun(rv,DPS::Client::Update);
     }
   }
   else
     _parent->EMessage(AMSClient::print(cid,"Producer_impl::Exiting-W- No Run Found"));
   return;
 }
}
_parent->EMessage(AMSClient::print(cid,"Producer_impl::Exiting-E- No Such Client"));
}
...
cout<<"Producer_impl::getSplitTDV-I- hostname="<<(const char *)cid.HostName<<
	  " uid="<<cid.uid<<" tdvname="<<(const char *)tdvname.Name<<
	  " pos="<<pos<<" size="<<tdvname.Size<<endl;
_UpdateACT(cid,DPS::Client::Active);
...
int length=0;
//!!!!!
// DPS::Producer::TDVbody_var qbody=new DPS::Producer::TDVbody();
// qbody->length(1000000*sizeof(uinteger));
// body=qbody._retn();
// return length;
//!!!!!
TIDI li=_findTDV(tdvname);
...
li->second->gettime(i,b,e);
 ...
 body=vbody._retn();
// cout <<"Producer_impl::getSplitTDV-I- exiting with len="<<length<<endl;
 return length;
}
}
...
cout<<"Producer_impl::sendTDV-I- uid="<<cid.uid<<" host="<<cid.HostName<<
		" TDV="<<(const char *)tdvname.Name<<" File="<<(const char *)tdvname.File<<endl;
...
#else
    DPS::Producer_ptr _pvar = reinterpret_cast<DPS::Producer_ptr>(cnarrow<DPS::Producer>::giop(_defaultorb,arf[i].IOR));
#endif
...
b=tdvname.Entry.Begin;
e=tdvname.Entry.End;
cout<<"Producer_impl::sendTDVUpdate-I- uid "<<cid.uid<<" host "<<cid.HostName<<
		" TDV "<<tdvname.Name<<" File "<<tdvname.File<<" second "<<li->second->getname()<<endl;
li->second->SetTime(i,b,e);
try{
//li->second->updatedb();
}
...
void Producer_impl::getRunEvInfo(const DPS::Client::CID &cid, DPS::Producer::RunEvInfo_out ro,DPS::Producer::DSTInfo_out dso)throw (CORBA::SystemException){
 ...
 pid.Type=getType();
 DPS::Client::ARS_var arf;
 int length=getARS(pid,arf,DPS::Client::LessThan,_parent->getcid().uid);
 for(unsigned int i=0;i<length;i++){
  try{
#ifndef NARROW_ONCE
    CORBA::Object_var obj=_defaultorb->string_to_object(arf[i].IOR);
...
//
  ...
  cout << " Producer_impl::getRunEvInfo-I- Threads requested "<< threads<<endl;
  if(threads<0 || threads>=244)threads=1;

	const char *sHostName = (const char *)cid.HostName;
	for(ACLI j=_acl.begin();j!=_acl.end();++j) {
		if ((*j)->id.uid == cid.uid) {
			if (strcmp(sHostName,(*j)->id.HostName)) {
				cout<<"  Producer_impl::getRunEvInfo-I- map "<<sHostName <<" to "<<(*j)->id.HostName<<endl;
				sHostName = (*j)->id.HostName;
			}
			break;
		}
	}
	for(AMSServerI * curp=getServer(); curp; curp=(curp->down())?curp->down():curp->next()) {
		if(curp->getType()==cid.Type) {
			for(AHLI i=curp->getahl().begin();i!=curp->getahl().end();++i) {
				if(!strcmp((const char *)(*i)->HostName,sHostName)) {
					cout << "  Producer_impl::getRunEvInfo-I- Threads found "<<(*i)->ClientsAllowed<<" "<<(*i)->ClientsRunning<<endl;
					// calculate processes on a give host once more
					int crun=0;
					for(ACLI j=_acl.begin();j!=_acl.end();++j) {
						if(!strcmp((const char *)(*j)->id.HostName,sHostName))
							if( (*j)->id.uid!=cid.uid) crun+=(*j)->id.threads;
					}
					cout <<" Producer_impl::getRunEvInfo-I- found "<<crun<< " on "<<(const char *)cid.HostName<<endl;
...
*/
     Server_impl* _pser=dynamic_cast<Server_impl*>(getServer()); 
     NHLI li=find_if(_pser->getnhl().begin(),_pser->getnhl().end(),NHL_find(sHostName));
     int maxn=12;
     if(li!=_pser->getnhl().end()){
         maxn=(*li)->CPUNumber;
     }
//       if(threads>maxn && strstr((const char *)cid.HostName,lxplus5))threads=maxn;        
     if(threads>maxn)threads=maxn;        
      ...
      cout << " Producer_impl::getRunEvInfo-I- Threads allowed "<<threads<<" "<<cid.uid<<" "<<(const char *)cid.HostName<<endl;


     for(ACLI j=_acl.begin();j!=_acl.end();++j){
     if((*j)->id.uid==cid.uid){
       if((*j)->StatusType == DPS::Client::OneRunOnly){
          cout <<" Producer_impl::getRunEvInfo-I- onerunonly client detected "<<endl;
...
if(dv->DieHard ==0){
 ...
 rv->SubmitTime=tt; 
  if(_parent->Debug()){
   _parent->IMessage(AMSClient::print (cid,"Producer_impl::getRunEvInfo-I- New Run Asked by"));
   _parent->IMessage(AMSClient::print(rv,"Producer_impl::getRunEvInfo-I- RunEvInfo "));
  }
  PropagateRun(rv, DPS::Client::Update);
  uinteger rndm[2]={0,0};
...
void Producer_impl::sendRunEvInfo(const  DPS::Producer::RunEvInfo & ne, DPS::Client::RecordChange rc)throw (CORBA::SystemException){
 ...
 //cout<<"Producer_impl::sendRunEvInfo-I rc="<<AMSClient::RC2string(rc)<<" uid="<<ne.uid<<" status="<<ne.Status<<endl;
 ...
 case DPS::Client::Update:
  if(li==_rl.end())_parent->EMessage(AMSClient::print(ne,"RunEv not found for editing"));
    ...
    if ((*li)->Status == DPS::Producer::Finished && ne.Status == DPS::Producer::Processing) {
	  cout<<"Producer_impl::sendRunEvInfo-E- uid="<<(*li)->uid<<", already finished"<<endl;
	  return;
    }
 ...
 case DPS::Client::Delete:
  if(li==_rl.end())
	_parent->EMessage(AMSClient::print(ne,"Producer_impl::sendRunEvInfo-E- runEv not found for deleting"));
  else {
	_parent->IMessage(AMSClient::print(ne,"Producer_impl::sendRunEvInfo-I- runEv deleted"));
	_rl.erase(li);
  }
  break;
 case DPS::Client::Create:
  if(li != _rl.end()){
     _parent->EMessage(AMSClient::print(ne,"Producer_impl::sendRunEvInfo-E- run already exists"));
  }
  else {
    _RunID++;
    ...
    _parent->IMessage(AMSClient::print(ne,"Producer_impl::sendRunEvInfo-I- runEv created"));
    _rl.push_back(rv);
  }
  break;
}
   ...
   for (int i=0;i<((*li)->ars).length();i++){
    try{
      ...
      DPS::DBServer_ptr dvar = reinterpret_cast<DPS::DBServer_ptr>(cnarrow<DPS::DBServer>::giop(_defaultorb,((*li)->ars)[i].IOR));
       ...
       // also remove remained dsts
       if (rc == DPS::Client::Delete) {
    	   int runs = 0;
    	   for (RLI rli=_rl.begin(); rli!=_rl.end(); ++rli) {
    		   DPS::Producer::RunEvInfo_var rei = *rli;
    		   if (rei->Run==ne.Run) {
    			   runs++;
    			   break;
    		   }
    	   ...
    	   if (!runs) {
			   vector<DPS::Producer::DST_var> dsts;
			   for(DSTLI dli=_dst.begin(); dli!=_dst.end(); ++dli){
				   DPS::Producer::DST_var dstv = dli->second;
				   if (dstv->Run == ne.Run)
					   dsts.push_back(dstv);
			   }
			   for (unsigned i=0; i<dsts.size(); i++) {
				   PropagateDST(dsts[i],rc);
				   _parent->IMessage(AMSClient::print(dsts[i],"Producer_impl::sendRunEvInfo-W- removed "));
			   }
    	   }
       }
    }
...
void Producer_impl::sendCurrentInfo(const DPS::Client::CID & cid, const  DPS::Producer::CurrentInfo &ci, int propagate)throw (CORBA::SystemException){
	...
	std::stringstream s;
	s<<"Producer_impl::sendCurrentInfo-I- uid="<<cid.uid<<" run="<<ci.Run<<" status="<<AMSClient::RS2string(ci.Status)<<
        " exitStatus="<<AMSClient::CSE2string(cid.Status)<<" threads="<<cid.threads;
	if (safe_strlen(ci.HostName,48)>=0) s<<" host="<<ci.HostName;
	cout<<s.str()<<endl;
...
void Producer_impl::sendDSTEnd(const DPS::Client::CID & ci, const  DPS::Producer::DST & ne, DPS::Client::RecordChange rc)throw (CORBA::SystemException){
  cout<<"Producer_impl::sendDSTEnd-I- rc="<<AMSClient::RC2string(rc)<<" ci.uid="<<ci.uid<<" run="<<ne.Run<<
	  " status="<<AMSClient::DSTS2string(ne.Status)<<" firstEv="<<ne.FirstEvent<<" lastEv="<<ne.LastEvent<<
	  " type="<<AMSClient::DSTT2string(ne.Type)<<" name="<<(const char *)ne.Name<<endl;
_UpdateACT(ci,DPS::Client::Active,false);
//!!!!!_UpdateACT(ci,DPS::Client::Active);
if(_parent->Debug() && ne.Status!=DPS::Producer::InProgress){
//!!!!!!!!!!  _parent->IMessage(AMSClient::print(ci,"senddstinfo get from "),false);
  _parent->IMessage(AMSClient::print(ci,"Producer_impl::sendDSTEnd-I- senddstinfo get from "));
  _parent->IMessage(AMSClient::print(ne,"Producer_impl::sendDSTEnd-I- senddstinfo DST "));
}
...
if (ne.Type ==DPS::Producer::EventTag){
  if(!strcmp((const char *)(li->second)->Name,(const char *)ne.Name)){
     ...
     case DPS::Producer::InProgress:
       ...
       return;
     default:
    _parent->EMessage(AMSClient::print(vne,"(0)Update:DST Already Exists "));
     (li->second)=vne;
      if(ci.Type!=DPS::Client::Server)PropagateDST(ne,DPS::Client::Update,DPS::Client::AnyButSelf,_parent->getcid().uid);
     return;
     }
	...
	//         cout <<" exiting Producer_impl::sendDSTEnd update"<<endl;
		  return;
		default:
		  _parent->EMessage(AMSClient::print(vne,"(1)Update:DST Already Exists "));
		  (li->second)=vne;
		  if(ci.Type!=DPS::Client::Server)PropagateDST(ne,DPS::Client::Update,DPS::Client::AnyButSelf,_parent->getcid().uid);
		  return;
	   }
 ...
 for(DSTLI li=b.first;li!=b.second;++li){
  if(!strcmp((const char *)(li->second)->Name,(const char *)ne.Name)) {
     _dst.erase(li);
     if(ci.Type!=DPS::Client::Server)
       PropagateDST(vne,rc,DPS::Client::AnyButSelf,_parent->getcid().uid);
     return;
  }
 }
...
cid.Type=getType();
DPS::Client::ARS_var arf;
int length=getARS(cid,arf,DPS::Client::LessThan,_parent->getcid().uid);
  time_t tt;
  time(&tt);
 for(unsigned int i=0;i<length;i++){
  try{
...
integer Server_impl::Kill( DPS::Client::ActiveClient & ac, int signal, bool self){
  //start  killer here
   ...
   AString submit;
    ...
    submit+=" ";
    string lf="";
     lf+=(const char*)((*cli)->LogPath);
     lf+="Killer.";
     lf+=tmp;
     lf+=".log ";
     if(!(*cli)->LogInTheEnd) submit+=lf.c_str();
	 AString submit_2 = submit;
	 string script = string((const char*)(*cli)->WholeScriptPath);
     submit += script.c_str();

     if(strstr(script.c_str(),"condor")){
       sprintf(tmp," %d.%d  ",ac.id.pid,ac.id.ppid);
       submit+=tmp;
       if(ac.id.coid==0) {
//
//        get condor pid 
//
       char t[1255];
       string pid=(const char *)submit;
       sprintf(t," \"ps -ef | grep %s | grep %d,%d \" ",(const char*)(ac.id.HostName),ac.id.pid,ac.id.ppid);
        pid+=t;
         char *lffc=new char[lf.size()+1];
         strcpy(lffc,lf.c_str());
         char *pch=strtok(lffc," ");
         string lff=pch;
         pch=strtok(NULL," ");
         lff+=" ";
         lff+=pch;
        ...
        if((*cli)->LogInTheEnd){
          pid+=lff.c_str();
          unlink(lff.c_str()+pos+1);
        }
        cout <<" CONDOR PID "<<pid<<endl;

        int i=system(pid.c_str());
        if(i)system(pid.c_str());
         ifstream fbin;
         fbin.open(lff.c_str()+pos+1);
         ...
         if(fbin){
           while(fbin.good() && !fbin.eof()){
             ...
             if(pch){
                unsigned int coid=atol(pch);
                if(ac.id.coid<coid)ac.id.coid=coid;
             }
           }
           if(ac.id.coid) PropagateAC(ac,DPS::Client::Update);
           cout <<"CONDOR PID found "<<ac.id.coid<<endl;
         }
         else
           cerr<<" CONDOR UNABLE TO OPEN "<<lff.c_str()+pos+1<<endl;
         unlink(lff.c_str()+pos+1);
       }
       submit+=" kill ";
     }

     if(strstr(script.c_str(),"condor") && ac.id.coid == 0) {
         sprintf(tmp," %d.%d  ",ac.id.pid,ac.id.ppid);
         submit = submit_2+"condor_rm"+tmp;
         goto adlog;
     }
     submit+=" -s ";
...
return true;	//!!!!!-debug-Server_impl::Lock-the source of memory leaks
time_t tt;
time(&tt);
switch (optype){
...
bool Server_impl::PingServer(const DPS::Client::ActiveClient & ac){
    ...
    cid.Interface= (const char *)" ";
    DPS::Client::ARS_var arf;
    int length=getARS(cid,arf,DPS::Client::Self,ac.id.uid);
  for(unsigned int i=0;i<length;i++){
  try{
#ifndef NARROW_ONCE
    CORBA::Object_var obj=_defaultorb->string_to_object(arf[i].IOR);
...
int Client_impl::getARS(const DPS::Client::CID & cid, DPS::Client::ARS_out arf, DPS::Client::AccessType type, uinteger id, int selffirst)
	throw (CORBA::SystemException){
...
CORBA::Boolean Client_impl::sendId(DPS::Client::CID& cid, float Mips,uinteger timeout) throw (CORBA::SystemException){
	...
	int lenInterf = safe_strlen(cid.Interface,32);
	if(lenHost<=0 || lenInterf<0) {
	   	cerr<<"Client_impl::sendId-E- wrong cid "<<cid.uid<<" "<<lenHost<<" "<<lenInterf<<endl;
	    return false;
	}
	cout<<"Client_impl::sendId-I- uid="<<cid.uid<<" threads="<<cid.threads<<
	   	" host="<<cid.HostName<<" interf="<<cid.Interface<<endl;
...
for(ACLI li=_acl.begin();li!=_acl.end();++li){
 ...
 if((*li)->Status!=DPS::Client::Killed && (*li)->LastUpdate+(*li)->TimeOut<tt){
   if(PingClient(*li)){
    ...
    DPS::Client::ActiveClient_var acv=*li;
    acv->Status=DPS::Client::TimeOut;
    if(_parent->Debug())_parent->EMessage(AMSClient::print(acv,"Client_impl::CheckClients-W- Client TIMEOUT"));
    PropagateAC(acv,DPS::Client::Update);
 }
 }
}
...
for( ACLI li=_acl.begin();li!=_acl.end();++li){
 if (cid.uid==((*li)->id).uid){
        if(_parent->Debug())_parent->IMessage(AMSClient::print((*li),"Client_impl::Exiting-I-Deleting Client "));
   DPS::Client::ActiveClient_var acv=*li;
   PropagateAC(acv,DPS::Client::Delete);
   return;
...
#else
        DPS::Monitor_ptr _mvar = reinterpret_cast<DPS::Monitor_ptr>(cnarrow<DPS::Monitor>::giop(_defaultorb,((*li)->ars)[0].IOR));
#endif
...
bool Client_impl::PingClient(const DPS::Client::ActiveClient & ac){
       if(ac.id.Type==DPS::Client::Monitor ){
        ...
        DPS::Monitor_var _mvar=DPS::Monitor::_narrow(obj);
#else
        DPS::DBServer_ptr _mvar = reinterpret_cast<DPS::DBServer_ptr>(cnarrow<DPS::DBServer>::giop(_defaultorb,ac.ars[0].IOR));
#endif
        if(!CORBA::is_nil(_mvar)){
         try{
          _mvar->ping();
    ...
    try{
      ...
      if(length){
        ...
        for(unsigned int i=0;i<length;i++){
          ...
          int lenHost = safe_strlen((*vre).HostName,48);
          int lenInterf = safe_strlen((*vre).Interface,32);
          if (lenHost<=0 || lenInterf<0)
              cerr<<"  AMSServerI::getHostInfoSDB-E- **getahs has suspicious record "<< lenHost <<" "<<lenInterf<<endl;
          else
        	  _ahl.push_back(vre);
        }
      }
      else{
       _parent->EMessage("  AMSServerI::getHostInfoSDB-E- UpdateHostTable-UnableToUpdate");
        return 0;
      }
       DPS::Client::ActiveHost *pre;
       ret=dvar->getFreeHost(tcid,pre);
       if (ret > 0)
    	   cout <<"  AMSServerI::getHostInfoSDB-I- get free host "<<AMSClient::print(*pre," ");
       prv=pre;
       return ret;
      }
      else
      ...
      cerr<<"  AMSServerI::getHostInfoSDB-E- oops corba error during getfreehost" <<endl;
      return 0;
     }
    }
    if(!retry)return 0;
...
void AMSServerI::HostClientFailed(DPS::Client::ActiveHost_var &ahlv){
     ...
     time_t tt;
     time(&tt);
     cout <<" HClF ::- Info: - "<<(const char *)ahlv->HostName<<" "<<ahlv->ClientsProcessed<<" "<<ahlv->ClientsFailed<<" "<<tt-ahlv->LastFailed<<endl;
     if (ahlv->Status!=DPS::Client::NoResponse &&
    		 (ahlv)->ClientsProcessed>3 &&
			 (ahlv)->ClientsFailed>((ahlv)->ClientsProcessed+1)/2 &&
			 tt-ahlv->LastFailed<86400) {
...
cout<<"AMSServerI::ReWriteTables-I- "<<endl;
...
cout<<"Producer_impl::ReReadTables-I"<<endl;
...
#else
    DPS::Producer_ptr _pvar = reinterpret_cast<DPS::Producer_ptr>(cnarrow<DPS::Producer>::giop(_defaultorb,(_refmap.begin())->second));
#endif
...
cout<<"Producer_impl::ReWriteTables"<<endl;
