//  $Id: timeid.C,v 1.179 2022/11/14 11:56:36 aegorov2 Exp $
// 
...
// paranoid check
uinteger crc=_CalcCRC();
if(crc!=_CRC){
 cerr<<"AMSTimeID::write-E-CRCError "<<getname()<<" "<<_CRC<<" "<<crc<<endl;
 return false;
}

  unsigned int ll=strlen(dirc)>32767?strlen(dirc)+1:32768;
  char * env=getenv(getname());
  if(env) cout<<"AMSTimeID::write-I- "<<getname()<<"="<<env<<", ll="<<ll<<endl;
  ...
  char dir[ll];
        ...
        if(env && strlen(env)&& *(env+strlen(env)-1)==0134){
          strcpy(dir,env);
          dir[strlen(env)-1]='/';
        }      
 ...
 unsigned int ll=strlen(dirc)>32767?strlen(dirc)+1:32768;
  ...
  strcpy(dir,dirc);
	...
	cout <<"AMSTimeID::read-W-Default value for TDV "<<getname()<<" will be used."<<endl;
  	...
  	cout <<"AMSTimeID::read-I- run="<<run<<" name="<<getname()<<" file="<<fnam<<endl;
	  ...
	  integer ns=_Nbytes/sizeof(pdata[0])+3;
	    ...
	    int num=url_fread((char*)(&ns),sizeof(ns),1,ffbin);
	      ...
	      if(fbin.good()){
              ...
              if(strstr(dir,"/cvmfs") && _Type!=Server){
	...
	if(
	  ...
	  if(ffbin){
		...
		fsdir+=namelistsubdir[is]->d_name;
          char *env=getenv(namelistsubdir[is]->d_name);
          if(env && strlen(env)&& *(env+strlen(env)-1)=='/'){
	        fsdir=env;  //! VC 20171122
   	        fsdir+=namelistsubdir[is]->d_name;
            cout<<"AMSTimeId::_fillDB-I-EnvFound "<<env<<" "<<fsdir<<endl;
          }
          else{
            //cout <<"AMSTimeId::_fillDB-I-env "<<(namelistsubdir[is]->d_name)<<endl;
          } 
		fsdir+="/";     
#if defined(__LINUXNEW__) || defined(__LINUXGNU__)
		dirent64 ** namelist;
	  ...
	  void AMSTimeID::_fillDBServer(const char *dir, int reenter, bool force){
		...
		setupdateablemapdir(_map_dir.c_str());	//!!!!!NOUPDATEABLEMAPDIRS added
	    fdir+=getname();
	    fdir+="/";
	    fmap+=".";
	    fmap+=getname();
...
again:
	    mtime=_stat_adv((const char*)fdir);
	    if((_updateable!=1 || (!stat64((const char *)fmap,&statbuf_map)&&
	    		mtime < statbuf_map.st_mtime)) && !force){	//!!!!!NOUPDATEABLEMAPDIRS added
usemap:
	      fbin.clear();
	      fbin.close();
	      fbin.open(fmap,ios::in);
