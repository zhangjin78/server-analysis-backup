// $Id: TrTrackSelection.C,v 1.9 2022/04/06 10:18:59 qyan Exp $
...
AMSPoint TrTrackSelection::GetMinDist(TrTrackR *track, int layerp, int nhmin)//have to fix for V6
{
  AMSPoint mdist;
...
int TrTrackSelection::FitidToAlgo(int fitid)
{
  ...
  else if(fitm == TrTrackR::kAlcaraz)     algo = 2;
  else if(fitm == TrTrackR::kChikanianF)  algo = 3;
  else if(fitm == TrTrackR::kChikanian)   algo = 4;
  else if(fitm == TrTrackR::kGEANE_DAF)   algo = 5;
  else if(fitm == TrTrackR::kGEANE_Kalman)algo = 6;
  else if(fitm == TrTrackR::kGBLFITTERAMS)algo = 7;
  else if(fitm == TrTrackR::kGBLFITTER)   algo = 8;

  if((fitid&TrTrackR::kMultScat)==0)      algo+=10;//No MS
  if((fitid&TrTrackR::kEnergyLoss)==0 && (fitm==TrTrackR::kGEANE_DAF||fitm==TrTrackR::kGEANE_Kalman||fitm==TrTrackR::kChikanianF))algo+=30;//No Eloss
...
int  TrTrackSelection::GetPatternForGoodHelium(TrTrackR* track, float beta) {
  ...
  // try to fit
  int id = track->iTrTrackPar(1,pattern,2); //have to fix for V6, if want to without MC, please use algo=11
  if (id<0) return -4;
  return pattern;
}
