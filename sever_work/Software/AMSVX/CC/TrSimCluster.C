...
double TrSimCluster::GetEta(){
  ...
  if      (cindex==0)               eta = (fabs(center+right)>0) ? right/(center+right) : 0; // nleft = 0
  else if ((cindex-GetWidth())==0)  eta = (fabs(center+left)>0)  ? center/(left+center) : 0; // nright = 0
  else if (right>left)              eta = (fabs(center+right)>0) ? right/(center+right) : 0;
  else                              eta = (fabs(center+left)>0)  ? center/(left+center) : 0;
  return eta;
}
...
void TrSimCluster::AddCluster(TrSimCluster& cluster, int nstrips, bool cycl) {  
  ...
  // use the final clusters size to find the best address shift for cyclicity 
  if (cycl) {
    int cluster_length = max(this_max,that_max)-min(this_min,that_min);
    int cluster_length_shifted1 = max(this_max,that_max+nstrips)-min(this_min,that_min+nstrips);
    int cluster_length_shifted2 = max(this_max+nstrips,that_max)-min(this_min+nstrips,that_min);
    if ( (cluster_length_shifted1<cluster_length)&&(cluster_length_shifted1<cluster_length_shifted2) ) {
      that_min += nstrips;
      that_max += nstrips;
    }
    else if ( (cluster_length_shifted2<cluster_length)&&(cluster_length_shifted2<cluster_length_shifted1) ) {
      this_min += nstrips;
      this_max += nstrips;
    }
  }
...
void TrSimCluster::GaussianizeFraction(int iside, int hcharge, double fraction,float IP) {
  ...
  if ((LL-2)>=0) {
    int prn=0;
    double prsignal=GetSignal(LL-1);
    for (int ii=LL-2;ii>=0;ii--){
      double ss = (GetSignal(ii)<1e-15) ? 0 : GetSignal(ii)*pow(fra3,kk);
      if(ss>prsignal)prn++;//increase 2times
      if(prn>=2||kk>=15)break;//bug fixed by QY
      prsignal=ss;
  ...
  if ((RR+2)<GetWidth()) {
    int prn=0;
    double prsignal=GetSignal(RR+1);
