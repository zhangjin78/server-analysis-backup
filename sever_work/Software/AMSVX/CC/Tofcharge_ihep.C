//  $Id: Tofcharge_ihep.C,v 1.27 2019/05/31 17:25:20 qyan Exp $
...
float TofChargePar::GetQ(int pmtype){
  ...
  if     (pmtype==0)return QLD>0?sqrt(QLD):QLD;
    ...
    number QL; 
    TofPDFH::GetHead()->ChooseDA(Layer,Bar,6.,number(QLA),number(QLD),number(QLARaw),number(QLDRaw),QL);//Choose
    return QL>0?sqrt(QL):float(QL);
  }
  else {
      if(QLDRaw<0&&QLARaw<0)return -1;
      TofRecPar::IdCovert(Layer,Bar);
      double wd=TofRecH::GetHead()->GetWeightDA(TofRecPar::Idsoft,0,QLDRaw);
      double wa=TofRecH::GetHead()->GetWeightDA(TofRecPar::Idsoft,1,QLARaw);
...
float TofChargePar::GetProbZ(float Z){
   ...
   float probz=TofPDFH::GetHead()->GetProbZ(Layer,Bar,number(Z),number(QLA), number(QLD),number(QLARaw),number(QLDRaw),number(Beta));//float double
   return probz;   
}
...
int TofChargeHR::DefaultQOpt=(TofRecH::kThetaCor|TofRecH::kBirkCor|TofRecH::kReAttCor|TofRecH::kDAWeight|TofRecH::kBetaCor|TofRecH::kQ2Q);
...
///--Select Matched-TOF Layer
  TofPDFH::GetHead()->SelectM(pattern,cpar,fTrTrack,cparu);

  number MeanQ,RMSQ;
///--LikeLihood -Calculation
  if(upi){
    vector<TofLikelihoodPar> plike;
    TofPDFH::GetHead()->LikelihoodCal(cparu,plike); 
    like.insert(pair<int,vector<TofLikelihoodPar> >(pattern,plike));
    TofPDFH::GetHead()->GetLikelihoodQ(cparu,MeanQ,RMSQ,0);
    Q.insert(pair<int,float>(pattern,float(MeanQ)));
    RQ.insert(pair<int,float>(pattern,float(RMSQ)));
  }
  if(upf){
//---LikeLihood Q
    float plikeQ=TofPDFH::GetHead()->GetLikelihoodQ(cparu,MeanQ,RMSQ,1);
...
int  TofChargeHR::ReFit(float fbeta,int opt,float frig){
  ...
  int optq2=(opt&(TofRecH::kBetaCor|TofRecH::kRigidityCor|TofRecH::kBetaRigV1Cor|TofRecH::kBetaRigV2Cor));
//----ReFit by new fbeta/frig
   number QLA1,QLD1;
   for(unsigned int im=0;im<cpar.size();im++){
     number nbeta=(fbeta==0)?cpar.at(im).Beta:fbeta;
     number nrig=(frig==0&&(opt&TofRecH::kRigidityCor))?Rigidity:frig;
     TofRecPar::IdCovert(cpar.at(im).Layer,cpar.at(im).Bar);
     QLA1= TofRecH::GetHead()->GetQSignal(TofRecPar::Idsoft,1,optq2,cpar.at(im).QLARaw,0,1,nbeta,nrig);
     QLD1= TofRecH::GetHead()->GetQSignal(TofRecPar::Idsoft,0,optq2,cpar.at(im).QLDRaw,0,1,nbeta,nrig);
   ...
   TofPDFH::GetHead()->FillProbV(cpar);
...
//========================================================
TofPDFH *TofPDFH::Head=0;
#ifdef __PPC64
#ifndef __ROOTSHAREDLIBRARY__
#pragma  omp threadprivate (TofPDFH::Head)
#endif
#endif

TofPDFH* TofPDFH::GetHead(){
  if(!Head)Head = new TofPDFH();
  return Head;
}

TofPDFH *TofPDFH::pHead=0;
#ifdef __PPC64
#ifndef __ROOTSHAREDLIBRARY__
#pragma  omp threadprivate (TofPDFH::pHead)
#endif
#endif
//========================================================
int TofPDFH::ReBuild(BetaHR *betah,TofChargeHR &tofch){
   return TofPDFH::GetHead()->ReBuildI(betah,tofch); 
}
...
//========================================================
int TofPDFH::ReBuildI(BetaHR *betah,TofChargeHR &tofch){
...
void TofPDFH::GetLikelihoodF(Int_t & /*nPar*/, Double_t * /*grad*/ , Double_t &fval, Double_t *par, Int_t /*iflag */ ){
   ...
   number likelihood=0;
   for(unsigned int im=0;im<pHead->chargepar.size();im++){
       number probv=pHead->chargepar.at(im).GetProbZ(par[0]);
       likelihood+=log(probv);
   }
   fval=-likelihood;
}
