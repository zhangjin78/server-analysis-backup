...
bool TrLikeDB::LoadPdfVer0(char* dirname) { 
  TFile* file = TFile::Open(Form("%s/%s/TrLikeDBver0/PDFDBv2.root",dirname,AMSCommonsI::getversion()),"read");//the directory has been changed to v6.00
...
void TrLikeDB::AddPdf(TH1* pdf, int index) {
  ...
  if (ret.second==false) {
    std::map<int,TH1*>::iterator it = ret.first;
    delete it->second;
    it->second = pdf;
  }
}
