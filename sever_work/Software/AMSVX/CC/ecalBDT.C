...
{
   // Dead-cell / side-leakage correction code. Only BDTv7 trained in EnergyD is available!
   if (enableNewDeadCellTreatment && AMSEventR::Head()->Version() >= 950 && iBDTVERSION == 7 && EnergyFlag == 0) {
     if (TMVAClassifier == 0)
       return fPreComputedBDTv7_EnergyD;
     if (TMVAClassifier == 1)
       return fPreComputedBDTv7_EnergyD_Smoothed;
     static bool sShowedMessage = false;
     if (!sShowedMessage) {
       sShowedMessage = true;
       std::cout << "EcalShowerR::GetEcalBDT-I-Unsupported TMVAClassifier '" << TMVAClassifier << "'. Either use 0 or 1." << std::endl;
     }
     return -999.9f;
   } else if (enableNewDeadCellTreatment && AMSEventR::Head()->Version() >= 950) {
      static bool sShowedMessage = false;
      if (!sShowedMessage) {
        sShowedMessage = true;
        std::cout << "EcalShowerR::GetEcalBDT-I-enableNewDeadCellTreatment is activated but you asked for an UNSUPPORTED BDT variant. Only BDTv7 / BDTv7_Smoothed trained in EnergyD bins is corrected - please switch to this estimator!" << std::endl;
      }
   }
   ...
   if (InitReader)     //if not already Init.....
   {
...
#else
      sprintf(WeightsDir,"%sbdt_root6/", AMSDATADIR.amsdatadir);//the directory has been changed to v6.00
#endif
      ...
      if ( iBDTVERSION == 3 )
	{
	  ...
	  ecalpisareader->BookMVA("BDTG_LAYERS", Form("%sECAL_PISA_BDT_412_v3.weights.xml", WeightsDir));
      ...
      if ( iBDTVERSION == 4 )
	{
	  ...
	  ecalpisareader->AddVariable("LayerS3FracNorm16",    &pisanormvar[ivar++]);
	  ecalpisareader->AddVariable("LayerS3FracNorm17",    &pisanormvar[ivar++]);
	  ecalpisareader->BookMVA("BDTG_LAYERS", Form("%sECAL_PISA_BDT_412_v4.weights.xml", WeightsDir));
      ...
      if ( iBDTVERSION >= 5 && TMVAClassifier == 0 && EnergyFlag==0 )
	{
	  ...
	  //
	  ecalpisareader_ODD->BookMVA("BDTG_LAYERS_ODD", Form("%sECAL_PISA_BDT_%d_v%dfinal_ODD.weights.xml", WeightsDir,ECALBDT_TMVAVER,iBDTVERSION));
	  ecalpisareader_EVEN->BookMVA("BDTG_LAYERS_EVEN", Form("%sECAL_PISA_BDT_%d_v%dfinal_EVEN.weights.xml", WeightsDir,ECALBDT_TMVAVER,iBDTVERSION));
	  if ( iBDTVERSION == 5 ) 
	    {
      ...
      if ( iBDTVERSION >= 5 && TMVAClassifier == 1 && EnergyFlag==0 )
	{
	  ...
	  //	  
	  ecalpisareaderS_ODD->BookMVA("BDTS_LAYERS_ODD", Form("%sECAL_PISA_BDTS_%d_v%dfinal_ODD.weights.xml", WeightsDir,ECALBDT_TMVAVER,iBDTVERSION));
	  ecalpisareaderS_EVEN->BookMVA("BDTS_LAYERS_EVEN", Form("%sECAL_PISA_BDTS_%d_v%dfinal_EVEN.weights.xml", WeightsDir,ECALBDT_TMVAVER,iBDTVERSION));
	  if ( iBDTVERSION == 5 ) 
	    {
     ...
     if ( iBDTVERSION >= 5 && TMVAClassifier == 0 && EnergyFlag!=0 )
	{
	  ...
	  //
	  ecalpisareader_E_ODD->BookMVA("BDTG_LAYERS_ODD", Form("%sECAL_PISA_BDT_%d_v%dfinal_E_ODD.weights.xml", WeightsDir,ECALBDT_TMVAVER,iBDTVERSION));
	  ecalpisareader_E_EVEN->BookMVA("BDTG_LAYERS_EVEN", Form("%sECAL_PISA_BDT_%d_v%dfinal_E_EVEN.weights.xml", WeightsDir,ECALBDT_TMVAVER,iBDTVERSION));
	  if ( iBDTVERSION == 5 ) 
	    {
      ...
      if ( iBDTVERSION >= 5 && TMVAClassifier == 1 && EnergyFlag!=0 )
	{
	  ...
	  //
	  ecalpisareaderS_E_ODD->BookMVA("BDTS_LAYERS_ODD", Form("%sECAL_PISA_BDTS_%d_v%dfinal_E_ODD.weights.xml", WeightsDir,ECALBDT_TMVAVER,iBDTVERSION));
	  ecalpisareaderS_E_EVEN->BookMVA("BDTS_LAYERS_EVEN", Form("%sECAL_PISA_BDTS_%d_v%dfinal_E_EVEN.weights.xml", WeightsDir,ECALBDT_TMVAVER,iBDTVERSION));
	  if ( iBDTVERSION == 5 ) 
	    {
