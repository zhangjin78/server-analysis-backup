...
void AMSJob::_retkinitjob(){
  ...
  AMSgObj::BookTimer.book("ZquTrackBuild");
  TrFit::InitKalman();
...
void AMSJob::_sitkinitjob(){
  ...
  if (TRMCFFKEY.SetActiveSiliconCuts==2) {
    char chp[22]="";
    integer itrtyp;
    geant mass;
    geant charge;
    ...
    GFPART(GCKINE.ikine,chp,itrtyp,mass,charge,tlife,ub,nwb);
    charge=fabs(charge);
    if (charge>2) TRMCFFKEY.MinDeltaRayRange = 73.;   
    else          TRMCFFKEY.MinDeltaRayRange = 4.2;
    cout << "AMSJob::sitkinitjob-I-TRMC Parameters 669 671 " << TRMCFFKEY.SetActiveSiliconCuts << " " << TRMCFFKEY.MinDeltaRayRange << endl;
  }
