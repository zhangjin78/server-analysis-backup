...
void VCon_root::eraseC(){
 if(!ev)  return ;
  ...
  if( strstr(contname,"TrTrack")) {
    ...
    for(unsigned int ii=0;ii<ev->NParticle(); ii++){ev->pParticle(ii)->setTrTrack(-1);}
    for(unsigned int ii=0;ii<ev->NBetaH();    ii++){ev->pBetaH(ii)->setTrTrack(-1);}
    for(unsigned int ii=0;ii<ev->NBeta();     ii++){ev->pBeta(ii)->setTrTrack(-1);}
    for(unsigned int ii=0;ii<ev->NRichRing(); ii++){ev->pRichRing(ii)->setTrTrack(-1);}
    for(unsigned int ii=0;ii<ev->NRichRingB();ii++){ev->pRichRingB(ii)->setTrTrack(-1);}
    for(unsigned int ii=0;ii<ev->NVertex();   ii++){ 
      for(int jj=0;jj<ev->pVertex(ii)->NTrTrack();jj++)ev->pVertex(ii)->setTrTrack(jj,-1);
    }
    ev->TrTrack().clear();
  }
  if( strstr(contname,"Vtx")) {
