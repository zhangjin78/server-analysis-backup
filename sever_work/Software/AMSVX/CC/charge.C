//  $Id: charge.C,v 1.119 2022/04/08 12:37:37 qyan Exp $
...
double AMSChargeSubD::probcalc(int fit, int nhit, double lkhd[]){
  ...
  vector<double> prob(MaxZTypes,0.0);
...
//------select candidates
  vector<AMSBetaH *> pbetahs;
  AMSBetaH *pbetah=(AMSBetaH*)AMSEvent::gethead()->getheadC("AMSBetaH",0, 2);//force sort
  while(pbetah){
    if(pbetah->gettrack()){pbetahs.push_back(pbetah);}///select with Tracker
    pbetah=pbetah->next();
  }
//------
  vector<AMSBeta *> pbetas;
  for(int patb=0; patb<npatb; patb++){
    AMSBeta *pbeta=(AMSBeta*)AMSEvent::gethead()->getheadC("AMSBeta",patb);
    while(pbeta){
      pbetas.push_back(pbeta);
      pbeta=pbeta->next();
    }
  }

//------BetaH first
  for(unsigned int ibh=0;ibh<pbetahs.size();ibh++){
    AMSBetaH *pbetah=pbetahs[ibh];
    AMSBeta *pbeta=0;
    for(unsigned int ib=0;ib<pbetas.size();ib++){
      if(pbetah->gettrack() && pbetas[ib]->getptrack()==pbetah->gettrack()){//share the same Track
        pbeta=pbetas[ib]; break;
      }
    }
    AMSCharge *pcharge = new AMSCharge(pbeta, pbetah);
    AMSEvent::gethead()->addnext(AMSID("AMSCharge",0),pcharge);
  }
  if(pbetahs.size()>=1)return 0;

//-----if BetaH failed, then try with Beta
  for(unsigned int ib=0;ib<pbetas.size();ib++){
    AMSBeta *pbeta=pbetas[ib];
    AMSCharge *pcharge = new AMSCharge(pbeta, 0);
    AMSEvent::gethead()->addnext(AMSID("AMSCharge",0),pcharge);
  }
...
int AMSCharge::BuildTOF() {
  // checks
  if (_pbeta == NULL && _pbetah==0) return 0;
  // whole TOF
...
int AMSCharge::BuildTracker() {
  // checks 
  if (_pbeta == NULL && _pbetah==0) return 0;
  if (_ptrtrack == NULL) return 0;
  if (_ptrtrack->getnhits() == 0) return 0; 
  // whole tracker
  if (((CHARGEFITFFKEY.RecEnable[kTracker]/trkAll)%10) != 0) {
    AMSChargeTracker *chargetrk = new AMSChargeTracker(_pbeta,_ptrtrack,_pbetah);
  ...
  if (((CHARGEFITFFKEY.RecEnable[kTracker]/trkInner)%10) != 0) {
    AMSChargeTrackerInner *chargetrkinner = new AMSChargeTrackerInner(_pbeta,_ptrtrack,_pbetah);
  ...
  if (((CHARGEFITFFKEY.RecEnable[kTracker]/trkUpper)%10) != 0) {
    AMSChargeTrackerUpper *chargetrkupper = new AMSChargeTrackerUpper(_pbeta,_ptrtrack,_pbetah);
  ...
  if (((CHARGEFITFFKEY.RecEnable[kTracker]/trkLower)%10) != 0) {
    AMSChargeTrackerLower *chargetrklower = new AMSChargeTrackerLower(_pbeta,_ptrtrack,_pbetah);
...
again:
  ...
  while(p){
#ifndef _PGTRACK_
    if (_ptrtrack->TRDMatch(p)) {
#else
    if (TkTRDMatch(_ptrtrack,p)) {
#endif
      ptrd=p;
      break;
    }
  ...
  if (ptrd == NULL) {
    if(notrefitted){
      notrefitted=false;
      if(TRDRECFFKEY.ResolveAmb && AMSTRDTrack::ResolveAmb(_ptrtrack))goto again;
      else return 0;
    }
    else return 0; 
  }
  ...
  //---Set TRD Match for BetaH
  if(ptrd!=NULL && _pbetah){
    _pbetah->settrdtrack(ptrd); //reset this trd to betah
  } 
  return 1; 
}
...
AMSChargeBase AMSCharge::getvotedcharge(const vector<TString> & detectors) {
  ...
  vector<int> Nused;
  Nused.assign(long(size),0);
  result._Indxz.assign(long(size),-1);
  result._Lkhdz.assign(long(size),0);
  result._Probz.assign(long(size),1);
...
AMSCharge::AMSCharge(AMSBeta *pbeta, AMSBetaH *pbetah) {
  // init 
  clear();
  _pbeta = pbeta;
  _pbetah = pbetah;
  if(_pbetah){_ptrtrack=_pbetah->gettrack(); _pbetah->setcharge(this);}
  else       {_ptrtrack=_pbeta->getptrack();}
...
int AMSChargeTRD::Fill(int refit) {
  // init
  float beta =(_pbetah)? _pbetah->GetBeta(): _pbeta->getbeta();
  _ptrd->ComputeCharge(AMSBeta::GetTRDBetaCorr(beta)); 
  ...
  _Probz.assign(long(nc),0);
  _Indxz.assign(long(nc),0);
...
int AMSChargeTracker::Fill(int refit) {
  ...
  for (int i=0; i<(int)likelielec.size(); i++) {
    if (likelielec[i].Z==1) {
      _Indxz.push_back(0);
      _Lkhdz.push_back(-likelielec[i].GetNormLogLike()); // convention
      _Probz.push_back(likelielec[i].GetNormProb());
    }
  }
...
int AMSChargeTracker::Fill(int refit) {
  ...
  double beta=(_pbetah)?_pbetah->GetBeta():  _pbeta->getbeta();
  ...
  int bstatus = !_pbeta->checkstatus(AMSDBc::AMBIG);
...
int AMSChargeTOF::Fill(int refit) {
  ...
  if (_pbetah) {
    ...
    _TruncatedMean =_pbetah->GetQ(npoints,rms,2,TofClusterHR::DefaultQOpt,pattern,1); // no beta correction
    // sort and fill root vectors
