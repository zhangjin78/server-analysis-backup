//  $Id: client.C,v 1.60 2018/05/15 12:14:44 aegorov2 Exp $
#include "client.h"
#include <stdio.h>
#include <unistd.h>
#include <iostream>
...
bool AMSClient::_getpidhost(uinteger uid, const char * iface){
 ...
 if(gethostname(name,len)){
   cout<<" AMSClient::_gethostname-E-UnableToGetHostName "<<endl;
   struct hostent *hent;
...
#else
_ost<<" LastUpdate "<<ctime(&lu)<<" Start "<<ctime(&st)<< "  Status " <<CS2string(a.Status)<<" Threads "<<a.id.threads<<ends;
#endif
...
_ost<<mes<< "COp Action "<<OPS2string(a.Action) << " ClientType "<<CT2string(a.Type)<<" ClientID "<<a.id<<" Time  "<<ctime(&ts)<<" TimeOut "<<a.TimeOut<<ends;
return _streambuffer;
}
...
_ost.clear();
_ost.seekp(0);
time_t submit=a.SubmitTime;
_ost<<mes<<" REI, ID "<<a.uid<<", Run "<<a.Run<<", DataMC "<<a.DataMC<<", FirstEvent "<<a.FirstEvent<<", LastEvent "<<a.LastEvent<<
	    ", EventsProcessed "<<a.cinfo.EventsProcessed<<", LastEventProcessed "<<a.cinfo.LastEventProcessed<<
		", Prio "<<a.Priority<<", Path "<<(a.FilePath?a.FilePath:" ")<< ", Status "<<RS2string(a.Status)<<", History "<<
		RS2string(a.History)<<", Failed "<<a.CounterFail<<", ClientID "<<a.cuid<<", SubmitTimeU "<<a.SubmitTime<<", SubmitTime "<<ctime(&submit);
print(a.cinfo,_ost);
_ost<<ends;
return _streambuffer;
}
...
_ost.clear();
_ost.seekp(0);
_ost<<mes<<", uid "<<a.uid<<", Host "<<a.HostName<<", Mode "<<RunMode2string(a.Mode)<<", UpdateFreq "<<a.UpdateFreq<<
		", DieHard "<<a.DieHard<<", DSType "<<DSTT2string(a.type)<<", NtupleOutput "<<(a.OutputDirPath?a.OutputDirPath:" ")<<
		", FreeMb "<<a.FreeSpace<<", TotalMb "<<a.TotalSpace<<ends;
return _streambuffer;
}
...
char * AMSClient::RC2string(DPS::Client::RecordChange a) {
switch (a){
