//  $Id: uzstat.C,v 1.43 2023/12/19 19:26:07 qyan Exp $
...
void AMSStat::book(const char *name, int freq,int mthr){
#ifdef __LVL3ONLY__
//  if(!strstr(name,"LVL3")){
...
void AMSStat::start(const char *name,number ut){
int thread=0;
#ifdef _OPENMP
#ifdef G4MULTITHREADED
...
number AMSStat::check(const char * name){
int thread=0;
#ifdef _OPENMP
#ifdef G4MULTITHREADED
int id=G4Threading::G4GetThreadId();
...
void AMSStat::print(const char *name){
int thread=0;
#ifdef _OPENMP
#ifdef G4MULTITHREADED
int id=G4Threading::G4GetThreadId();
...
number AMSStat::stop(const char *name, integer force){
int thread=0;
#ifdef _OPENMP
#ifdef G4MULTITHREADED
int id=G4Threading::G4GetThreadId();
...
number AMSStat::Get(const char *name){
int thread=0;
#ifdef _OPENMP
#ifdef G4MULTITHREADED
int id=G4Threading::G4GetThreadId();
...
#else
#if defined(_PGTRACK_) and not defined(JUQUEEN)
  return AMSgettime();
#else
...
#ifdef _OPENMP
#ifdef G4MULTITHREADED
//    count= G4MTRunManager::GetMasterRunManager()?G4MTRunManager::GetMasterRunManager()->GetNumberOfThreads():1;
//    count-=GCFLAG.IFINIT[15];
//    if(count<=0)count=1;
#else
...
void AMSStatErr::book(const char *name, char severity){
  if( add(*(Timer.add(new AMSStatErrNode(name,severity))))==-1)
    cerr<<" AMSStatErr-Book-E-Name "<<name<<" already exists"<<endl;
}

void AMSStatErr::print(const char *name, char *message, uinteger maxprint){
int thread=0;
#ifdef _OPENMP
#ifdef G4MULTITHREADED
int id=G4Threading::G4GetThreadId();
