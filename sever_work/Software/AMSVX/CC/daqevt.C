//  $Id: daqevt.C,v 1.281 2021/09/01 13:31:38 aegorov2 Exp $
#ifdef __CORBA__
#include "producer.h"
#endif
#include <stdio.h>
...
uint64 DAQEvent::_RunEv[maxthread]={};
...
#ifdef __PPC64
#pragma omp threadprivate (DAQEvent::_DAQEvent)
#endif

  DAQEvent* DAQEvent::gethead(){return _DAQEvent;}
 DAQEvent::DAQEvent():AMSlink(),_BufferOwner(0),_Checked(0),_Length(0),_Event(0),_Run(0),
           _RunType(0),_Offset(0),_Time(0),_usec(0),_pcur(0),_pData(0){
            if(AMSEvent::gethead() )_RunEv[AMSEvent::get_thread_num()]=AMSEvent::gethead()->getrunev();
             ...
             for (unsigned int i=0;i<sizeof(_JLength)/sizeof(_JLength[0]);i++)_JLength[i]=0;
           for (unsigned int i=0;i<sizeof(_JLengthU)/sizeof(_JLengthU[0]);i++)_JLengthU[i]=0;
             for (unsigned int i=0;i<sizeof(_SubLength)/sizeof(_SubLength[0]);i++)_SubLength[i]=0;
             for (unsigned int i=0;i<sizeof(_SubLengthU)/sizeof(_SubLengthU[0]);i++)_SubLengthU[i]=0;
...
void DAQEvent::setfiles(char *ifile, char *ofile){
  ...
  if(ofile){
    ...
    int beg=0;
    for(int k=0;k<strlen(ofile);k++){
       if(ofile[k]==' ')beg++;
       else break;
    }  
    strcpy(ofnam,ofile+beg);
  }
}
...
//#pragma omp critical (builddaq)
  ...
  while(fpl){
	...
	cerr<<" DAQEvent::buildDaq-E-ErrorsSet "<<" tdr  "<<lv3.err_tdr<<" rdr "<<lv3.err_rdr<<" udr "<< lv3.err_udr<<" edr "<<lv3.err_edr<<" sdr "<<lv3.err_sdr<< " id "<<fpl->getid()<<endl;
        ...
        if(fpl->is_tdr()){
          _SubLength[0]=abs(len);
        }
        else if(fpl->is_udr()){
          _SubLength[1]=abs(len);
        }
        else if(fpl->is_edr()){
          _SubLength[4]=abs(len);
        }
        else if(fpl->is_rdr()){
          _SubLength[3]=abs(len);
        }
        else if(fpl->is_sdr()){
          _SubLength[2]=abs(len);
        }
        else if(fpl->is_lvl1()){
          _SubLength[5]=abs(len);
        }
        else if(fpl->is_lvl3()){
          _SubLength[6]=abs(len);
        }
      }
...
integer DAQEvent::_EventOK(){
  ...
  wrong:
    ...
    static bool conversion_not_yet_tried=true;
#pragma omp threadprivate(conversion_not_yet_tried)
retry:
    if(!conversion_not_yet_tried)AMSDBc::convert2((char*)(_pData+preset+_cll(_pData+preset)),l2u,true);
	...
	if (err != Z_STREAM_END) {
	  inflateEnd(&stream);
	  fprintf(stderr,"R__unzip: error %d in inflate (zlib) %d\n",err,ierr);
      if(conversion_not_yet_tried){
          conversion_not_yet_tried=false;
          if(ierr<maxerr)goto retry;
      }
      else{
          ierr++;
          conversion_not_yet_tried=true;
      }
	...
	inflateEnd(&stream);
    if(!conversion_not_yet_tried)AMSDBc::convert2(tgt+preset*sizeof(_pData[0]),l2c/2,true);
...
integer DAQEvent::_HeaderOK(){
  ...
  for(_pcur=_pData+getpreset(_pData);_pcur < _pData+_Length;_pcur+=_cl(_pcur)){
	...
	_Time-=_OffsetT;
      ...
      if(_PRun==_Run && _PEvent && _Event<_PEvent  && AMSJob::gethead()->isRealData()){
...
#pragma omp critical (calibdata)
	...
	if(_GetBlType()==5 && AMSJob::gethead()->isRealData()){
	  cerr<<"DAQEvent::Headerok-E-EventSeqBroken for Run "<<_PRun<<" "<<_PEvent<<" "<<_Event<<endl;
	  _PRun=_Run;
	  _PEvent=_Event;
	  return 0;
...
integer DAQEvent::_DDGSBOK(){
  for(_pcur=_pData+getpreset(_pData);_pcur < _pData+_Length;_pcur+=_cl(_pcur)){
	...
	for(int16u *p=_pcur+_cll(_pcur)+1+_clll(_pcur);p<_pcur+_cl(_pcur)-2;p+=*p+1){
	  ...
	  bool room=(*(p+*p)>>11&0xf)==2;
#ifdef __AMSDEBUG__
	  int16u status=(*(p+*p))>>5;
	  cout <<"  JINJ Port "<<port <<" "<<_PortNamesJ[port]<<"  Length "<<*p<<" Status "<<status<<endl;
#endif
	  if (room)_JRoomError|=(1<<port);
	  _JLength[port]=*p;
	  _JLengthU[port]=room?*(p+2):*p;
	  switch(port){
	  case 0:
	  case 1:
	  case 3:
	  ...
	  case 23:
	    _SubLength[0]+=*p;
	    _SubLengthU[0]+=room?*(p+2):*p;
	    _SubCount[0]++;
	    break;
	  case 2:
	  case 8:
	    _SubLength[1]+=*p;
	    _SubLengthU[1]+=room?*(p+2):*p;
	    _SubCount[1]++;
	    break;
	  case 4:
	  case 5:
	  ...
	  case 21:
	    _SubLength[2]+=*p;
	    _SubLengthU[2]+=room?*(p+2):*p;
	    _SubCount[2]++;
	    break;
	  case 10:
	  case 11:
	    _SubLength[3]+=*p;
	    _SubLengthU[3]+=room?*(p+2):*p;
	    _SubCount[3]++;
	    break;
	  case 12:
	  case 13:
	    _SubLength[4]+=*p;
	    _SubLengthU[4]+=room?*(p+2):*p;
	    _SubCount[4]++;
	    break;
	  case 14:
	  case 15:
	    _SubLength[5]+=*p;
	    _SubLengthU[5]+=room?*(p+2):*p;
	    _SubCount[5]++;
	    break;
	  }
...
void DAQEvent::write(){
if(mpi.getMPIStatus()==1 && mpi.RunMode>2){
return;
}
...
DAQEvent::InitResult DAQEvent::init(){
  ...
  string castor2="/eos/ctapublic/archive/ams";
  if (getenv("TapeArPath")) 
    castor2=getenv("TapeArPath");
  const string sto="/afs/cern.ch/ams/local/bin/timeout";
  const string eosfs="/eos/ams";
  const string eosfs2="/eosams/";
 
 castor:
  bool errEosOrTape=false;
  if(fnam){
#ifdef __ALPHA__
    fbin.close();
#else
...
againcp:
	...
	ftxt.open(utmp);
	if(ftxt){
	  string castor=castor2;
	  string eos=eosfs;
	  string txt;
	  getline(ftxt,txt);
          if(strstr(fnam, castor.c_str()) || strstr(fnam, eos.c_str()) || txt.find(eosfs2)!=string::npos){
		      errEosOrTape=true;
              string path;
              string file;
	      if(txt.find("->")!=string::npos){
		path = txt.c_str()+txt.find("->")+2;
	        file = txt.c_str()+txt.rfind("/")+1;
              }
              else {
                path = fnam;
                file = path.c_str()+path.rfind("/")+1;
              }
...
again:
	    if(getenv("NtupleDir")){
	      ...
	      string cp=sto+" --signal 9 900 /usr/bin/xrdcp -OSsvcClass=amscdr -ODsvcClass=amscdr -f root://eosctapublic.cern.ch//";
		  if (getenv("TransferRawBy")) cp=getenv("TransferRawBy");
	      cp+=castor;
	      cp+=" ";
	      cp+=local;
		  ...
		  // copy eos
          int result=-1;
		  if (path.find(castor2.c_str()) == string::npos) {
	      	  string cpeos=sto+" --signal 9 600 /afs/cern.ch/project/eos/installation/ams/bin/eos.select cp";
		      if (getenv("EosTransferRawBy")) cpeos=getenv("EosTransferRawBy");
			  cpeos += " ";
		      cpeos += eos+" "+local+"/";
			  if(MISCFFKEY.ReadEOS){
				// first try read mounted eos
				fstream fsin(eos.c_str(),fstream::in);
				if (fsin.is_open()) {
					fsin.close();
					cout<<"DAQEvent::init-I-FoundEOS file "<<eos<<endl;
					setenv("TMPRawFile",eos.c_str(),1);
					strcpy(fnam,eos.c_str());
				    ftxt.close();
				    unlink(utmp);
		    		goto castor;
				}
			  ...
			  result = system(cpeos.c_str());
		      if(!result) goto okcp;
			  cerr<<"DAQEvent::init-E-Unableto "<<cpeos.c_str()<<", error: "<<result<<endl;
			  unlink(rmfile.c_str());
		  }

		  // copy castor
		  cout<<"DAQEvent::InitResult-I-Copying "<<cp<<endl;
		  result = system(cp.c_str());
//!!!!!
// Debug: test castor
// result = 1;
//!!!!!
	      if(result){
		cerr <<"DAQEvent::init-E-Unableto "<<cp.c_str()<<", error: "<<result<<endl;
...
//  try scp once more
//
  if(errEosOrTape && getenv("TransferRawByB") && strlen(getenv("TransferRawByB"))){
...
againscp:
		...
		setenv("LD_LIBRARY_PATH",getenv("TransferSharedLib"),1);
	      ...
	      if(i){
		cerr <<"DAQEvent::init-E-Unableto "<<cp.c_str()<<", exit code: "<<i<<endl;
		if(getenv("NtupleDir2") ){
		  char *nt2=getenv("NtupleDir2");
  ...
  again:
    ...
    */
    ...
    try{
      ...
      strcpy(cmd,"touch ");
      strcat(cmd,rootfile);
      if(system_bg::system(cmd))cerr<<"DAQEvent-E-ParserUnableTo "<<cmd<<endl;
      setRootDir(rootdir);
