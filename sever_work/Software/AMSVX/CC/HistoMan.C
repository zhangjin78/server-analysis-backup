/// $Id: HistoMan.C,v 1.76 2023/12/07 22:27:07 choutko Exp $ 
#include <math.h>
#include "HistoMan.h"
#include "TFile.h"
#include "TH3.h"
...
HistoMan::HistoMan():enabled(0), booked(false) { 
...
TH1* HistoMan::Get(const char * name){
  if(enabled==2){//slow
     return (TH1*) fhist.FindObject(name);
  }
  else {//fast
    std::map<std::string, TObject *>::iterator it=fhist2.find(std::string(name));
    if(it==fhist2.end())return 0;
    else {
      return (TH1 *)it->second;
    }
  }
}
void HistoMan::Add(TH1* hist ){fhist.Add(hist); if(hist)fhist2[hist->GetName()]=hist;}
...
void HistoMan::BookHistos(int simmode){
  ...
  if (simmode & 1) {
    ...
    Add(new TH2D("TrSimResPrimx","; #theta_{XZ} (rad); #Delta x (#mum)",10,0,1.6,500,-100,100));
    Add(new TH2D("TrSimResPrimy","; #theta_{YZ} (rad); #Delta y (#mum)",10,0,1.6,500,-100,100)); 
    Add(new TH2D("TrSimEDepTot","; log_{10}(R/GV); E_{Dep}",40,-1,3,300,0,3));
   ...
   for(int ilay=0;ilay<4;ilay++){
    ...
    for(int it=0;it<3;it++){
      sprintf(histn,"fragnpar%d",at[it]);
      Add(TH2D_L(histn,histn,100,0.2,10000,100,   1, 400, 1,0));
      sprintf(histn,"fragen%dv",at[it]);
      Add(TH2D_L(histn,histn,100,0.2,10000,100,0.95,1.05, 1,0));
      sprintf(histn,"fragzmap%dm",at[it]);
      Add(TH2D_L(histn,histn,100,0.2,10000,84,    1,  85, 1,0));
      sprintf(histn,"fragamap%dm",at[it]);
      Add(TH2D_L(histn,histn,100,0.2,10000,100,   1,  101,1,0));
      sprintf(histn,"frageva%dvm",at[it]);
      Add(TH2D_L(histn,histn,100,0.2,10000,100,   0,  1.2,1,0));
    }
    const int np=10;
    for(int ip=0;ip<np;ip++){
      sprintf(histn,"fragnpar%ds", ip);
      Add(new TH1D(histn,histn,1000,0,1000));
      sprintf(histn,"fragen%dvs",  ip);
      Add(new TH1D(histn,histn,1000,0.95,1.05));
      sprintf(histn,"fragzmap%dms",ip);
      Add(new TH1D(histn,histn,84, 1,85));
      sprintf(histn,"fragamap%dms",ip);
      Add(new TH1D(histn,histn,209,1,210));
      sprintf(histn,"frageva%dvms",ip);
      Add(new TH1D(histn,histn,1000,0,1.2));
    }
