//  $Id: bcorr.C,v 1.19 2022/03/16 16:47:05 qyan Exp $
#include "root.h"
...
void MagnetVarp::MagnetTemperature::loadValues(unsigned int utime,bool ccebload, string dtmc){
...
#ifdef __ROOTSHAREDLIBRARY__
  if(!setup){
    static AMSSetupR setup0;
#pragma omp threadprivate (setup0)
    setup=&setup0;
    if(utime<setup->fHeader.FEventTime||utime>setup->fHeader.LEventTime){
      setup->fHeader.LEventTime=setup->fHeader.FEventTime=utime;
      setup->LoadExt();
    }
  }
#endif
  ...
  for(unsigned int k=0;k<sizeof(stemp)/sizeof(stemp[0]);k++){
    vector<float>value;
    int ret=setup->fSlowControl.GetData(sensort[k].c_str(),utime,0.5,value);
    if(!ret && value.size()){
      nsens|=(1<<k);
      stemp[k]=value[0];
    }
  ...
  if(ccebload){
    for(int k=0;k<4;k++){
      for(int j=0;j<4;j++){
	vector<float>value;
	int ret=setup->fSlowControl.GetData(sensorb[k][j].c_str(),utime,0.5,value);
	if(!ret && value.size()){
	  int l=k*4+j;
	  ncceb|=(1<<l);
	  cceb[k][j]=value[0];
...
float MagnetVarp::_RAW_btempcor(float temp ){
