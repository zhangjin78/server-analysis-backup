//  $Id: Tofrec02_ihep.C,v 1.97 2023/05/08 10:28:00 qyan Exp $
...
//========================================================
TofRecH* TofRecH::Head=0;
#ifdef __PPC64
#ifndef __ROOTSHAREDLIBRARY__
#pragma omp threadprivate(TofRecH::Head)
#endif
#endif
TofRecH* TofRecH::GetHead(){
  if(!Head)Head = new TofRecH();
  return Head;
}
...
int TofRecH::BuildTofClusterH(){
    TofRecH::GetHead()->BuildTofClusterHI();
    return 0;
}

//========================================================
int TofRecH::BuildBetaH(int verse){
    TofRecH::GetHead()->BuildBetaHI(verse);
    return 0;
}

//========================================================
int TofRecH::DoAdvancedTrackFit(){
    return TofRecH::GetHead()->DoAdvancedTrackFitI();
}

//========================================================
int TofRecH::DoPrimaryTrackTag(int direction){
   return TofRecH::GetHead()->DoPrimaryTrackTagI(direction);
}
...
//--already check get data
     TofSideRec(&tfraws[i],adca[is],nadcd[is],adcd[is],sdtm[is],sstatus[is],ltdcw[is],htdcw[is],shtdcw[is],realdata);
...
int TofRecH::TofSideRec(TofRawSideR *ptr,number &adca, integer &nadcd,number adcd[],number &sdtm,uinteger &sstatus,
                        vector<number>&ltdcw, vector<number>&htdcw, vector<number>&shtdcw,int urealdata,int unbiase){
//---
     integer i,j;
     integer ltok=0;
...
//---Put to calidate
      ...
      for(i=0;i<nlt;i++){//notTooYoung(>40ns befFT), notTooOld(<300 befFT)
         if(((ltdc[i]> TofRecPar::FTgate[0])&&(ltdc[i]<TofRecPar::FTgate[1]))||BuildOpt<0||unbiase>=1){
            ltdcw.push_back(ltdc[i]);
         }
       }
     for(i=0;i<nht;i++){//notTooYoung(>40ns befFT), notTooOld(<300 befFT)
         if(((htdc[i]> TofRecPar::FTgate[0])&& (htdc[i]<TofRecPar::FTgate[1]))||BuildOpt<0||unbiase>=1){
           htdcw.push_back(htdc[i]);
         }
       }
     for(i=0;i<nsht;i++){//notTooYoung(>40ns befFT), notTooOld(<300 befFT)
         if(((shtdc[i]> TofRecPar::FTgate[0])&& (shtdc[i]<TofRecPar::FTgate[1]))||BuildOpt<0||unbiase>=1){
...
//----Set Bad Status 
       ...
       if((sstatus&(TOFDBcN::NOFT|TOFDBcN::NOWINDOWLT|(unbiase>=2?0:TOFDBcN::NOADC)))>0){sstatus|=TOFDBcN::BAD;}
...
//----For One HT many LT //may not accurate
         ...
         if((htdcw.size()==0)&&(shtdcw.size()==0)){
           for(i=0;i<int(ltdcw.size());i++){//tight guide
         ...
         if(htdcw.size()>0){
           for(i=0;i<int(ltdcw.size());i++){//real impossible two LT match HT due to 30ns LT lock
              for(j=0;j<int(htdcw.size());j++){//With self associate j
         ...
         //---Otherwise try using sht
          if((!ltok)&&(shtdcw.size()>0)){
           for(i=0;i<int(ltdcw.size());i++){//to find LT should match in windows
              for(j=0;j<int(shtdcw.size());j++)     
...
//--search by mindis
    for(unsigned int i=0;i<ltdcw[0].size();i++){//side0
      sdtm1[0]=ltdcw[0].at(i);

      for(unsigned int j=0;j<ltdcw[1].size();j++){//side1
...
//----
    ...
    tms[0]=(((status&TOFDBcN::BADN)>0)&&((status&TOFDBcN::LTREFIND)==0))? 0: (sdtm[0]+(adca[0]>0?2*sl1/pow(adca[0],index):0)+c0+c1);//s1 missing or not
    tms[1]=(((status&TOFDBcN::BADP)>0)&&((status&TOFDBcN::LTREFIND)==0))? 0: (sdtm[1]+(adca[1]>0?2*sl2/pow(adca[1],index):0)+c0-c1);//s2 missing or not
...
//--coo
    number tmsc[2];
    tmsc[0]=tms[0]-(adca[0]>0?2*dsl/pow(adca[0],index):0);//coo compensate
    tmsc[1]=tms[1]+(adca[1]>0?2*dsl/pow(adca[1],index):0);//coo compensate
...
//--Birk Correction -2 mean Overflow Birk Correction faild
  if(optc&kBirkCor)  {signalc=BirkCor(idsoft,signalc,1);signalc=GetMCtuneQ2(signalc,(idsoft/100)*100);/*MCtuning*/ if(signalc<=0)return signalc;}
//--ReAtt Correction
   if(optc&kReAttCor){signalc=SciReAttCor(idsoft,lcoo,signalc,1);}
//--Beta Correction
   if(optc&kBetaCor)   {
     signalc=BetaCor(idsoft,signalc,beta,rig,isanode,0);
     if((optc&kBetaRigV1Cor)==0&&TofCAlignIonPar::Version>=2)signalc=BetaCor(idsoft,signalc,beta,rig,isanode,1);
     if((optc&kBetaRigV1Cor)==0&&(optc&kBetaRigV2Cor)==0&&TofCAlignIonPar::Version>=3&&isanode==0)signalc=BetaCor(idsoft,signalc,beta,rig,isanode,2);//SHZ
     if(signalc<=0)return signalc;
   }
//--Rigidity Correction
   if(optc&kRigidityCor){
     signalc=RigCor(idsoft,signalc,rig,isanode,0);
     if((optc&kBetaRigV1Cor)==0&&TofCAlignIonPar::Version>=2)signalc=RigCor(idsoft,signalc,rig,isanode,1);
     if((optc&kBetaRigV1Cor)==0&&(optc&kBetaRigV2Cor)==0&&TofCAlignIonPar::Version>=3&&isanode==0)signalc=RigCor(idsoft,signalc,rig,isanode,2); //SHZ
     if(signalc<=0)return signalc;
   }
...
//========================================================
number TofRecH::GetMCtuneQ2(number rawq2,int id){

  if(TofMCPar::MCtuneQ2[0]==0||rawq2<=0||Init()!=0||realdata)return rawq2;
  number rnd[2];
#ifdef __ROOTSHAREDLIBRARY__
  AMSEventR::GetRandArray(4445357+id, 1,  1, &rnd[0]);//flat
  AMSEventR::GetRandArray(3357885+id, 2,  1, &rnd[1]);//gaus
#else
  return rawq2;//As rand would be different, I add proection from gbatch.
/*  rnd[0]=RNDM(-1);//flat
  rnd[1]=rnormx();//gaus*/
#endif
  number q2=rawq2;
  if(TofMCPar::MCtuneQ2[0]>0){//manual tuning one by one
    int is=(rnd[0]<TofMCPar::MCtuneQ2[2])?1:0;//choose 2nd sigma or 1st sigma by 2nd gaus fraction, TofMCPar::MCtuneQ2[2] can be 0
    q2=rawq2+(rawq2*TofMCPar::MCtuneQ2[is])*rnd[1];//TofMCPar::MCtuneQ2[0-1]=sigma/mean
    if(q2<=0)q2=0;
  }
  else if(TofMCPar::MCtuneQ2[0]<0){//final tuning (not correct number for the momemnt)
    const int nq=3;
    number z2[nq]={6*6,   14*14, 26*26};//tuning based on charge Z~6/~14/~26
    number f2[nq]={0.1,   0.1,   0.1  };//2nd guas fraction, f2 can be 0
    number s1[nq]={0.01,  0.01,  0.01 };//1nd guas sigma
    number s2[nq]={0.03,  0.03,  0.03 };//2nd gaus sigma
    int ihq=0;
    for(;ihq<nq;ihq++){if(rawq2<z2[ihq])break;}//find z2[ilq]<=rawq2<z2[ihq]
    int ilq=(ihq==0)?0:ihq-1;//protect underflow
    if(ihq>=nq-1)ihq=nq-1;//protect overflow 
    number wwl=fabs(z2[ihq]-rawq2);//using linear interpolation
    number wwh=fabs(rawq2-z2[ilq]);
    number f2n=(wwl+wwh==0)?f2[ilq]:(wwl*f2[ilq]+wwh*f2[ihq])/(wwl+wwh);//2nd gaus fraction
    number s1n=(wwl+wwh==0)?s1[ilq]:(wwl*s1[ilq]+wwh*s1[ihq])/(wwl+wwh);//1nd guas sigma
    number s2n=(wwl+wwh==0)?s2[ilq]:(wwl*s2[ilq]+wwh*s2[ihq])/(wwl+wwh);//2nd gaus sigma
    number sig=(rnd[0]<f2n)?s2n:s1n;//choose 2nd sigma or 1st sigma by 2nd gaus fraction
    q2=rawq2+(rawq2*sig)*rnd[1];
    if(q2<=0)q2=0;
  }
  return q2;
}
...
//========================================================
number TofRecH::RigCor(int idsoft,number q2,number rig,int isanode,int ncor){
  ...
  if(q2<=0||fabs(rig)<=1e-6)return q2;

///--Finding Algorithem
   int begch=(ncor==0)?TofCAlignIonPar::RigCh[0]:CParI->RigChC[isanode][0];
   if(ncor==2)begch=CParI->RigChS[0];
   if(begch<=0)return q2;
   int nowch=begch;
   number cor1,cor2,ch1,ch2;
   number rigcor=1;
   while (1){
      cor1=GetRigCalCh(idsoft,nowch,rig,isanode,ncor);
      cor2=GetRigCalCh(idsoft,nowch+1,rig,isanode,ncor);
      ch1=sqrt(q2/cor1);
      ch2=sqrt(q2/cor2);
  ///--Find LowLimit
      if(ch1<begch||ch2<begch||nowch==0){
        rigcor=cor1;break;
      }
///--Find Gap
...
//========================================================
number TofRecH::GetRigCalCh(int idsoft,int charge,number rig,int isanode,int ncor){

  if(charge<=0)return 1;
   TofCAlignIonPar  *CParI=TofCAlignIonPar::GetHead();

   number corvar=1;
   int nch=(ncor==0)?TofCAlignIonPar::nRigCh:CParI->nRigChC[isanode];
   const int *ch=(ncor==0)?TofCAlignIonPar::RigCh:CParI->RigChC[isanode];
   if(ncor==2){nch=CParI->nRigChS; ch=CParI->RigChS;}
   for(int ich=0;ich<nch;ich++){
///---Find in arr or at end
       if(charge<ch[0]||charge==ch[ich]||ich==nch-1){
         corvar=GetRigCalI(idsoft,ich,rig,isanode,ncor); //Beta Correction
         break;
      }
///---Find In middle need interpolation
     else if(charge>ch[ich]&&charge<ch[ich+1]){
        number  ww1=charge*charge-ch[ich]*ch[ich];
        number  ww2=ch[ich+1]*ch[ich+1]-charge*charge;
        corvar=(ww2*GetRigCalI(idsoft,ich,rig,isanode,ncor)+ww1*GetRigCalI(idsoft,ich+1,rig,isanode,ncor))/(ww1+ww2);//Beta Correction
        break;
      }
   }
...
//========================================================
number TofRecH::GetRigCalI(int idsoft,int chindex,number rig,int isanode,int ncor){//Counter Level

   TofCAlignIonPar  *CParI=TofCAlignIonPar::GetHead();
///----
   number rigcut=1.;////Const
   if(ncor>=2)rigcut=1.8;
...
//---
   ...
   for(int ipar=0;ipar<7;ipar++){
     if     (ncor==0)par[ipar]=CParI->rigcor[isanode][chindex][ipar][idsoft];
     else if(ncor==1)par[ipar]=CParI->rigcorc[isanode][chindex][ipar][idsoft];
     else            par[ipar]=CParI->rigcors[chindex][ipar][idsoft];
   }
//--Rigidity Correction
...
//========================================================
number TofRecH::BetaCor(int idsoft,number q2,number beta,number rig,int isanode,int ncor){
 
   if(q2<=0)return q2;
   TofCAlignIonPar  *CParI=TofCAlignIonPar::GetHead();

///--Finding Algorithem
   int begch=(ncor==0)?TofCAlignPar::BetaCh[0]:CParI->BetaChC[isanode][0];
   if(ncor==2)begch=CParI->BetaChS[0];
   if(begch<=0)return q2;
   int nowch=begch;
   number cor1,cor2,ch1,ch2;
   number betacor=1;
   while (1){
      cor1=GetBetaCalCh(idsoft,0,beta,0,nowch,rig,isanode,ncor);
      cor2=GetBetaCalCh(idsoft,0,beta,0,nowch+1,rig,isanode,ncor);
      ch1=sqrt(q2/cor1);
      ch2=sqrt(q2/cor2);
///--Find LowLimit
      if(ch1<begch||ch2<begch||nowch==0){
...
//========================================================
number TofRecH::GetBetaCalCh(int idsoft,int opt,number beta,number q2,int charge,number rig,int isanode,int ncor){

   if(charge<=0)return 1;
   TofCAlignIonPar  *CParI=TofCAlignIonPar::GetHead();

   number corvar=1;
   int nch=(ncor==0)?TofCAlignPar::nBetaCh:CParI->nBetaChC[isanode];
   const int *ch=(ncor==0)?TofCAlignPar::BetaCh:CParI->BetaChC[isanode];
   if(ncor==2){nch=CParI->nBetaChS; ch=CParI->BetaChS;}
   for(int ich=0;ich<nch;ich++){
///---Find in arr or at end
      if(charge<ch[0]||charge==ch[ich]||ich==nch-1){
         if(opt==0)corvar=GetBetaCalI(idsoft,0,beta,ch[ich],ich,rig,isanode,ncor); //Beta Correction
         else      {
            q2=q2/(charge*charge);
            corvar=GetBetaCalI(idsoft,1,beta,q2,ich,rig,isanode,ncor);//Edep Beta
          }
         break;
      }
///---Find In middle need interpolation
        ...
        number  ww1=charge*charge-ch[ich]*ch[ich];
        number  ww2=ch[ich+1]*ch[ich+1]-charge*charge;
        if(opt==0)corvar=(ww2*GetBetaCalI(idsoft,0,beta,ch[ich],ich,rig,isanode,ncor)+ww1*GetBetaCalI(idsoft,0,beta,ch[ich+1],ich+1,rig,isanode,ncor))/(ww1+ww2);//Beta Correction
        else     {
            q2=q2/(charge*charge);
            corvar=(ww2*GetBetaCalI(idsoft,1,beta,q2,ich,rig,isanode,ncor)+ww1*GetBetaCalI(idsoft,1,beta,q2,ich+1,rig,isanode,ncor))/(ww1+ww2);//Edep Beta
        }
        break;
      }
   }
...
//========================================================
number TofRecH::GetBetaCalI(int idsoft,int opt,number beta,number q2norm,int chindex,number rig,int isanode,int ncor){//Counter Level
...
//--Normal Beta 
   number rigcut=20;//>20GeV not Apply Beta Correction
   number betacut=1.;
   if(q2norm>=9){rigcut=20;betacut=1;}//fluorine
   number betap=fabs(beta)>betacut? betacut :fabs(beta);
   if(fabs(rig)>rigcut)betap=betacut;
//--Protection
   if(betap<0.38)betap=0.38;
   if(ncor>=1){
     if(!CParI->isreal&&TofCAlignIonPar::Version>=4&&ncor==1){//new MC
        if(q2norm<9){if(betap<0.4) betap=0.4;}
        else        {if(betap<0.57)betap=0.57;} 
     }
     else if(betap<0.6)betap=0.6;
  }
...
//    fun=new TF1("TOF_VE","pol4",0.2,1.1);
    for(int ipar=0;ipar<=4;ipar++){
       if     (ncor==0)fun->SetParameter(ipar,CPar->betacorn[chindex][ipar][idsoft]);
       else if(ncor==1)fun->SetParameter(ipar,CParI->betacorc[isanode][chindex][ipar][idsoft]);
       else            fun->SetParameter(ipar,CParI->betacors[chindex][ipar][idsoft]);
    }
...
int TofRecH::BuildBetaHI(int verse){
//---
   if(Init()!=0)return -1;
   ClearBuildBetaH();
//---
...
///---If EcalShower
    if((ecalshow.size()>0)&&(!(BuildOpt>=10&&((BuildOpt/100)%10==0)))){
       number cooshow[3];
       for(unsigned int ish=0;ish<ecalshow.size();ish++){
...
///--First search
    ...
    for(unsigned int i=0;i<tofclh[ilay].size();i++){
...
//---
        if(transcut){
          ...
          if(ib==int(tofclh[ilay].size())-1)nextbar=-1000;
...
int TofRecH::TOFPairSel(int ud,TofClusterHR* tfhit[2]){
    ...
    for(unsigned int icl=0;icl<tofclc[ud].size();icl++){//Up part
...
///---
    ...
    for(unsigned int iclp=0;iclp<tofclp[ud].size();iclp++){//Up part
...
//--pair+pair
   if(opt==0){
    for(unsigned int icl=0;icl<tofclp[1-sedud].size();icl++){
...
//--From From Layer vector
    for(unsigned int iclh=0;iclh<tofclh[ilay].size();iclh++){
  ...
  if(ptrack){
     for(unsigned int ii=0;ii<ev->NParticle();ii++){
       if(ev->pParticle(ii)->pTrTrack()==ptrack){
         ev->pParticle(ii)->setBetaH(ev->NBetaH()-1);
         ChargeR *pcharge=ev->pParticle(ii)->pCharge();
         if(pcharge)pcharge->setBetaH(ev->NBetaH()-1);
         break;
       }
     }
   }
...
///---Find Candidate Not to attached to particle
    ...
    for(int ii=0;ii<ev->nBetaH();ii++){//candidate
       bool used=0; 
       for(unsigned int jj=0;jj<ev->NParticle();jj++){
...
//----Refind  TRD or TRDH or EcalShower
      ...
      for(unsigned int jj=0;jj<fBetaH.size();jj++){
///---Trd
...
int  TofRecH::DoAdvancedTrackFitI(){

#ifndef _PGTRACK_
  return -1;
#else
  VCon* cont = GetVCon()->GetCont("AMSBetaH");
  if(!cont)return -1;
  
  int nbetah=cont->getnelem();
  int ntrk=0;
  bool usev6=(TRCLFFKEY.recflag/10000000%10>=1);
  for(int ibetah=0;ibetah<nbetah;ibetah++){
   //----
   BetaHR* betah=(BetaHR*) cont->getelem(ibetah);
   TrTrackR *track=betah->pTrTrack();
   if(track==0)continue;

   //----recalculate coordinate first and assign charge/mass;
   float qtrk=track->GetInnerQ(betah->GetBeta(), 0);
   int chrg=int(qtrk+0.5);
   if(chrg<1)chrg=1;
   float mass=TrFit::Mproton;
   if (chrg>=2){mass = TrFit::Mhelium/2*chrg;}
   int nv=usev6?2:1;
   for(int iv=0;iv<nv;iv++){//V5 or V6
     int mfit0=track->Gettrdefaultfit();
     int mfit1=(iv==0)?mfit0&(~TrTrackR::kAlignV6):mfit0|TrTrackR::kAlignV6;//V5 or V6
     track->RecalcHitCoordinates(mfit1,0,0,chrg);
     if(track->FitT(mfit1,-1,true,0,mass,chrg)>0)track->RecalcHitCoordinates(mfit1,0,0,chrg);
     //-----update advance fit 
     const int npatt=4;
     int patt[npatt]={0, TrTrackR::kFitLayer8, TrTrackR::kFitLayer9, TrTrackR::kFitLayer8|TrTrackR::kFitLayer9};
     for(int ipatt=0;ipatt<npatt;ipatt++){
       if( (ipatt==1||ipatt==3) && ! track->TestHitLayerJ(1))continue;//require L1
       if( (ipatt==2||ipatt==3) && ! track->TestHitLayerJ(9))continue;//require L9
       track->DoAdvancedFit(((iv==0)?0:TrTrackR::kAlignV6)|patt[ipatt],mass,chrg,(iv==nv-1)?true:false);
      }
    }
    ntrk++;
  }
...
int TofRecH::DoPrimaryTrackTagI(int direction){
...
#else
  VCon* cont = GetVCon()->GetCont("AMSBetaH");
  int nbetah=cont? cont->getnelem(): 0;
  int ibetah=-1;
  float tksq=0;
//----first sort Track by MaxInnerQ
  for(int ibh=0;ibh<nbetah;ibh++){
    BetaHR* betah=(BetaHR*) cont->getelem(ibh);
    TrTrackR *track=betah->pTrTrack();
    if(track==0)continue;
    float ntksq=track->GetInnerQ()*track->GetNhitsI();
    if(ntksq>tksq){ibetah=ibh; tksq=ntksq;}
  }
  
//----double Check(Recover) Wrong-Tk and Good Candidate
  if(ibetah>=0){
    map<pair<int,pair<double,int> >,int>ibhg;
//----sort good candidate again
    for(int ibh=0;ibh<nbetah;ibh++){
      BetaHR* betah=(BetaHR*) cont->getelem(ibh);
      TrTrackR *track=betah->pTrTrack();
      if(track==0)continue;
      ...
      else if(direction==0){//downward and upward
        if(fabs(obeta)<0.4)continue;
      }
      else {//upward
        if(obeta>-0.4)continue;
      }
//---InnerNHit Suppress
      int ntk_hitb[2]={0};
      ntk_hitb[0]=track->GetBitPatternXYJ();
      ntk_hitb[1]=track->GetBitPatternJ();
      int nhiti[2]={0};
...
//---Charge Suppress Wrong-Tk
      double tkiq=track->GetInnerQ(obeta,0);
      if(tkiq<0.6)continue;
      int npi=track->GetNPlanesI();
//---L1L9 Par
      bool lhxy[2]={0};
      for(int iex=0;iex<2;iex++){
        int ilay=(iex==0)?0:8;
        lhxy[iex]=((ntk_hitb[1]&(1<<ilay))>0&&(ntk_hitb[0]&(1<<ilay))>0);//HitXY
        if(!lhxy[iex])continue;
        double tkexq=track->GetLayerJQ(ilay+1,obeta,0);
        if     (tkiq>=2.5)          lhxy[iex]=(tkexq>2.2&&tkexq>tkiq*0.5);//Z>=3
        else if(tkiq>=1.5&&tkiq<2.5)lhxy[iex]=(tkexq>1.5&&tkexq<3);//Z>=2
        else                        lhxy[iex]=(tkexq>0.5&&tkexq<2);//Z>=1
      }
      int nex=0,iz=0;
      vector<pair<int,int> > mex;
      if(lhxy[0]&&lhxy[1])mex.push_back(make_pair(TrTrackR::kFitLayer8|TrTrackR::kFitLayer9,2));
      if(lhxy[0]){mex.push_back(make_pair(TrTrackR::kFitLayer8,1));nex++;}
      if(lhxy[1]){mex.push_back(make_pair(TrTrackR::kFitLayer9,1));nex++;}
      mex.push_back(make_pair(0,0));
//-----
      double tkrig=track->GetRigidity();
      int iid=(track->Gettrdefaultfit()&(~(TrTrackR::kFitLayer8|TrTrackR::kFitLayer9)));//inner tracker
      for(unsigned int isp=0;isp<mex.size();isp++){//highest priority first
        int mfit=iid|mex[isp].first;
        if(!track->FitDone(mfit))continue;
        if(isp==0)tkrig=track->GetRigidity(mfit);//max-span
        if(track->GetNormChisqY(mfit)<20){//max-span good-track
          tkrig=track->GetRigidity(mfit);
          iz=2+mex[isp].second;
          break;
        }
      }
//---Rigidity Suppress Wrong-Tk
      ...
      iz+=1000*int(tkiq+0.5)+nex*100+npi*10;
      int nhitsw=2*(nex+npi)+(track->GetNhitsI()-npi);
      double quality=tkiq*fabs(tkrig)*nhitsw;
      pair<double,int> qval(quality,nhitsw);
      ibhg[make_pair(iz,qval)]=ibh;
    }
//---Compare to Get Best Candidate
    if(ibhg.size()>=1){
      map<pair<int,pair<double,int> >,int>::reverse_iterator sit=ibhg.rbegin();
      map<pair<int,pair<double,int> >,int>::reverse_iterator rit=sit;
      ++rit;
      for(;rit!=ibhg.rend();++rit){
        int iz[2]={0},gz[2]={0},nex[2]={0},npi[2]={0},gchis[2]={0},nhitsw[2]={0};
        double quality[2]={0};
        for(int j=0;j<2;j++){
          iz[j]=(j==0)?(sit->first).first:(rit->first).first;
          gz[j]=iz[j]/1000;//charge
          if(gz[j]>=3)gz[j]=3;//Z=3 is the highest Z group
          nex[j]=(iz[j]/100)%10;
          npi[j]=(iz[j]/10)%10;
          gchis[j]=iz[j]%10;
          quality[j]=(j==0)?(sit->first).second.first:(rit->first).second.first;
          nhitsw[j]=(j==0)?(sit->first).second.second:(rit->first).second.second;
        }
        int ib=0;
        if(gz[1]==gz[0]){//the same group
          if(iz[0]/1000>iz[1]/1000){//different charge
            if     (nex[0]>=nex[1]&&npi[0]>npi[1])ib=0;
            else if(npi[0]>npi[1])ib=(quality[1]>quality[0]&&(gchis[1]>0&&gchis[0]==0))?1:0;
            else if(nex[0]>nex[1])ib=(quality[1]>quality[0]&&(gchis[1]>=gchis[0]-1&&gchis[1]>0))?1:0;
            else                  ib=(quality[1]>quality[0]&&(gchis[1]>=gchis[0]-1||gchis[1]>0))?1:0;
          }
          else if(nex[0]>nex[1]){//same charge, different external layers
            if     (npi[0]>npi[1])ib=0;
            else if(npi[0]==npi[1]&&nex[0]==nex[1]+2)ib=(quality[1]>quality[0]&&(gchis[1]>=gchis[0]-1&&gchis[1]>0))?1:0;
            else                  ib=(quality[1]>quality[0]&&(gchis[1]>=gchis[0]-1||gchis[1]>0))?1:0;
          }
          else if(npi[0]>npi[1]){//same charge and external layers, different inner-planes
            ib=(quality[1]>quality[0]&&(gchis[1]>=gchis[0]+2))?1:0;
          }
          else if(gchis[0]>0&&gchis[1]==0){//same charge external layers and inner-planes, different chis
            ib=(quality[1]>quality[0]*2.&&nhitsw[1]>nhitsw[0])?1:0;
          }
          else {
            ib=(quality[1]>quality[0])?1:0;
          }
        }
        else if(gz[1]<=1){//Z>=3||Z=2 VS Z=1
          if(gz[0]>=3){//Z>=3 VS Z=1
            if(npi[0]<=3&&nex[1]==nex[0]+2){if(quality[1]>quality[0]&&(gchis[1]==4||gchis[0]==0))ib=1;}//Z>=3 3-pla VS Z=1 L1IL9
          }
          else {//Z=2 VS Z=1
            if     ((npi[0]<=3&&nex[0]==0)&&nex[1]>nex[0]){if(quality[1]>quality[0]&&(gchis[1]>=3||gchis[0]==0))ib=1;}//Z=2 3-pla VS Z=1 L1Inner
            else if(npi[1]>=npi[0]&&nex[1]==nex[0]+2){if(quality[1]>quality[0]&&(gchis[1]==4||gchis[0]==0))ib=1;}//Z=2 4-pla VS Z=1 L1IL9
          }
        }
        else {//Z>=3 VS Z=2
           if     ((npi[0]<=3&&nex[0]==0)&&(npi[1]>npi[0])){if(quality[1]>quality[0]&&gchis[1]>=gchis[0])ib=1;}//Z>=3 3-pla VS Z=2 4-pli
           else if((npi[0]<=3&&nex[0]==0)&&(nex[1]>nex[0])){if(quality[1]>quality[0]&&(gchis[1]>=3||gchis[0]==0))ib=1;}//Z>=3 3-pla VS Z=2 L1I
           else if(npi[1]>=npi[0]&&nex[1]==nex[0]+2){if(quality[1]>quality[0]&&(gchis[1]==4||gchis[0]==0))ib=1;}//Z>=3 4-pla VS Z=2 L1IL9
        }
        if(ib==1)sit=rit;
      }//end of iteration
      ibetah=sit->second;
...
//-----clean primary tag first
  VCon* contt = GetVCon()->GetCont("AMSTrTrack");
  int ntrk=contt? contt->getnelem(): 0;
  for(int itr=0; itr<ntrk; itr++){
    TrTrackR *track=(TrTrackR *)contt->getelem(itr);
    int rtype=track->GetRecType();
    track->SetRecType(rtype-((rtype/10000)%10)*10000);
  }

//-----set primary tag afterward
  if(ibetah>=0){
    BetaHR* betah=(BetaHR*) cont->getelem(ibetah);
    TrTrackR *track=betah->pTrTrack();
    int rtype=track->GetRecType();
    track->SetRecType(rtype+10000);
  } 
  else if(ntrk>=1){//otherwise use the frist from reconstruction
    TrTrackR *track=(TrTrackR *)contt->getelem(0);
    int rtype=track->GetRecType();
    track->SetRecType(rtype+10000);
  }
  
  if(contt)delete contt;
  if(cont)delete cont;
  return ibetah;
#endif
