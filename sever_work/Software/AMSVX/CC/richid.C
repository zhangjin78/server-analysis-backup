...
///////////////////////////////////////////////////////////////////
  int RichPMTGridInfo::FindBinX(double x) {
    // transform to local coordinates in the grid
    double xLookup = x - centerX;

    // special handling of gap in grid 1 and grid 5
    if (hasGapX) {
      if (xLookup >= gapBeginX && xLookup < gapEndX)
        return -1;
      else if (xLookup >= gapEndX)
        xLookup -= gapWidthX;
    }

    // x bin number in the grid
    int bin_x = (xLookup - x0) / binwidthX;
    if (bin_x < 0 || bin_x >= nBinsX)
      return -1;

    return bin_x;
  }

  int RichPMTGridInfo::FindBinY(double y) {
    // transform to local coordinates in the grid
    double yLookup = y - centerY;

    // y bin number in the grid
    int bin_y = (yLookup - y0) / binwidthY;
    if (bin_y < 0 || bin_y >= nBinsY)
      return -1;

    return bin_y;
  }

  int RichPMTGridInfo::FindPMT(double x, double y) {

    int bin_x = FindBinX(x);
    if (bin_x < 0)
      return -1;

    int bin_y = FindBinY(y);
    if (bin_y < 0)
      return -1;

    // pmt candidate number for the grid position
    return pmts_2D[bin_x][bin_y];
  }
...
RichPMTGridInfo RichPMTsManager::_grid_info[8];
...
//#pragma omp master
  {
	...
	*/
	...
	if(stream.is_open() && !stream.fail()){
      ...
      AMSDBc::convert4((char*)table_gain,RICmaxpmts*RICnwindows*2);
       AMSDBc::convert4((char*)table_sigma_gain,RICmaxpmts*RICnwindows*2); 
	  if(!stream.eof()){
		...
		for(int mode=0;mode<2;mode++){
		  ...
		  RichPMTsManager::_GainSim(_pmts[pmt]._geom_id,channel,mode)=my_gain;
	  ...
	  for(int pmt=0;pmt<RICmaxpmts;pmt++)
        ...
        if(sizeof(_pmts[0]._step[0][0])==4)AMSDBc::convert4((char*)(&_pmts[pmt]._step[channel][mode]),1);
        else cerr<<"RichPMTsManager::Init-E-ConvertionNotDefined "<<sizeof(_pmts[0]._step[0][0])<<endl; 
		stream.read((char*)(_pmts[pmt]._cumulative_prob[channel][mode]),sizeof(_pmts[0]._cumulative_prob[0][0][0])*RIC_prob_bins);
        if(sizeof(_pmts[0]._cumulative_prob[0][0][0])==4)AMSDBc::convert4((char*)(_pmts[pmt]._cumulative_prob[channel][mode]),RIC_prob_bins);
        else  cerr<<"RichPMTsManager::Init-E-ConvertionNotDefined "<<sizeof(_pmts[0]._cumulative_prob[0][0][0])<<endl;
...
#include "richid_default_values.h"  // Tables with default values of PMT information
  ...
  for(int i=0;i<RICH_JINFs*RICH_CDPperJINF;i++){
    ...
    //////////////////////////////////////////////
    // Setup RichPMTGridInfo structures

    // Find x0, x1, y0 and Y1 from PMT coordinates in the grid
    for(int i=0;i<RICmaxpmts;i++) {
      int g = _pmts[i].grid()-1;
      assert(g >= 0 && g < 8);
      double pmt_x = _pmts[i]._local_position[0];
      double pmt_y = _pmts[i]._local_position[1];
      if (pmt_x < _grid_info[g].x0) _grid_info[g].x0 = pmt_x;
      if (pmt_x > _grid_info[g].x1) _grid_info[g].x1 = pmt_x;
      if (pmt_y < _grid_info[g].y0) _grid_info[g].y0 = pmt_y;
      if (pmt_y > _grid_info[g].y1) _grid_info[g].y1 = pmt_y;
    }

    // Special handling for Grid 1 and Grid 5, which have a 0.5 cm gap in the binning.
    _grid_info[0].hasGapX = true;
    _grid_info[0].gapBeginX = 1.6;
    _grid_info[0].gapEndX = 2.1;
    _grid_info[0].gapWidthX = _grid_info[0].gapEndX - _grid_info[0].gapBeginX;
    _grid_info[4].hasGapX = true;
    _grid_info[4].gapBeginX = -2.1;
    _grid_info[4].gapEndX = -1.6;
    _grid_info[4].gapWidthX = _grid_info[4].gapEndX - _grid_info[4].gapBeginX;

    // Complete the Grid initialization
    for(int g=0;g<8;g++) {
      _grid_info[g].centerX = -grid_x[g];
      _grid_info[g].centerY = -grid_y[g];
      _grid_info[g].x0 -= _grid_info[g].binwidthX/2.;
      _grid_info[g].y0 -= _grid_info[g].binwidthY/2.;
      _grid_info[g].x1 += _grid_info[g].binwidthX/2.;
      _grid_info[g].y1 += _grid_info[g].binwidthY/2.;
      if (_grid_info[g].hasGapX)
        _grid_info[g].x1 -= _grid_info[g].gapWidthX;
      _grid_info[g].nBinsX = (_grid_info[g].x1 - _grid_info[g].x0) / _grid_info[g].binwidthX;
      _grid_info[g].nBinsY = (_grid_info[g].y1 - _grid_info[g].y0) / _grid_info[g].binwidthY;
      _grid_info[g].pmts_2D = new int*[_grid_info[g].nBinsX];
      for(int i=0;i<_grid_info[g].nBinsX;i++) {
        _grid_info[g].pmts_2D[i] = new int[_grid_info[g].nBinsY];
        for(int j=0;j<_grid_info[g].nBinsY;j++) {
          _grid_info[g].pmts_2D[i][j] = -1;
        }
      }
    }

    // Setup the pmts_2D map in the grid.
    for(int i=0;i<RICmaxpmts;i++) {
      int g = _pmts[i].grid()-1;
      assert(g >= 0 && g < 8);
      double pmt_x = _pmts[i]._global_position[0];
      double pmt_y = _pmts[i]._global_position[1];
      int bin_x = _grid_info[g].FindBinX(pmt_x);
      int bin_y = _grid_info[g].FindBinY(pmt_y);
      assert(bin_x >= 0 && bin_x < _grid_info[g].nBinsX);
      assert(bin_y >= 0 && bin_y < _grid_info[g].nBinsY);
      _grid_info[g].pmts_2D[bin_x][bin_y] = i;

      // Check that the _grid_info[g] maps the coordinates of the PMT to the correct PMT.
      assert(_grid_info[g].FindPMT(pmt_x, pmt_y) == i);
    }
...
integer RichPMTsManager::detcer(geant photen)
{
  ...
  int RichPMTsManager::FindGrid(geant x, geant y) {
    ...
    if(x>RICHDB::hole_radius[0]-RICpmtsupport/2.){
      if(y>RICHDB::hole_radius[1]-RICpmtsupport/2.)
        return 6; //return 2;
      else if(-y>RICHDB::hole_radius[1]-RICpmtsupport/2)
        return 8; //return 4;
      else
        return 7; //return 3;
    }
...
int RichPMTsManager::FindPMTOld(geant x,geant y){
  //
...
int RichPMTsManager::FindChannelOld(geant x,geant y){
...
int RichPMTsManager::FindPMT(geant x,geant y){
    ...
    integer grid = FindGrid(x, y);
    if (grid < 0)
      return -1;

    assert(grid>0 && grid <= 8);
    grid--;
    int pmt_candidate = _grid_info[grid].FindPMT(x, y);
    if (pmt_candidate < 0)
      return -1;

    assert(pmt_candidate >= 0 && pmt_candidate < RICmaxpmts);
    double pmt_x = _pmts[pmt_candidate]._global_position[0];
    double pmt_y = _pmts[pmt_candidate]._global_position[1];
    if(std::abs(x-pmt_x) < RICHDB::lg_length/2. && std::abs(y-pmt_y) < RICHDB::lg_length/2.)
      return pmt_candidate;
...
int RichPMTsManager::FindChannel(geant x,geant y){
  ...
  RichPMT &pmt=RichPMTsManager::GetPMT(geom_pmt);
    ...
    assert(RICnwindows == 16);
    int channel=-1;
    double x0 = pmt._channel_position[0][0] - RICHDB::lg_length/4./2.; // left edge
    double x1 = pmt._channel_position[3][0] + RICHDB::lg_length/4./2.; // right edge
    double deltaX = (x1-x0)/4.;
    int binX = (x - x0)/deltaX;
    if (binX < 0 || binX >= 4)
      return -1;

    double y0 = pmt._channel_position[0][1] - RICHDB::lg_length/4./2.; // bottom edge
    double y1 = pmt._channel_position[3*4][1] + RICHDB::lg_length/4./2.; // top edge
    double deltaY = (y1-y0)/4.;
    int binY = (y - y0)/deltaY;
    if (binY < 0 || binY >= 4)
      return -1;

    channel = binY*4 + binX;
    assert(channel >= 0 && channel < RICnwindows);
...
void _Update_RICHPMT(){
  // Recompute all the tables: disabled as everything is read from tables
