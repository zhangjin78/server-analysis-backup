//  $Id: trcalib.C,v 1.64 2018/03/04 11:43:29 qyan Exp $
#include "trcalib.h"
#include "event.h"
#include <math.h>
#include "timeid.h"
      ...
      if(npart ==1 && ppart && ppart->getcharge()==charge ){
           ...
           ptr->getParSimpleFit(chi2_3d,chi2z,rid);
            ...
            if(chi2_3d<1. && cth > TRCALIB.HitsRatioCut[alg] &&
               ...
               ppart->getmomentum()<momentum*TRCALIB.MomentumCut[alg][1]  && pbeta){
              if(Init(pbeta,ppart->getptrack(),pmcg,pattern,alg)){
               AMSEvent::gethead()->addnext(AMSID("AMSTrCalibration",0),
               new AMSTrCalibration(pattern));
               return 1;
              }
