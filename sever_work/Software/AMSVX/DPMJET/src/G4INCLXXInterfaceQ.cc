...
G4INCLXXInterfaceQ::G4INCLXXInterfaceQ(G4VPreCompoundModel * const aPreCompound) :
  ...
  fEexc(1.)
{
  ...
  if(getenv("G4INCLXX_NO_DE_EXCITATION")) {
    ...
    theDeExcitation=thePreCompoundModel;
/*    G4HadronicInteraction* p =
      G4HadronicInteractionRegistry::Instance()->FindModel("PRECO");
  ...
  theBackupModel = new G4BinaryLightIonReactionQ;
  theBackupModelNucleon = new G4BinaryCascade;
}

G4INCLXXInterfaceQ::~G4INCLXXInterfaceQ()
{
  delete theINCLXXLevelDensity;
  delete theINCLXXFissionProbability;
}

G4bool G4INCLXXInterfaceQ::AccurateProjectile(const G4HadProjectile &aTrack, const G4Nucleus &theNucleus) const {
...
G4HadFinalState* G4INCLXXInterfaceQ::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& theNucleus)
{
...
//-----
        G4double px2     = fourMomentum.px();
        G4double py2     = fourMomentum.py();
        G4double pz2     = fourMomentum.pz();
        G4double e2      = fourMomentum.e();
        G4double mass2   = e2*e2-(px2*px2+py2*py2+pz2*pz2);
        if(mass2>0)mass2 = std::sqrt(mass2);
        G4double mass0   = G4NucleiProperties::GetNuclearMass(A, Z);
...
//----------
	G4Fragment remnant(A, Z, fourMomentum2);
...
G4ReactionProductVector* G4INCLXXInterfaceQ::Propagate(G4KineticTrackVector* , G4V3DNucleus* ) {
  return 0;
}

G4INCL::ParticleType G4INCLXXInterfaceQ::toINCLParticleType(G4ParticleDefinition const * const pdef) const {
...
G4INCL::ParticleSpecies G4INCLXXInterfaceQ::toINCLParticleSpecies(G4HadProjectile const &aTrack) const {
...
G4double G4INCLXXInterfaceQ::toINCLKineticEnergy(G4HadProjectile const &aTrack) const {
  return aTrack.GetKineticEnergy();
}

G4ParticleDefinition *G4INCLXXInterfaceQ::toG4ParticleDefinition(G4int A, G4int Z, G4int PDGCode) const {
...
G4DynamicParticle *G4INCLXXInterfaceQ::toG4Particle(G4int A, G4int Z, G4int PDGCode,
						 G4double kinE,
						 G4double px,
...
G4double G4INCLXXInterfaceQ::remnant4MomentumScaling(G4double mass,
						  G4double kineticE,
...
void G4INCLXXInterfaceQ::ModelDescription(std::ostream& outFile) const {
   outFile
...
G4String const &G4INCLXXInterfaceQ::GetDeExcitationModelName() const {
