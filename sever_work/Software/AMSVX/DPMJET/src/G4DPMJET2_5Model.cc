...
//
   ...
   if(verboseLevel >= 2){
       G4cout<<" epn="<<epn<<" AT1="<<AT1<<" ZT1="<<ZT1<<" AP1="<<AP1<<" ZP1="<<ZP1<<" iiipro="<<iiipro<<" kkmat="<<kkmat<<" iiitar="<<iiitar<<" nnkkh1="<<nhkkh1<<G4endl;
    }
  G4cout <<" CALLING KKINC "<<iiipro<<" "<<nucc_.ijproj<<G4endl;
...
//
  ...
  if(pomtab_.ipomta!=2)pomtab_.ipomta  = 1;				// IPOMTA = 1
...
//  }
  ...
  if (GetVerboseLevel()>0) {
    ...
    G4cout <<"OPENING NUCLEAR.FMT.BIN ON FILE UNIT " <<lunber <<G4endl;
  }
#endif
  ...
  defaultDirName = G4String(getenv("G4DPMJET2_5DATA")) + "/NUCLEAR.FMT.BIN";
...
//
//
// Problems with locating NUCLEAR.FMT.BIN file.
//
    G4cout <<"NUCLEAR.FMT.BIN FILE NOT FOUND IN DIRECTORY " <<defaultDirName
           <<G4endl;
    throw G4HadronicException(__FILE__, __LINE__,
    "NUCLEAR.FMT.BIN file not present.");
  }
#ifdef G4VERBOSE
  if (GetVerboseLevel()>0)
    G4cout <<"AT G4DPMJET2_5Model::Initialise: after NUCLEAR.FMT.BIN"  <<G4endl;
#endif
...
#ifdef G4VERBOSE
  if (GetVerboseLevel()>=0)
    G4cout <<"AT G4DPMJET2_5Model::Initialise: NUCLEAR.FMT.BIN closed"  <<G4endl;
#endif
...
//
  ...
  if (GetVerboseLevel()>0) {
    ...
    G4cout <<"SINGDIFF " <<diffradpm_.isingd <<G4endl;
...
struct haxsec
{
G4double a[28][4][maxpro_+2][2];
G4double b[28][maxpro_+2][5];
G4int i[28][maxpro_+2][3];
};
struct haoutl
{
G4int i[3];
...
extern  struct hacons hacons_;
extern struct haxsec haxsec_;
extern struct haoutl haoutl_;
extern struct haqqap haqqap_;
extern struct hapdco hapdco_;
extern struct hapara hapara_;
#pragma omp threadprivate(hacons_,haxsec_,haoutl_,haqqap_,hapdco_,hapara_)


extern "C" void harini_copy_(int &init){

static struct hacons haconsi;
static struct haxsec haxseci;
static struct haoutl haoutli;
static struct haqqap haqqapi;
static struct hapdco hapdcoi;
static struct hapara haparai;


if(init==0){
 G4cout <<" harini_copy_-I-CopyingTo "<<G4endl;
 memcpy(&haconsi,&hacons_,sizeof(hacons));
 memcpy(&haxseci,&haxsec_,sizeof(haxsec));
 memcpy(&haoutli,&haoutl_,sizeof(haoutl));
 memcpy(&haqqapi,&haqqap_,sizeof(haqqap));
 memcpy(&hapdcoi,&hapdco_,sizeof(hapdco));
 memcpy(&haparai,&hapara_,sizeof(hapara));
}
else{
 G4cout <<" harini_copy_-I-CopyingFrom "<<G4endl;
 memcpy(&hacons_,&haconsi,sizeof(hacons));
 memcpy(&haxsec_,&haxseci,sizeof(haxsec));
 memcpy(&haoutl_,&haoutli,sizeof(haoutl));
 memcpy(&haqqap_,&haqqapi,sizeof(haqqap));
 memcpy(&hapdco_,&hapdcoi,sizeof(hapdco));
 memcpy(&hapara_,&haparai,sizeof(hapara));
}
}
