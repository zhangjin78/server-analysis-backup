...
//#define debug_G4BinaryCascadeQ 1
...
//-------
//#if defined(debug_G4BinaryCascadeQ)
#if 0
...
G4BinaryCascadeQ::G4BinaryCascadeQ(G4VPreCompoundModel* ptr) :
...
/*
G4BinaryCascadeQ::G4BinaryCascadeQ(const G4BinaryCascadeQ& )
: G4VIntraNuclearTransportModel("Binary Cascade")
{
}
 */

G4BinaryCascadeQ::~G4BinaryCascadeQ()
{
...
void G4BinaryCascadeQ::ModelDescription(std::ostream& outFile) const
{
    outFile << "G4BinaryCascadeQ is an intra-nuclear cascade model in which\n"
...
void G4BinaryCascadeQ::PropagateModelDescription(std::ostream& outFile) const
{
    outFile << "G4BinaryCascadeQ propagtes secondaries produced by a high\n"
...
//----------------------------------------------------------------------------
G4HadFinalState * G4BinaryCascadeQ::ApplyYourself(const G4HadProjectile & aTrack,
        G4Nucleus & aNucleus)
//----------------------------------------------------------------------------
{
    ...
    if(!getenv("I_Am_G4BinaryCascadeQ_Developer") )
    {
        ...
        {
            G4cerr << "You are trying to use G4BinaryCascadeQ with " <<definition->GetParticleName()<<" as projectile."<<G4endl;
            G4cerr << "G4BinaryCascadeQ should not be used for projectiles other than nucleons or pions."<<G4endl;
            G4cerr << "If you want to continue, please switch on the developer environment: "<<G4endl;
            G4cerr << "setenv I_Am_G4BinaryCascadeQ_Developer 1 "<<G4endl<<G4endl;
            throw G4HadronicException(__FILE__, __LINE__, "G4BinaryCascadeQ - used for unvalid particle type - Fatal");
        }
    }
...
//----------------------------------------------------------------------------
G4ReactionProductVector * G4BinaryCascadeQ::Propagate(
        G4KineticTrackVector * secondaries, G4V3DNucleus * aNucleus)
//----------------------------------------------------------------------------
{
    G4ping debug("debug_G4BinaryCascadeQ");
#ifdef debug_BIC_Propagate
    G4cout << "G4BinaryCascadeQ Propagate starting -------------------------------------------------------" <<G4endl;
#endif
...
#ifdef debug_G4BinaryCascadeQ
       G4cout << "G4BinaryCascadeQ::Propagate: warning - high energy model failed energy conservation, returning unchanged high energy final state" << G4endl;
#endif
    ...
    {
#ifdef debug_G4BinaryCascadeQ
        G4cerr << "G4BinaryCascadeQ: Warning, have active particles at end" << G4endl;
    ...
    {
            #ifdef debug_G4BinaryCascadeQ
              	  G4cerr << "G4BinaryCascadeQ-Warning: negative excitation energy ";
...
//----------------------------------------------------------------------------
G4double G4BinaryCascadeQ::GetExcitationEnergy()
//----------------------------------------------------------------------------
{

    // get A and Z for the residual nucleus
#if defined(debug_G4BinaryCascadeQ) || defined(debug_BIC_GetExcitationEnergy)
    ...
    {
#ifdef debug_G4BinaryCascadeQ
        G4cout << "G4BinaryCascadeQ::GetExcitationEnergy(): Warning - invalid nucleus (A,Z)=("
...
//----------------------------------------------------------------------------
void G4BinaryCascadeQ::BuildTargetList()
//----------------------------------------------------------------------------
{

    if(!the3DNucleus->StartLoop())
    {
        //    G4cerr << "G4BinaryCascadeQ::BuildTargetList(): StartLoop() error!"
        //	   << G4endl;
        return;
    }
    ...
    {
        G4cerr << "G4BinaryCascadeQ::BuildTargetList(): Fatal Error - invalid nucleus (A,Z)=("
                << currentA << "," << currentZ << ")" << G4endl;
...
#ifdef debug_BIC_BuildTargetList
    G4cout << "G4BinaryCascadeQ::BuildTargetList():  nucleus (A,Z)=("
...
//----------------------------------------------------------------------------
G4bool  G4BinaryCascadeQ::BuildLateParticleCollisions(G4KineticTrackVector * secondaries)
//----------------------------------------------------------------------------
{
...
#ifdef debug_G4BinaryCascadeQ
      if ( ! success ) {
         G4cout << "G4BinaryCascadeQ::BuildLateParticleCollisions(): Proj.e / initial excitation: " << theProjectile4Momentum.e() << " / " << excitation << G4endl;
...
//----------------------------------------------------------------------------
G4ReactionProductVector *  G4BinaryCascadeQ::DeExcite()
//----------------------------------------------------------------------------
{
...
//---QY add binding energy bias
         G4double px2     = pFragment.px();
         G4double py2     = pFragment.py();
         G4double pz2     = pFragment.pz();
         G4double e2      = pFragment.e();
         G4double mass2   = e2*e2-(px2*px2+py2*py2+pz2*pz2);
         if(mass2>0)mass2 = std::sqrt(mass2);
         G4double mass0   = fragment->GetGroundStateMass();
         G4double excmass = mass2-mass0;
         if(excmass>0){
           mass2=mass0+fEexc*excmass;
           e2=std::sqrt(px2*px2+py2*py2+pz2*pz2+mass2*mass2);
         }
         G4LorentzVector fourMomentum2 = G4LorentzVector(px2,py2,pz2,e2);
         fragment->SetMomentum (fourMomentum2);
//------
...
//----------------------------------------------------------------------------
G4ReactionProductVector *  G4BinaryCascadeQ::DecayVoidNucleus()
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
G4ReactionProductVector * G4BinaryCascadeQ::ProductsAddFinalState(G4ReactionProductVector * products, G4KineticTrackVector & fs)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
G4ReactionProductVector * G4BinaryCascadeQ::ProductsAddPrecompound(G4ReactionProductVector * products, G4ReactionProductVector * precompoundProducts)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void  G4BinaryCascadeQ::FindCollisions(G4KineticTrackVector * secondaries)
//----------------------------------------------------------------------------
{
            ...
            i != secondaries->end(); ++i)
    {
        for(std::vector<G4BCAction *>::iterator j = theImR.begin();
                j!=theImR.end(); j++)
        {
            //      G4cout << "G4BinaryCascadeQ::FindCollisions: at action " << *j << G4endl;
...
//----------------------------------------------------------------------------
void  G4BinaryCascadeQ::FindDecayCollision(G4KineticTrack * secondary)
//----------------------------------------------------------------------------
{
    const std::vector<G4CollisionInitialState *> & aCandList
...
//----------------------------------------------------------------------------
void  G4BinaryCascadeQ::FindLateParticleCollision(G4KineticTrack * secondary)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
G4bool G4BinaryCascadeQ::ApplyCollision(G4CollisionInitialState * collision)
//----------------------------------------------------------------------------
{
    G4KineticTrack * primary = collision->GetPrimary();

#ifdef debug_BIC_ApplyCollision
    G4cerr << "G4BinaryCascadeQ::ApplyCollision start"<<G4endl;
    theCollisionMgr->Print();
...
//       }
    ...
    if (!lateParticleCollision)
    {
       if( !products || products->size()==0 || !CheckPauliPrinciple(products) )
       {
          ...
          G4cerr << "G4BinaryCascadeQ::ApplyCollision blocked"<<G4endl;
#endif
          Success=false;
       }
    ...
    {
        G4cout << "G4BinaryCascadeQ: Error in Balancing: " << G4endl;
...
//----------------------------------------------------------------------------
G4bool G4BinaryCascadeQ::Absorb()
//----------------------------------------------------------------------------
{
    ...
    for(iter = absorbList.begin(); iter != absorbList.end(); ++iter)
    {
        ...
        if(!absorber.FindAbsorbers(*kt, theTargetList))
            throw G4HadronicException(__FILE__, __LINE__, "G4BinaryCascadeQ::Absorb(): Cannot absorb a particle.");
        ...
        while(!CheckPauliPrinciple(products) && --maxLoopCount>0)   /* Loop checking, 31.08.2015, G.Folger */
        {
            ...
            if(!absorber.FindProducts(*kt))
                throw G4HadronicException(__FILE__, __LINE__,
                        "G4BinaryCascadeQ::Absorb(): Cannot absorb a particle.");
        }
		  if ( --maxLoopCount < 0 ) throw G4HadronicException(__FILE__, __LINE__, "G4BinaryCascadeQ::Absorb(): Cannot absorb a particle."); 
        // ------ debug
...
//----------------------------------------------------------------------------
G4bool G4BinaryCascadeQ::Capture(G4bool verbose)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
G4bool G4BinaryCascadeQ::CheckPauliPrinciple(G4KineticTrackVector * products)
//----------------------------------------------------------------------------
{
    G4int A = the3DNucleus->GetMassNumber();
    G4int Z = the3DNucleus->GetCharge();
...
//----------------------------------------------------------------------------
void G4BinaryCascadeQ::StepParticlesOut()
//----------------------------------------------------------------------------
{
    G4int counter=0;
    G4int countreset=0;
	                                                    ...
	                                                    // if countreset reaches limit, there is a break from while, see below.
    {
        ...
        for(i = theSecondaryList.begin(); i != theSecondaryList.end(); ++i)
        {
            ...
            if( kt->GetState() == G4KineticTrack::inside )
            {
                ...
                if ( ! intersect );
                {
                    ...
                    throw G4HadronicException(__FILE__, __LINE__, "G4BinaryCascadeQ::StepParticlesOut() particle not in nucleus");
                }
#endif
        ...
        {
#ifdef debug_G4BinaryCascadeQ
            G4cerr << "G4BinaryCascadeQ.cc: Warning - aborting looping particle(s)" << G4endl;
...
//----------------------------------------------------------------------------
G4double G4BinaryCascadeQ::CorrectShortlivedPrimaryForFermi(
...
//----------------------------------------------------------------------------
G4bool G4BinaryCascadeQ::CorrectShortlivedFinalsForFermi(G4KineticTrackVector * products,
...
//----------------------------------------------------------------------------
void G4BinaryCascadeQ::CorrectFinalPandE()
//----------------------------------------------------------------------------
//
...
//
{
    ...
    {
#ifdef debug_BIC_CorrectFinalPandE
        G4cout << "G4BinaryCascadeQ::CorrectFinalPandE() : error! " << G4endl;
...
//----------------------------------------------------------------------------
void G4BinaryCascadeQ::UpdateTracksAndCollisions(
        //----------------------------------------------------------------------------
        G4KineticTrackVector * oldSecondaries,
...
//----------------------------------------------------------------------------
G4bool G4BinaryCascadeQ::DoTimeStep(G4double theTimeStep)
//----------------------------------------------------------------------------
{
    ...
    G4cerr <<"G4BinaryCascadeQ::DoTimeStep: enter step="<< theTimeStep
    ...
    //  G4cerr <<"G4BinaryCascadeQ::DoTimeStep: exit "<<G4endl;
...
//----------------------------------------------------------------------------
G4KineticTrackVector* G4BinaryCascadeQ::CorrectBarionsOnBoundary(
...
//----------------------------------------------------------------------------
{
    ...
    if (out->size())
    {
        ...
        if (currentA < 0 )
        {
            G4cerr << "G4BinaryCascadeQ - secondaryBarions_out,secondaryCharge_out " <<
            ...
            G4cerr << "G4BinaryCascadeQ - currentA, currentZ " << currentA << " " << currentZ << G4endl;
            throw G4HadronicException(__FILE__, __LINE__, "G4BinaryCascadeQ::CorrectBarionsOnBoundary() - fatal error");
        }
        G4double mass_final=GetIonMass(currentZ,currentA);
        G4double correction= mass_initial - mass_final - secondaryMass_out;
        // G4cout << "G4BinaryCascadeQ::CorrectBarionsOnBoundary() total out correction: " << correction << G4endl;
...
G4Fragment * G4BinaryCascadeQ::FindFragments()
//----------------------------------------------------------------------------
{
...
G4LorentzVector G4BinaryCascadeQ::GetFinal4Momentum()
//----------------------------------------------------------------------------
...
//  ie. difference of (initial state(primaries+nucleus) - final state) particles, ignoring remnant nucleus
{
    ...
    if(final4Momentum.e()> 0 && (final4Momentum.vect()/final4Momentum.e()).mag()>1.0 && currentA > 0)
    {
        ...
        G4cerr << G4endl;
        G4cerr << "G4BinaryCascadeQ::GetFinal4Momentum - Fatal"<<G4endl;
        G4KineticTrackVector::iterator i;
...
//----------------------------------------------------------------------------
G4LorentzVector G4BinaryCascadeQ::GetFinalNucleusMomentum()
//----------------------------------------------------------------------------
{
...
#     ifdef debug_BIC_FinalNucleusMomentum
            G4cerr << "G4BinaryCascadeQ::GetFinalNucleusMomentum - Fatal"<<G4endl;
...
//----------------------------------------------------------------------------
G4ReactionProductVector * G4BinaryCascadeQ::Propagate1H1(
        //----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
G4ThreeVector G4BinaryCascadeQ::GetSpherePoint(
...
void G4BinaryCascadeQ::ClearAndDestroy(G4KineticTrackVector * ktv)
//----------------------------------------------------------------------------
{
    std::vector<G4KineticTrack *>::iterator i;
...
//----------------------------------------------------------------------------
void G4BinaryCascadeQ::ClearAndDestroy(G4ReactionProductVector * rpv)
//----------------------------------------------------------------------------
{
...
//----------------------------------------------------------------------------
void G4BinaryCascadeQ::PrintKTVector(G4KineticTrackVector * ktv, std::string comment)
//----------------------------------------------------------------------------
{
    if (comment.size() > 0 ) G4cout << "G4BinaryCascadeQ::PrintKTVector() " << comment << G4endl;
    if (ktv) {
        ...
        G4cout << "G4BinaryCascadeQ::PrintKTVector():No KineticTrackVector given " << G4endl;
    }
}
//----------------------------------------------------------------------------
void G4BinaryCascadeQ::PrintKTVector(G4KineticTrack * kt, std::string comment)
//----------------------------------------------------------------------------
{
    if (comment.size() > 0 ) G4cout << "G4BinaryCascadeQ::PrintKTVector() "<< comment << G4endl;
    if ( kt ){
        ...
        G4cout << "G4BinaryCascadeQ::PrintKTVector(): No Kinetictrack given" << G4endl;
    }
}


//----------------------------------------------------------------------------
G4double G4BinaryCascadeQ::GetIonMass(G4int Z, G4int A)
//----------------------------------------------------------------------------
{
    G4double mass(0);
    if ( Z > 0 && A >= Z )
    ...
    {
        G4cerr << "G4BinaryCascadeQ::GetIonMass() - invalid (A,Z) = ("
                << A << "," << Z << ")" <<G4endl;
        throw G4HadronicException(__FILE__, __LINE__, "G4BinaryCascadeQ::GetIonMass() - giving up");

    }
	 //  G4cout << "G4BinaryCascadeQ::GetIonMass() Z, A, mass " << Z << " " << A << " " << mass << G4endl;
    return mass;
}
G4ReactionProductVector * G4BinaryCascadeQ::FillVoidNucleusProducts(G4ReactionProductVector * products)
{
...
G4ReactionProductVector * G4BinaryCascadeQ::HighEnergyModelFSProducts(G4ReactionProductVector * products,
...
void G4BinaryCascadeQ::PrintWelcomeMessage()
{
    G4cout <<"Thank you for using G4BinaryCascadeQ. "<<G4endl;
}

//----------------------------------------------------------------------------
void G4BinaryCascadeQ::DebugApplyCollisionFail(G4CollisionInitialState * collision,
...
G4bool G4BinaryCascadeQ::CheckChargeAndBaryonNumber(G4String where)
{
...
//----------------------------------------------------------------------------
void G4BinaryCascadeQ::DebugApplyCollision(G4CollisionInitialState * collision,
        G4KineticTrackVector * products)
{
...
//----------------------------------------------------------------------------
G4bool G4BinaryCascadeQ::DebugFinalEpConservation(const G4HadProjectile & aTrack,
...
//----------------------------------------------------------------------------
G4bool G4BinaryCascadeQ::DebugEpConservation(const G4String where)
//----------------------------------------------------------------------------
{
    G4cout << where << G4endl;
...
//----------------------------------------------------------------------------
G4ReactionProductVector * G4BinaryCascadeQ::ProductsAddFakeGamma(G4ReactionProductVector *products )
//----------------------------------------------------------------------------
{
   //    else
//    {
