// $Id: TrRecon.h,v 1.73 2022/06/07 14:36:54 qyan Exp $ 
#ifndef __TrRecon__
#define __TrRecon__
...
///
///\class TrRecon
///\brief Procedures for the Tracker TrClusterR and TrRecHitR Reconstruction. TrTrackR Reconstruction was switched to TrReconQ starting from Pass7 in 2018.
///
...
///\date  2018/05/12 QY  Pass7 TrClusterR and TrRecHitR reconstruction
///\date  2022/06/04 QY  Pass8 TrClusterR and TrRecHitR reconstruction
///
...
//   static TrRecon*  Create(int recreate=0);
  ...
  /*!
    ...
    \param hist Histograms are filled flag  0=no 1 yes
  ...
  */
  ...
  int  BuildTrClusters(int rebuild=0, int flag=1); 
  /// Sort Signal Highest to Lowest
  template <class T>
  static bool CompSignal(const pair<double, T*> &a,const pair<double, T*> &b){return a.first>b.first;}
  // Reorder TrCluster container (higher signal first) && erase NCluster>nmaxcls  
  int  ReorderTrClusters(int nmaxcls);
  /// Builds the TrClusters in a buffer interval (cyclicity if !=0) 
  int  BuildTrClustersInSubBuffer(int tkid, int first, int last, int cyclicity = 0, double dShapeCut = -1);
  /// Builds the seed list in the buffer interval (cyclicity if !=0) 
  int  BuildSeedListInSubBuffer(int tkid, int first, int last, int cyclicity = 0, double dShapeCut = -1);
...
public:
  ...
  // Reorder TrRecHit container (0: no reordering, 1: probability, 2: y-signal, 3: x-signal) && erase NRecHits>nmaxhits
  int  ReorderTrRecHits(int nmaxhits,int type);
...
public:
  ...
  int CountTracks(int trstat = 0, int refit = 0);
