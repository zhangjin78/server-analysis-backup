//  $Id: GammaFit.h,v 1.2 2016/07/25 17:21:09 shaino Exp $
#ifndef __GammaFit__
#define __GammaFit__
...
///\date  2016/07/25 SH  He rigidity estimator
///
///$Date: 2016/07/25 17:21:09 $
///
///$Revision: 1.2 $
///
//////////////////////////////////////////////////////////////////////////
...
public:
 ...
 ~GammaFit() {}
  ...
  /// Fit rigidity for He hypothesis from TRD signal (fix array)
  /*
    \param[in]   len  Array of path length (cm) (N=20)
    \param[in]   amp  Array of amplitude  (ADC) (N=20)
    \param[out]  nh   Number of hits used
    \param[out]  csq  Chisquare/NDF
    \param[out]  dr   Estimated rigidity error (GV)
    \return           Estimated He rigidity (GV)
   */
  static Double_t HeFit(Double_t *len, Double_t *amp, Int_t &nh,
			Double_t &csq, Double_t &dr);

  /// Fit rigidity for He hypothesis from TRD signal
  /*
    \param[in]   n    Array size of lay, len and amp
    \param[in]   lay  Layer ID (0-19)
    \param[in]   len  Path length (cm)
    \param[in]   amp  Amplitude (ADC)
    \param[out]  nh   Number of hits used
    \param[out]  csq  Chisquare/NDF
    \param[out]  dr   Estimated rigidity error (GV)
    \return           Estimated He rigidity (GV)
   */
  static Double_t HeFit(Int_t n,
			Int_t *lay, Double_t *len, Double_t *amp,
			Int_t  &nh, Double_t &csq, Double_t &dr) {
    Double_t l[20];
    Double_t a[20];
    for (Int_t i = 0; i < 20; i++) a[i] = 0;
    for (Int_t i = 0; i <  n; i++)
      if (0 <= lay[i] && lay[i] < 20 &&
	  a[lay[i]] < amp[i]) { l[lay[i]] = len[i];
	                        a[lay[i]] = amp[i]; }
    return HeFit(l, a, nh, csq, dr);
  }

  /// Wrapper for Float_t
  static Double_t HeFit(Float_t  *len, Float_t  *amp, Int_t &nh,
			Double_t &csq, Double_t &dr) {
    Double_t l[20];
    Double_t a[20];
    for (Int_t i = 0; i < 20; i++) { l[i] = len[i]; a[i] = amp[i]; }
    return HeFit(l, a, nh, csq, dr);
  }

  /// Wrapper for Float_t
  static Double_t HeFit(Int_t n,
			Int_t *lay, Float_t  *len, Float_t *amp,
			Int_t  &nh, Double_t &csq, Double_t &dr) {
    Double_t l[20];
    Double_t a[20];
    for (Int_t i = 0; i < 20; i++) a[i] = 0;
  ...
  /// Main function for He PDF
  static Double_t HeFunc(Double_t *xp, Double_t *par);
