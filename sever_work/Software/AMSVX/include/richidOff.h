#ifndef __RICHIDOFF__
#define __RICHIDOFF__
...
#define GETRLRS(A1,A2,A3,A4) CCALLSFSUB4(GETRLRS,getrlrs,FLOAT,FLOAT,PFLOAT,PFLOAT,A1,A2,A3,A4)
  ...
  struct RichPMTGridInfo {
    RichPMTGridInfo()
      : x0(999999.9), y0(999999.9), x1(-999999.9), y1(-999999.9)
      , binwidthX(3.7), binwidthY(3.7)
      , nBinsX(0), nBinsY(0)
      , centerX(0), centerY(0)
      , hasGapX(false), gapBeginX(0.0), gapEndX(0.0), gapWidthX(0.0)
      , pmts_2D(0) {
    }
    ...
    bool hasGapX;
    double gapBeginX, gapEndX, gapWidthX;
    int** pmts_2D;

    int FindBinX(double x);
    int FindBinY(double x);
    int FindPMT(double x, double y);
  };
  ...
  private:
    ...
    static RichPMTGridInfo _grid_info[8];
  ...
  public:
    static void Init(const char *filename="RichDefaultCalibration.dat");             // Init geometry and kinds and so on
    ...
    static int FindGrid(geant x, geant y);
