...
public:
    ...
    int GetXePressure();
    float GetDefaultMCXePressure(){return DefaultMCXePressure;}
    static void SetDefaultMCXePressure(float xe){ DefaultMCXePressure=xe;return;}

    // Helper function to get MC defulat Calibration
    float GetDefaultMCGainCalibration(){return DefaultMCGainCalibration;}
    static void SetDefaultMCGainCalibration(float corr){ DefaultMCGainCalibration=corr;return;}
    ...
    int GetRigidityEstimate(float threshold, float rig_init, double& rig, double& rig_err);
...
private:
    ...
    vector<float> v_amp;
    vector<float> v_path;
    vector<float> v_layer;
    vector<float> v_xe;
    static TMinuit *gMinuit_RigidityEstimate;
    static void fcn_RigidityEstimate(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag);
    double EstimateRigidityFromLikelihood(Double_t *par);


    ClassDef(TrdKCluster,2)
