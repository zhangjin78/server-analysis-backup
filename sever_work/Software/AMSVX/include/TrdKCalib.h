...
public:
	...
	double timediff=asktime-Time_A;
        if(fabs(timediff)>3600)timediff=0;
	return Par[tubeid]+timediff*Rate[int(tubeid/16)]*Par[tubeid]/Gain[int(tubeid/16)];
    };
	...
	double timediff=asktime-Time_A;
        if(fabs(timediff)>3600)timediff=0;
	return Gain[Moduleid]+timediff*Rate[Moduleid];
    };
