// $Id: TrChargeLossDB.h,v 1.7 2017/06/26 21:35:13 oliva Exp $
...
class TrChargeLossTable : public TObject {
 ...
 public:
  ...
  //! Create an index (version*1000 + z*10 + type), also set default limits
   ...
   */
  static int CreateIndex(int type, int z, int ver = 0);
  //! Get type from index 
...
class TrChargeLossDB : public TObject {
 ...
 private:
  ...
  //! Array for even faster const lookup
  static TrChargeLossTable** fTrChargeLossArray;
...
#pragma omp threadprivate(fUsedVersion)
 public:
  ...
  //! Load the ver3 tables (MC) from directory
  bool   LoadDefaultTablesVer3(char* dirname = getenv("AMSDataDir"));
  //! Load the ver3 tables (MC.B1113) from directory
  bool   LoadDefaultTablesVer4(char* dirname = getenv("AMSDataDir"));
