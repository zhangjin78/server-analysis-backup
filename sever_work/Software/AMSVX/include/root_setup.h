//  $Id: root_setup.h,v 1.137 2022/12/16 08:43:20 qyan Exp $
...
virtual ~SlowControlR() {}
	 ...
	 */
int GetData(const char * elementname,unsigned int time, float frac, vector<double> &value , int imethod=1, const char *nodename="", int dt=-1, int st=-1);
...
/// @cond CALDOX
//! Tsyganenko-2005 Cutoff
/*!
\sa AMSSetupR
\author qyan@cern.ch davide.grandi@mib.infn.it davide.rozza@mib.infn.it 
 */
class Tsy05Cutoff{
protected:
  typedef map<pair<unsigned int, unsigned int>, TH2D *> hist_m;
  typedef hist_m::iterator hist_i;
  hist_m histcf[4][2];
public:
  TH2D  *gethistcf(int &stat,unsigned int xtime=0,int ifv=0,int icf=0);
/// get Tsy05 cutoff
/*!
  * @param[out] cfn  max cutoff for 25,30,35,40 degrees (GV) for neg+pos
  * @param[in] icf   0: Tsy05(Max-Secondary), 1: Tsy05(Min-Primary)
  * @param[in] xtime 0: 2011-2013 Total, >0: UTC time
  * @param[in] theta gtod (rad)
  * @param[in] phi gtod (rad)
  * @return >=0: success, =1: silver time, <=-2: error
*/
  int    getcutoff(float cfn[4][2],int icf=0,unsigned int xtime=0,float theta=0,float phi=0);
  Tsy05Cutoff();
  virtual ~Tsy05Cutoff() {}
  ClassDef(Tsy05Cutoff,1)
};
/// @endcond
...
protected:
	static TGraph *SAAedge;	///< outer boundaries of the SAA region
        ...
        static Tsy05Cutoff *tsy05cfhead;///< Tsy05Cutoff
public:
	...
	bool IsInSAA();				///< True if ISS in in SAA, false otherwize
        ...
        static Tsy05Cutoff *gettsy05cf(); ///< Tsy05Cutoff
         ///  BadRuns accessor
         /*!
        ...
        ///  MC Cite
        /*!
         * @param[in]  run !=0 search Cite Name by run, =0 search Cite Name by index
         * @param[out] index Cite Index 
         * return  Cite Name for MC Runs;
        */
        static const char *GetCite(unsigned int run,int &index); 
  
unsigned int run;  ///< run
unsigned int runtype;///< runtype
unsigned int evno;///< first event no in one second
unsigned int evnol;///< last event no in one second
float lf;  ///< life time
float cf[4][2];  ///< max Stoermer cutoff for 25,30,35,40 degrees (GV) for neg+pos
float cfi[4][2]; ///< max IGRF cutoff for 25,30,35,40 degrees (GV) for neg+pos
float cfl[2][2]; ///< lowest IGRF cutoff for 25,40 degrees (GV) for neg+pos
...
float mtrcl; ///< average trrawcluster number for one event
int good;    ///< 0 if good
/*!<
...
/*!
  * @param[out] cfn  max cutoff for 25,30,35,40 degrees (GV) for neg+pos
  * @param[in] icf  0: Stoermer, 1: IGRF(RTI), 12: IGRF-12, 22: IGRF-12+, 32: IGRF-12++, 3: Tsy05(Max-Secondary), 4: Tsy05(Min-Primary)
  * @return >=0: success, -1: failure, <=-2: error
  */
int    getcutoff(float cfn[4][2],int icf=1);
static int Version; ///< RTI Version id
/*!< 
  731: 2019-05 B1130(p7-std) (TTCS-Off) \n
  720: 2018-04 B1130(p7-std) \n
  711: 2019-05 B1130(p7) (TTCS-Off) \n
  700: 2018-04 B1130(p7) \n
  800: 2022-12 B1236(p8) latest+default \n
*/
static int Loadopt;//< load option
/// use latest RTI Version
/*!
 * @param[in] pass  7: B1130-p7, 8: B1236-p8
 * @return Version id
*/
static int UseLatest(int pass=8);
//---
 RTI():run(0),runtype(0),evno(0),evnol(0),lf(0),mphe(0),theta(0),phi(0),r(0),zenith(0),glat(-2),glong(-2),nev(0),nerr(0),ntrig(0),nhwerr(0),npart(0),mtrdh(0),mtrcl(0),good(-1),utime(0){
        for(int ifv=0;ifv<4;ifv++){
          for(int ipn=0;ipn<2;ipn++){cf[ifv][ipn]=cfi[ifv][ipn]=0;if(ifv<2)cfl[ifv][ipn]=0;}
        }
...
*/
 ...
 public:
   ...
   DSPError(const DSPError &o){
     TimeStart=o.TimeStart;TimeEnd=o.TimeEnd;
     Nodes_000_01F=o.Nodes_000_01F;Nodes_020_03F=o.Nodes_020_03F;Nodes_040_05F=o.Nodes_040_05F;Nodes_060_07F=o.Nodes_060_07F;
     Nodes_080_09F=o.Nodes_080_09F;Nodes_0A0_0BF=o.Nodes_0A0_0BF;Nodes_0C0_0DF=o.Nodes_0C0_0DF;Nodes_0E0_0FF=o.Nodes_0E0_0FF;
     Nodes_100_11F=o.Nodes_100_11F;Nodes_120_13F=o.Nodes_120_13F;Nodes_140_15F=o.Nodes_140_15F;Nodes_160_17F=o.Nodes_160_17F;
     Nodes_180_19F=o.Nodes_180_19F;Nodes_1A0_1BF=o.Nodes_1A0_1BF;Nodes_1C0_1DF=o.Nodes_1C0_1DF;Nodes_1E0_1FF=o.Nodes_1E0_1FF;
   }
   virtual ~DSPError() {}
 ...
 */
 ...
 void CorrectBugs(unsigned int version,unsigned int run); 
 private:
 //--------DSP Errors-----------------------
...
ClassDef(AMSSetupR,28)       //AMSSetupR
#ifdef VERSION6
#else
#pragma omp threadprivate(fgIsA)
#endif
