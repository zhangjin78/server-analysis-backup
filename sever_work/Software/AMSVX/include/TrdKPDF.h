...
//        f_func->SetNpx(4000);
    ...
    void GetPar_Smooth(float Pmom, float Length, int Layer, float Press){
        //        cout<<"KPDF, GetPar: "<<Pre_Layer<<endl;
        Pre_Momentum=Pmom;
        Pre_PathLength=Length;
        Pre_Xe=Press*1000;
        ...
        if(particle==0)GetPar_Smooth_Electron(Pmom,Length,Layer,Press);
        else if(particle==1)GetPar_Smooth_Proton(Pmom,Length,Layer,Press);
        else if(particle==2)GetPar_Smooth_Helium(Pmom,Length,Layer,Press);
    ...
    void GetPar_Smooth_Proton(Double_t Pmom, Double_t Length, Int_t Layer, Double_t Press);
    void GetPar_Smooth_Electron(Double_t Pmom, Double_t Length, Int_t Layer, Double_t Press);
    void GetPar_Smooth_Helium(Double_t Pmom, Double_t Length, Int_t Layer, Double_t Press);
    ...
    Double_t GetNormalization(double start=0);
    ...
    TH1F* DrawFunc(){
      TString s_name=Form("h_TrdKPDF_M%.0f_P%.2f_Xe%.0f_L%i_%s",Pre_Momentum,Pre_PathLength,Pre_Xe,Pre_Layer,s_particle.Data());
        cout<<s_name<<endl;

        s_name.ReplaceAll(".","_");
        TF1 *f = new TF1(s_name,this,15,3015,0,"TrdKPDF");
        f->SetNpx(3000);
...
//        Norm=GetNormalization(Normalization_Start);
    ...
    TSpline3 *sp_m[16];
    double EvalSpline(TSpline3 *sp_m, double pmom){
        if(!sp_m)return 0;
        if(pmom>sp_m->GetXmax())pmom=sp_m->GetXmax();
        return sp_m->Eval(pmom);
    };
