...
#pragma omp critical (TrInnerDzDBGetHead)
{
    if(regen && Head){ Head->ClearAndSetTDVSwap(1);delete Head; Head=0;}//clean pos and TDV
    if(!Head)
  ...
  /// Clear the DB content, set TDVSwap[0] to tpar
  void ClearAndSetTDVSwap(float tpar=0){pos.clear(); TDVSwap[0]=tpar; return;}
...
#pragma omp critical (DzElemAddentry)
{
    DzElem el(dz);
    pos[Timeid]=el;
    std::cout<<"TrInnerDZDB-I-AddEntry "<<Timeid<<std::endl;
}
    return;
  }
