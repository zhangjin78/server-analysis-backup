//  $Id: TrFit.h,v 1.57 2022/06/04 22:40:31 qyan Exp $
#ifndef __TrFit__
#define __TrFit__
...
/// Another genfit track propogation routine was integrated by Q.Yan.
/// For the purpose of, for example, interpolation, TrProp can be used.
///
...
/// GEANE Kalman (DAF) filter: Fitting method imported from GenFit package
/// GBLFITTERAMS (GBLFITTER) fitter: General broken lines (Gbl, NIM A 673 (2012) 107–110)
...
public:
  ...
  /// Kick the direction by (theta, phi) (rad)
  void Kick(double theta, double phi);
  ...
  /// interpolation to a given plane, <=-1000 interpolation failed: direction>0 (downward going),direction<0(upward going)
  /*!
   ...
   */
  double Interpolate(AMSPlane &pl, double &rigidity, int eloss=0, int direction=1);
public:
...
public:
  ...
  enum EMethods{ LINEAR, CIRCLE, SIMPLE, ALCARAZ, CHOUTKO, CHIKANIANC,
		 CHIKANIANF, GAUSBF, GEANE_DAF, GEANE_Kalman, GFGBL, GBLFITTER, GBLFITTERAMS};
...
protected:
  ...
  double _xchi2[LMAX];   ///< Fitting chisquare in X (position)
  double _ychi2[LMAX];   ///< Fitting chisquare in Y (position)
  double _xkchi2[LMAX];   ///< Fitting chisquare in XZ (angle) i=0 (0), i=1 (kink betwen Hit 0-1),...
  double _ykchi2[LMAX];   ///< Fitting chisquare in YZ (angle) i=0 (0), i=1 (kink betwen Hit 0-1),...
  double _chiL[LMAX];
...
public:
  ...
  int    GetNdof  (void) const { return _ndof;   }
  ...
  double GetChisq   (void) const { return _chisq; }
   ///  p-value of fitting, suggest to use p-value instead of chisq to eval fitting quality 
  double GetPVal    (int ixy=2) const;
  double GetErrRinv (void) const { return _errrinv;  }

  /// layer 1-9
  int iHit(int layer);
  /// layj 1-9
  int iHitJ(int layerj);
  ...
  /// Chisquare for x position, pleae note YTotalChi2=GetYChi2(position)+GetYKChi2(angle)
  double GetXChi2(int i)const {return (0 <= i && i < LMAX)? _xchi2[i] : 0;}
  /// Chisquare for y position, pleae note YTotalChi2=GetYChi2(position)+GetYKChi2(angle)
  double GetYChi2(int i)const {return (0 <= i && i < LMAX)? _ychi2[i] : 0;}
  /// Chisquare for xz angle: i=0 (0), i=1 (kink betwen Hit 0-1), i=2 (kink betwen Hit 1-2),...
  double GetXKChi2(int i)const {return (0 <= i && i < LMAX)? _xkchi2[i] : 0;}
  /// Chisquare for yz angle: i=0 (0), i=1 (kink betwen Hit 0-1), i=2 (kink betwen Hit 1-2), ...
  double GetYKChi2(int i)const {return (0 <= i && i < LMAX)? _ykchi2[i] : 0;}

  /// Estimate external layer (L1 or L9) chisquare: support Kalman and Gbl algorithms only
  /*!
   * \param[in] iex     0 L1, 1 L9 
   * \param[in] ixy     0 X, 1 Y, 2 X+Y
   * \return            Chisquare including position and angle (kink)
   */
  double GetExtLayerChi2(int iex,int ixy);
  AMSPlaneM* GetHit(int i) {return (0 <= i && i < int(_hits.size()))? &_hits[i] : 0;}
  ...
  /// Add a new hit with positon from plane 
  int Add(AMSPlaneM plm,int at = -1); 

  /// resort hits from measurement z highest to lowest
  int SortHits();

  /// Fill dummyx using linear fit, only actived when ex=-1
  int FillDummyx();
  ...
  /*!
   ...
   \param fixrig fix the rigidity for the fit (available with ALCARAZ/GBLFITTER/GBLFITTERAMS algo)
   \param uiter  fitting iteration number, 0:default, -1:1 iteration, -2: 2 iterations 
  */
  double DoFit(int method = CHOUTKO, int mscat = 1, int eloss = 1,
	       float charge = 1, float mass = Mproton, float beta = 0,float fixrig=0,int uiter=0);
  ...
  /// Build fitTrack from hit
  int BuildfitTrack(double mass,double charge,int seedfit=1,float fixrig=0);

  /// GEANE Kalman (DAF) filter
  double KalmanFit(double mass,double charge,int mscat,int eloss,int method);
 
  /// General broken lines fitter (NIM A 673 (2012) 107–110)
  double GblFit(double mass,double charge,int mscat,int eloss,int method,float fixrig=0,int uiter=0);  

  /// General broken lines scaning AMS materials
  int GblScanMat(double mass,double charge,double seedrig,double seedpos[3], double seeddir[3],double rad[20],double fds[20][2],double ftheta[20][2],double fscatmd[20][2]);
  ...
  /// Estimate layer number [0:trconst::maxlay-1] from the hit Z-coodinates (1-9)
  int GetLayer(double z);

  ///Estimate layer number J (1-9)
  int GetLayerJ(double z);
  ...
  /// Interpolation onto plane with GEANE Kalman (DAF) filter
  /*!
   * \param[in,out] pl        destination Plane, call pl.getP() and pl.getD() to get track position and direction (intersection point with the Plane) in global coordinate system, or call pl.getPLocal() and pl.getDLocal() to get them in local (Plane) coordinate system
   * \param[out]    rigidity  Track rigidity at the Plane (if interpolation failed, rigidity=0)
   * \param[in]     ustate    -1 last forward state, 1 last backward state, 0 intermediate forward and backward combined
   * \param[in]     ulength   0: do not calculate path length (fast), 1: calculate path length (slow)
   * \return                  Path length between Z=P0z(usually 0) and the Plane, <=-1000 interpolation failed
   */
  double InterpolateKalman(AMSPlane &pl, double &rigidity, int ustate=1, bool ulength=0);

  /// Interpolation onto Z=const. plane with GEANE Kalman (DAF) filter
  /*!
   * \param[in]  zpl      Plane position (Z=zpl)
   * \param[out] pnt      Track position  at Z=zpl
   * \param[out] dir      Track direction at Z=zpl
   * \param[out] rigidity Rigidity at Z=zpl (if interpolation failed, rigidity=0)
   * \param[in]  ustate   -1 last forward state, 1 last backward state, 0 intermediate forward and backward combined
   * \param[in]  ulength  0: do not calculate path length (fast), 1: calculate path length (slow)
   * \return              Path length between Z=P0z(usually 0) and Z=zpl (ulength=0), <=-1000 interpolation failed
   */ 
  double InterpolateKalman(const double zpl, AMSPoint &pnt, AMSDir &dir, double &rigidity, int ustate=1, bool ulength=0);

  /// Interpolation onto plane with General broken lines (Gbl) fitter
  /*!
   * \param[in,out] pl    destination Plane, call pl.getP() and pl.getD() to get track position and direction (intersection point with the Plane) in global coordinate system, or call pl.getPLocal() and pl.getDLocal() to get them in local (Plane) coordinate system
   * \param[in]  ulength  0: do not calculate path length (fast), 1: calculate path length (slow)
   * \return              Path length between Z=P0z(usually 0) and the Plane, <=-1000 interpolation failed
   */
  double InterpolateGbl(AMSPlane &pl, bool ulength=0);

  /// Interpolation onto Z=const. plane with General broken lines (Gbl) fitter
  /*!
   * \param[in]  zpl      Plane position (Z=zpl)
   * \param[out] pnt      Track position  at Z=zpl
   * \param[out] dir      Track direction at Z=zpl
   ...
   * \return              Path length between Z=P0z(usually 0) and Z=zpl (ulength=0), <=-1000 interpolation failed
   */
  double InterpolateGbl(const double zpl, AMSPoint &pnt, AMSDir &dir,bool ulength=0);
...
public:
  ...
  genfit::AbsKalmanFitter* kalmanfitter;
  genfit::AbsFitter* gblfitter;
  int kalmanmethod;
  int gblmethod;
  ClassDef(TrFit, 4);
};
