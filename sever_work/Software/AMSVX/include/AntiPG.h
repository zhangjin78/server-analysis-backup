...
access with: \n
  ...
  version 12 is the last one (default)\n
*/
class AntiRecoPG {
...
#pragma omp threadprivate (head)
 ...
 void clear();
    ...
    //! Get a calibration version.
    int GetCalibVersion() const {return NVER;}
  ...
  */
 static AntiRecoPG* gethead(int version=12, int Istolerant=0);
 ...
 */
  ...
  */
  ...
  void InitCal(int run_type = 1, int VERNUM = 12);
  ...
  */
  int LoadCaliFile(int VERNUM = 12, int iforce = 0);
  /*! load the static calibrations from a file\n
  iforce != 0 force the file to be reloaded
  */
  int LoadHistoCaliFile(int VERNUM = 12, int iforce = 0);

  //! version NUMBER default is 12 that point to the last one
  int NVER;
