...
111 FORMAT(' compiled with pgi ',i0,'.',i0,'.',i0)
    ...
    IF (ncache < 0) THEN
        ...
        IF (scache>0) ncache=ncache*scache;
    ENDIF
    rows=6; cols=mthrdr
...
SUBROUTINE peread(more)
    ...
    !!!$OMP THREADPRIVATE(i, iact, ierrc, ioffp, ios, ithr, jfile, jrec, k, kfile, l, lun, mpri, n, noff, noffs, nbuf)
...
#ifdef READ_C_FILES
                !!!$OMP CRITICAL
                !!WRITE(*,*) 'ithr=',ithr,lun,kfile,nfilf,jfile,keepOpen
                !!!$OMP END CRITICAL
                !!!$OMP CRITICAL
    ...
    DO k=2,nthr
        ...
        IF(nbuf>0) THEN
          !$OMP PARALLEL DO SHARED(readBufferPointer,nrd,ioffp,nbuf) PRIVATE(l)
          DO l=1,nbuf
...
#endif        
        !!! parallelize record loop
        !!!$OMP PARALLEL DO SHARED(numReadBuffer) &
        !!!$OMP  PRIVATE(ibuf,nerr,ist,nst,ja,jb,jsp,j,itgbi) &
        !!!$OMP  REDUCTION(+:nbad) &
        !!!$OMP  SCHEDULE(DYNAMIC,ICHUNK)
        DO ibuf=1,numReadBuffer ! buffer for current record
...
1177            FORMAT(i9,3I13,f10.2,f11.6)
    ...
    END DO
    ncachr=NINT(REAL(ncache,mpd)*fcache(1),mpl) ! read cache
    !     define read buffer
    ...
    ncachi=NINT(REAL(ncache,mpd)*fcache(2),mpl) ! index cache
...
SUBROUTINE intext(text,nline)
    ...
    IF(keyb /= 0) THEN
        ...
        mat=matint(text(keya:keyb),keystx,npat,ntext) ! comparison
        IF(100*mat >= 80*max(npat,ntext)) THEN ! 80% (symmetric) matching
            IF (nums > 0.AND.dnum(1) >= 0.) scache=NINT(dnum(1),mpi) ! cache size, <0 keeps default
