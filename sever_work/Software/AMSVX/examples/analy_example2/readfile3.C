#ifndef _USEREADFILE_
#define _USEREADFILE_
#include <cstdio>
#include <string>
#include "root_RVSP.h"
#include "amschain.h"
...
//---Search List
int SearchList(const char *listd,string fname,bool forcesearch=0){//0 not found, 1 fond in list, -1 Error

    if(listd==0)return 0;

    std::string listds=listd;
    if(listds.size()>=2){//remove '' or ""
      if((listds[0]=='\''&&listds[listds.size()-1]=='\'')||(listds[0]=='\"'&&listds[listds.size()-1]=='\"')){
        listds=listds.substr(1,listds.size()-2);
      }
    }
    if(listds.size()==0)return 0;//empty string
...
//---Now Search 
     static map <string,unsigned int > runlist;//will be reset if use forcesearch=1 
     static int pry=0,prm=0,prd=0;//will be reset if use forcesearch=1
     time_t tn=run;
     tm *tmp=gmtime(&tn);
///-Now Time Load
     if((tmp->tm_year!=pry||tmp->tm_mon!=prm||tmp->tm_mday!=prd||forcesearch)){//Change
         char  nfile[256],nfile1[1000];
         strftime(nfile,80,"%Y_%j",tmp);
         sprintf(nfile1,"%s/Run_%s-24H.cvs",listds.c_str(),nfile);
    ...
    if(runit!=runlist.end())found=1;//found in list
    return found;
}
   ...
   for(int iw=0;iw<2;iw++){
     ...
     for(it=runlistn.begin();it!=runlistn.end();it++){
       ...
       if((tmp->tm_year!=pry||tmp->tm_mon!=prm||tmp->tm_mday!=prd)){//Change
         ...
         pry=tmp->tm_year; prm=tmp->tm_mon; prd=tmp->tm_mday;
         strftime(nfile,80,"%Y_%j",tmp);
         if(iw==0)sprintf(nfile1,"%s/Run_%s-24H.cvs",listds.c_str(),nfile);
         else     sprintf(nfile1,"%s/RunN_%s-24H.cvs",listds.c_str(),nfile);
         fp=fopen(nfile1,"a+");
...
//------
int readfile(vector<string> &runlist, TChain &ch,int nroot=-1,int mode=0,int castype=0){

   int cont=0;
   for(unsigned int i=0;i<runlist.size();i++){
      string ifname=runlist[i];
      ...
      if(fond!=string::npos){
         ifname="root://eosams.cern.ch/"+ifname;
      }
...
//        ifname+="?svcClass=amsuser";
      ...
      if(fond!=string::npos){
         ifname="root://eosams.cern.ch/"+ifname;
//         ifname="root://eosams02/"+ifname;
      }
//--Not Process
      if(otxt!=0){
        int found=SearchList(otxt,ifname,forcesearch);//clean cache
        if(found>=0){forcesearch=0;}
        if(found==1){//found in list
...
//---CP
   ...
   struct stat st_buf;
   if((sofilecp[sofilecp.size()-1]=='/')||(stat(sofilecp.c_str(),&st_buf)==0&&S_ISDIR(st_buf.st_mode))){//ofilecp is a directory
     if((sofilecp[sofilecp.size()-1]!='/'))sofilecp+='/';
     string sofile=ofile;
...
//     if     (fonde!=string::npos)lsopt="/afs/cern.ch/project/eos/installation/0.3.15/bin/eos.select ls";
     ...
     if(cperror==0){cout<<"success-lsopt="<<lsopt<<" cperror="<<cperror<<endl;break;}
     cout<<"failed-lsopt="<<lsopt<<" cperror="<<cperror<<endl;
     if(icp>=ncp)break;
     sleep(10);
