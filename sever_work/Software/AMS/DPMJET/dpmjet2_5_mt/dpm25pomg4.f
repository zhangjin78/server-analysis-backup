...
C!$OMP THREADPRIVATE (/POLMN/)
      COMMON /POLMN1/ PLMNEE(0:MXPA25,0:MXPU50,0:MXPA13,28)
C!$OMP THREADPRIVATE (/POLMN1/)
      DATA IUNIT/37/
C
C!$OMP THREADPRIVATE (IUNIT)
      CHARACTER*80 INIDAT
C
C$  Statement : setenv INIDAT /nfs/hpmac1/macro02/dpmjet/POMTAB_06.DAT 
C$  in the job file is required
C
C      CALL GETENV('INIDAT',INIDAT)
CC     OPEN(IUNIT,FILE=INIDAT,STATUS='UNKNOWN',ERR=99)
C        OPEN(UNIT=IUNIT,FILE='pomtab.dat'
C     *  ,STATUS='UNKNOWN',ERR=99)
Cvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
      CHARACTER DPMDIRNAME*400, POMFILENAME*400
      CALL GETENV('G4DPMJET2_5DATA',DPMDIRNAME)
C      d='     '
      POMFILENAME = DPMDIRNAME(1:LEN_TRIM(DPMDIRNAME))//
     &'/pomtab.dat'
      if(IPOMTA.Eq.1)
     + OPEN(UNIT=IUNIT,FILE=POMFILENAME,STATUS='OLD',ERR=99)
C^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
C
      IF (IPOMTA.EQ.0) THEN
        DO 1 II=1,NESTEP
...
************************************************************************
*
*         POMDI,SIGMASDPM,SIGSHD,PRBLM0..9,SAMPLM,SIGMA1
*
...
************************************************************************
*
*         POMDI,SIGMASDPM,SIGSHD,PRBLM0..9,SAMPLX,SIGMA1
*
*                  routines called by code word
*                            SIGMAPOM :
*
...
* *** /OUTLEV/ controls output level for POMDI and parton X distribution
!$OMP THREADPRIVATE (/HISTOO/)
      COMMON /OUTLEV/IOUTPO,IOUTPA,IOUXEV,IOUCOL
* --- used only in SIGMAPOM-routines
* *** /POMPAR/ contains pomeron parameters used in some options
*     ALFA,ALFAP,A,BH,C,BS are soft Pomeron parameters chosen in SIGMASDPM
*     appearing in SIGMASDPM, PRBLM..,  AK is K-factor
!$OMP THREADPRIVATE (/OUTLEV/)
      COMMON/POMPAR/ALFA,ALFAP,A,C,AK
