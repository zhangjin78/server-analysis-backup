...
C      FUNCTION RNDMDPM(I)                     : GENERATOR
...
C#######################################################################
C===========================================================
C     DOUBLE PRECISION FUNCTION RNDMDPM(VDUMMY)
...
!$OMP THREADPRIVATE (U)
      ...
      DO 10 II1 = 1,20000
   10 XX = RNDMDPM_DUMMY(V)
      SD        = 0
      DO 20 II2 = 1,6
        X(II2)  = 4096.D0*(4096.D0*RNDMDPM_DUMMY(V))
        D(II2)  = X(II2)-U(II2)
   20 SD = SD+D(II2)
...
C
   ...
   10 R = RNDMDPM(R)
      ...
      YYY=LOG(RNDMDPM(Y)+1.0D-9)/F
      IF(ABS(YYY).GT.50.0D0)                                   GO TO 20
      Y = EXP(YYY)
      IF(LOG(RNDMDPM(R)+1.0D-9).GT.-Y)                         GO TO 10
                                                               GO TO 40
   20 Y = 0
                                                               GO TO 50
   30 Y = 1.-LOG(RNDMDPM(Y)+1.0D-9)
      IF(RNDMDPM(R).GT.Y**(F-1.D0))                            GO TO 10
...
*===daltra=============================================================*
*
      SUBROUTINE DALTRA(GA,BGX,BGY,BGZ,PCX,PCY,PCZ,ECC,P,PX,PY,PZ,E)
...
!$OMP THREADPRIVATE (/R48ST1/)
      PARAMETER (MODCNS=1000000000)
      SAVE CD, CM, TWOM24,  ZERO, ONE, NTOT, NTOT2, IJKL
!$OMP THREADPRIVATE (CD, CM, TWOM24,  ZERO, ONE, NTOT, NTOT2, IJKL)
...
*===  rndm  ===========================================================*
*
      DOUBLE PRECISION FUNCTION RNDMDPM_DUMMY (RDUMMY)
...
*=== End of function rndm =============================================*
      ...
      DOUBLE PRECISION FUNCTION RNDMDPM (RDUMMY)
C      CALL AMSDPMJET_RNDM(RNDNUM);
C      REAL RNDNUM (1)
C      CALL GRNDM(RNDNUM,1)
C      RNDMDPM=RNDNUM(1)
c       RNDMDPM=RNDM(RDUMMY);
       CALL RNDMG4(RNDMDPM);
C      write(*,*)'RNDM=',RNDMDPM
      RETURN
      END
