...
*KEND.
!$OMP THREADPRIVATE (/ABRSV/)
      LOGICAL LSEADI
      COMMON /SEADIQ/LSEADI
!$OMP THREADPRIVATE (/SEADIQ/)
      COMMON/RECOM/IRECOM
!$OMP THREADPRIVATE (/RECOM/)
      COMMON/DIQUAX/AMEDD,IDIQUA,IDIQUU
!$OMP THREADPRIVATE (/DIQUAX/)
      COMMON /XSVTHR/ XSTHR,XVTHR,XDTHR,XSSTHR
!$OMP THREADPRIVATE (/XSVTHR/)
...
C     XXSEAM=1.0 - XVTHR*(1.D0+RNDMDPM(V1)) - XDTHR*(1.D0+RNDMDPM(V2))
C    *             -0.01*(1.D0+5.D0*RNDMDPM(V3))
C                                      28.4.97 test
      XXSEAM=1.0 - XVTHR*(1.D0+0.3D0*RNDMDPM(V1)) 
     *            - XDTHR*(1.D0+0.3D0*RNDMDPM(V2))
     *             -0.01*(1.D0+1.5D0*RNDMDPM(V3))
C..............................................................  
C                           1/x seaquarks
...
C                  MAXIMUM NUMBER OF SEA-PAIRS ALLOWED KINEMATICALLY
        XXSEAM=1.0 - XVTHR*(1.D0+RNDMDPM(V1)) - XDTHR*(1.D0+RNDMDPM(V2))
     *             -0.01*(1.D0+5.D0*RNDMDPM(V3))
      ENDIF
C..............................................................      
      IF(XSTHR.LE.1.D-12)THEN
	ICOUN=ICOUN+1
...
C--------------------------------------------------------------
          XTEST=XTVDI**1.5
	  VV=ITT
C	  IF(RNDMDPM(VV).GT.XTEST)GO TO 169
C--------------------------------------------------------------
          IXTV=IXTV+1
          XTVQ(IXTV)=XTVQI
          XTVD(IXTV)=XTVDI
...
C                        COLLECTION OF THE SEA-VALENCE PAIRS
     ...
     *' XKSAMP s-v loop I(NN),J(IXPS),iitt,iittv,NSV,NDV',
     +    I,J, IITT,IITTV,NSV,NDV
              ZUOVT(IITTV)=.FALSE.
              INTLO(I)=.FALSE.
C             IF(LSEADI.AND.RNDMDPM(V).GT.AMEDD.AND.IDIQUA.EQ.1)THEN
              IF(RNDMDPM(V).GT.AMEDD.AND.IDIQUA.EQ.1)THEN
...
C                 IF(IRECOM.EQ.1.AND.RNDMDPM(VALFRA).GT.VALFRA)THEN
                  IF(IRECOM.EQ.1)THEN
...
C
C                 IF(LSEADI.AND.RNDMDPM(V).GT.AMEDD.AND.IDIQUA.EQ.1)THEN
                  IF(RNDMDPM(V).GT.2.D0*AMEDD-1.D0.AND.IDIQUA.EQ.1)THEN
...
C-------------------------------------------------------------------    
       IF(ICASAD.EQ.1)THEN
         IF(RNDMDPM(VV).LE.CASAXX)THEN
	   IF(RNDMDPM(VVV).LE.0.5D0)THEN
	     ISCASA=IPSQ(IS1) 
	     IPVCAS=IPPV1(III)
	     IPSQ(IS1)=IPVCAS
	     IPPV1(III)=ISCASA
...
!$OMP THREADPRIVATE (/JSPART/)
      COMMON /JSPAR/PXJ(1000),PYJ(1000),PZJ(1000),HEJ(1000),NNNPJ
************************************************************************
************************************************************************
!$OMP THREADPRIVATE (/JSPAR/)
      COMMON/IFRAGM/IFRAG
!$OMP THREADPRIVATE (/IFRAGM/)
...
c      PZSM0=PZSM
c      ESUM0=ESUM
c      CALL DALTRA(GAM,BGX,BGY,BGZ,PXSM0,PYSM0,PZSM0,ESUM0,
c     *PPA,PXSM,PYSM,PZSM,ESUM)
...
c      PZDIF=PZSM-PZSC
c      EDIF=ESUM-ESMC
c      DIFFL=PXDIF+PYDIF+PZDIF+EDIF
c      if(diffl.ge.1.d-2)then
c        write(*,*)'lortr2 ', GAM,BGX,BGY,BGZ,PXSM0,PYSM0,PZSM0,ESUM0 
c        do i=1,n
c         J = NAUX + I - 1
c         write(*,*)i,PXP(I),PYP(I),PZP(I),HEPP(I),
c     +   PXJ(J),PYJ(J),PZJ(J),HEJ(J)
c        enddo
c      endif
c      IF(DIFFL.GE.1.D-2)
c     1WRITE(6,2)NUM,PXDIF,PYDIF,PZDIF,EDIF,PXSM,PXSC,
cC     WRITE(6,2)NUM,PXDIF,PYDIF,PZDIF,EDIF,PXSM,PXSC,
c     1PYSM,PYSC,PZSM,PZSC,ESUM,ESMC
c 2    FORMAT(' ',2X,'LORTRA:NUM=',I5,2X,'PXDIF=',1PE15.6,2X,'PYDIF=',
c     21PE15.6,2X,'PZDIF=',1PE15.6,2X,'EDIF=',1PE15.6/2X,'PXSM=',1PE15.6,
c     32X,'PXSC=',1PE15.6,2X,'PYSM=',1PE15.6,2X,'PYSC=',1PE15.6/2X,'PZSM'
c     4,1PE15.6,2X,'PZSC=',1PE15.6,2X,'ESUM=',1PE15.6,2X,'ESMC=',1PE15.6/
c     52X,'LORTR2 DIFFERENCES DUE TO ALTRA'/)
      RETURN
      END
*
************************************************************************
...
C                           changed from 3.  j.r.21.8.93
      ...
      ES=-2./(B33**2)*LOG(ABS(RNDMDPM(V)*RNDMDPM(U))+0.00000001)
...
c      PZSM0=PZSM
c      ESUM0=ESUM
c      CALL DALTRA(GAM,BGX,BGY,BGZ,PXSM0,PYSM0,PZSM0,
c     + ESUM0, PPA,PXSM,PYSM,
c     +PZSM,ESUM)
cC
cC     PXSC,ETC,ARE SUMS FOR BAMJET FRAGMENTS IN PROJ,TARGET CMS
c 
c      PXDIF=PXSM-PXSC
c      PYDIF=PYSM-PYSC
c      PZDIF=PZSM-PZSC
c      EDIF=ESUM-ESMC
c      DIFFL=PXDIF+PYDIF+PZDIF+EDIF
c      IF(ESUM.LT.TINY)ESUM=TINY
c      DIFFL=DIFFL/ESUM
c      IF(DIFFL.GE.1.D-4)WRITE(6,1000)NUM,PXDIF,PYDIF,PZDIF,EDIF,PXSM,
c     +PXSC, PYSM,PYSC,PZSM,PZSC,ESUM,ESMC
c 1000 FORMAT(' ',2X,'LORTRA:NUM=',I5,2X,'PXDIF=',1PE15.6,2X,'PYDIF=', 1
cc     +PE15.6,2X,'PZDIF=',1PE15.6,2X,'EDIF=',1PE15.6/2X,'PXSM=',1PE15.6,2
c     +X,'PXSC=',1PE15.6,2X,'PYSM=',1PE15.6,2X,'PYSC=',1PE15.6/2X,'PZSM',
c     +1PE15.6,2X,'PZSC=',1PE15.6,2X,'ESUM=',1PE15.6,2X,'ESMC=',1PE15.6/2
c     +X,'LORTRA DIFFERENCES DUE TO ALTRA'/)
      RETURN
      END
*-- Author :
C-------------------------------------------------------------------
...
!$OMP THREADPRIVATE (/ABRHH/)
      COMMON /NUCJTN/NONUJ1,NONUJT,NONUS1,NONUST
!$OMP THREADPRIVATE (/NUCJTN/)
...
C----------------------------------------------------------------
 ...
 7755         FORMAT (' NHAD.GT.NFIMAX ',2I10)
              RETURN
            ENDIF
 	 IF(NDONE.EQ.-107801) WRITE(6,*)' First chain HADRHH'
            DO 22 J=1,NHAD
C             NHKK=NHKK+1
              IF (NHKK.EQ.NMXHKK) THEN
...
C                               PUT NN-CMS HADRONS INTO /HKKEVT/
     ...
     *                                               IDHKK(NHKK)
C	 IF(NDONE.EQ.-107801) WRITE(6,*)' First chain HADRHH'
              IF (NDONE.EQ.-107801) WRITE(6,5001) J,NHKK,
