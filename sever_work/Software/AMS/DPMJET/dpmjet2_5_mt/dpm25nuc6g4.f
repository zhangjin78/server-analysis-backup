...
C       ENDIF
      ...
      IF(IP.LE.208.AND.NSTART.EQ.1)THEN
        IF ((LFZC).AND.(IFINAL.EQ.0)) THEN
	 IF(IPEV.GE.1)WRITE(6,*)'  before CHECKF ',cmhis,eproj,pproj,irej
          IF ((CMHIS.EQ.0.D0))
     ...
     +         CALL CHECKO(EPROJ,PPROJ,IREJ,1)
         ENDIF
c         write(*,*)' cmhis ',   EPROJ,PPROJ,IREJ     
      ENDIF
      IF(IREJ.EQ.1)THEN
...
!$OMP THREADPRIVATE (IDXPOT)
      DATA AN     /0.4D0/
!$OMP THREADPRIVATE (AN)
      DATA LSTART /.TRUE./

!$OMP THREADPRIVATE (LSTART)
      IF (MODE.EQ.0) THEN
         EBINDP(1) = ZERO
         EBINDN(1) = ZERO
         EBINDP(2) = ZERO
...
!$OMP THREADPRIVATE (/REJFBK/)
      COMMON /NEUTYY/NEUTYP,NEUDEC

!$OMP THREADPRIVATE (/NEUTYY/)
      DIMENSION EXPNUC(2),EXC(2,210),NEXC(2,210)
      DATA EXC,NEXC /420*ZERO,420*0/
!$OMP THREADPRIVATE (EXC,NEXC)
      DATA EXPNUC /4.0D-3,4.0D-3/
!$OMP THREADPRIVATE (EXPNUC)
      DATA INIEX/0/
!$OMP THREADPRIVATE (INIEX)
      DATA INIWA/0/

!$OMP THREADPRIVATE (INIWA)
      IREJ   = 0
      LRCLPR = .FALSE.
      LRCLTA = .FALSE.
...
!$OMP THREADPRIVATE (/ FINUC /)
     ...
     &                  IBHEAV  ( 12 ) , NPHEAV
!$OMP THREADPRIVATE (/ FHEAVY /)
      COMMON / FHEAVC / ANHEAV  ( 12 )
!$OMP THREADPRIVATE (/ FHEAVC /)
      LOGICAL LRNFSS, LFRAGM
      COMMON /RESNUC/  AMNTAR, AMMTAR, AMNZM1, AMMZM1, AMNNM1, AMMNM1,
     &                   ANOW,   ZNOW, ANCOLL, ZNCOLL, AMMLFT, AMNLFT,
     &                   ERES,  EKRES, AMNRES, AMMRES,  PTRES,  PXRES,
...
*   projectile/target with probab. 1/2
            IF ((INCMOD.EQ.1).OR.(IDCH(I).GT.0)) THEN
               IF (RNDMDPM(V).GT.OHALF) NCAS = -NCAS
...
*   sample from exp(-del/del0)
      DEL1   = -DEL0*LOG(MAX(RNDMDPM(V),TINY10))
* save formation time
...
c         STOP
        call fortran_throw_ex('EVTPUT: NHKK exeeds NMXHKK')
      ENDIF
...
!$OMP THREADPRIVATE (/TMPEMC/)
      COMMON /NUCC/   IT,ITZ,IP,IPZ,IJPROJ,IBPROJ,IJTARG,IBTARG
!$OMP THREADPRIVATE (/NUCC/)
      DATA INII/0/

!$OMP THREADPRIVATE (INII)
      IREJ = 0
...
!$OMP THREADPRIVATE (/ EVA0 /)
     ...
     &                  INWISM (0:NAMSMX)
*
!$OMP THREADPRIVATE (/ ISOTOP /)
      SAVE KA0, KZ0, IZ0
!$OMP THREADPRIVATE (KA0,KZ0,IZ0)
...
*  |  Only protons:
     ...
     &   ' ^^^FLUKA stopped in energy: mass number =< atomic number !!',
     &        KA0, KZ0
c               STOP 'ENERGY:KA0-KZ0'
                call fortran_throw_ex('ENERGY:KA0-KZ0')
            END IF
         ELSE
            ENERGY = WAPS ( 1, 2 )
            IZ0    = -1
...
!$OMP THREADPRIVATE (LFIRST, EXHYDR, EXNEUT)
    
      DATA LFIRST / .TRUE. /
      DATA NERG1/ 0/
*
!$OMP THREADPRIVATE (NERG1)
      IF ( LFIRST ) THEN
         LFIRST = .FALSE.
...
!$OMP THREADPRIVATE (/ NUCGEO /)
     ...
     &                  LPNRHO, LNWRAD
!$OMP THREADPRIVATE (/ NUCGE2 /)
      COMMON / NUCPWI / ALMBAR, BIMMAX, SIGGEO, LLLMAX, LLLACT
!$OMP THREADPRIVATE (/ NUCPWI /)
...
!$OMP THREADPRIVATE (/ NUCGII /)
      ...
      EQUIVALENCE ( SIGMASDPM, SIGPAB (3) )
...
* get decay-channel
    ...
    2 CONTINUE
      KCHAN = KCHAN+1
      IF ((RNDMDPM(V)-TINY17).GT.WT(KCHAN)) GOTO 2
