//  $Id: server.h,v 1.68 2022/05/10 09:10:26 aegorov2 Exp $
...
#else
  #include <serverrc-cpp-common.h> 
  #include <serverrc-cpp-skels.h>
#endif
#include "node.h"
#include "astring.h"
#include "client.h"
#include "timeid.h"
#ifdef NARROW_ONCE	// see build_server
  #include <typeinfo>
#endif
...
// [ae] forced added to avoid excess dbd operations
void _UpdateACT(const DPS::Client::CID & cid, DPS::Client::ClientStatus status, bool forced=true);
void _UpdateHosts();
 public:
...
public:
	...
	cnarrow_obj(const cnarrow_obj& c) throw() {
		*this = c;
	};
	cnarrow_obj& operator =(const cnarrow_obj& c) throw() {
		_pObj = c._pObj;
		_sior = c._sior;
		_updated = c._updated;
	};
...
#ifdef NARROW_ONCE
  map<const char*, cnarrow_obj> _orbobj;
#endif
protected:
...
#ifdef NARROW_ONCE
template <typename T> class cnarrow {
public:
	static CORBA::ORB_ptr giop(CORBA::ORB_ptr orb, const CORBA::String_var& sior) throw (CORBA::SystemException) 
	{
		CORBA::ORB_ptr result = find(sior);
		
		if (!result) {
			try {
				CORBA::Object_var obj = orb->string_to_object(sior);
				if(!CORBA::is_nil(obj))
					if ((result = reinterpret_cast<CORBA::ORB_ptr>(T::_narrow(obj)))) {
						cnarrow_obj c;
						c._pObj = result;
						c._sior += (const char *)sior;
						time(&c._updated);
						AMSServer::Singleton()->_orbobj[typeid(T).name()] = c;
						cout << "cnarrow<" << typeid(T).name() << ">::giop-I- new _narrow asked" << endl;
					}
					else
						cout << "cnarrow<" << typeid(T).name() << ">::giop-F- got zero result" << endl;
				else
					cout << "cnarrow<" << typeid(T).name() << ">::giop-F- string_to_object failed" << endl;
			}
			catch(CORBA::SystemException &ex) {
				cout << "cnarrow<" << typeid(T).name() << ">::giop-E- _narrow exception" << endl;
				throw;
			}
		}
		return result;
	...
	static void erase() {
		typedef typename map<const char*, cnarrow_obj>::iterator iter;
		iter it = AMSServer::Singleton()->_orbobj.find(typeid(T).name());
		if (it != AMSServer::Singleton()->_orbobj.end())
			AMSServer::Singleton()->_orbobj.erase(it);
	};
protected:
	static CORBA::ORB_ptr find(const CORBA::String_var& sior) {
		typedef typename map<const char*, cnarrow_obj>::iterator iter;
		iter it = AMSServer::Singleton()->_orbobj.find(typeid(T).name());
		if (it != AMSServer::Singleton()->_orbobj.end()) {
			if (it->second._sior == (const char *)sior) {
				time(&it->second._updated);
				return it->second._pObj;
			}
			else {
				cnarrow<T>::erase();
				cout << "cnarrow<" << typeid(T).name() << ">::giop-I- old _narrow removed" << endl;
			}
		}
		return static_cast<CORBA::ORB_ptr>(0);
	};
};
#endif
...
public:
 ...
 bool Master(bool advanced=true);
 integer Kill( DPS::Client::ActiveClient & ac, int signal, bool self);
...
public:
 ...
 void getId(DPS::Client::CID_out cid) throw (CORBA::SystemException);
  ...
  int getSplitDSTS(const DPS::Client::CID & ci, DPS::Producer::DSTS_out dsts, unsigned int pos, unsigned int len, DPS::Producer::TransferStatus &st) throw (CORBA::SystemException);
