//  $Id: geant4.h,v 1.33 2024/03/04 19:40:05 qyan Exp $
#ifndef __GEANT4H__
#define __GEANT4H__
...
class AMSG4MagneticField : public G4MagneticField{
  public:
    AMSG4MagneticField(G4VPhysicalVolume*pv=0);
...
public:
    ...
    void SetParticleGun(int part, number mom, AMSPoint  Pos,  AMSPoint Dir, AMSDir PolarDir = 0);
    void Reset(){_cpart=0;}
private:
    G4int        _npart;
    G4int        _cpart;
...
class AMSG4RunAction: public G4UserRunAction{
 ...
 public:
    ...
    void DumpCrossSections(int verbose, G4int ipart, G4int At = 12, G4int Zt = 6);
    void SampleProcess();
};
...
class AMSG4SteppingAction : public G4UserSteppingAction
{
 ...
 private:
    void FillProcessInfo(const G4Step* aStep);
    void FillBackSplash(const G4Step* Step);
    bool FillPrimaryInfo(const G4Step* Step);
    static const double facc_pl[23];
