// $Id: TrRawCluster.h,v 1.21 2019/03/14 22:49:27 qyan Exp $ 
...
/**@}*/	
  ...
  */    
  ...
  static float MCConvert(float a, bool back=false); 
  ...
  static bool CompId(TrRawClusterR *a,TrRawClusterR *b){
    int ida=std::abs(a->GetTkId())*20000+a->GetAddress();
    int idb=std::abs(b->GetTkId())*20000+b->GetAddress();
    if(a->GetTkId()>0)ida+=10000;
    if(b->GetTkId()>0)idb+=10000;
    return (ida<idb);
  }
