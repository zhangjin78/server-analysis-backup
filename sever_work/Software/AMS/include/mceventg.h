//  $Id: mceventg.h,v 1.67 2024/11/28 14:08:12 pzuccon Exp $
...
private:
  ...
  static TH3F* ratio;
#pragma omp threadprivate (ratio)  

  integer operator < (AMSlink & o) const {
    AMSmceventg* other = dynamic_cast<AMSmceventg*>(&o);
    if (other) {
      // make sure that AMSmceventg from the primary come first.
      if (_trkid == 1 && other->_trkid != 1)
        return 1;
      else if (_trkid != 1 && other->_trkid == 1)
        return 0;
      return _mom > other->_mom;
    }
    else return 1;
  }
  void _copyEl();
...
public:
  ...
  static int    readposfromfile(char* filename,vector<tbpos> &poslist,int opt=0);
  ...
  AMSmceventg(integer ip, integer trkid, integer parentid, geant mom, const AMSPoint & coo, const AMSDir & dir,const AMSDir & poldir,integer parinfo,float mass, float charge, integer nskip=0);
  AMSmceventg(integer seed[2]){_next=0;_nskip=0;setseed(seed);_tbline=0;_trkid=1;_parentid=0;}
  AMSmceventg( const AMSIO & io);
  ~AMSmceventg(){}
  void setTBline(short int tb){_tbline=tb;}
  ...
  //wxq_begin
  integer getnskip()const {return _nskip;}
  integer gettrkid()const {return _trkid;}
  ...
  static int NaturalFlux_stormer(float theta,float phi, float rig);
