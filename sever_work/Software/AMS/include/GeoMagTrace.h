//  $Id: GeoMagTrace.h,v 1.7 2015/07/10 07:39:27 shaino Exp $
...
public:
 ...
 virtual ~GeoMagTrace() {}
  ...
  /// In space or not
  bool IsSpace() const { return (_stat == SPACE); }
...
public:
  ...
  /// Get magnetic field components and total field in nT
  double GetBfield(double *bf) const;

protected:
  int _stat;             ///< Status defined with enum EStat
  int _nstep;            ///< Number of steps
  ...
  double _rmax;          ///< Maximum R
  double _dmin;          ///< Minimum dR/dL
...
public:
  ...
  // Temporary buffer; RPT,VPT,YPR,xtime,th,ph
  static double _buf[12];
  static void SetBuf(double rpt[3], double vpt[3], double ypr[3],
		     double xtime,  double theta,  double phi);

  // Repeat propagation with the same initial values from buf
  static GeoMagTrace Gprop(double rigidty, int nmax = NmaxStep);

  ClassDef(GeoMagTrace, 2)
};
