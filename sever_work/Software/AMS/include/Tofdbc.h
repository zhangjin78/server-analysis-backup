//  $Id: Tofdbc.h,v 1.54 2024/02/28 21:59:35 yjia Exp $
...
//------------
  ...
  const uinteger HTMANY=16384;    //HT 
//BetaH TOF2 TRACK BetaH
  ...
  const uinteger MCTIMESMEAR=16384; //MC Time Smearing BetaH 
//--------------
...
//
private:
  number bini;// bin width in integr.distr
  number times[TOFCSN::SCANMT+1]; // array of time points
//
public:
  TOFTpointsN(){};
 ~TOFTpointsN(){};
  TOFTpointsN(int _nbin, geant _bnl, geant _bnw, number arr[]);
//
//function to get time-point for given value of integral r(0-1) :
  number gettime(geant r){
    integer i;
    number ri,t;
    ri=r/bini;
    i=integer(ri);
    if(i>TOFCSN::SCANMT)i=TOFCSN::SCANMT;
    if(i<TOFCSN::SCANMT)t=times[i]+(times[i+1]-times[i])*(ri-i);
    else t=times[i];
    return t;
  }
//
  void getarr(number arr[]){
    for(int i=0;i<=TOFCSN::SCANMT;i++)arr[i]=times[i];
  }
  number getbini(){ return bini;}
};
//=====================================================================
...
// <--- functions to get efficiency ( getxbin should be called first!!!)
  ...
  number gettm1(const geant r, const int i1, const int i2);
  number gettm2(const geant r, const int i1, const int i2);
};
...
//   Func. to calc. pm1/pm2 times for division div:
  number gettm1(const int div, const geant r, const int i1, const int i2){return(wscan[div].gettm1(r, i1, i2));}
  number gettm2(const int div, const geant r, const int i1, const int i2){return(wscan[div].gettm2(r, i1, i2));}
  static void build();
};
#endif
...
//----
     static const int nMaxChC=30;
     int nBetaChC[2];
     int BetaChC[2][nMaxChC];
     std::map<int, float>betacorc[2][nMaxChC][5];
     int nRigChC[2];
     int RigChC[2][nMaxChC];
     std::map<int, float>rigcorc[2][nMaxChC][7];
//----Dynode only (Supper High Charge) V3
     int nBetaChS;
     int BetaChS[nMaxChC];
     std::map<int, float>betacors[nMaxChC][5];
     int nRigChS;
     int RigChS[nMaxChC];
     std::map<int, float>rigcors[nMaxChC][7];
 public:
     TofCAlignIonPar(int real);
     TofCAlignIonPar(int real,float *arr,int brun,int erun);//load 
     static int Version;
...
#endif
#endif
     static TofCAlignIonPar *GetHead(int real=-1);
     static void HeadLoadTDVPar(){GetHead()->LoadTDVPar();}
     void LoadOptPar(int opt=0);//copy TDV to class 0 From TDV 1 Read From Default
     void LoadTDVPar();//copy TDV to class
     int  LoadFromFile(char *file,char *file1,char *file2,char *file3[2],char *file4[2],char *file5[2]);//read data from file->Block data
     int  UpdateVersion(int ivers=-1);
     void PrintTDV();
};
...
//---
     ...
     static float CooSigmaA[MaxCharge];
     static int   iLay;
     static int   iBar;
     static int   iSide;   
     static int   Idsoft;
#pragma omp threadprivate (iLay,iBar,iSide,Idsoft)
 public:
     ...
     static float GetTimeSigma(int id, int icha=1, int mode=2);
     static float GetCooSigma(int id, int icha=1);
};

//==========================================================
class TofMCPar: public TofTDVTool<float>{
public:
    static float MCtuneDT;//MC Smear Par(ps)
    static float MCtuneST;//MC Shift Par(ps)
    static const  int  MaxCtune=8;//Max tuned charge
    static float MCtuneDT2[MaxCtune];//MC Smear Par(ps)
    static float MCtuneST2[MaxCtune][4][10];//MC Shift Par(ps) 4: number of layers 10: number of paddles
    static float MCtuneDS;//MCCluster and TOFCluster matched distance in cm
    static float MCtuneQ2[3];//MC Tune Q2[0]=1st gaus sigma/mean,Q2[1]=2nd gaus sigma/mean,Q2[2]=frac of 2st gaus
public:
     TofMCPar(){};
};
