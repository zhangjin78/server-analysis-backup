//  $Id: commons.h,v 1.415 2025/05/13 16:04:07 choutko Exp $
...
class MISCFFKEY_DEF{
 public:
  ...
  integer NewCRC;
  integer AnyThreadNumber;
  float G4GbPerThread;
  float MPIFinished;
  int BTRaise;
  int ReadEOS;
  int BCast;
  float MinimalRatioReconstructed;
  float MemReservedWeightPerThread;
};
...
class ECMCFFKEY_DEF {
 public:
  ...
  integer TuneMC;//0: original MC, 1: tune MC shower shape //wxq
  float RelDen;
};
...
class ECALHFFKEY_DEF {
 public:
  ...
  int     BadCellFit; // Apply Average Shower Correction
  float   BadCellR[2]; //Conditions for above
};
...
class CCFFKEY_DEF {
 public:
  ...
  integer DirFile[40];
  float Angle;
  geant momrsp[2];      //119-120
  geant Z;   //          stop simulation if primary particle died above Z;
  int PType; //         do not write particle types g(1)pos(10) el(100) 
};
//
...
//--Qi Yan
  ...
  float HCrossSectionBias[7];
  int DumpCrossSectionsAt;
  int DumpCrossSectionsZt;
  int ApplyCPULimit;
//--ZQU
  int GamaPolar;   //I30
  float polarrate ;    //R31, polarization percentage
  float HEexcBias[2];//R32
  int RadDec;//I34
  int MergeMCCluster;//I35
  float IonParametrisedLossStep;//R36
//--NBelyaev
  integer VisFile[40];//R37
};
...
class RICCONTROLFFKEY_DEF{
 public:
  ...
  geant   mcgeomshift[3]; // RICH MC geometry position shift in cm
};
...
class LIPF2C_DEF{
 public:
  ...
  geant resc_arw[LIP_NMAXLIPREC][2];
  ...
  geant resc_arwmsec[LIP_NMAXLIPREC][LIP_NMAXMIRSECCC];
