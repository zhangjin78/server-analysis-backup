//  $Id: TrMCCluster.h,v 1.30 2017/10/24 16:01:20 oliva Exp $
...
class TrMCClusterR : public TrElem {
 ...
 public:
  ...
  //! Status word (first 5 bits used for storing |Z| = 0, ..., 30; 31 is for >30, bit5: used in the merging procedure, bit6: cluster result of a merging)  
  int       Status;
  short int _effch;    //! Effective charge for secondaries, charge for primaries 
 ...
 public:
  ...
  short int & effch(){return _effch;}  //getset _effch
   int getZ(){return Status&0x3f;}
 ...
 //! Get the GEANT4 track ID
  ...
  //! Get charge and mass 
  void     GetChargeAndMass(int& Z, double& M);
  //! Get rigidity
  double   GetRigidity();   
  //! Get kinetic energy per nucleon  
  double   GetKn();    
  /**@}*/
  ...
  void GenSimClusters();
  //! Creation of the simulated clusters objects (TrSimCluster) of TrSim(QY)
  void GenSimClusters3();
  //! Add TrSimCluster(QY)
  int AddSimCluster(TrMCClusterR &that);
  //! Apply Saturation(QY)
  void ApplySaturation();
  //! Apply Normalization (keV to ADC) 
  void ApplyNormalization(); 

  //! Fit of the mean energy loss in MC
  double GetMeanEnergyLoss(double bg, int Z);
  //! Reshaping of the straggling 
  double GetReshapedEnergyLossStretch(double edep, double costheta, double z, double bg);
