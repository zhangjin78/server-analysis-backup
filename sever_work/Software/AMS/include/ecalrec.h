//  $Id: ecalrec.h,v 1.74 2017/10/21 11:27:05 xueqiang Exp $
//
...
namespace ecfit{
inline
double coreFit(double x, double *par){
        return 0.5*par[0]*((x+0.45)/pow((pow(par[1],2)+pow(x+0.45,2)),.5)-(x-0.45)/pow((pow(par[1],2)+pow(x-0.45,2)),.5));
}
inline
double tailFit(double x, double *par){
        return 0.5*par[0]*((x+0.45)/pow((pow(par[1],2)+pow(x+0.45,2)),.5)-(x-0.45)/pow((pow(par[1],2)+pow(x-0.45,2)),.5));
}
inline
...
double allFit(double x, double *par){
        double *p1 = &par[0];
        double *p2 = &par[2];
        return coreFit(x,p1)+tailFit(x,p2);
}
//Parameters
inline
double N_fit(double e=1, int L=1){
        return 1000*e*TMath::GammaDist(L,0.5*log(e)+2,0,2.4);
}
inline
double p_fit(double e=1, int L=1){
        return 2.6*exp((6.2-L)/7.3-exp((6.2-L)/7.3));
}
inline
double R_C_fit(double e=1, int L=1){
        return 0.16+0.024*L;
}
inline
double R_T_fit(double e=1, int L=1){
        return 0.96*(exp(-0.17*(L-9.2))+exp(-0.097*(L-9.2)));
}
inline
double ShowerLShape(double x, double en, int L){
        double params[4]={N_fit(en,L)*p_fit(en,L),R_C_fit(en,L),N_fit(en,L)*(1-p_fit(en,L)),R_T_fit(en,L)};
        return  allFit(x,&params[0]);
}
}
...
#pragma omp threadprivate(trigfl,trigconf,trigtm,trsum,dynadc,trpatt)
 ...
 public:
  ...
  AMSEcalRawEvent(integer idsoft, integer status, integer side,  
    ...
    static void SetLatPar(double ene, double Fcx_ISS[30], double Rcx_ISS[30], double Rtx_ISS[30], double Fcy_ISS[30], double Rcy_ISS[30], double Rty_ISS[30], double Fcx_MC[30], double Rcx_MC[30], double Rtx_MC[30], double Fcy_MC[30], double Rcy_MC[30], double Rty_MC[30]);
    static double ShowerShapeIdeal(float ene, float fZmax, double depth, double dist, double fc, double rc, double rt, float &saturation_factor);
    static double ShowerShape(float ene, float fZmax, int proj, double depth, double dist, double Fcx[30], double Rcx[30], double Rtx[30], double Fcy[30], double Rcy[30], double Rty[30], float &saturation_factor);
    //wxq_end
...
class Ecal1DCluster: public AMSlink{ 
 ...
 public:
  ...
  Ecal1DCluster(integer status, integer proj, integer pl, integer left, integer right, integer max, number en,number en3, number en5, number en9, number leak, number dead,AMSPoint coo, number w, number rms, AMSEcalHit * ptr[]): AMSlink(status,0),_proj(proj), _plane(pl),_Energy3C(en3),_Energy5C(en5),_Energy9C(en9),_EnergyC(en),_SideLeak(leak),_DeadLeak(dead),_Coo(coo),_MaxCell(max),_Left(left),_Right(right),_Weight(w),_RMS(rms){
    ...
    integer getleft()const{return _Left;}
    integer getright()const{return _Right;}
...
class AMSEcal2DCluster: public AMSlink{ 
 ...
 protected:
    ...
    friend class EcalShowerDirectionFit;
#ifdef __WRITEROOT__
...
class AMSEcalShower: public AMSlink{
 ...
 public:
  ...
  static bool s_executingFirstReconstructionPass; // Debugging purposes
};
