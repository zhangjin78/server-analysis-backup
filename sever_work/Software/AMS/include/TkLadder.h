// $Id: TkLadder.h,v 1.14 2025/04/11 16:10:05 nbelyaev Exp $
...
public:
  ...
  int GetPlaneId()const{
    ...
    else if(_layer==10 || _layer==11) return 7;
    else
      return -1;
  }
  ...
  int GetTkId()const{
    if (_layer>9)
      return abs(_slot)/_slot*(50*(_layer-10)+abs(_slot));
    else
      return abs(_slot)/_slot*(abs(_slot)+100*_layer);
    }
