// $Id: TkDBc.h,v 1.55 2025/04/11 16:10:03 nbelyaev Exp $
...
namespace trconst{
  //! number of planes
  const integer maxplanes=7;
  //! number of layers FIX ME remove this
  const integer maxlay=11;
  //! maximal number of ladders
  const integer maxlad=18;
...
public:
  ...
  */
  ...
  // Plane8 (L0) distance from the center to the flat edge of octagon support skin
  number _sup_l0_hc_skin_l;

  // Plane8 (L0) QLCS thickness
  number _sup_qlcs_w;

  // Plane8 (L0) QLCS skin thickness
  number _sup_qlcs_skin_w;

  // Plane8 (L0) gap between honeycomb skin and QLCS skin
  number _sup_hc_skin_qlcs_skin_gap;

  // Plane8 (L0) cupola thickness
  number _cupola_w;

  // Plane8 (L0) QLCS shape parameters
  number _sup_qlcs_l[2];

  // Parameters describing QLCS steps sizes in X
  number _sup_qlcs_dx[3];

  // Parameters describing QLCS steps sizes in Y
  number _sup_qlcs_dy[2];
...
public:
 ...
 //  //Silicon sensors
  ...
  number _ssize_active_l0[2];
  number _ssize_inactive_l0[2];
  int _NL0ReadoutChanK;
  number _L0SensorGapK;
  number _L0GapBetweenSensorsK;
...
public:
  ...
  TkLadder* FindTkId(int tkid){
    unsigned int ia=tkid+1000;
    if(_tkidfast==0||ia>=max_tkidmap_entries)
      return Findmap(tkidmap,tkid);
    else
      return tkidmap_fast[ia];
  } 
