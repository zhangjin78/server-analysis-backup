// $Id: tkdcards.h,v 1.139 2025/04/11 16:10:09 nbelyaev Exp $
#ifndef tkcommons_h
#define tkcommons_h
#ifdef _PGTRACK_
#include "typedefs.h"
...
public:
  ...
  float L2AlignPar[5];
  ///[841-844] Time dependent rigidity correction parameters (used with PM6)
  float RigidityCorr[4];
  ///845 fullspan acceptance and l1inner acceptance relative rigidity scale
  float RigidityGeomCorr;
  ///[846-847] new e+/e- measured absolute rigidity scale(QY)
  float RigidityScaleCorr[2]; 
  /// Option to enable/disable reading TKGEOMFFKEY from AMSRoot file
  static int ReadFromFile;

  void init();
  TKGEOMFFKEY_DEF():TObject(){init();}
  ClassDef(TKGEOMFFKEY_DEF,12);
...
public:
  ...
  // Draft DSP seed strip threshold per layer for L0
  geant   DSPSeedThrL0[2]; // Put this in use in the L0 reconstruction <<##>>
                           // Probably add corresponding TRMCFFKEY key <<##>>
  ...
  int EnableTrackerFeet;
  geant PEEKDensityBias;
  geant FeetAlDensityBias;
  geant MergeD[5];

  // Normalize SimCluster total signal 
  int   NormalizeSimCluster;
  geant NormAdjBefore[2];
  geant EdepAddNoise;
  geant NormAdjAfter[2];
  ...
  // Max secondary K_n for MergeMCCluster3
  float MaxSecondaryKn;

  // Outer smearing to simulate New V6 external layer(L1/L9) misalignment
  geant OuterSmearingN[2][2];
  static int ReadFromFile;
...
public:
  ...
  geant     ThrSeed[2][9];
  //! Draft thresholds for the L0
  geant     ThrSeedL0[2];  // Put this in use in the L0 reconstruction <<##>>
  ...
  //! I 76 0: do not use, 1: use always, 2: use only with 2.5 < Qtrk < 12.5, +10: use TDV (time dependent) correction, +20: use temperature dependent correction
  int   UseNonLinearity;

  //! I 77 
  //! 0: Do not use, do noting 
  //! 1: Use Z.Qu's fix on TrRecon to recover L6,8, this will correct Official Track reconstruction model, should always be this option. (default) 
  int   UseZQFix;
  
  
  //! I 78 
  //! 0: Do not use, do nothing 
  //! 1: tracks' been fit in all pattern
  int   UseZQRebuild;
 
  //! I 79 not used anymore
  int   MergeDiffRecon;

  //! I 80
  //! Build tracks with preferance to specific particle
  //! 0: default, general particle
  //! 1: Nuclei will be found first, better charge consistance.
  //! 2: to be added
  int   TrParticleType;

  //! I 81 0: no mask of negative signal strip in center of gravity(CofG) reconstruction, could give unphysical coordinate, 1: mask of negative signal strip(QY) 
  int   ClusterCofGOpt;

  //! I 82 0: neither reconstruction of Clusters (Hits) nor Track for NClusters>=MaxCls(Hits),  1: always keep the best reconstruction of Clusters(Hits) and Track, Clusters sort by amplitude
  int   KeepBestTrClHits;

  //! I 83 Max Highest YCluster for ZQ Rec
  int   MaxZQccList;
  //! I 84 Max Highest YCluster Offset
  int   MaxZQccListF;
  //! I 85 Max Tracks for ZQ Rec
  int   MaxZQ2ndTrack;
  //! R 86 Drop Hit YChis Threshold for ZQ Rec 
  float ZQDropYChisq;
  //! R 87 XChis Threshold for ZQ Rec 
  float ZQXChisqCut;
  //! I 88 Advanced fit bits for half span
  int   AdvancedFitFlagHalf;
  //! I 89 Advanced fit bits for CIEMAT/CIEMAT+PG
  int   AdvancedFitFlagExtAll;
  //! R 90 ZSEED Q Threshould
  float ZSEEDClQThr;
  //! R 91 Max Angle Threshould 4th rec
  float MaxZQAngleF;
  //! R 92 Drop Hit YChis Ratio Threshold
  float ZQDropYRth;
  //! R [93,94] X search windows
  float MaxZQXWindow[2];
  //! R [95,96] distance scale to refind X and Y hits 
  float MergeLim[2];
  //! R [97,98] distance scale to find external X and Y hits at high rigidity
  float MergeExtLimH[2];
  static int ReadFromFile;
  TRCLFFKEY_DEF():TObject(){init();}
  void init();
  static TRCLFFKEY_DEF & A();
  ClassDef(TRCLFFKEY_DEF,22);
...
public:
  ...
  /// R [27,56] Y Resolutions
  float ErcZY[30];
  /// R 57 Additional fitting Xerror (Layer 1) due to misalignment
  float ErrXL1;
  /// R 58 Additional fitting Xerror (Layer 9) due to misalignment
  float ErrXL9;
  TRFITFFKEY_DEF():TObject(){init();}
  void init();
  static TRFITFFKEY_DEF & A();

  /// Read from AMSRoot file or not
  static int ReadFromFile;

  ClassDef(TRFITFFKEY_DEF,12);
};
//
