...
class TrSim {
 ...
 public: 
  ...
  enum ESimulationType {
    ...
    kTrSim2010 = 2,
    /// TrSim2017 (new simulation with some detail+)
    kTrSim2017 = 3,
    /// TrSim2010 without non-linearities and cluster reshaping (+ new charge)
    kTrSim2010Renewed = 4
  };
...
#pragma omp threadprivate (sitkangl)
 ...
 public:
  ...
  //! Trying to improve the merge clusters method for secondaries
  static void MergeMCCluster3(); 
  static void MergeMCCluster4(); 
  static void MergeMCCluster5(); 
  //! Merge clusters with same track id (approximately safe)
  static void MergeMCClusterSameTrkId();
