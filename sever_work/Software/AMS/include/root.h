//  $Id: root.h,v 1.763 2025/05/23 12:03:25 choutko Exp $
//
//  NB
...
//--------DSP Errors-----------------------
 ...
 int getDSPErrorp(AMSSetupR::DSPError*& dsperrp); ///< Get AMSSetupR::DSPError pointer (if any) for the current event time. \retval return values from AMSSetupR::getDSPError() \retval 2 in case of not AMSSetupR object in HeaderR
...
/// Estimate IGRF Geomagnetic Cutoff and try to detect penumbra
        /*!
   * @param[out]  Rcut[2] minimum and maximum Cutoff (in GV)
   * @param[in]   Charge sign (+1 or -1)
   * @param[in]   amsdir direction in AMS reference frame (theta=0:up)
   * @param[in]   RPT[3]    ISS coordinates (Rs,PhiS,ThetaS)
   * @param[in]   VelPT[3]  ISS velocity (rad/sec,rad,rad)
   ...
   *  return  >0: number of cutoff values found, -1: failure;
*/
  int GetIGRFCutoff(double Rcut[2], int sign, AMSDir amsdir, 
        double RPT[3], double VelPT[3], double YPR[3],
        double xtime, int niter = 5);
...
*/
  ...
  unsigned int JLength[24];  ///<Untruncated length in bytes of corresponding slave in jinj block
  unsigned int JRoomError;  ///<Flag jinj slaves with Room Errors
  unsigned int SDLength(); ///<Return sum of lengths of SubDetector blocks
  unsigned int SDLengthU(); ///<Return sum of Untruncated lengths of SubDetector blocks
  unsigned int SDLength(int i); ///<Return length of SubDetectors (0-5: T,U,S,R,E,J)
  unsigned int SDLengthU(int i); ///<Return Untruncated length of SubDetectors (0-5: T,U,S,R,E,J)
  unsigned int SDRoomError(int i); ///< Return number of jinj slaves with room errors for SunDetectors (0-5: T,U,S,R,E,J)
  ...
  DaqEventR(){JRoomError=0;memset(JLength,0,sizeof(JLength));}
  virtual ~DaqEventR(){};
...
ClassDef(DaqEventR,9)       //DaqEventR
#ifdef VERSION6
#else
#pragma omp threadprivate(fgIsA)
#endif
...
class EcalAxis;                // <<< forward declaration for EcalShower::SharedEcalAxis()

class EcalShowerR {
static char _Info[4096];
public:
  static bool enableAutomaticRecoveryOfBrokenBuilds; ///< Allows to enable or disable the automatic recovery
  static bool enableNewDeadCellTreatment; ///< Allows to enable or disable the new ECAL dead-cell correction
...
protected:
  ...
  void FillPreComputedBDTs(DeadSideCorr*);
  void FillPreComputedDepositedEnergyArrays(DeadSideCorr*);
  float fPreComputedDepositedEnergyInMeVPerLayer[18]; //! DO NOT STORE!
  float fPreComputedDepositedEnergyInMeVPerProjection[2]; //! DO NOT STORE!
  float fPreComputedBDTv7_EnergyD; //! DO NOT STORE!
  float fPreComputedBDTv7_EnergyD_Smoothed; //! DO NOT STORE!
public:
 ...
 */
  ...
  /** Returns the deposited energy in MeV for a given layer. 
    * @param[in] layer: ECAL layer (0 - 17)
    */
  float DepositedEnergyInMeVPerLayer(unsigned int layer);

  /** Returns the deposited energy in MeV for a given projection.
    * @param[in] projection: X=0, Y=1
    */
  float DepositedEnergyInMeVPerProjection(unsigned int projection);

  /** Returns the deposited energy corrected for anode efficiency in GeV. */
  float DepositedEnergyCorrectedForAnodeEfficiencyInGeV();

  /** Returns the energy fraction in the last two layers. */
  float EnergyFractionInLastTwoLayers();

/// ECAL charge estimator based on Qiang Li's work/code, implementation L. Basara (lbasara@cern.ch)
/*!
 *   @param[in] ptr: Tracker track used for position interpolation.
 *   @param[in] kCor: bit-wise corrections applied (PMT: 1, Path Length: 2, Rigidity: 4, Birk's Law: 8)
 *   @param[in] beta: beta...
 *   @param[in] ri:   rigidity
 *   @return a MIPQ struct (int NPnt: Nb of points used in calculation, float RMS: Combined Q rms, float Mean: Q Mean value, float dE[18], dX[18]: individual dE -after kCor- and dX per layer))
 */
  MIPQ GetMIPQ(TrTrackR* ptr, char kCor=1|2|4|8,  float beta=1, float ri=99999);  


 //// Re-execute parts of the ECAL reconstruction online on the AMS ROOT files, as it is partially broken in pass6.
  //// NOTE: The 'Status' field only is NOT recomputed, only the CATLEAK bit is correctly reverted and recomputed.
  ////       Don't trust the 'Status' field except for the 'CATLEAK' bit, which is guaranteed to be correct.
  //// VARIABLES THAT ARE AFFECTED BY THE BUG AND RECOMPUTED:
  //// - Status (only CATLEAK bit)
  //// - ParProfile / Chi2Profile / Chi2ProfileBack
  //// - Entry / Exit
  //// - EMDir / Dir
  //// VARIABLES THAT ARE AFFECTED BY THE BUG AND NOT RECOMPUTED:
  //// - Chi2Dir
  //// - SphericityEV
  //// - FirstLayerEdep
  void ReExecuteECALReconstruction();

  //// Re-execute parts of the ECAL energy reconstruction online on the AMS ROOT files, as it is partially broken in pass6.
  //// VARIABLES THAT ARE AFFECTED BY THE BUG AND RECOMPUTED:
  //// - S13LeakXPI / S13LeakYPI
  //// - All 'Edep' fields of all 1D/2D clusters + associated ECAL hits.
  //// This mainly recomputes EnergyE/EnergyD and also masks dead-channels. Be sure to use ElectronEnergy2017() - which is valid for electrons/positrons.
  //// to retrieve a CORRECT energy scale that is consistent between BT/ISS/MC. Requires datasets >= B1082 for MC/BT.
  void ReExecuteECALEnergyReconstruction();
  ...
  float GetCorrectedEnergy(int partid=2,int method=2, int version=0);
  // Revised version of 'GetCorrectedEnergy(2,2,0)' - removing the 3/850 GeV kinks from the energy scale - superior to GetCorrEne(2,2,0)/GetCorrEne(2,2,1) - prefered energy scale for pass6/7 lepton analysis
  float ElectronEnergy2017();
 ...
 float GetESEv3Rejection();
  ...
  virtual ~EcalShowerR(){};
  ClassDef(EcalShowerR,19)       //EcalShowerR
#ifdef VERSION6
#else
#pragma omp threadprivate(fgIsA)
#endif
 ...
 */
  /// Counter Hit Time Error(ns)
  /*!
   * @param[in] q nomalized error for rawq 
   */
  float GetETime(float q=0,int mode=2); 
  /// Counter Fired Time Estimate Coo Error(cm)
  /*!
   * @param[in] q nomalized error for rawq 
   */
  float GetECoo(int ixyz,float q=0);
  ...
  /// Correct Time(ns) FT-LT(HT,SHT) for Side 0-1 (the same defination as Stime[2], LT Real Time=-Stime). The cable delay and slewing delay has been corrected.
  /*!
   * @param[out] lt[2]  FT-LT for all LT recorded times of Counter side 0 (lt[0]) and side 1 (lt[1]) 
   * @param[out] ht[2]  FT-HT for all HT recorded times of Counter side 0 (ht[0]) and side 1 (ht[1])
   * @param[out] sht[2] FT-SHT for all SHT recorded times of Counter side 0 (sht[0]) and side 1 (sht[1])
   * @return =0 if sucess, <0 error
   */
  int GetSTimeAll(vector<float> lt[2], vector<float> ht[2], vector<float> sht[2]);
...
11- 20 same for richringx
21-23 hit type for richringx direct,reflect,noise  (1st ring)
24-26 ....                                         (2nd ring)
27-28 unised 
...
*/
 ...
 ///
  ...
  static float  getMaxPMTCol(bool countrCrossed=false);
ClassDef(RichHitR,7)       // RichHitR
#ifdef VERSION6
#else
#pragma omp threadprivate(fgIsA)
#endif
...
class RichRingR {
public:
  enum{ready=-1,noCorrection=0,tileCorrection,fullUniformityCorrection,extendedUniformityCorrection,fullUniformityCorrectionV2};
 ...
 *   RichRingR::extendedUniformityCorrection -  A fine grained correction to the refractive index, which also takes into account the direction of the incident particle, is loaded. Further corrections are applied to correct some bias on the reflected hits, as well as some time dependence effects

 */
  ///@{
  /// Sets the initial correction
  ///\ param Modes:  RichRingR::noCorrection, RichRingR::tileCorrection, RichRingR::fullUniformityCorrection, RichRingR::extendedUniformityCorrection, 
  ///@}
 ...
 */
 ...
 static void switchDynCalibration();
  ...
  double betaCorrection(int extended=10,float z=0);
...
*/
 ...
 public:
  ...
  TrTrackR * pTrTrack();
  /// set Track index
  void setTrTrack(int iTrTrack){fTrTrack=iTrTrack;}
 ...
 */
  ...
  float getBeta(bool correctBias=false,float z=0);
  float getBetaBaseline(){return BetaRefit*betaCorrection(0);}
  float getBetaQ(float z,bool correctBias=false){return getBeta(correctBias,z);}
  ...
  float getBetaError()     {
    if(!IsNaF()) return sqrt(3.02035e-03*3.02035e-03/Used+2.47571e-04*2.47571e-04);
    return sqrt(7.74510e-03*7.74510e-03/Used);
  }
...
/** @name Interface to the refit function. Currently only a very simple version is implemented allowing to remove hits from the ring
 */
  ///@{
  /// Returns a new ring refitting the current one using the hits in the list (given as indexes)
#ifdef _PGTRACK_
  /// Rebuild the ring beta using the offline recontruction. The ring is modified.
  vector<float> fRawBeta[2];  //! Direct and reflected for offline, so not stured
  vector<int>   fUsedBeta;    //! Direct and reflected
  vector<int>   fHitBeta;     //! Direct and reflected

  int RawBetas(){return fUsedBeta.size();}
  float UsedBeta(int i){if(RawBetas()<=i || i<0 || fUsedBeta[i]<0) return 0; else return fUsedBeta[i];}
  float RawBeta(int i,int j){return fRawBeta[j][i];}
  int   HitBeta(int i){if (RawBetas()<=i || i<0) return -1; else return fHitBeta[i];}

  RichRingR *ReBuild(TrTrackR *tr);
  RichRingR *ReBuild();
  // Rebuild the ring removing the RichRingR hits indexed in the input array. The ring is modified.
  RichRingR *Refit(std::vector<int> &skip_hits); 
  RichRingR *Refit(double &fac, double ns=2.25,int verbose=0);
   int FillVector(double &fac, std::vector<int> &skip_hits, double ns=2.25, int verbose=0); 

   /// Vectors to store clusters
   vector<float> ClusterMean; //!
   vector<float> ClusterRms;  //!
   vector<int> ClusterBetas;  //!
   /// Search for beta clusters, optimized for Z=1. Returns the number of clusters
   int ClusterizeZ1(double dist=3e-3,int min_size=2);
   void GetClusters(int i,int &betas,float &mean,float &rms){betas=mean=rms=-1;if(i>=int(ClusterMean.size())) return;betas=ClusterBetas[i];mean=ClusterMean[i];rms=ClusterRms[i];}



  /// Build a ring given a track. Allocate and returns a new ring. 
  static RichRingR *Build(TrTrackR *tr);
  /// Compute the number of photoelectrons expected for a given track, beta and charge
  static double ComputeNpExp(TrTrackR *track,double beta,double Z=1);
#endif


  ///@}
  
  /// This function estimates and corrects the expected bias for the calibrated reconstructed beta, given a bias                             
  /// in the refractive index, in the expansion length, the average emission point, and a bias of the                           
  /// reference beta due to using a finite rigidity during calibration.
  /// These biases cannot be properly accounted by the calibration at beta=1 due to the degeneracy in the different parameters
  /// as well as the (small) uncertainty on the optical efficiencies affecting the effective NaF refractive index. 
  /// The correction parameters have been obtained from Yi Jia  Li analysis, so it must used the calibration used by her. At the moment
  /// NaF is not corrected.
  /// \param beta    Reconstructed calibrated beta, to be corrected
  /// \param naf_rad Set to true for naf radiator.
  /// \return Calibrated beta corrected for calibration bias
  static double CorrectCalibrationBias(double beta,bool naf_rad);
  static double CalibrationBias(double beta,bool naf_rad,double,double,double);
  ...
  virtual ~RichRingR(){};
  ClassDef(RichRingR,31)           // RichRingR
#ifdef VERSION6
#else
#pragma omp threadprivate(fgIsA)
#endif
...
static char _Info[4096];
 ...
 public:
  ...
  char * Info(int number=-1){
    ...
    sprintf(_Info,"RichRingB No %d Track=%d %s %s%s%s%s%s%s  N_{Hits}=%d #beta=%7.4g #Delta#beta %8.5g #theta_{c}=%6.3g ",
      number,fTrTrack,
...
public:
  ...
  char * Info(int number=-1){
   sprintf(_Info,"TRD Cluster No %d Layer %d TubeDir %s Coo %5.1f,%5.1f,%5.1f  Mult  %d HMult %d E_{Dep}(Kev) %5.1f Amp %5.1f Haddr %d Status %x",number,Layer,(Direction==0?"x":"y"),Coo[0], Coo[1], Coo[2],Multip,HMultip,EDep,pTrdRawHit()?pTrdRawHit()->Amp:4095,pTrdRawHit()?pTrdRawHit()->Haddr:0,Status);
   return _Info;
  }
  virtual ~TrdClusterR(){};
...
public:
  ...
  int five(time_t ut); //<return numberof anti in mc trigger
  Level1R(){};
/// \param level -1 any tof level
///  \return true if any tof related level1 was set
 bool isTOFTrigger(int level=-1);
/// \param mg !=0 restore pattern anycase physbpat==0  otherwise 
 void RestorePhysBPat( int mg=0 );
 
 int RebuildTrigPatt(int &L1TrMemPatt,int &PhysTrPatt,time_t tt=0);///< Rebuid JMembPatt/PhysBPatt according to Flight tr.setup(mainly for old MC root-files)
/// \return 1/0 (was physical/unbiased trigger)
 int RebuildTrigPatt(int &L1TrMemPatt,int &PhysTrPatt,int &AccPatt, time_t tt=0);///< Rebuid JMembPatt/PhysBPatt, also return ACC-sectors pattern (AccPat 8 lsb)
/// \return 1/0 (was physical/unbiased trigger)
 ...
 */
 ...
 public:
  ...
  ///  Access BetaH Normalized Data
  TofBetaPar GetNormTofBetaPar();
  /// Set BetaH All Data
 ...
 */
 public:
  ///  Do MC Beta Resolution Tuning, TofMCPar::MCtuneDT(TofMCPar::MCtuneST) KEY, TofMCPar::MCtuneDT=-40: 40ps time resolution smearing, TofMCPar::MCtuneST=-40: 40ps shift
  /// \return 0 success,  <0 failure
  int  DoMCtune();
  /// MC True Beta
  /*!
   * @param[in] smearopt 0: no smearing, 1: smearing according to the ISS data
   */
  float GetMCBeta(int smearopt=1);
  /// Beta Value
  float GetBeta  () {return BetaPar.Beta; }
  /// Fitting Error on 1/Beta
  float GetEBetaV () {return BetaPar.InvErrBeta; }
  /// Fitting Normalized Error on 1/Beta
  float GetNormEBetaV (); 
  ...
  /// Fitting Time Normalized Chis on Beta
  float GetNormChi2T();
  /// Fitting Coo  Normalized Chis On Beta(Match with Tracker)
  float GetNormChi2C();
  ...
  /// Time Measument Eror for iLayer
  float GetETime  (int ilay,float q=0,int mode=2);
  /// TCoo Measument Eror for iLayer
  float GetETCoo  (int ilay,float q=0);
  ...
  /// ClusterH Measured Coordinate for iLayer
  AMSPoint GetClusterHLCoo (int ilay);
 ...
 */
  ...
  */
  int  MassReFit(double &mass, double &emass,double rig=0,double charge=0,double erigv=0,int isbetac=0,int update=0);
    ...
    * @param[in] pattern  mmmm: m=1 or 0,  1011 using Tof Lay0 2 3 for Beta Fit(No Lay1). -1: Normal ReFit.  -2: ReSelect Good-Layer To Fit(Good Coo+Time)
    * @param[in] mode =0(same time weight) =1(different time weight) =2(different time weight RawQ normalized) =3(different time weight RawQL normalized) =4 (different time weight QL normalized)
  ...
  int  BetaReFit(double &beta,double &ebetav,int pattern=1111,int mode=1,int update=0);
    ...
    * @param[in]   pattern  mmmm: m=1 or 0, 1011 using TOF Lay0 2 3 for Beta Fit. -1: Normal ReFit. -2: ReSelect Good-Layer To Fit(Good Coo+Time)
    * @param[in]   mode =0(same time weight) =1(different time weight) =2(different time weight RawQ normalized) =3(different time weight RawQL normalized) =4 (different time weight QL normalized)
    * @param[in] update 1:Copy Beta ReFit Result to BetaH; 0:No Copy
    * @return    0: success, -1: failure
   */
...
/// iLay TOF Q Or Q^2 Measurement from One PMT Estimate ///Adding BetaH PathLength+Birk+Beta+Attenuation ReCorr
    ...
    * @param[in] pm  PMTid For Each Side: Dyndoe 0~2  Anode=0
    * @param[in] opt  DefaultQOpt Q Estimate, DefaultQ2Opt Q^2 Estimate
    * @param[in] frig:  rigidity correction: 0-no rigidity correction;  !=0-use input rigidity correction
    * @return   =0  No Response or Bad PMT(mask)  <0 PMT Saturation
  */
   float GetQLPM(int ilay,int pmtype,int is,int pm=0,int opt=TofClusterHR::DefaultQOpt,float frig=0);
...
public:
  ...
  /// set Track index
  void setTrTrack(unsigned int i,int iTrTrack){if(i<fTrTrack.size())fTrTrack[i]=iTrTrack;}
  VertexR(){};
  VertexR(AMSVtx *ptr);
  friend class AMSVtx;
  friend class AMSEventR;
...
/*!
  AMS Particle types:
     - "Normal" Particle:
      -# Derived from ChargeR, BetaHR and TrTrackR  objects
      -# Has   Charge, Rigidity, Velocity and DirCos properties set up
      -# Has fBetaH,fCharge,fTrTrack set up
     ...
     - Particle without TrTrackR:
      -# Derived from ChargeR, BetaHR and optionally TrdTrack  objects
      -# Has rigidity set up to 100000000 gev
      -# Has fBetaH,fCharge set up
      -# fTrTrack set to -1
      -# Optionally has fTrdTrack set up in case TrdTrackR was found
     ...
     - Particle based on EcalShower object:
      ...
      -# fBetaH, fCharge, fTrTrack, fTrdTrack and fRichRing set to -1
     ...
     - Particle based on VertexR (i.e. converted photon candidate or electron/positron ):
      ...
      -# Velocity may or may not be set depending on fBetaH index
...
*/
  ...
  float Local[9];  ///< contains the minimal distance to sensor edge in sensor length units; in new pass8, Local is the minimal distance to sensor edge in cm, >0 (<=0) outside (inside) sensor volume
...
protected:
  ...
  int  fRichRingX;     ///<index of  RichringXR used
...
public:
  ...
  /// access function to RichRingX object used
  /// \return index of RichRingXR object in collection or -1
  int iRichRingX()const {return fRichRingX;}
  /// access function to RichRingXR object used
  /// \return pointer to RichRingXR object or 0
  RichRingXR * pRichRingX();
...
int UpdateTrTrack(float DisMax=5, float DirMax=10, int iTrTrack=-1); ///< Update TrTrack index
/// assemble imformation again from sub-detector
int UpdateParticle();
 ...
 char * pType(){
   ...
   if(iTrTrack()>=0){
     strcpy(type,"Tr");
      ...
      if(iRichRingX()>=0)strcat(type,"RichX");
...
//-----------------------------------------------------------------------------------------------------------
  ...
  ClassDef(ParticleR,17)       //ParticleR
#ifdef VERSION6
#else
#pragma omp threadprivate(fgIsA)
#endif
...
/// TofMCClusterR structure
/*!

   \author e.choumilov@cern.ch qyan@cern.ch
*/
class TofMCClusterR {
public:
  int   Idsoft;  ///< software id  (ask E.Choumilov for details)
  ...
  float Edep;    ///< energy dep for photon convention(=EdepR*birksat) Gev
  float EdepR;   ///< energy dep Gev
...
static bool Rebuild;
  ...
  /*!< Three distinct cases:\n
    1. If primary particle: 
      a. Nskip =~ -1000: If particle was killed in this step 
      b. Nskip == -1100, otherwise 
      c. Nskip == (-1000 - (index of acceptance plane [0..20]))
      d. Nskip == -1200   elastic scattering \n
    2. If 2nd generation particle (only available for the daughter from photon): Nskip =~ -2000: If particle was killed in this step: Nskip == -2100, otherwise Nskip == (-2000 - (index of acceptance plane [0..20])).\n
    3. If backsplash: Nskip == -2\n
    4. If secondary particle (except for backsplash): Nskip == geant4 particle creation process: upper 8 bit: ProcessType of G4VProcess, lower 24 bit: ProcessSubType of G4VProcess.
  */
...
*/
  ...
  float Coo[3];   ///< coo (cm)
  float Dir[3];   ///< dir cos
  float PolDir[3];///< polarization directory;  if elastic scattering (Nskip==-1200) incoming direction
  float Momentum;  ///< momentum (gev)
  float Mass;      ///< mass (gev)
  float Charge;    ///< charge (signed)
  short int TBl;      ///< For TB generation mode postion id (line number of the file of positions) | if (Nskip ==-1200) Scattering nucleus A(8 MSB) and Z (8 LSB) 
...
char *Info3(int number){
    sprintf(_Info,"McParticle No %7d Nskip=%d Pid=%6d, TrkId=%i, ParentId=%i, Coo=(%5.2f,%5.2f,%5.2f) #theta=%4.2f #phi=%4.2f  #theta2=%4.2f #phi2=%4.2f Momentum(Gev)=%7.2g Mass=%7.2g Q=%4.0f PI=%d A= %d Z=%d",
    number,Nskip, Particle, trkID, parentID, Coo[0],Coo[1],Coo[2],acos(Dir[2]),atan2(Dir[1],Dir[0]),acos(PolDir[2]),atan2(PolDir[1],PolDir[0]),Momentum,Mass,Charge,PartInfo,TBl>>8,TBl&0xFF);
  return _Info;
}

  double GetElasticAngle() const{
    if(Nskip!=-1200) return 0.; 
    else{ 
      double aa=sqrt(Dir[0]*Dir[0]+Dir[1]*Dir[1]+Dir[2]*Dir[2]);
      double bb=sqrt(PolDir[0]*PolDir[0]+PolDir[1]*PolDir[1]+PolDir[2]*PolDir[2]);
      double costh=(Dir[0]*PolDir[0]+Dir[1]*PolDir[1]+Dir[2]*PolDir[2])/(aa*bb);
      return acos(costh)/M_PI*180;
    }
  }
  ...
  virtual ~MCEventgR(){};
ClassDef(MCEventgR,6)       //MCEventgR
#ifdef VERSION6
#else
#pragma omp threadprivate(fgIsA)
#endif
...
/// @cond RECDOX
/// Container of TrTracks and BetaHs at specified Run/Event
class TrReconR {
public:
  typedef unsigned int uint;

  int              iTrTrack;   ///< Index of TrTrack in ParticleR
  vector<TrTrackR> fTrTrack;   ///< Vector of TrTrackR
  vector<BetaHR>   fBetaH;     ///< Vector of BetaHR
  int              fVidx[2];   ///< TrTrack index for vertex (-1: no vertex)
  AMSPoint         fVertex;    ///< Vertex position
  AMSPoint         fVdist;     ///< Distance of 2 tracks
  float            fCpuTime;   ///< CPU time used for this TrRecon

  TrReconR();
  virtual ~TrReconR() {}

  uint NTrTrack() const { return fTrTrack.size(); }
  uint NBetaH  () const { return fBetaH  .size(); }

        TrTrackR *pTrTrack()       { return pTrTrack(iTrTrack); }
  const TrTrackR *pTrTrack() const { return pTrTrack(iTrTrack); }
  const TrTrackR *pTrTrack(uint i) const
                             { return i < NTrTrack() ? &fTrTrack[i] : 0; }
  TrTrackR *pTrTrack(uint i) { return i < NTrTrack() ? &fTrTrack[i] : 0; }
  BetaHR   *pBetaH  (uint i) { return i < NBetaH()   ? &fBetaH  [i] : 0; }

  void Set(AMSEventR *event, int iTrRecon = -1);
  void Add(TrTrackR  *track) { fTrTrack.push_back(*track); }
  void Add(BetaHR    *betah) { fBetaH  .push_back(*betah); }
...
/// Container of TrReconRs with different TrRecon builds
class TrTree {
public:
  enum ERecon { Org = 0,   ///< Originally stored in AMSEventR
    Fix = 1,   ///< Official Track finding with ZQFix
    New = 2,   ///< New Track finding by Z.Qu
    N   = 3    ///< Total number of reconstructions
              };
  int Run;
  int Event;
  int Entry;
  ...
  const TrReconR *pTrRecon(int rec) const {
    return (0 <= rec && rec < N) ? &fTrRecon[rec] : 0; 
  }
  TrReconR *pTrRecon(int rec) {
    return (0 <= rec && rec < N) ? &fTrRecon[rec] : 0; 
  }

  /// Get TrReconR pointer at specified run, event and rec
  TrReconR *pTrRecon(int run, int event, ERecon rec);

  int getEntries(int run);
  int GetEvent(int event);

  AMSEventR *GetEvent();
  int Process(AMSEventR *event, int entry = -1);

  int  Write();
  void Print(Option_t *option = "") const;

  static TrTree *Head;
  static TrReconR *GetTrRecon(int run, int event, ERecon rec);
  static int GetEntries(int run);

  static const char *BranchName(void) { return "tr"; }
  static const char *TreeName  (void) { return "TrTree"; }
  static const char *TreeTitle (void) { return
                                        "Collection of TrTrackR and BetaHR"; }

protected:
  int Nevent;        //! Not stored
  int Offset;        //! Not stored
  int TreeNum;       //! Not stored

  TFile   *fFile;    //! Not stored
  TChain  *fChain;   //! Not stored
  TBranch *fBranch;  //! Not stored
...
static bool _InitDBForEveryFileInChain;
static bool &InitDBForEveryFileInChain();
...
/// @endcond
public:
 static const int mtshr=128;
static bool fgThickMemory;
static int fgThreads;
 static TFile* fgOutSep[mtshr];
 static TDirectory* fgOutSepDir[mtshr];
...
#pragma omp threadprivate (bStatus,bHeader,bEcalHit,bEcalCluster,bEcal2DCluster,bEcalShower,bEcalH,bRichHit,bRichRing,bRichRingB,bRichRingX,bTofRawCluster,bTofRawSide,bTofCluster,bTofClusterH,bAntiRawSide,bAntiCluster,bTrRawCluster,bTrCluster,bTrRecHit,bTrTrack,bTrdRawHit,bTrdCluster,bTrdSegment,bTrdTrack,bTrdHSegment,bTrdHTrack,bLevel1,bLevel3,bBeta,bBetaH,bVertex,bCharge,bParticle,bAntiMCCluster,bTrMCCluster,bTofMCCluster,bTofMCPmtHit,bEcalMCHit,bTrdMCCluster,bRichMCCluster,bMCTrack,bMCEventg,bDaqEvent,bAux)
...
static TTree     * _ClonedTree[mtshr];   //max 32 threads
static TTree     * _ClonedTreeSetup[mtshr]; //max 32 threads
...
///
bool AntiMatteriSWTrigger() {return (fHeader.Error>>14)%2;} ///< \return AntiMatter SW Trigger

//----------------------------------------------------------------------------------------------------------
     /// MC Cite
     /*!
      * @param[in]  run!=0 search Cite Name by run, run=0 search Cite Name by index
      * @param[out] cite index(0-32) 
      * return  Cite Name for MC Runs;
      */
static const char *GetCite(unsigned int run,int &index);
...
//--------------------------------------------------------------------------------------------------
         ...
         *  return  0: success, -1: if failure;
         */
         int GetStoermerCutoff(double &Rcut, int sign, AMSDir amsdir);
///
   ...
   *  return  >0: number of cutoff values found, -1: failure;
   */
        int GetIGRFCutoff(double Rcut[2], int sign, AMSDir amsdir, int niter = 5);
   ...
   *  return  >0: number of cutoff values found, -1: failure;
   */
         static int GetIGRFCutoff(double Rcut[2], double xtime, int sign,
          AMSDir amsdir, int niter = 5);
      ...
      retrun -1: if failure;  
            ...
            output           : cutoff[0] = Max. Rcut [GV] for negative particles (Z==-1); 
                               cutoff[1] = Max. Rcut [GV] for positive particles (Z== 1);
             cutoff[2] = Min. Rcut [GV] for negative particles (Z==-1);
             cutoff[3] = Min. Rcut [GV] for positive particles (Z== 1);
        */
        int GetMaxIGRFCutoff(double AMSfov, double degbin, double cutoff[4]);
       ...
       input parameter  : version (=22): IGRF version IGRF-11(=11), IGRF-12(=12), IGRF-12+(=22), IGRF-12++(=32)
        */
        static int GetMaxIGRFCutoff(double AMSfov, double cutoff[4],unsigned int xtime,const char *fdir=0,int version=32);
...
///
       ...
       */
       ...
       //!  get number of events with external-layer hit in a given time window (dt)
       /*    \param[out] nxyz    Number of events with hits of L1 (nxyz[0], XYZ) and L9 (nxyz[1], XYZ)
             \param[in]  xtime   JMDC Time
             \param[in]  dt      Time window (second)
             \return 0: if sucess, !=0 RTI second has problem 
        */
       static int GetRTInL1L9(AMSPoint nxyz[2],unsigned int xtime,unsigned int dt=90);
...
//--------------------------------------------------------------------------------------------------
       ...
       //! Get time-dependent corrected rigidity. Please do not apply this correction to V6 rigidity!
       /*!    \param[in]  rigidity  Rigidity (for any span)
        *     \param[in]  opt       0 raw-rigidity to corrected-rigidity, 1 corrected-rigidity to raw-rigidity
        *     \param[in]  version   0 old rigidity-scale, 1 5years new rigidity-scale(QY), 3 7years new rigidity-scale(QY) 
        *     \param[in]  pgcm      0: PG, 1: CIEMAT, 2: PG+CIEMAT
  *     \return               Corrected rigidity
  */
       double GetCorrectedRigidity(double rigidity, int opt = 0, int version = 3, int pgcm = 2);

       //! Get time-dependent corrected rigidity (static method). Please do not apply this correction to V6 rigidity!
       /*!   \param[in]  rigidity  Rigidity (for any span)
        *    \param[in]  time      UTC time (AMSEventR::UTime())
        *    \param[in]  opt       0 raw-rigidity to corrected-rigidity, 1 corrected-rigidity to raw-rigidity
        *    \param[in]  version   0 old rigidity-scale, 1 5years new rigidity-scale(QY), 3 7years new rigidity-scale(QY)
        *    \param[in]  pgcm      0: PG, 1:CIEMAT, 2: PG+CIEMAT
  *    \return               Corrected rigidity
  */
       static double GetCorrectedRigidity(double rigidity, unsigned int time, int opt = 0, int version = 3, int pgcm = 2);

       //! Get corrected rigidity(scale) for different acceptance (ISS only). Please do not apply this correction to V6 rigidity!
        /*!   \param[in]  rigidity  Rigidity (for any span)
         *    \param[in]  igeom     0 total-inner acceptance, 1 total-l1inner acceptance, 2 fullspan acceptance
         *    \param[in]  opt       0 raw-rigidity to corrected-rigidity, 1 corrected-rigidity to raw-rigidity
         *    \param[in]  version   0 new rigidity-scale(with GetCorrectedRigidity version=1||version=3), 2 5years new rigidity-scale(without GetCorrectedRigidity)
         *    \return               Corrected rigidity 
         */       
       static double GetCorrectedGeomRigidity(double rigidity, int igeom = 1, int opt = 0, int version = 0);

       //! Get sensor-dependent corrected rigidity (static method). Please do not apply this correction to V6 rigidity!
       /*! This should NOT be applied together with GetCorrectedGeomRigidity.\n
  *  This should NOT be applied for Full span and Inner+L9 rigidity.\n
  *  This is for ISS data only.
        * @param[in]  rigidity  Rigidity (Inner or Inner+L1 only)
  * @param[in]  pntIn     Reference point[cm]
  * @param[in]  dirIn     Reference dir      
        * @param[in]  span      0:Inner, 1:Inner+L1
  * @param[in]  opt       0:raw->corr., 1:corr.->raw, 2:corr.-raw
  * @return     return    Corrected rigidity
  */
       static double GetCorrectedSensRigidity(double rigidity, AMSPoint pntIn, AMSDir dirIn, int span = 0, int opt = 0);

#ifdef _PGTRACK_
   //! Check if the specified TrTrack is picking up noise hit or not
   /*!
    * @param[in]  range  Search range in cm of additional hits
    * @param[in]  qmin   Minimum charge limit
    * @param[in]  track  TrTrack pointer, use pParticle(0)->pTrTrack if 0
    * @param[out] nhit   Number of additional hits found in layer i+1, 
    * @param[out] dmin   Minimum distance in cm to additional hits in layer i+1
    * @param[out] emin   Ebk/(Ebk+Eh) (if Eh >0) or -Ebk (if Eh ==0)
    *
    * @return     <0 in case of errors. 
    *             Otherwise total number of possible noise pick up 
    *             for layers 2-8 on the following criteria: either \n
    *             (1) 1 hit found in layer which no hit in TrTrack \n
    *             OR \n
    *             (2) 2 hits found in layers with the hit in TrTrack \n
    *             Note that the return value considers inner tracker only; \n
    *             It is needed to provide nhit to investigate for outer layers.
    */
  int IsTrackPickingUpNoise(float range = 1,
          float qmin  = 0, TrTrackR *track = 0,
          int nhit[9] = 0, float dmin[9] = 0,
                           float emin[9] = 0);

   //! Get an array of raw ADC signal around TrTrack from TrRawClusterR
   /*!
    * @param[in]  track  TrTrack pointer, use pParticle(0)->pTrTrack if 0
    * @param[in]  layer  layer number
    * @param[in]  n      array size
    * @param[out] sig    arry of ADC (0 if no data)
    * @param[out] iuse   0:unused 1:used for hit 2:used for seed
    * @param[out] flag   strip flag from TrCalDB
    *
    * @return  number of strips filled, <0 in case of errors
    */
  int GetTrackerStripRawSignal(TrTrackR *track,
             int layer, int n, float *sig, int *iuse = 0,
                                           int *flag = 0);

   //! Get a ratio of raw ADC used for the hit over the sum of n strips around
   /*!
    * @param[in]  layer  layer number
    * @param[in]  n      number of strips around (+/- n/2)
    * @param[in]  track  TrTrack pointer, use pParticle(0)->pTrTrack if 0
    *
    * @return  raw ADC ratio for the hit over the sum of n strips around
    */
  float GetTrackerRawSignalRatio(int layer, int n = 10, TrTrackR *track = 0);


   //! Get a He survival probability at L56 estimated from ISS data
   /*!
    * @param[in]  track  TrTrack pointer, use pParticle(0)->pTrTrack if 0
    * @return     He survival probability at L56 (%)
    */
  float GetHeSurvProbAtL56(TrTrackR *track = 0);

   //! Get a distance to the closest Al insert (TkFeet) in Tracker
   /*!
    * @param[in]  layer  layer number
    * @param[in]  track  TrTrack pointer, use pParticle(0)->pTrTrack if 0
    *
    * @return     distance to the closest Al insert in Tracker
    */
  float GetTkFeetDist(int layer, TrTrackR *track = 0);

   //! Get a distance to the closest Al insert (TkFeet) in Tracker
   /*!
    * @param[in]  layer    layer number
    * @param[in]  pnt      Refernce point [cm]
    * @param[in]  dir      Refernce dir
    * @param[in]  rigidity Rigidity [GV]
    *
    * @return     distance to the closest Al insert in Tracker
    */
  static float GetTkFeetDist(int layer, AMSPoint pnt,
    ...
    * @param[in]  dir      Refernce dir
    * @param[in]  rigidity Rigidity [GV]
    * @param[out]  distance to insert  in cm
    *
    * @return     0 if ok error if not
    */
  static int GetTkFeetDist(int layer, AMSPoint pnt,
...
float LiveTime(unsigned int time=0); ///< trying to get livetime from scalers map return -1 if error
   ...
   * @return     return  0: Out of Tracker 1: Inside tracker and ladder 2: Inside tracker but in the gap between ladders 3: but in the gap between sensors
   */
static int IsInsideTracker(int ilay, const AMSPoint &pntIn,
  ...
  //! Get sequence numbers of Tracker sensors
  /*!@param[in]  tr      TrTrack pointer (or pParticle(0)->pTrTrack() if 0)
   * @param[out] s0      Sensor grid ID at Z=0
   * @param[out] s2      Sensor grid ID at Layer 2
   * @param[out] s8      Sensor grid ID at Layer 8
   * @return     return  0: Success -1: Error
   */
  int GetTrSensorGridID(TrTrackR *tr, int &s0, int &s2, int &s8);

  //! Get sequence numbers of Tracker sensors (static version)
  /* @param[in]  pntIn   Reference point[cm](can be tk-tof-or-shower position )
   * @param[in]  dirIn   Reference dir      (can be tk-tof-or-shower direction)
   * @param[in]  rigidity  Rigidity  (GV)   (0 in case of linear track)
   * @param[out] s0      Sensor grid ID at Z=0
   ...
   * @return     return  0: Success -1: Error
   */
  static int GetTrSensorGridID(AMSPoint pntIn, AMSDir dirIn, double rigidity,
             int &s0, int &s2, int &s8);

  //! Get distance of TrTrack to the sensor edges
  /* @param[in]  tr      TrTrack pointer (or pParticle(0)->pTrTrack() if 0)
   * @param[out] dx      Distance in X (cm) to the sensor edges at layer i+1
   * @param[out] dy      Distance in Y (cm) to the sensor edges at layer i+1
   * @return     return  0: Success -1: Error
   */
  int GetTrSensorDistance(TrTrackR *tr, float dx[9], float dy[9]);
   ...
   * @return     return  0: Success -1: Error
   */
  static int GetTrSensorDistance(AMSPoint pntIn, AMSDir dirIn, double rigidity,
         float dx[9], float dy[9]);

  //! Get 1/rigidity bias estimated from ISS He data at SensorGridID
  /* @param[in]  s       Output of GetTrSensorGridID
   * @return     return  1/Rigidity bias
   */
  static float GetTrSensorRbias(int s);
...
///
int   nRichRingX()const { return fHeader.RichRingXs;} ///< \return number of RichRingXR elements (fast)
///
...
///
MCEventgR* GetPrimaryMC(int Pos=1 ); ///< Return pointer to the Primary MC Particle at the point of Generation (Pos=1) or Exiting (Pos=-1,B800++ only) or NULL;
static bool   nMCEventgC();
///
int   nDaqEvent()const { return fHeader.DaqEvents;} ///< \return number of MCEventgR elements (fast)

///  Temp use, Selt Track from different rebuild method.  1:TrRecon Rebuild with zqufix,  2:zqu rebuild,  else do nothing.
int UseReTrackGroup(  int rtype  );

 ///  Create RichRingX function
 /// \param q  q/100 minimal charge for given track to run the RichRingX reconstruction
 //  \param mthr   number of threads to run RichRingX reconstruction /only valid if __ROOTSHAREDLIBRARY__ defined/
 /// \return number of rich rings created
int CreateRichRingX(int q,int mthr=1);  ///<  \return number of rich ring created 
 ...
 ///  \return pointer to corresponding EcalShowerR element
  ...
  return fEcalH.at(l);
      ...
      ///  \return number of RichRingXR
      ///
      unsigned int   NRichRingX()  {
        if(fHeader.RichRingXs && fRichRingX.size()==0){
           bRichRingX->GetEntry(_Entry);
        }
        return fRichRingX.size();
      }
      ///  \return reference of RichRingXR Collection
      ///
      vector<RichRingXR> & RichRingX()  {
        if(fHeader.RichRingXs && fRichRingX.size()==0)bRichRingX->GetEntry(_Entry);
         return  fRichRingX;
       }

       ///  RichRingXR accessor
       /// \param l index of RichRingXR Collection
      ///  \return reference to corresponding RichRingXR element
      ///
       RichRingXR &   RichRingX(unsigned int l) {
        if(fHeader.RichRingXs && fRichRingX.size()==0)bRichRingX->GetEntry(_Entry);
         return fRichRingX.at(l);
      }

       ///  RichRingXR accessor
       /// \param l index of RichRingXR Collection
      ///  \return pointer to corresponding RichRingXR element
      ///
      RichRingXR *   pRichRingX(unsigned int l) {
        if(fHeader.RichRingXs && fRichRingX.size()==0)bRichRingX->GetEntry(_Entry);
        return l<fRichRingX.size()?&(fRichRingX[l]):0;
      }
...
*/
      ...
      */
      ...
      /// Primary Particle accessor
      /// \param direction   preferred direction to filter primary particle, 1 (-1) means higher priority for downward (upward) going particle, 0 means the same priority for downward and upward particles 
      ///  \return pointer to the Primary Particle 
      ///
      ParticleR *   GetPrimaryParticle(int direction=1);
...
bool HasElScat() {
  bool has=false;
  for (unsigned int ii=0;ii<nMCEventg();ii++)
  if(MCEventg(ii).Nskip==-1200) {has=true; break;}
  return has;
}
 public:
...
void         AddAMSObject(RichRingXR *ptr);
...
ClassDef(AMSEventR,28)       //AMSEventR
#ifdef VERSION6
#else
#pragma omp threadprivate(fgIsA)
#endif
...
extern double GetCorrectedRigidityG(double rigidity, int version, int pgcm);
extern BetaHR *FindBetaH(TrTrackR *track);
extern float GetBeta(BetaHR *betah);
#ifdef _PGTRACK_
extern int  UpdateInnerDz();
extern int  UpdateExtLayer(int type=0,int lad1=-1,int lad9=-1);
extern void set_charge_calibration_tracker(TrClusterR* cluster);
extern int  MCtune(AMSPlaneM &plmr, int tkid, float dmax, float ds, int ixy);
extern int  MCSmearExtAlign(AMSPlaneM &plmr, int sid, float dmax, int npar, float *spar, int opt=0);
extern int  MCshift(AMSPoint &coo, double ds);
extern int  MCscat(AMSPoint &coo, int layj, double prob, double scat, double pwr);
extern int  MCscatq2(AMSPoint &coo, int layj, float b,float prob);
extern int  DropExtHits(void);
#endif
