...
//--L1L9Hit X+Y Match + L1L9Q
  for(int uexl=0;uexl<2;uexl++){
    ...
    if(qvs>1.5){
      tk_exqvn[0][uexl]=tkhit->GetQYJ(2,betahsc,urig,mult,dadz[0],dadz[1]);
      tk_exqvn[1][uexl]=tkhit->GetQH(2,betahsc,urig,mult,dadz[0],dadz[1]);
    }
  }
...
//--Track Pos
    for(int ilay=0;ilay<NTKL;ilay++){
      ...
      for(int ixy=0;ixy<3;ixy++){
        ...
        if(tk_q[1]<=1.5)continue;
        tk_exqln[0][uexl][ixy]=tkhit->GetQYJ(ixy,obetac,frig,mult,dadz[0],dadz[1]);
        tk_exqln[1][uexl][ixy]=tkhit->GetQH(ixy,obetac,frig,mult,dadz[0],dadz[1]);
      }
