...
// ********************************************************************
//
// $Id: G4ExcitationHandler.C,v 1.29 2024/11/28 14:08:10 pzuccon Exp $
//
...
#include "event.h"
#include "mceventg.h"
G4VParticleChange* G4HadronElasticProcess::PostStepDoIt(const G4Track& track,
                                     const G4Step& /*step*/)
{
...
// BEGIN PZ ADD
    if(strstr((const char*)(hadi->GetModelName()),"ionelasticVC")){
      G4double mass=track.GetParticleDefinition()->GetPDGMass();
      G4int    charge=track.GetParticleDefinition()->GetPDGCharge();
      G4LorentzVector in4mom=track.GetDynamicParticle()->Get4Momentum();
      G4double outKinEne=result->GetEnergyChange();
      G4double outTotEne=outKinEne+mass;
      G4double outMom= sqrt(outTotEne*outTotEne-mass*mass);
      G4LorentzVector  out4mom(outdir*outMom, outTotEne);
      G4double angle=acos(indir.dot(outdir))*180/3.142;
      G4StepPoint* post=track.GetStep()->GetPostStepPoint();
      // G4cout << "PZKNOWS Track ID: "<<track.GetTrackID()<<" parent: "<<track.GetParentID()
      //         <<" particle: "<<track.GetParticleDefinition()->GetParticleName()
      //         << " scattered off Z= " << targNucleus->GetZ_asInt()
      //         << " A= " << targNucleus->GetA_asInt();
      // G4cout<< " Position X: "<<post->GetPosition().x()
      //       << " Position Y: "<<post->GetPosition().y()
      //       << " Position Z: "<<post->GetPosition().z()
      //       << " Proj: 4-mom " << in4mom<< " Final: " << out4mom <<" angle: "<<angle<<" deg"
      //       << G4endl;
      if(track.GetTrackID()==1 && track.GetParentID()==0){
        AMSmceventg* mcline=new AMSmceventg( 
                                      track.GetParticleDefinition()->GetPDGEncoding(),  
                                      track.GetTrackID(),
                                      track.GetParentID(),
                                      (out4mom.e()-mass)/GeV,//kinetic energy
                                      AMSPoint(post->GetPosition().x()/cm,post->GetPosition().y()/cm,post->GetPosition().z()/cm),
                                      AMSDir(out4mom.vect().x(),out4mom.vect().y(),out4mom.vect().z()), //dir
                                      AMSDir( in4mom.vect().x(), in4mom.vect().y(), in4mom.vect().z()), //poldir -- here incoming dir
                                      2, //partinfo -- 2-> G4 PID
                                      charge,
                                      mass/GeV,
                                      -1200 // NSKIP -1200 -- elastic scattering
                                    ); 
        short int az=(targNucleus->GetA_asInt())<<8 | (targNucleus->GetZ_asInt() &0xFF);
        mcline->setTBline(az);
        AMSEvent::gethead()->addnext(AMSID("AMSmceventg",0), mcline);
      }
    }
// END PZ ADD
  } else {
                                  ...
                                  G4int Z, G4int A)
{
  ...
  if(strstr((const char*)(this->GetModelName()),"ionelasticVC")){
    ...
    int icas=1;
    if(int(G4FFKEY.HCrossSectionBias[3])/10%10>0)icas+=10;//elastic exponent const with energy,meantime quasi-elastic XS would be more flat(Q.Yan)
...
//-----
  double p_pn=std::fabs(p_p/a_p);//P/A
  if(G4FFKEY.HCrossSectionBias[6]<=-1){//Shape-Tuning(Q.Yan)
    double r_pn=p_pn*2.;
    if(z_p==6)bias=bias*(1.+2.98559/(1.+6.87319e-04*std::pow(r_pn,2.41803)));
  }
  else {
    double pth=100;//100GeV
    if(p_pn<pth){//quasi-elastic XS energy-dependence(Q.Yan)
      bias=bias*(1.+(1.-std::log(p_pn)/std::log(pth))*G4FFKEY.HCrossSectionBias[6]);
    }
  }
 // std::cout <<" aa  cc "<<aa<<" "<<cc<<" "<<bias<<" "<<plab/GeV<<" "<<bb<<" "<<dd<<std::endl;
  ...
  if((s1 + s2)*G4UniformRand() < s2) {
   ...
   bb*=G4FFKEY.HCrossSectionBias[5];
   q1 = 1.0 - std::exp(-bb*tmax); 
 }
    int mtry=100;
    if(z_p<3 || int(G4FFKEY.HCrossSectionBias[3])%10==0)mtry=0;
...
G4bool G4ionIonisation::IsApplicable(const G4ParticleDefinition& p)
{
  return (p.GetPDGCharge() != 0.0 && !p.IsShortLived() &&
          (p.GetParticleType() == "nucleus" || p.GetParticleType() == "anti_nucleus"));
}
