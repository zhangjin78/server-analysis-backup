...
//        _trddb=TRDCalibPar();
      int status=  readDB("TRDCalibration",&_trddb,asktime);

      if(status!=1){
	Error("TrdKCalib::readDB_GainCalibration-E-", "readDB Status: %i",status);
        return 0;
      }

        if(_trddb.Time_A==0){
            
	  if(_trddb.Time_S==0 || _trddb.Time_E==0){
	  Error("TrdKCalib::readDB_GainCalibration-E-", "Gain Calibration Period average time return zero : %f", asktime);
	  return 0;

	  }else _trddb.Time_A=0.5*((double)_trddb.Time_S+(double)_trddb.Time_E);
...
//        _trdaligndb_Plane=TRDAlignmentDB_Plane();
      int status=  readDB("TRDAlignmentPlane",&_trdaligndb_Plane,asktime);

      if(status!=1){
        Error("TrdKCalib::readDB_Alignment_Plane-E-", "readDB Status: %i",status);
        return 0;
      }
...
//        _trdaligndb_Global=TRDAlignmentDB_Global();
     int status=   readDB("TRDAlignmentGlobal",&_trdaligndb_Global,asktime);


      if(status!=1){
        Error("TrdKCalib::readDB_Alignment_Global-E-", "readDB Status: %i",status);
        return 0;
      }
