//  $Id: g4physics.C,v 1.152 2024/02/10 09:30:23 qyan Exp $
//
//
// 
...
#endif
AMSG4Physics::AMSG4Physics():  AMSNode(AMSID("AMSG4Physics",0)),G4VModularPhysicsList(),_pg3tog4(0),_pg4tog3(0),_Ng3tog4(0),phadronphysicsinclxx_holder(0)
{
...
//    add elastice scattering to ions
  ...
  G4CrossSectionElastic* nucxs = new G4CrossSectionElastic(pgg);
  G4CrossSectionElastic* anucxs = new G4CrossSectionElastic(apgg);
  theParticleIterator->reset();
  while( (*theParticleIterator)() )
  {
    G4ParticleDefinition* particle = theParticleIterator->value();
    ...
    if( (G4FFKEY.IonPhysicsModel/1000)%10 &&(
      ...
      if ((G4FFKEY.IonPhysicsModel/1000)%10==3){//New elastic model added by Q.Yan
        G4NuclNuclDiffuseElastic* NNElastic = new G4NuclNuclDiffuseElastic();
        NNElastic->SetMinEnergy(0.0);
        NNElastic->SetMaxEnergy(100*TeV);
        hel->RegisterMe(NNElastic);
      }
      else if((G4FFKEY.IonPhysicsModel/1000)%10==2){//VC
        G4HadronElastic* lhep3 = new G4HadronElastic(G4String("ionelasticVC"));
...
//      G4HadronElasticProcess* ahel = new G4HadronElasticProcess("aionelastic");
//      ahel->AddDataSet(anucxs);
//      ahel->RegisterMe(lhep3);
//      pmanager->AddDiscreteProcess(ahel);
    }
//----
}
#endif
   ...
   pqgsp->tp->theProtonInelastic->BiasCrossSectionByFactor2(G4FFKEY.HCrossSectionBias[0]);
...
#else
pqgsp->tp->theProtonInelastic->BiasCrossSectionByFactor2(G4FFKEY.HCrossSectionBias[0]);
pqgsp->ta->theAntiProtonInelastic->BiasCrossSectionByFactor2(G4FFKEY.HCrossSectionBias[0]);
...
#else
	G4HadronPhysicsQGSP_BERT * pqgsp=new G4HadronPhysicsQGSP_BERT();
#endif
	if(G4FFKEY.ProcessOff/100%10==0)pqgsp->ConstructProcess();
	if(G4FFKEY.HCrossSectionBias[0]!=1){ 
		cout<<"HadronicInElasticCrossectionWillBeBiasedBy   "<<G4FFKEY.HCrossSectionBias[0]<<endl;
...
#else
	G4HadronPhysicsQGSP_FTFP_BERT * pqgsp=new G4HadronPhysicsQGSP_FTFP_BERT();
#endif
	if(G4FFKEY.ProcessOff/100%10==0)pqgsp->ConstructProcess();
	if(G4FFKEY.HCrossSectionBias[0]!=1){
		cout<<"HadronicInElasticCrossectionWillBeBiasedBy   "<<G4FFKEY.HCrossSectionBias[0]<<endl;
...
#else
	G4HadronPhysicsFTFP_BERT * pqgsp=new G4HadronPhysicsFTFP_BERT();
#endif
	if(G4FFKEY.ProcessOff/100%10==0)pqgsp->ConstructProcess();
	if(G4FFKEY.HCrossSectionBias[0]!=1){
		cout<<"HadronicInElasticCrossectionWillBeBiasedBy   "<<G4FFKEY.HCrossSectionBias[0]<<endl;
...
#else
	G4HadronPhysicsFTFP_BERT_TRV * pqgsp=new G4HadronPhysicsFTFP_BERT_TRV();
#endif
	if(G4FFKEY.ProcessOff/100%10==0)pqgsp->ConstructProcess();
	if(G4FFKEY.HCrossSectionBias[0]!=1){
		cout<<"HadronicInElasticCrossectionWillBeBiasedBy   "<<G4FFKEY.HCrossSectionBias[0]<<endl;
...
if(G4FFKEY.PhysicsListUsed%100==10){
	...
	abort();
#else
	G4HadronPhysicsNuBeam * pqgsp=new G4HadronPhysicsNuBeam();
	if(G4FFKEY.ProcessOff/100%10==0)pqgsp->ConstructProcess();
	if(G4FFKEY.HCrossSectionBias[0]!=1){
		cout<<"HadronicInElasticCrossectionWillBeBiasedBy   "<<G4FFKEY.HCrossSectionBias[0]<<endl;
#if G4VERSION_NUMBER < 1040
...
//--Qi Yan
      else if ((G4FFKEY.IonPhysicsModel%10==3 || (G4FFKEY.IonPhysicsModel%10>=5&& G4FFKEY.IonPhysicsModel%10<=8)) && worker) {
         int useinclxx=G4FFKEY.IonPhysicsModel%10-3;
         cout<<"AMS Ion NewList will be used(DPMJET model; Ion cross section)."<<endl; 
...
//---
  ...
  G4OpBoundaryProcess* theBoundaryProcess0 = new G4OpBoundaryProcess();
...
//--TOF Geant4
    ...
    if (particleName == "opticalphoton") {
      ...
      if((G4FFKEY.TFNewGeant4/100)%10>=1)pmanager->AddDiscreteProcess(theBoundaryProcess0);//Raw G4OpBoundaryProcess
      else pmanager->AddDiscreteProcess(theBoundaryProcess);
    }
  }
//G4cout<< "exitingg4physics "<<G4endl;
...
/*
   G4StateManager *st=G4StateManager::GetStateManager();
   cout<<"before "<<st->GetStateString(st->GetCurrentState())<<" "<<st->GetStateString(st->GetPreviousState())<<endl;
    st->SetNewState( G4State_PreInit);
    cout<<" ConstructGeneral-I-RadioactiveDecayAdded"<<endl;
     RegisterPhysics(new G4DecayPhysics());
  // Radioactive decay
     RegisterPhysics(new G4RadioactiveDecayPhysics());
     st->SetNewState(st->GetPreviousState());
     cout<<"after "<<st->GetStateString(st->GetCurrentState())<<" "<<st->GetStateString(st->GetPreviousState())<<endl;
*/ 
cout<<" ConstructGeneral-I-RadioactiveDecayAdded"<<endl;
...
if(G4FFKEY.DetectorCut%10==1){
   ...
   G4cout<<" (G4FFKEY.DetectorCut-ISetCut="<<acutv<<G4endl;
   ...
   SetCutValue(acutv, "deuteron");
   SetCutValue(acutv, "GenericIon");
 }
 else {
...
//    EcalCuts->SetProductionCut(ecutv, "gamma");  // removed gamma for backspash
    ...
    EcalCuts->SetProductionCut(ecutv, "deuteron");
    EcalCuts->SetProductionCut(ecutv, "GenericIon");
  ...
  // tracker related production cuts 
  if (TRMCFFKEY.SetActiveSiliconCuts) TrSimUtil::SetCuts(cut);
...
#if G4VERSION_NUMBER  > 999
#ifdef __G4POLARGAMMA__
      if(G4FFKEY.GamaPolar%10>=1) {// add gamma polarization
        cout<<"#############3  enering gamma polarisation process "<<endl;
//        pmanager->AddDiscreteProcess( new G4PolarizedPhotoElectricEffect()  );  // working fine, but not necessary
//        pmanager->AddDiscreteProcess( new G4PolarizedCompton() );  // hidden bug
...
// not supported yet (G4BetheHeitler5D has part)
...
ssm->SetLowEnergyLimit(highEnergyLimit);
 ssm->SetActivationLowEnergyLimit(highEnergyLimit);
      ...
      if(GCPHYS.IMULS)ph->RegisterProcess(msc, particle);
      else cerr<<" no mscat for electrons "<<endl;
 ph->RegisterProcess(eioniPAI, particle);
 if(GCPHYS.IBREM)ph->RegisterProcess(new G4eBremsstrahlung(), particle);
 else cerr<<" no brem for electrons "<<endl;
ph->RegisterProcess(ss, particle);
ph->RegisterProcess(processXTR, particle);
#else
      pmanager->AddProcess(new G4eMultipleScattering, -1, 1, 1);
      pmanager->AddProcess(eioniPAI,                 -1, 2, 2);
      pmanager->AddProcess(new G4eBremsstrahlung,    -1, 3, 3);
...
//               particleName == "triton" ||      
      ...
      if( particleName == "GenericIon" ) {
        if(G4FFKEY.IonParametrisedLossStep>0){
          theIonIonisation->SetEmModel(new G4IonParametrisedLossModel());
          theIonIonisation->SetStepFunction(0.1, G4FFKEY.IonParametrisedLossStep*um);
        }
      }
      if(TRDMCFFKEY.PAIModel ){
	G4PAIModel*     pai = new G4PAIModel(particle,"PAIModel");
	theIonIonisation->AddEmModel(0,pai,pai,gasregion); 
      ...
      if(GCPHYS.IMULS==1){
       ...
       if(G4FFKEY.IonParametrisedLossStep!=0)pmanager->AddProcess(pnuc,-1,4,-1);//nuclear stopping
       pmanager->AddProcess(piss);//this may not regisited
//       pmanager->AddProcess(piss,-1,-1, 4);//this is correct procedure
      }
      else if(GCPHYS.IMULS==2) {  // old ms
       pmanager->AddProcess(new G4hMultipleScattering,-1,1,1);
       pmanager->AddProcess(theIonIonisation,-1,2,2);
       if(G4FFKEY.IonParametrisedLossStep!=0)pmanager->AddProcess(pnuc,-1,3,-1);//nuclear stopping
      }
      else{
       cerr<<" AMSG4Physics::ConstructEM2-W-NoMultipleScatteringForUIons "<<endl;
       pmanager->AddProcess(theIonIonisation,-1,1,1);
       if(G4FFKEY.IonParametrisedLossStep!=0)pmanager->AddProcess(pnuc,-1,2,-1);//nuclear stopping
      }
      pmanager->AddDiscreteProcess(processXTR);
	...
	thehIonisation->AddEmModel(0,pai,pai,gasregion);
      ...
      if(GCPHYS.IMULS==1){
       ...
       pmanager->AddProcess(new G4hPairProduction,     -1,4, 4);
       pmanager->AddProcess(piss);//this may not regisited
//       pmanager->AddProcess(piss,-1,-1, 5);//this is correct procedure
      }
...
#include "job.h"
#include <memory>
bool AMSG4Physics::RemoveProcess(G4ParticleDefinition* theParticle, const string & name){
				G4ProcessManager * processManager = theParticle->GetProcessManager();
				G4ProcessVector * processVector = processManager->GetProcessList();
                                G4VProcess *rm=0;
				for(G4int j=0; j<processVector->size(); j++){
				if((*processVector)[j]->GetProcessName()==G4String(name)){
                                     rm=(*processVector)[j];
                                     break;
                                }
                                }
                               if(rm && processManager->RemoveProcess(rm))return true;
                                else return false;
}
	...
	//	cout << "----------------> Particle Name = " << name << endl;
		...
		if(theParticle){
			...
			for(G4int i=0; i<nElements; i++){
				...
				for(G4int j=0; j<processVector->size(); j++){
					bool inelok=false;
					if(theParticle==G4Proton::Definition()) inelok= ((*processVector)[j]->GetProcessName()=="ProtonInelastic" || (*processVector)[j]->GetProcessName()=="protonInelastic") ;
					else if(theParticle==G4AntiProton::Definition())inelok= ((*processVector)[j]->GetProcessName()=="AntiProtonInelastic" || (*processVector)[j]->GetProcessName()=="anti_protonInelastic") ;
					else if(theParticle== G4Alpha::Definition())inelok= (*processVector)[j]->GetProcessName()=="alphaInelastic";
#if G4VERSION_NUMBER  > 999
					else if(theParticle== G4AntiAlpha::Definition())inelok= ((*processVector)[j]->GetProcessName()=="aalphaInelastic" || (*processVector)[j]->GetProcessName()=="anti_alphaInelastic");
#endif
					else if(theParticle==   G4He3::Definition())inelok= (*processVector)[j]->GetProcessName()=="He3Inelastic";
#if G4VERSION_NUMBER  > 999
					else if(theParticle==   G4AntiHe3::Definition())inelok= ((*processVector)[j]->GetProcessName()=="aHe3Inelastic" || (*processVector)[j]->GetProcessName()=="anti_He3Inelastic")    ;
#endif
					else if(theParticle==   G4Triton::Definition())inelok=  (*processVector)[j]->GetProcessName()=="tInelastic"    ;
#if G4VERSION_NUMBER  > 999
					else if(theParticle==   G4AntiTriton::Definition())inelok=  ((*processVector)[j]->GetProcessName()=="atInelastic" || (*processVector)[j]->GetProcessName()=="anti_tritonInelastic")    ;
#endif					
else if(theParticle==   G4Deuteron::Definition())inelok= (*processVector)[j]->GetProcessName()=="dInelastic"    ;
#if G4VERSION_NUMBER  > 999
					else if(theParticle==   G4AntiDeuteron::Definition())inelok= ((*processVector)[j]->GetProcessName()=="adInelastic" || (*processVector)[j]->GetProcessName()=="anti_deuteronInelastic")    ;
#endif
...
//						cout << "g4physics::SaveXS(): " << hadronInelasticProcess->GetProcessName() << ", aScaleFactor=" << hadronInelasticProcess->aScaleFactor << endl;
				...
				for(G4int j=0; j<processVector->size(); j++){
					...
					else if(theParticle== G4Alpha::Definition())inelok= ((*processVector)[j]->GetProcessName()=="ionelastic" || (*processVector)[j]->GetProcessName()=="NNDiffuseElastic")  ;
					else if(theParticle==   G4He3::Definition())inelok= ((*processVector)[j]->GetProcessName()=="ionelastic" || (*processVector)[j]->GetProcessName()=="NNDiffuseElastic") ;
#if G4VERSION_NUMBER  > 999
					else if(theParticle== G4AntiAlpha::Definition())inelok= ((*processVector)[j]->GetProcessName()=="aionelastic" || (*processVector)[j]->GetProcessName()=="hadElastic");
					else if(theParticle==   G4AntiHe3::Definition())inelok= ((*processVector)[j]->GetProcessName()=="aionelastic" || (*processVector)[j]->GetProcessName()=="hadElastic");
#endif
					else if(theParticle==   G4Triton::Definition())inelok=  ((*processVector)[j]->GetProcessName()=="ionelastic" || (*processVector)[j]->GetProcessName()=="NNDiffuseElastic")   ;
					else if(theParticle==   G4Deuteron::Definition())inelok= ((*processVector)[j]->GetProcessName()=="ionelastic"|| (*processVector)[j]->GetProcessName()=="NNDiffuseElastic")    ;
#if G4VERSION_NUMBER  > 999
					else if(theParticle==   G4AntiTriton::Definition())inelok= ((*processVector)[j]->GetProcessName()=="aionelastic" || (*processVector)[j]->GetProcessName()=="hadElastic");
					else if(theParticle==   G4AntiDeuteron::Definition())inelok= ((*processVector)[j]->GetProcessName()=="aionelastic" || (*processVector)[j]->GetProcessName()=="hadElastic")    ;
#endif
					else if ( strstr((const char*)theParticle->GetParticleName(),"strangelet"))inelok=  (*processVector)[j]->GetProcessName()=="strangeletelastic"    ;
					else if( strstr((const char*)theParticle->GetParticleType(),"nucleus"))inelok=  ((*processVector)[j]->GetProcessName()=="ionelastic" || (*processVector)[j]->GetProcessName()=="NNDiffuseElastic");               
