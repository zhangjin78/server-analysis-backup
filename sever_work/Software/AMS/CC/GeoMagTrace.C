//  $Id: GeoMagTrace.C,v 1.9 2016/03/28 18:25:08 shaino Exp $
...
double GeoMagTrace::Rs = GeoMagTrace::Re*3.5;   // Escape limit         in km
...
int GeoMagTrace::Propagate(double step)
{
  ...
  double dx = vout[0]-vin[0], dy = vout[1]-vin[1], dz = vout[2]-vin[2];
  ...
  double r1 = GetRadi();
  if (r1 > _rmax) _rmax = r1;

  double dr = (r1-r0)/l;
  if (_dmin == 0 || dr < _dmin) _dmin = dr;
...
double GeoMagTrace::GetBfield(double *bf) const
{
  double *b = bf, bb[3];
  if (!b) b = bb;

  GuFld(_x, _y, _z, b);
  return TMath::Sqrt(b[0]*b[0]+b[1]*b[1]+b[2]*b[2]);
}
...
C.
   ...
   Imported to C++ by SH
   Bugfix found by H.Chou 2014/1/7 ; 2016/3/28
*/
...
double GeoMagTrace::_buf[12];

void GeoMagTrace::SetBuf(double rpt[3], double vpt[3], double ypr[3],
		         double xtime,  double theta,  double phi)
{
  double *p = _buf;
  for (int i = 0; i < 3; i++) *(p++) = rpt[i];
  for (int i = 0; i < 3; i++) *(p++) = vpt[i];
  for (int i = 0; i < 3; i++) *(p++) = ypr[i];
  *(p++) = xtime;
  *(p++) = theta;
  *(p++) = phi;
}

GeoMagTrace GeoMagTrace::Gprop(double rigidity, int nmax)
{
  GeoMagTrace gp(&_buf[0], &_buf[3], &_buf[6],
		  _buf[9],  _buf[10], _buf[11], rigidity, 1);
  gp.Propagate(nmax);
  return gp;
}
