//  $Id: particle.C,v 1.278 2024/02/20 13:55:34 choutko Exp $
...
integer AMSParticle::build(integer refit){
  ...
  if(!partfound || 1){//alwasy enter
	...
	AMSBeta * pbeta=pcharge->getpbeta();
        ...
        AMSTrTrack * ptrack=pcharge->getptrack();
#ifdef __AMSDEBUG__
	//          assert (ptrack && pbeta && pcharge);
#endif   
	if(!ptrack || (!pbetah&&!pbeta) || !pcharge){
...
/*          if(charge<2 && partfound){
          ...
          number beta=0,ebeta=0;
          if(pbetah){
            beta=pbetah->GetBeta();
            ebeta=pbetah->GetEBetaV()*beta*beta;
...
if(_pbetah){
  for(int ilay=0;ilay<4;ilay++){
    AMSPoint tofhc=_pbetah->GetClusterHLCoo(ilay);
   _ptrack->interpolate(tofhc,dir,_TOFCoo[ilay],theta,phi,_TOFTLength[ilay]);
    if(!(_pbetah->TestExistHL(ilay))){//small Z-shift correction for no hit layer
      float dis; bool isinbar=0;
      tofhc=TOFGeom::GetBarCoo(ilay,TOFGeom::FindNearBar(ilay,_TOFCoo[ilay][0],_TOFCoo[ilay][1],dis,isinbar));
      _ptrack->interpolate(tofhc,dir,_TOFCoo[ilay],theta,phi,_TOFTLength[ilay]);
    }
  }
}
...
void AMSParticle::trd_likelihood(){
  ...
  for (unsigned int n=0;n<AMSTRDIdSoft::ncrates();n++){
    AMSTRDRawHit * ptr=
...
void AMSParticle::trdfit(){
  ...
  if(ptr->checkstatus(AMSDBc::DELETED))continue;
    ...
    if(_ptrack->checkstatus(AMSDBc::NOTRACK)){
     ...
     AMSPoint tofe(0,0,0);
     if(_pbetah){
       ntof=_pbetah->NTofClusterH();
       for(int kk=0;kk<ntof;kk++){tofe=tofe+AMSPoint(_pbetah->pTofClusterH(kk)->ECoo);}
     }
     else {//for beta
      if(!_pbeta){
        ntof=0;
...
void AMSParticle::pid(){
  ...
  number prob[maxp];
  number pfit[maxp];
  if((_pbeta||_pbetah) && !_pvert){
    if(_pbetah){
      _beta=1./fabs(_pbetah->GetBeta());
      _ebeta=_pbetah->GetEBetaV();
    }
    else {
    ...
    if((antimatter || _Momentum <0) && !_ptrack->checkstatus(AMSDBc::NOTRACK) && !_ptrack->checkstatus(AMSDBc::TRDTRACK)&& !_ptrack->checkstatus(AMSDBc::ECALTRACK)){
      ...
      AMSEvent::gethead()->setmoreerror(14); //AntiMatter Trigger Present
...
void AMSParticle::refit(int fast){
  ...
  else if(_pbeta) beta=_pbeta->getbeta();
  _Coo.setp(0,0,0);
#ifdef _PGTRACK_
...
/*	_TrCoo[layer]=_ptrack->GetResidualO(layer+1);
	...
	AMSDir dir;
        ...
        if(_ptrack->intercept(_TrCoo[layer],layer,theta,phi,_Local[layer])==-2){//QY
          setstatus(AMSDBc::BADINTERPOL);//interpolation error
          continue;
        }
#else
        bool bad=false;
	...
	if(beta>=0 && _TrCoo[layer][2]>_Coo[2]){
	  _Theta=theta;
	  _Phi=phi;
	  _Coo=_TrCoo[layer];
	...
	else if(beta<0 && _TrCoo[layer][2]<_Coo[2]){
	   _Theta=theta;    
	   // change theta according to beta
	   _Phi=phi;
	   _Coo=_TrCoo[layer];
...
///-Adding BetaH Vertex Association (higher priority) 
...
#endif
            number tofhq=betah->getsumq();
            if(tofhq>maxhq){_pbetah=betah; _Beta=_pbetah->GetBeta(); _ErrBeta=_pbetah->GetEBetaV()*_Beta*_Beta; maxhq=tofhq;}
          }
       }
       betah=betah->next();
    }
