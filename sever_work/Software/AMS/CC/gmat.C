//  $Id: gmat.C,v 1.142 2024/03/04 19:39:15 qyan Exp $
// Author V.Choutko.
...
//
//  Changed to compaund VC 106-02-07
//
//  mat.add (new AMSgmat("MYLAR",  8.7   ,4.5,  1.39  ,28.7, 43.));
{
  ...
  geant z[]={6.,1.,8.};
  geant w[]={10.,8.,4.};
  mat.add (new AMSgmat("MYLAR",a,z,w,3,  1.39  ,28.7, 43.));
}
...
// 30/11/2011  updated with info from Lubelsmeyer
  ...
  // Track Feet PEEK ( (C H_2 O_5)n ) 

  {
    float a[3] = {1.00999999, 12.0100002, 16};
    float z[3] = {1, 6, 8 };
    float w[3] = {2, 1,  5};
    density=1.4*TRMCFFKEY.PEEKDensityBias;
    mat.add(new AMSgmat("TrPEEK",a,z,w,3,density));
  }

  // Track Feet Al
  density=2.7*TRMCFFKEY.FeetAlDensityBias;
  mat.add(new AMSgmat("TrFeet-Alloy",26.981,13,density, 8.897*2.7/density,  39.7*2.7/density));
...
// PORON
{
  mat.add (new AMSgmat("RICH_PORON", 12.01, 6., 0.3 , 33., 66.));
}
...
{ // PMTs
  ...
  geant z[]={14.,8.};
  geant w[]={1.,2.};
  mat.add (new AMSgmat("PMT_BODY",26.98, 13., 1.7, 17.8, 74.4));
}
...
{ // Absorber walls: plexiglass too... C5H8O2
  ...
  mat.add (new AMSgmat("RICH_CARBONF", 12.01, 6., 1.6 , 33., 66.));

  // Magnetic shielding
  mat.add (new AMSgmat("RICH_BSHIELD",55.85,26.,7.87,17.6,168.));
...
void AMSgtmed::CUTGAM(geant cut){
  _G4EPh=cut;
#ifdef __AMSVMC__
#else
...
void AMSgtmed::CUTHAD(geant cut){
  _G4EHad=cut;
#ifdef __AMSVMC__
#else
...
void AMSgtmed::CUTNEU(geant cut){
  _G4EHNeu=cut;
#ifdef __AMSVMC__
#else
...
void AMSgtmed::CUTELE(geant cut){
  _G4EEl=cut;
#ifdef __AMSVMC__
#else
...
void AMSgtmed::CUTMUO(geant cut){
  _G4EMu=cut;
#ifdef __AMSVMC__
#else
...
void AMSgtmed::TOFMAX(geant cut){
  _G4TofMax=cut;
#ifdef __AMSVMC__
#else
...
//---------------------------------------------------
#ifdef _PGTRACK_
if (TRMCFFKEY.SetActiveSiliconCuts) { 
  AMSgtmed* pgtmedium = (AMSgtmed*) tmed.add (new AMSgtmed("ACTIVE_SILICON","SILICON",1,'N',0,1,20,10,1e-4*TRMCFFKEY.MaxStepInSilicon,-1,0.001,-1));
  pgtmedium->CUTELE(1e-6*TRMCFFKEY.MinDeltaRayEnergy);
}
else {
...
tmed.add (new AMSgtmed("ELECTRONICS","CARBON-TK",0));
 tmed.add ( new AMSgtmed("TrPEEK-M","TrPEEK",0));
 tmed.add ( new AMSgtmed("TrFeet-Alloy-M","TrFeet-Alloy",0));
...
//           "RICH_AEROGEL",1,'N',0,1,20,10,my_step));   //32, sensitive volume enable to be seen
...
//                                             "RICH_NaF",1,'N',0,1,20,10,RICHDB::max_step(RICHDB::naf_index)/2));sensitive volume enable to be seen
...
// Electronics
  ...
  // PMT Body
   pgtmed= (AMSgtmed*)tmed.add (new AMSgtmed("RPMT BODY","PMT_BODY",0));
  pgtmed->AGSCKOV(RICHDB::entries,p,abs_l,dummy,index,0);



  // PORON
  pgtmed= (AMSgtmed*)tmed.add (new AMSgtmed("RICH PORON","RICH_PORON",0));
  pgtmed->AGSCKOV(RICHDB::entries,p,abs_l,dummy,index,0);
