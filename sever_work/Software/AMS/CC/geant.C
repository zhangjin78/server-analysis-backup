//  $Id: geant.C,v 1.163 2024/05/28 13:17:34 nbelyaev Exp $
...
cout<<"  REMOVE-geant-I G4INIT finished"<<endl; 
#endif
#ifndef __AMSVMC__
GPHYSI();
#endif
  ...
  static int Out=0;
   ...
   unsigned long ulimit_size = ulimit_v.rlim_cur / 1024;
      ...
      if ( long(ulimit_size - vm_usage) < Threshold ) {
               cerr << "gams::mem_not_enough-E-Not enough memory limit : ULIMIT=" << ulimit_size << ", CUR_USAGE=" << vm_usage << ". Trying to terminate gracefully..." << endl;
               return 1;
                }
...
#else
   uint64_t free_mem;
   Kernel_GetMemorySize(KERNEL_MEMSIZE_HEAP, &free_mem);
   free_mem/=1024;
   free_mem=mpi.Memsizekb-free_mem;
   if (1) {
       //cout << "gams::mem_not_enough-I-Available memory space : " << free_mem << endl;
       if ( (MISCFFKEY.DynThreads && free_mem < Threshold-value*Out/3 )|| free_mem<2*value) {
    ...
    cerr << "gams::mem_not_enough-E-Not enough memory space : FREE PHY+SWAP=" << free_mem ;
    cerr<<", CUR_USAGE=" << vm_usage << ". Trying to terminate gracefully..." << mpi.RunMode<<endl;
#ifdef G4MULTITHREADED
       if ( free_mem < 2*value) {
           cerr<<" gams::mem_not_enough-E-AbortingRun "<<G4Threading::G4GetThreadId()<<endl;
           GCFLAG.IEORUN=1;
          GCFLAG.IEOTRI=1;
          G4AllocatorPool::gThreshold=0;
       } 
           cerr<<" gams::mem_not_enough-E-Exiting "<<G4Threading::G4GetThreadId()<<" "<<Out<<endl;
#pragma omp atomic
           Out++;
           pthread_exit(NULL);
#endif
...
#endif
   if ( free_mem < value ) {
     cerr << "gams::mem_not_enough-E-Not enough memory space : FREE PHY+SWAP=" << free_mem << ", CUR_USAGE=" << vm_usage << ". Trying to terminate gracefully..." << mpi.RunMode<<endl;
     GCFLAG.IEORUN=1;
     GCFLAG.IEOTRI=1;
     sleep(120);
     if(mpi.RunMode<2)raise(SIGINT);
     else sleep(12000);
     return 1;
   }
   return 0;
