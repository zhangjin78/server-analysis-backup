...
#ifdef _PGTRACK_
          integer iid=ptrack->Gettrdefaultfit()&(~(AMSTrTrack::kFitLayer8|AMSTrTrack::kFitLayer9));//no external layer
          if(
	     (ptrack->GetPar(iid|AMSTrTrack::kUpperHalf).FitDone)&&
	     (ptrack->GetPar(iid|AMSTrTrack::kLowerHalf).FitDone)
	     )
	    {
	      hrid[0]=ptrack->GetRigidity(iid|AMSTrTrack::kUpperHalf);
	      hrid[1]=ptrack->GetRigidity(iid|AMSTrTrack::kLowerHalf);
	    }
#else
          if(AdvFit)ptrack->
