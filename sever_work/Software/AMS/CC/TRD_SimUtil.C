...
//    w[i]=trdTubeGasMaterial->GetFractionVector()[i]/a[i]*1.e5;
    if(TRDMCFFKEY.debug)printf("comp %s - A %.2f Z %.2f W %.2f\n",trdTubeGasMaterial->GetElementVector()->at(i)->GetSymbol().data(),a[i],z[i],w[i]);
  }

  geant density=trdTubeGasMaterial->GetDensity()/g*cm3;
  geant temp=   trdTubeGasMaterial->GetTemperature()/kelvin;
  geant radl=   trdTubeGasMaterial->GetRadlen()/cm;
  geant absl=   trdTubeGasMaterial->GetNuclearInterLength()/cm;
  AMSgmat *thismat=new AMSgmat ("TrdGasGmat", a, z, w, n, density,radl,absl,temp);
...
//    w[i]=trdRadiatorArtificial->GetFractionVector()[i]/a[i]*1.e4;
    if(TRDMCFFKEY.debug)printf("comp %s - A %.2f Z %.2f W %.2f\n",trdRadiatorArtificial->GetElementVector()->at(i)->GetSymbol().data(),a[i],z[i],w[i]);
  }

  geant density=trdRadiatorArtificial->GetDensity()/g*cm3;
  geant temp=   trdRadiatorArtificial->GetTemperature()/kelvin;
  geant radl=   trdRadiatorArtificial->GetRadlen()/cm;
  geant absl=   trdRadiatorArtificial->GetNuclearInterLength()/cm;

  AMSgmat *thismat=new AMSgmat ("TrdArtRadGmat", a, z, w, n, density,radl,absl,temp);
...
//    w[i]=trdKaptonMaterial->GetFractionVector()[i]/a[i]*1.e4;
    if(TRDMCFFKEY.debug)printf("comp %s - A %.2f Z %.2f W %.2f\n",trdKaptonMaterial->GetElementVector()->at(i)->GetSymbol().data(),a[i],z[i],w[i]);
  }

  geant density=trdKaptonMaterial->GetDensity()/g*cm3;
  geant temp=   trdKaptonMaterial->GetTemperature()/kelvin;
  geant radl=   trdKaptonMaterial->GetRadlen()/cm;
  geant absl=   trdKaptonMaterial->GetNuclearInterLength()/cm;

  //  delete H,C,N,O;
  AMSgmat *thismat=new AMSgmat ("Kapton", a, z, w, n, density,radl,absl,temp,trdKaptonMaterial);
