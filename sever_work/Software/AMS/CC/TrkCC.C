...
void TrkCC::_LoadFiles(){
  ...
  if( _runtype==0) {
    ...
    char* dbPath_temp = getenv("TrkCCDataDir");
    TString dbPath = (dbPath_temp && strlen(dbPath_temp))? dbPath_temp: Form("%sTrkCC",AMSDATADIR.amsdatadir);//the directory has been changed to v6.00
...
int TrkCC::ProcessEvent(TrkCCVar* extTrkCCVar, int maskingnumber, float energyGuess){
  ...
  if (MyAbs(_normRIG)==1) algCC->_evenergy=MyAbs(varCC->rigidity_ch);
  if (MyAbs(_normRIG)==2) algCC->_evenergy=MyAbs(varCC->rigidity_ch);
  if (algCC->_evenergy==0) algCC->_evenergy=MyAbs(varCC->rigidity_ch);
...
int TrkCC::_NormalizeList(){
  ...
  double logenergy=log10(MyAbs(algCC->_evenergy));
  ...
  for( int iv=0; iv<_nvars; iv++){
	...
	normvalue=_NormBozNoz(value, logenergy, 
      ...
      {normvalue = value;}
    if( !(MyAbs(normvalue)<1.e37)){ // Avoid NAN
	if(normvalue>1.e36)  normvalue=1.e36; //clamp + inf
	if(normvalue<-1.e36) normvalue=-1.e36;//clamp - inf
	if( !(MyAbs(normvalue)<1.e37)) normvalue = 0; // correct NAN
    }
...
int TrkCC::FillTH1(TH1* histo, int regtype, unsigned long long masktype){
  ...
  int ifill = 0;
  for (int ivv=0;ivv<_nvars;ivv++){
    if (!(MyAbs(algCC->_varnorm[ivv]) >=0)) continue;
    double a = 1.;
    if (_algo.Contains("Nozzoli")){
    ...
    if(MyAbs(a)<1.e-30) continue;
    if( (algCC->_usedmask & ((unsigned long long) 1 << ivv)) == 0) continue;
    ifill++;
    double varfill = algCC->_varnorm[ivv]*a/MyAbs(a);
    histo->Fill(varfill);
  }
return ifill;
}
...
unsigned long long TrkCC::_FillVarList(int maskingnumber){
  ...
  for( int iv=0; iv<_nvars; iv++){
    ...
    if (islog==0) algCC->_varlist[iv]=MyAbs(varlistfull[vpos]);
    if (islog==3) algCC->_varlist[iv]=(varlistfull[vpos]);
    if (errfull[vpos]==1) {
      recommendmask &= ~((unsigned long long) 1 << iv); // unset the iv bit
      //cout << iv << " masked1" << endl;
}
    if( islog==1) algCC->_varlist[iv] = log10(MyAbs(varlistfull[vpos])+1.e-09);
    if( islog==2 && vpos==0) { //log(E/P)  (iv=21)
      algCC->_varlist[iv] =log10(MyAbs(varlistfull[66]/varlistfull[5]));
      if (errfull[66]==1 || errfull[5]==1) {recommendmask &= ~((unsigned long long) 1 << iv); // unset the iv bit
	//cout << iv << " masked2" << endl;
      }
    }
    if( islog==2 && vpos==1) {//inner half Rigidity check (iv=2)
      algCC->_varlist[iv] = log10(MyAbs((varlistfull[13]-varlistfull[14]+1.e-6)/(1.e-6+varlistfull[13]+varlistfull[14])));
      if (errfull[13]==1 || errfull[14]==1) {recommendmask &= ~((unsigned long long) 1 << iv); // unset the iv bit
	//cout << iv << " masked3 " <<errfull[13]<<errfull[14]<< endl;
      }
    }
    if( islog==2 && vpos==2) {// top-bottom Rigidity check (iv=27) (only fullspan)
      algCC->_varlist[iv] = log10(MyAbs((varlistfull[11]-varlistfull[12]+1.e-6)/(1.e-6+varlistfull[11]+varlistfull[12])));
      if (errfull[11]==1 || errfull[12]==1 || algCC->_evpatt!=5) {recommendmask &= ~((unsigned long long) 1 << iv); // unset the iv bit
	//cout << iv << " masked4" << endl;
      }
    }
    if( islog==2 && vpos==3) {// inner-maxspan Rigidity check (iv=5) (no inner pattern)
      algCC->_varlist[iv] = log10(MyAbs((varlistfull[10]-varlistfull[5]+1.e-6)/(1.e-6+varlistfull[10]+varlistfull[5])));
      if (errfull[10]==1 || errfull[5]==1 || algCC->_evpatt<2) {recommendmask &= ~((unsigned long long) 1 << iv); // unset the iv bit
	//cout << iv << " masked5" << endl;
      }
    }
    if( islog==2 && vpos==4) { // choutko vs alcaratz Rigidity check (iv=28)
      algCC->_varlist[iv] = log10(MyAbs((varlistfull[5]-varlistfull[6]+1.e-6)/(1.e-6+varlistfull[5]+varlistfull[6])));
      if (errfull[5]==1 || errfull[6]==1) {recommendmask &= ~((unsigned long long) 1 << iv); // unset the iv bit
	//cout << iv << " masked6 " << errfull[5]<<errfull[6]<<endl;
      }
    }
    ...
    if( islog==2 && vpos==10) {// |1-E/P| (iv=22)
      algCC->_varlist[iv] =MyAbs(1.-MyAbs(varlistfull[66]/varlistfull[5]));
      if (errfull[66]==1 || errfull[5]==1) {recommendmask &= ~((unsigned long long) 1 << iv); // unset the iv bit
	//      cout << iv << " masked8" << endl;
      }
    }
    ...
    if( islog==11 && log10(MyAbs(varlistfull[vpos]))>fisflat) intint++;
    if( islog==11 && log10(MyAbs(varlistfull[vpos]))>hlow)    intint++;
    if( islog==11 && log10(MyAbs(varlistfull[vpos]))>hhigh)   intint++;
    if( islog==11 && log10(MyAbs(varlistfull[vpos]))>fnbins)  intint++;
    if( islog==11 && log10(MyAbs(varlistfull[vpos]))>hloglow) intint++;
    if( islog==11 && log10(MyAbs(varlistfull[vpos]))>hloghigh) intint++;
    if( islog==11 && log10(MyAbs(varlistfull[vpos]))>flinint) intint++;
			   ...
			   Int_t ilin, Bool_t cureTails)
{
  ...
  if (ilin == 0){
    ...
    Double_t cv = (lv+uv)/2.;
    Double_t dlt = MyAbs(lv-uv)/2.;  // the sigma width
    if (dlt == 0) dlt = 1;
...
Double_t  TrkCC::_Findc(Double_t a,Double_t b,Double_t& ex){
  ...
  xright = x;
  Double_t goal = MyAbs(ex);   
  if (goal >0.1) goal = 0.1;
  ex = MyAbs((xleft-xright)/(xleft+xright));
  int n=0;
  while (ex>goal && y != 0){               
    n++;
...
Double_t TrkCC::_Nozly(Double_t eta, Double_t x)
{
  ...
  if (eta >0){
    ...
    T = (x/MyAbs(eta*x))*log(MyAbs(eta*x)+1); 
  }
  return T;
}
...
// varnumber = -1 Init all
void TrkCCVar::Init(int varnumber){
  if(varnumber<0 || varnumber==0) nmsk=0;
  else if(varnumber==-1 || varnumber==1) ly1_q=-1;
  else if(varnumber==-1 || varnumber==2) ly2_q=-1;
  else if(varnumber==-1 || varnumber==3) ly9_q=-1;
  else if(varnumber==-1 || varnumber==4) pat=9999;
  else if(varnumber==-1 || varnumber==5) rigidity_ch=0;
  else if(varnumber==-1 || varnumber==6) rigidity_al=0;
  else if(varnumber==-1 || varnumber==7) x0rad=0;
  else if(varnumber==-1 || varnumber==8) chix_ch=0;
  else if(varnumber==-1 || varnumber==9) chiy_ch=0;
  else if(varnumber==-1 || varnumber==10) rigidity_in=0;
  else if(varnumber==-1 || varnumber==11) rigidity_bt=0;
  else if(varnumber==-1 || varnumber==12) rigidity_tp=0;
  else if(varnumber==-1 || varnumber==13) rigidity_ibt=0;
  else if(varnumber==-1 || varnumber==14) rigidity_itp=0;
  else if(varnumber==-1 || varnumber==15) err_ch=0;
  else if(varnumber==-1 || varnumber==16) axischi2=0;
  else if(varnumber==-1 || varnumber==17) trk_qmaxl9=1;
  else if(varnumber==-1 || varnumber==18) tof_qd=1;
  else if(varnumber==-1 || varnumber==19) tof_qmax=1;
  else if(varnumber==-1 || varnumber==20) rich_nunused=0;
  else if(varnumber==-1 || varnumber==21) pMDR=9999;
  else if(varnumber==-1 || varnumber==22) nlbks=0; 
  else if(varnumber==-1 || varnumber==23) rich_ratio=1;
  else if(varnumber==-1 || varnumber==24) rich_res=0;
  else if(varnumber==-1 || varnumber==25) trk_cofgx=0;
  else if(varnumber==-1 || varnumber==26) trk_dirx=0;
  else if(varnumber==-1 || varnumber==27) tof_chi2c=0;
  else if(varnumber==-1 || varnumber==28) tof_chi2t=0;
  else if(varnumber==-1 || varnumber==29) nbks=0;
  else if(varnumber==-1 || varnumber==30) ntrkcl9=1;
  else if(varnumber==-1 || varnumber==31) ntrk=1;
  else if(varnumber==-1 || varnumber==32) nbeta=1;
  else if(varnumber==-1 || varnumber==33) nrings=1;
  else if(varnumber==-1 || varnumber==34) trk_cofgy=0;
  else if(varnumber==-1 || varnumber==35) trk_diry=0;
  else if(varnumber==-1 || varnumber==36) trd_diry=0;
  else if(varnumber==-1 || varnumber==37) tof_qu=1;
  else if(varnumber==-1 || varnumber==38) tof_qrms=0;
  else if(varnumber==-1 || varnumber==39) tof_nused=0;
  else if(varnumber==-1 || varnumber==40) tof_beta=1;
  else if(varnumber==-1 || varnumber==41) trk_q=1;
  else if(varnumber==-1 || varnumber==42) trk_yrms=0;
  else if(varnumber==-1 || varnumber==43) trk_qmaxl18=1;
  else if(varnumber==-1 || varnumber==44) trd_ntrk=1;
  else if(varnumber==-1 || varnumber==45) trd_nhit=20;
  else if(varnumber==-1 || varnumber==46) trd_ratio=1;
  else if(varnumber==-1 || varnumber==47) trd_ratioff=1;
  else if(varnumber==-1 || varnumber==48) trd_sumv=0;
  else if(varnumber==-1 || varnumber==49) trd_noff=0;
  else if(varnumber==-1 || varnumber==50) trd_path=0;
  else if(varnumber==-1 || varnumber==51) trd_mindist=0;
  else if(varnumber==-1 || varnumber==52) trd_q=1;   
  else if(varnumber==-1 || varnumber==53) trd_chi2=0;
  else if(varnumber==-1 || varnumber==54) trk_qrms=0;
  else if(varnumber==-1 || varnumber==55) trk_qmax=1;
  else if(varnumber==-1 || varnumber==56) ntrky5cl18=1;
  else if(varnumber==-1 || varnumber==57) nint=0;
  else if(varnumber==-1 || varnumber==58) dtrkavgy=0;
  else if(varnumber==-1 || varnumber==59) rich_beta=1;
  else if(varnumber==-1 || varnumber==60) rich_rms=0;
  else if(varnumber==-1 || varnumber==61) minoise1=0;
  else if(varnumber==-1 || varnumber==62) minoise2=0;
  else if(varnumber==-1 || varnumber==63) minoise3=0;
  else if(varnumber==-1 || varnumber==64) minoise4=0;
  else if(varnumber==-1 || varnumber==65) nlayer=7;
  else if(varnumber==-1 || varnumber==66) energy=0;
  else if(varnumber==-1 || varnumber==67) maxresx0=0;
  else if(varnumber==-1 || varnumber==68) maxresy0=0;
  else if(varnumber==-1 || varnumber==69) maxresx1=0;
  else if(varnumber==-1 || varnumber==70) maxresy1=0;
  else if(varnumber==-1 || varnumber==71) minresx=0;
  else if(varnumber==-1 || varnumber==72) minresy=0;
  else if(varnumber==-1 || varnumber==73) chix_inn_ch=0;
  else if(varnumber==-1 || varnumber==74) chiy_inn_ch=0;
  else if(varnumber==-1 || varnumber==75) trkinn_q=1;
  else if(varnumber==-1 || varnumber==76) trk_ql2l4=1;
  else if(varnumber==-1 || varnumber==77) trk_ql5l8=1;
...
int TrkCCVar::CheckVar(int varnumber){
  if(varnumber==0 && !(MyAbs(nmsk)<1.e20)) return -1;
  else if(varnumber==1 && !(MyAbs(ly1_q)<1.e20)) return -1;
  else if(varnumber==2 && !(MyAbs(ly2_q)<1.e20)) return -1;
  else if(varnumber==3 && !(MyAbs(ly9_q)<1.e20)) return -1;
  else if(varnumber==4 && !(MyAbs(pat)<1.e20)) return -1;
  else if(varnumber==5 && !(MyAbs(rigidity_ch)<1.e20)) return -1;
  else if(varnumber==6 && !(MyAbs(rigidity_al)<1.e20)) return -1;
  else if(varnumber==7 && !(MyAbs(x0rad)<1.e20)) return -1;
  else if(varnumber==8 && !(MyAbs(chix_ch)<1.e20)) return -1;
  else if(varnumber==9 && !(MyAbs(chiy_ch)<1.e20)) return -1;
  else if(varnumber==10 && !(MyAbs(rigidity_in)<1.e20)) return -1;
  else if(varnumber==11 && !(MyAbs(rigidity_bt)<1.e20)) return -1;
  else if(varnumber==12 && !(MyAbs(rigidity_tp)<1.e20)) return -1;
  else if(varnumber==13 && !(MyAbs(rigidity_ibt)<1.e20)) return -1;
  else if(varnumber==14 && !(MyAbs(rigidity_itp)<1.e20)) return -1;
  else if(varnumber==15 && !(MyAbs(err_ch)<1.e20)) return -1;
  else if(varnumber==16 && !(MyAbs(axischi2)<1.e20)) return -1;
  else if(varnumber==17 && !(MyAbs(trk_qmaxl9)<1.e20)) return -1;
  else if(varnumber==18 && !(MyAbs(tof_qd)<1.e20)) return -1;
  else if(varnumber==19 && !(MyAbs(tof_qmax)<1.e20)) return -1;
  else if(varnumber==20 && !(MyAbs(rich_nunused)<1.e20)) return -1;
  else if(varnumber==21 && !(MyAbs(pMDR)<1.e20)) return -1;
  else if(varnumber==22 && !(MyAbs(nlbks)<1.e20)) return -1;
  else if(varnumber==23 && !(MyAbs(rich_ratio)<1.e20)) return -1;
  else if(varnumber==24 && !(MyAbs(rich_res)<1.e20)) return -1;
  else if(varnumber==25 && !(MyAbs(trk_cofgx)<1.e20)) return -1;
  else if(varnumber==26 && !(MyAbs(trk_dirx)<1.e20)) return -1;
  else if(varnumber==27 && !(MyAbs(tof_chi2c)<1.e20)) return -1;
  else if(varnumber==28 && !(MyAbs(tof_chi2t)<1.e20)) return -1;
  else if(varnumber==29 && !(MyAbs(nbks)<1.e20)) return -1;
  else if(varnumber==30 && !(MyAbs(ntrkcl9)<1.e20)) return -1;
  else if(varnumber==31 && !(MyAbs(ntrk)<1.e20)) return -1;
  else if(varnumber==32 && !(MyAbs(nbeta)<1.e20)) return -1;
  else if(varnumber==33 && !(MyAbs(nrings)<1.e20)) return -1;
  else if(varnumber==34 && !(MyAbs(trk_cofgy)<1.e20)) return -1;
  else if(varnumber==35 && !(MyAbs(trk_diry)<1.e20)) return -1;
  else if(varnumber==36 && !(MyAbs(trd_diry)<1.e20)) return -1;
  else if(varnumber==37 && !(MyAbs(tof_qu)<1.e20)) return -1;
  else if(varnumber==38 && !(MyAbs(tof_qrms)<1.e20)) return -1;
  else if(varnumber==39 && !(MyAbs(tof_nused)<1.e20)) return -1;
  else if(varnumber==40 && !(MyAbs(tof_beta)<1.e20)) return -1;
  else if(varnumber==41 && !(MyAbs(trk_q)<1.e20)) return -1;
  else if(varnumber==42 && !(MyAbs(trk_yrms)<1.e20)) return -1;
  else if(varnumber==43 && !(MyAbs(trk_qmaxl18)<1.e20)) return -1;
  else if(varnumber==44 && !(MyAbs(trd_ntrk)<1.e20)) return -1;
  else if(varnumber==45 && !(MyAbs(trd_nhit)<1.e20)) return -1;
  else if(varnumber==46 && !(MyAbs(trd_ratio)<1.e20)) return -1;
  else if(varnumber==47 && !(MyAbs(trd_ratioff)<1.e20)) return -1;
  else if(varnumber==48 && !(MyAbs(trd_sumv)<1.e20)) return -1;
  else if(varnumber==49 && !(MyAbs(trd_noff)<1.e20)) return -1;
  else if(varnumber==50 && !(MyAbs(trd_path)<1.e20)) return -1;
  else if(varnumber==51 && !(MyAbs(trd_mindist)<1.e20)) return -1;
  else if(varnumber==52 && !(MyAbs(trd_q)<1.e20)) return -1;
  else if(varnumber==53 && !(MyAbs(trd_chi2)<1.e20)) return -1;
  else if(varnumber==54 && !(MyAbs(trk_qrms)<1.e20)) return -1;
  else if(varnumber==55 && !(MyAbs(trk_qmax)<1.e20)) return -1;
  else if(varnumber==56 && !(MyAbs(ntrky5cl18)<1.e20)) return -1;
  else if(varnumber==57 && !(MyAbs(nint)<1.e20)) return -1;
  else if(varnumber==58 && !(MyAbs(dtrkavgy)<1.e20)) return -1;
  else if(varnumber==59 && !(MyAbs(rich_beta)<1.e20)) return -1;
  else if(varnumber==60 && !(MyAbs(rich_rms)<1.e20)) return -1;
  else if(varnumber==61 && !(MyAbs(minoise1)<1.e20)) return -1;
  else if(varnumber==62 && !(MyAbs(minoise2)<1.e20)) return -1;
  else if(varnumber==63 && !(MyAbs(minoise3)<1.e20)) return -1;
  else if(varnumber==64 && !(MyAbs(minoise4)<1.e20)) return -1;
  else if(varnumber==65 && !(MyAbs(nlayer)<1.e20)) return -1;
  else if(varnumber==66 && !(MyAbs(energy)<1.e20)) return -1;
  else if(varnumber==67 && !(MyAbs(maxresx0)<1.e20)) return -1;
  else if(varnumber==68 && !(MyAbs(maxresy0)<1.e20)) return -1;
  else if(varnumber==69 && !(MyAbs(maxresx1)<1.e20)) return -1;
  else if(varnumber==70 && !(MyAbs(maxresy1)<1.e20)) return -1;
  else if(varnumber==71 && !(MyAbs(minresx)<1.e20)) return -1;
  else if(varnumber==72 && !(MyAbs(minresy)<1.e20))  return -1;
  else if(varnumber==73 && !(MyAbs(chix_inn_ch)<1.e20)) return -1;
  else if(varnumber==74 && !(MyAbs(chiy_inn_ch)<1.e20)) return -1;
  else if(varnumber==75 && !(MyAbs(trkinn_q)<1.e20)) return -1;
  else if(varnumber==76 && !(MyAbs(trk_ql2l4)<1.e20)) return -1;
  else if(varnumber==77 && !(MyAbs(trk_ql5l8)<1.e20)) return -1;
...
int TrkCCVar::FillFromGbatch(ParticleR* part, int rebuildRefit, int gbatchPRODversion, bool fullProcess){
  ...
  // MyAbs(rebuildRefit)=iTrTrackPar 3rd parameter (0 = stored, 1 refit if necessary, 2 force trk refit, 3 fit and rebuild also Coo)
  ...
  x0rad=evt->GetRadiationLength(trk0,dtrk0,rigidity_ch/sqrt(MyAbs(btcor)),200.,0.); //BTCOR truth is in the middle?    
                                                           
  err_ch = 0.;
  int  iddc = trk->iTrTrackPar(11,0,ifit); // without MS 
  ...
  for (int layer=1; layer<=9; layer++) {
    ...
    noise_y[layer-1]=MyAbs(pnoise.y());
    laypa=laypa*10;
  }
  nlayer = 0;
  minoise1 = 7.;
  ...
  for (int layer=1; layer<=9; layer++) {
    ...
    if (trk_layer_pattern[layer-1] ==1){
      ...
      resappox[layer-1]=MyAbs(pres[0]);
      resappoy[layer-1]=MyAbs(pres[1]);
      if (maxresx0 < MyAbs(pres[0])) maxresx0 = MyAbs(pres[0]); 
      if (maxresy0 < MyAbs(pres[1])) maxresy0 = MyAbs(pres[1]); 
    }
  }
  ...
  for (int icl = 0; icl<ntrclu; icl++){ //scan trk cluster 
    ...
    if(trcl->GetSide()==1){nytrcl[layer]++;
      ...
      if(MyAbs(trcl->GetGCoord(0)-ytrk[layer])<5.) nytrcl5[layer]++;
  ...
  if (!ecal) {iret = iret*2;}
    ...
    double vv = ecalaxis->process(ecal,2,MyAbs(rigidity_ch)/rigidity_ch);//process ecalaxis using lateral fit method               
    if(vv>0){
  ...
  for(int i=0; i<ntrdtrk; i++){
    ...
    //      double cosapp = MyAbs(Dhtrd.prod(PDtrd));
    AMSPoint Dcrs = Dhtrd.crossp(PDtrd);
    double norm = Dcrs.norm();
    AMSPoint Phtrd;
    Phtrd.setp(Ptrd[0]-trdx,Ptrd[1]-trdy,Ptrd[2]-trdz);
    double minapp = MyAbs((Phtrd.prod(Dcrs))/norm);  // componente perpendicolare distanza                 
  ...
  if( trk && idd>=0 && evt->NTrdRawHit()>0 ){
    ...
    if (! (MyAbs(trd_q)<1.e20)) trd_q = 1.; // avoid nan
    /* no more necessary
  ...
  rich_res=MyAbs(xrich-trkrh[0])+MyAbs(yrich-trkrh[1]);
  rich_rms=0.;
...
#define ONLYY 
  ...
  DZ=MyAbs(inmiddleout[2].z()-inmiddleout[0].z());
  DY=MyAbs(inmiddleout[2].y()-inmiddleout[0].y());
  DX=MyAbs(inmiddleout[2].x()-inmiddleout[0].x());
  D_ZY=sqrt(DZ*DZ+DY*DY);
  D_ZX=sqrt(DZ*DZ+DX*DX);
  L=D_ZY;
  SenThetaX=MyAbs(DZ/D_ZX);//vertical means ThetaX=90 -> SenThetaX=1
  SenThetaY=MyAbs(DZ/D_ZY);//vertical means ThetaY=90 -> SenThetaY=1
  /*
    static Double_t y1;
    static Double_t y2;
    static Double_t y3;
...
Int_t TMVAxmlReader::_GetVariables(TString varhead,  TString key, FILE *file,vector <TString> &bdtvars){
  ...
  for( int i=0; i<nofvars; i++){
    if(fgets(buf,sizeof(buf),file)!=0){
      ...
      bdtvars.push_back((TString)expression.c_str());
    }
  }
  return nofvars;
}
...
int TrkCCNozzoli::EvaluateRegression(float varnor[], float a[], float b[], float ps[], float reg[], int nvr, float& vreg, int debbugg, unsigned long long masskk){
  ...
  for (int ivv=0;ivv<nvr;ivv++){  //single variable regression
    ...
    if (!(MyAbs(varnor[ivv]) >=0)) nanflag=ivv;
    if (!(MyAbs(varnor[ivv]) >=0)) vappo=0;
    if (ps[ivv]<0 && vappo!=0) vappo = ((log10(MyAbs(vappo))+MyAbs(vappo))-0.523158)/1.03787; //normalized fabs distribution
    if(vappo>7) vappo=7.;
    if(vappo<-7) vappo=-7.;
    reg[ivv]=0;
    if( (! (MyAbs(a[ivv])<1.e30)) || (MyAbs(a[ivv])<1.e-30)) { reg[ivv]=0; nanflag=ivv;} 
  ...
  for (int ivv=0;ivv<nvr;ivv++){  //total regression                           
    ...
    reg0 = reg0+MyAbs(pss[ivv])*reg[ivv];
      if (! (MyAbs(reg0)<1.e9)){
...
float TrkCCNozzoli::GetRegression(int regtype, unsigned long long masktype, int fixbin){
  ...
  //if you call with fixbin<0 use a fixbin and not use the second normalization
  if (fixbin!=0) { // forced the training energy bin (for equalization/calibration purposes)
    int efbin = MyAbs(fixbin)-1; 
...
int TrkCCNozzoli::NormWeights(float ps[], int nvars, int imatr, float nval){
  ...
  for(int ivv=0;ivv<nvv;ivv++){
    ...
    ppar = ppar+MyAbs(ps[ivv]);
  }
  ptot = MyAbs(ppar/(1.-npes));
  for(int ivv=0;ivv<nvv;ivv++){
    ps[ivv]=ps[ivv]/ptot;
