...
bool EcalFastRec::IsSkipCalibration=0; // by default 
...
//==================================================================
//-->Read Edep Map from AMSEvent, Used if IsSkipCalibration 
//==================================================================
int EcalFastRec::ReadEdepMapFromAMSEvent(AMSEventR *pev){
	...
	static const double MeV2GeV = 1e-3;
	//-->ADC/Ped Array
	if(pev->nEcalHit()<=0) return -2;
	for(Int_t ihit=0; ihit<pev->nEcalHit(); ihit++){
		phit = pev->pEcalHit(ihit);
		Edep[phit->Plane][phit->Cell] = phit->Edep*MeV2GeV;
		EdepAttC[phit->Plane][phit->Cell] = phit->Edep*MeV2GeV;
		ADCH[phit->Plane][phit->Cell] = phit->ADC[0];
		ADCL[phit->Plane][phit->Cell] = phit->ADC[1];
		PedH[phit->Plane][phit->Cell] = phit->GetECALPed( phit->Plane, phit->Cell, 0 );
		PedL[phit->Plane][phit->Cell] = phit->GetECALPed( phit->Plane, phit->Cell, 1 );
