//  $Id: TofSimUtil.C,v 1.31 2024/02/01 16:48:23 qyan Exp $
...
//-----New3
G4double TofSimUtil::DHoneyUL3[9][3]={//Manually shink 0.2cm to allow carbon fibre
 {125.*cm,116.*cm,0.05*cm},//USkin1
 {125.*cm,116.*cm,4.90*cm},//UHoney1
 {125.*cm,116.*cm,0.05*cm},//USkin1+2
 {125.*cm,116.*cm,4.90*cm},//UHoney2
...
//----
 ...
 {129.*cm,109.*cm,4.70*cm},//LHoney
 {129.*cm,109.*cm,0.05*cm},//LSkin
 {129.*cm,109.*cm,0.23*cm},//LSkinExt
};

G4double TofSimUtil::PHoneyUL3[9][3]={// ZR=(76.695,66.745)  (-71.670,-66.820)
 {0.*cm, 0.*cm, 76.670*cm},//USkin1   Z=(76.695,76.645)
 {0.*cm, 0.*cm, 74.195*cm},//UHoney1  Z=(76.645,71.745)
 {0.*cm, 0.*cm, 71.720*cm},//USkin1+2 Z=(71.745,71.695)
 {0.*cm, 0.*cm, 69.245*cm},//UHoney2  Z=(71.695,66.795)
...
//----
 ...
 {0.*cm, 0.*cm,-69.220*cm},//LHoney   Z=(-66.870,-71.570)
 {0.*cm, 0.*cm,-71.595*cm},//LSkin    Z=(-71.570,-71.620)
 {0.*cm, 0.*cm,-71.735*cm},//LSkinExt Z=(-71.620,-71.850) //below -71.87 is RICH 
};
...
//Aluminum
  ...
  double alexdens = (TFMCFFKEY.lmatref==1)? 3.864: 0.84*TFMCFFKEY.lmatref;//=0.1932/24.01=0.00805*(lmatref-1)X0 (raw Skin+SkinExt=0.6432/24.01=0.0268X0)
  TOFAl_MExt = new G4Material("TOFAlExt",z=13.,a=26.98*g/mole,density=alexdens*g/cm3); 
...
//---Total:0.2066X0   Al: 1.705g/cm^2  C: 1.764g/cm^2  C8H10: 4.128g/cm^2
  ...
  G4double (*pDHoneyUL)[3] = (TFMCFFKEY.lmatref==1)? DHoneyUL2: DHoneyUL3;
  G4double (*pPHoneyUL)[3] = (TFMCFFKEY.lmatref==1)? PHoneyUL2: PHoneyUL3;
  for(int iul=0;iul<2;iul++){
...
//---Honeycomb
   ...
   for(int ip=0;ip<np;ip++){
     ...
     for(int i=0;i<3;i++){tofspar[i]=pDHoneyUL[iul*5+ip][i]/2.; tofgvolpar[i]=tofspar[i]/cm; tofppar[i]=pPHoneyUL[iul*5+ip][i];}
   ...
   int iulh=iul*5+np-1;
   if(VerboseLevel>1)G4cout<<"---------->>"<<Form("TFHoneyUL%d",iul)<<" Z="<<(pPHoneyUL[iul*5][2]+pDHoneyUL[iul*5][2]/2.)/cm<<" "<<(pPHoneyUL[iulh][2]-pDHoneyUL[iulh][2]/2.)/cm<<G4endl;

//---Carbon Fibre1
   for(int i=0;i<3;i++){if(i<2)tofspar[i]=pDHoneyUL[iul*5][i]/2.;else tofspar[i]=DCarbonUL/2.; tofgvolpar[i]=tofspar[i]/cm; tofppar[i]=PCarbonUL[iul][i];}
...
//---Carbon Fibre2+Foam
   for(int i=0;i<3;i++){if(i<2)tofspar[i]=pDHoneyUL[iul*5][i]/2.;else tofspar[i]=DCarbon2UL2/2.; tofgvolpar[i]=tofspar[i]/cm; tofppar[i]=PCarbon2UL2[iul][i];} 
