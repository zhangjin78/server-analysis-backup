//  $Id: mceventg.C,v 1.229 2025/01/31 11:34:22 nbelyaev Exp $
...
AMSmceventg::AMSmceventg(integer ipart, integer trackid, integer parentid, geant mom, const AMSPoint & coo, const AMSDir & dir,const AMSDir & poldir,integer parinfo,float charge,float mass,integer nskip):_coo(coo),_dir(dir),_poldir(poldir),_mom(mom),_mass(mass),_charge(charge),_delay(0),_ipart(ipart),_trkid(trackid), _parentid(parentid), _nskip(nskip),_tbline(0),_parinfo(parinfo){
if(abs(_parinfo)==1)init(ipart);
...
int  AMSmceventg::readposfromfile(char* filename,vector<tbpos> &poslist,int opt){
  FILE* ff=fopen(filename,"r");
  if(!ff) return -1;
  while(1){
    float a,b,c,d,e,f;
    int uid=poslist.size();
    tbpos aa;
    if(opt>=1)fscanf(ff,"%d %f %f %f %f %f %f",&uid,&a,&b,&c,&d, &e, &f);
    else      fscanf(ff,"%f %f %f %f %f %f",&a,&b,&c,&d, &e, &f);
    if(feof(ff)) break;
//    cout<<"uid"<<uid<<","<<a<<","<<b<<","<<c<<","<<d<<","<<e<<","<<f<<endl;
    aa.id=uid;
...
if(CCFFKEY.low==8){ // simple test beam mode
  ...
  else if(CCFFKEY.low==9||CCFFKEY.low==19){ // test beam mode/TOF photon scan simulation mode from file
...
//        sprintf(filename,"/afs/cern.ch/user/q/qyan/work/tofmc/tofgeomscan1.txt");
        sprintf(filename,"%s/tofgeomscan1.txt",AMSDATADIR.amsdatadir);
        maxlist=readposfromfile(filename,poslist,1);
      }
      if(maxlist<0) {
	printf("AMSmceventg::gener-F-: Cannot open file with TB postions filename:%s low=%d\n",filename,CCFFKEY.low);
	printf("Sorry I give up \n");
	abort();
      }
      readfile=1;
    }
    index=RNDM(d)*poslist.size();//int random between 0 and poslist.size()-1;
    if(index<0||index>=poslist.size()) {cerr<<"AMSmceventg::gener-F-index="<<index<<endl; index=0;}
    _coo[0]=_coo[1]=_coo[2]=0;
    if(CCFFKEY.low==9){
...
//       _dir[ii]=d0[0]*rotmat[ii][0]+d0[1]*rotmat[ii][1]+d0[2]*rotmat[ii][2];
    ...
    if(CCFFKEY.low==9)_mom   =extract(CCFFKEY.momr[0],CCFFKEY.momr[1]);
    else {
      number lrange=log(fabs(_momrange[1]))-log(fabs(_momrange[0]));//uniform log
      _mom=_momrange[0]*exp(lrange*RNDM(d));
    }     

//    _tbline=poslist[index].id;
   _tbline=index;
//     cout<<"index="<<index<<" coo="<<_coo[0]<<","<<_coo[1]<<","<<_coo[2]<<" dir="<<_dir[0]<<","<<_dir[1]<<","<<_dir[2]<<" mon="<<_mom<<" part="<<GCFLAG.NEVENT<<","<<GCFLAG.IEVENT<<endl;
//     printf("Want to fire mom %f from %f %f %f dir %f %f %f\n",_mom,
// 	   _coo[0],_coo[1],_coo[2],
// 	   _dir[0],_dir[1],_dir[2]);
    }
    ...
    bool isL0=0; // Old setup
    if (!strcmp(AMSJob::gethead()->getsetup(),"AMS02P.2")) isL0=1; // New setup
    orb->GenDir(isL0);
    for(int ii=0;ii<3;ii++){
      _dir[ii]=orb->Dir[ii];
      _coo[ii]=orb->Coo[ii];
...
//         cout <<" **********mom    .... "<<_mom<<endl; 
  ...
  _poldir.setp(0.,0.,0.);
  geant d(-1);
  if(G4FFKEY.polarrate>0&&RNDM(d)<=G4FFKEY.polarrate) {  //polarization perpendicular to momentum
    TVector3 w( _dir[0],_dir[1],_dir[2]);
    TVector3 u(-_dir[2],   0,   _dir[0]);//u defined in xz plane, orthogonal to w
    u.SetMag(1.);
//    TVector3 v=w.Corss(u);//u,v,w
    number angle=RNDM(d)*2.*M_PI;
    u.Rotate(angle,w);//roating u to be uniformly along perpendicular plane
    _poldir.setd(u[0],u[1],u[2]);
  }
  ...
  HMDIR("//PAWC/GEN","S");
    ...
    if(ipart == 3 && low==3 ){
     ...
     AMSEventR::hbook1(_hid+1,"NaturalFlux Generated GeV",nchan/20,al/1000,bl/1000);
      ...
      // find a modulation
      //
      geant modul[24]={470.,430.,410.,500.,630.,945.,850.,935.,955.,675.,640.,400.,360.,335.,270.,365.,480.,565.,605.,645.,645.,445.,380.,330.};
      integer year=(begindate%10000+enddate%10000)/2-1997+24;
      year=year%24; 
      if(year <0 || year > 24){
...
//           cout <<" passed 1 "<<(_coo-_dir*t).norm()<<endl;
  ...
  if((cut>=4 && cut<=7) || cut==9 ){
      ...
      if(!initdone){
       if(cut==4 || cut==9)cout<<" AMSmceventg::Specualcut-I-Layer1FocusSelected"<<endl;
    ...
    if(cut==4 || cut==9)return layer1;
      ...
      if(!initdone){
       cout<<" AMSmceventg::Specualcut-I-HostSpot5/6Selected "<<endl;
       initdone=true;
      }
      int l5=5;
      int l6=6;
      double mindist=1;
      float rig=_charge?_mom/_charge:100000;   
#ifdef _PGTRACK_
      float d5=AMSEventR::GetTkFeetDist(l5,_coo,_dir,rig);
      float d6=AMSEventR::GetTkFeetDist(l6,_coo,_dir,rig);
//      cout <<" d56 "<<d5<<" "<<d6<<endl;
      return (d5< mindist || d6<mindist);
#else
return false;
#endif
  }
...
integer AMSmceventg::accept(){
  _nskip=Orbit.Ntot;
 if(CCFFKEY.low==8|| CCFFKEY.low==9 || CCFFKEY.low==19 || CCFFKEY.low==10)  return 1;
...
// cout <<_coo<<" "<<_dir<<endl;
   ...
   gen->SetParticleGun(_ipart,_mom,_coo,_dir,_poldir);
...
//---
   ...
   AMSEvent::gethead()->addnext(
         AMSID("AMSmceventg",0),
         new AMSmceventg( g3code,  aTrack->GetTrackID(), aTrack->GetParentID(),  ekin/GeV, point/cm, dir,poldir,parinfo,charge,mass,nskip) // negetive code for secondary
         );
