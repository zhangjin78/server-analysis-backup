...
// Total number of pattern combinations
uinteger  PATT::_Npat;

// Number of fired layers in each pattern
integer * PATT::_patpoints;

// Set of fired layers in each pattern
integer * PATT::_patconf[trconst::maxlay];

// Set of non-fired layers in each pattern
integer * PATT::_patmiss[trconst::maxlay];

// Cummulative number of pattern combinations with nFired >= nLay+1-i
...
void PATT::init(int atreffekypar){
  ...
  for(cpat=0;cpat<npat();cpat++){
    // AMS02
    // Excluding L0 hits from the consideration for now <<##>>
    if((_patpoints[cpat]-nl0hits(cpat))>4)_patallow[cpat]=1;
    else _patallow[cpat]=0;
    if((_patpoints[cpat]-nl0hits(cpat))>4)_patallow2[cpat]=1;
  ...
  for(cpat=0;cpat<npat();cpat++){
    // AMS02
    // The L0 wasn't properly integrated below in this function
    // These arrays are not used any more anywhere (I hope) <<##>>
    // If needed, another set of arrays for tracker without L0 can be created
    if((_patpoints[cpat]-nl0hits(cpat))==6){
      ...
      if((_patpoints[cpat]-nl0hits(cpat))==5 && atreffekypar<1){ //for ANTI-calib to allow(if=1) any 5
        if(_patmiss[TkDBc::Head->nlay()-3][cpat]==1 || _patmiss[TkDBc::Head->nlay()-2][cpat]==1 || 
	        _patmiss[TkDBc::Head->nlay()-1][cpat]==1)
            _patallow[cpat]=0;
        if(_patmiss[TkDBc::Head->nlay()-3][cpat]==8 || _patmiss[TkDBc::Head->nlay()-2][cpat]==8 || 
	        _patmiss[TkDBc::Head->nlay()-1][cpat]==8)
            _patallow[cpat]=0;
        if(_patallow[cpat]){
...
void PATT::InitPattern(){
  integer ordermiss[maxlay]={0};
  integer vmiss[maxlay]={0};
  int minc;
  int iq=0;
...
#endif
  // Initialize patterns
  // Calculating number of possible patterns
  // Minimum 3 hits
  ...
  unsigned int cpat=0;
  // Processing all possible patterns
  for (cpat=0;cpat<npat();cpat++){\
      	...
      	switch(npt-1){
        	...
        	case 5:        
        	  for(i1=0;i1<TkDBc::Head->nlay();i1++){
                ...
                for(i3=i2+1;i3<TkDBc::Head->nlay();i3++){
        		      for(i4=i3+1;i4<TkDBc::Head->nlay();i4++){
        		        for(i5=i4+1;i5<TkDBc::Head->nlay();i5++){
        		          if(vmini==count){
            ...
            for(i1=0;i1<TkDBc::Head->nlay();i1++){
              for(i2=i1+1;i2<TkDBc::Head->nlay();i2++){
                for(i3=i2+1;i3<TkDBc::Head->nlay();i3++){
                  for(i4=i3+1;i4<TkDBc::Head->nlay();i4++){
                    for(i5=i4+1;i5<TkDBc::Head->nlay();i5++){
                      for(i6=i5+1;i6<TkDBc::Head->nlay();i6++){
                        if(vmini==count){
              ...
              for(i2=i1+1;i2<TkDBc::Head->nlay();i2++){
                for(i3=i2+1;i3<TkDBc::Head->nlay();i3++){
                  for(i4=i3+1;i4<TkDBc::Head->nlay();i4++){
                    for(i5=i4+1;i5<TkDBc::Head->nlay();i5++){
                      for(i6=i5+1;i6<TkDBc::Head->nlay();i6++){
                        for(i7=i6+1;i7<TkDBc::Head->nlay();i7++){
                          if(vmini==count){
                            vmiss[TkDBc::Head->nlay()-1]=ordermiss[i7];
                            vmiss[TkDBc::Head->nlay()-2]=ordermiss[i6];
                            vmiss[TkDBc::Head->nlay()-3]=ordermiss[i5];
                            vmiss[TkDBc::Head->nlay()-4]=ordermiss[i4];
                            vmiss[TkDBc::Head->nlay()-5]=ordermiss[i3];
                            vmiss[TkDBc::Head->nlay()-6]=ordermiss[i2];
                            vmiss[TkDBc::Head->nlay()-7]=ordermiss[i1];
                          }
                          count++;
                        }
                      }            
                ...
                for(i3=i2+1;i3<TkDBc::Head->nlay();i3++){
                  for(i4=i3+1;i4<TkDBc::Head->nlay();i4++){
                    for(i5=i4+1;i5<TkDBc::Head->nlay();i5++){
                      for(i6=i5+1;i6<TkDBc::Head->nlay();i6++){
                        for(i7=i6+1;i7<TkDBc::Head->nlay();i7++){
                          for(i8=i7+1;i8<TkDBc::Head->nlay();i8++){
                            if(vmini==count){
                              vmiss[TkDBc::Head->nlay()-1]=ordermiss[i8];
                              vmiss[TkDBc::Head->nlay()-2]=ordermiss[i7];
                              vmiss[TkDBc::Head->nlay()-3]=ordermiss[i6];
                              vmiss[TkDBc::Head->nlay()-4]=ordermiss[i5];
                              vmiss[TkDBc::Head->nlay()-5]=ordermiss[i4];
                              vmiss[TkDBc::Head->nlay()-6]=ordermiss[i3];
                              vmiss[TkDBc::Head->nlay()-7]=ordermiss[i2];
                              vmiss[TkDBc::Head->nlay()-8]=ordermiss[i1];
                            }
                            count++;
                          }
                        }
  ...
  for (int cpat=0;cpat<npat();cpat++){
    cout <<" cpat "<<cpat<<" "<<_patpoints[cpat]<<" "<<nl0hits(cpat)<<endl;
...
integer PATT::nl0hits(int cpat) {
  int count=0;
  for(int k=0;k<TkDBc::Head->nlay();k++)
    if (_patconf[k][cpat] && _patconf[k][cpat]>9) count++;
  return count;
}