//  $Id: ntuple.C,v 1.309 2025/05/12 18:41:54 choutko Exp $
//
...
AMSEventR *AMSNtuple::_pevroot02=0;
AMSEventR* AMSNtuple::Get_evroot02(){
  if(_pevroot02==0)_pevroot02=new AMSEventR();
  return _pevroot02;
}
...
AMSNtuple::~AMSNtuple(){
  ...
  //for(int k=0;k<sizeof(_evroot02)/sizeof(_evroot02[0]);k++)if(_evroot02[k])delete _evroot02[k];
    ...
    if(_rfile){
     endR();
     cout<<" AMSNtuple::~AMSNtuple()-I-FileClosed "<<endl;
    }
#endif
}
...
#pragma omp critical (wr2)
  ...
  if(_rfile){
    ...
    if(AMSJob::gethead() && !(AMSJob::gethead()->isMonitoring()))writeRSetup();
_rfile->cd();
...
if(AMSJob::gethead() && AMSJob::gethead()->isSimulation()){
    ...
    if ((AMSJob::gethead()->isMonitoring() || (IOPA.histoman/10)%10==1) && ptrman!=0) ptrman->GetHeader()->AddRunNumber(getrun());
...
*/
    ...
    _rfile->Write();
     ...
     if ((AMSJob::gethead()->isMonitoring() || (IOPA.histoman/10)%10==1) && ptrman!=0) {
       delete ptrman;
       ptrman = 0;
     }
#endif
    ...
    _rfile->Close();
    delete _rfile;
     cout<<" AMSNtuple::EndR-I-FileClosed "<<endl;
...
#ifdef VERSION614
else _rfile->SetCompressionAlgorithm(ROOT::kLZ4);
cout<<"Set Compress Alg LZ4 ..."<<endl;
#else
...
//        cerr<<"AMSNtuple::writeR-W-RunMode "<<mpi.RunMode<<" will return promptly "<<endl;
//        pthread_exit(NULL);
//    }
...
vector<AMSEventR*> del;
    Get_evroot02()->SetCont();
    Get_evroot02()->CreateRichRingX(RICRECFFKEY.createrichringx);
...
//     evn=new AMSEventR(_evroot02);
     evn=new AMSEventR(*Get_evroot02());
     cout <<" evn "<<evn->Run()<<" "<<evn->Event()<<" "<<evn->nParticle()<<" "<<evn->nRichRing()<<endl;
...
#ifdef _PGTRACK_
   if ((AMSJob::gethead()->isMonitoring() || (IOPA.histoman/10)%10==1) && ptrman!=0) ptrman->Fill(evn);
#endif
...
#ifndef JUQUEEN
         if(mpi.RunMode<=2)_tree->Fill();
#else
if(mpi.RunMode<=2)_tree->Fill();
#endif
         //const int count=MISCFFKEY.BTRaise*16;
          const int count=1000000;
        if((mpi.getMPIStatus()==1) && mpi.RunMode<=2 && AMSEventR::Head()->Event()%count==0)_rfile->Flush();
      }
    }
#pragma omp critical (wr1)
    {
...
//     evn=new AMSEventR(_evroot02);
     Get_evroot02()->CreateRichRingX(RICRECFFKEY.createrichringx);
     evn=new AMSEventR(*Get_evroot02());
     //evn->CreateRichRingX(RICRECFFKEY.createrichringx);
     //cout <<" evn data "<<evn->Run()<<" "<<evn->Event()<<" "<<evn->nParticle()<<" "<<evn->nRichRing()<<" "<<evn->nRichRingX()<<endl;
...
if(!landau)landau=new TF1("landau","landau(0)");
