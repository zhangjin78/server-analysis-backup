/// $Id: TkCoo.C,v 1.27 2025/04/11 16:09:43 nbelyaev Exp $ 
...
double TkCoo::GetLadderLength(int tkid) {
  ...
  if(abs(tkid)<100){
    //<<##>>
    if (abs(tkid)==1 ||abs(tkid)==2 ||
        abs(tkid)==17||abs(tkid)==18||
        abs(tkid)==51||abs(tkid)==52||
        abs(tkid)==67||abs(tkid)==68)
      return 74.07;  // Short ladder
    else if (abs(tkid)==3 ||abs(tkid)==4 ||
             abs(tkid)==15||abs(tkid)==16||
             abs(tkid)==53||abs(tkid)==54||
             abs(tkid)==65||abs(tkid)==66)
      return 90.09;  // Medium ladder
    else
      return 106.11; // Long ladder
  }
  else
