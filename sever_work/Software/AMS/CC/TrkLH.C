...
int TrkLH::TRLKINIT_v1(){
  ...
  const char* dbPath = getenv("TrkLHDataDir");
  if (dbPath==NULL) {
     dbPath=Form("%sTrkLH",AMSDATADIR.amsdatadir);//the directory has been changed to v6.00
  }
...
int TrkLH::filltrklhvarfromgbatch(TrkLHVar& var1, AMSEventR* evt, int nptc){
  ...
  var1.err_ch = 0.;
  int  iddc = trk->iTrTrackPar(11,0,1); // without MS   
