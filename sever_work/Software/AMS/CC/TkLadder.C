//  $Id: TkLadder.C,v 1.12 2025/04/11 16:09:47 nbelyaev Exp $
...
TkLadder::TkLadder(char* Name,int Trid,int HwId,int nsensors):TkObject(Name){
  ...
  if (abs(Trid)<100){
    _layer=11;
    _slot=Trid/abs(Trid)*(abs(Trid)-50);
    if (abs(Trid)<50){
      _layer=10;
      _slot=Trid;
    }
...
// Change this when the L0 integration scheme will be fixed <<##>>
  if (_layer==10||_layer==11) return 1;
