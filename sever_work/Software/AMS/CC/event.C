//  $Id: event.C,v 1.639 2024/12/18 19:43:25 choutko Exp $
...
uint64 AMSEvent::_RunEv[maxthread]={};
AMSEvent* AMSEvent::_Head[maxthread]={};
int AMSEvent::_Wait[maxthread]={};
uinteger AMSEvent::_Size[maxthread]={};
...
#if defined(G4MULTITHREADED) || defined(__PPC64)
...
void AMSEvent::_retriginitevent(){
   ...
   add (new AMSContainer(AMSID("AMSContainer:TriggerLVL1",0),0));
   add (new AMSContainer(AMSID("AMSContainer:TrigPhysLVL1",0),0));
   add (new AMSContainer(AMSID("AMSContainer:TrigPhysLVL1PS",0),0));
   add (new AMSContainer(AMSID("AMSContainer:TriggerLVL3",0),0));
}
...
void AMSEvent::_retrdinitevent(){
  ...
  if(1 || TRDFITFFKEY.FitMethod/10){
    for(unsigned int i=0;i<2*AMSTRDIdSoft::ncrates();i++) add (
  new AMSContainer(AMSID("AMSContainer:AMSTRDRawHitHC",i),0));
}
  if((TRDFITFFKEY.FitMethod%10)!=1){
...
void AMSEvent::_reaxinitevent(){
  ...
  add (new AMSContainer(AMSID("AMSContainer:AntiMatter",0),0));
   
  add (new AMSContainer(AMSID("AMSContainer:NotAProton",0),0));
...
void  AMSEvent::write(int trig){
  ...
  if((IOPA.hlun || IOPA.WriteRoot) && AMSJob::gethead()->getntuple()){
    ...
    for (int i=0;;){
      ...
      if(cur){
//        AMSContainer *ct=dynamic_cast<AMSContainer*>(cur);
//        cout<<"cont "<<cur->getname()<<" "<<cur->getid()<<" "<<ct<<endl; 
...
// renabled by PZ on 05-03-2014
// the magnet temperature is available via rootsetup or by TDV
// correction factor is stored also in the fitresult (TrTrackPar) 
  if(TRFITFFKEY.magtemp &&AMSCommonsI::getbuildno()>=800&& AMSJob::gethead()->isRealData()) MagField::GetPtr()->UpdateMagTemp(gettime());
#endif
...
if(DAQEvent::FileSize()){
  ...
  if(AMSJob::gethead()->isSimulation() && AMSJob::gethead()->gettriggerPhys()){
    if(!getC("TrigPhysLVL1",0)->getnelem()){
    // skip event
    for(int i=0;;i++){
      AMSContainer *pctr=getC("TriggerLVL1",i);
      if(pctr)pctr->eraseC();
...
//
      ...
      if((TFREFFKEY.TFHTDVCalib/10000)%100>0){
        TofRecH::BuildTofClusterH();//RawSide->ClsuterH
      }
...
#endif
if(TRDFITFFKEY.FitMethod/10){
AMSTRDCluster::setused();
 buildC("AMSTRDCluster",1);
 buildC("AMSTRDSegment",1);
buildC("AMSTRDTrack",1);
}
...
/*
  for(int i=0;;i++){
    AMSContainer *pctr=getC("AMSTRDRawHitHC",i);
      if(pctr)pctr->eraseC();
      else break ;
  }
*/
...
try{
  ...
  if((TFREFFKEY.TFHTDVCalib/10000)%100>0){
    AMSgObj::BookTimer.start("ReTOFBetaH");
    TofRecH::BuildBetaH();//BetaH
    AMSgObj::BookTimer.stop ("ReTOFBetaH");
//------
    AMSgObj::BookTimer.start("ReTKRefitB");
#ifdef _PGTRACK_
    if((TRCLFFKEY.ClusterCofGOpt%10)>=2){
      TofRecH::DoAdvancedTrackFit();//Update Track Fitting
    }
    TofRecH::DoPrimaryTrackTag();//Tag Primary Track
#endif
    AMSgObj::BookTimer.stop ("ReTKRefitB");
  }
...
#ifdef _PGTRACK_
  if (TRCLFFKEY.recflag%100000 >= 20000) {
    try{
...
//        }
        ...
        if(ptr){
         if(ptr->getpbetah()){
           int z1=ptr->getpbetah()->GetBeta()>0?1:0;
           __status1|=(z1<<1);
         }
         else if(ptr->getpbeta()){
...
if(strstr(AMSJob::gethead()->getsetup(),"AMS02P.2")){
  int iz1=0;
  if( ptr &&ptr->_ptrack ){
     ...
     if(cosgm<0.1736)icos=0;
      ...
      __status1|=(icos<<13);
       ...
       if(ptr){
         int z1=0;
         if(ptr->getpbetah()){
           z1=ptr->getpbetah()->NTofClusterH()-1;
           if(z1<=1)z1=1;
         }
         else if(ptr->getpbeta()){
