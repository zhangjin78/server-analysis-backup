//  $Id: ecaldbc.C,v 1.114 2024/05/28 13:17:34 nbelyaev Exp $
...
void ECALDBc::readgconf(){
  ...
  //
    ...
    else if(strstr(AMSJob::gethead()->getsetup(),"AMS02P.2")){
      cout <<"<------ ECALGeom-I- AMS02P.2(perm.magnet) configuration is used..."<<endl;
...
void ECALDBc::fid2cida(integer fid, integer cid[4], number w[4]){
  ...
  ss=fidd/100;//super-layer number (1-9)
  fshift=_gendim[9+ss];//fibers shift from ideal(simmetric) position
...
//wxq_begin
//add fiber rotation in fid2cida, and change it to static table format
void ECALDBc::ct2cida(number ct, integer ss, number cz, integer cell_id[2], number fiber_weight[2])
{
    integer npm,pm,cell,bran,tbc;
    number tleft,bdis,ww;
    geant pmdis,dist,pmsiz,pxsiz,fr;
    geant effmn,deffmx,slope;
    npm=_slstruc[3];// numb.of PM's per super-layer
    pmsiz=_rdcell[4]*2.;//PM_size
    pxsiz=_rdcell[4];// SubCell(pixel) size
    fr=_rdcell[3]/2;//   fiber radious
    ...
    if(cz>fr)tbc=0;// below i imply no sharing in Z (even number of f-layers/s-layer)
      ...
      cerr<<"ECALDBc::ct2cida: bad fiber-PM matching in Z, cz="<<cz<<endl;
      exit(1);
    }
    //
    pmdis=dist-pm*pmsiz;//   fiber-center dist from the left edge of current(pm) PM
    ...
    case 1:  //<-- near the left PM-boundary
      ...
      ww=segarea(fr,bdis);
      if(bdis<0){//special case: fiber is outside 1st PM, butslightly(<=fr) overlape with its left side
        fiber_weight[0]=ww;//smaller overlape area 
        cell_id[0]=1000*(ss+1)+(pm+1)*10+(cell+1);
        fiber_weight[0]*=effmn;//fixed at pixel edge level (0.6)
        return;
      }
      //bdis>0(normal case):
      fiber_weight[0]=1.-ww;
      cell_id[0]=1000*(ss+1)+(pm+1)*10+(cell+1);
      if(bdis<deffmx)fiber_weight[0]*=(effmn+slope*bdis);//corr.for pixel eff.uniformity
      if(pm==0)return;// 1st PM
      fiber_weight[1]=1.-fiber_weight[0];
      cell=1+tbc;
      cell_id[1]=1000*(ss+1)+(pm)*10+(cell+1);// left neighbour
      fiber_weight[1]*=effmn;//fixed 
      break;
      //        
    case 2:  //<-- near the right PM-boundary
      cell=1+tbc;
      ...
      fiber_weight[1]=1.-fiber_weight[0];
      cell=0+tbc;
      cell_id[1]=1000*(ss+1)+(pm+2)*10+(cell+1);// right neighbour
      fiber_weight[1]*=effmn;//fixed 
      break;
      //        
    case 3:  //<-- completely inside PM
      ...
      if(bdis<=-fr){// <-- completely in the left half of PM
        ...
        cell_id[0]=1000*(ss+1)+(pm+1)*10+(cell+1);
        fiber_weight[0]=1.;
        if(-bdis<deffmx)fiber_weight[0]*=(effmn-slope*bdis);//corr.for pixel eff.uniformity (at right side)
        else if((pxsiz+bdis)<deffmx)fiber_weight[0]*=(effmn+slope*(pxsiz+bdis));//..........(at left side)
      }
      else if(bdis>=fr){//>0, <-- completely in the right half of PM
        cell=1+tbc;
        cell_id[0]=1000*(ss+1)+(pm+1)*10+(cell+1);
        fiber_weight[0]=1.;
        if(bdis<deffmx)fiber_weight[0]*=(effmn+slope*bdis);//corr.for pixel eff.uniformity(at left side)	  
        else if((pxsiz-bdis)<deffmx)fiber_weight[0]*=(effmn+slope*(pxsiz-bdis));//........(at rigth side)
      }
      else{ // <-- left-right sharing (|bdis|<fr)
        ww=segarea(fr,bdis);
        cell=0+tbc;
        cell_id[0]=1000*(ss+1)+(pm+1)*10+(cell+1);
        cell=1+tbc;
        cell_id[1]=1000*(ss+1)+(pm+1)*10+(cell+1);
        if(bdis<0.){//bigger area is in left pix
          fiber_weight[0]=1.-ww;	    
          if(-bdis<deffmx)fiber_weight[0]*=(effmn-slope*bdis);//corr.for pixel eff.uniformity
          fiber_weight[1]=ww;
  	  ...
  	  fiber_weight[0]*=effmn;//fixed	    
  	  fiber_weight[1]=1.-ww;	    
  	  if(bdis<deffmx)fiber_weight[1]*=(effmn+slope*bdis);//corr.for pixel eff.uniformity
        }
      }
      break;
      //
    default:
      cerr<<"ECALDBc::ct2cida: problem with PM/Fiber transv.matching, pmdis="<<pmdis<<endl;
      exit(1);
    }
}

integer ECALDBc::is_fiberinfo_init = 0; 
geant ECALDBc::fibercoot[9][10][500] = {0}; 
geant ECALDBc::fibercooz[9][10][500] = {0}; 
integer ECALDBc::CellID[9][10][500][2] = {0}; 
number ECALDBc::FiberWeight[9][10][500][2] = {0}; 

void ECALDBc::init_fiberinfo_table()
{
    if(is_fiberinfo_init==0)
    {
        is_fiberinfo_init = 1;

        integer ip,nfl;
        number cleft,ztop,ct,cz;
        geant pit,piz;
        geant fshift;
        nfl=_slstruc[1];// numb.of fiber-layers per super-layer
        pit=_fpitch[0];// fiber pitch(transv)
        piz=_fpitch[1];// fiber pitch(in z)
        number fiber_disp,ct_minus,ct_plus;

        int ipm;
        int n_fid2cid[9][10][500] = {0};
        for(int isl=0;isl<9;isl++)
        {
            fshift=_gendim[10+isl];//fibers shift from ideal(simmetric) position
            fiber_disp = ECMCFFKEY.FiberDisplacement[isl];//add fiber rotation. FiberDisplacement is defined as ("+" end coo) - ("-" end coo).
            for(int ifl=0;ifl<10;ifl++)
            {
                ...
                {
                    integer cid_minus[2]={0};
                    integer cid_plus[2]={0};
                    number w_minus[2]={0.};
                    number w_plus[2]={0.};
                    //("out-of-lead_volume" case is impossible due to lead size and limited max.number of fibers)
                    ct=cleft+ifn*pit;//       transv.coord. of fiber in ECAL r(eference) s(ystem)
                    fibercoot[isl][ifl][ifn] = ct;
                    ztop=(nfl-1)*piz/2.;//       z-pos of 1st(from top) f-layer of s-layer
                    cz=ztop-ifl*piz;//      z-pos of fiber in Slayer r.s.(z=0->middle of super-layer)
                    fibercooz[isl][ifl][ifn] = _gendim[6]-(_gendim[8]+2.*_gendim[9])*(isl+0.5)+cz;//-142.73-(1.83+2*0.01)*(isl+0.5)+cz
                    ct_minus = ct - fiber_disp/2.;//assume readout at "-" end
                    ct_plus = ct + fiber_disp/2.;//assume readout at "+" end
                    ct2cida(ct_minus,isl,cz,cid_minus,w_minus);
                    ct2cida(ct_plus,isl,cz,cid_plus,w_plus);

                    for(int k=0;k<2;k++)
                    {
                        if(w_minus[k]>0)
                        {
                            ipm = (cid_minus[k]%1000)/10-1;
                            if( (isl%2) == (ipm%2) )//ipm is really at "-" end
                            {
                                if(n_fid2cid[isl][ifl][ifn]>=2)
                                {
                                    cerr<<"ECALDBc::init_fiberinfo_table Error: 1 fiber shared by more than 2 PMT"<<endl;
                                    exit(1);
                                }
                                CellID[isl][ifl][ifn][n_fid2cid[isl][ifl][ifn]] = cid_minus[k];
                                FiberWeight[isl][ifl][ifn][n_fid2cid[isl][ifl][ifn]] = w_minus[k];
                                n_fid2cid[isl][ifl][ifn]++;
                            }
                        }

                        if(w_plus[k]>0)
                        {
                            ipm = (cid_plus[k]%1000)/10-1;
                            if( (isl%2) != (ipm%2) )//ipm is really at "+" end
                            {
                                if(n_fid2cid[isl][ifl][ifn]>=2)
                                {
                                ...
                                CellID[isl][ifl][ifn][n_fid2cid[isl][ifl][ifn]] = cid_plus[k];
                                FiberWeight[isl][ifl][ifn][n_fid2cid[isl][ifl][ifn]] = w_plus[k];
                                n_fid2cid[isl][ifl][ifn]++;
                            }
                        }
                    }
                    ...
                    if(n_fid2cid[isl][ifl][ifn]==2)
                    {
                        if(FiberWeight[isl][ifl][ifn][0]<FiberWeight[isl][ifl][ifn][1])
                        {
                            integer CellID_tmp = CellID[isl][ifl][ifn][0];
                            number FiberWeight_tmp = FiberWeight[isl][ifl][ifn][0];
                            CellID[isl][ifl][ifn][0] = CellID[isl][ifl][ifn][1];
                            FiberWeight[isl][ifl][ifn][0] = FiberWeight[isl][ifl][ifn][1];
                            CellID[isl][ifl][ifn][1] = CellID_tmp;
                            FiberWeight[isl][ifl][ifn][1] = FiberWeight_tmp;
                        }
                    }
                }
            }
...
void ECALDBc::fid2cida_fibrot(integer fid, integer cid[4], number w[4])
{
    integer fidd,sl,fl,fn;
    fidd=fid/1000;
    fn=fid%1000-1;//fiber number (0-...)
    fl=fidd%100-1;//fiber layer number (0-9) 
    sl=fidd/100-1;//super-layer number (0-8)
    for(int k=0;k<2;k++)
    {
        if(FiberWeight[sl][fl][fn][k]>0)
        {
            cid[k] = CellID[sl][fl][fn][k];
            w[k] = FiberWeight[sl][fl][fn][k];
        }
        else
        {
           cid[k] = 0;
