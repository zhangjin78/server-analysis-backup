//  $Id: gvolume.C,v 1.84 2025/02/07 17:08:27 nbelyaev Exp $
#include "gvolume.h"
#include <math.h>
#include "amsgobj.h"
#include "gmat.h"
...
AMSgvolume::AMSgvolume (const char matter[],integer rotmno,const char name[], 
   ...
   if(gonly)strcpy(_gonly,gonly);
    ...
    if(!_rotmno && !_rel){
        cerr <<"AMSgvolume::ctor-W-RotMatrixZeroInAbsModeIsNotAllowed "<<getname() << " " <<getid()<<endl;
       _rotmno=1;
     }
...
//       for(int i=0;i<11;i++)cout <<"par "<<i<<" "<<_par[i]<<endl;
     ...
     else if (shape=="CUPOLA"){
      ams_geometryconverter converter;
      G4TessellatedSolid *cupolaTessellatedSolid = converter.createTessellatedSolid("/cvmfs/ams.cern.ch/Offline/AMSDataDir/Geometry/GDML/cupola/cupola.gdml");
      psolid=cupolaTessellatedSolid;
      //maxstep=_par[0];
     }
     else {
...
#ifdef __G4AMS__
      // Tracker active silicon region
               if ( (TRMCFFKEY.SetActiveSiliconCuts)&&(strcmp(_pgtmed->getname(),"ACTIVE_SILICON")==0) ) {
        TrSimUtil::AddRootLogicalVolume(_pg4l);
        _pg4l->SetUserLimits(new AMSUserLimits(10.*getpgtmed()->getstemax())); // cm to mm
      }
      else {
#endif
...
//       }
//      else _pg4l->SetUserLimits(new AMSUserLimits());
...
char n1[5]="";
char n2[5]="";
for(int k=0;k<(strlen(getname())>4?4:strlen(getname()));k++)n1[k]=getname()[k];
for(int k=0;k<(strlen(o->getname())>4?4:strlen(o->getname()));k++)n2[k]=o->getname()[k];
if(strcmp(n1,n2))return 0;
if(strcmp(_shape,o->_shape))return 0;
if(_pgtmed != o->_pgtmed)return 0;
