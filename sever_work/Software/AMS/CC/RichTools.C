...
void serialize_vector(ofstream &out, vector<float>& vec){
  size_t size = vec.size();
  out.write(reinterpret_cast<const char*>(&size), sizeof(size));
  out.write(reinterpret_cast<const char*>(vec.data()), size * sizeof(float));
}

void deserialize_vector(ifstream &in,vector<float>& vec) {
  size_t size;
...
void serialize_vector_d(ofstream &out, vector<double>& vec){
  size_t size = vec.size();
  out.write(reinterpret_cast<const char*>(&size), sizeof(size));
  out.write(reinterpret_cast<const char*>(vec.data()), size * sizeof(double));
}

void deserialize_vector_d(ifstream &in,vector<double>& vec) {
  size_t size;
  in.read(reinterpret_cast<char*>(&size), sizeof(size));
  vec.resize(size);
...
// Serialize TH2D to binary file
void serializeTH2D(ofstream &out, TH2D& hist){
  // Extract binning information
  int nBinsX = hist.GetNbinsX();
  int nBinsY = hist.GetNbinsY();
  
  // Write binning dimensions
  out.write(reinterpret_cast<const char*>(&nBinsX), sizeof(nBinsX));
  out.write(reinterpret_cast<const char*>(&nBinsY), sizeof(nBinsY));
  
  // Write bin edges
  std::vector<double> xEdges(nBinsX + 1);
  std::vector<double> yEdges(nBinsY + 1);

  for (int i = 0; i <= nBinsX; ++i) xEdges[i] = hist.GetXaxis()->GetBinLowEdge(i + 1);
  for (int i = 0; i <= nBinsY; ++i) yEdges[i] = hist.GetYaxis()->GetBinLowEdge(i + 1);

  out.write(reinterpret_cast<const char*>(xEdges.data()), xEdges.size() * sizeof(double));
  out.write(reinterpret_cast<const char*>(yEdges.data()), yEdges.size() * sizeof(double));

    // Write bin contents
  std::vector<double> contents((nBinsX + 2) * (nBinsY + 2)); // Including underflow/overflow bins
  for (int i = 0; i <= nBinsX + 1; ++i) {
    for (int j = 0; j <= nBinsY + 1; ++j) {
      contents[i * (nBinsY + 2) + j] = hist.GetBinContent(i, j);
    }
  }

  out.write(reinterpret_cast<const char*>(contents.data()), contents.size() * sizeof(double));
}


// Deserialize binary file to TH2D
TH2D deserializeTH2D(ifstream &in) {
  // Read binning dimensions
  int nBinsX, nBinsY;
  in.read(reinterpret_cast<char*>(&nBinsX), sizeof(nBinsX));
  in.read(reinterpret_cast<char*>(&nBinsY), sizeof(nBinsY));
  ...
  // Construct histogram
  TH2D hist("hist", "Deserialized Histogram", nBinsX, xEdges.data(), nBinsY, yEdges.data());
  
  // Read bin contents
  std::vector<double> contents((nBinsX + 2) * (nBinsY + 2));
  in.read(reinterpret_cast<char*>(contents.data()), contents.size() * sizeof(double));
  
  for (int i = 0; i <= nBinsX + 1; ++i) {
    for (int j = 0; j <= nBinsY + 1; ++j) {
      hist.SetBinContent(i, j, contents[i * (nBinsY + 2) + j]);
    }
  }
  return hist;
}
...
void serialize(GeomHash &hash,ofstream& out){
  out.write(reinterpret_cast<const char*>(&hash.dimension), sizeof(hash.dimension));
  out.write(reinterpret_cast<const char*>(&hash.numNodes), sizeof(hash.numNodes));
  out.write(reinterpret_cast<const char*>(&hash.minSize), sizeof(hash.minSize));

  size_t size = hash.points.size();
  out.write(reinterpret_cast<const char*>(&size), sizeof(size));
  out.write(reinterpret_cast<const char*>(hash.points.data()), size * sizeof(unsigned char));

  size = hash.limit.size();
  out.write(reinterpret_cast<const char*>(&size), sizeof(size));
  out.write(reinterpret_cast<const char*>(hash.limit.data()), size * sizeof(float));

  for (int i = 0; i < 2; i++) {
    size = hash.nodes[i].size();
    out.write(reinterpret_cast<const char*>(&size), sizeof(size));
    out.write(reinterpret_cast<const char*>(hash.nodes[i].data()), size * sizeof(int));
  }
  serialize_vector(out,hash.Means);
  serialize_vector(out,hash.Rms);
  serialize_vector(out,hash.Template);
  serialize_vector(out,hash.TemplateRms);
  serialize_vector(out,hash.Peak);
  serialize_vector(out,hash.PeakWidth);

  size = hash.Entries.size();
  out.write(reinterpret_cast<const char*>(&size), sizeof(size));
  out.write(reinterpret_cast<const char*>(hash.Entries.data()), size * sizeof(int));
}


void deserialize(GeomHash &hash,ifstream& in) {
  in.read(reinterpret_cast<char*>(&hash.dimension), sizeof(hash.dimension));
  in.read(reinterpret_cast<char*>(&hash.numNodes), sizeof(hash.numNodes));
  in.read(reinterpret_cast<char*>(&hash.minSize), sizeof(hash.minSize));

  size_t size;
  in.read(reinterpret_cast<char*>(&size), sizeof(size));
  hash.points.resize(size);
  in.read(reinterpret_cast<char*>(hash.points.data()), size * sizeof(unsigned char));

  in.read(reinterpret_cast<char*>(&size), sizeof(size));
  hash.limit.resize(size);
  in.read(reinterpret_cast<char*>(hash.limit.data()), size * sizeof(float));

  for (int i = 0; i < 2; i++) {
    in.read(reinterpret_cast<char*>(&size), sizeof(size));
    hash.nodes[i].resize(size);
    in.read(reinterpret_cast<char*>(hash.nodes[i].data()), size * sizeof(int));
  }
  
  deserialize_vector(in,hash.Means);
  deserialize_vector(in,hash.Rms);
  deserialize_vector(in,hash.Template);
  deserialize_vector(in,hash.TemplateRms);
  deserialize_vector(in,hash.Peak);
  deserialize_vector(in,hash.PeakWidth);

  in.read(reinterpret_cast<char*>(&size), sizeof(size));
  hash.Entries.resize(size);
  in.read(reinterpret_cast<char*>(hash.Entries.data()), size * sizeof(int));
}

void serialize(GeomHashEnsemble &ensemble,ofstream& out) {
  out.write(reinterpret_cast<const char*>(&ensemble.dimension), sizeof(ensemble.dimension));
  size_t size = ensemble.hashes.size();
  out.write(reinterpret_cast<const char*>(&size), sizeof(size));
  for (auto& hash : ensemble.hashes) {
    serialize(hash,out);
  }
}

void deserialize(GeomHashEnsemble &ensemble,ifstream& in) {
  in.read(reinterpret_cast<char*>(&ensemble.dimension), sizeof(ensemble.dimension));
  size_t size;
  in.read(reinterpret_cast<char*>(&size), sizeof(size));
  ensemble.hashes.resize(size);
  for (auto& hash : ensemble.hashes) {
    deserialize(hash,in);
  }
}
...
GeomHashEnsemble *GeomHashEnsemble::readBin(TString input){
  GeomHashEnsemble *ensemble=new GeomHashEnsemble;
  try{
    ifstream inFile(input.Data(), ios::binary);
    deserialize(*ensemble,inFile);
  }catch(...){
    cout<<"*** Error at GeomHashEnsemble::readBin"<<endl;
    delete ensemble;
    ensemble=0;
  }
  return ensemble;
...
///////////////////////////////////////////
// Merge two ensembles in the first one. //
// This allows creating large ensembles. //
///////////////////////////////////////////


void merge(GeomHashEnsemble &receiver,GeomHashEnsemble &small){
...
void serialize(GAMModel &hash,ofstream& out){
  serialize_vector_d(out,hash.buffer);
  size_t size=hash.centroids.size();
  out.write(reinterpret_cast<const char*>(&size), sizeof(size));
  for (size_t i = 0; i < size; i++) serialize_vector(out,hash.centroids[i]);
  serialize_vector_d(out,hash.scaling);
  serialize_vector_d(out,hash.means);
  serializeTH2D(out,hash.index_correction);
}

void deserialize(GAMModel &hash,ifstream& in){
  deserialize_vector_d(in,hash.buffer);
  size_t size;
  in.read(reinterpret_cast<char*>(&size), sizeof(size));
  for (size_t i = 0; i < size; i++){
    std::vector<float> centroids;
    deserialize_vector(in,centroids);
    hash.centroids.push_back(centroids);
  }
  deserialize_vector_d(in,hash.scaling);
  deserialize_vector_d(in,hash.means);
  hash.index_correction=deserializeTH2D(in);
}
...
void GAMModel::retrieve_weights(float *args){
  ...
  for(int i=0;i<dimensions;i++) buffer.push_back(args[i]); // Linear part
...
double GAMModel::eval(float *args){
  ...
  double v=0;
  for(unsigned int i=0;i<means.size();i++) v+=buffer[i]*means[i];
  return v;
}
...
double GAMModel::get_index_correction(float x,float y){
  if(index_correction.GetNbinsX()==1 || index_correction.GetNbinsY()==1) return 1;
...
GAMModel *GAMModel::readBin(TString input){
  GAMModel *ensemble=new GAMModel;
  try{
    ifstream inFile(input.Data(), ios::binary);
    deserialize(*ensemble,inFile);
  }catch(...){
    cout<<"*** Error at GAMModel::readBin"<<endl;
    delete ensemble;
    ensemble=0;
  }
  return ensemble;
