//  $Id: amschain.C,v 1.117 2024/05/28 13:17:33 nbelyaev Exp $
#include "amschain.h"
...
void AMSChain::Reset(Option_t* option) {
  TChain::Reset(option);
  m_chain_Entries = 0;
  m_tree_entry = -1;
  m_chain_Runs.clear();
...
long long AMSChain::GetEntryNoSeq(UInt_t run, Int_t ev, bool runinfilename, unsigned long long maxent){
int prcs=AMSEventR::ProcessSetup;
unsigned long long runev=((unsigned long long)run)<<32;
runev+=ev;
map<unsigned long long, unsigned long long>::iterator it=m_chain_entryindex.find(runev);
if(it!=m_chain_entryindex.end())return std::distance(m_chain_entryindex.begin(),it);
...
it=m_chain_entryindex.find(runev);
if(it==m_chain_entryindex.end())return -1;
else return std::distance(m_chain_entryindex.begin(),it);
}
...
long long   AMSChain::GetEntryFromList(int delta, const char* fnam, bool runinfilename, unsigned long long maxent){

  static   std::vector< std::pair<int,int> > ll;
  static int first=1;
  static int snum=0;
  if(first){
    first=0;
    ...
    while (1){
      ...
      fscanf(ff,"%d %d",&rr,&ee);
      if(feof(ff)) break;
      ll.push_back(std::make_pair(rr,ee));
    }
    fclose(ff);
  }
  ...
  if(snum<0) snum=0;
  if(snum>=int(ll.size())) snum=ll.size()-1;

  long long evv=GetEntryNo(ll[snum].first,ll[snum].second ,runinfilename,maxent);
  ...
  if (listfile) {
    ...
    while (!feof(listfile)){
      fscanf(listfile,"%s\n", rname );
       ...
       string stager_get;
       if(getenv("OsCmdTimeout")) stager_get=getenv("OsCmdTimeout");
         else stager_get="timeout";
       stager_get+=" --signal 9 9 ";
...
int AMSChain::AddFromFile(const char* fnameg,int first,int last, int stagedonly,unsigned int timeout,const char *pattern){
  TString a(fnameg);
  const char *fname=fnameg;
  bool tmpf=false;
  TRegexp a1("^data:",false);
  TRegexp a2("^mc:",false);
...
again:
    string gnp=localbin;
    gnp+="/DataManagement/exe/linux/getntuplepath ";
    gnp+=(const char*)a;
    char tmp[80];
    sprintf(tmp,"/tmp/gnp.%d",getpid());
    gnp+=" >& ";
    gnp+= tmp;
    int i=system(gnp.c_str());
    if(i){
      unlink(tmp);
      cerr<<" main-E-Unable to "<<gnp<<endl;
        if(!again){
        localbin=local;
        again=true;
        goto again;
      ...
      ftxt.open(tmp);
      getline(ftxt,line);
      if(ftxt.good() && !ftxt.eof()){
       getline(ftxt,line);
       replace(line.begin(),line.end(),',','\n');
        ofstream fo;
        char tmpo[255];
        sprintf(tmpo,"/tmp/gnpo.%d",getpid());
        fo.open(tmpo);
        if(fo.good()){
          fo<<line<<endl;
          tmpf=true;
          ...
          fname=tmpo;
           ...
           cerr<<"AddFromFile-E-UnableToWriteFile "<<tmp<<endl; 
           unlink(tmpo);              
        }
       }
      else{
       cerr<<"AddFromFile-E-UnableToReadFile "<<tmp<<endl; 
      }
      ftxt.close();
      unlink(tmp);
    }
...
//             rn.replace(rn.begin()+pos,rn.begin()+pos+castor.length(),eos);
       ...
       string eoscheck;
       if(getenv("OsCmdTimeout")) eoscheck=getenv("OsCmdTimeout");
         else eoscheck="timeout";
       eoscheck+=" --signal 9 30 ";
...
//             eoscheck+=rn.c_str()+pos+eos.length();
    ...
    cerr << "AMSChain::AddFromFile-E-ErrorOpeningFile "<< fname <<endl;
    if(tmpf)unlink(fname);
    return -1;
  }
...
int  AMSChain::LoadUF(const char* fname){
...
#if defined (__X8664__) ||defined (__DARWIN__)
  char m32[80]="-fPIC";
  char elf[11]="";
#else
  char m32[80]="-m32 -fPIC";
  ...
  if(!i){
#ifdef __DARWIN__
    sprintf(cmd1,"ld  -dylib -ldylib1.o -undefined dynamic_lookup %s.o -o libuserAMSEVD.so",nameonly.c_str());
	(void)elf;
#else
