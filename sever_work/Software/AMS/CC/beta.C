//  $Id: beta.C,v 1.106 2022/06/07 14:36:50 qyan Exp $
...
integer AMSBeta::build(integer refit){
  ...
  if(old_build){
    ...
    if(nTrtracks>0){ // Build Beta from a real TrTrack!!
      AMSTrTrack *ptrack=(AMSTrTrack*)AMSEvent::gethead()->getheadC("AMSTrTrack",0,2);//force sort again
      for ( ; ptrack ; ptrack=ptrack->next()) {   
	bfound+=BuildBeta(ptrack);
      }
    ...
    if(!bfound){
      ...
      if(ptrack) bfound+=BuildBeta(ptrack);
...
integer AMSBeta::build_old(integer refit){
  // Pattern recognition + fit
  AMSPoint SearchReg(BETAFITFFKEY.SearchReg[0],BETAFITFFKEY.SearchReg[1],
		     BETAFITFFKEY.SearchReg[2]);

  // Loop on track patterns and sort tracks
  std::vector<AMSTrTrack*> tr;
  for (AMSTrTrack *ptrack= (AMSTrTrack*)AMSEvent::gethead()->getheadC("AMSTrTrack",0,2); ptrack ; ptrack=ptrack->next()) {//force sort again
     tr.push_back(ptrack);
  }
#ifdef _PGTRACK_
  if(tr.size()>=1&&tr[0]->GetRecType()<10)std::sort(tr.begin(),tr.end(),TRCmp);//already sorted for new reconstruction
#else
...
double AMSBeta::GetTRDBetaCorr(float beta){
//              double p[3]={0.522677,-0.16927,0.676221};
              double betamc1=0;
              for(int k=0;k<int(sizeof(BETAFITFFKEY.TRDP)/sizeof(BETAFITFFKEY.TRDP[0]));k++)betamc1+=BETAFITFFKEY.TRDP[k]*pow(fabs(beta),k);
