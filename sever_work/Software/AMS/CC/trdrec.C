//  $Id: trdrec.C,v 1.74 2024/12/18 19:43:25 choutko Exp $
#include <stdio.h>
#include <string.h>
#include "trdrec.h"
#include "event.h"
...
void AMSTRDCluster::setused(){
 for (unsigned int n=0;n<2*AMSTRDIdSoft::ncrates();n++){
  AMSTRDRawHit*ptr=(AMSTRDRawHit*)(AMSEvent::gethead()->getheadC("AMSTRDRawHitHC",n,0));
  while(ptr){
   if(ptr->checkstatus(AMSDBc::USED)){
    AMSTRDRawHit *ptra=(AMSTRDRawHit*)(AMSEvent::gethead()->getheadC("AMSTRDRawHit",n,0));
    while(ptra){
     if(ptr->getidsoft() == ptra->getidsoft()){
      ptra->setstatus(AMSDBc::USED);
      break;
     }
     ptra=ptra->next();
    }
...
integer AMSTRDCluster::build(int rerun){
  bool hc=TRDFITFFKEY.FitMethod/10 && rerun;
...
//for(int mode=0;mode<AMSTRDIdSoft::ncrates();mode+=AMSTRDIdSoft::ncrates()){
    ...
    number adc[trdconst::maxtube+3]={0};
    AMSTRDRawHit * ptra[trdconst::maxtube+3]={0};
    for (unsigned int n=mode;n<mode+AMSTRDIdSoft::ncrates();n++){
     AMSTRDRawHit * ptr=
     (AMSTRDRawHit*)(hc?AMSEvent::gethead()->getheadC("AMSTRDRawHitHC",n,2):AMSEvent::gethead()->getheadC("AMSTRDRawHit",n,2));  
     while(ptr){
       if(ptr->checkstatus(AMSDBc::USED)){
         ptr=ptr->next();
        continue;
	...
	void AMSTRDCluster::_copyEl(){
	 ...
	 if(PointerNotSet())return;
	   TrdClusterR & ptr=AMSJob::gethead()->getntuple()->Get_evroot02()->TrdCluster(_vpos);
	    if (_pmaxhit && !_pmaxhit->checkstatus(AMSDBc::DELETED)) {
                ptr.fTrdRawHit= _pmaxhit->GetClonePointer();
             }
...
integer AMSTRDTrack::_TrSearcher(int icall){
           if( _NoMoreTime()){
            AMSEvent::gethead()->setmoreerror(1);
            throw amsglobalerror(" AMSTRDTrack::_TrSearcher-E-Cpulimit Exceeded ",1);
...
double AMSTRDTrack::_Time=0;
double AMSTRDTrack::_TimeLimit=0;


void AMSTRDTrack::_Start(){_Time=AMSgettime();}
double AMSTRDTrack::_CheckTime(){return AMSgettime()-_Time;}
