...
OrbGen::OrbGen():Lat(0),Lon(0),CLat(0),CLon(0),Rigidity(0),cutoff(0),Pid(0){
  ...
  char name[300];
  sprintf(name,"%sOrbitGeneration.root",AMSDATADIR.amsdatadir);//the directory has been changed to v6.00
  TFile f(name);
...
void OrbGen::GenDir(bool isL0){
  ...
  if (isL0){
    Coo[0]=RNDM(dd)*(AMSDBc::ams_size_l0[0])-(AMSDBc::ams_size_l0[0]/2);
    Coo[1]=RNDM(dd)*(AMSDBc::ams_size_l0[1])-(AMSDBc::ams_size_l0[1]/2);
    Coo[2]=AMSDBc::ams_size_l0[2]/2;
  } else {
    Coo[0]=RNDM(dd)*(AMSDBc::ams_size[0])-(AMSDBc::ams_size[0]/2);
    Coo[1]=RNDM(dd)*(AMSDBc::ams_size[1])-(AMSDBc::ams_size[1]/2);
    Coo[2]=AMSDBc::ams_size[2]/2;
  }
