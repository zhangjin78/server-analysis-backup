//  $Id: LxMCcutoff.C,v 1.13 2022/06/16 10:30:27 qyan Exp $
...
double LxMCcutoff::GetWeight(double Rgen,double Rrec,int randseed)
{
  ...
  //double w=hExp->Interpolate(fabs(Rrec));
  //cout << "w " <<w<<" "<<Weight<<endl;
  if (hExp->GetBinContent(i0)==0) return Weight/hExp->Interpolate(fabs(Rrec));
  else return Weight/hExp->GetBinContent(i0);
}
...
QBincutoff::QBincutoff(){
  Init();
  TFile *f=TFile::Open(Form("%sExpotime_Cutoff20120520_20131126.root",AMSDATADIR.amsdatadir));
