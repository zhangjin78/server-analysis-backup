//  $Id: trdsim.C,v 1.68 2024/03/27 16:47:26 choutko Exp $
#include "trdsim.h"
#include "event.h"
#include "extC.h"
#include "tofsim02.h"
...
void AMSTRDRawHit::sitrddigi(){
    ...
    geant itrack=0;
    geant edep=0;
    geant amplitudePrimaryIonization = 0.0;
    geant amplitudeTransitionRadiation = 0.0;
	...
	i++;
      ...
      int processId = ptr->getprocid();
      int processType = processId >> 24;
      int processSubType = processId & 0xFFFFFF;

      // treat all energy depositions except transition radiation as ionization,
      // transition radiation is always associated with secondary particles
      if (ptr->getgtrkid() == 1)
	amplitudePrimaryIonization += ptr->getedep()*TRDMCFFKEY.GeV2ADC;
      else if (processType == 2 && processSubType == 24)
	amplitudeTransitionRadiation += ptr->getedep()*TRDMCFFKEY.GeV2ADC;
...
//      if(ptr->testlast()){
      if(ptr->next()==0||ptr->next()->_idsoft.cmpt()!=ptr->_idsoft.cmpt()){
        ...
        // apply ADC non-linearity correction
	// correction allowed for now only if TRDMCFFKEY.electronCutOnTubeWall is set small enough, preferably to 0.0
	if (TRDMCFFKEY.electronCutOnTubeWall < 0.1) {

	  if (TRDMCFFKEY.adcCorrection == 1) {
            if (amp <= 3000.0)
              amp = amp * 0.9957 - amp * amp * 0.00007455;
            else
              amp = 670.95 + amp * 0.5484;

	  } else if (TRDMCFFKEY.adcCorrection == 2) {

	    // apply MC gain
	    amplitudePrimaryIonization *= idsoft.getmcgain();
	    amplitudeTransitionRadiation *= idsoft.getmcgain();

	    double fractionPrimaryIonization = amplitudePrimaryIonization / amp;
	    double fractionTransitionRadiation = amplitudeTransitionRadiation / amp;

	    // pre-scale amplitude
	    amp *= 0.975;

	    // reduce amplitude according to a second-order polynomial correction
	    double amplitudeCorrected = 0.0;
	    if (amp <= 2300.0)
	      amplitudeCorrected = amp - amp * amp * 0.000052;
	    else
	      amplitudeCorrected = 275.08 + amp * 0.7608;

	    // reduce amplitude even more depending on the fraction of primary ionization
	    double correction = amp - amplitudeCorrected;
	    amp = amplitudeCorrected - correction * fractionPrimaryIonization * fractionPrimaryIonization * 0.85;

	    // compensate the reduction partly depending on the fraction of transition radiation
	    // but the compensation is not allowed to exceed the correction done so far
	    amp += std::min(correction, correction * correction * fractionTransitionRadiation * 0.0012);
	  }
	}
	...
	if (TRDMCFFKEY.electronCutOnTubeWall < 0.1 && TRDMCFFKEY.adcCorrection == 2) {
	  // include the non-linearity of electronics
	  if (amp <= 5200.0)
	    amp = amp + 0.0000139 * amp * amp - 0.0000000103 * amp * amp * amp;
	  else
	    amp = 2520.8 + amp * 0.309;
	}
        ...
        edep=0;
	amplitudePrimaryIonization = 0.0;
	amplitudeTransitionRadiation = 0.0;
      }
...
//--Merge TRDMCCluster
    AMSContainer * cont=AMSEvent::gethead()->getC("AMSTRDMCCluster",0);
    if((((G4FFKEY.MergeMCCluster/10)%10)>=1)&&cont){//merge the same geom and g4trkid
      ptr=(AMSTRDMCCluster *)cont->gethead();
      AMSTRDMCCluster *sptr=0;
      vector<AMSTRDMCCluster *>trdmccl;
      while (ptr){
       if((sptr==0)||(ptr->_idsoft.cmpt()!=sptr->_idsoft.cmpt())||(ptr->getgtrkid()!=sptr->getgtrkid())){
         sptr=new AMSTRDMCCluster(*ptr);
         trdmccl.push_back(sptr);
       }
       else {
//--xgl corresponds to the generated point, no change
/*         if(ptr->_edep>0){
           sptr->_xgl=(sptr->_xgl*sptr->_edep+ptr->_xgl*ptr->_edep)/(sptr->_edep+ptr->_edep);
         }*/
         sptr->_edep+=ptr->_edep;
         sptr->_step+=ptr->_step;
       }
       ptr=ptr->next();
     }
//---second merge
     if(((G4FFKEY.MergeMCCluster/10)%10)>=2){
        for(integer ii=integer(trdmccl.size())-1;ii>=0;ii--){//from end to begin
         ptr=trdmccl[ii];
         integer pid=ptr->_ipart;
         if(pid!=1&&pid!=2&&pid!=3)continue;//only merge gamma/e+/e-
         for(integer jj=ii-1;jj>=0;jj--){//front
           sptr=trdmccl[jj];
           if (ptr->_idsoft.cmpt()!=sptr->_idsoft.cmpt()||ptr->_itra>sptr->_gtrkid){//different idsoft || id is before
             break;
           }
           else if(ptr->_itra==sptr->_gtrkid){//just match id (only merge child to parent)
             number dx=ptr->_xgl[0]-sptr->_xgl[0];
             number dy=ptr->_xgl[1]-sptr->_xgl[1];
             number dr=sqrt(dx*dx+dy*dy);
             if(dr<2&&ptr->_step<2){//only merge r<2cm&&path<2cm
               sptr->_edep +=ptr->_edep;
               delete ptr;
               trdmccl.erase(trdmccl.begin()+ii);
             }
             break;
           }
         }
...
//---
     cont->eraseC();
     for(uinteger i=0;i<trdmccl.size();i++){
       cont->addnext(trdmccl[i]);
     }
   }
...
DAQCFFKEY.Mode=len%2==0?2:1;
  for (int j=DAQCFFKEY.Mode+rawl;j<len;j+=2){
        ...
        if(!id.dead()){
         ...
         AMSTRDRawHit *ph=new AMSTRDRawHit(id,amp);
         AMSEvent::gethead()->addnext(AMSID("AMSTRDRawHit",ic),ph); 
         bool hc=amp>TRDMCFFKEY.ThrHC;
         if(hc){
             AMSTRDRawHit *phhc=new AMSTRDRawHit(id,amp);
             phhc->setstatus(AMSDBc::DELETED);
             AMSEvent::gethead()->addnext(AMSID("AMSTRDRawHitHC",ic),phhc); 
         }
...
#ifdef __WRITEROOT__
    if(!checkstatus(AMSDBc::DELETED))AMSJob::gethead()->getntuple()->Get_evroot02()->AddAMSObject(this);
#endif
/*
