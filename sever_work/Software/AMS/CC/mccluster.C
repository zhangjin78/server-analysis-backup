//  $Id: mccluster.C,v 1.93 2018/12/14 11:24:04 qyan Exp $
...
void AMSTRDMCCluster::sitrdhits(
				integer idsoft , geant vect[],geant edep, geant ekin, geant step, integer ipart, integer itra, integer gtrkid, integer gtrkids, integer procid ){


        AMSPoint xgl(vect[0],vect[1],vect[2]);
        AMSDir xvec(vect[3],vect[4],vect[5]);
      AMSEvent::gethead()->addnext(AMSID("AMSTRDMCCluster",0),
                                   new AMSTRDMCCluster(idsoft,xgl,xvec,step,ekin,edep,ipart,itra, gtrkid, gtrkids, procid));
...
void AMSTOFMCCluster::sitofhits(integer _idsoft, geant _vect[],geant _edep, 
geant _tofg, geant _ekin,geant _beta, geant _edepr, geant _step, integer _parentid, integer _particle, integer _gtrkid){
  //        Very Temporary
  AMSPoint pnt(_vect[0],_vect[1],_vect[2]);
   AMSEvent::gethead()->addnext(AMSID("AMSTOFMCCluster",0),
   new AMSTOFMCCluster(_idsoft,pnt,_edep,_tofg,_ekin,_beta,_edepr,_step,_parentid,_particle,_gtrkid));
}
