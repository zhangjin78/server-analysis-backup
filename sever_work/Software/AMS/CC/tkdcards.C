// $Id: tkdcards.C,v 1.185 2025/04/11 16:09:56 nbelyaev Exp $
#include "tkdcards.h"
#include "TrCluster.h"
#include <math.h>
#include <float.h>
...
//L2AlignPar[2] = 0.31;       //  Shift (um) / year
  ...
  RigidityCorr[0] = 1347000000; // Reference time
  RigidityCorr[1] = 1431200000; // Time since when the correction is constant
  RigidityCorr[2] =  0.0896;    // p1 of pol2 correction (1/TV/year)
  RigidityCorr[3] = -0.0168;    // p2 of pol2 correction (1/TV/year)

  RigidityGeomCorr = 0.0993;// fullspan acceptance and l1inner acceptance relative rigidity scale [TV-1]
  RigidityScaleCorr[0]= -0.085; // 5Years-ISS e+/e- new measured absolute rigidity scale with GetCorrectedRigidity[TV-1]
  RigidityScaleCorr[1]= -0.035; // 5Years-ISS e+/e- new measured absolute rigidity scale without GetCorrectedRigidity[TV-1]
}
...
//  recflag=11111;
//  recflag=1010011;//P7
  recflag=11010022;//P8 with V6 (search P8 in this file)
  for (int ii=0;ii<9;ii++){
    ThrSeed[0][ii] = 3.50;
    ThrSeed[1][ii] = 3.50;
  }
  for (int ii=0;ii<2;ii++){
    ThrSeedL0[ii] = 3.50; // Put this in use in the L0 reconstruction <<##>>
  }
  ThrNeig[0]  = 1.00;
  ThrNeig[1]  = 1.00;
...
//  MaxNtrHit      = 1600;
  ...
  TrDEBUG = 0;
  PZDEBUG = 0;
  AdvancedFitFlag = 0x5f;//(kChoutko,kChoutko|kMultScat,kAlcaraz,kAlcaraz|kMultScat,kGEANE_Kalman|kMultScat|kEnergyLoss,kGBLFITTERAMS|kMultScat) are on
  //TAS
  TasRecon = 0;
  TasCurr  = 0;
  TasLDDR  = 0;
...
//  MergeDiffRecon = 1;
  ...
  TrParticleType = 1;
//  ClusterCofGOpt = 0;
  ClusterCofGOpt = 13;//P7/P8
//  KeepBestTrClHits = 0;
  KeepBestTrClHits = 1;//P7/P8
//  MaxZQccList  = 50; //50 highest Y
  MaxZQccList  = 100; //100 highest Y P8 with V6
  MaxZQccListF = 2;
  MaxZQ2ndTrack = 4; //secondary track
//  ZQDropYChisq=1000.;//P7
  ZQDropYChisq=100.;//P8 with V6 (with MS)
//  ZQXChisqCut = 85.;//with error 80um: with ~20um err, ChisX/Ndf=85*4*4/(NHitX-2)~340 (NHitX=6)
  ZQXChisqCut = 250.;//P8 with V6 with error 80um: with ~20um err, ChisX/Ndf=250*4*4/(NHitX-2)~1000 (NHitX=6)
  AdvancedFitFlagHalf = 0x5a;//(kChoutko|kMultScat,kAlcaraz|kMultScat,kGEANE_Kalman|kMultScat|kEnergyLoss,kGBLFITTERAMS|kMultScat) are on
  AdvancedFitFlagExtAll = 0x5a;//(kChoutko|kMultScat,kAlcaraz|kMultScat,kGEANE_Kalman|kMultScat|kEnergyLoss,kGBLFITTERAMS|kMultScat) are on
  ZSEEDClQThr = 0.6;
//  MaxZQAngleF = 0.;//P7
  MaxZQAngleF = -6.;//P8 1./sqrt(6),0.24(0.4)GV->0.1(0.18)GV->0.04(0.072)GV
  ZQDropYRth=4.;
  MaxZQXWindow[0] = 1.;//0.0136/1.GV*sqrt(0.01)*40e4~500um*this=500um windows for X search
  MaxZQXWindow[1] = 2.5;//P8 500um*this=1250um windows for X search together with MaxZQAngleF
  MergeLim[0]=1.;//distance scale to refind X hits
  MergeLim[1]=1.;//distance scale to refind Y hits
  MergeExtLimH[0]=1.;//distance scale to find external X hits at high rigidity
  MergeExtLimH[1]=1.;//distance scale to find external Y hits at high rigidity
}
...
#endif
TRCLFFKEY_DEF & TRCLFFKEY_DEF::A(){
return  TRCLFFKEY;
}

int TRCLFFKEY_DEF::ReadFromFile = 1;
...
void TRMCFFKEY_DEF::init(){
  ...
  // Noise type 
  NoiseType = 1;
  // [563,584] DSP seed strip S/N threshold per layer (updated 03/03/2011)
  ...
  */
  ...
  DSPSeedThrL0[0]=4.000; DSPSeedThrL0[1]=4.000;
  ...
  //  2106.02.05 PZ feet material bias parameters
  EnableTrackerFeet=1;
  PEEKDensityBias=1.;
  FeetAlDensityBias=1.;
  MergeD[0]=0.015;
  ...
  // Normalize SimCluster total signal 
  NormalizeSimCluster = 0;
  NormAdjBefore[0] = -1;
  NormAdjBefore[1] = -1;
  EdepAddNoise = 0.02;
  NormAdjAfter[0] = 1;
  ...
  // Sub-stepping for Belau diffusion model (TrSim2010_DiffType = 5) 
  BelauSubStep = 10.; // um
  // Minimum diffusion radius for Belau diffusion model (TrSim2010_DiffType = 5) 
  BelauMinDiffusion = 1.; // um
  // Multiplicative factor to diffusion radius for Belau diffusion model (TrSim2010_DiffType = 5) 
  BelauFactDiffusion = -1.; // <0 for hard-coded diffusion dependence on particle charge
  // Hall mobilities (TrSim2010_DiffType = 5) 
  BelauHallMobility[0] = 0; // (1800) electrons, cm^2/V/s 
  BelauHallMobility[1] = 0; // (400) holes, cm^2/V/s

  // Apply active silicon specific cuts
  SetActiveSiliconCuts = 2; // 0: disabled, 1: enabled, 2: enable, automatic settings (z<=2 4.2um/10 keV, z>2 73um/100 keV)
  // Maximum step in active silicon material   
  MaxStepInSilicon = 1000.0;
  // Minimum delta-ray production range (um)
  MinDeltaRayRange = 4.2;   
  // Minimum delta-ray production/propagation range (keV), cut "a la GEANT3" 
  MinDeltaRayEnergy = 10.; 
  // Apply to MC cluster better energy deposition and direction estimations
  SetBetterEnergyAndDir = 1; // 0: disabled, 1: enabled

  // Cluster asymmetry
  Asymmetry[0] = 0.040; 
  Asymmetry[1] = 0.005; 

  // Apply dX and dY shift dependence with impact angle  
  BelauShift = 0;
  // Disable ADC overflow   
  DisableOverflow = 0;
  TPar[0]=500;
  TPar[1]=8;

  // Max secondary energy for MergeMCCluster3
  MaxSecondaryKn = 0.01;
  
  // Outer smearing to simulate New V6 external layer(L1/L9) misalignment
  OuterSmearingN[0][0]=0.; //L1 X
  OuterSmearingN[0][1]=0.; //L1 Y
  OuterSmearingN[1][0]=0.; //L9 X
  OuterSmearingN[1][1]=0.; //L9 Y
}
...
//  MergeExtLimX=0.5;//P7
  MergeExtLimX=2.5;//P8 with V6
  /// R 14 max dist to add external planes hits Y
//  MergeExtLimY=0.5;//P7
  MergeExtLimY=2.5;//P8 with V6
...
//  RthdDropX[0] = 100;//P7
  RthdDropX[0] =  50.;//P8 with V6
  RthdDropX[1] =  10.;

  /// R [19,20] Factor on ErrX/ErrY for He
  ErcHeX = 0.65;
//  ErcHeY = 0.65;   // use realistic Y Errors;
  ErcHeY = 0.;
...
// I  23 magnet temp correction enabled
  ...
  /// R 24 Additional fit point Yerror (Layer 1)
  ErrYL1 =  9.0e-4/1.4;
  /// R 25 Additional fit point Yerror (Layer 9)
  ErrYL9 = 11.5e-4/1.4;
...
//  Zshift = 0;
  Zshift = 2;//=2 ISS, =-1 MC 
   ...
   /// R 57 Additional fit point Xerror (Layer 1)
   ErrXL1 = 9.0e-4;
   /// R 58 Additional fit point Xerror (Layer 9)
   ErrXL9 = 11.5e-4; 
}
...
#endif
TRFITFFKEY_DEF & TRFITFFKEY_DEF::A(){
return  TRFITFFKEY;
}
