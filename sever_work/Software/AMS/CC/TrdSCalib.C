...
//  2012.07.10 move TrdCalDBs into AMSDATADIR.amsdatadir/TRD/
//  2012.07.11 update PDFs (p,e,He) in rigidity bin instead of momentum bin
//  2012.07.20 released version 6 
//  2012.08.09 update ProcessTrdEvtWithTrTrack for multi-tracks event
//--------------------------------------------------------------------------------------------------
...
#ifdef __PPC64
#pragma omp threadprivate (TrdSCalibR::head)
#endif
  void  TrdSCalibR::KillPointer()
   {
           delete head;
               head=0;
                 }
...
const char *TrdSCalibR::TrdElectronProtonHeliumLFname[] =
  {
    ...
    //"TrdScalibPdfs_v22.root"       //== extract from v16 
    "TrdScalibPdfs_v23.root"       //== extract from v13                                            
  };
...
const char *TrdSCalibR::TrdGeomUpdateDir[] =
  {
    "TRD/Geom",//"DataBase/TrdCalNew/Geom",
...
bool TrdSCalibR::AC_InitTrdGeom(int Debug){
  char tdname[200];
  sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdGeomUpdateDir[0], TrdGeomDB[0]);//the directory has been changed to v6.00
  if( AC_InitTrdShim( tdname, Debug ) ) return false;
    
  sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdGeomUpdateDir[0], TrdGeomDB[1]);
...
bool TrdSCalibR::GetcTrkdTrk(TrTrackR *trt){
  ...
  if(fitcode < 0) {
    std::cout << Form("TrdSCalibR::GetcTrkdTrk-W-  TrackFit algo %d and patt %d Not AVAILABLE Fitcode=%d !!!!",
...
//--------------------------------------------------------------------------------------------------
bool TrdSCalibR::GetcTrkdTrk(TrTrackR *trt, int fitcode){
  if(!trt || fitcode<0 ) return false;
  trt->Interpolate(trdconst::ToFLayer1Z,cTrk,dTrk, fitcode);
  return true;
}
//--------------------------------------------------------------------------------------------------
...
int TrdSCalibR::GetModCalibTimePeriod(TString fname, TString hname, int Debug) {
  ...
  if (!input->IsOpen()) {  
    Error("TrdSCalibR::GetModCalibTimePeriod-E- ","Error Openning File %s", (const char*) fname);
...
bool TrdSCalibR::Get01TrdCalibration(TString fname, int Debug) {
  ...
  TFile *input = TFile::Open(fname);
  if (!input->IsOpen()) {
    Error("TrdSCalibR::Get01TrdCalibration-E- ","Error Openning File %s", (const char*) fname);
    delete input; input = 0;
    return false;
  }
  int nLost = 0;
...
bool TrdSCalibR::Get02TrdCalibration(TString fname, int Debug) { 
  ...
  TFile *input = TFile::Open(fname);
  if (!input->IsOpen()) {
    Error("TrdSCalibR::Get02TrdCalibration-E- ","Error Openning File %s", (const char*) fname);
    delete input; input = 0;
    return false;
  }
  int nLost = 0;
...
bool TrdSCalibR::Get03TrdCalibration(TString fname, int Debug) {
  ...
  TFile *input = TFile::Open(fname);
  if (!input->IsOpen()) {
    Error("TrdSCalibR::Get03TrdCalibration-E- ","Error Openning File %s", (const char*) fname);
    delete input; input = 0;
    return false;
  }
...
bool TrdSCalibR::Init(int CalibLevel, int Debug){
  ...
  /// get TRD calibration graphs / each module level (v6)
  else if(CalibLevel < 7) ok_gain = GetTrdV5CalibHistos(Debug);
...
bool TrdSCalibR::GetTrdCalibHistos(int CalibLevel, int Debug) {
  ...
  if (CalibLevel>0) 
    {
      sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdCalDBUpdate[0]);
      TrdCalib_01 = Get01TrdCalibration(tdname, Debug);
    }
  if (CalibLevel>1) 
    {
      sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdCalDBUpdate[1]);
...
bool TrdSCalibR::GetTrdV3CalibHistos(int CalibLevel, int Debug) {
  ...
  if (CalibLevel>2) 
    {
      sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdCalDBUpdate[2]);
...
bool TrdSCalibR::TrdLR_CalcIniPDF(int Debug) {
  ...
  /// version 2
  if(SCalibLevel < 3)
    sprintf(fname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0],TrdElectronProtonHeliumLFname[3]);
  
  /// version 3
  else if(SCalibLevel < 4) 
    sprintf(fname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0],TrdElectronProtonHeliumLFname[4]);
  
  /// version 4
  else if(SCalibLevel < 5)  
    sprintf(fname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0],TrdElectronProtonHeliumLFname[6]); 
  
  else 
    sprintf(fname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0],TrdElectronProtonHeliumLFname[7]); 
...
//--------------------------------------------------------------------------------------------------
float TrdSCalibR::PathLenCorr(int iXe, float EadcCS, float Len3D) {

  static float par_Xe_0[6]={  2.0061E+02, -1.6242E+02,  6.7361E+01, -1.4213E+01,  1.5740E+00, -7.1921E-02};
  static float par_Xe_1[6]={  2.0841E+02, -1.6918E+02,  7.0184E+01, -1.5019E+01,  1.7025E+00, -7.9701E-02};
  static float par_Xe_2[6]={  2.1589E+02, -1.7224E+02,  7.1600E+01, -1.5710E+01,  1.8396E+00, -8.8564E-02};
  static float par_Xe_3[6]={  2.1663E+02, -1.7184E+02,  7.1737E+01, -1.6149E+01,  1.9678E+00, -9.8553E-02};
  static float par_Xe_4[6]={  2.2346E+02, -1.7919E+02,  7.5057E+01, -1.6650E+01,  1.9651E+00, -9.4950E-02};
  static float par_Xe_5[6]={  1.8856E+02, -1.4539E+02,  5.6280E+01, -1.1047E+01,  1.1788E+00, -5.3757E-02};
  
  if (Len3D<=0.0) return EadcCS;
  
  // 0. order path len correction dE => dE/dx
  float  Eadc 	= EadcCS * 2.0*trdconst::TrdStrawRadius/Len3D;
  
  float  Len3D_MM = Len3D*10.0;
  if (Len3D_MM< (trdconst::TrdMinPathLen3D*10.) || Len3D_MM>(trdconst::TrdMaxPathLen3D*10.)) return Eadc;
  
  // 1. order path len correction, i.e. correct for non-linearities in dE/dx
  float Corr =  60.;
  switch (iXe) {
  case 0: 
    Corr = par_Xe_0[0] + par_Xe_0[1]*Len3D_MM + par_Xe_0[2]*pow(Len3D_MM,2) + par_Xe_0[3]*pow(Len3D_MM,3) + 
      par_Xe_0[4]*pow(Len3D_MM,4) + par_Xe_0[5]*pow(Len3D_MM,5); break;
  case 1: 
    Corr = par_Xe_1[0] + par_Xe_1[1]*Len3D_MM + par_Xe_1[2]*pow(Len3D_MM,2) + par_Xe_1[3]*pow(Len3D_MM,3) + 
      par_Xe_1[4]*pow(Len3D_MM,4) + par_Xe_1[5]*pow(Len3D_MM,5); break;
  case 2: 
    Corr = par_Xe_2[0] + par_Xe_2[1]*Len3D_MM + par_Xe_2[2]*pow(Len3D_MM,2) + par_Xe_2[3]*pow(Len3D_MM,3) + 
      par_Xe_2[4]*pow(Len3D_MM,4) + par_Xe_2[5]*pow(Len3D_MM,5); break;
  case 3: 
    Corr = par_Xe_3[0] + par_Xe_3[1]*Len3D_MM + par_Xe_3[2]*pow(Len3D_MM,2) + par_Xe_3[3]*pow(Len3D_MM,3) + 
      par_Xe_3[4]*pow(Len3D_MM,4) + par_Xe_3[5]*pow(Len3D_MM,5); break;
  case 4: 
    Corr = par_Xe_4[0] + par_Xe_4[1]*Len3D_MM + par_Xe_4[2]*pow(Len3D_MM,2) + par_Xe_4[3]*pow(Len3D_MM,3) + 
      par_Xe_4[4]*pow(Len3D_MM,4) + par_Xe_4[5]*pow(Len3D_MM,5); break;
  case 5: 
    Corr = par_Xe_5[0] + par_Xe_5[1]*Len3D_MM + par_Xe_5[2]*pow(Len3D_MM,2) + par_Xe_5[3]*pow(Len3D_MM,3) + 
      par_Xe_5[4]*pow(Len3D_MM,4) + par_Xe_5[5]*pow(Len3D_MM,5); break;
  }
  return Eadc * trdconst::TrdMeanMPV/Corr;
...
int TrdSCalibR::TrdLR_CalcXe(double xDay, float Rabs, int iFlag, int Debug) {
  ...
  if( Rabs < trdconst::CalMomMin || Rabs > trdconst::CalMomMax ) return 21;
  ...
  for (vector<AC_TrdHits>::iterator iter = TrdSHits.begin(); iter != TrdSHits.end(); ++iter) {
    ...
    if ( Len < 0. ) continue; 
    if (TrdTrackLevel<2 && Len < trdconst::TrdMinPathLen3D) continue;
    ...
    par[0] 	        = iXe; 
    par[1] 	        = iR;	
    myPair <double> myProtonL  (1E-7, fabs(TrdSCalibR::TrdS_PDF_fProton(x,par)));
    double L_Proton     = myProtonL.GetMax();

    myPair <double> myHeliumL  (1E-7, fabs(TrdSCalibR::TrdS_PDF_fHelium(x,par)));
    double L_Helium   = myHeliumL.GetMax();

    par[1]              = Layer;  
    myPair <double> myElectronL(1E-7, fabs(TrdSCalibR::TrdS_PDF_fElectron(x,par)));
    double L_Electron   = myElectronL.GetMax();

    if(L_Proton==1 || L_Helium==1 || L_Electron==1) continue;

    if(Debug > 1) 
      std::cout << Form("%3d Len=%+5.1f EadcR=%4d EadcCS=%4d EadcPL=%4d L_Proton=%12.4E L_Helium=%12.4f L_Electron=%12.4f",
			nCount, Len, (int)(*iter).EadcR, (int)(*iter).EadcCS, (int) Eadc,L_Proton,L_Helium,L_Electron) << std::endl;
  ...
  if(nCount < trdconst::nTrdMinHits) return 25;
 
  if(Debug > 1) 
    std::cout << Form("Step 1. N=%3d |R|=%6.2f(GeV/c) Lprod_Electron=%12.4E Lprod_Helium=%12.4E Lprod_Proton=%12.4E", 
		      nCount, Rabs, Lprod_Electron, Lprod_Helium, Lprod_Proton) << std::endl;
...
int TrdSCalibR::TrdLR_MC_CalcXe(double xDay, float Rabs, vector<bool> PartId, int Debug) {
  ...
  if( Rabs<trdconst::CalMomMin || Rabs>trdconst::CalMomMax ) return 2;
...
bool TrdSCalibR::Get04TrdCalibration(TString fname, int Debug) {
  ...
  TFile *input = new TFile(fname);
  if (!input->IsOpen()) {
    Error("TrdSCalibR::Get04TrdCalibration-E- ","Error Openning File %s", (const char*) fname);
    return false;
  }
...
bool TrdSCalibR::GetTrdAlignment(TString fname, int Debug) {
  ...
  TFile *input = new TFile(fname);
  if (!input->IsOpen()) {
    Error("TrdSCalibR::GetTrdAlignment-E- ","Error Openning File %s", (const char*) fname);
    delete input; input=0;
    return false;
  }
...
bool TrdSCalibR::GetTrdV4CalibHistos(int CalibLevel, int Debug) {
  ...
  if ( CalibLevel < 5 )  sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdCalDBUpdate[4]);   
  else if ( CalibLevel < 6 )  sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdCalDBUpdate[5]);
  else return false;
  ...
  if(SetTrdAlign) 
	...
	if ( CalibLevel < 5 ) sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdAlignDBUpdate[0]); 
	else if ( CalibLevel < 6 )  sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdAlignDBUpdate[1]); 
...
bool TrdSCalibR::GetTrdV5CalibHistos(int Debug) {
  ...
  TrdGain_00 = false, TrdGain_01 = false;  TrdGain_02 = false;  TrdGain_03 = false;
  TrdAlign_00 = false, TrdAlign_01 = false; TrdAlign_02 = false; TrdAlign_03 = false;

  if(SCalibLevel < 5) return false;
  if(TrdGainMethod  > 3) return false;
  if(TrdAlignMethod > 3) return false;

  char tdname[200]=""; 
  if (TrdGainMethod == 0) {       /// no gain correction
    TrdGain_00 = true;
   
  }else if(TrdGainMethod == 1) {  //== method from S.Schael
    /// Call TRD Calibration Histograms 
    sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdCalDBUpdate[5]);
  ...
  if(SetTrdAlign) 
    {
      ...
      if(TrdAlignMethod == 0) { 
	TrdAlign_00 = true;
      } else if(TrdAlignMethod == 1) { //== method from S.Schael & T.Siedenburg
	sprintf(tdname, "%s%s/%s", AMSDATADIR.amsdatadir, TrdDBUpdateDir[0], TrdAlignDBUpdate[1]); 
	TrdAlign_01 = GetTrdAlignment(tdname, Debug);
  ...
  if ( (TrdGain_00 || TrdGain_01 || TrdGain_02 || TrdGain_03) && (TrdAlign_00 || TrdAlign_01 || TrdAlign_02 || TrdAlign_03) ) 
    return true;
  else return false;
...
int TrdSCalibR::BuildTrdSCalib(time_t evut, double fMom, TrdHTrackR *TrdHtrk, TrTrackR *Trtrk, double &s1, double &s2, double &s3, int Debug){
  ...
  if(BuildTrdSHits(TrdHtrk, Debug)) {ClearTrdSHits(); return 4;} 

  int ilr = TrdLR_CalcXe(Xtime, fMom, iFlag, Debug);
...
int TrdSCalibR::BuildTrdSCalib(time_t evut, double fMom, TrdTrackR *Trdtrk, TrTrackR *Trtrk, double &s1, double &s2, double &s3, int Debug){
  ...
  if( !GetdTrd(Trdtrk) ||  !GetcTrd(Trdtrk) ) return 2;
  if( !GetcTrkdTrk(Trtrk) ) return 3;
  ...
  if(BuildTrdSHits(Trdtrk, Debug)) {ClearTrdSHits(); return 4;} 
...
//--------------------------------------------------------------------------------------------------
int TrdSCalibR::BuildTrdSHits(TrdHTrackR *TrdHtrk, int Debug){

  if(!TrdHtrk) return 1;
  
  TrdSHits.clear();
  TrdNHits.clear();
  nTrdHitLayer.assign(long(trdconst::nTrdLayers), 0);
  ...
  for (int iseg=0; iseg < TrdHtrk->NTrdHSegment(); iseg++) {
    ...
    for(int ir=0; ir < (int)TrdHtrk->pTrdHSegment(iseg)->fTrdRawHit.size();ir++) {
      ...
      if(!rhit) continue;
      TrdNHits.push_back(AC_TrdHits(rhit));  
      itrdhit++;
    }
  }
  ...
  for (vector<AC_TrdHits>::iterator iter = TrdNHits.begin(); iter != TrdNHits.end(); ++iter) {
    (*iter).TrkDraw = (*iter).GetTrdHitTrkDistance(*iter, cTrk, dTrk);
    (*iter).TrdDraw = (*iter).GetTrdHitTrkDistance(*iter, cTrd, dTrd);
    (*iter).TrkD    = (*iter).TrkDraw; (*iter).TrdD = (*iter).TrdDraw; 

    /// apply gain and time-dep. alignment correction
    if( GetThisTrdHit( (*iter), TrdGainMethod, TrdAlignMethod, Debug) ) continue;
  ...
  _nOntrackhit  = jtrdhit;
  _nOfftrackhit = itrdhit - jtrdhit;
...
//--------------------------------------------------------------------------------------------------
int TrdSCalibR::BuildTrdSHits(TrdTrackR *Trdtrk, int Debug){
...
int TrdSCalibR::ProcessTrdHit(TrdHTrackR *TrdHtrk, int Debug){
  ...
  if( GetTrdMedian(Debug) ) return 16;
  if( GetTruncatedMean(Debug) ) return 17;
  */

  int ilr = TrdLR_CalcXe(Xtime, iRabs, iFlag, Debug);
  if(ilr >0) return ilr;
...
int TrdSCalibR::ProcessTrdHit(TrTrackR *Trtrk, int Debug){
  ...
  */
  ...
  return TrdLR_CalcXe(Xtime, iRabs, iFlag, Debug); 
...
int TrdSCalibR::GetnTrdHitLayer(vector<AC_TrdHits> &TrdHits, int Debug){
  ...
  if(Debug)
    std::cout << Form("*** TrdNlayer=%d  TrdNhits=%d HitLayer=(%2d %2d %2d)", 
		      TrdNlayer, TrdNhits, ihitlayer[0],ihitlayer[1],ihitlayer[2] ) 
	      << std::endl;
...
vector<int> TrdSCalibR::TrdFillHits( vector<AC_TrdHits> &TrdHits, int Debug) {
  ...
  Elay1.assign(long(trdconst::nTrdLayers),0);
  Elay2.assign(long(trdconst::nTrdLayers),0);
  ...
  for (vector<AC_TrdHits>::iterator iter = TrdHits.begin(); iter != TrdHits.end(); ++iter) 
    {
      ...
      (*iter).TrkD = iTrkD;
      if ( fabs(iTrkD)>trdconst::CutTrdTrkD ) continue;
...
vector<int> TrdSCalibR::TrdFillHits2( vector<AC_TrdHits> &TrdHits, int Debug) {
  ...
  if(Debug)
    std::cout << Form("*** TrdSCalibR::TrdFillHits2-I After  TrdHits.size=%d  Erased=%d", 
		      (int) TrdHits.size(), nerase) << std::endl;
...
vector<int> TrdSCalibR::CalPathLen3D(vector<AC_TrdHits> &TrdHits, TrTrackR *Trtrk, int TrdStrkLevel, int Debug) {
  ...
  if (nHitOnStrk>17 && (nHitNearStrk-nHitOnStrk)<4) return nTrdStat;
  //if (nHitOnStrk > 15 && (nHitNearStrk-nHitOnStrk) < 6 ) return nTrdStat;
...
bool TrdSCalibR::NeedTrkSpline(TrTrackR *Trtrk, int Debug){
  ...
  if (fabs(DiffX)<1.0 && fabs(DiffY)<1.0 ) return true;
  else return false;
}
//--------------------------------------------------------------------------------------------------
...
int TrdSCalibR::GetTrdNewHits_ms(vector<AC_TrdHits> TrdHits, int Debug) {
  ...
  if(Debug) 
    std::cout << Form("TrdSCalibR::GetTrdNewHits_ms-I- NHits=%d SHits=%d", 
		      (int)TrdHits.size(), (int)TrdSHits.size()) << std::endl;
...
int TrdSCalibR::GetTrdNewHits(vector<AC_TrdHits> TrdHits, int Debug) {
  ...
  if(Debug) 
    std::cout << Form("TrdSCalibR::GetTrdNewHits-I- NHits=%d SHits=%d", 
		      (int) TrdHits.size(), (int) TrdSHits.size()) << std::endl;
...
int TrdSCalibR::GetThisTrdHit(AC_TrdHits &TrdHit, int iGainMethod, int iAlignMethod, int Debug) {
  
  aP.assign(long(trdconst::nTrdModules), 0);
  aA.assign(long(trdconst::nTrdModules), 0);
  ...
  /// initiate trdK base
  if(iGainMethod ==2 || iAlignMethod == 2)
     TrdKCluster _trdcluster= TrdKCluster();

  /// apply gain correction
  switch (iGainMethod) {
  case 0: //if(TrdGainMethod == 0) {
    TrdHit.EadcCS = TrdHit.EadcR;
    break;

  case 1:  //if(TrdGainMethod == 1) {         //== Default from AC
  ...
  case 3: //if(TrdGainMethod == 3) {  //== AC-MM
  ...
  default:
    _ierror++;
    if(_ierror<trdconst::nMaxError)
      Error("TrdSCalibR::GetThisTrdHit-E- ", "Error in GainMethod = %d", iGainMethod);
    return 1;
    break;
  }
  ...
  case 0: //== no alignment
    ...
    TrdHit.hitXY = TrdHit.hitXYraw;
    break;

  case 1: //== AC based on module level
  ...
  case 2: //== MIT based on layer level
  ...
  case 3: //== AC based on module level
  ...
  default: // else {
    _ierror++;
    if(_ierror<trdconst::nMaxError)
      Error("TrdSCalibR::GetThisTrdHit-E- ", "Error in AlignMethod=%d", iAlignMethod);
    return 2;
    break;
...
int TrdSCalibR::InitiateTrdRawHit(AMSEventR *pev, vector<AC_TrdHits> &TrdHits, int Debug){  
  ...
  for (int i=0; i<pev->nTrdRawHit(); i++) {
    ...
    if(!rhit) continue;
    TrdHits.push_back(AC_TrdHits(rhit));
  }
...
//--------------------------------------------------------------------------------------------------
bool TrdSCalibR::CheckEvtMatchingTimePeriodCalDB(AMSEventR *pev, int Debug) {
...
int TrdSCalibR::ProcessTrdEvt(AMSEventR *pev, int Debug) {
  ...
  int ifitcode=-1, iret=0;
  /// check tracker object
  Trtrk   = pev->pParticle(0)->pTrTrack();
  if(Trtrk) {
    ifitcode = Trtrk->iTrTrackPar(algo, patt, refit);
  ...
  if( !GetcTrkdTrk(Trtrk, ifitcode) ) return 4;
  ...
  case 2: //== TrdHtrack (AC)         
    if( pev->nTrdHTrack() == 0 ) return 6;
    if(!pev->pParticle(0)->pTrdHTrack())  return 7;
    TrdHtrk = pev->pParticle(0)->pTrdHTrack();
    if( !GetdTrd(TrdHtrk) ||  !GetcTrd(TrdHtrk) ) return 8;
    return ProcessTrdHit(TrdHtrk, Debug); 
    break;

  case 3: //== Trdtrack   (MIT)
    if( pev->nTrdTrack() == 0 ) return 6;
    if(!pev->pParticle(0)->pTrdTrack())   return 7;
    Trdtrk = pev->pParticle(0)->pTrdTrack();
    if( !GetdTrd(Trdtrk) ||  !GetcTrd(Trdtrk) ) return 8;
...
//--------------------------------------------------------------------------------------------------
int TrdSCalibR::ProcessTrdEvtWithTrdTrack(AMSEventR *pev, TrdTrackR *Trdtrk, TrTrackR *Trtrk, int fitcode, int Debug){
  ...
  /// check trd track  
  if( !Trdtrk || !GetdTrd(Trdtrk) ||  !GetcTrd(Trdtrk) ) return 2;

  /// check tracker track
  if( !Trtrk  || !GetcTrkdTrk(Trtrk, fitcode) ) return 3;

  /// check trtrack
  if( ProcessTrTrack(Trtrk, fitcode) ) return 6;
   
  itrdcalib = ProcessTrdHit(Trdtrk, Debug);

  return itrdcalib;

}
//--------------------------------------------------------------------------------------------------
int TrdSCalibR::ProcessTrdEvtWithTrdHTrack(AMSEventR *pev, TrdHTrackR *TrdHtrk, TrTrackR *Trtrk, int fitcode, int Debug){
  ...
  /// check trd track  
  if( !TrdHtrk || !GetdTrd(TrdHtrk) ||  !GetcTrd(TrdHtrk) ) return 2;
  ...
  itrdcalib = ProcessTrdHit(TrdHtrk, Debug);
...
int TrdSCalibR::ProcessTrdEvtWithTrTrack(AMSEventR *pev, TrTrackR *Trtrk, int fitcode, int Debug){
  ...
  /// get tracker track point and direction
  /// global cTrk, dTrk from given fitcode
  if( !GetcTrkdTrk(Trtrk, fitcode) ) return 4;
  ...
  if( GetTrkCoordinates(Trtrk, Debug) ) return 7;
  ...
  if( !init_nhit || init_nhit > trdconst::nTrdMaxHits) {
    ...
    return 8; //== no hit or too many hits
  }
...
//--------------------------------------------------------------------------------------------------
void TrdSCalibR::SetParsTrdSCalib(int CalVer, int TrackType, int GainMethod, int AlignMethod) {
  SCalibLevel=CalVer; TrdTrackLevel=TrackType; TrdGainMethod=GainMethod; TrdAlignMethod=AlignMethod;
  return;
}

void TrdSCalibR::GetParsTrdSCalib(int &CalVer, int &TrackType, int &GainMethod, int &AlignMethod) {
  CalVer=SCalibLevel; TrackType=TrdTrackLevel; GainMethod=TrdGainMethod; AlignMethod=TrdAlignMethod;
  return;
}
