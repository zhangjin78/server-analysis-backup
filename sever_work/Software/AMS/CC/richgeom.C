//  $Id: richgeom.C,v 1.69 2024/05/28 13:17:35 nbelyaev Exp $
#include "gmat.h"
#include "gvolume.h"
#include "commons.h"
#include "richdbc.h"
...
void amsgeom::Put_rad(AMSgvolume * mother,integer copia,int tile)
{
  ...
  //
  // Add the Carbon Fibre tabe
  //

  par[0]=RichRadiatorTileManager::get_tile_boundingbox(tile,0)+RICaethk/2;
  par[1]=RichRadiatorTileManager::get_tile_boundingbox(tile,1)+RICaethk/2;
  par[2]=0.2/2;
  coo[0]=0;
  coo[1]=0;
  coo[2]=RICHDB::rad_height/2-par[2];

  mother->add(new AMSgvolume("RICH CARBON",
			     0,
			     "RADT",
			     "BOX",
			     par,
...
// Electronics
  ...
  dummy=lig->add(new AMSgvolume("RPMT BODY", 
				0,
...
void amsgeom::richgeom02(AMSgvolume & mother, float ZShift)
{
  ...
  par[4]=(RICradiator_box_radius+RICradiator_box_thickness+RICepsln)/
    cos(2.0*M_PI/RICradiator_box_sides/2.0);
  ...
  if(!(AMSJob::gethead()->isRealData())){
    if(strstr(AMSJob::gethead()->getsetup(),"AMS02P.2") && !RichAlignment::IgnoreDB()){
      // Force the expected misalignment
//      RichAlignment::Set(-0.11,0.08,0.93,0,0,0);
      RichAlignment::Set(-RICCONTROLFFKEY.mcgeomshift[0],-RICCONTROLFFKEY.mcgeomshift[1],-RICCONTROLFFKEY.mcgeomshift[2],0,0,0);
      RichAlignment::SetMirrorShift(0,0,0);
    } else if(strstr(AMSJob::gethead()->getsetup(),"AMS02P") && !RichAlignment::IgnoreDB()){
      // Force the expected misalignment
//      RichAlignment::Set(-0.11,0.08,0.93,0,0,0);
      RichAlignment::Set(-RICCONTROLFFKEY.mcgeomshift[0],-RICCONTROLFFKEY.mcgeomshift[1],-RICCONTROLFFKEY.mcgeomshift[2],0,0,0);
      RichAlignment::SetMirrorShift(0,0,0);
 ...
 mirror->add(new AMSgvolume("RICH MIRROR S3",
  ...
  rad2=radiator_box->add(new AMSgvolume("RICH VACUUM",
					0,
					"NAFB",
					"BOX",
					par,
  ...
  par[0]=full_naf_block_length/2;
  par[1]=par[0];
  par[2]=0.3/2; // Modified by A.Oliva (2016-02-08), original: 0.15/2 
  coo[0]=0;
  coo[1]=0;
  coo[2]=RICHDB::naf_height-RICHDB::rad_height/2+par[2];

  rad2->add(new AMSgvolume("RICH CARBON",
			   0,
			   "NAFT",
			   "BOX",
			   par,
  ...
  par[12]=full_naf_block_length/2;
  coo[0]=0;
  coo[1]=0;
