//  $Id: geant4.C,v 1.266 2024/05/28 13:17:34 nbelyaev Exp $
...
cout<<"  geant4i-I-visman closed"<<endl; 
#endif
#endif
}
...
/*          if(PostPV->GetName()=="NAF "){//enter NAF
            cout<<"NAF0="<<PostPoint->GetPosition().z()/cm<<endl;
          }
          if(PrePV->GetName()=="NAF "){//eixt NAF
            cout<<"NAF1="<<PostPoint->GetPosition().z()/cm<<endl;
          }
          if(PostPV->GetName()=="RAD "){//enter RAD
            cout<<"RAD0="<<PostPoint->GetPosition().z()/cm<<endl;
          }
          if(PrePV->GetName()=="RAD "){//eixt RAD
            cout<<"RAD1="<<PostPoint->GetPosition().z()/cm<<endl;
          }*/
...
const double AMSG4SteppingAction::facc_pl[23] = {
  158.92,  141.2, 86.1, 
  ...
  -142.792,
  -158.457,
   -73.62, -74.62//(21,22)at middle radiator of ALG(+-2.5/2.)/NAF(+-0.5/2.), will add RichAlignment::RichToAMS(local_position)
};
...
bool AMSG4SteppingAction::FillPrimaryInfo( const G4Step *Step){
  ...
  if( aTrack->GetTrackStatus()!=fAlive && aTrack->GetTrackStatus()!=fSuspend) ipl = 100;

  while( (pl<23) && (ipl<0) ){
    double facc_pz=facc_pl[pl];
    if(pl>=21&&pl<=22){//This is the middel of rich radiator ALG/NAF
      AMSPoint test_point(0,0,0);
      test_point=RichAlignment::RichToAMS(test_point);//current=-0.93 cm
      facc_pz+=test_point[2];
    }
    if(  ( (z_post < facc_pz*cm) && (z_pre > facc_pz*cm ) ) || //top-bottom
         ( (z_pre < facc_pz*cm) && (z_post > facc_pz*cm ) ) ){ //bot-top
