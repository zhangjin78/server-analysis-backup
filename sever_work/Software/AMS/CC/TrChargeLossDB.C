// $Id: TrChargeLossDB.C,v 1.11 2022/06/16 10:30:27 qyan Exp $
...
TrChargeLossTable** TrChargeLossDB::fTrChargeLossArray = 0;
...
void TrChargeLossDB::Init(bool force) {
  ...
  if (LoadDefaultTablesVer3()) printf("TrChargeLossDB::GetHead()-V TrChargeLossDBver3 correction loaded, %d tables in memory.\n",(int)fTrChargeLossMap.size());
  else                         printf("TrChargeLossDB::GetHead()-V TrChargeLossDBver3 correction NOT loaded.\n");
  if (LoadDefaultTablesVer4()) printf("TrChargeLossDB::GetHead()-V TrChargeLossDBver4 correction loaded, %d tables in memory.\n",(int)fTrChargeLossMap.size());
  else                         printf("TrChargeLossDB::GetHead()-V TrChargeLossDBver4 correction NOT loaded.\n");
  fInitDone = true;
} 
...
void TrChargeLossDB::Clear(Option_t* option) { 
  ...
  if (fTrChargeLossArray)
    delete [] fTrChargeLossArray;
...
bool TrChargeLossDB::AddTable(int index, TrChargeLossTable* table) {
  ...
  if (!fTrChargeLossArray) {
    fTrChargeLossArray = new TrChargeLossTable*[5000];
    for (int i = 0; i < 5000; ++i)
      fTrChargeLossArray[i] = 0;
  }
  fTrChargeLossArray[index] = table;
  return true;
}


TrChargeLossTable* TrChargeLossDB::GetTable(int index) {
  assert(index >= 0 && index < 5000);
  return fTrChargeLossArray[index];
...
double TrChargeLossDB::GetChargeLossCorrectedValue(int iside, int inter, double ip, double ia, double adc, int ver) { 
  ...
  for (int Z=1; Z<=CHARGELOSS_ZMAX; Z++) {
    ...
    double sqrt_mpv = sqrt(mpv);
    // no steps
    if ((Z-z_previous)/(sqrt_mpv-sqrt_mpv_previous)>10) continue;
    x[n] = sqrt_mpv;
    y[n] = 1.0*Z;
    n++;
    // previous storing 
    mpv_previous = mpv;
    sqrt_mpv_previous = sqrt_mpv;
    z_previous = Z;
  }
  // error message
  static int maxerr = 0;
...
bool TrChargeLossDB::LoadDefaultTablesVer0(char* dirname) {
  ...
  for (int iz=0; iz<14; iz++) {
    ...
    bool loadX = tableX->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver0/ChargeLossTable_N_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z));//the directory has been changed to v6.00
    if (loadX) AddTable(tableX,0,Z,0);
    // P-side
    TrChargeLossTable* tableY = new TrChargeLossTable();
    bool loadY = tableY->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver0/ChargeLossTable_P_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z));
...
bool TrChargeLossDB::LoadDefaultTablesVer1(char* dirname) {
  ...
  for (int Z=1; Z<=26; Z++) {
    ...
    bool load3S = table3S->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver1/ChargeLossTable_3S_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z));
    ...
    bool load1K = table1K->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver1/ChargeLossTable_1K_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z));
    ...
    bool load0K = table0K->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver1/ChargeLossTable_0K_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z));
...
bool TrChargeLossDB::LoadDefaultTablesVer2(char* dirname) {
  ...
  for (int i=0; i<4; i++) {
    // 3S-side
    TrChargeLossTable* table3S = new TrChargeLossTable();
    bool load3S = table3S->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver2/ChargeLossTableSmooth_3S_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
    if (load3S) TrChargeLossDB::GetHead()->AddTable(table3S,1,Z[i],2);
    else delete table3S;
    // 1K-side (B)
    TrChargeLossTable* table1KB = new TrChargeLossTable();
    bool load1KB = table1KB->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver2/ChargeLossTableSmooth_1K_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
    if (load1KB) TrChargeLossDB::GetHead()->AddTable(table1KB,2,Z[i],2);
    else delete table1KB;
    // 0K-side
    TrChargeLossTable* table0K = new TrChargeLossTable();
    bool load0K = table0K->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver2/ChargeLossTableSmooth_0K_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
    if (load0K) TrChargeLossDB::GetHead()->AddTable(table0K,3,Z[i],2);
    else delete table0K;
    // 1K-side (A) 
    TrChargeLossTable* table1KA = new TrChargeLossTable();
    bool load1KA = table1KA->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver2/ChargeLossTableSmooth_1KA_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
...
bool TrChargeLossDB::LoadDefaultTablesVer3(char* dirname) {
  TrChargeLossTable::fBadValue = 2;
  int pre = int(fTrChargeLossMap.size());
  int Z[4] = {3,5,6,8};
  for (int i=0; i<4; i++) {
    // 3S-side
    TrChargeLossTable* table3S = new TrChargeLossTable();
    bool load3S = table3S->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver3/ChargeLossTableSmooth_MC_3S_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
    if (load3S) TrChargeLossDB::GetHead()->AddTable(table3S,1,Z[i],3);
    else delete table3S;
  }
  return (int(fTrChargeLossMap.size())-pre>0);
}
...
bool TrChargeLossDB::LoadDefaultTablesVer4(char* dirname) {
  TrChargeLossTable::fBadValue = 0.001;
  int pre = int(fTrChargeLossMap.size());
  int Z[10] = {1,2,3,4,5,6,7,8,14,26}; 
  for (int i=0; i<10; i++) {
    // 3S-side
    TrChargeLossTable* table3S = new TrChargeLossTable();
    bool load3S = table3S->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver4/ChargeLossTable_MC.B1113_3S_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
    if (load3S) TrChargeLossDB::GetHead()->AddTable(table3S,1,Z[i],4);
    else delete table3S;
    // 1K-side (B)
    TrChargeLossTable* table1KB = new TrChargeLossTable();
    bool load1KB = table1KB->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver4/ChargeLossTable_MC.B1113_1KB_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
    if (load1KB) TrChargeLossDB::GetHead()->AddTable(table1KB,2,Z[i],4);
    else delete table1KB;
    // 0K-side
    TrChargeLossTable* table0K = new TrChargeLossTable();
    bool load0K = table0K->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver4/ChargeLossTable_MC.B1113_0K_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
    if (load0K) TrChargeLossDB::GetHead()->AddTable(table0K,3,Z[i],4);
    else delete table0K;
    // 1K-side (A) 
    TrChargeLossTable* table1KA = new TrChargeLossTable();
    bool load1KA = table1KA->InitTableFromTxtFile(Form("%s/%s/TrChargeLossDBver4/ChargeLossTable_MC.B1113_1KA_Z%02d.txt",dirname,AMSCommonsI::getversion(),Z[i]));
    if (load1KA) TrChargeLossDB::GetHead()->AddTable(table1KA,4,Z[i],4);
    else delete table1KA;
  }
  return (int(fTrChargeLossMap.size())-pre>0);
}
...
int TrChargeLossTable::CreateIndex(int type, int z, int ver) { 
  TrChargeLossTable::fBadValue = (ver<4) ? 2 : 0.001;
  return type + z*10 + ver*1000; 
}
...
int TrChargeLossTable::CreateType(int iside, int inter, int ver) {
  ...
  if ( (ver>=2)&&(inter==1)&&( (k7==1)||(k7==3) ) ) type = 4; // ver2,3: 1KA
  return type;
}
