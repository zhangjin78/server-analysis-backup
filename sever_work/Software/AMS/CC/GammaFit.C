...
Double_t GammaFit::HeFunc(Double_t *xp, Double_t *par)
{
  Double_t lx = TMath::Log10(xp[0]);
  Double_t r  = par[0];

  Double_t P0[4] = { 0.004, 0.0267, -2.67, 1450 };
  Double_t P1[4] = {   700,    220,  1.02,  120 };
  Double_t P5[4] = {     0, 0.0236,  2.57, 1400 };
  Double_t P6[4] = {  1700,    690,  2.94, 2500 };

  Double_t p0 = P0[0]+P0[1]*(1+TMath::Erf(P0[2]*TMath::Log10(r/P0[3])));
  Double_t p1 = P1[0]+P1[1]*(1+TMath::Erf(P1[2]*TMath::Log10(r/P1[3])));
  Double_t p5 = P5[0]+P5[1]*(1+TMath::Erf(P5[2]*TMath::Log10(r/P5[3])));
  Double_t p6 = P6[0]+P6[1]*(1+TMath::Erf(P6[2]*TMath::Log10(r/P6[3])));

  Double_t p2 = TMath::Log10(1.60);
  Double_t p3 = TMath::Log10(0.55);
  Double_t p4 = TMath::Log10(1.65*p1);
  Double_t p7 = TMath::Log10(1.90);
  ...
  if (lx < p4) g = p0*TMath::Exp(-s*s);
  else {
    Double_t ss = (p4-p1)/p2;
    Double_t q1 = 2*ss/p2;
    Double_t q0 = p0*TMath::Exp(-ss*ss)/TMath::Exp(-q1*p4);
    g = q0*TMath::Exp(-q1*lx);
  }

  Double_t t = (lx-p6)/p7;
  return g+p5*TMath::Exp(-t*t);
}

Double_t GammaFit::HeFit(Double_t *len, Double_t *amp, Int_t &nh,
			 Double_t &csq, Double_t &dr)
{
  enum { Nb = 20, Nl = 20 };
  ...
  if (!b) {
    b = new Double_t[Nb+1];
    for (Int_t i = 0; i <= Nb; i++) b[i] = TMath::Power(10, 2+(i+3)*0.1);
  }

  static TF1 *f = 0;
  if (!f) f = new TF1("f", GammaFit::HeFunc, 100, 2e4, 1);

  nh = 0;
  Double_t c[Nb], e[Nb], a[Nb];
  for (Int_t i = 0; i < Nb; i++) c[i] = 0;
  for (Int_t i = 0; i < Nl; i++) {
    if (len[i] < 0.3) continue;
    for (Int_t j = 1; j < Nb; j++)
      if (amp[i]/len[i] < b[j]) { c[j-1]++; nh++; break; }
  }
  if (nh < 2) return -1;
  ...
  for (Int_t i = 0; i < Nb; i++) {
    e[i]  = ((c[i] > 0) ? TMath::Sqrt(c[i]) : 1)/s;
    c[i] /= s;
    a[i]  = TMath::Sqrt(b[i]*b[i+1]);
  }
  ...
  for (Int_t i = 0; i < 20; i++) {
    Double_t x = (i+0.5)*0.5e3;
    f->SetParameter(0, x);

    Double_t cs = 0;
    for (Int_t j = 0; j < Nb; j++) {
      Double_t d = (c[j]-f->Eval(a[j]))/e[j];
      cs += d*d;
    }
    c1 = c2; c2 = c3; c3 = cs;
    x1 = x2; x2 = x3; x3 = x;
  ...
  if (c1 < c2 && c2 < c3) return x1;

  if (c1 > c2 && c2 < c3) {
  //  p1*(x1-x2)*(x2-x3)+p2*(x1*x1-x2*x2)*(x2-x3) = (c1-c2)*(x2-x3);
  //  p1*(x1-x2)*(x2-x3)+p2*(x2*x2-x3*x3)*(x1-x2) = (c2-c3)*(x1-x2);

    Double_t p2 = ((c1-c2)*(x2-x3)-(c2-c3)*(x1-x2))/
	          ((x1*x1-x2*x2)*(x2-x3)-(x2*x2-x3*x3)*(x1-x2));
    Double_t p1 = ((c1-c2)*(x2-x3)-p2*(x1*x1-x2*x2)*(x2-x3))/
                  ((x1-x2)*(x2-x3));
    Double_t p0 = c1-p1*x1-p2*x1*x1;

    Double_t pm = -p1/2/p2;
    dr  =  TMath::Sqrt(1/p2);
    csq = (p0+p1*pm+p2*pm*pm)/(nh-1);
    return pm;
  }
