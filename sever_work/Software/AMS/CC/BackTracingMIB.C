...
//------------------- file -----------------------------
        ...
        char parmod_File[300]; 
         sprintf(parmod_File,"%s%s", AMSDATADIR.amsdatadir, "parmod_new.dat");//the directory has been changed to v6.00
...
//cout <<" BackTracingMIB::LoadParmodFile == yy-doi-hh last "<<yearL1<<"-"<<doyL1<<"-"<<hourL1<< " --> UnixL1 "<<UnixL1<<endl;
	...
	//if(ext==2){
        char OMNI_File[300];
         sprintf(OMNI_File,"%s%s", AMSDATADIR.amsdatadir, "OMNI_W1_W6.dat");//the directory has been changed to v6.00
