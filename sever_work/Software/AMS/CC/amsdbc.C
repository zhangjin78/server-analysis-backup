//  $Id: amsdbc.C,v 1.64 2025/01/31 11:34:21 nbelyaev Exp $
...
geant AMSDBc::ams_size_l0[3]={440.,440.,440.};
...
unsigned int AMSDBc::convert4(char *c, unsigned int nw){
  if(AMSDBc::BigEndian){
    unsigned char tmp;
...
unsigned int AMSDBc::convert2(char *c, unsigned int nw,bool force){
  if(AMSDBc::BigEndian || force){
    unsigned char tmp;
    unsigned char *pc = (unsigned char*)(c);
    for(unsigned int i=0;i<nw;i++){
      tmp=*pc;
...
unsigned int AMSDBc::convert8(char *c, unsigned int nw){
  if(AMSDBc::BigEndian){
    unsigned char tmp;
    unsigned char *pc = (unsigned char*)(c);
    for(unsigned int i=0;i<nw;i++){
