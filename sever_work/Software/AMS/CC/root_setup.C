//  $Id: root_setup.C,v 1.207 2024/04/11 08:04:15 aegorov2 Exp $
...
int AMSSetupR::SlowControlR::GetData(const char * elementname, unsigned int time, float frac,  vector<double> &value,int method , const char *nodename,int dt, int st){
//work around vector<float> problem in root5 cint
vector<float>vf;
value.clear();
int ret=GetData(elementname, time, frac,  vf,method , nodename, dt, st);
for(unsigned int i=0;i<vf.size();i++)value.push_back(vf[i]);
return ret;
}
...
//  if(i->second.fTable.size()>=1)cout<<"first="<<i->second.fTable.begin()->first<<","<<i->second.fTable.rbegin()->first<<endl;
...
return retcode;
 ...
 void AMSSetupR::CorrectBugs(unsigned int version, unsigned int run){

// tle bug
      if(run>1366721606 && run<1371032409 && version<653 )fISSData.clear();

// gps.csv bug
//
//
   if(version<1098 && run>=1449415456 && run<=1480199980)fJGCR.clear();
  }
 ...
 void AMSSetupR::Init(TTree *tree){
  ...
  if (tree == 0 ) return;
#ifdef VERSION6
if(AMSEventR::ProcessSetup==0)return;
#endif
  _Head=this;
...
//sprintf(tmps,"/%u",mktime(tmp)-3600-tzz);
	...
	if(nptr>0)free(namelist);
        if(fHeader.FEventTime==fHeader.LEventTime){//special design for the standalone by QY
          if(mtv.size()>=1){    
            unsigned int pend=0;
            map<unsigned int, trio> mtvn;
            map<unsigned int, trio>::iterator itpre;
            for(map<unsigned int, trio>::iterator it=mtv.begin();it!=mtv.end();it++){
              if((it->second).t2<=(it->second).t1||(it->second).t2<=pend){//remove wrong data
                cerr<<"AMSSetupR::getSlowControlFilePath-E-WrongFile="<<(it->second).filename<<endl;
                continue;
              }
              unsigned int tn=it->first;
              if(pend!=0&&pend>tn){tn=(tn+pend)/2;(itpre->second).t2=tn;}//overlap using current begin and previous end averaged
              mtvn[tn]=it->second;
              itpre=mtvn.find(tn);
              (itpre->second).t1=tn;
              pend=(it->second).t2;
            }
            map<unsigned int, trio>::iterator it=mtvn.upper_bound(fHeader.LEventTime);//t1 > fHeader.LEventTime
            if(it!=mtvn.begin())it--;
            if(mtvn.size()>=1&&(it->second).t1<=fHeader.FEventTime && (it->second).t2>=fHeader.LEventTime){
              fHeader.FEventTime=(it->second).t1;
              fHeader.LEventTime=(it->second).t2;
              slc=(it->second).filename;
              cout <<"AMSSetupR::getSlowControlFilePath-I-FoundFile "<<slc<<" t1="<<t1<<" reset-range="<<fHeader.FEventTime<<","<<fHeader.LEventTime<<endl; 
            }
          }
          if(fHeader.LEventTime<=fHeader.FEventTime){//time not found
            cerr<<"AMSSetupR::getSlowControlFilePath-E-UnableToFindfile t1="<<t1<<endl;
...
if(verb)cout <<"AMSSetupR::LoadSlowcontrolDB-I-fillSlowControl "<<file<<endl;
    ...
    cerr<<"AMSSetupR::LoadSlowcontrolDB-I-Set 'AMSSetupR::SlowControlR::ReadFromExternalFile=false;' to suppress this message   "<<endl;
    return false;
  }
  return true;
}
	...
	char stime[80],namei[80], line1i[80], line2i[80];
			...
			ifbin.getline(line2i,75);
                        if(!ifbin.eof()  ){
                          ...
                          if(tme>t2 && strstr((const char *)namei,"ISS (ZA"))break;
                        }
                       }
                       ifbin.close();
...
AMSSetupR::Tsy05Cutoff::Tsy05Cutoff(){
   TFile *f=TFile::Open(Form("%s/altec/RTI/TS05cutoffMaps01OK.root",getenv("AMSDataDir")));
   const int nt=5;
   const char *histn[nt]={
     "TS05cutoffTOT", 
     "TS05cutoff2011",
   ...
   unsigned int tm[nt][2]={
      {0,0},//default 2011-07,2013-06 
      {1309478400,1325375999},//2011-07,2011-12
      {1325376000,1341100799},//2012-01,2012-06
      {1341100800,1356998399},//2012-07,2012-12
      {1356998400,1372636799},//2013-01,2013-06
   };
   const int nfv=4;
   int fv[nfv]={25,30,35,40};
   for(int ifv=0;ifv<4;ifv++){
      for(int it=0;it<nt;it++){
        pair<unsigned int,unsigned int>tp(tm[it][0],tm[it][1]);
        histcf[ifv][0][tp]=(TH2D *)f->Get(Form("%s_MaxSec%d",histn[it],fv[ifv]))->Clone();
        histcf[ifv][1][tp]=(TH2D *)f->Get(Form("%s_MinPri%d",histn[it],fv[ifv]))->Clone();
     }
   }
}

TH2D *AMSSetupR::Tsy05Cutoff::gethistcf(int &stat, unsigned int xtime,int ifv, int icf){
   hist_i iter=histcf[ifv][icf].begin();
   TH2D *hn=(*iter).second;
   stat=0;
   if(xtime!=0){
      iter++;
      for(;iter!=histcf[ifv][icf].end();iter++){
         if     (xtime<(*iter).first.first){hn=(*iter).second; stat=1; break;}//LowEdge
         else if(xtime>=(*iter).first.first&&xtime<=(*iter).first.second){hn=(*iter).second; stat=0; break;}
         else   {hn=(*iter).second; stat=1;}
      }
   }
   return hn;
}

int AMSSetupR::Tsy05Cutoff::getcutoff(float cfn[4][2],int icf,unsigned int xtime,float theta,float phi){
    int stat=0;
    float Long = phi*TMath::RadToDeg();
    float Lat  = theta*TMath::RadToDeg();
    if(Long >180.)Long-=360.;
    for(int ifv=0;ifv<4;ifv++){
      int nstat=0;
      TH2D *hcf=gethistcf(nstat,xtime,ifv,icf);
      int ibx=hcf->GetXaxis()->FindBin(Long);
      int iby=hcf->GetYaxis()->FindBin(Lat);
      double cutoff=hcf->GetBinContent(ibx,iby);
      if     (cutoff<=0)stat=-2;
      else if(nstat!=0)stat=nstat;      
      cfn[ifv][0]=cfn[ifv][1]=cutoff;
    }
    return stat;
}
...
///---RTI function
//int AMSSetupR::RTI::Version=700;
int AMSSetupR::RTI::Version=800;
int AMSSetupR::RTI::Loadopt=0;

int AMSSetupR::RTI::UseLatest(int pass){

  if(pass>=8)RTI::Version=800;//pass8+version8
  else       RTI::Version=700;//pass7+version7
  static int vrti=-1;
  if(RTI::Version!=vrti){
     cout<<"AMSSetupR::RTI::UseLatest Pass"<<pass<<" Version="<<RTI::Version<<endl;
     if(pass>=8)cout<<"AMSSetupR::RTI::UseLatest is now using latest pass8-RTI Version"<<endl;
     else       cout<<"AMSSetupR::RTI::UseLatest is now using pass7-RTI Version"<<endl;
     vrti=RTI::Version;
  }
  return RTI::Version;
}
...
AMSSetupR::Tsy05Cutoff * AMSSetupR::RTI::tsy05cfhead=0;

AMSSetupR::Tsy05Cutoff * AMSSetupR::RTI::gettsy05cf(){

   if(tsy05cfhead==0)tsy05cfhead=new Tsy05Cutoff();
   return tsy05cfhead;
}
...
const char *AMSSetupR::RTI::GetCite(unsigned int run,int &index){

   return AMSEventR::GetCite(run,index);
}
...
int AMSSetupR::RTI::getcutoff(float cfn[4][2],int icf){
    
    for(int ifv=0;ifv<4;ifv++){
      for(int ipn=0;ipn<2;ipn++){
        if     (icf%10==0)cfn[ifv][ipn]=cf[ifv][ipn];
        else if(icf%10==1)cfn[ifv][ipn]=cfi[ifv][ipn];
        else              cfn[ifv][ipn]=0;
      }
    }
    double time=(utctime[0]+utctime[1])/2.;
    if(icf%10==2){
       ...
       for(int ifv=0;ifv<4;ifv++){
          ...
          AMSEventR::GetMaxIGRFCutoff(fv,cfv,int(time),0,igrfvers);
          cfn[ifv][0]=cfv[0]; cfn[ifv][1]=cfv[1];
      }
    }
    else if(icf%10==3||icf%10==4){
       return gettsy05cf()->getcutoff(cfn,icf%10-3,int(time),theta,phi);
    }
    return 0;
}
...
//---NewV
 bool isv7p7=(RTI::Version==700);
 bool isv7p7t=(RTI::Version==711);
 bool isv7p7std=(RTI::Version==720);
 bool isv7p7tstd=(RTI::Version==731);
 if     (isv7p7)    AMSISSlocal+="V7_20180420_P7/";//isv7p7-700
 else if(isv7p7t)   AMSISSlocal+="V7_20190509_P7T/";//isv7p7t-TTCSOFF-711
 else if(isv7p7std) AMSISSlocal+="V7_20180420_P7STD/";//isv7p7std-STD-720
 else if(isv7p7tstd)AMSISSlocal+="V7_20190509_P7TSTD/";//isv7p7tstd-TTCSOFFSTD-731
 else               AMSISSlocal+="V8_20221225_P8/";//isv8p8-800
...
//---Input
         ...
         if(a.run!=0){//missing second
           if(RTI::Version>700||a.run>=1599484451)fbin>>a.runtype;
           fbin>>a.evno;
           fbin>>a.evnol;
           ...
           for(int ifv=0;ifv<4;ifv++){//cutoffi
             for(int ipn=0;ipn<2;ipn++)fbin>>a.cfi[ifv][ipn];
           }
           for(int ifv=0;ifv<2;ifv++){
             for(int ipn=0;ipn<2;ipn++)fbin>>a.cfl[ifv][ipn];
           }
...
//---
 ...
 //---use IGRF table to fill cfi
           if(RTI::Loadopt%10==2)a.getcutoff(a.cfi,RTI::Loadopt%100); 
//---
         }
         else continue;
...
int AMSSetupR::LoadAMSSTK(unsigned int t1, unsigned int t2){
 ...
 AMSISSlocal+="STKM/";
{
 const char * AMSSTK=getenv("AMSSTK");
   if (AMSSTK && strlen(AMSSTK))
    AMSISSlocal=AMSSTK;
}
 AMSISS=AMSISSlocal.c_str();
if(t1>t2){
...
//   change  to fpatb3
        fname=AMSISS;
        char utmp[255];
        sprintf(utmp,"/%s%u%03u",fpatb3,yc,dc);
        fname+=utmp;
        fname+=fpate;
        fbin.close();
        fbin.clear();
...
// change 24H to 24h
     ...
     if(fbin){
      ...
      while(fbin.good() && !fbin.eof()){
        ...
        if(strstr(line,"GPS")){
          gps=true;
          cerr<<"LoadISSAtt-W-GPSTimeFormatFound"<<endl;
        }
         ...
         if(pch){
          ...
          if(gps)tf-=AMSEventR::gpsdiff(tf-tzd);
          pch=strtok(NULL,",");
          char tm1[80];
          sprintf(tm1,".%s",pch);
	  ...
	  if(a.Roll==a.Roll && a.Pitch==a.Pitch && a.Yaw==a.Yaw)fISSAtt.insert(make_pair(tc,a));
          else continue;
          if(tc>=t1 && tc<=t2){
           if(abs(bfound)!=2){
               fISSAtt.clear();
...
#pragma omp threadprivate (time)
  ...
  for(ISSData_i i=fISSData.begin();i!=fISSData.end();i++){
    if(fabs(i->first-xtime)<fabs(tl-xtime) && strstr((const char *)i->second.Name,"ISS (ZA"))tl=i->first;
  }
  if(tl!=time && tl){
#ifdef __ROOTSHAREDLIBRARY__
    resetmaybe=true;
...
// change 24H to 24h
     ...
     if(fbin){
       bool gps=false;
        ...
        if(strstr(line,"GPS")){
          gps=true;
          cerr<<"LoadISSINTL-W-GPSTimeFormatFound"<<endl;
        }
        else if(isdigit(line[0])){
         char *pch;
         pch=strtok(line,".");
         ...
         if(pch){
          ...
          if(!pch)continue;
          a.Roll=atof(pch)*3.14159267/180;
	  if(a.Roll==a.Roll && a.Pitch==a.Pitch && a.Yaw==a.Yaw)fISSINTL.insert(make_pair(tc,a));
          else continue;
          if(tc>=t1 && tc<=t2){
           if(abs(bfound)!=2){
               fISSINTL.clear();
...
const char fpatb2[]="GTOD/ISS_SV_ITRF-";
const char fpate[]="-24H.csv";
const char fpate2[]="-24h.csv";
...
// change 24h back
     ...
     if(fbin){
      ...
      while(fbin.good() && !fbin.eof()){
        ...
        if(strstr(line,"GPS")){
          gps=true; 
          cerr<<"LoadISSGTOD-W-GPSTimeFormatFound"<<endl;
        }
        else if(isdigit(line[0])){
         char *pch;
         pch=strtok(line,".");
         ...
         if(pch){
          ...
          time_t tf=mktime(&tmf)+tzd;
          if(gps)tf-=AMSEventR::gpsdiff(tf-tzd);
          else if(tf-tzd>1372636800 && tf-tzd<  1527292800){
           gps=true;
           cerr<<"LoadISSGTOD-E-GPSTimeFormatForced"<<endl;
           tf-=AMSEventR::gpsdiff(tf-tzd);
          }
          pch=strtok(NULL,",");
          char tm1[80];
          sprintf(tm1,".%s",pch);
          ...
          pch=strtok(NULL,",");
          if(!pch)continue;
          if(!isdigit(pch[0]) && pch[0]!='-' && pch[0]!='.' && pch[0]!=' ')continue;
          a.r=atof(pch)*100;
          //cout <<" alpha "<<a.x<<" "<<tf<<" "<<tc<<endl;
          pch=strtok(NULL,",");
          if(!pch)continue;
...
int AMSSetupR::LoadRichConfig(unsigned int run){
  ...
  return false;
   ...
   sprintf(defaultDir,"%sRichDefaultPMTCalib",AMSDATADIR.amsdatabase);
