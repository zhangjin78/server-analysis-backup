// $Id: MagField.C,v 1.26 2016/06/02 06:23:22 nzimmerm Exp $
...
int MagField::Read(const char *fname)
{
  ...
  AMSDBc::convert4(cheader,_header_size);
  fin.close();
...
//     }
  ...
  bool check = true;
  for (int i = 0; i < _nx-1; i++) if (x[i] >= x[i+1]) {
        check = false;
        break;
  }
...
void BCorr::Set(unsigned int t, float v)
{
  _m[t] = v;
}

float BCorr::Get(unsigned int t) const
{
  Ic i1 = _m.upper_bound(t);
  if (i1 == _m.begin()) return -2;
  if (i1 == _m.end())   return -3;

  Ic i0 = i1; i0--;
  float dt0 = t-i0->first;
  float dt1 = i1->first-t;
  if (dt0 < 0 || dt1 < 0) return -4;

  return (i0->second*dt1+i1->second*dt0)/(dt0+dt1);
}

#include "TString.h"

void BCorr::Print(Option_t *opt) const
{
  cout << "N= " << GetN() << endl;
  for (Ic i = _m.begin(); i != _m.end(); i++)
    cout << Form("%d %7.5f", i->first, i->second) << endl;
}

#include "TFile.h"

BCorr *BCorr::Load(const char *fname)
{
  return Load(TFile::Open(fname));
}

BCorr *BCorr::Load(TFile *f)
{
  if (!f) return 0;

  BCorr *b = (BCorr *)f->Get("BCorr");
  if (b) _h = b;
  return b;
}
