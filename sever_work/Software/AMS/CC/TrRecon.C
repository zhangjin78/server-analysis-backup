/// $Id: TrRecon.C,v 1.232 2025/01/31 11:34:21 nbelyaev Exp $ 
...
///\date  2015/10/08 SH  Z.Qu's bug fix (activation by TRCLFFKEY.UseZQFix)
///\date  2018/05/12 QY  Pass7 TrCluster and TrRecHit reconstruction
///\date  2022/06/04 QY  Pass8 TrCluster and TrRecHit reconstruction
///
...
void TrReconPar::SetParFromDataCards()
{
  ...
  TrTrackR::AdvancedFitBitsHalf  = TRCLFFKEY.AdvancedFitFlagHalf;
  TrTrackR::AdvancedFitBitsExtAll= TRCLFFKEY.AdvancedFitFlagExtAll;
}
...
int TrRecon::Build(int iflag, int rebuild, int hist)
{
  ...
  int nmaxcls=lowdt? RecPar.MaxNtrCls_ldt: RecPar.MaxNtrCls;
  //////////////////// TrCluster reconstruction ////////////////////
  if (flag%10 >= 1 && (TRCLFFKEY.KeepBestTrClHits!=0 || nraw < RecPar.MaxNrawCls) ) {
#ifndef __ROOTSHAREDLIBRARY__
    AMSgObj::BookTimer.start("TrCluster");
#endif
    ncls = BuildTrClusters(rebuild, flag);
#ifndef __ROOTSHAREDLIBRARY__
    ...
    if (TRCLFFKEY.ReorderTrClusters>=1 || (TRCLFFKEY.KeepBestTrClHits!=0 && ncls>=nmaxcls) ){
#ifndef __ROOTSHAREDLIBRARY__
      AMSgObj::BookTimer.start("TrClusterReorder");
#endif
      ncls=ReorderTrClusters(nmaxcls);
#ifndef __ROOTSHAREDLIBRARY__
      AMSgObj::BookTimer.start("TrClusterReorder");
#endif
    }
  ...
  int nmaxhits=RecPar.MaxNtrHit;
  //////////////////// TrRecHit reconstruction ////////////////////
  if (flag%100 >= 10 && ncls >  0  && ncls<nmaxcls ){
#ifndef __ROOTSHAREDLIBRARY__
    ...
    //////////////////// TrRecHit reordering ////////////////////
    if (TRCLFFKEY.ReorderTrRecHits>=1 || (TRCLFFKEY.KeepBestTrClHits!=0 && nhit>=nmaxhits)){
#ifndef __ROOTSHAREDLIBRARY__
      AMSgObj::BookTimer.start("TrRecHitReorder");
#endif
      int ordertype = (TRCLFFKEY.KeepBestTrClHits!=0)? 4: TRCLFFKEY.ReorderTrRecHits;
      nhit = ReorderTrRecHits(nmaxhits,ordertype);
#ifndef __ROOTSHAREDLIBRARY__
  ...
  if ( (flag%1000>=100)&&(nhit>0) ) {
    ...
    if (simple) {
      if (TRCLFFKEY.BuildTracksSimpleChargeSeedTag==1) { 
        if (GetNHitsWithTag(TrRecHitR::ZSEED)<nmaxhits) 
        ...
        if (nhit<nmaxhits) ntrk += BuildTrTracksSimple(rebuild);
      }
    } 
...
int TrRecon::BuildTrClusters(int rebuild, int flag) { 
  ...
  while (lad!=RawMap.LadMap.end()){
    ...
    int TkId = lad->first;
    flag = flag%10;
    double dshape=-1;//normal
    if     (flag==2)dshape=0;//normal+vertex
    else if(flag==3)dshape=0.5;//normal+vertex2
    ncls += BuildTrClustersInSubBuffer(TkId,0,640,0,dshape);      // S
    ...
    if ( ll->IsK7()) { 
      ncls += BuildTrClustersInSubBuffer(TkId,640,1024,1,dshape); // K7
    }
    else { 
      ncls += BuildTrClustersInSubBuffer(TkId,640,832,0,dshape);  // K5 - sensor1
      ncls += BuildTrClustersInSubBuffer(TkId,832,1024,0,dshape); // K5 - sensor2 
    }
    lad++;
  }
...
int TrRecon::ReorderTrClusters(int nmaxcls) {//QY

  VCon* cont = GetVCon()->GetCont("AMSTrCluster");

  int ncl = cont->getnelem();
  vector<pair<double, TrClusterR*> > clxy[2];

  //----set inform
  for(int icl=0; icl<ncl; icl++){
    TrClusterR* cl = (TrClusterR*) cont->getelem(icl);
    double sig = cl->GetTotSignal();
    if(cl->checkstatus(TrClusterR::ZSEED))sig=sig*100;//set higher  priority for ZSEED
    clxy[cl->GetSide()].push_back(make_pair(sig,cl));
  }
 
  //----sort + cleand and reoder + backup though fastest pointer 
  for(int ixy=0;ixy<2;ixy++)std::sort(clxy[ixy].begin(),clxy[ixy].end(),CompSignal<TrClusterR>);//Highest sig to lowest
  vector<TrClusterR> cls; 
  unsigned int ii=0;
  while(ii<clxy[0].size()||ii<clxy[1].size()){
   for(int ixy=1;ixy>=0;ixy--){//Highest Y-X-Y-X... make sure NY~NX
      if(TRCLFFKEY.KeepBestTrClHits!=0 && int(cls.size())>=nmaxcls-1)break;//the rest would erase
      if(ii>=clxy[ixy].size())continue;
      cls.push_back(*(clxy[ixy].at(ii).second)); 
    }
    ii++;
  }

  //---then copy to Vcon
  cont->eraseC();//would delete all
  for(unsigned int icl=0; icl<cls.size(); icl++){
#ifndef __ROOTSHAREDLIBRARY__ 
    cont->addnext(new AMSTrCluster(cls[icl]));
#else
    cont->addnext(new TrClusterR(cls[icl])); 
#endif
  }
...
int TrRecon::BuildTrClustersInSubBuffer(int tkid, int first, int last, int cyclicity, double dShapeCut){
  int   ntrclusters = 0;
  ...
  int nseeds = BuildSeedListInSubBuffer(tkid,first,last,cyclicity,dShapeCut);
 
  VCon* cont=GetVCon()->GetCont("AMSTrCluster");
  for (int ss=0; ss<nseeds; ss++) {
    int seedaddressr = _seedaddresses.at(ss);
    int seedaddress  = abs(seedaddressr);//negative address indicate vertex Cluster
...
#ifndef __ROOTSHAREDLIBRARY__
    AMSTrCluster *tcl = new AMSTrCluster(tkid,side,leftaddress,length,seedind,sigg,0);
#else
    TrClusterR *tcl = new TrClusterR(tkid,side,leftaddress,length,seedind,sigg,0);
    if (!tcl->Check(1)) tcl->Print(10);
#endif
    if (seedaddressr<0) tcl->setstatus(TrClusterR::VERTEX);
    if(cont)cont->addnext(tcl);    
    ntrclusters++;
  }
  if(cont) delete cont; 
  return ntrclusters;
...
int TrRecon::BuildSeedListInSubBuffer(int tkid, int first, int last, int cyclicity, double dShapeCut) {
  int lay=abs(tkid)/100;
  _seedaddresses.clear();
  int side    = (first>639) ? 0 : 1;  // 0:n(X) 1:p(Y)
  int address = 0;
  int badstat = 0x8000; // (0x8000) user defined bad region channel
  int noisestat = 0x8;
  for (int ii=first; ii<last; ii++) {
    // 1. the strip has to be well defined
    // 2. a strip with badstatus can't be a seed strip
    // 3. the seed must exceed the ThrSeed S/N threshold
    if (  (_sigbuf[ii]<1.e-6)||((_stabuf[ii]&badstat)!=0)||( (_adcbuf[ii]/_sigbuf[ii])<RecPar.ThrSeed[side][lay-1] )  ) continue;
    ...
    if( ii<(last-1)&&(_adcbuf[ii+1]>_adcbuf[ii]))
      if ((_sigbuf[ii+1]>0.)&&((_stabuf[ii+1]&badstat)==0)&&( (_adcbuf[ii+1]/_sigbuf[ii+1])>=RecPar.ThrSeed[side][lay-1]))
	continue;
    
    // 5. the seed has to be the local maximum among 2*SeedDist+1 good strips
    int localmax[2]={2,2};
    double amin[2]={100.,100.};
    for(int irl=0; irl<2; irl++){//right/left
      for(int jj=1; jj<=RecPar.SeedDist[side]; jj++){//strip
        int jjrl=(irl==0)? ii+jj: ii-jj;
        address = GetAddressInSubBuffer(jjrl,first,last,cyclicity);
        if (address<0)continue;
        if ((_adcbuf[ii]<=_adcbuf[address])&&((_stabuf[address]&badstat)==0)) {
          bool noisejj=((jj>=2) && (_stabuf[address]>0) && (_stabuf[address]&noisestat));
          if(noisejj){//further judgement with Cluster shape
            int jjrl2=(irl==0)? jjrl-1: jjrl+1;
            int address2 = GetAddressInSubBuffer(jjrl2,first,last,cyclicity);//find clear split
            noisejj=((address2<0) || (_adcbuf[ii]/2.>_adcbuf[address2]));//ii+jj-1 is the real minimal
          }
          if(!noisejj){localmax[irl]=0; break;}
          else localmax[irl]=1;
        }
        else if(((_stabuf[address]&badstat)==0)&&localmax[irl]==2){
          if(_sigbuf[address]>=1.e-6&&_adcbuf[address]/_sigbuf[address]<=RecPar.ThrNeig[side])amin[irl]=0;//small signal truncation
          else if(_adcbuf[address]/_adcbuf[ii]<amin[irl])amin[irl]=_adcbuf[address]/_adcbuf[ii];
        }
      }
    }
    // A SEED IS FOUND
    if(localmax[0]>=1&&localmax[1]>=1){
      int seedaddress = ii;
      _seedaddresses.push_back(seedaddress);
    }
    else if(dShapeCut>=0&&((localmax[0]>=1&&amin[1]<=dShapeCut)||(amin[0]<=dShapeCut&&localmax[1]>=1))){//weak, vertex like
      int seedaddress = -ii;
      _seedaddresses.push_back(seedaddress);//negative address indicates vertex weak seed 
    }
  }
...
int TrRecon::GetBoundariesInSubBuffer(int index, int first, int last, int cyclicity) {
  ...
  int seedaddress  = abs(_seedaddresses.at(index));
  ...
  int left  = (index<=0) ? first-1 : abs(_seedaddresses.at(index-1));
  if ( (cyclicity!=0)&&(index<=0) ) left = abs(_seedaddresses.at(nseeds-1));
  int right = (index>=(nseeds-1)) ? last : abs(_seedaddresses.at(index+1));
  if ((cyclicity!=0)&&(index>=(nseeds-1))) right =abs(_seedaddresses.at(0));
...
int TrRecon::BuildTrRecHits(int rebuild) {
  ...
  for (ITClusterMap lad = _ClusterTkIdMap.begin(); lad != _ClusterTkIdMap.end(); lad++) {
    ...
    for (int iy=0; iy<ny; iy++) {
      for (int ix=0; ix<nx; ix++) {
...
#endif
        if (clX->checkstatus(TrClusterR::VERTEX) || clY->checkstatus(TrClusterR::VERTEX)) hit->setstatus(TrRecHitR::VERTEX);
        ...
        bool iszseedcl=(clX->checkstatus(TrClusterR::ZSEED) && clY->checkstatus(TrClusterR::ZSEED));
        // bad association
	if (prob<TRCLFFKEY.CorrelationProbThr && !iszseedcl) { 
          delete hit;
	  continue;  
	}
        // compatibility between hit clusters and charge seeds in status
        if ( (TRCLFFKEY.ChargeSeedTagActive!=0)&&(CompatibilityWithChargeSeed(hit)) ){
             if(iszseedcl)hit->setstatus(TrRecHitR::CSEED);
             hit->setstatus(TrRecHitR::ZSEED);
             if((TRCLFFKEY.ClusterCofGOpt%10)>=2){//QY
               clY->SetQtrk(_htmq[0]);//Use X to linearize Y
               clX->SetQtrk(_htmq[1]);//Use Y to linearize X
               hit->BuildCoordinate();
             }
         }
        // eliminate hit based on charge seed (high-z clean rec.)        
        if ( (TRCLFFKEY.ChargeSeedSelActive!=0)&&(!CompatibilityWithChargeSeed(hit)) && (!TRCLFFKEY.KeepBestTrClHits) ) { 
          delete hit; 
          continue; 
        }
        // add
  ...
  for (ITClusterMap lad = _ClusterTkIdMap.begin(); lad != _ClusterTkIdMap.end(); lad++) {
    ...
    for (int iy=0; iy<ny; iy++) {
...
#endif
      if (clY->checkstatus(TrClusterR::VERTEX)) hit->setstatus(TrRecHitR::VERTEX); 
      bool iszseedcl=clY->checkstatus(TrClusterR::ZSEED);
      // compatibility between hit clusters and charge seeds in status
      if ( (TRCLFFKEY.ChargeSeedTagActive!=0)&&(CompatibilityWithChargeSeed(clY)) ) {
         if(iszseedcl)hit->setstatus(TrRecHitR::CSEED);
         hit->setstatus(TrRecHitR::ZSEED);
         if((TRCLFFKEY.ClusterCofGOpt%10)>=2){//QY
           clY->SetQtrk(_htmq[0]);//Use X to linearize Y
      ...
      // eliminate hit based on charge seed (high-z clean rec.)
      if ( (TRCLFFKEY.ChargeSeedSelActive!=0)&&(!CompatibilityWithChargeSeed(clY)) && (!TRCLFFKEY.KeepBestTrClHits) ) { 
        delete hit; 
        continue; 
      }
      // add
...
int TrRecon::ReorderTrRecHits(int nmaxhits, int type) {//QY
  if (type<=0) return -1;
  VCon* cont = GetVCon()->GetCont("AMSTrRecHit");
  if (!cont) { 
  ...
  int nhit = cont->getnelem();
  vector<pair<double, TrRecHitR*> > hitxy[2];

  //----set inform
  for(int ihit=0; ihit<nhit; ihit++){
    TrRecHitR* hit = (TrRecHitR*) cont->getelem(ihit);
    double sig = 0;
    if     (type==1)sig=hit->GetCorrelationProb();//probability
    else if(type==2)sig=hit->GetSignalCombination(1,TrClusterR::DefaultCorrOpt)*100+hit->GetCorrelationProb();//y signal+probability
    else if(type==3)sig=hit->GetSignalCombination(1,TrClusterR::DefaultCorrOpt)*100+hit->GetSignalCombination(0,TrClusterR::DefaultCorrOpt);//y signal+ x signal
    else if(type==4){//priority: CSEED>ZSEED>Normal
      sig=hit->GetSignalCombination(4,TrClusterR::DefaultCorrOpt);//Normal Compared Y+X Total 
      if     (hit->checkstatus(TrRecHitR::CSEED))sig=sig*100;//set highest priority for CSEED
      else if(hit->checkstatus(TrRecHitR::ZSEED))sig=sig*10; //set higher  priority for ZSEED
    }
    hitxy[hit->OnlyY()?1:0].push_back(make_pair(sig,hit));
  }

  //----sort + cleand and reoder + backup though fastest pointer  
  for(int ixy=0;ixy<2;ixy++)std::sort(hitxy[ixy].begin(),hitxy[ixy].end(),CompSignal<TrRecHitR>);//Highest sig to lowest
  vector<TrRecHitR> hits;
  unsigned int ii=0;
  while(ii<hitxy[0].size()||ii<hitxy[1].size()){//
   for(int ixy=0;ixy<=1;ixy++){//Highest YX-Y-YX-Y... make sure NXY~NY
      if(TRCLFFKEY.KeepBestTrClHits!=0 && int(hits.size())>=nmaxhits-1)break;//the rest would erase
      if(ii>=hitxy[ixy].size())continue;
      hits.push_back(*(hitxy[ixy].at(ii).second));
    }
    ii++;
  }
...
#ifndef __ROOTSHAREDLIBRARY__ 
    cont->addnext(new AMSTrRecHit(hits[ihit]));
#else
    cont->addnext(new TrRecHitR(hits[ihit]));
#endif
  }
...
void TrRecon::BuildHitsTkIdMap() 
{
  ...
  for (int i = 0; i < nhit; i++) {
    ...
    if(TkDBc::Head->GetSetup()==3||TkDBc::Head->GetSetup()==4){
...
#pragma omp critical (trrecsimple)
  ...
  AMSPoint cc[NP][NH];
  int    ilqu[NP][NH];  // Added by ZQ,SH 151008
  int      ic[NP][NH], nc[NP];
  double   qc[NP][NH];
  ...
  ilqu[ip][nc[ip]] = ly;    // Added by ZQ,SH 151008
  ...
  struct TrSeed {
    ...
    // Added by ZQ,SH 151008
    int IntpQu(AMSPoint *pint, double *zlay  ) {
      TrFit trfit;
      double err = 300e-4;
      bool iscc[NL]={0,0,0,0,0,0,0};
      for (int j = 0; j < NL; j++) 
      {
        if(cc[j].x()==0&&cc[j].y()==0&&cc[j].z()==0  ) continue;
        trfit.Add(cc[j], err, err, err);
        int ilay = ic[j]%10 -1;
        pint[ilay].setp(0,cc[j].y(),cc[j].z());
        iscc[ilay] = 1;
      }
      if (trfit.SimpleFitTop() < 0) return -1;
      for(int j=0;j<NL;j++)
        if( !iscc[j] ) pint[j].setp(0,0,zlay[j]);
      for(int j=0;j<NL;j++)
      {
        if(!iscc[j] ) 
        {
          trfit.Propagate( zlay[j] );
          AMSPoint tmppnt = trfit.GetP0();
          pint[j].setp(0,tmppnt.y() ,zlay[j]);
        }
      }
      return 1;
    }
...
#define CZ(I) cc[I][i[I]].z()
  ...
  for (int jc = 0; jc < NC*2; jc++) {
    ...
    AMSPoint pint[NP-1];
    AMSPoint pintQu[NL];
    if (TRCLFFKEY.UseZQFix) {
      if (tmin[jc].IntpQu(pintQu, zlay) < 0) continue;
    }
    else {
	...
	if (TRCLFFKEY.UseZQFix) {
	  int il = ilqu[j][k];
	  r = std::fabs(cc[j][k].y()-pintQu[il-1].y());
	}
	// Added by ZQ,SH 151008    
...
//      }
	...
	for (int j = 0; j < nmrg; j++) {
      ...
      float qx = TrCharge::GetMean(TrCharge::kInner|TrCharge::kTruncMean|TrCharge::kSqrt,track,TrCharge::kX).Mean;
      float qy = TrCharge::GetMean(TrCharge::kInner|TrCharge::kTruncMean|TrCharge::kSqrt,track,TrCharge::kY).Mean;
...
int TrRecon::PreselTrTracksVertex()
{
  ...
  int tkreczq= (TRCLFFKEY.recflag/1000000%10);//using tkreczq
  int simple = (TRCLFFKEY.recflag%10000 >= 1000) ? 1 : 0;
  if ((simple||tkreczq) && ntrk == 0) return 1;
...
int TrRecon::RecoverExtHits()
{
  ...
  for (vector<TrClusterR>::iterator i = i0; i != vc.end();) {
	...
	float prob = hit.GetCorrelationProb();
        bool iszseedcl=(i->checkstatus(TrClusterR::ZSEED) && j->checkstatus(TrClusterR::ZSEED));
	if (DEBUG >= 2)
	  cout << Form("hit %4d %5.3f %d %5.1f %5.1f", tk, prob,
	    ...
	    (TRCLFFKEY.ChargeSeedTagActive == 0 ||
	     CompatibilityWithChargeSeed(&hit))) {
	      ...
	      hit.setstatus(TrRecHitR::ZSEED);
              if((TRCLFFKEY.ClusterCofGOpt%10)>=2){//QY
                j->SetQtrk(_htmq[0]);//Use X to linearize Y
                i->SetQtrk(_htmq[1]);//Use Y to linearize X
                hit.BuildCoordinate();
              }
	      vh.push_back(hit);
	}
...
int TrRecon::CountTracks(int trstat, int refit)
{
  VCon* cont = GetVCon()->GetCont("AMSTrTrack");
...
#pragma omp critical (trcount)
 ...
 if (vertex && !(simple || tkreczq)) {
  ...
  double tkmom=0;
  int itrku=-1;
  for (int itrk = 0; itrk < ntrk; itrk++) {//select the largest momentum
    ...
    // Look for a track with at least one hit at all the ININNER PLANES
    if (!(trk->GetBitPattern() & 0x006) ||
	!(trk->GetBitPattern() & 0x018) ||
	!(trk->GetBitPattern() & 0x060)) continue;
    double innerq = trk->GetInnerQ();
    double mom=fabs(argt*innerq);
    if(mom>tkmom){itrku=itrk;tkmom=mom;}//the largest momemtum 
  }

  if(itrku>=0){
    TrTrackR *trk = (TrTrackR*)cont->getelem(itrku);
    int mfi = trk->iTrTrackPar(malgo, 3, refit); // Inner fitting
    double csqx = trk->GetNormChisqX(mfi);
    double csqy = trk->GetNormChisqY(mfi);
    double argt = fabs(trk->GetRigidity(mfi));
    ...
    double innerq = trk->GetInnerQ();
//-------
    if ((span & spflag[0]) && trk->GetNhitsI(0) >=3) fTrackCounter[11]++;
    if ((span & spflag[0]) && argt>2 && innerq>1.5 ) fTrackCounter[12]++;
    if ((span & spflag[0]) && csqx < 20 && csqy < 20) {
      fTrackCounter[6]++;
      trstat |= 0x400;

      int mff = trk->iTrTrackPar(malgo, 7, refit); // Full span
...
int TrRecon::FillHistos(int trstat, int refit)
{
  ...
  for (int j = 0; j < TkDBc::Head->nlay(); j++) {
	...
	if (cly) nhyi++;
      ...
      if(refit/10==3){//V6 version
        double length,rigidity;
        AMSPlaneM plm=trk->InterpolateLayerJN(length,rigidity,TkPlaneN::convertlayid(j)+1);
        TkSensorN *sensor=(TkSensorN *)plm.getmodule();
        if(length>-1000&&sensor){
          double dist=sensor->GetDistance(plm.getP(),plm.getD(),-2,-2,1);
          if(dist<=0){//within sensor
            int tkid=sensor->getmother()->getid();//ladder id
            hman.Fill("TrLadTrk",tkid%100,abs(tkid)/100);
          }
        }
      }
      else {
  ...
  int mf8 = trk->iTrTrackPar(malgo, 5, refit); // With L1N
  int mf9 = trk->iTrTrackPar(malgo, 6, refit); // With L9
  int mff = trk->iTrTrackPar(malgo, 7, refit); // Full span
...
void TrRecon::PrintStats()
{
 ...
 int vertex = (TRCLFFKEY.recflag%100000 >= 10000) ? 1 : 0;
 if(simple||tkreczq) {
  cout << Form("Events with-plI track        :  %8d (%5.3f)",
	       nc[1], 1.*nc[1]/nc[0]) << endl;
  cout << Form("  with all-plI  track        :  %8d (%5.3f)",
	       nc[2], 1.*nc[2]/nc[1]) << endl;
  cout << Form("  with plI&HitX track        :  %8d (%5.3f)",
               nc[11],1.*nc[11]/nc[1]) << endl;
  cout << Form("  with plI&Chi2 track        :  %8d (%5.3f)",
               nc[6], 1.*nc[6]/nc[1]) << endl;
  cout << Form("  with BZ&HRig  track        :  %8d (%5.3f)",
               nc[12],1.*nc[12]/nc[0]) << endl;
...
int TrRecon::MergeLowSNHits(TrTrackR *track, int mfit, int select_tag)
{
  ...
  for (int i = 0; i < nhit; i++) {
    TrRecHitR *hit = (TrRecHitR*)cont->getelem(i);
    if (!hit || hit->Used() ||
	((TkDBc::Head->GetSetup()==3||TkDBc::Head->GetSetup()==4) && hit->GetLayer() >= 8)) continue;
...
int TrRecon::MergeExtHits(TrTrackR *track, int mfit, int select_tag)
{
  ...
  if(good_base1==good_base2)good_base2=base;//try to extend mfit length
 ...
 if (PZDEBUG){
  ...
  if (!MagFieldOn()) {
    ...
    if (TryDropX(track, track->Gettrdefaultfit())) {
      ...
      track->RecalcHitCoordinates(track->Gettrdefaultfit());
    }
  }
...
int TrRecon::TryDropX(TrTrackR *track, int mfit)
{
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (hit1 && hit2) {
	  ...
	  track->GetChisqY(mfit)*rthd2 < rcsq) {//QY Bug Fixed (setstatus would remove the XCluster forever), charge to find YONLY is VCon
//-------
          ...
          for (int ii=0; ii< cont->getnelem(); ii++){
            ...
            if (!hit || !hit->OnlyY()) continue;
            if(hit!=hit1 && hit->GetYCluster()==hit1->GetYCluster()){//Replace by YONLY Cluster
              track->AddHit(hit);//This would replace previous Hit
              hit1->ClearUsed();
              hit->SetUsed();
              if(hit1==hit2)break;
            }
            if(hit1==hit2)continue;
            if(hit!=hit2 && hit->GetYCluster()==hit2->GetYCluster()){
              track->AddHit(hit);//This would replace previous Hit
              hit2->ClearUsed();
              hit->SetUsed();
            }
...
//-------
  ...
  track->FitT(mfit);//have to go back to the original bit
  return 0;
}
...
//-------
  _htmq[0]=_htmq[1]=1;
  if ((TRCLFFKEY.ClusterCofGOpt%10)>=2){//QY add seed cluters protection 

   //-----mask noisy calib strips not to be ZSEED candidate
   int calibmask=0x8;
   TrClusterR *mcluster[9][2]={{0}},*mclusterx[9][2]={{0}};
   TrCharge::GetHighestClustersXY(TrCharge::kY,TrClusterR::kAsym|TrClusterR::kGain|TrClusterR::kAngle,mcluster, calibmask);//Y as seed/associate with X
   TrCharge::GetHighestClustersXY(TrCharge::kX,TrClusterR::kAsym|TrClusterR::kGain|TrClusterR::kAngle,mclusterx,calibmask);//X as seed/associate with Y
   //-----calibmask=0 no mask
   calibmask=0;
   TrClusterR *mclustera[9][2]={{0}},*mclusterxa[9][2]={{0}};
   TrCharge::GetHighestClustersXY(TrCharge::kY,TrClusterR::kAsym|TrClusterR::kGain|TrClusterR::kAngle,mclustera, calibmask);//Y as seed/associate with X
   TrCharge::GetHighestClustersXY(TrCharge::kX,TrClusterR::kAsym|TrClusterR::kGain|TrClusterR::kAngle,mclusterxa,calibmask);//X as seed/associate with Y
   

   //-----Inner charge seed cluster
   float ubeta=1,urig=0,umass=0,udxdz=0,udydz=0;
   vector<pair<double,TrClusterR*> > qs[2];
   vector<float> as[2];

   //-----Charge seed possible threshould highest->lowest HZ/He/Pr/All (4 threshold)
   const int nth=3;
   float hqt[nth]={2.5,1.5,0.5};
   double thq4[2]={0,0};

   //-----First Inner clusterY/Then clusterX
   for(int ixy=1;ixy>=0;ixy--){
     int npls[4]={0,0,0,0};//4 inner planes
     for(int jlayer=0+1; jlayer<9-1; jlayer++){
       TrClusterR* cluster=(ixy==1)? mcluster[jlayer][1]:mclusterx[jlayer][0];//Y/X
       double clq=cluster?cluster->GetQ(ubeta,urig,umass,udxdz,udydz):0;
       if(clq>0){qs[ixy].push_back(make_pair(clq,cluster)); npls[jlayer/2]++;}
     }

     std::sort(qs[ixy].begin(),qs[ixy].end(),CompSignal<TrClusterR>);

     //tag top 4 highest located planes
     int hplbit=0;
     for(unsigned int ii=0; ii<qs[ixy].size(); ii++){
       if(ii>=4)break;//only account top 4
       TrClusterR* cluster=qs[ixy].at(ii).second;
       int ipl=(cluster->GetLayerJ()-1)/2;
       hplbit|=(1<<ipl);
     }

     //get threshould for topologic extention
     int nhpl=0;//top 4 highest signal located planes
     for(int ihpl=0;ihpl<4;ihpl++)if(hplbit&(1<<ihpl))nhpl++;//avoid uncorrelated noise particle passing through 2 planes
     unsigned int ia=3;//using 4th highest cluster q as threshould to eliminate noise
     double q4=0;
     if    ((qs[ixy].size()>ia+1)&&(nhpl==2))q4=qs[ixy].at(ia+1).first;//5th highest cluster q as threshould (extend topology)
     else if(qs[ixy].size()>ia)q4=qs[ixy].at(ia).first;//4th highest cluster q as threshould
     else if(qs[ixy].size()>=1)q4=qs[ixy].back().first;//last cluster q as threshould
     double th1=0;
     for(int it=0;it<nth;it++){//find first threshould for lowZ
        if(q4>=hqt[it]){th1=hqt[it];break;}
     }
     double th2=q4*TRCLFFKEY.ZSEEDClQThr;//second threshould for highZ
     thq4[ixy]=(th2>th1) ? th2: th1;//highest between two

     //remove lowest below threshould not to be ZSEED
     while(qs[ixy].size()>4){
       double qn=qs[ixy].back().first;
       TrClusterR* cluster=qs[ixy].back().second;
       int ipl=(cluster->GetLayerJ()-1)/2;
       if(qn<thq4[ixy]){qs[ixy].pop_back(); npls[ipl]--;}
       else break;
     }
       
     //remove lowest hitedge/1strip YCluster not to be ZSEED
     if(ixy==1){
       for(int ii=int(qs[ixy].size())-1; ii>=4; ii--){
        TrClusterR* cluster=qs[ixy].at(ii).second;
        int mult=0,seed_index=-1; 
        int striptype=cluster->GetSeedSecondPairType(mult,seed_index,TrClusterR::DefaultCorrOpt);
        bool isedge=(((striptype/1000)%10>=1)||((striptype/10000)%10>=1));//seed/second hit at edge of sensor
        bool nosecond=((striptype/100000)%10==1);//no second strip
        int ipl=(cluster->GetLayerJ()-1)/2;
        if((isedge||nosecond)&&(npls[ipl]==2)){//seed/second at edge of sensor or no second from double measured layers
          qs[ixy].erase(qs[ixy].begin()+ii); npls[ipl]--;
        }
       }
     }

     //replace X with Yseed if Y selected as ZSEED 
     for(unsigned int i=0;i<qs[ixy].size();i++){
       TrClusterR* cluster=qs[ixy].at(i).second;
       int jlayer=cluster->GetLayerJ()-1;
       as[ixy].push_back(cluster->GetTotSignal(TrClusterR::kAsym|TrClusterR::kGain|TrClusterR::kAngle));
       if(ixy==1){//replace X seed by Y seed as this has been defined
         mclusterx[jlayer][0]=mcluster[jlayer][0];
         mclusterx[jlayer][1]=mcluster[jlayer][1];  
       }
     }
   }


   //-----pre-requirement+protection 
   if(qs[1].size()>=4&&qs[0].size()>=3){//>=4YHits && >=3XHits
  
     //---using Y refine/remove X noise for YQ>2.5
     if(qs[1].size()>=5&&qs[0].size()>=4){//X efficiency not that high
       bool shqy=(qs[1].back().first>=hqt[0]);//last Y >2.5
       while(shqy&&(qs[0].size()>=4)){
         if(qs[0].back().first<hqt[1]){qs[0].pop_back();if(thq4[0]<hqt[1])thq4[0]=hqt[1];}//remove XQ<1.5
         else break;
       }
     }

     //---set zseed for inner and exteranl
     for(int ixy=1;ixy>=0;ixy--){
       _htmq[ixy]=0;
       int npls[4]={0,0,0,0};//4 inner planes
       for(unsigned int i=0;i<qs[ixy].size();i++){
         TrClusterR* cls=qs[ixy].at(i).second;
         cls->setstatus(TrClusterR::ZSEED);//set ZSEED
         if(i>=1)_htmq[ixy]+=qs[ixy].at(i).first;//TruncMean
         int ipl=(cls->GetLayerJ()-1)/2;
         npls[ipl]++;
       }

       //---try to recover one InnerY Layer for missing plane from noisy candidate 
       if(ixy==1){
         for(int ipl=0;ipl<4;ipl++){
           if(npls[ipl]>0)continue;//check if missing plane
           TrClusterR *clys=0,*clxs=0;
           for(int il=0;il<2;il++){
             int jlayer=2*ipl+il;
             if(jlayer==0)continue;//ipl=0 only has L2
             TrClusterR* cly=mclustera[jlayer][ixy];
             TrClusterR* clx=mclustera[jlayer][1-ixy];
             if(!cly||(cly==mcluster[jlayer][ixy]))continue;//check this is the new noisy cluster
             TrClusterR *clxc=0;//already in the selection
             for(unsigned int i=0;i<qs[1-ixy].size();i++){
               if(qs[1-ixy].at(i).second->GetLayerJ()==jlayer+1){clxc=qs[1-ixy].at(i).second; break;}
             }
             if(clx!=clxc)continue;//XCluster did not match with exist X candidate
             double clq=cly->GetQ(ubeta,urig,umass,udxdz,udydz);
             if((clq>=qs[ixy].back().first)||(clq>=thq4[ixy])){//match with amplitude
               if(!clys||(!clxs&&clx)||(clq<clys->GetQ(ubeta,urig,umass,udxdz,udydz))){clys=cly;clxs=clx;} //to be more safe, choose lower amplitude layer
             }
           }
           if(clys)clys->setstatus(TrClusterR::ZSEED);//recover and set ZSEED
         }
       }
 

       //---External-Layer treatment ZSEED
      for(int ixt=0;ixt<2;ixt++){
        int jlayer=(ixt==0)?0:8;
        for(int imask=0;imask<2;imask++){//external has double chances: not noisey clusters first then all clusters
          TrClusterR* clt=(ixy==1)? mcluster[jlayer][1]:  mclusterx[jlayer][0]; //Y or X no noisey clusters
          if(imask==1)clt=(ixy==1)? mclustera[jlayer][1]: mclusterxa[jlayer][0];//Y or X all clusters
          if(!clt)continue;
          double clq=clt->GetQ(ubeta,urig,umass,udxdz,udydz);
          if((clq>=qs[ixy].back().first) || (clq>=thq4[ixy])){//two chances again 
            clt->setstatus(TrClusterR::ZSEED);
            if(ixy==1){//Y found
              if(imask==0){//try with no noisy
                mclusterx[jlayer][0]=mcluster[jlayer][0];
                mclusterx[jlayer][1]=mcluster[jlayer][1];
              }
              else {//try with all
                mclusterx[jlayer][0]=mclustera[jlayer][0];
                mclusterx[jlayer][1]=mclustera[jlayer][1];
              }
            }
            break;//break from imask if found
          }
         }
       }
 
       //--------
       _htmq[ixy]/=(qs[ixy].size()-1);
       if(ixy==0)_htmx_corr=TrCharge::GetMean(TrCharge::kInner|TrCharge::kTruncMean,as[ixy]).Mean;//re-assign threshould
       else      _htmy_corr=TrCharge::GetMean(TrCharge::kInner|TrCharge::kTruncMean,as[ixy]).Mean;//re-assign threshould
     }
   }
...
bool TrRecon::CompatibilityWithChargeSeed(TrClusterR* cluster) {
  if (cluster->checkstatus(TrClusterR::ZSEED))return true;//QY
  int   iside  = cluster->GetSide();
