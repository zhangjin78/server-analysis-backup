...
// ST_Orient ( used instead of CAM_Pitch)
    double CAM_Yaw,CAM_Roll,CAM_Pitch;
  if (CamID==1){ /*Port Camera*/
    CAM_Yaw=235./180.*pi;   /* Nominal Value*/
    CAM_Roll=-40.15/180.*pi;  /* MK Value*/
    CAM_Pitch=0;
    ST_Orient+=0.56/180.*pi; /*MK value*/

  }
  if (CamID==0){ /*StarBoard Camera*/
    CAM_Yaw=55./180.*pi;    /* Nominal Value*/
    CAM_Roll=-40.02/180.*pi;  /* MK Value*/
    CAM_Pitch=0;
    ST_Orient+=-2.3/180.*pi; /*MK value*/

  }
  ST_AMS2CAM(x, y, z, CAM_Yaw, CAM_Roll,CAM_Pitch);  
...
ST_ECI2tangPlane( x, y, z, ST_RA, ST_dec);
  ...
  ST_CAM2AMS(x, y, z, CAM_Yaw, CAM_Roll,CAM_Pitch);  
...
void ST_AMS2CAM(double& x, double& y, double& z, double Y, double R, double P){
...
// P: Pitch rotation angle
  double oldX=x;double oldY=y;double oldZ=z;
 
  x= cos(Y)*cos(P)*oldX        + sin(Y)*cos(P)*oldY-sin(P)*oldZ;
  y=(cos(Y)*sin(P)*sin(R)-cos(R)*sin(Y))*oldX + (sin(R)*sin(P)*sin(Y)+cos(R)*cos(Y))*oldY   + cos(P)*sin(R)*oldZ;
  z= (cos(R)*cos(Y)*sin(P)+sin(R)*sin(Y))*oldX +(sin(Y)*sin(P)*cos(R)- cos(Y)*sin(R))*oldY   + cos(P)*cos(R)*oldZ;
// explanation:
// this is the common YPR matrix, where P=0. the CCD frame was measured during assembly procedure.
}

void ST_CAM2AMS(double& x, double& y, double& z, double Y, double R, double P){
...
// P: Pitch rotation angle
  double oldX=x;double oldY=y;double oldZ=z;
  x= cos(Y)*cos(P)*oldX  +(cos(Y)*sin(P)*sin(R)      - cos(R)*sin(Y))*oldY   + (cos(R)*cos(Y)*sin(P)+sin(R)*sin(Y))*oldZ; 
  y= sin(Y)*cos(P)*oldX        + (sin(Y)*sin(R)*sin(P)+cos(R)*cos(Y))*oldY  +(sin(Y)*sin(P)*cos(R) - cos(Y)*sin(R))*oldZ;
  z=           -sin(P)*oldX         + sin(R)*cos(P)*oldY          + cos(P)*cos(R)*oldZ;
// explanation:
...
double FT_UTC_JD(double itime){
 ...
 if(itime>=1483228800)TAItime=itime-37;
 else if(itime>=1435708800)TAItime=itime-36;
 else if (itime>=UTC1Ju2012) TAItime=itime-35;
