...
TGeoShape* ams2tgeo::AMSgvolume2TGeoShape(AMSgvolume* amsvol) {
  ...
  if      (strcmp(amsvol->getshape(),"BOX ")==0) {
  ...
  else if (strcmp(amsvol->getshape(),"TRDSPX")==0){//use box to replace trdspx
    sprintf(shaname,"BOX_%05d",nsha);
    Double_t dx = amsvol->getpar(0);
    Double_t dy = amsvol->getpar(1);
    Double_t dz = amsvol->getpar(2);
    ...
    tshape->SetTransform(0);
    if (debug_geo)
      cout << "AMSgvolume2TGeoShape::Transformation from BOX-TRDSPX to TGeoBBox: "
           << "  dX " << dx << "  dY " << dy << "  dZ " << dz << endl;
    if (amsvol->getnpar()!=5) cout << "AMSgvolume2TGeoShape-E-Wrong npars BOX-TRDSPX (" << amsvol->getnpar() << ")" << endl;
  }
...
//      if (debug_mat) 
      cout << "  Element of " << matname << " n. " << i << " --> " << a[i] << " " << z[i] << " " << w[i] << endl;
...
//      if (debug_mat) 
cout << "  Material " << matname << " created    A: " << tmat->GetA() << "   Z: " << tmat->GetZ() 
...
//      if (debug_mat) 
cout << "  Mixture " << matname << " created    A: " << tmix->GetA() << "   Z: " << tmix->GetZ() 
  ...
  while (amsmed) {
    ...
    if (debug_med) for (int i=0; i<8; ++i) 
    cout << param[i] << endl;
