// $Id: job.C,v 1.1141 2025/05/29 14:36:01 choutko Exp $
...
#ifdef G4MULTITHREADED
  MISCFFKEY.NoOrderedWrite=11;
  MISCFFKEY.G4AllocatorSize=-4000000;
#else
  MISCFFKEY.NoOrderedWrite=0;
  MISCFFKEY.G4AllocatorSize=0;
#endif
  MISCFFKEY.NewCRC=0;
  MISCFFKEY.AnyThreadNumber=0;
  MISCFFKEY.MinimalRatioReconstructed=1.;
  MISCFFKEY.MemReservedWeightPerThread=0.333;
...
//  G4FFKEY.TFNewGeant4=12;//Using New TOF support structure geometry in MC 2014-07-27
  G4FFKEY.TFNewGeant4=22;//Using New TOF support structure geometry in MC 2015-02-03
  ...
  G4FFKEY.HCrossSectionBias[4]=1;
  G4FFKEY.HCrossSectionBias[5]=1;
  G4FFKEY.HCrossSectionBias[6]=0;
  G4FFKEY.ApplyCPULimit=1;  
  G4FFKEY.DumpCrossSectionsAt=12;
  G4FFKEY.DumpCrossSectionsZt=6;
  G4FFKEY.GamaPolar = 0;
  G4FFKEY.polarrate = -1;
  G4FFKEY.HEexcBias[0]=1.;//binding energy bias for hardon/nulear inelastic interaction
  G4FFKEY.HEexcBias[1]=1.;
  G4FFKEY.RadDec=0;
  G4FFKEY.MergeMCCluster=12;// TRD/TOF
  // G4FFKEY.IonParametrisedLossStep = 50.; // 50 um new model ; 0 old model
  G4FFKEY.IonParametrisedLossStep = 0.; // 50 um new model ; 0 old model
  VBLANK(G4FFKEY.VisFile,40);//37
...
void AMSJob::_sirichdata(){
  ...
  *******************************************************/
  ...
  RICCONTROLFFKEY.mcgeomshift[0]=0.11; // RICH MC geometry position shift in X [cm]
  RICCONTROLFFKEY.mcgeomshift[1]=-0.08;// RICH MC geometry position shift in Y [cm]
  RICCONTROLFFKEY.mcgeomshift[2]=-0.93;// RICH MC geometry position shift in Z [cm]
  ...
  RICRECFFKEY.createrichringx=0;
  RICRECFFKEY.dphi=200;
  RICRECFFKEY.dz=200;
...
//  TFMCFFKEY.g4hfnoise=9.;//146 new g4tof high freq. noise 
  TFMCFFKEY.g4hfnoise=0.;//146 new g4tof high freq. noise
  ...
  TFMCFFKEY.simfvern=509400;//149  simulation DB version/g4DB(5)/PMDB(09)/tsfDB(4)/other use
...
//  TFMCFFKEY.threref[2]=0.95;//152  SHT reference to normal value
  ...
  TFMCFFKEY.dcnoise=0.;//154 LT discriminator noise reference to normal value (20ps)
//  TFMCFFKEY.lmatref=1.;//155 LTOF supporting materials reference to normal value: new adding 0.00805*(lmatref-1)X0
  TFMCFFKEY.lmatref=2.2;//155 LTOF supporting materials reference to normal value: new adding 0.00805*(lmatref-1)X0
  //---
...
void AMSJob::_siecaldata(){
  ...
  ECMCFFKEY.mev2mev=28.90;  //(6) Geant dE/dX(MeV)->MCEmeas(=Evis,MeV,noRelGainsApplied,PmCouplingIncl, PE-fluct.incl)
...
ECMCFFKEY.mev2mev=27.57;
  ...
  ECMCFFKEY.SbMassFrac=0.020; // (33) Antimonium mass fraction in Lead-Antimonium (corresponds to 98% Lead relative density);
  ...
  ECMCFFKEY.cladgluex=0.011; //(36) fiber clad+glue horizontal thickness (cm)  
  ECMCFFKEY.cladgluey=0.011; //(37) fiber clad+glue vertical thickness (cm) 
  ...
  ECMCFFKEY.TuneMC=0;//(87) 0: original MC, 1: tune MC shower shape //wxq
  ECMCFFKEY.RelDen=1; // (88) RelDensity
...
void AMSJob::_reecaldata(){
  ...
  ECREFFKEY.mon[0]=0;  
  ECREFFKEY.mon[1]=0;
  ECREFFKEY.year[0]=108; 
  ECREFFKEY.year[1]=136;
  ECREFFKEY.Tref=10.;// (121)
  ...
  ECREFFKEY.mipTB2ISS=1.005;  // (127) Factor to convert TB mip (400 GeV protons) to ISS mip (E>5 GeV)
...
void AMSJob::_sitrddata(){
  ...
  TRDFITFFKEY.MaxSegAllowed=120;
  TRDFITFFKEY.FitMethod=12;
...
void AMSJob::_reaxdata(){
  ...
  ECALHFFKEY.BadCellFit =0;
  ECALHFFKEY.BadCellR[0] =0.05;
  ECALHFFKEY.BadCellR[1] =5;
...
void AMSJob::udata(){
 if(MISCFFKEY.NumThreads>maxthread){
   MISCFFKEY.NumThreads=maxthread;
   cout<<"AMSJOb::udata-W-NumThreadsChangedTo "<<MISCFFKEY.NumThreads<<endl;
}
    ...
    cout<<"<---- AMSJob::udata-I-Geant4Selected"<<endl<<endl;
{
    ...
    if (ulimit_v.rlim_cur / 1024 < uinteger((3.+MISCFFKEY.NumThreads*MISCFFKEY.G4GbPerThread)*1024000)) {
        cerr<<"<---- AMSJob::udata-F-VirMemNotEnough: "<< ulimit_v.rlim_cur / 1024 << " < " << (3.+MISCFFKEY.NumThreads*MISCFFKEY.G4GbPerThread)*1024000 <<endl<<endl;
        abort();
    }
...
}
{
  ...
  TRDDBc::init();
  {
	...
	triggername[i]='\0';
        if(strstr(triggername+nold,"TrigPhys"))phys=1;
	if(i-nold>0)settrigger(triggername+nold,ntrig++,orr,phys);
	nold=i+1;
      }
    }
  }
  ...
  // datacard depdends on MC/Data, isSimulation or isReconstruction should come after setjobtype
  if(isSimulation()){
...
#ifdef _PGTRACK_
  if(isSimulation()){
    if(TRFITFFKEY.Zshift==2)TRFITFFKEY.Zshift=-1;
  }
  // Book histograms
  hman.Enable();
  hman.BookHistos(isSimulation()?3:0);
#endif
  ...
  if(strstr(getsetup(),"AMS02") ){    
    ...
    else if(strstr(getsetup(),"AMS02P.2") ){
      cout <<"AMSJob::udata-I-PMTrackerConfigurationRequestred "<<endl;
      //      AMSTrIdSoft::inittable(4);
      STD_DB_ver=4; // Not updated, since _PGTRACK_ should always be used
      pgtrack_DB_ver=4;
    }
    ...
    if(TKGEOMFFKEY.LoadMCDisalign%10>0){
      ...
      if(TKGEOMFFKEY.LoadMCDisalign%10==2)
        ...
        if(strstr(getsetup(),"AMS02P.2"))
	        sprintf(fname,"TkMCDis_4_5.txt");
        else
...
char name[4095];
   sprintf(name,"%s/%s/RichDefaultCalibration.dat",getenv("AMSDataDir"),AMSCommonsI::getversion());
    RichOffline::RichPMTsManager::Init(name);
#ifndef  __DARWIN__ 
    feclearexcept(FE_ALL_EXCEPT);
    if(env && MISCFFKEY.RaiseFPE){
      cout <<"Init-I-EnableFPE "<<env<<endl;
      feenableexcept(env);        
    }
    else{
     cout<<"Init-I-DisableFPE"<<endl;
      fedisableexcept(FE_ALL_EXCEPT);
    }
#endif
    RichRadiatorTileManager::Init();	
    RichOffline::RichRadiatorTileManager::Init();
    RichAlignment::Init();
  ...
  // create the gain database singleton, and linear array
  if (isSimulation()) TrSimSignalDB::GetHead();
  ...
  if(isSimulation())TKGEOMFFKEY.MdExAlignTag=1;
   ...
   // default charge for MC 
    if ( (isSimulation())&&(AMSCommonsI::getbuildno()>=1106) ) TrTrackR::DefaultChargeVersion = 3;
  ...
  TrChargeYJDB::GetHead(isRealData())->Init();
  TrChargeYJTimeDB::GetHead()->Init();
  RichRingXR::create=RICRECFFKEY.createrichringx;
  RichRingXR::dz=RICRECFFKEY.dz;
  RichRingXR::dphi=RICRECFFKEY.dphi;
...
void AMSJob::_dbinitjob(){
  if (AMSFFKEY.Update>1 && AMSFFKEY.Update!=3){
...
void set_geant4_10_01_environment_variables() {
	...
	setenv("G4DPMJET2_5DATA" ,g4is.c_str(),1);
        g4is=g4i;
        g4is+="/data/DPMJET/dpmjet3_0/tabs";
        setenv("G4DPMJET3_DATA" ,g4is.c_str(),1);
...
void set_geant4_10_03_environment_variables() {
	...
	std::string g4i=getenv("G4INSTALL");
	string g4is=g4i;
	g4is+="/data/PhotonEvaporation4.3.2";
	setenv("G4LEVELGAMMADATA", g4is.c_str(),1);
	g4is=g4i;
	g4is+="/data/RadioactiveDecay5.1.1";
	setenv("G4RADIOACTIVEDATA"  ,g4is.c_str(),1);
	g4is=g4i;
	g4is+="/data/G4EMLOW6.50";
	setenv("G4LEDATA" ,g4is.c_str(),1);
	g4is=g4i;
	g4is+="/data/G4NDL4.5";
	setenv("NeutronHPCrossSections" ,g4is.c_str(),1);
	...
	setenv("G4SAIDXSDATA" ,g4is.c_str(),1);
	g4is=g4i;
	g4is+="/data/G4ENSDFSTATE2.1";
	setenv("G4ENSDFSTATEDATA" ,g4is.c_str(),1);
}
void set_geant4_10_04_environment_variables() {
	...
	std::string g4i=getenv("G4INSTALL");
	string g4is=g4i;
	g4is+="/data/PhotonEvaporation5.2";
	setenv("G4LEVELGAMMADATA", g4is.c_str(),1);
	g4is=g4i;
	g4is+="/data/RadioactiveDecay5.3";
	setenv("G4RADIOACTIVEDATA"  ,g4is.c_str(),1);
	g4is=g4i;
	g4is+="/data/G4EMLOW7.3";
	setenv("G4LEDATA" ,g4is.c_str(),1);
	g4is=g4i;
	g4is+="/data/G4NDL4.5";
	setenv("NeutronHPCrossSections" ,g4is.c_str(),1);
...
bool try_perform_geant4_autodetection() {
  ...
  bool isGeant4_10_03_build = strstr((const char *)G4Version,"geant4-10-03") != 0;
  bool isGeant4_10_04_build = strstr((const char *)G4Version,"geant4-10-04") != 0;
  if (!isGeant4_9_06_build && !isGeant4_10_01_build&& !isGeant4_10_03_build&& !isGeant4_10_04_build) {
...
if(!g4basedir){
  ...
  if (isGeant4_9_06_build) {
    ...
    if(!getenv("G4AUTODETECT") || !strstr(getenv("G4AUTODETECT"),"NOSET"))setenv("G4INSTALL", g4installdir.c_str(), 1);
//  reset g4install according to rank
    char *g4ad=getenv("G4AUTODETECT");
    if(g4ad && strlen(g4ad) && strstr(g4ad,"NOSET")){
      char tmp[strlen(g4ad)+1];
      strcpy(tmp,g4ad);
      char *pch=strtok(tmp,"_");
      int mn=1;
      if(pch)mn=atol(pch);
      if(mn)cout<<"AMSJob::_signinitjob()-I-SplitFoundforG4INSTALL "<<mn<<endl;
      char *g4i=getenv("G4INSTALL");
      if(mn && g4i && strlen(g4i)){
        char tmp1[strlen(g4i)+255];
        sprintf(tmp1,"%s_%d",g4i,mpi.getRank()%mn);
        setenv("G4INSTALL",tmp1,1);
      }
     }
    cout << "AMSJob::_signinitjob()-I-Set G4INSTALL to " << getenv("G4INSTALL") << endl;

    set_geant4_10_01_environment_variables();
  } else if (isGeant4_10_03_build) {
    g4installdir += "/geant4.10.03-ams";
    setenv("G4INSTALL", g4installdir.c_str(), 1);
    cout << "AMSJob::_signinitjob()-I-Set G4INSTALL to " << g4installdir << endl;
    set_geant4_10_03_environment_variables();
  } else if (isGeant4_10_04_build) {
    g4installdir += "/geant4.10.03-ams";
...
//  reset g4install according to rank
    ...
    cout << "AMSJob::_signinitjob()-I-Set G4INSTALL to " << getenv("G4INSTALL") << endl;
    set_geant4_10_04_environment_variables();
  }
  ...
  if(g4i && !strstr((const char *)G4Version,"geant4-09-04" )&& strstr (g4i,"geant4.9.4") && isSimulation()){
   if(!g4i && isSimulation() && MISCFFKEY.G4On){
	...
	cout<<"AMSJob::_signinitjob-W-G4LEDATA "<<getenv("G4LEDATA")<<endl;
      ...
      else if(strstr((const char *)G4Version,"geant4-10-03")){
	string add="geant4.10.03-ams";
	g4is.replace(g4is.begin()+pos,g4is.begin()+strlen(g4i),add);
	setenv("G4INSTALL",g4is.c_str(),1);
	cout<<"AMSJob::_signitjob-W-G4INSTALLRedefined "<<getenv("G4INSTALL")<<endl;
	set_geant4_10_03_environment_variables();
...
void AMSJob::_sitof2initjob(){
  ...
  AMSgObj::BookTimer.book("SiTofa");
AMSgObj::BookTimer.book("SiTofb");
AMSgObj::BookTimer.book("SiTofba");
AMSgObj::BookTimer.book("SiTofc");
...
void AMSJob::_siecalinitjob(){
  ...
  //
  if(ECMCFFKEY.TuneMC==1) ECALDBc::init_fiberinfo_table();//wxq
}
//-----------------------------------------------------------------------
...
void AMSJob::_rerichinitjob(){
  ...
  AMSgObj::BookTimer.book("RERICHHITS");
     ...
     sprintf(tmp,"%s/%s/",getenv("AMSDataDir"),AMSCommonsI::getversion());
             RichRingXR::init(tmp,RichRingXR::ideal_mc);
...
void AMSJob::_remfinitjob(){
  ...
  //     char name[150];
  //     //sprintf(name,"%s/%s",AMSDATADIR.amsdatadir,AMSDATADIR.fname);
...
void AMSJob::_reaxinitjob(){
  ...
  AMSgObj::BookTimer.book("ReTKRefitB");
...
void AMSJob::settrigger(char *setup, integer N,integer orr, integer phys){
  assert(N < maxtrig);
  if(setup){
...
void AMSJob::_timeinitjob(){
...

  {
    ...
    */
    (void) ssize;
    {
      ...
      time_t begin,end;
      AMSTimeID *ptdv;
      if(strcmp(getsetup(),"AMS02P") && strcmp(getsetup(),"AMS02P.2")){
  ...
  int trcalib=(CALIB.SubDetRequestCalib/100)%10;//std: trcalib=0, pass: trcalib=1, testbeam(without dynalign): trcalib=2
#ifdef _PGTRACK_
...
//------
    bool usev6=(TRCLFFKEY.recflag/10000000%10>=1);//reconstruction based on V6
    if(trcalib==0 && !usev6) {//std: V6 reconstruction will skip it, only applied to V5 std
      if (TRFITFFKEY.MergeExtLimX < 5) TRFITFFKEY.MergeExtLimX = 5;
      if (TRFITFFKEY.MergeExtLimY < 5) TRFITFFKEY.MergeExtLimY = 5;
    }

//------
    TrLinearEtaDB  *TrLinearEta =TrLinearEtaDB::GetHead();
    TrLinearXEtaDB *TrLinearXEta=TrLinearXEtaDB::GetHead();
    if(isRealData()){
      TID.add ( new AMSTimeID(AMSID(TrLinearEta->TDVName,isRealData()),begin,end,
                              TrLinearEta->GetTDVLength(),
                              TrLinearEta->TDVBlock,
                              server,need,TrLinearEtaDB::HeadLoadTDVPar));
      TID.add ( new AMSTimeID(AMSID(TrLinearXEta->TDVName,isRealData()),begin,end,
                              TrLinearXEta->GetTDVLength(),
                              TrLinearXEta->TDVBlock,
                              server,need,TrLinearXEtaDB::HeadLoadTDVPar));
    }
    TkTrackN *tkv6g=TkTrackN::GetHeadG();//V6 version
    if(isRealData()){if(trcalib==2)tkv6g->UseAlignVersion(-1,10);}//no dynamic alignment for testbeam, std will use latest dynalign extension
    else            {tkv6g->UseAlignVersion(-1,-1);}//no alignment for MC
    for(int ia=0;ia<tkv6g->nalig;ia++){//V6 version
      TkAlignParDB &paligndb=tkv6g->aligndb[ia];
      if(paligndb.TDVVers<0)continue;
      TID.add ( new AMSTimeID(AMSID(paligndb.TDVName[1],isRealData()),begin,end,
                              paligndb.GetTDVLength(),
                              paligndb.TDVBlock,
                              server,need));
    }
//------
...
//-----
if(TKGEOMFFKEY.exalignver<6){
      TrExtAlignDB::CreateLinear();

      int needtrextalig = (trcalib==1)?1:0;//only apply to pass
      ...
      int needtrindzalig = (trcalib==1)?1:0;//only apply to pass
      ...
      DynAlManager::need2bookTDV=(trcalib==1);//only apply to pass
...
else DynAlManager::ignoreAlignment=true;
}
    begin.tm_isdst=0;
    end.tm_isdst=0;    
    begin.tm_sec  =0;
	...
	if(TkDBc::Head->GetSetup()==3||TkDBc::Head->GetSetup()==4){
	  if(TKGEOMFFKEY.alignver==1)
	    ...
	    TID.add (new AMSTimeID(AMSID("TrackerAlignPM4",isRealData()),begin,end,
				   TkDBc::GetLinearSize(),TkDBc::linear,
				   server,need,SLin2Align));
	  else 	  if(TKGEOMFFKEY.exalignver==5 )
    ...
    if (isSimulation()) {
      // new signal database for simulation (April 2017)
      begin.tm_isdst = end.  tm_isdst = 0;
      begin.tm_sec   = begin.tm_min = begin.tm_hour = begin.tm_mday = begin.tm_mon = begin.tm_year = 0;
      end.  tm_sec   = end.  tm_min = end.  tm_hour = end.  tm_mday = end.  tm_mon = end.tm_year   = 0;
      TID.add (new AMSTimeID(AMSID("TrackerSimSignal1",isRealData()),begin,end,
                             TrSimSignalDB::GetLinearSize(),TrSimSignalDB::GetLinear(),
                             server,need,FunctionLinearToSimSignalDB));
    }
...
#else
  {
    ...
    }else if(!strcmp(getsetup(),"AMS02P.2")){
      TID.add (new AMSTimeID(AMSID(AMSTrIdSoft::TrackerPedestals(0),isRealData()),
			     begin,end,sizeof(AMSTrIdSoft::peds[0])*AMSTrIdSoft::_numel,
			     (void*)AMSTrIdSoft::peds,server,need));
      TID.add (new AMSTimeID(AMSID(AMSTrIdSoft::TrackerRawSigmas(0),isRealData()),
  ...
  //
  {
    ...
    if(TFREFFKEY.TFHTDVCalib/10000%100>=10){
      ...
      if(!isRealData()){
        ...
        if(tsfdbv<4)TofTAlign->TDVName="TofAlign2MC";//20140419 MC Calibration. The lastest using TofAlign2 (20231108 MC Calibration)
      }
      ...
      //---Charge Ion Par
      TofCAlignIonPar *TofCAlignIon=TofCAlignIonPar::GetHead(isRealData());
      TofCAlignIon->UpdateVersion(4);
  ...
  { //RICH: TDV reservation for calibration parameters of all 
   ...
   if(trcalib==1 && isRealData()){
    ...
    begin.tm_mon=CHARGEFITFFKEY.mon[0];
       begin.tm_year=CHARGEFITFFKEY.year[1];
      ...
      end.tm_year=CHARGEFITFFKEY.year[0];
AMSTimeID *a=new AMSTimeID(AMSID(TrChargeYJDB::GetHead()->TDVName,isRealData()),begin,end,
TrChargeYJDB::GetHead()->GetTDVLength(),
TrChargeYJDB::GetHead()->TDVBlock,
server,trcalib,TrChargeYJDB::loadtdvpar);
     TID.add(a);
      TrChargeYJDB::GetHead()->tdvload=a;
      AMSTimeID *atime=new AMSTimeID(AMSID(TrChargeYJTimeDB::GetHead()->TDVName,isRealData()),begin,end,
			TrChargeYJTimeDB::GetHead()->GetTDVLength(),
			TrChargeYJTimeDB::GetHead()->TDVBlock,
			server,trcalib,TrChargeYJTimeDB::loadtdvpar);
      TID.add(atime);
      TrChargeYJTimeDB::GetHead()->tdvload=atime;
   }
  ...
  */
  ...
  //PZ FIXME ALIGN
  {
    ...
    if(!strcmp(getsetup(),"AMS02P.2")) ptdv = (AMSTimeID*) TID.add(new AMSTimeID(strcmp(getsetup(),"AMS02P.2")?AMSTrAligFit::getTDVGLDB():AMSTrAligFit::getTDVGLDBP(),begin,end,AMSTrAligFit::gettraliggldbsize(),AMSTrAligFit::gettraliggldbp(),server,TRALIG.UpdateDB?0:1));
  ...
  if(!isRealData()){
    ...
    if(!strcmp(getsetup(),"AMS02P.2")) ptdv = (AMSTimeID*) TID.add(new AMSTimeID(strcmp(getsetup(),"AMS02P.2")?AMSTrAligFit::getTDVAGLDB():AMSTrAligFit::getTDVAGLDBP(),begin,end,AMSTrAligFit::gettraliggladbsize(),AMSTrAligFit::gettraliggladbp(),server,TRALIG.UpdateDB>1?0:1));
  }
...
AMSJob::~AMSJob(){
...
#endif
  delete _pntuple;
  cout <<"AMSJob::~AMSJob-I-pntuple deleted"<<endl;
#ifdef __CORBA__
  ...
  cout << "AMSJob::~AMSJob-I-MPIStatus " << mpi.getMPIStatus() << endl;
if (mpi.getMPIStatus() > 0) {
  cout << "AMSJob::~AMSJob-I-Trying to MPI_Finalize()" << endl;
  mpi.MPI_Finalize();
//  cout << "Trying to delete AMSgOjb::BookTimer" << endl;
//  delete &(AMSgObj::BookTimer);
//  cout << "deleting AMSgOjb::BookTimer completed, now abort() ..." << endl;
//  abort();
}
#endif
}
...
void AMSJob::urinit(integer run, integer eventno, time_t tt)
  ...
  if(IOPA.WriteRoot){
    ...
    if(eventno){
      char event[80];  
      if(isProduction()){
	int i=system_bg::system((const char*)mdir);
	if(i){
	  ...
	  if(getenv("NtupleDir") && !strstr((const char*)mdir,getenv("NtupleDir"))){
	    ...
	    mdir+=_rootfilename;
        system_bg::system((const char*)mdir);
	  }
	}
