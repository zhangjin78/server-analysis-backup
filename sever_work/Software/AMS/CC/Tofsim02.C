//  $Id: Tofsim02.C,v 1.33 2024/01/19 14:56:10 qyan Exp $
...
#ifdef __PPC64
map<integer,TH1F*> *TOF2TovtN::phmap[65]={0};
#else
map<integer,TH1F*> *TOF2TovtN::phmap=0;
#endif
...
void TOF2TovtN::covtoph(integer idsoft, geant vect[], geant edep,geant tofg, geant tofdt,geant stepl,integer parentid){
 ...
 cerr <<"TOF2TovtN::covtoph-E-PointerIzZeroTo "<<  AMSID(vname,id)<<" "<<p<<" "<<cglo<<" "<<kk<<" "<<id<<" "<<ilay<<" "<<ibar<<" "<<AMSEvent::gethead()->get_thread_num()<<endl;
  ...
  nel0=edep1*convr;
  if(!phmap)phmap=new map<integer, TH1F*>;
...
//----Merge TOFMCCluster
   AMSContainer * cont=AMSEvent::gethead()->getC("AMSTOFMCCluster",0);
   if(((G4FFKEY.MergeMCCluster%10)>=1)&&cont){//merge the same geom and g4trkid
     AMSTOFMCCluster *sptr=0;
     vector<AMSTOFMCCluster *>tofmccl;
     while (ptr){
       if((sptr==0)||(ptr->idsoft!=sptr->idsoft)||(ptr->gtrkid!=sptr->gtrkid)){
         sptr=new AMSTOFMCCluster(*ptr);
         tofmccl.push_back(sptr);
       }
       else {
         sptr->edep +=ptr->edep;
         sptr->edepr+=ptr->edepr;
         sptr->step +=ptr->step;
       }
       ptr=ptr->next();
     }
//---second merge
     if((G4FFKEY.MergeMCCluster%10)>=2){
       for(integer ii=integer(tofmccl.size())-1;ii>=0;ii--){//from end to begin
         ptr=tofmccl[ii];
         integer pid=ptr->particle;
         if(pid!=1&&pid!=2&&pid!=3)continue;//only merge gamma/e+/e-
         for(integer jj=ii-1;jj>=0;jj--){//front
           sptr=tofmccl[jj];
           if (ptr->idsoft!=sptr->idsoft||ptr->parentid>sptr->gtrkid){//different idsoft || id is before
             break;
           }
           else if(ptr->parentid==sptr->gtrkid){//just match id (only merge child to parent)
             number dx=ptr->xcoo[0]-sptr->xcoo[0];
             number dy=ptr->xcoo[1]-sptr->xcoo[1];
             number dr=sqrt(dx*dx+dy*dy);
             if(dr<1&&ptr->step<1){//only merge r<1cm&&path<1cm
               sptr->edep +=ptr->edep;
               sptr->edepr+=ptr->edepr;
               delete ptr;
               tofmccl.erase(tofmccl.begin()+ii);
             }
             break;
           }
         }
...
//---
     cont->eraseC();
     for(uinteger i=0;i<tofmccl.size();i++){
       cont->addnext(tofmccl[i]);
     }
     ptr=(AMSTOFMCCluster *)cont->gethead();
  }
...
//        cout<<"id="<<id<<endl;
//--for pmt bin
        const uinteger nBinsX = (*phmapiter).second->GetNbinsX();
        const float* dataArray = (*phmapiter).second->GetArray();
        for(ii=1;ii<=nBinsX;ii++){
           if(dataArray[ii]<1)continue;
           nph=dataArray[ii];
           nphoton+=nph;
//------sum pulse
...
//--ready  reset histos
     for(phmapi phmapiter=phmap->begin(); phmapiter!=phmap->end(); ++phmapiter){
      if(phmapiter->second->GetEntries()>0){
#ifdef __PPC64
          if(phmapiter->second->fIntegral){
              cerr<<"Tofsim02::build-E-IntegralNonZero "<<endl;
              phmapiter->second->fIntegral=0;
          }
#endif   
...
//                                      (Guido: ACTEL-inp is faster than Discr, so no ACTEL-inp DT check)
    daqp10=TOF2DBc::daqpwd(10)*TFMCFFKEY.dcnoise;// generic discr. intrinsic accuracy
  }
