// $Id: TrTrack.C,v 1.306 2025/01/31 11:34:21 nbelyaev Exp $
		     ...
		     float mass, float chrg, float beta, float fixrig, int ualign)
{
  ...
  // External fit
  if ((TkDBc::Head->GetSetup()==3||TkDBc::Head->GetSetup()==4) && (id & kExternal) ){
    int idx2[4];
...
int  TrTrackR::iTrTrackPar(int algo, int pattern, int refit, float mass, float  chrg, float beta, float fixrig){
  ...
  if(refit/100==0){
	  int rf1=refit%10;
	  if(first++<10 &&(refit/10)%10!=3)cout <<" TrTrackR::iTrTrackPar-W-refit changed to pass8 alignment "<<30+rf1<<" use 100+refit to change this behaviour"<<endl;
  refit=30+rf1;
  }
  else if(refit/1000==0)refit=refit-100;
