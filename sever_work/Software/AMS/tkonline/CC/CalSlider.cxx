...
CalSlider::CalSlider(char *name,char *title,int xsize,int ysize) : SliderI(name,title,xsize,ysize){
  ...
  char refname[200];
  char refpath[256];
  if (getenv("AMSWD")!=NULL) {
    sprintf(refpath, "%s/tkonline/refs/", getenv("AMSWD"));
    //    printf("refpath is %s\n", refpath);//only for debug
  }
  else {
    sprintf(refpath, "../tkonline/refs/");
    //    printf("refpath is %s\n", refpath);//only for debug
  }
  sprintf(refname,"%s/CalDB_1302257285.root", refpath);
...
int CalSlider::try2Draw(TrLadCal *cc){
  if(cc && cc->IsFilled()){
    ...
    if (canvas && (gROOT->GetListOfCanvases()->FindObject(canvas))) {
      canvas->Clear();
      if(!label)
...
void CalSlider::setRootFile(char *filename, bool kDoNotTryToDraw){
  ...
  if(calit_cur->second->IsFilled())
    if (!kDoNotTryToDraw) try2Draw(calit_cur->second);
  else
    showNext();
}
