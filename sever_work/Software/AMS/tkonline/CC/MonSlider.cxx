...
void MonSlider::setRootFile(char *filename, bool kDoNotTryToDraw){
  // closing
  if (rootfile!=0) { 
    rootfile->Close(); 
    rootfile = 0; 
  ...
  */
  if (!kDoNotTryToDraw) try2Draw(ladder);
}
...
void MonSlider::TestAndDrawText(int side, TH1D* profile, float xmin, float xmax, int alternative) {
  int nbads=0;
  int ndeads=0;
  for (int ibin=1; ibin<=profile->GetNbinsX(); ibin++) {
    TkLadder* ladder=NULL;
    if (alternative==0) {
      ...
      int slot=(ibin+25)%50;
      int layer=(ibin+25-slot)/50;
      int _tkid = (slot/fabs(slot))*(layer*100+fabs(slot));
      ladder = TkDBc::Head->FindTkId(_tkid);
      if (ladder) {
	if (layer!=ladder->GetLayer() || slot!=ladder->GetSlot()) {
	  printf("Wrong TkID) %d!=%d || %d!=%d\n", 	layer, ladder->GetLayer(), slot, ladder->GetSlot());
	}
      }
    }
    ...
    if ( (x<xmin)||(x>xmax) ) {
      ...
      double xdraw = 0;
      gPad->Update();
      //      printf("x=%f, xmin=%f, xmax=%f, Y1=%f, Y2=%f\n", x, xmin, xmax, gPad->GetFrame()->GetY1(), gPad->GetFrame()->GetY2());
      if (x<xmin) {
	xdraw=std::max(x, (float)(gPad->GetFrame()->GetY1()+0.001));
	ndeads++;
      }
      if (x>xmax) {
	xdraw=std::min(x, (float)(gPad->GetFrame()->GetY2()-0.026));
	nbads++;
      }
      //      printf("ibin=%d (%f), xdraw=%f, tkid=%s\n", ibin, profile->GetXaxis()->GetBinUpEdge(ibin), xdraw, Form("%+03d",tkid));
      text->DrawText(profile->GetXaxis()->GetBinUpEdge(ibin), xdraw, Form("%+03d",tkid));
    }
  }
  
  float textsize = text->GetTextSize();
  printf("%s %d %d) %f %f %f, %f\n", profile->GetName(), side, alternative, gPad->GetHNDC(), gPad->GetFrame()->GetY2(), gPad->GetFrame()->GetY1(), gPad->GetWNDC());
  text->SetTextSize(0.025/gPad->GetHNDC());
  //  double _X = 1.0-0.25/gPad->GetWNDC();
  double _X = 0.55;
  if (side==0) _X-=0.255/gPad->GetWNDC();
  text->DrawTextNDC(_X, 0.95, Form("Number of {\"bad\",\"dead\"} ladders = {%d,%d}", nbads, ndeads));
...
void MonSlider::DrawSignalSummary(int alternative) {
  ...
  signal_n_prof->SetYTitle("Average Cluster Signal (ADC)");
  signal_p_prof->SetYTitle("Average Cluster Signal (ADC)");
  ...
  TestAndDrawText(0,signal_n_prof,10,70, alternative);
  TestAndDrawText(1,signal_p_prof,10,70, alternative);
  ...
  width_n_prof->GetYaxis()->SetRangeUser(0.,10.);
  width_n_prof->Draw();
  width_p_prof->Draw("SAME");
  TestAndDrawText(0,width_n_prof,1, 5, alternative);
  TestAndDrawText(1,width_p_prof,1, 5, alternative);
...
void MonSlider::DrawSizeSummary(int alternative) {
  ...
  TestAndDrawText(0,size_n_prof,1,5, alternative);
  TestAndDrawText(1,size_p_prof,1,5, alternative);
  canvas->Update();
}
...
void MonSlider::DrawDtSummary(int alternative) {
  ...
  TestAndDrawText(0,sizelowdt_n_prof,0.1,100, alternative);
  TestAndDrawText(1,sizelowdt_p_prof,0.1,100, alternative);
  canvas->Update();
}
...
void MonSlider::DrawClustersSummary(int alternative) {
  ...
  pad1->SetGridy();
  pad1->SetGridx();
  nraw_vs_ladder_n_prof->GetYaxis()->SetRangeUser(0.,0.6);
  nraw_vs_ladder_n_prof->Draw();
  nraw_vs_ladder_p_prof->Draw("SAME");
  // gPad->Update();
  // printf("Frame=[%f,%f]\n", gPad->GetFrame()->GetY1(), gPad->GetFrame()->GetY2());
  TestAndDrawText(0,nraw_vs_ladder_n_prof,0.00001,0.5, alternative);
  TestAndDrawText(1,nraw_vs_ladder_p_prof,0.00001,0.5, alternative);
  ...
  pad2->SetGridy();
  pad2->SetGridx();
  cluontrk_vs_ladder_n_prof->GetYaxis()->SetRangeUser(0.,0.1);
  cluontrk_vs_ladder_n_prof->Draw();
  cluontrk_vs_ladder_p_prof->Draw("SAME");
  TestAndDrawText(0,cluontrk_vs_ladder_n_prof,0.00001,0.08, alternative);
  TestAndDrawText(1,cluontrk_vs_ladder_p_prof,0.00001,0.08, alternative);
  canvas->Update();
}
