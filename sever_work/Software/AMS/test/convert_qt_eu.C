...
//  today only
 ...
 for( int i=1;i<argc;i++){
   ...
   else if(strstr(argv[i],"lastquarter")){
    time_t timenow;
    time(&timenow);
    strftime(tmp,80,"%Y",gmtime(&timenow));
    y_max=atol(tmp);
   ...
   else if(strstr(argv[i],"lastsemester")){
    time_t timenow;
    time(&timenow);
    strftime(tmp,80,"%Y",gmtime(&timenow));
    y_max=atol(tmp);
   ...
   else if(strstr(argv[i],"--date=")&& strstr(argv[i],"_") && strlen(argv[i])==15){
    char tmp[5];
    strncpy(tmp,argv[i]+7+1+4,3);
    d_min=atol(tmp);
    d_max=d_min;
    strncpy(tmp,argv[i]+7,4);
    y_min=atol(tmp);
    y_max=y_min;

   }
   else{
     cerr<<"main-F-AvailableOptions are --lastweek --yesterday --lastmonth --lastquarter --lastsemester  --date=YYYY_DDD NULL==today"<<endl;
     return 1;
   }
 }
char *ad=getenv("AMSDataDir");
char *adw=getenv("AMSDataDirRW");
if(!ad){
