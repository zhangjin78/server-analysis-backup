...
void DAF::processTrackWithRep(Track* tr, const AbsTrackRep* rep, bool resortHits) {
  ...
  for(unsigned int iBeta = 0;; ++iBeta) {
    ...
    int nzero = 0;
    try{
      converged = calcWeights(tr, rep, betas_.at(iBeta),nzero );
...
bool DAF::calcWeights(Track* tr, const AbsTrackRep* rep, double beta, int &nzero) {
