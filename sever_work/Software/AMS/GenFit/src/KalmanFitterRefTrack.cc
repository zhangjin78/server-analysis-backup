...
void KalmanFitterRefTrack::processTrackWithRep(Track* tr, const AbsTrackRep* rep, bool resortHits)
{
  ...
  double oldRigvFW(0), oldRigvBW(0);
  double RigvFW(0),    RigvBW(0);
  double eRigvBW(0),   oldeRigvBW(0);
  int nFailedHits(0);
  ...
  for (;;) {
    ...
    try {
      ...
      if (deltaRigv_<1 && lastProcessedPoint != nullptr) {
        KalmanFitterInfo* lastInfo = static_cast<KalmanFitterInfo*>(lastProcessedPoint->getFitterInfo(rep));
        KalmanFittedStateOnPlane kfsop(*(static_cast<KalmanFitterInfo*>(lastInfo)->getForwardUpdate()));
        RigvFW=kfsop.getState()[0];//1/R
        if (debugLvl_ > 0) {
           debugOut <<"forward fit"<<" niter" <<nIt<<" nrig="<<(1./RigvFW) << "\n";
        }
      }
      ...
      if (deltaRigv_<1 && lastProcessedPoint != nullptr) {
        KalmanFitterInfo* lastInfo = static_cast<KalmanFitterInfo*>(lastProcessedPoint->getFitterInfo(rep));
        KalmanFittedStateOnPlane kfsop(*(static_cast<KalmanFitterInfo*>(lastInfo)->getBackwardUpdate()));
        const TVectorD&  state0 = kfsop.getState();
        const TMatrixDSym& cov0 = kfsop.getCov();
        RigvBW = state0[0];//1/R
        eRigvBW = (ndfBW<=0)? 0: cov0[0][0];//1/R error
        if (debugLvl_ > 0) {
           debugOut <<"backward fit"<<" niter" <<nIt<<" nrig="<<(1./RigvBW) <<" erig="<<eRigvBW<< "\n"; 
        }
      }
      ...
      double PvalFW = (deltaFBWPval_<1 || debugLvl_ > 0) ? std::max(0.,ROOT::Math::chisquared_cdf_c(chi2FW, ndfFW)) : 0; // Don't calculate if not debugging as this function potentially takes a lot of time.
      ...
      if (debugLvl_ > 0) {
        ...
        debugOut << "old Rigvs: " << oldRigvBW <<", " << oldRigvFW
            << " new Rigvs: " << RigvBW <<", " << RigvFW << std::endl;
        debugOut << "old eRigvs: " << oldeRigvBW
            << " new eRigvs: " << eRigvBW << std::endl;
      }
...
//      bool convergepval2=(deltaFBWPval_>=1 || fabs(PvalBW - PvalFW)  < deltaFBWPval_);
//      bool convergerigv =(deltaRigv_>=1    || fabs(1.-oldRigvBW/RigvBW) < deltaRigv_);
      bool convergepval2=(deltaFBWPval_>=1 || (fabs(PvalBW - PvalFW)  < deltaFBWPval_ && fabs(oldPvalFW - PvalFW)  < deltaPval_));//QY
      bool convergerigv =(deltaRigv_>=1    || (fabs(1.-oldRigvBW/RigvBW) < deltaRigv_ && fabs(1.-oldRigvFW/RigvFW) < deltaRigv_));//QY
      bool convergerigv2=(deltaRigv_>=1    || (oldeRigvBW==eRigvBW || fabs(1.-oldeRigvBW/eRigvBW) < deltaRigv_));//fitting err QY
      bool converged(false);
      bool finished(false);
//      minIterations_=6;
      if (nIt >= minIterations_ && fabs(oldPvalBW - PvalBW) < deltaPval_ && convergepval2 && convergerigv && convergerigv2)  {
      ...
      if (finished) {
        ...
        oldRigvFW = RigvFW;
        oldeRigvBW= eRigvBW;
        if (deltaFBWPval_<1 || debugLvl_ > 0) {
          oldPvalFW = PvalFW;
          oldChi2FW = chi2FW;
        }
      }
      ...
      if(debugLvl_ > 0)errorOut << e.what();//Qi Yan
...
KalmanFitterRefTrack::processTrackPoint(KalmanFitterInfo* fi, const KalmanFitterInfo* prevFi, const TrackPoint* tp, double& chi2, double& ndf, int direction)
{
  ...
  double chi2inc = 0;
  double ndfInc = 0;
  double chi2inuv[2] = {0,0};
  double ndfInuv[2]  = {0,0};
  ...
  for (std::vector<MeasurementOnPlane *>::const_iterator it = measurements.begin(); it != measurements.end(); ++it) {
    ...
    const int dim=5;//QY
    const double au[dim] = {0, 0, 0, 1, 0};//Projection to U
    const double av[dim] = {0, 0, 0, 0, 1};//Projection to V
    TVectorD vu(H->Hv(TVectorD(dim,au)));
    TVectorD vv(H->Hv(TVectorD(dim,av)));
    // (weighted) cov
    ...
    if (res_ != 0) {
      ...
      if (couldInvert) {
        ...
        TVectorD resu(vu), resv(vv);
        for(int i=0;i<res_.GetNrows();i++){resu[i]=res_[i]*vu[i]; resv[i]=res_[i]*vv[i];}
        chi2inc += Rinv_.Similarity(res_);
        chi2inuv[0] += Rinv_.Similarity(resu);
        chi2inuv[1] += Rinv_.Similarity(resv);
      }
    }
    ...
    if (!canIgnoreWeights()) {
      ...
      for(int i=0;i<vu.GetNrows();i++){if(vu[i]!=0)ndfInuv[0]+=m.getWeight();}
      for(int i=0;i<vv.GetNrows();i++){if(vv[i]!=0)ndfInuv[1]+=m.getWeight();}
    }
    else{
      ndfInc += m.getState().GetNrows();
      for(int i=0;i<vu.GetNrows();i++){if(vu[i]!=0)ndfInuv[0]++;}
      for(int i=0;i<vv.GetNrows();i++){if(vv[i]!=0)ndfInuv[1]++;}
    }
  ...
  upState->setChiSquareIncrementuv(chi2inuv);
  upState->setNdfuv(ndfInuv);
  upState->setAuxInfo(fi->getReferenceState()->getAuxInfo());
  fi->setUpdate(upState, direction);


  if (debugLvl_ > 0) {
    debugOut << " chi² inc " << chi2inc <<"," <<chi2inuv[0] <<"," <<chi2inuv[1]<< "\t";
    debugOut << " ndf inc  " << ndfInc  <<"," <<ndfInuv[0]  <<"," <<ndfInuv[1] << "\t";
