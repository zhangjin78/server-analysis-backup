...
TGeoMaterialInterface::~TGeoMaterialInterface(){
}

TGeoMaterialInterface::TGeoMaterialInterface(){
}
bool TGeoMaterialInterface::initTrack(double posX, double posY, double posZ,
                                   double dirX, double dirY, double dirZ){
  #ifdef DEBUG
  debugOut << "TGeoMateriaglInterface::initTrack. \n";
  debugOut << "Pos    "; TVector3(posX, posY, posZ).Print();
  debugOut << "Dir    "; TVector3(dirX, dirY, dirZ).Print();
  #endif

  // Move to the new point.
  bool result = !gGeoManager->IsSameLocation(posX, posY, posZ, true);
