    ...
    const AbsTrackRep* rep, double& chi2, double& ndf, int direction)
{
  ...
  if (!squareRootFormalism_) {
    ...
    for (std::vector<MeasurementOnPlane *>::const_iterator it = measurements.begin(); it != measurements.end(); ++it) {
      ...
      for(int i=0;i<resNew.GetNrows();i++){resu[i]=resNew[i]*vu[i]; resv[i]=resNew[i]*vv[i];}
      chi2inc += HCHt.Similarity(resNew);
      chi2inuv[0] += HCHt.Similarity(resu);
      chi2inuv[1] += HCHt.Similarity(resv);

      if (!canIgnoreWeights()) {
        ndfInc += weight * measurement.GetNrows();
        for(int i=0;i<vu.GetNrows();i++){if(vu[i]!=0)ndfInuv[0]+=weight;}
        for(int i=0;i<vv.GetNrows();i++){if(vv[i]!=0)ndfInuv[1]+=weight;}
      }
      else {
      ...
      if (debugLvl_ > 0) {
        debugOut << "chi² increment = " << chi2inc <<"," <<chi2inuv[0] <<"," <<chi2inuv[1]<< std::endl;
      }
    ...
    for (std::vector<MeasurementOnPlane *>::const_iterator it = measurements.begin(); it != measurements.end(); ++it) {
      ...
      TVectorD resu(vu), resv(vv);
      for(int i=0;i<res.GetNrows();i++){resu[i]=res[i]*vu[i]; resv[i]=res[i]*vv[i];}
  ...
  updatedSOP->setChiSquareIncrementuv(chi2inuv);
  updatedSOP->setNdfuv(ndfInuv);
