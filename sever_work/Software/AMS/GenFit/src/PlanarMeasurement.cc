...
bool PlanarMeasurement::isValidCalib(){
  return ((gmalignders_.first.size()>=1)&&(int(gmalignders_.first.size())==gmalignders_.second.GetNcols())&&(gmalignders_.second.GetNrows()==3));
}


TMatrixD PlanarMeasurement::globalDerivatives(const StateOnPlane* sop){
  const TVectorD& state5=sop->getState();
  TMatrixD Pv(2,3);
  Pv.Zero();
  Pv(0,0)=-1;Pv(0,2)=state5[1];//dudw
  Pv(1,1)=-1;Pv(1,2)=state5[2];//dvdw

  TMatrixD gders(2,gmalignders_.second.GetNcols());
  gders=Pv*gmalignders_.second;
  return gders; 
}
