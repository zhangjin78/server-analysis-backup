...
namespace genfit {
#ifdef JUQUEEN
MaterialEffects* MaterialEffects::instance_[65] = {};
#else
MaterialEffects* MaterialEffects::instance_=0;
#endif
...
{
#ifdef JUQUEEN
    if (!instance_[AMSEvent::get_thread_num()] ) instance_[AMSEvent::get_thread_num()] = new MaterialEffects();
  return instance_[AMSEvent::get_thread_num()];
#else
...
{
#ifdef JUQUEEN
    if (instance_[AMSEvent::get_thread_num()] ) {
    delete instance_[AMSEvent::get_thread_num()];
    instance_[AMSEvent::get_thread_num()] = 0;
  }
#else
...
//#pragma omp critical (genfitstepper)
{ 
...
void MaterialEffects::setParticleMassCharge(double mass, double charge)
{
 charge_=charge;
 mass_=mass;
}
...
void MaterialEffects::drawdEdx(int pdg) {
  ...
  TFile outfile("dEdx_" + TString(Result.c_str()) + ".root", "recreate");
  outfile.cd();
  hdEdxBethe.Write();
  hdEdxBrems.Write();
  outfile.Close();
