//  $Id: AMSNtupleV.h,v 1.66 2025/05/16 20:07:36 choutko Exp $
...
protected:
  ...
  int recType;
public:
  TrTrackV():AMSDrawI(NULL,-1),TPolyLine3D(){rtype = -1;recType = 0;};
  TrTrackV(AMSEventR *ev,int ref,bool drawinter=true):AMSDrawI(ev,ref),TPolyLine3D(){
    if (ev==0) return;
    rtype = 0;
    TrTrackR *pcl=ev->pTrTrack(ref);
    if(pcl){
      pcl->ReFitHitStat();
      recType = pcl->GetRecType();
...
//	    hh->BuildCoordinates();
          ...
          AMSDir dir;
          if(drawinter)pcl->Interpolate(aa[2],aa,dir);
	...
	if((bit & (1<<ii))>0){
	  if(TrRecHitR* hh=pcl->GetHitLO(ii+1)){
	    AMSPoint aa=hh->GetCoord();
//	    cout <<"Hit Point"<<aa<<endl;
	    if (!(hh->OnlyY () && hh->GetDummyX()==-1)){
             AMSDir dir;
             if(drawinter)pcl->Interpolate(aa[2],aa,dir);
//  	     cout <<"HitA Point"<<aa<<endl;
	      SetPoint(ind++,aa[0],aa[1],aa[2]);
            }
	  }
	}
  ...
  char * GetObjectInfo(Int_t px, Int_t py) const{
    return fRef>=0?const_cast<char*>(fEv->pTrTrack(fRef)->Info(fRef)):0;
  }
...
protected:
  int rtype;
public:
  TrTrackV():AMSDrawI(NULL,-1),THelix(){ rtype = -1; };
...
//      Double_t Range[2]={-75,75};
  ...
  char * GetObjectInfo(Int_t px, Int_t py) const{
    return fRef>=0?fEv->pTrTrack(fRef)->Info(fRef):0;
  }
...
class RichRingXV: public TPolyLine3D, public AMSDrawI{
	protected:
	public:
		  RichRingXV():AMSDrawI(NULL,-1),TPolyLine3D(){};
		    RichRingXV(AMSEventR *ev,int ref, bool drawplex=false);
		      char * GetObjectInfo(Int_t px, Int_t py) const;
