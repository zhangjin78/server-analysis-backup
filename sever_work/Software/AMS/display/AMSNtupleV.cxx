//  $Id: AMSNtupleV.cxx,v 1.66 2025/05/27 17:27:29 choutko Exp $
...
void* gAMSUserFunction;
  ...
  char * RichRingXV::GetObjectInfo(Int_t px, Int_t py) const{
	     return fRef>=0 && fEv->pRichRingX(fRef)?fEv->pRichRingX(fRef)->Info(fRef):0;
  }
...
{
	 int cand=-1;
	  for(int i=0;i<fRichRingXV.size();i++){
		     int current=fRichRingXV[i].DistancetoPrimitive(px,py);
		       if(abs(current)<abs(dist)){
			          dist=current;
				     cand=i;
				       }
	   ...
	   if(dist<7 && cand>=0){
		      ...
		      info=fRichRingXV[cand].GetObjectInfo(px,py);
		       }
}
...
void AMSNtupleV::Prepare( EAMSType type){
  ...
  if(type==kall || type==kusedonly || type==kanticlusters){
	...
	if (!gAMSDisplay->DrawUsedOnly() && pAntiCluster(i)) {
#ifdef __USEANTICLUSTERPG__
 ...
 if(gAMSDisplay->DrawObject(ktrtracks)){
  for(int i=0;i<NTrTrack();i++){
    ...
    if(!gAMSDisplay->DrawUsedOnly() || ((pTrTrack(i)->getstatus())/32)%2){
      fTrTrackV.push_back( TrTrackV(this,i,gAMSDisplay->DrawTrTracksInter()));
    }
#else
...
if(Version()<1406){
	  if(!RichRingXR::create)CreateRichRingX(-1,40);
          RichRingXR::create=-1;
          RichRingXR::mthr=40;
          RichRingXR::dphi=4000;
   
}
}
else RichRingXR::create=0;
for(int i=0;i<NRichRing();i++){
   ...
   for(int i=0;i<NRichRingX();i++){
	      if( (!gAMSDisplay->DrawUsedOnly() || ((pRichRingX(i)->Status)/32)%2)){
if(gAMSDisplay->DrawRichRingX()){
		      fRichRingXV.push_back( RichRingXV(this,i,false));
			      if(gAMSDisplay->DrawRichRingsFromPlex())fRichRingXV.push_back( RichRingXV(this,i,true));
}
}
	        }
...
RichRingXV::RichRingXV(AMSEventR *ev,int ref, bool drawplex):AMSDrawI(ev,ref),TPolyLine3D(){
 RichRingXR *pcl=ev->pRichRingX(ref);
...
for(int i=0;i<ev->nParticle();i++){
 if( ev->pParticle(i)->iRichRingX() == ref){
   pp=ev->pParticle(i);
   break;
}
}
if(pp){
   ...
   double theta=acos(1/pcl->RI/pcl->Beta);

    TVector3 z(pp->RichCoo[1][0]-pp->RichCoo[0][0],pp->RichCoo[1][1]-pp->RichCoo[0][1],pp->RichCoo[1][2]-pp->RichCoo[0][2]);
// TVector3 z(sin(pcl->Theta)*sin(pcl->Phi),sin(pcl->Theta)*cos(pcl->Phi),cos(pcl->Theta));
   double rcoo[3];
       TRotation r;
           r.SetZAxis(z);
 ...
 double posz=-2+rad_thick*(float(rand())/RAND_MAX-0.5);
    double thick=rad_thick*0.5-posz;
        rcoo[0]=pp->RichCoo[0][0]+z.X()/z.Z()*posz;
	    rcoo[1]=pp->RichCoo[0][1]+z.Y()/z.Z()*posz;
	        rcoo[2]=pp->RichCoo[0][2]+posz;
		    phi+=dphi;
		          ...
		          ray=TVector3(u1,v1,w1);
			       double addon=4;
					    ...
					    double rc=sqrt(xc*xc+yc*yc)+r2+(rcoo[2]-z2)*tc;
						    ...
						    double zl=pp->RichCoo[1][2]-rcoo[2]+addon;
						        array[k*3+0]=rcoo[0]+ray.X()/ray.Z()*zl;
							    array[k*3+1]=rcoo[1]+ray.Y()/ray.Z()*zl;
							        array[k*3+2]=pp->RichCoo[1][2]+addon;
								    rp=sqrt(array[k*3+0]*array[k*3+0]+array[k*3+1]*array[k*3+1]);
									    ...
									    const int miter=1000;
										    ...
										    while(rp>rc  && ray.Z()<0 && iter++<miter){
											          zl=pp->RichCoo[1][2]-rcoo[2]+addon;
						        ...
						        double rv=ray.Y()-2*cc*cv;
							     double rw=ray.Z()-2*cc*cw;
							          ray=TVector3(ru,rv,rw);
								       array[k*3+0]=rcoo[0]+ru/rw*(pp->RichCoo[1][2]+addon-rcoo[2]);
								            array[k*3+1]=rcoo[1]+rv/rw*(pp->RichCoo[1][2]+addon-rcoo[2]);
									         rp=sqrt(array[k*3+0]*array[k*3+0]+array[k*3+1]*array[k*3+1]);
										     }
										         if(ray.Z()>0){
												             k--;
