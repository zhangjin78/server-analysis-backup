//  $Id: ControlFrame.cxx,v 1.26 2025/05/27 09:30:43 choutko Exp $
...
//     Control
    ...
    case kCM_MENU:
      ...
      case 105:
        ...
        break;
      case 106:
        gAMSDisplay->DrawTrTracksInter()=!gAMSDisplay->DrawTrTracksInter();
        fOptionMenu->DeleteEntry(106);
        if(gAMSDisplay->DrawTrTracksInter()){
         fOptionMenu->AddEntry("Do Not Draw &TrTracks By Inerpolation",106);
        }
        else   fOptionMenu->AddEntry("Draw &TrTracks By Inerpolation",106);
        gAMSDisplay->GetNtuple()->Prepare(ktrtracks);
        gAMSDisplay->SetView(gAMSDisplay->GetView());
        gAMSDisplay->DrawTitle();
        break;
      case 107:
        gAMSDisplay->ReadFromList()=!gAMSDisplay->ReadFromList();
        fOptionMenu->DeleteEntry(107);
        if(gAMSDisplay->ReadFromList()){
         fOptionMenu->AddEntry("&Not List Read",107);
        }
        else fOptionMenu->AddEntry("&List Read",107);
        gAMSDisplay->SetView(gAMSDisplay->GetView());
        gAMSDisplay->DrawTitle();
        break;
  case 108:
        gAMSDisplay->DrawRichRingX()=!gAMSDisplay->DrawRichRingX();
	fOptionMenu->DeleteEntry(108);
	if(gAMSDisplay->DrawRichRingX()){
	  fOptionMenu->AddEntry("Do Not Draw Richring&X",108);

       }
       else   fOptionMenu->AddEntry("Draw RichRing&X",108);
       gAMSDisplay->GetNtuple()->Prepare(krichrings);
       gAMSDisplay->SetView(gAMSDisplay->GetView());
       gAMSDisplay->DrawTitle();
       break;
...
AMSControlFrame::AMSControlFrame(const TGWindow *p, const TGWindow *main,
  ...
  TGTransientFrame(p, main, w, h, options){
    ...
    fOptionMenu->AddEntry("Draw &TrTracks by Interpolation",106);
    fOptionMenu->AddEntry("&List Read",107);
    fOptionMenu->AddEntry("Draw RichRing&X",108);
...
//    _pvis.push_back(new TGCheckButton(_pvisfr,"TrClustersAsBoxes",marker++));
    ...
    for(int i=0;i<_pvis.size();i++){
     ...
     gAMSDisplay->SetVisible(ktrdhtracks,_pvis[2]->GetState());
     gAMSDisplay->SetVisible(ktrdclusters,_pvis[3]->GetState());
     gAMSDisplay->SetVisible(kanticlusters,_pvis[4]->GetState());
     gAMSDisplay->SetVisible(ktofclusters,_pvis[5]->GetState());
     gAMSDisplay->SetVisible(ktrtracks,_pvis[6]->GetState());
     gAMSDisplay->SetVisible(ktrclusters,_pvis[7]->GetState());
     gAMSDisplay->GetNtuple()->SetTkMult(_pvis[8]->GetState());    //tkhit mult
     gAMSDisplay->GetNtuple()->SetTkDispY(_pvis[9]->GetState());  //tkhit yonly
     gAMSDisplay->SetVisible(krichrings,_pvis[10]->GetState());
     gAMSDisplay->SetVisible(krichhits,_pvis[11]->GetState());
     gAMSDisplay->SetVisible(kecalshowers,_pvis[12]->GetState());
     gAMSDisplay->SetVisible(kecalclusters,_pvis[13]->GetState());
     gAMSDisplay->SetVisible(kparticles,_pvis[14]->GetState());
     gAMSDisplay->SetVisible(kmcinfo,_pvis[15]->GetState());
     gAMSDisplay->SetVisible(kgeometry,_pvis[16]->GetState());
