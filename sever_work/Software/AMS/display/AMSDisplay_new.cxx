//  $Id: AMSDisplay_new.cxx,v 1.31 2025/05/16 20:07:35 choutko Exp $
//////////////////////////////////////////////////////////////////////////
//                                                                      //
// AMSDisplay                                                           //
//                                                                      //
...
AMSDisplay::AMSDisplay(const char *title, TGeometry * geo, AMSChain * chain, int sec, bool monit,bool scanout):
  m_chain(chain), m_sec(sec),m_nodate(false),TObject(),m_chain_Entries(0){
     ...
     m_drawrichringx=false;
     m_drawtrtrackinter=true;
     m_rebuildacc=false;
     m_orderedread=false;
     m_listread=false;
     m_seq=0;
 ...
 // Set front view by default
  ...
  for(int i1=1;i1<m_chain->GetListOfFiles()->GetEntries();i1++){
      ...
      if(tevt)m_chain_Runs.push_back(tevt->Run());
...
void AMSDisplay::DrawAxis(Int_t index, Option_t * option)
{
   ...
   TView3D *eventView = (TView3D*)gPad->GetView();
...
//    Draw current view using OPENGL
   ...
   pad->cd();
   TView3D *view = (TView3D*)pad->GetView();
   if (!view) return;
   pad->x3d("OPENGL");
}
...
bool AMSDisplay::ShowNextEvent(Int_t delta){
  ...
  if(ReadOrdered()){
    m_seq+=delta;
    entry=m_chain->GetEntryNo(m_seq);
    //   cout <<"  delta seq "<<m_seq<<" "<<delta<<endl;
  } else if (ReadFromList()){
    entry=m_chain->GetEntryFromList(delta);
  }else
    entry=m_chain->Entry()+delta;
