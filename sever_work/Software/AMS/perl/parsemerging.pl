...
my $sql = "select fevent,levent,nevents,sizemb from amsdes.datafiles where run = ? and (status = 'OK' or status like '%TTCS%')";
...
foreach my $jou (@journals) {
    ...
    chomp $tag;
    $tag =~ s/Tag=//g;
    my $size = (grep /Size=/, @buf)[0];
    chomp $size;
    $size =~ s/Size=//g;
    $size = int($size/2);
    $sth->execute($run);
    my ($ofe, $ole, $one, $osize);
    $sth->bind_columns(\$ofe, \$ole, \$one, \$osize);
    $sth->fetch;
    my $runtype;
    if ($type == 5 or $type == 6) {
        if ($type == 5) {
        ...
        print "$run: Type $type\n";
        printf "             %10s%10s%10s%10s\n", "FEVENT", "LEVENT", "NEVENTS", "SIZEMB";
        printf "Before Merge:%10d%10d%10d%10d\n", $ofe, $ole, $one, $osize;
        printf "After Merge: %10d%10d%10d%10d\n\n", $fe, $le, $ne, $size;
    }
    if (($fe <= $ofe or $ofe == 0) and $le >= $ole and ($ne > $one or $size > $osize) and $type == 5 or ($ne > $one) and $type == 6) {
        if ($ne < $one) {
            next;
        }
        if ($tag % 256 == 204 and $tag < 20480) { # LAS run, ignore ...
            next;
        }
        my $percentage = 'new';
        if ($ofe) {
