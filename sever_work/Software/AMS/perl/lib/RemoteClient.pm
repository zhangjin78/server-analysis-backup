# $Id: RemoteClient.pm,v 1.903 2025/03/04 15:56:42 dema Exp $
#
...
#
    ...
    if (system("$self->{eosselect} version >/dev/null 2>&1") != 0 and -e '/usr/bin/eos') {
        $self->{eosselect} = '/usr/bin/eos';
    }
...
jjid=datafiles.run and datafiles.type like '%MC%'  and jpid=productionset.did and realtriggers<=0 and cid=".$self->{CCID}." and productionset.status='Active' ");
        ...
        if($max_jobs <= 0){
         ...
         # Added by Baosong SHAN to load the site script
         my $scriptdir="$self->{AMSSoftwareDir}/scripts/";
         my $sitescriptfile=$scriptdir."$self->{CCA}";
         if(not open(FILEI,"<".$sitescriptfile)){
             $self->{SITESCRIPT} = "";
         }
         else{
             read(FILEI,$self->{SITESCRIPT},16384);
             close FILEI;
         }
         # Added on 2018-10-09
...
#              $self->{DataMC}=1;
    ...
    push @tdebug,time()-$t0Init;
      ...
      if($job =~ /\.job$/){
       ...
       my @farray=("TOTALEVENTS","PART","PMIN","PMAX","TIMBEG","TIMEND","CPUPEREVENTPERGHZ","OPENCLOSE","THREADS","EXCLTIMBEG","EXCLTIMEND");
              ...
              foreach my $ds (@{$datasetsDB}){
               ...
               if (trimblanks($datasetsNameDB) eq trimblanks($dataset->{name}) and trimblanks($datasetsVDB) eq trimblanks($dataset->{version})  ) {
                 ...
                 $dataset->{did}=$datasetsDidDB;
                  $sql="select sum(realtriggers) from jobs where did=$dataset->{did} and  substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}' and realtriggers>0".$pps;
                  my $rtn1=$self->{sqlserver}->Query($sql);
                   my $tm=time();
                  $sql="select sum(calcevents(time+timeout-$tm,Triggers)) from jobs where did=$dataset->{did} and  substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}' and realtriggers<0 and time+timeout>=$tm and timekill=0".$pps;
...
# Modified by Baosong to Add 10% contingency for requesting jobs
#                 $template->{TOTALEVENTS}-= $rtrig+$subm;
                 my $rate_contingency = 0.1;
                 if ($rtrig >= $template->{TOTALEVENTS}) {
                     $template->{TOTALEVENTS}=0;
                 }
                 else {
                     # Fixed by Baosong to avoid float number of  total events
                     $template->{TOTALEVENTS} = int($template->{TOTALEVENTS} * (1+$rate_contingency) - $rtrig - $subm);
                 }
...
#         }
     foreach my $job (@jobs){
      ...
      if($job =~ /\.job$/){
       ...
       $template->{filename}=$job;
       my @sbuf=split "\n",$buf;
       my @farray=("TOTALEVENTS","RUNMIN", "SERVERNO", "RUNMAX","OPENCLOSE","CPUPEREVENTPERGHZ","QTYPE","HOST","ROOTNTUPLE","RUNLIST","RUNALIST","PRIO","TAG","EVENTMIN","THREADS","EXCLRUNMIN","EXCLRUNMAX","TAGMIN","TAGMAX");
       my @farray_musthave=("TOTALEVENTS","RUNMIN", "RUNMAX","CPUPEREVENTPERGHZ","QTYPE");
          $template->{THREADS}=1;   
          $template->{TAG}=-1;   
          $template->{TAGMIN}=-1;   
          $template->{TAGMAX}=-1;   
          ...
          $template->{EXCLRUNMIN}=0;
          $template->{EXCLRUNMAX}=0;   
          $template->{EVENTMIN}=1; 
           foreach my $ent (@farray){
            foreach my $line (@sbuf){
               if($line =~/$ent=/){
...
#
       ...
       if(defined $dataset->{"$self->{CCA}.runalist"}){
          $template->{RUNALIST}=$dataset->{"$self->{CCA}.runalist"};   
           ...
           $template->{initok}=1;
           foreach my $ent (@farray_musthave){
             if(not defined $template->{$ent}) {
               $template->{initok}=undef;
             }
           }
...
# Added by Baosong SHAN to remove the trailing spaces and commas in the runlist and runalist 
#           my $purerunlist="";
#            if(defined $template->{RUNLIST}){
#                my @junk=split   ",",$template->{RUNLIST};
#                $purerunlist=" SELECT DISTINCT RUN FROM $datafiles WHERE (";
#                foreach my $r (@junk){
#                    $purerunlist=$purerunlist." run=$r or ";
#                }
#                $purerunlist=$purerunlist." run=-1)";
#            }
#           my $unionrunalist="";
#            if(defined $template->{RUNALIST}){
#                my @junk=split   ",",$template->{RUNALIST};
#                $unionrunalist=" UNION SELECT DISTINCT RUN FROM $datafiles WHERE (";
#                foreach my $r (@junk){
#                    $unionrunalist=$unionrunalist." run=$r or ";
#                }
#                $unionrunalist=$unionrunalist." run=-1)";
#            }
           my $qtype="and  $datafiles.type not like '%CAL%' ";
           if($template->{QTYPE} ne ""){
               $qtype="and $datafiles.type like '$template->{QTYPE}%'";
            }
# Following SQL was chagned because it is more than 200x slower than the new one
...
# This is not enough if we have $runlist, so a second round of optimization
#                   my $sql="select sum($datafiles.nevents),count($datafiles.run) from $datafiles where run>=$template->{RUNMIN} and $datafiles.nevents>$template->{EVENTMIN} and run<=$template->{RUNMAX}  and ($datafiles.status='OK' or $datafiles.status='Validated') $qtype $runlist  $runalist and $datafiles.nevents>0 and run not in  (select run from dataruns,jobs where  dataruns.jid=jobs.jid and jobs.did=$dataset->{did} and jobs.jobname like '%$template->{filename}' union select distinct run from ntuples,jobs where  ntuples.jid=jobs.jid and jobs.did=$dataset->{did} and jobs.realtriggers>0 and jobs.jobname like '%$template->{filename}') ";

# 14-Aug-2018 by Baosong SHAN: Another time optimisation, expected to be faster by factor 2
#
#                   my $excluded_runs = " (select run from dataruns,jobs where  dataruns.jid=jobs.jid and jobs.did=$dataset->{did} and jobs.jobname like '%$template->{filename}' UNION select distinct run from ntuples,jobs where  ntuples.jid=jobs.jid and jobs.did=$dataset->{did} and jobs.realtriggers>0 and jobs.jobname like '%$template->{filename}' $unionrunalist) ";
#                   my $runlist_condition = "";
#                   if ($purerunlist ne "") {
#                       $runlist_condition = " AND RUN IN ( $purerunlist MINUS $excluded_runs )";
#                   }
#                   else {
#                       $runlist_condition = " AND RUN NOT IN $excluded_runs ";
#                   }
#                   my $sql="select sum($datafiles.nevents),count($datafiles.run) from $datafiles where run>=$template->{RUNMIN} and $datafiles.nevents>$template->{EVENTMIN} and run<=$template->{RUNMAX} and not (run>=$template->{EXCLRUNMIN} and run<=$template->{EXCLRUNMAX}) and ($datafiles.status='OK' or $datafiles.status='Validated') $qtype and $datafiles.nevents>0 $runlist_condition ";
              my $runlist_condition = "";
              if (defined $template->{RUNLIST} and $template->{RUNLIST} ne "" ) {
                  $template->{RUNLIST} =~ s/\s+$//;
                  $template->{RUNLIST} =~ s/,+$//;
                  $runlist_condition .= " AND RUN IN ($template->{RUNLIST})";
              }
              if (defined $template->{RUNALIST} and $template->{RUNALIST} ne "" ) {
                  $template->{RUNALIST} =~ s/\s+$//;
                  $template->{RUNALIST} =~ s/,+$//;
                  $runlist_condition .= " AND RUN NOT IN ($template->{RUNALIST})";
              }
              my $tag_condition = "";
              if($template->{TAG}>=0){
                  $tag_condition .= " and $datafiles.tag=$template->{TAG} ";
              }
              else {
                  if($template->{TAGMIN}>=0){
                      $tag_condition .= " and $datafiles.tag>=$template->{TAGMIN} ";
                  }
                  if($template->{TAGMAX}>=0){
                      $tag_condition .= " and $datafiles.tag<=$template->{TAGMAX} ";
                  }
              }
              my $excl_requested = " AND NOT EXISTS (SELECT 1 FROM AMSDES.JOBS JOIN AMSDES.DATARUNS ON JOBS.JID=DATARUNS.JID WHERE JOBS.DID=$dataset->{did} AND substr(jobs.jobname,instr(jobs.jobname,'.',1,2)+1) = '$template->{filename}' AND DATARUNS.RUN=DATAFILES.RUN UNION SELECT 1 FROM AMSDES.NTUPLES JOIN AMSDES.JOBS ON JOBS.JID=NTUPLES.JID WHERE JOBS.DID=$dataset->{did} AND substr(jobs.jobname,instr(jobs.jobname,'.',1,2)+1) = '$template->{filename}' AND JOBS.REALTRIGGERS>0 AND NTUPLES.RUN=DATAFILES.RUN)";
              my $sql="select sum($datafiles.nevents),count($datafiles.run) from $datafiles where run>=$template->{RUNMIN} and $datafiles.nevents>$template->{EVENTMIN} and run<=$template->{RUNMAX} and not (run>=$template->{EXCLRUNMIN} and run<=$template->{EXCLRUNMAX}) and ($datafiles.status='OK' or $datafiles.status='Validated') $qtype and $datafiles.nevents>0 $runlist_condition $tag_condition $excl_requested ";
...
#- 20.06.05 a.k.       $dataset =~ s/ /\% /g;
       ...
       $sqlNT = "SELECT Ntuples.path, Ntuples.run, Ntuples.nevents, Ntuples.neventserr,
                        Ntuples.timestamp, Ntuples.status, Ntuples.sizemb, Ntuples.ctatime,ntuples.levent,ntuples.fevent,ntuples.jid
...
#
      ...
      $sqlNT = "SELECT Ntuples.path, Ntuples.run, Ntuples.nevents, Ntuples.neventserr,
                        Ntuples.timestamp, Ntuples.status, Ntuples.sizemb, Ntuples.ctatime,ntuples.levent,ntuples.fevent
                    FROM Runs, Jobs, runcatalog, NTuples
                     WHERE Jobs.DID=$did AND Jobs.JID=Runs.JID AND
...
#                      die "$sql $ret->[0][0]";
                     if(not defined $ret->[0][0]){
                         ...
                         $buf=~ s/#/C /;
                         $buf =~ s/'/''/g;
                         $sql = "INSERT INTO DatasetsDesc (DID, DATASET, DIRPATH, JOBNAME, JOBDESC, JOBDATA, TIMEREAD, TIMEUPDATE) Values($ret->[0][0],'$dataset->{name}','$self->{AMSSoftwareDir}/DataSets','$cite->{filename}','$junk[0]','$buf',$timenow,$timenow)";
#                         die "$sql";
                         $self->{sqlserver}->Update($sql);
                     }
                     }
        ...
        foreach my $arsref (@{$self->{arpref}}){
            try{
               ...
               if($rn>=0){
                if($rn<$maxr){
                 $jbs=$maxr-$rn;
# Commented by Baosong Shan (Why at least request half of maxr??)
#                 if($jbs<$maxr/2){
#                   $jbs=$maxr/2;
#                 }
                }
...
`;
              ...
              my $nthreads_default = "";
              my @junk_th = grep /^THREADS=/, $self->{SITESCRIPT};
              if (scalar @junk_th) {
                  my @junk_th2 = split /=/, $junk_th[0];
                  $nthreads_default = $junk_th2[1];
              }
              else {
                  $nthreads_default = ($dataset->{jobs})->[0]->{THREADS};
              }
              htmlTextField("Number of threads","number",3,$nthreads_default,"QNThreads","");
              my ($rid) = $self->getRID();
              htmlHiddenTextField("rid","hidden",12,$rid,"QRNDMS"," ");
              htmlText("<i>rndm sequence number </i>",abs($rid));
            htmlTableEnd();
...
# Script Custom/Generic
             ...
             print "<table border=0 width=\"100%\" cellpadding=0 cellspacing=0>\n";
             print "<tr><td><font size=\"-1\"<b>\n";
             # Added/Modified by Baosong SHAN to display the customized script of site
             print "<INPUT TYPE=\"radio\" NAME=\"JST\" VALUE=\"G\"  onclick=\"document.getElementsByName('SITESCRIPT')[0].style.display = 'none';\" CHECKED><b> Generic </b><i> (AMS generic)</i><BR>\n";
             if ($self->{SITESCRIPT} ne "") {
                 print "<INPUT TYPE=\"radio\" NAME=\"JST\" VALUE=\"C\" onclick=\"document.getElementsByName('SITESCRIPT')[0].style.display = 'block';\" ><b> Custom </b><i> ";
                 print "(specific for for cite <font color=\"green\"> $self->{CCA} </font>)</i><BR>\n";
                 print $q->textarea(-name=>"SITESCRIPT",-default=>"$self->{SITESCRIPT}",-rows=>10,-columns=>80,-style=>"display: none");
             }
             # Added on 2018-10-09
             print "</b></font></td></tr>\n";
            htmlTableEnd();
...
#           print "<input type=\"submit\" name=\"ProductionQuery\" value=\"Submit Request\"> <b>(it will take a while to tar DB and execs)</b></br><br>";
           print "<input type=\"submit\" name=\"ProductionQuery\" value=\"Submit Request\"> <INPUT TYPE=\"checkbox\" NAME=\"TarDBExec\" VALUE=\"TDE\"> <b>(Check this to tar DB and execs, which may take longer)</b></br><br>";
...
#  check par
        ...
        if(defined($self->{q}->param("BasicQuery")) and $self->{q}->param("BasicQuery") eq "Save"     or
	     ...
	     open(my $fh, '<', '.crypt-01') or die "Cannot open crypt file: $!";
	     chomp(my $crypt01 = <$fh>);  
	     close($fh);
	     open(my $fh, '<', '.crypt-02') or die "Cannot open crypt file: $!";
	     chomp(my $crypt02 = <$fh>);  
	     close($fh);
             my $crypt=crypt($pass,"ams");
             my $vr=$self->{q}->param("FEM");
             if(!($crypt eq $crypt01 and ($vr=~/v4/ or $vr=~/AMS02MC/)) and !($crypt eq $crypt02 and $vr=~/v5/)) {
...
#  03.12.2007 vc
         foreach my $tmp (@{$dataset->{jobs}}) {
            if($template eq $tmp->{filename} and $tmp->{TOTALEVENTS}>0){
                ...
                $q->param("QTagMin",$tmp->{TAGMIN});
                $q->param("QTagMax",$tmp->{TAGMAX});
                ...
                $q->param("QExclRunMi",$tmp->{EXCLRUNMIN});
                $q->param("QExclRunMa",$tmp->{EXCLRUNMAX});
                if($tmp->{QTYPE} ne ""){
       ...
       if( $self->{CCT} eq "local"){
        ...
        my $exclrunmi=$q->param("QExclRunMi");
        my $exclrunma=$q->param("QExclRunMa");
...
#           my $purerunlist="";
           my $tag="";
            if(defined $q->param("QTag") and $q->param("QTag")>=0){
               ...
               if(defined $q->param("QTagMin") and $q->param("QTagMin")>=0){
                   my $tagmin = $q->param('QTagMin');
                   $tag=" and tag>=$tagmin";
                }
                if(defined $q->param("QTagMax") and $q->param("QTagMax")>=0){
                   my $tagmax = $q->param('QTagMax');
                   $tag .= " and tag<=$tagmax";
                }
            }
# 14-Aug-2018 by Baosong SHAN to optimise the speed of data run query
...
#                $unionrunalist=" SELECT DISTINCT RUN FROM DATAFILES WHERE (";
#                foreach my $r (@junk){
#                    $unionrunalist=$unionrunalist." run=$r and ";
#                }
#                $unionrunalist=$unionrunalist." run=-1)";
#            }
           ...
           my $runlist_condition = "";
           if(defined $q->param("QRunList")){
               my $runlist = $q->param("QRunList");
               $runlist =~ s/\s+$//;
               $runlist =~ s/,+$//;
               if ($runlist ne "") {
                   $runlist_condition .= " AND RUN IN ($runlist)";
               }
           }
           if(defined $q->param("QRunAList")){
               my $runalist = $q->param("QRunAList");
               $runalist =~ s/\s+$//;
               $runalist =~ s/,+$//;
               if ($runalist ne "") {
                   $runlist_condition .= " AND RUN IN ($runalist)";
               }
           }
           my $qtype="and  datafiles.type not like '%CAL%' ";
...
#                  my $excluded_runs = " (select run from dataruns,jobs where  dataruns.jid=jobs.jid and jobs.did=$dataset->{did} and jobs.jobname like '%$template' $unionrunalist) ";
#                  my $runlist_condition = "";
#                  if ($purerunlist ne "") {
#                      $runlist_condition = " AND RUN IN ( $purerunlist MINUS $excluded_runs )";
#                  }
...
#                      $runlist_condition = " AND RUN NOT IN $excluded_runs ";
#                  }
#                  my $sql="select datafiles.run,datafiles.path,datafiles.paths , datafiles.fevent, datafiles.levent from datafiles where run>=$runmi and run<=$runma   and  datafiles.nevents>0 and (datafiles.status='OK' or datafiles.status='Validated')  $qtype $runlist_condition order by datafiles.run desc";
                   my $excl_requested = " AND NOT EXISTS (SELECT 1 FROM AMSDES.JOBS JOIN AMSDES.DATARUNS ON JOBS.JID=DATARUNS.JID WHERE JOBS.DID=$dataset->{did} AND substr(jobs.jobname,instr(jobs.jobname,'.',1,2)+1) = '$template' AND DATARUNS.RUN=DATAFILES.RUN UNION SELECT 1 FROM AMSDES.NTUPLES JOIN AMSDES.JOBS ON JOBS.JID=NTUPLES.JID WHERE JOBS.DID=$dataset->{did} AND substr(jobs.jobname,instr(jobs.jobname,'.',1,2)+1) = '$template' AND JOBS.REALTRIGGERS>0 AND NTUPLES.RUN=DATAFILES.RUN)";
                  my $sql="select datafiles.run,datafiles.path,datafiles.paths , datafiles.fevent, datafiles.levent from datafiles where run>=$runmi and run<=$runma and not (run>=$exclrunmi and run<=$exclrunma)  and  datafiles.nevents>0 and (datafiles.status='OK' or datafiles.status='Validated')  $qtype $runlist_condition $tag $excl_requested order by datafiles.nevents desc";
    ...
    if(not defined $job){
        ...
        if( not defined $res->[0][0]){
            ...
            # Modified by bshan to improve the performance
            # $sql="select max(jid) from jobs where jobname like '$self->{CCA}.%'";
            $sql="select max(jid) from jobs where cid = (select cid from cites where name='$self->{CCA}')";
...
#read header
         ...
         my $job_name = $junk[0];
         for my $i (1 .. scalar(@junk)-2) {
             $job_name .= "\.$junk[$i]";
         }
         $buf=~ s/JOB=/JOBNAME=$job_name \nJOB=/;
         }
...
#      $tmpb=~ s/TERM\n!/$bufb\nTERM\n!/;
       $tmpb=~ s/LIST\n/LIST\n$bufb\n/;
         ...
         if($self->{CCT} eq "local" or $self->{CCID} == 2){
...
# Added condition by B. Shan because test site should use RUNDIR as well;
           if ($self->{CCID} != 2) {
...
#             close FILEI;
#             my @ssbuf = split ',;',$sbuf;
             my $sbuf = $q->param("SITESCRIPT");
             $sbuf =~ s/\r\n/\n/g;
             my @ssbuf = split ',;',$sbuf;
             if($#ssbuf ne 1){
#              die " Could not parse script $newfile $#ssbuf";
              die " Could not parse script $sbuf $#ssbuf";
             }
...
#         }
        ...
        $buf=~s:export:export AMSFSCRIPT=$path$script \nexport:;
         if (defined $q->param("QNThreads")) {
             my $nthreads = $q->param("QNThreads");
             $buf =~ s/THREADS=.*/THREADS=$nthreads/g;
         }
         print FILE $buf;
         $tmpb =~ s/DAQC 1=610 42='\$NtupleDir/DAQC 1=610 42='\n\$NtupleDir/;
         print FILE $tmpb;
         if($self->{CCT} eq "local"){
...
my $rtype=" and type not like 'MC%' ";
if(defined $dataset->{MC} and $dataset->{MC}==1){
  $rtype=" and type like 'MC%' ";
} 
         $sql ="select fevent,levent,fetime,letime,nevents,type from datafiles where run=$run $rtype";
         my $r1=$self->{sqlserver}->Query($sql);
         my $ri={};
         $ri->{uid}=$job;
         $ri->{Run}=$run;
...
# check last download time
     if ($cite_status eq "remote" && $q->param("TarDBExec")) {
       ...
       if(defined $dataset){
         foreach my $tmp (@{$dataset->{jobs}}) {
            if($template eq $tmp->{filename} and $tmp->{TOTALEVENTS}>0){
                ...
                if(not defined $q->param("QTimeExclB")){
                    $q->param("QTimeExclB",$tmp->{EXCLTIMBEG});
                }
                if(not defined $q->param("QTimeExclE")){
                    $q->param("QTimeExclE",$tmp->{EXCLTIMEND});
                }
...
#randomize pmin
        ...
        if($cputime < 86400  and $self->{CCA} ne 'test' and $self->{CCA} ne 'mit'){
        ...
        if($cputime*$cput*$corr < 100000000  and $self->{CCA} ne 'test' and $self->{CCA} ne 'mit' and $q->param("QSINGLEJOB")==0){
        ...
        if ($dataset->{buildno} > 906) {
            # For 2.0GHz CPU with correction 1.0, CPULIM set to 30. sec
            $cput = 30. * 2000. / $q->param("QCPU") / $corr;
        }
        ...
        my $MAX_RUN_MC = 1000;
        if ($self->{CCA} eq 'juqueen' or $self->{CCA} =~ /nersc/ or $self->{CCA} eq 'alcf') {
            $MAX_RUN_MC = 8000;
        }
        if(not $runno =~/^\d+$/ or $runno <$a or $runno>$MAX_RUN_MC){
             $self->ErrorPlus("Runs no $runno is out of range ($a,$MAX_RUN_MC)");
        }
        ...
        my ($particleid,$timbeg,$timend,$timbegu,$timendu,$excl_timbeg, $excl_timend);
        my (@excl_timbeg_arr, @excl_timend_arr, @excl_timbegu_arr, @excl_timendu_arr);
         $timbeg=$q->param("QTimeB");
         $timend=$q->param("QTimeE");
         @excl_timbeg_arr = split(' ', $q->param("QTimeExclB"));
         @excl_timend_arr = split(' ', $q->param("QTimeExclE"));
        if (scalar @excl_timbeg_arr != scalar @excl_timend_arr) {
            $self->ErrorPlus("Entity numbers of EXCLTIMBEG and EXCLTIMEND in the template file do not match.");
        }
        if(not $timbeg =~/^\d+$/ ){
        ...
        # Added by Baosong to support EXCLTIMBEG and EXCLTIMEND
        my $j = 0;
        for my $i (0 ... scalar @excl_timbeg_arr - 1) {
            if(not $excl_timbeg_arr[$i] =~/^\d+$/ ){
                $self->ErrorPlus("ExclTimeBegin $excl_timbeg_arr[$i] is not an integer");
            }
            my $year=$excl_timbeg_arr[$i]%10000-1900;
            my $month=int($excl_timbeg_arr[$i]/10000)%100-1;
            my $date=int($excl_timbeg_arr[$i]/1000000)%100;
            if($year < 96 or $year > 118 or $month<0 or $month>11 or $date<1 or $date>31){
                $self->ErrorPlus("ExclTimeBegin $excl_timbeg_arr[$i] is out of range $date $month $year");
            }
            my $start_ts = timelocal(1,0,0,$date,$month,$year);

            if(not $excl_timend_arr[$i] =~/^\d+$/ ){
                $self->ErrorPlus("ExclTimeEnd $excl_timend_arr[$i] is not an integer");
            }
            $year=$excl_timend_arr[$i]%10000-1900;
            $month=int($excl_timend_arr[$i]/10000)%100-1;
            $date=int($excl_timend_arr[$i]/1000000)%100;
            if($year < 96 or $year > 128 or $month<0 or $month>11 or $date<1 or $date>31){
                $self->ErrorPlus("ExclTimeEnd $excl_timend_arr[$i] is out of range $date $month $year");
            }
            my $end_ts = timelocal(1,0,0,$date,$month,$year);
            if ($start_ts < $timbegu or $end_ts > $timendu or $end_ts < $start_ts) {
                $self->ErrorPlus("Invalid EXCLTIMBEG/EXCLTIMEND value(s).");
                next;
            }
            $excl_timbegu_arr[$j] = $start_ts;
            $excl_timendu_arr[$j] = $end_ts;
            $j += 1;
        }
#       End of adding (Baosong)
...
# write readme file
        ...
        my $last_timendu = 0;
        my $extra_gap = 0;
        my $covering_time = int($timendu-$timbegu);
        for my $i (0 ... scalar @excl_timbegu_arr - 1) {
            $covering_time -= ($excl_timendu_arr[$i] - $excl_timbegu_arr[$i]);
        }
        my $origjobspan = int($covering_time/$runno);
        my $jobspan = $origjobspan;
#         if ($jobspan > 86400*30) {
#             $jobspan = 86400*30;
#         }
        my $MAXSPANMC = 7*24*3600;
        if ($origjobspan > $MAXSPANMC) {
            $extra_gap = $origjobspan - $MAXSPANMC;
            $jobspan = $MAXSPANMC;
        }
        for my $i (1 ... $runno){
         #find buffer and patch it accordingly
...
#         my $start=$timbegu+$jobspan*($i-1);
         ...
         if ($last_timendu == 0) {
             $start = $timbegu;
             if ($extra_gap > 0) {
                 $start += int(rand($extra_gap));
             }
         }
         else {
             $start = $last_timendu + 1;
         }
         if ($start > $timendu) {
             last;
         }
         my $end=$start+$jobspan-1;
#         if ($i eq $runno) {
#             $end = $timendu;
#         }
#        To exclude the area $excl_timbeg -- $excl_timendu
         for my $i (0 ... scalar @excl_timbegu_arr - 1) {
             if ($start <= $excl_timendu_arr[$i] and $end >= $excl_timbegu_arr[$i]) {
                 if ($start < $excl_timbegu_arr[$i]) {
                     $end = $excl_timbegu_arr[$i] - 1;
                 }
                 else {
                     $start = $excl_timendu_arr[$i] + 1;
                     $end = $start+$jobspan-1;
                 }
                 last;
             }
...
if(defined $dataset->{buildno} ){
    ...
    $rootntuple=~s/$tago/$tagn/;
        ...
        if($pmaxr>1000000){
         ...
         $buf=~ s/PART=/JOBNAME=$job_name \nPART=/;
         }
         if(defined $q->param("TTHREADS")){
            my $threads=$q->param("TTHREADS");
            $buf=~ s/PART=/THREADS=$threads \nPART=/;
         }
...
if(length($buf2)>4000){
    ...
    $mes="CUTTED:$len"; 
    $buf2 = substr $buf2, 0, 4000-length($mes);
    if (substr($buf2, -2, 2) eq "\\'") {
        $buf2 = substr $buf2, 0, 4000-length($mes)-1;
    }
    $buf2="$buf2$mes";
}
            $insertjobsql="INSERT INTO Jobs VALUES
...
#        }
       ...
       if ($self->{CCT} eq "remote"){
          ...
          my $retcode = system("gpg --batch --passphrase-file /var/www/cgi-bin/mon/lib/.gpgphrase --output $file2tar.asc --detach-sign $file2tar.gz");
          if ($retcode == 0) {
              $attach .= ";$file2tar.asc,signature.asc";
          }
...
sub findemail(){
    my $self=shift;
        my $cem=shift;
        foreach my $chop (@{$self->{MailT}}) {
            if(defined $chop->{address} and $chop->{address} eq $cem and $cem ne "" and $cem ne " " and defined $cem){
                $self->{CEM}=$cem;
    ...
    if(defined $r3->[0][0]){
      ...
      foreach my $msg (@{$r3}){
          ...
          if($i==0 and $curtime-$msg->[3]>3600*3){
            ...
            $self->sendmailmessage('41754114733@mail2sms.cern.ch',"-S-NetMonitorNotResponded"," ");
      $self->sendmailmessage('Alexandre.Eline@cern.ch',"-S-NetMonitorNotResponded"," ");
            ...
            $self->sendmailmessage('41754111287@mail2sms.cern.ch',"-S-NetMonitorNotResponded"," ");
           } 
          if($cat =~/I/){
              $color="green";
          }
...
#
  ...
  my $ctatime = shift; # last castor copy time
  my $datamc=shift;
    my $fetime=shift;
    my $letime=shift;
#
...
if(not defined $fetime or not defined $letime){ 
   ...
   my $rj=$self->{sqlserver}->Query(" select content,jobname from jobs where jid=$jid");   
    ...
    my $stype='MC'." ".$ver." ".$ds;
     ...
     $sql="select run,path from datafiles where run=$run and type like 'MC%'";
     $ret= $self->{sqlserver}->Query($sql);
     foreach my $nt (@{$ret}){
    my $cmd="rm     $nt->[1]";
    if($path ne $nt->[1]){
    ...
    $sql="delete from datafiles where run=$run and type like 'MC%'";
     $self->{sqlserver}->Update($sql);
    $sql=" insert into datafiles (RUN,VERSION,TYPE,FEVENT,LEVENT,NEVENTS,NEVENTSERR,TIMESTAMP,SIZEMB,STATUS,PATH,PATHB,CRC,CRCTIME,CTATIME,BACKUPTIME,TAG,FETIME,LETIME,PATHS) values($run,'$version','$stype',$fevent,$levent,$events,$errors,$timestamp,$sizemb,'$status','$path',' ',$crc,$crctime,$ctatime,0,$part,$fetime,$letime,'$paths')";
}
  }
  else{
#
                               ...
                               DATAMC)
                                  VALUES( $run,
                                         ...
                                         '$type',
                                          ...
                                          '$path',
                                           $crc,
                                           $crctime,$crcflag,$ctatime,$buildno,$datamc)";
}
  if($type ne 'RawFile'){
...
sub calculateMipsVC {
    ...
    my $fast=shift;
    my $vrb=shift;
    my $rate_contingency = shift;
    if(not defined $vrb){
        $vrb=0;
    }
# Default 10% contingency for job requesting
    if(not defined $rate_contingency){
        $rate_contingency = 0.1;
    }
        ...
        my $sql="select did from DataSets where name='$dataset->{name}'";
              ...
              if($vrb){
               print "  template $dataset->{name} $template->{filename} $template->{TOTALEVENTS} $template->{CPUPEREVENTPERGHZ} total \n";
                 ...
                 $dataset->{did}=$ret->[0][0];
# Use indexed substr(jobname,instr(jobname,'.',1,2)+1) instead of LIKE 
# for performance improvement (by bshan)
                 my $sqlsum="select sum(cputime*mips/1000/realtriggers) from Jobs where did=$ret->[0][0]  and substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}' and realtriggers>0 and mips>0".$pps;
              my $r2= $self->{sqlserver}->Query($sqlsum);
              my $sum=$r2->[0][0];
                  $sqlsum="select count(cputime*mips/1000/realtriggers) from Jobs where did=$ret->[0][0]  and substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}' and realtriggers>0 and mips>0".$pps;
              $r2= $self->{sqlserver}->Query($sqlsum);
              my $count=$r2->[0][0];
               $sqlsum="select sum(cputime*mips/1000/realtriggers*cputime*mips/1000/realtriggers) from Jobs where did=$ret->[0][0]  and substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}' and realtriggers>0 and mips>0".$pps;
              $r2= $self->{sqlserver}->Query($sqlsum);
              my $sum2=$r2->[0][0];
#              print " $sum $count $sum2 \n";
              ...
              if($fast){
                  $sql="select sum(realtriggers) from jobs where did=$ret->[0][0] and  substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}' and realtriggers>0".$pps;
                  my $rtn1=$self->{sqlserver}->Query($sql);
                   my $tm=time();
                  $sql="select sum(calcevents(time+timeout-$tm,Triggers)) from jobs where did=$ret->[0][0] and  substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}' and realtriggers<0 and time+timeout>=$tm and timekill=0".$pps;
                  my $rtn2=$self->{sqlserver}->Query($sql);
                  $completed+=$rtn1->[0][0];
                  $template->{TOTALEVENTS}-=$rtn1->[0][0];
                  if ($template->{TOTALEVENTS} > 0) {
                      $template->{TOTALEVENTS} *= (1+$rate_contingency);
                  }
                  $submitted+=$rtn2->[0][0];
                  $template->{TOTALEVENTS}-=$rtn2->[0][0];
              }
              else{
                 $sql="select jid,time,triggers,timeout,realtriggers from Jobs where did=$ret->[0][0]  and substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}'".$pps;
...
#
              ...
              if($fast){
                  $sql="select sum($datafiles.nevents/($datafiles.levent+1-$datafiles.fevent)*jobs.realtriggers)  from jobs,dataruns,$datafiles where jobs.jid=dataruns.jid and $datafiles.run=dataruns.run and jobs.did=$ret->[0][0] and  jobs.substr(jobname,instr(jobname,'.',1,2)+1) = '$template->{filename}' and jobs.realtriggers>0".$pps;
                  my $rtn1=$self->{sqlserver}->Query($sql);
                   my $tm=time();
                  $sql="select sum($datafiles.nevents)  from jobs,dataruns,$datafiles where jobs.jid=dataruns.jid and $datafiles.run=dataruns.run and jobs.did=$ret->[0][0] and  substr(jobs.jobname,instr(jobs.jobname,'.',1,2)+1) = '$template->{filename}' and jobs.realtriggers<0 and $datafiles.status='OK'".$pps;
                  my $rtn2=$self->{sqlserver}->Query($sql);
                  $completed+=$rtn1->[0][0];
                  $submitted+=$rtn2->[0][0];
                  $template->{TOTALEVENTS}-=$rtn1->[0][0];
...
#
              ...
              if( defined $ret->[0][0]){
                 ...
                 $sql = "SELECT SUM(triggers)   FROM Jobs, Runs, Cites
                  WHERE
                    Jobs.did  = $did AND
                    substr(Jobs.jobname,instr(Jobs.jobname,'.',1,2)+1) = '$job' AND
...
sub getMips {
    ...
    my $sql = "SELECT jobs.triggers, jobs.cputime, jobs.mips
              FROM Jobs, Runs
              WHERE jobs.did = $did AND substr(jobs.jobname,instr(jobs.jobname,'.',1,2)+1) = '$dataset' AND
                    jobs.jid=runs.jid AND
...
";
  ...
  foreach my $chop  (@ARGV){
   ...
   $sql = "SELECT path, ctatime FROM ntuples WHERE PATH LIKE '$dbpath%'";
   my $ret=$self->{sqlserver}->Query($sql);
   if (not defined $ret->[0][0]) {
   ...
   foreach my $dataset (@datasets) {
     ...
     if (defined $ret->[0][0]) {
      ...
      if (not defined $ret->[0][0]) {
        ...
        $sql = "INSERT INTO DatasetsDesc (DID, DATASET, DIRPATH, JOBNAME, JOBDESC, JOBDATA, TIMEREAD, TIMEUPDATE) Values($did,'$dataset','$topdir','$job','$desc','$buf',$timenow,$timenow)";
        if ($verbose) {print "$sql \n";}
        if ($update)  {
         $self->{sqlserver}->Update($sql);
        }
...
sub checkEOS {
    ...
    my $eosmount = "$self->{eosselect} -b fuse mount";
    my $eosumount = "$self->{eosselect} -b fuse umount";
    $self->{eoshome} = '/eos/ams';
    $self->{eoslink} = '/eosams';
    $self->{eosreservegb} = 100;
    my $chkeoscmd = "$self->{eosselect} root://eosams.cern.ch quota -m $self->{eoshome}$path 2>/dev/null | grep \"space=$self->{eoshome}/ \" | grep \"gid=\"; test \${PIPESTATUS[0]} -eq 0";
    my $out = `$chkeoscmd`;
    chomp $out;
    my $retcode = $?;
    my $mounted = 0;
    ...
    if ($retcode != 0) {
        warn "$chkeoscmd returned $retcode";
        return -4, $mounted;
    }
    my @junk = split " ", $out;
    my $eosquota = {};
    for my $junk1 (@junk) {
        my @junk2 = split "=", $junk1;
        $eosquota->{$junk2[0]} = $junk2[1];
    }
    if (not defined $eosquota->{maxlogicalbytes} or not defined $eosquota->{usedlogicalbytes} or not defined $eosquota->{statusbytes} or not defined $eosquota->{statusfiles}) {
        warn "$chkeoscmd\n output:\n $out.";
        return -5, $mounted;
    }
    my $factor = 1024*1024;
    my $eos_total = int($eosquota->{maxlogicalbytes})/$factor;
    my $eos_used = int($eosquota->{usedlogicalbytes})/$factor;
    my $eos_avail = int ($eos_total - $eos_used);
    if ($updatedb > 0) {
        my $ts = time();
        my $sql = "update filesystems set isonline=1, totalsize=$eos_total, occupied=$eos_used, available=$eos_avail, timestamp = $ts where disk like '/eos/%'";
        $self->{sqlserver}->Update($sql);
        $self->{sqlserver}->Commit();
    }
    if (($eosquota->{statusbytes} ne 'ok' and $eosquota->{statusbytes} ne 'warning') or $eosquota->{statusfiles} ne 'ok') {
        warn "EOS quota problem: \n$out";
    }
    return ($eos_total - $eos_used)/1000000000, $mounted;
}
...
#
 ...
 if(not defined $validate){
  ...
  my $castorPrefix = '/castor/cern.ch/ams/MC';
    my $delim="MC";
    my $adelim="HZC";
    if($datamc ==1 or $datamc==3 or $datamc==2){
...
# 
              ...
              foreach my $ntuple (@{$ret_nt}){
                ...
                my $castor=$castorPrefix."/$name$junk[1]";
                my $tms=time(); 
                $sql="update ntuples set ctatime=$timenow where path='$ntuple->[0]'";
                if($datamc>1){
                $sql="update datafiles set ctatime=$timenow  where path='$ntuple->[0]'";
                }
                $self->{sqlserver}->Update($sql);
              }
...
#
   if($cmp>0){
      ...
      $sql="select path,run from ntuples where   path like '%$dir%' and ctatime>0 and path not like '/castor%' and datamc=$datamc";
      if($datamc>1){
      $sql="select path,run from datafiles where   path like '%$dir%' and ctatime>0 and path not like '/castor%' and type like '$delim%' and type not like '$adelim%' ";
      } 
      my $ret_nt =$self->{sqlserver}->Query($sql);
       foreach my $ntuple (@{$ret_nt}){
         if($ntuple->[0]=~/^#/){
...
my $sql2="select max(jid)-min(jid),run from ntuples where path like '%$dir%' group by run";    
    ...
    if($#duplicated>=0){
        ...
        foreach my $run (@duplicated){
           ...
           my $sql="select path,jid,run,ctatime,crctime from ntuples where path like '%$dir%' and run=$run";  
           my $ret_nt =$self->{sqlserver}->Query($sql);
           my $jidmax=-1;
           my $ctimemax=-1;
           my $valid=1;
    ...
    return;
    my $sql="select path,jid,run,ctatime from ntuples where path like '%$dir%'";    
    if($run2p!=0){
                      ...
                      if($verbose){
                          ...
                          if($update){
                              ...
                              if($ct==0 and $nt1->[0]=~/^\/castor/){
                                  $sql="update ntuples set ctatime=$nt1->[3] where path=$path";
                                  $self->{sqlserver}->Update($sql);
                                  print " setting ctatime $nt1->[3] for $path \n";
                               } 
...
#check and copy from castor raw files
    ...
    my $nbad=0;
    my $ntot=0;
    my $sql=" select path, run from datafiles where path like '$dir%' and ctatime>0";
        my $ret_nt =$self->{sqlserver}->Query($sql);
  ...
  if (not defined $whoami or $whoami =~ 'ams' or $whoami =~'casadmva') {
   ...
   return 0;
    ...
    if($datamc>1){
        ...
        my $sql = "select path,crc,sizemb,ctatime from datafiles where run=$run2p  and path  like '$dir' ";
    if(defined $nocastoronly and $nocastoronly == 1){
       $sql=$sql. " and ctatime=0 ";
    }
        my $ret_nt =$self->{sqlserver}->Query($sql);
...
#          
    ...
    my $sql="select ntuples.path,ntuples.crc,ntuples.ctatime,ntuples.jid,ntuples.fevent,ntuples.levent,ntuples.sizemb,ntuples.run from ntuples where  ntuples.path not like  '$castorPrefix%' and datamc=$datamc "; 
    if(!$force){
...
# 
  ...
  if ($whoami =~ 'ams' or $whoami =~'casadmva') {
   ...
   foreach my $ds (@{$ret}){
    ...
    if($run2p eq 0){
    $sql = "SELECT path  from $ntuples where ctatime>0 and path like '%$dir%'";
}
   else{
...
$srun=" and run in ($run2p)";
    ...
    $sql = "SELECT path  from $ntuples where ctatime>0 and path like '%$dir%' $srun";
   ...
   $ret =$self->{sqlserver}->Query($sql);
    ...
    if($run2p ne 0 and $run2p ne $run->[0]){
      next;
        ...
        $sql="select path,crc from ntuples where  jid=$run->[1]  and ctatime>0 and path not like '/castor%'";
      my $ret_nt =$self->{sqlserver}->Query($sql);
      my $suc=1;
      $runs++;
...
#       $sql="select path,crc from ntuples where  jid=$run->[1] and path like '$castorPrefix%$dir%' and ctatime>0 "; 
       $sql="select path,crc from ntuples where  jid=$run->[1] and path like '$castorPrefix%' and ctatime>0 "; 
   ...
   my $clocal=undef;
    ...
    if($run2p ne 0 and $run2p ne $run->[0]){
      next;
        ...
        $sql="select path,crc from datafiles where  run=$run->[0]  and ctatime>0 and path not like '/castor%'";
      my $ret_nt =$self->{sqlserver}->Query($sql);
      my $suc=1;
      $runs++;
      if(defined $ret_nt->[0][0] and !$force){
       ...
       $sql="select path,crc,paths from datafiles where  run=$run->[0] and path like '%$dir%' and ctatime>0 "; 
       $ret_nt =$self->{sqlserver}->Query($sql);
       my $disk=undef;
        my $dir=undef;
       if(defined $ret_nt->[0][0]){
...
# 
  ...
  if ($whoami =~ 'ams' or $whoami =~'casadmva') {
   ...
   foreach my $ds (@{$ret}){
       ...
       $sql=" select path,ctatime from $ntuples where run=$ds->[0] and path like '$dir%'";
       my $r1=$self->{sqlserver}->Query($sql);
       my $path='/MC';
       if(defined $r1->[0][0]){
           if($r1->[0][0]=~/$path/){
   ...
   $ret =$self->{sqlserver}->Query($sql);
    ...
    if($run2p < 0 and -$run2p < $run->[0]){
      ...
      if ($eos) {
        ...
        $sql="select path,crc from ntuples where  jid=$run->[1] and path like '%$dir%' and ctatime>0 and $cond and datamc=$datamc";
      my $ret_nt =$self->{sqlserver}->Query($sql);
      my $suc=1;
      if(not defined $ret_nt->[0][0]){
        next;
...
# 
              ...
              if($update){
               foreach my $ntuple (@{$ret_nt}){
                   ...
                   if ($eos and not $ntuple->[0] =~ /\/eosams\//) {
                       $sql="update ntuples set eostime=0 where path='$ntuple->[0]'";
                       $self->{sqlserver}->Update($sql);
                   }
                   else {
                       $sql="insert into ntuples_deleted select * from ntuples where path='$ntuple->[0]'";
                       $self->{sqlserver}->Update($sql);
                       my $eostimereset = '';
                       if ($eos) {
                           $eostimereset = ', eostime=0 ';
                       }
                       $sql="update ntuples set path='$castor', timestamp=$timenow $eostimereset where path='$ntuple->[0]'";
                       $self->{sqlserver}->Update($sql);
                       $self->datasetlink($ntuple->[0],"/Offline/DataSetsDir",0);
                   }
...
#
    ...
    my $runsn='runs';
  my $castorPrefix = '/castor/cern.ch/ams/MC';
  my $eosPrefix = '/eosams/MC';
    if ($dir=~'/Data/'){
     $castorPrefix = '/castor/cern.ch/ams/Data';
     $eosPrefix = '/eosams/Data';
     $datamc=1;
     $runsn='dataruns';
    }
  my $rfcp="/usr/bin/rfcp ";    
...
if($run2p){
   ...
   $ret =$self->{sqlserver}->Query($sql);
    foreach my $run (@{$ret}){
       print "Processing $run->[0]\n";
       my $timenow = time();
    if($run2p ne 0 and $run2p ne $run->[0]){
      next;
    }
        $sql="select path,crc,paths from datafiles where  run=$run->[0] and path like '%$dir%' and ctatime>0 and path not like '/castor%' ";
      my $ret_nt =$self->{sqlserver}->Query($sql);
      my $suc=1;
      if(not defined $ret_nt->[0][0]){
        next;
      ...
      foreach my $ntuple (@{$ret_nt}){
         ...
         my @junk=split $name_s,$ntuple->[0];
         my $castor=$castorPrefix."/$name$junk[1]";
         my $eospath=$eosPrefix."/$name$junk[1]";
         my @junk2=split /\//,$ntuple->[0];
         $sys=$rfcp.$castor." $tmp";
         if($notverify){
             if ($eos) {
                 print "With -eos, -notverify will be disabled automatically.\n";
                 $notverify = 0;
             }
             else {
            my $ctmp="/tmp/castor.tmp";
         ...
         if($i){
          ...
          my $path_for_crc = "$tmp/$junk2[$#junk2]";
          if ($eos) {
              $eospath=~ s#eosams#eos/ams#g;
              $path_for_crc = "root://eosams.cern.ch/$eospath";
          }
          my $crccmd    = "$self->{AMSSoftwareDir}/exe/linux/crc $path_for_crc  $ntuple->[1]";
          my $rstatus   = system($crccmd);
          $rstatus=($rstatus>>8);
          if($notverify){
           $rstatus=1;
          ...
          if($rstatus!=1){
           if($verbose){
              print "$path_for_crc crc error:  $rstatus \n";
           }  
           $suc=0;
           last;
          }
              ...
              if($update){
               foreach my $ntuple (@{$ret_nt}){
                   ...
                   if($did<0){
                       my @junk=split '\/',$ntuple->[0];
                       for my $j (3...$#junk){
                        $newdest=$newdest."/$junk[$j]";
                    }
                   }
                   else{
                    my @junk=split $name_s,$ntuple->[0];
                     $newdest=$newdest."/$name$junk[1]";
                   }
                $sql="update datafiles set path='$newdest', timestamp=$timenow where path='$ntuple->[0]'";
                $self->{sqlserver}->Update($sql);
                $self->datasetlink($ntuple->[0],"/Offline/RunsDir",0,$ntuple->[2]);
                if ($eos) {
                    $self->datasetlink($newdest,"/Offline/RunsDir",1,$ntuple->[2]);
                }
...
#
    ...
    my $sql= "select path,sizemb,run from ntuples where path not like '/castor%' and ctatime>0 order by run";
    my $ret_nt=$self->{sqlserver}->Query($sql);
    my $runo=-1;
    my @path=();
     my @disk=();
