#!/usr/bin/perl -w
#  $Id: rdlastraw.perl,v 1.9 2025/03/11 15:12:10 bshan Exp $
use strict;
use lib qw(../perl);
use lib::DBSQLServer;
...
if($ok && !$nook){
    ...
    $goodtime[2]=0;
         ...
         $sql = "SELECT path,run,timestamp fROM amsdes.ntuples  where datamc=1 and version like 'v6.0%' and path not like '%hbk' and timestamp>$timenow-3600 and path like '%/std%'  $run2p  ORDER BY timestamp desc";
         my $rdlast=$o->Query($sql);
         $sql = "SELECT path,run,timestamp fROM amsdes.ntuples  where datamc=1 and version like 'v6.0%' and path not like '%hbk' and path like '%/std%' and timestamp>$timenow-3600  $run2p  ORDER BY run desc";
         ...
         if(defined $rdlastr->[0][0]){
           my $delayrun=$timenow+3600*24-$rdlastr->[0][2];
                 $good[2]=$delayrun<$dl+$dlr*7?"OK":"NOFRESHRUNS";
                 $goodplus[2]=$delayrun<$dl+$dlr*7+$dlr?"OK":"NOFRESHRUNS";
...
#   form message
             $sql="select expert_notified, expert_name from amsdes.filesprocessing";
             my $exp=$o->Query($sql);
            my $time=0; 
            my $expert_name = '';
            if(defined $exp->[0][0]){
         ...
         if($time>$goodtime[0] or $time>$goodtime[1] or $time>$goodtime[2]){
             $addmessage="Expert [$expert_name] was notified ";
             $expert_notified=1;
          } 
             if(not $expert_notified){
                 my @address=('Alexandre.Eline@cern.ch', '41754114733@mail2sms.cern.ch', 'Baosong.Shan@cern.ch', '41754119642@mail2sms.cern.ch');
             foreach my $add (@address){
              ...
              if($warnplus){
                my @addressplus=('Alexander.Egorov@cern.ch', '41754116488@mail2sms.cern.ch');
