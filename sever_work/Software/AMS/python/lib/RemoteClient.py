...
class RemoteClient:
    ...
    logger = None
    cmd = None
    cta_prefix = '/eos/ctapublic/archive/ams'
    cta_xrd_server = 'eosctapublic.cern.ch'
    timeout_cmd = '/afs/cern.ch/ams/local/bin/timeout'
    hostname = socket.gethostname()


    def __init__(self):
        self.cmd = CmdOld.CmdOld()
        self.__setup_eos()


    def __setup_eos(self):
        self.eos_prefix = '/eos/ams'
        self.eoslink = '/eosams'
        self.eos_xrd_server = 'eosams.cern.ch'
        localeos = '/usr/bin/eos'
        self.eosselect = localeos
        if os.path.exists(localeos):
            self.env['EOS_MGM_URL'] = "root://%s" %(self.eos_xrd_server)
            os.environ['EOS_MGM_URL'] = "root://%s" %(self.eos_xrd_server)
            retcode = os.system("/usr/bin/timeout 5 %s ls %s > /dev/null" %(localeos, self.eos_prefix))
            if (retcode == 0):
                self.eosselect = localeos
                print "EOS setup: Setting eosselect to be /usr/bin/eos"
            else:
                print "EOS setup: Error with eos ls: %d" %(retcode)
    ...
    def CheckCRC(self,v,irm,upd,run2p,force,dir,notapeonly):
        #
        #  check crc of files on disks
        #  copy from tape if bad crc found
        #  remove from disk if tape copy is eq damaged
        ...
        #  $notapeonly  only check crc for files without tape
        #  output par:
        #   1 if ok  0 otherwise
        #
        # 
        ...
        if not (whoami == None or whoami =='ams' or whoami=='casadmva' or whoami=='choutko'):
            print "tapePath -ERROR- script cannot be run from account : ",whoami 
            return 0

        sql="select ntuples.path,ntuples.crc,ntuples.ctatime,ntuples.run,ntuples.fevent,ntuples.levent,ntuples.sizemb,ntuples.datamc from ntuples where  ntuples.path not like  '"+self.cta_prefix+"%' ";
        if force==0 :
        ...
        if notapeonly !=None and notapeonly==1:
            sql=sql+" and ntuples.ctatime=0 "
        ...
        for ntuple in ret_nt:
            ...
            if rstatus !=1:
                ...
                if ntuple[2]>0:
                    ...
                    #no tape file found and bad crc remove ntuple
                    ntpf=ntpf+1
                    if(ntuple[7]==1):
    ...
    def CheckCRCT(self,v,irm,upd,run2p,force,dir,notapeonly):
        #
        ...
        for ntuple in ret_nt:
            ...
            if rstatus !=1:
                ...
                if ntuple[2]>0:
                    ...
                    if len(junk)>1:
                        ctant=self.cta_prefix+"/"+delimiter+junk[1]
                        sys=rfcp+" "+ctant+" "+ntuple[0]+".cta"
                        mutex.release()
                        i=self.cmd.cmd(sys)
                        mutex.acquire()
                        if(i):
                            suc=0
                            if(self.v):
                                print sys," failed for ",ctant
                            self.ntpf=self.ntpf+1
                            self.cmd.cmd("rm "+ntuple[0]+".cta")
                            continue
                        else:
                            crccmd="/Offline/AMSDataDir/DataManagement/exe/linux/crc "+ntuple[0]+".cta "+str(ntuple[1])
                            mutex.release()
                            rstatus=self.cmd.cmd(crccmd)
                            mutex.acquire()
                            rstatus=(rstatus>>8)
                            if rstatus !=1:
                                if self.v:
                                    self.sendMailMessage(address,"crc cta failed"+ctant," ")
                                    print ctant," cta crc error: ",rstatus
                            if(suc):
                                if(self.update):
                                    self.cmd.cmd("mv "+ntuple[0]+".cta "+ntuple[0])
                                else: self.cmd.cmd("rm "+ntuple[0]+".cta")
                            else:
                                self.cmd.cmd("rm "+ntuple[0]+".cta")
                                self.ntpf=self.ntpf+1
                                ...
                                self.sqlserver.Update(sql)
                                sql="update ntuples set ntuples.path='"+ctant+"' where ntuples.path='"+ntuple[0]+"' ";
                                ...
                                if(self.update):
                                    ...
                                    if(res==0):
                                      ...
                                      sys=rm+" "+fname
                                      if(fname.find(self.cat_prefix)>=0):
                                          i=self.__file_remove(fname)
                    ...
                    if(self.update):
                        ...
                        if(res==0):
                            ...
                            sys=rm+" "+fname
                            if(fname.find(self.cta_prefix)>=0):
                                i=self.__file_remove(fname)
                            if(i): self.sendMailMessage(address,"unable to "+sys," ")
                    else: self.sqlserver.Commit(0)
    ...
    def checkFS(self, updatedb=0, cachetime=60, path='/', vrb=0):
        ...
        for fs in ret:
               ...
               #
               if (fs[0].find('/eos') == 0):
                   gb, eosmounted = self.checkEOS(path, updatedb)
                   continue
               whoami=os.getuid()
               ...
               self.cmd.cmd("ls "+fs[0]+" 1>"+stf+" 2>&1 &")
               time.sleep(1)
               ...
               pair = self.cmd.cmd_output("cat %s | grep Data | wc -l" % stf)
               ...
               pair = self.cmd.cmd_output("%s df -P %s | grep -v ^Filesystem | awk '{print $2, $3, $4}'" %(tmout, fs[0]))
               df_output = pair[1]
...
#                   ffree=res[6]
        ...
        if(len(ret)<=0):
           ...
           ret=self.sqlserver.Query(sql)
           if(len(ret)<=0):
               self.sendMailMessage('baosong.shan@cern.ch', 'FileSystems are Offline: Exited!!!', sql)
               return None
    ...
    def sendMailMessage(self, add, sub, mes="", att=""):
        if self.logger != None:
            self.__log_info("MAIL: to='%s', subject='%s', message='%s', att='%s'" %(add, sub, mes, att))
    ...
    def ValidateRuns(self,run2p,i,v,d,h,b,u,mt,datamc=0,force=0,nfs=0,tapeonly=0,tapecopy=0,eos=0,skipcrc=0):
        self.s=""
        self.tapecopy=tapecopy
        self.tapeonly=tapeonly
            ...
            for run in self.dbclient.rtb:
                ...
                if(status=='Finished' or status=='Foreign' or status == 'Canceled'):
                    ...
                    if(datamc==0 and uid==run.Run):
                        ...
                        ret=self.sqlserver.Query(sql)
                        sql=" select datafiles.path from datafiles,runs where datafiles.run=runs.run and runs.status='Completed' and substr(type,1,instr(type,' ')-1) = 'MC' and runs.jid=%d " %(uid)
                        ret2=self.sqlserver.Query(sql)
                    for ntuple in self.dbclient.dsts:
                        if(self.dbclient.ct(ntuple.Type)=="EventTag"):
                            if(ntuple.Run==run.Run and ntuple.uid==run.uid):
...
#                                    print " deleting eventtag ",ntuple.Name
                        ...
                        print " deleting ",run.Run, " ,", run.uid
        ...
        for run in rtb_tuples:
            ...
            if( run.DataMC==datamc and mt ):
                exitmutexes[run.uid]=thread.allocate_lock()
                timenow=int(time.time())
                try:
	...
	runupdate="update runs set "
        ...
        r2=self.sqlserver.Query("select count(path)  from datafiles where substr(type,1,instr(type,' ')-1) = 'MC' and run="+str(run.Run))
        status=ro[0][1]
        status2=""
        ...
        if(self.dbclient.cr(run.Status) == "Finished" and status != "TimeOut" and status !="Completed"):
            print "Check Run %d UID %d Status %s DBStatus %s" %(run.Run, run.uid,self.dbclient.cr(run.Status),status)
            output.write("Check Run %d UID %d Status %s DBStatus %s" %(run.Run, run.uid,self.dbclient.cr(run.Status),status))
 			...
 			if(cputime=="nan"):
				cputime=elapsed
                        ...
                        for ntuple in self.dbclient.dsts:
                            ...
                            if( (self.dbclient.cn(ntuple.Status) == "Success" or  self.dbclient.cn(ntuple.Status) == "Validated") and ntuple.Run == run.Run and ntuple.uid == run.uid):
                                ntuplelist.append(ntuple)
                        ntuplelist.sort(lambda x,y: cmp(y.Insert,x.Insert))
                        fevt=-1
                        dat0=0
                        for ntuple in ntuplelist:
                            if(fevt>=0 and ntuple.LastEvent>fevt and self.dbclient.ct(ntuple.Type)!="RawFile"):
                                ntuple.EventNumber=-1
                                print " problems with %d %d %s " %(run.uid, run.Run,ntuple.Name)
                            else:
                        ...
                        for ntuple in ntuplelist:
                            ...
                            if(ntuple.EventNumber==-1):
                                ...
                                if(fpath.find(self.cta_prefix)>=0):
                                    rmbad.append("xrdfs %s rm %s" %(self.cta_xrd_server, fpath))
                                else:
                                ...
                                status="OK"
                                rcp=1
                                if( not os.path.isfile(fpath) and fpath.find(self.cta_prefix)<0 and fpath.find('/eosams/')<0):
                                ...
                                if(retcrc !=1):
                                            ...
                                            if( i == 0xff00 or (i & 0xff)):
                                                ...
                                                if(i/128):
                                                    ...
                                                    (outputpatha,rstatus,odisk,ctatime)=self.doCopy(run.Run,fpath,ntuple.crc,ntuple.Version,outputpath)
                                                    outputpath=None
                                                    if(outputpatha != None):
                                                    ...
                                                    if(rstatus==1):
                                                        ...
                                                        self.InsertNtuple(run.Run,ntuple.Version,self.dbclient.ct(ntuple.Type),run.Run,ntuple.FirstEvent,ntuple.LastEvent,events,badevents,ntuple.Insert,ntuple.size,status,outputpath,ntuple.crc,crctime,1,ctatime,run.DataMC)
                                                        ...
                                                        if(outputpath != None):
                                                            cmd="rm "+outputpath
                                                            if(outputpath.find(self.cta_prefix)>=0):
                                                                cmd="xrdfs %s rm %s" %(self.cta_xrd_server, outputpath)
                                                            rstat=self.cmd.cmd(cmd)
                        ...
                        if(copyfailed==0):
                            warn="Validation done Run %d UID %d " %(run.Run, run.uid)
                            print warn
                            output.write(warn)
                        if(len(cpntuples)>0 and len(mvntuples)>0):
                               status='Completed'
                        elif(len(mvntuples)>0):
                             warn="Validation/copy failed run %d UID %d " %(run.Run, run.uid)
                             print warn
                             output.write(warn)
                             status="Unchecked"
                             for ntuple in mvntuples:
                                 cmd="rm "+ntuple
                                 if(ntuple.find(self.cta_prefix)>=0):
                                     cmd="nsrm "+ntuple
                                     cmd="xrdfs %s rm %s" %(self.cta_xrd_server, ntuple)
                                 self.cmd.cmd(cmd)
...
#                        print "status ",status,len(status2),status2
                        ...
                        if(len(r4)==0 or r4[0][0]==None):
                            sql="select sum(datafiles.levent-datafiles.fevent+1),min(datafiles.fevent)  from datafiles,runs where datafiles.run=runs.run and substr(type,1,instr(type,' ')-1) = 'MC' and runs.run="+str(run.Run)
                            r4=self.sqlserver.Query(sql)
                            if(len(r4)==0):
                                ntevt=0
                                fevt=0
                        ...
                        for ntuple in cpntuples:
                            ...
                            if(ntuple.find(self.cta_prefix)>=0):
                                cmd="xrdfs %s rm %s" %(self.cta_xrd_server, ntuple)
                                rmdir="xrdfs %s rmdir  " %(self.cta_xrd_server)
                            elif(ntuple.find('/eosams/')>=0):
                 ...
                 #           print ntuple.Run,run.Run,self.dbclient.cn(ntuple.Status),ntuple.Name
                                ...
                                if(retcrc !=1):
                                            ...
                                            if( i == 0xff00 or (i & 0xff)):
                                                ...
                                                if(i/128):
                                                    ...
                                                    levent=levent-(ntuple.LastEvent-ntuple.FirstEvent+1)
                                                    if(fpath.find(self.cta_prefix)>=0):
                                                        rmbad.append("xrdfs %s rm %s" %(self.cta_xrd_prefix, fpath))
                                                    else:
                                                        rmbad.append("rm "+fpath)
                                                else:
                                                    status="OK"
                                                    ...
                                                    if(run.DataMC==0):
                                                        ...
                                                        mcpath="/Data"
                                                    (outputpatha,rstatus,odisk,ctatime)=self.doCopy(run.uid,fpath,ntuple.crc,ntuple.Version,outputpath,mcpath)
                                                    outputpath=None
                                                    if(outputpatha != None):
                                                        outputpath=outputpatha[:]
                                                        mvntuples.append(outputpath)
                                                    if(rstatus==1):
                                                        ...
                                                        if (self.skipcrc):
                                                            ...
                                                            crctime = ntuple.Insert
                                                        self.InsertNtuple(run.Run,ntuple.Version,self.dbclient.ct(ntuple.Type),run.uid,ntuple.FirstEvent,ntuple.LastEvent,events,badevents,ntuple.Insert,ntuple.size,status,outputpath,ntuple.crc,crctime,1,ctatime,run.DataMC)
                                                        output.write("insert %d %s %s %d" %(run.Run, outputpath, status,int(ntuple.size)))
                                                        self.copied=self.copied+1
                                                        self.gbDST=self.gbDST+ntuple.size
                                                        cpntuples.append(fpath)
                        ...
                        if(len(cpntuples)>0 and len(mvntuples)>0):
                             ...
                             for ntuple in mvntuples:
                                 cmd="rm "+ntuple
                                 if(ntuple.find(self.cta_prefix)>=0):
                                     cmd="xrdfs %s rm %s" %(self.cta_xrd_prefix, ntuple)
                                 if(self.deleteuncnt):
                                     self.cmd.cmd(cmd)
                                 print "Validation failed "+cmd
                                 output.write("Validation failed "+cmd)
...
#       self.sqlserver.Commit()
       ...
       filesize=1
       if(inputfile.find(self.cta_prefix)<0 and inputfile.find('/eosams/')<0):
           try:
       ...
       if(filesize>0):
           ...
           elif(outputpath.find(self.cta_prefix)>=0):
               stime=100
               while(stime>60):
                   if(odisk!=None):
                       if(fsmutexes.has_key(odisk)):
                   ...
                   if(len(r2)>0):
                       ...
                       if(cmdstatus==0):
                           ...
                           if(cmdstatus==0 or cmdstatus==1024):
                               ...
                               if(rstatus==1):
                                   (ctatime,ctapath)=self.moveCTA(inputfile,outputpath)
                                   if(self.tapeonly):
                                       if (ctatime>0 and ctapath!=None):
                                           i2=inputfile
                                           inputfile=outputpath
                                           outputpath=ctapath
                                           if(self.tapecopy<0 and outputpath!=inputfile and inputfile.find(self.cta_prefix)<0 and i2.find(self.cta_prefix)<0):
                                               cmd="rm "+inputfile
                                               if (outputpath.find('/eosams/') == 0):
                                       ...
                                       # originaly on tape, tapeonly specified, and cta copy failed
                                       if (inputfile.find(self.cta_prefix)>=0 and (ctatime<=0 or ctapath==None)):
                                           print "doCopy-E-ErorrMoveTape "
                                           return outputpath,0,odisk,0
                                   return outputpath,1,odisk,ctatime
                               else:
    ...
    def __doCopyRaw(self, journalFile, inputfile, path='/MC', rawType=''):
       time0=time.time()
       time00=0
#       (period,prodstarttime,periodid,vrs)=self.getActiveProductionPeriodByName(path)
       ...
       file=junk[len(junk)-1]
       filesize=1
       if(inputfile.find(self.cta_prefix)):
           filesize = os.stat(inputfile)[ST_SIZE]
       ...
       if(filesize>0):
           ...
           if(outputpath == None):
               ...
               while(stime>60):
                   ...
                   if(odisk==None):
                       self.__log_info("odisk Problem to __getOutputPathRaw file %s" %(path))
                       return None,0,None

                   self.__log_info("acquired: %s %s %s %s" %(outputpatha, gb, odisk, stime))
           ...
           outputpath = "%s/RawData/%s" % (outputpath, rawType)
           ...
           if(cmdstatus==0):
               ...
               if(cmdstatus==0):
                   ...
                   if(rstatus==1):
                       ctatime = self.__uploadtoCTA(outputpath)
                       return outputpath, 1, ctatime
                   else:
                       self.__log_error("__doCopyRaw-E-ErorrCRC %d" %(rstatus))
                       self.BadCRC[self.nCheckedCite] += 1
                       return outputpath, 0, 0
               else:
                   self.__log_error("__doCopyRaw-E-cannot copyfile %d %s %s" %(cmdstatus, inputfile, outputpath))
                   self.BadDSTCopy[self.nCheckedCite] += 1
                   return outputpath,0,0
           else:
               self.__log_error("__doCopyRaw-E-cannot %s" %(cmd))
               self.BadDSTCopy[self.nCheckedCite] += 1
               return None,0,0
       else:
           self.__log_error("__doCopyRaw-E-cannot stat %s" %(inputfile))
           self.BadDSTs[self.nCheckedCite] += 1
           return None,0,0
    

    def __uploadtoCTA(self, input):
#
# only one file to uploads
#
        buf=input.split('/')
        output=self.cta_prefix
        buf2=input.split(buf[1])
        output=output+buf2[1]
        self.__file_remove(output, 1, 1, 10, 9)
        self.mkdirCTA(os.path.dirname(output))
        ossize=10000000000
        ...
        force = 1
        signal = 9
        cmdstatus = self.__xrdcp(input, output, noprogress, verbose, force, timeout, signal)
        if(cmdstatus):
            print "Error uploadToCTA via xrdcp",input,output,cmdstatus
            self.__file_remove(output, 1, 1, 10, 9)
        else:
    ...
    def getOutputPath(self,period,idisk,path='/Data'):
        ...
        ct=360
        outputdisk=None
        # idisk == '/eos' means CTA, /eos/ctapublic...
        if(idisk == '/eos/'):
            idisk=None
        if(idisk!=None):
        ...
        for disk in ret:
            ...
            self.cmd.cmd("mkdir -p "+outputpath)
    ...
    def checkEOS(self, path='/Data', updatedb=0):
        ...
        chkeoscmd = "/usr/bin/timeout 30 %s root://eosams.cern.ch quota -m %s%s 2>/dev/null | grep \"space=%s/ \" | grep \"gid=\"; test ${PIPESTATUS[0]} -eq 0" %(self.eosselect, self.eoshome, path, self.eoshome)
        pair = self.cmd.cmd_output(chkeoscmd)
        ...
        if (updatedb > 0):
            factor=1024*1024
#            sql="update filesystems set isonline=1, totalsize=%d, occupied=%d, available=%d, timestamp = %d where disk like '/eos/%%'" %(int(quota['maxlogicalbytes'])/factor, int(quota['usedlogicalbytes'])/factor, (int(quota['maxlogicalbytes'])-int(quota['usedlogicalbytes']))/factor, int(time.time()))
            sql="update filesystems set isonline=1, totalsize=%d, occupied=%d, available=%d, timestamp = %d where disk like '/eos/%%'" %(int(quota['maxbytes'])/factor, int(quota['usedbytes'])/factor, (int(quota['maxbytes'])-int(quota['usedbytes']))/factor, int(time.time()))
            self.sqlserver.Update(sql)
            self.sqlserver.Commit()
        if ((quota['statusbytes'] == 'ok' or quota['statusbytes'] == 'warning') and (quota['statusfiles'] == 'ok' or quota['statusfiles'] == 'ignored') ):
#            return (int(quota['maxlogicalbytes'])-int(quota['usedlogicalbytes']))/1000000000, os.path.exists(self.eoslink)
            return (int(quota['maxbytes'])-int(quota['usedbytes']))/1000000000, os.path.exists(self.eoslink)
        else:
            print "EOS quota problem: ", quota
#            self.sendMailMessage('baosong.shan@cern.ch, oleg.demakov@cern.ch', 'EOS quota', str(quota))
#            return (int(quota['maxlogicalbytes'])-int(quota['usedlogicalbytes']))/1000000000, os.path.exists(self.eoslink)
            return (int(quota['maxbytes'])-int(quota['usedbytes']))/1000000000, os.path.exists(self.eoslink)
    ...
    def __getOutputPathRaw(self, period, path='/MC'):
        ...
        for disk in disks:
            ...
            self.cmd.cmd("mkdir -p %s" % outputpath)
            try:
                mtime=os.stat(outputpath)[9]
            except:
                self.__log_info("Enable to open %s" %(outputpath))
                continue
        ...
        timew=time.time()
        self.__log_info("getoutputpath raw %s %s" %(outputpath, sql))
        return outputpath, gb, outputdisk, time.time()-timew
    ...
    def mkdirCTA(self, cdir):
        cmd = self.__get_timeout_command(90, 9)
        cmd.append('xrdfs')
        cmd.append(self.cta_xrd_server)
        cmd.append('mkdir')
        cmd.append('-p')
        cmd.append(cdir)
        return self.__popen(cmd)

               
    def moveCTA(self,input,output):
        output_orig = output
        junk=output.split('/')
        cdir = self.cta_prefix
        mutex.release()
        for i in range (2,len(junk)-1):
            cdir += '/'+junk[i]
        cmove = cdir + '/' + junk[len(junk)-1]
        if(input.find(self.cta_prefix)<0):
            if(self.tapecopy):
                if (self.mkdirCTA(cdir)):
                    mutex.acquire()
                    return 0, None
                force = 1
                noprogress = 1
                ...
                timeout = 1800
                signal = 9
                i = self.__xrdcp(output, cmove, noprogress, verbose, force, timeout, signal)
                if(i==0):
                    if(self.tapecopy>0):
                        mutex.acquire()
                    ...
                    print "moveCTA-E-2 "
                    self.__file_remove(cmove)
                    mutex.acquire()
                    return 0,None   
            else:
                print "moveCTA-W-1 ",input,output
                mutex.acquire()
                return 0,None   
        if (self.mkdirCTA(cdir)):
            mutex.acquire()
            return 0, None
        i = self.__file_rename(input, cmove)
        if(i==0):
            mutex.acquire()
            return int(time.time()),cmove
        else:
            print "moveCTA-W-failed ",cmd
            cmdstatus = self.__file_ls(cmove)
            if(cmdstatus):
                mutex.acquire()
                print "moveCTA-E-failed ",cmdn
                return 0,None
            else:
                mutex.acquire()
                return int(time.time()),cmove
    ...
    def getmoveCTA(self,input,output):
        junk=output.split('/')
        if(input.find(self.cta_prefix)<0):
            return None   
        cmove = self.cta_prefix
    ...
    def eosReal2Xrootd(self, path):
        return path.replace('/eos/ams/', 'root://eosams.cern.ch//eos/ams/')
    ...
    def eosGetSize(self, path):
        cmd = "%s ls -l %s 2>/dev/null" % (self.eosselect, self.eosLink2Real(path))
        pair = self.cmd.cmd_output(cmd)
        eossize = -1
        if (pair[0] != 0 or len(pair) < 2):
            print "%s\nreturned %d\n" %(cmd, pair[0])
    ...
    def ctaGetSize(self, path):
        cmd = "xrdfs %s stat %s | grep ^Size | awk '{print $2}" % (self.cta_xrd_server, path)
        pair = self.cmd.cmd_output(cmd)
        ctasize = -1
        if (pair[0] != 0 or len(pair) < 2):
            print "%s\nreturned %d\n" %(cmd, pair[0])
            print "ctaGetSize-E-of %s" %(path)
        return pair[1]
    ...
    def copyFile(self,input,output):
        ...
        if(input.find(self.cta_prefix)>=0):
            ...
            cmdstatus=self.__file_ls(input)
            if(cmdstatus):
                self.__log_error("copyFile-W-Failed %s" % cmdn)
                self.__log_error("copyFile-W-Failed %s" % output)
                cmove=self.getmoveCTA(input,output)
                if(cmove!=None):
                    input=cmove
                else:
                    if(self.tapeonly):
                        mutex.acquire()
                        self.__log_info("copyFile-E-FailedTapeOnly %s %s" % (input, output))
                        return 1
            if(self.tapeonly):
                mutex.acquire()
                return 1024
...
#
        ...
        if(ifa[1]==ofa[1]):
            ...
            cmdstatus = self.__file_rename(input, output)
            self.__log_info("calling __file_rename(%s, %s) ..." %(input, output))
        elif (input.find(self.eoslink) >= 0 or output.find(self.eoslink) >= 0 or xrdcopy):
            force = 1
            noprogress = 1
            timeout = 1800
            signal = 9
            cmdstatus = self.__xrdcp(input, output, noprogress, self.v, force, timeout, signal)
            self.__log_info("calling __xrdcp(%s, %s) ..." %(input, output))
        else:
            #cmd="cp -pi -d -v "+input+" "+output
            self.__log_info("running command %s " % cmd)
            cmdstatus=self.cmd.cmd(cmd)
...
#                        break
    ...
    def InsertNtuple(self,run,version,type,jid,fevent,levent,events,errors,timestamp,size,status,path,crc,crctime,crcflag,ctatime,datamc,fetime=None,letime=None,eostime=0):
...
#            self.cmd.cmd(cmd)
            ...
            sql=" insert into datafiles (RUN,VERSION,TYPE,FEVENT,LEVENT,NEVENTS,NEVENTSERR,TIMESTAMP,SIZEMB,STATUS,PATH,PATHB,CRC,CRCTIME,CTATIME,BACKUPTIME,TAG,FETIME,LETIME,PATHS,EOSTIME) values(%d,'%s','%s',%d,%d,%d,%d,%d,%s,'%s','%s',' ',%d,%d,%d,0,%s,%d,%d,'%s',%d)" %(run,version,stype,fevent,levent,events,errors,timestamp,sizemb,status,path,crc,crctime,ctatime,part,fetime,letime,paths,eostime)
        ...
        self.sqlserver.Update(sql);
        sql = "INSERT INTO ntuples (RUN,VERSION,TYPE,JID,FEVENT,LEVENT,NEVENTS,NEVENTSERR,TIMESTAMP,SIZEMB,STATUS,PATH,CRC,CRCTIME,CRCFLAG,CTATIME,BUILDNO,DATAMC,EOSTIME) VALUES( %d, '%s','%s',%d,%d,%d,%d,%d,%d,%s,'%s','%s',%d,%d,%d,%d,%s,%d,%d)" %(run,version,type,jid,fevent,levent,events,errors,timestamp,sizemb,status,path,crc,crctime,crcflag,ctatime,buildno,datamc,eostime)
	print "  insert ntuple before ",sql
    ...
    def setenv(self):
        ...
        elif(os.environ.has_key('AMSDataDirRW')):
    ...
    def validateDST(self,fname,nevents,ftype,levent):
        ...
        if(dtype!=None):
            try:
                ...
                print "OK..."
            if(fname.find(self.cta_prefix)>=0):
                prefix="root://%s/" %(self.cta_xrd_server)
            else:
                prefix=""
            ...
            vcode=self.cmd.cmd(validatecmd)
#            mutex.release()
            if (fname.find(self.cta_prefix)>=0 and vcode != 0 and vcode/256==134):
                self.__stager(fname)
#                time.sleep(5)
                while (sys.argv[0].find('parsejf') >=0 and sys.argv[0].find('parsejf.data') < 0 and vcode/256==134):
                    print "Timeout '%s', waiting 30 seconds ..." %(validatecmd)
                    time.sleep(30)
                    vcode=self.cmd.cmd(validatecmd)
            try:
        ...
        print "validateDST ", fname, " returning (ret=", ret, ", vcode=", vcode, ")."
        return ret,vcode
    ...
    def compareCRC(self, filename, crc, crc32=0):
        if (hasattr(self, 'skipcrc') and self.skipcrc):
            return 1

        if (hasattr(self, 'crcCalls')):
            self.crcCalls=self.crcCalls+1
        time0=time.time()
        if (filename.find('/eosams/') == 0):
            filename = self.eosLink2Xrootd(filename)
        elif (filename.find('/eos/ams/') == 0):
            filename = self.eosReal2Xrootd(filename)
        ...
        if(filename.find(self.cta_prefix) >= 0):
            rstatus=1
        else:
            rstatus=self.cmd.cmd(crccmd)
            rstatus=rstatus>>8
        ...
        if (hasattr(self, 'crcTime')):
            self.crcTime=self.crcTime+time.time()-time0
        if(crc==0 and hasattr(self, 'crczero') and self.crczero==1):
            return 1
        else:
            return rstatus


    def parseJournalFiles(self,d,i,v,h,s,m,mt=1,to=0,eos=0,force=0,skipcrc=0,ciid=0,superfast=0):
        self.ciid=ciid
        self.tapeonly=to
        self.tapecopy=0
        self.eos=eos
        ...
        if(ciid==0):
            ...
            sql="SELECT dirpath,journals.timelast,name,journals.cid FROM journals,cites WHERE cites.cid=%d and journals.cid=cites.cid" %(ciid)
            ret=self.sqlserver.Query(sql)
        cid=-1
        global mutex
#        mutex=thread.allocate_lock()
        ...
        if(len(ret)>0):
            for jou in ret:
                ...
                if(cid != jou[3]):
                    ...
                    joudir=dir+"/jou"
                    ntdir=dir+"/nt"
                    if (joudir.find('/eos/ams/') == 0):
                        p = subprocess.Popen(['eos','ls',joudir],stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                        out, err = p.communicate()
                        if (p.returncode != 0):
                            print("Cannot list %s" %(joudir))
                            continue
                        allfiles = out.split()
                    else:
                        try:
                    ...
                    for file in allfiles:
                        ...
                        if(len(junk)>1):
                            ...
                            if (not re.match("\.journal$", file)):
                                continue
#                            elif (junk[1] == "work"):
#                                ret = self.cmd.cmd("mv %s/%s %s/%s" %(joudir,file,joudir,file.replace(".work","")))
#                                if (ret == 0):
...
#                                    continue
                        ...
                        if(writetime> timestamp-24*60*60*30):
                            ...
                            if (self.mt):
                                while threading.activeCount() > maxt:
#                                    print "waiting..."
                                    time.sleep(1)
                                try:
    ...
    def printParserStat(self):
        ...
        for i  in range(0,self.nCheckedCite+1):
            ...
            if self.JF[i] > 0:
                ...
                l1 = "   DSTs (Checked, Good, Bad, CRC, CopyFail, BadRunID) :  %d %d %d %d %d %d" %(self.CheckedDSTs[i],  self.GoodDSTs[i], self.BadDSTs[i], self.BadCRC[i], self.BadDSTCopy[i], self.BadRunID[i])
                print l0
                print l1
                output.write(l0 + "\n")
                output.write(l1 + "\n")
...
#        outputpath=None
        ...
        try:
            mtim=os.stat(inputfile)[ST_MTIME]
        except OSError,e:
            mtim = 0
        tnow=int(time.time())
        if(tnow-mtim<30):
            print "journal %s not yet completed.  Postponed " %(inputfile)
            return 0, ""

        mutex.acquire()
        inputwork = inputfile + ".work"
        inputorig = inputfile
        ret = self.cmd.cmd("mv %s %s" %(inputfile, inputwork))
        if (ret == 0):
            inputfile = inputwork
        else:
        ...
        try:
            ...
            print e
            self.cmd.cmd("mv %s %s" %(inputwork, inputorig))
            mutex.release()
            return 0, ""
        sp1=buf.split(', UID ')
        if(len(sp1)>1):
            ...
            if(inputfile.find(sp2[0])<0):
                ...
                try:
                    ...
                    print e
                    self.cmd.cmd("mv %s %s" %(inputwork, inputfile))
                    mutex.release()
                    return 0, ""
                tnow=int(time.time())
                unstaged = 0
                for file in files:
                    if(file.find(sp2[0])>=0):
                        ...
                        if (realpath.find(self.cta_prefix) >=0 ):
                            (ret,out) = self.__stager_qry(realpath)
                            if (ret != 55):
                                self.__stager(realpath)
                                unstaged += 1
...
#                self.cmd.cmd("mv %s %s.0" %(inputfile, inputorig))
#                output.write(lastline + "\n")
#                output.close
#                mutex.release()
#                return (jobid, copylog)
            if(block.find('RunIncomplete')>=0):
                if(startingrunR ==1):
                    ...
                    runfinishedR=1
                    jj=joufile.split('.journal')
                    jobid=jj[0]
#                    mailto="vitali.choutko@cern.ch"
                    mailto="baosong.shan@cern.ch"
                    ...
                    sql = "SELECT Mails.address FROM Jobs, Mails WHERE Jobs.jid=%d AND Mails.mid = Jobs.mid" %(int(jobid))
                    ret = self.sqlserver.Query(sql)
                    if(len(ret)>0):
...
#
                ...
                if(len(rq)>0 and rq[0][0].find('Completed')>=0):
                    ...
                    self.cmd.cmd("mv -f %s %s.1" %(inputfile,inputorig))
                    mutex.release()
                    return 0, copylog
                ...
                if (patternsmatched == len(CloseDSTPatterns)-1):
                    if(len(closedst[fileIndx].split(':')) < 2):
                        output.write("CloseDST format error")
                    elif(closedst[crcIndx]==0):
                        ...
                        if (re.match("^root://%s///eos" %(self.cta_xrd_server), pathwithouthost)):
                            pathwithouthost = pathwithouthost.replace("root://%s///eos" %(self.cta_xrd_server), "/eos")
                            print "path modified to %s" %(pathwithouthost)
                        junk=closedst[fileIndx].split('/')
                        dstfile=self.trimblanks(junk[len(junk)-1])
                        filename=dstfile
                        dstfile=dirpath+"/"+dstfile
                        if (not os.path.exists(dstfile) and (re.match("^/eosams/", pathwithouthost) or re.match("^/eos/ams/", pathwithouthost) or re.match("^%s" %(self.cta_prefix), pathwithouthost))):
                        ...
                        dstlink = dstfile
                        if (re.match("^%s" %(self.cta_prefix), inputfilel)):
                            dstfile = inputfilel
                            if (outputpath is None):
                                ouputpath = self.cta_prefix
...
#                                outputpath = '/eosams'
                            ...
                            if(len(rq)>0 and rq[0][0] == disk):
                                ...
                                if(len(rq)>0 and rq[0][0] == disk):
                                    outputpath = disk
                        if (re.match("^%s" %(self.cta_prefix), dstfile)):
                            pid = os.getpid()
                            tmpf = "/tmp/cta.%d" %(pid)
                            if (os.path.isfile(tmpf)):
                                try:
                                    ...
                                    print e
#                            self.cmd.cmd("/afs/cern.ch/ams/local/bin/timeout --signal 9 600 stager_get -M %s" %(dstfile))
                            cmd = "xrdfs %s ls -l %s >%s" %(self.cta_xrd_server, dstfile, tmpf)
#                           output example: ---- 2020-12-10 16:17:03         832 /eos/ctapublic/archive/ams/test/test.job
                            i = self.cmd.cmd(cmd)
                            if (i == 0):
                                try:
                                    fo = open(tmpf, "r")
                            ...
                            os.unlink(tmpf)
                        elif (re.match("^%s" %(self.cta_prefix), inputfilel)):
                            dstsize = self.ctaGetSize(inputfilel)
                            if (dstsize < 0):
                        ...
                        if (dstsize == -1):
                            ...
                            if (i == 0xff00 or i & 0xff):
                                ...
                                if(i==134):
                                    ntstatus="STAGEIN"
                                    self.cmd.cmd("mv -v %s %s" %(inputwork, inputorig))
                                    mutex.release()
                                    return 0, copylog
                                if (int(i/128)):
                                    ntevents = 0
                                    ...
                                    if (runtype % 2 == 0):
                                        ...
                                        path = '/Data'   
                                    ctatime = 0
                                    (outputpatha, rstatus, odisk, ctatime) = self.doCopy(jobid, dstfile, ntcrc, version, outputpath, path)
                                    outputpath = outputpatha
                                    if (outputpath != None):
                                        mvntuples.append(outputpath)
                                    else:
                                    ...
                                    if (rstatus == 1):
                                        if (self.skipcrc or (self.tapeonly and outputpath.find(self.cta_prefix) >= 0)):
                                            crctime = 0
                                        else:
                                            crctime = timestamp
                                        self.InsertNtuple(run, version, nttype, jobid, dstfevent, dstlevent, ntevents, badevents, timestamp, dstsize, ntstatus, outputpath, ntcrc, crctime, 1, ctatime, runtype%2, feti, leti)
                                        self.sqlserver.Commit(1)
...
#
                ...
                try:
                    ...
                    output.write("parseJournalFile -W- RunFinished - invalid format %s\n" %(block))
                    formatOK = False
                    
                    
                if ((patternsmatched == len(RunFinishedPatterns) or patternsmatched == len(RunFinishedPatterns) - 1) and formatOK == True):
...
#            if (copyfailed == 0):
            if (validated_ntuples > 0):
                if (len(cpntuples) > 0):
                    ...
                    for ntuple in cpntuples:
                        cmd = self.rm + ntuple
                        if (ntuple.find('/eosams/') == 0 or ntuple.find('/eos/ams/') == 0):
            ...
            if (startingrun[2] != 0):
                ...
                if (status == 'Failed' or status == 'Unchecked'):
                    ...
                    if (ret[0][0] != None):
                        ...
                        output.close
                        self.cmd.cmd("mv %s %s" %(inputfile, inputfileLink))
                        mutex.release()
                        self.suc[jobid] = jobid
                        return (jobid, copylog)
            self.cmd.cmd("mv %s %s" %(inputfile, inputfileLink))
            self.cmd.cmd("cat %s >> %s" %(inputfileAdd, inputfileLink))
...
#                        self.sqlserver.Update(sql)
    ...
    def CheckDataSet(self,run2p,dataset,v,f,tab=0,runmin=0,runmax=2000000000,tagmin=0,tagmax=65535):
        self.verbose=v
        self.run2p=run2p
        self.force=0         
        ...
        if(dataset.find("/")>=0):
            ...
            if(len(ds)==1):
                did=ds[0][0]
                if(f==2):
                    sql="select path from ntuples where path like '%%%s/%%' and datamc=1  and (path not like '%s%%' or eostime>0)" %(datapath, self.cta_prefix)
                    files=self.sqlserver.Query(sql)
                    for file in files:
                        filepath=file[0].replace(self.cta_prefix, self.eoslink)
                        self.linkdataset(filepath,"/afs/cern.ch/ams/Offline/DataSetsDir",1)
	    elif(len(ds)>1):
        ...
        runadd=" and datafiles.run>=%s and datafiles.run<=%s and datafiles.tag>=%s and datafiles.tag <=%s " %(runmin,runmax,tagmin,tagmax)
        ...
        if(self.force>1):
            eos_prefix="/eos/ams"
#            eosmount="%s -b fuse mount %s" (self.eosselect, self.eos_prefix)
#            self.cmd.cmd(eosmount)
            sql="select path,run,sizemb,jid,buildno from ntuples where path like '%%%s/%%' and datamc=1  %s " %(datapath,runn)
            paths=self.sqlserver.Query(sql)
            for path in paths:
                eospath = re.sub(r'.*/Data/', '/eos/ams/Data/', path[0])
                ret = os.system("/bin/ls %s" %(eospath))
#                print "Checking %s ..." %(eospath)
                if (ret):
                    badeos.append(path[1])
                    print "Run ",path[1] ," have eos problems "
                continue
                if(paht[0].find(self.cta_prefix) >= 0):
                    patheos = path[0].replace(self.cta_prefix, self.eos_prefix, 1)
                    eos_size = eosGetSize(patheos)
                    cta_size = catGetSize(path[0])
                    if (eos_size != cta_size or eos_size == 0):
                        badeos.append(path[1])
                        print "Run ",path[1] ," have eos problems "
                elif ( abs(int(eos_size)/1024/1024-sizemb)>2):
                    if(int(eos_size)/1000/1000-sizemb<2):
                        print " correcting size ",int(int(eos_size)/1000/1000),sizemb
                        sizemb=int(int(eos_size)/1024/1024+0.5)
                        sql="update ntuples set sizemb=%d where path like '%s'" %(sizemb,path[0])
                        self.sqlserver.Update(sql)
                        self.sqlserver.Commit(1)
                    else:
#                         badeos.append(path[1])
                         print "Run ",path[1] ," have eos problems size ",eos_size,cta_size,sizemb
	    ...
	    print "pass1 ended"
            if(len(files)>0):
  			...
  			tot=tot+1
                        if(tab==0):
                            print "Run ",run,"  not found in dataset ",dataset,tot
                            eosfind = "ls -l /eos/ams/Data/AMS02/2014/ISS.B700/pass5/%d*.root" % run[0]
                            eosout = self.cmd.cmd_output(eosfind)
    ...
    def DeleteDataSet(self,run2p,dataset,u,v,f,donly,datamc,buildno,tapeonly):
        self.update=u
        self.verbose=v
        self.run2p=run2p
        self.force=f
        ...
        if(tapeonly!=0):
            buildno=1
            if(tapeonly>0):
                runbuild=runbuild+" and ntuples.ctatime>0 "
                runbuildd=runbuildd+" and ntuples_deleted.ctatime>0 "
            if(tapeonly<0):
                runbuild=runbuild+" and ntuples.ctatime=0 "
                runbuildd=runbuildd+" and ntuples_deleted.ctatime=0 "
        if(datamc==0):
        ...
        if(datamc!=0):
            ...
            if(run2p/10000000000>0):
                ...
                runn=" and ntuples.run<%d " %(run2p)
                runnd=" and ntuples_deleted.run<%d " %(run2p)
        sql="select path,ctatime,eostime from ntuples where path like '%%%s/%%' and datamc=%d %s %s %s" %(dataset,datamc%10,runn,runbuild,arun2p) 
        df=0
        files=self.sqlserver.Query(sql)
        datapath=dataset
        ds1=""
        ...
        if(self.update):
            for file in files:
                ...
                if(file[1]>0):
                    ...
                    if len(junk)>=2:
                        ctafile=self.cta_prefix+delimiter+junk[1]
                        print "Removing %s ..." %(ctafile)
                        i=self.__file_remove(ctafile)
                        if(i):
                            print " CTARemove Failed ",ctafile
                if(file[2]>0 or self.force):
                    ...
                    if len(junk)>=2:
                        ...
                        if len(junk2) >= 2:
                            ...
                            eoschmod = "%s chmod " %(self.eosselect)
                            self.cmd.cmd(eoschmod + " 750 " + eosdir)
                            eosdel="%s rm %s" %(self.eosselect, eosfile)
                            i=self.cmd.cmd(eosdel)
#                            self.cmd.cmd(eoschmod + " 550 " + eosdir)
                            if(i):
        ...
        if(len(files)==0):
            if(datamc==0):
                if(donly==0):
                    df=1
                    sql="select path,ctatime,eostime from datafiles where path like '%%%s/%%' and type like 'MC%%'  %s %s " %(datapath,runndf,arun2p) 
                    files=self.sqlserver.Query(sql)
        if(len(files)>0 or self.force!=0):
            arun2p_ntuples = arun2p.replace('run=', 'ntuples.run=')
            if(buildno>0):
                sql="insert into jobs_deleted select jobs.* from jobs,%s,ntuples where jobs.jobname like '%%%s.job'  and jobs.jid=%s.jid and ntuples.jid=jobs.jid %s %s  %s and ntuples.jid=%s.jid %s and ntuples.path like '%%%s/%%'" %(runsname,dataset,runsname,runst,rund,runbuild,runsname,arun2p_ntuples,datapath)
            else:
                sql="insert into jobs_deleted select jobs.* from jobs,%s,ntuples where jobs.jobname like '%%%s.job'  and jobs.jid=%s.jid and ntuples.jid=jobs.jid %s %s %s and ntuples.jid=%s.jid and ntuples.path like '%%%s/%%' " %(runsname,dataset,runsname,runst,rund,arun2p_ntuples,runsname,datapath)
                
            if(donly==0):
                self.sqlserver.Update(sql)
            if(buildno>0):
                sql=" delete from jobs where exists (select * from %s,ntuples where %s.jid=jobs.jid and jobs.jid=ntuples.jid and jobs.jobname like '%%%s.job' %s %s %s  and ntuples.jid=%s.jid %s and ntuples.path like '%%%s/%%')" %(runsname,runsname,dataset,runst,rund,runbuild,runsname,arun2p_ntuples,datapath)
            else:
                sql=" delete from jobs where exists (select * from %s,ntuples where %s.jid=jobs.jid and jobs.jid=ntuples.jid and jobs.jobname like '%%%s.job' %s %s %s and ntuples.jid=%s.jid and ntuples.path like '%%%s/%%')" %(runsname,runsname,dataset,runst,rund,arun2p_ntuples,runsname,datapath)
        ...
        while 1:
            for filej in os.listdir(joudir):
                ...
                if(not os.path.isfile(pfile)):
                    self.__log_error("File not found %s" %(pfile))
                    continue

                journalFile = JournalFile.JournalFile(pfilej)
                if journalFile.ok():
                    self.__log_info("Run %d" %(journalFile.run))

                    if(run2p!=0 and journalFile.run!=run2p):
                        continue

		    if(abs(journalFile.tfevent - journalFile.run) > 3600):
                        self.__log_error("TransferDataRuns-E-WrongRun %d %d" %(journalFile.run, journalFile.tfevent))
                        cmd = "mv %s %s.0" % (journalFile.file_path, journalFile.file_path)
                        self._cmd(cmd)
                        continue

                    ret = self.sqlserver.sql("SELECT PATH,RUN,NEVENTS,TYPE,FEVENT,LEVENT,PATHB,STATUS FROM DATAFILES WHERE PATH LIKE '%' || :path", path=file);
                    calibnotfull=False
                    if(len(ret)>0):
                        ...
                        if(ret[0][3].find("SCI")>=0 and (ret[0][2]<journalFile.events or (ret[0][2]==journalFile.events and ret[0][6] != origpath) ) and ret[0][4]>=journalFile.fevent and ret[0][5]<=journalFile.levent):
                           calibnotfull=True
                        if(ret[0][3].find("SCI")>=0 and ret[0][2]<journalFile.events and ret[0][7].find("SHORT")>=0):
                           calibnotfull=True
                        ...
                        if((calibnotfull or replace) and (run2p==0 or ret[0][1] == run2p) and (disk==None or ret[0][0].find(disk)>=0)):
                            ...
                            i = self.cmd.cmd(cmd)
                            if(i):
                                self.__log_error("command failed '%s'" %(cmd))
                                if(replace>1):
                    ...
                    if(journalFile.t0=="5"):
                        ...
                        try:
                           ...
                           itag=0
                        if(itag==204  and int(journalFile.tag)<20480):
                           type="LAS"
                        ...
                        type="CAB"
                    (outputpath, ret, ctatime) = self.__doCopyRaw(journalFile, pfile, '/Data', type)
                    ...
                    if(ret==1):
                        ...
                        result = self.sqlserver.sql(
                            ...
                            (RUN,VERSION,TYPE,FEVENT,LEVENT,NEVENTS,NEVENTSERR,TIMESTAMP,SIZEMB,STATUS,PATH,PATHB,CRC,CRC32,CRCTIME,CTATIME,BACKUPTIME,TAG,FETIME,LETIME,PATHS,EOSTIME) \
                            VALUES \
                            (:run,:version,:type,:fevent,:levent,:nevents,:neventserr,:timestamp,:sizemb,:status,:path,:pathb,:crc,:crc32,:crctime,:ctatime,:backuptime,:tag,:fetime,:letime,:paths,:eostime)",
                            run = journalFile.run,
                        ...
                        if(outputpath!=None):
                            cmd = "rm %s" % outputpath
                            self.__log_error("TransferDataFiles-ECopyFail, run '%s'" %(cmd))
                            time.sleep(60) 
...
#
                ...
                if (CloseDSTPatterns['Type'] == 'RootFile'):
                    if (CloseDSTPatterns['Name'] != None):
                        ...
                        self.cmd.cmd(validatecmd)
                        fnam += '.jou'
                        self.cmd.cmd("sed -i \"s/\\.job.*/.job/g\" %s" %(fnam))
                        bufj = ''
                        try:
                            jouf = open(fnam, 'r')
                        except IOError, e:
    ...
    def __log_debug(self, str):
        if self.logger == None:
            print str
        else:
            self.logger.debug(str)

    def __log_warning(self, str):
        if self.logger == None:
            print str
        else:
            self.logger.warning(str)

    def __log_critical(self, str):
        if self.logger == None:
            print str
        else:
            self.logger.critical(str)

    def __exists_on_EOS(self, path):
        p = subprocess.Popen([self.eosselect, 'ls', path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = p.communicate()
        return p.returncode

    def __get_cta_xrootd_url(self, cta_p):
        return "root://%s/%s" %(self.cta_xrd_server, cta_p)

    def __get_eos_xrootd_url(self, eos_p):
        return "root://%s/%s" %(self.eos_xrd_server, eos_p)

    def __update_eos_time_db(self, eos_p):
        (mtime, size) = self.xrdGetStatTimeSize(eos_p)
        common_p = self.getCommonPartOfPath(eos_p)
        if re.match(r".*\.root$", common_p):
            table = 'NTUPLES'
        else:
            table = 'DATAFILES'
        sql = "UPDATE AMSDES.%s SET PATH=REPLACE(PATH, '%s/', '%s/'), EOSTIME=%d WHERE REGEXP_SUBSTR(path,'/(Data|MC)/.*') = '%s'" %(table, self.cta_prefix, self.eoslink, mtime, common_p)
        self.sqlserver.Update(sql)
        self.__log_debug(sql)
        return self.sqlserver.Commit(1)

    def __get_xrootd_url(self, path):
        if (path.find("root://") >= 0):
            return path
        elif (path.find(self.eoslink) >= 0 or path.find(self.eos_prefix) >= 0):
            return self.eosReal2Xrootd(self.eosLink2Real(path))
        elif (path.find(self.cta_prefix) >= 0):
            return self.__get_cta_xrootd_url(path)
        else:
            return path

    def __get_timeout_command(self, timeout = 0, signal = 0):
        cmd = []
        if (timeout > 0):
            cmd.append(self.timeout_cmd)
            if (signal > 0):
                cmd.append('--signal')
    ...
    def __popen(self, cmd):
        ...
        out, err = p.communicate()
        self.__log_info(cmd)
        self.__log_info(p.returncode)
        self.__log_info(out)
        self.__log_info(err)
        if (p.returncode):
            self.__log_debug(cmd)
            self.__log_debug(out+err)
        return p.returncode


    def __file_ls(self, path, timeout = 0, signal = 0):
        cmd = self.__get_timeout_command(timeout, signal)
        if (path.find(self.eoslink) >= 0 or path.find(self.eos_prefix) >= 0):
            path = self.eosLink2Real(path)
            cmd.append("eos")
        elif (path.find(self.cta_prefix) >= 0):
            cmd.append("xrdfs")
            cmd.append(self.cta_xrd_server)
    ...
    def __file_remove(self, path, verbose = 1, force = 0, timeout = 0, signal = 0):
        xrdfs = 0
        cmd = self.__get_timeout_command(timeout, signal)
        if (path.find(self.eoslink) >= 0 or path.find(self.eos_prefix) >= 0):
            cmd.append('eos')
            ...
            xrdfs = 1
        cmd.append('rm')
        if (verbose and xrdfs == 0):
            cmd.append('-v')
        if (force and xrdfs == 0):
            cmd.append('-f')
        cmd.append(path)
        return self.__popen(cmd)


    def __xrdcp(self, orig, dest, noprogress = 1, verbose = 1, force = 0, timeout = 0, signal = 0):
        cmd = self.__get_timeout_command(timeout, signal)
        cmd.append('/usr/bin/xrdcp')
        if (noprogress):
            cmd.append('-np')
        if (verbose):
            cmd.append('-v')
        if (force):
            cmd.append('-f')
        cmd.append(self.__get_xrootd_url(orig))
        cmd.append(self.__get_xrootd_url(dest))
        return self.__popen(cmd)


    def __file_rename(self, orig, dest, verbose = 1, force = 0, timeout = 0, signal = 0):
        cmd = self.__get_timeout_command(timeout, signal)
        if (orig.find(self.eoslink) >= 0 or orig.find(self.eos_prefix) >= 0):
            if (dest.find(self.eoslink) >= 0 or dest.find(self.eos_prefix) >= 0):
                orig = self.eosLink2Real(orig)
                dest = self.eosLink2Real(dest)
                cmd.append("eos")
                cmd.append("file")
                cmd.append("rename")
            else:
                self.__log_error("Both paths must be on the same service, EOS, CTA, or other FS.")
                return -1
        elif (orig.find(self.cta_prefix) >= 0):
            if (dest.find(self.cta_prefix) >= 0):
                cmd.append("xrdfs")
                cmd.append(self.cta_xrd_server)
                cmd.append("mv")
            else:
            ...
            if (dest.find(self.eoslink) >= 0 or dest.find(self.eos_prefix) >= 0 or dest.find(self.cta_prefix) >= 0):
                self.__log_error("Both paths must be on the same service, EOS, CTA, or other FS.")
                return -1
            else:
                cmd.append("mv")
    ...
    def __stager_qry(self, path):
        if (path.find(self.cta_prefix) >= 0):
            (ret, out) = self.cmd.cmd_output("xrdfs %s stat -q Offline %s" %(self.cta_xrd_server, path))
            return (ret/256, out)
        return (0, '')


    def __evict(self, path):
        if (path.find(self.cta_prefix) >= 0):
            (ret, out) = self.cmd.cmd_output("xrdfs %s prepare -e %s" %(self.cta_xrd_server, path))
            return (ret/256, out)
        return (0, '')

    def __stager(self, path):
        if (path.find(self.cta_prefix) >= 0):
            path = re.sub(".*%s" %(self.cta_prefix), self.cta_prefix, path)
            (ret,out) = self.__stager_qry(path)
            self.__log_debug("%d : %s" %(ret, out))
            if (ret == 55):
                return 0
            else:
                self.cmd.cmd_output("xrdfs %s prepare -s %s" %(self.cta_xrd_server, path))
        else:
            return 0
        return 1
        
    def CopyToEOS(self, path, force = 0):
        noprogress = 1
        verbose = 1
        common_p = self.getCommonPartOfPath(self.eosLink2Real(path))
        eos_p    = "%s%s" %(self.eos_prefix, common_p)
        path_adler = self.xrdGetChecksum(self.eosLink2Real(path))
        if (path_adler == ""):
            self.__log_error("Cannot get adler checksum for original file: %s" % (path))
            return -1
        if (self.__exists_on_EOS(eos_p) != 0 or self.xrdGetChecksum(eos_p) != path_adler):
            if (self.__stager(path) != 0):
                self.__log_warning("%s is not staged yet, skipping ..." %(path))
                return 1
            retcode = self.__xrdcp(path, eos_p, noprogress, verbose, force)
            if (retcode != 0):
                self.__log_error("xrdcp %s %s failed, return code is: %d" %(path, eos_p, retcode))
                return -2
            else:
                (ret,out) = self.__evict(path)
                self.__log_info("Success: %s copied to %s, evict returned %d" %(path, eos_p, ret))
            eos_adler = self.xrdGetChecksum(eos_p)
            if (path_adler != eos_adler):
                self.__log_error("EOS adler is different from original path: %s vs %s" %(eos_adler, path_adler))
                return 1
        self.__update_eos_time_db(eos_p)

    def AutoUploadToEOS(self):
        if self.logger == None:
            FORMAT = '[%(asctime)-19.19s %(levelname)-8s] %(message)s'
            logging.basicConfig(format=FORMAT)
#            self.logger =  Logger.Logger(inspect.stack()[0][3]).getLogger(level_log=logging.DEBUG)
            self.logger =  logging.getLogger(inspect.stack()[0][3])
            self.logger.setLevel(logging.getLevelName('DEBUG'))
        paths2copy = []
        sql_ds2copy = "SELECT DISTINCT DATASET,DATASETS.DATAMC,PRODUCTIONSET.NAME,JOBNAME FROM AMSDES.DATASETSDESC LEFT JOIN AMSDES.DATASETS ON DATASETSDESC.DID=DATASETS.DID LEFT JOIN AMSDES.PRODUCTIONSET ON DATASETS.VERSION=PRODUCTIONSET.VDB WHERE KEEPONEOS=1"
        ret_ds2copy = self.sqlserver.Query(sql_ds2copy)
        for ds in ret_ds2copy:
            dataset = ds[0]
            prodset = ds[2]
            if (ds[1] == 1):
                datamc = 'Data'
                ...
                datamc = 'MC'
            template = re.sub('\.job$', '', ds[3])
            paths2copy.append("/%s/%s/%s/%s/" %(datamc, prodset, dataset, template))

        list2copy = list()
        for common_path in paths2copy:
#            sql_file2copy = "SELECT PATH FROM AMSDES.NTUPLES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME<=0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '%s/') UNION SELECT PATH FROM AMSDES.DATAFILES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME<=0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '%s/')" %(common_path, self.cta_prefix, common_path, self.cta_prefix)
            sql_file2copy = "SELECT PATH FROM AMSDES.NTUPLES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME<=0 AND REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) != '%s/') UNION SELECT PATH FROM AMSDES.DATAFILES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME<=0 AND REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) != '%s/')" %(common_path, self.cta_prefix, common_path, self.cta_prefix)
            ret_file2copy = self.sqlserver.Query(sql_file2copy)
            if (len(ret_file2copy) > 0):
                self.__log_info("%s %d" %(common_path, len(ret_file2copy)))
                list2copy.append((len(ret_file2copy), common_path))

        list2copy.sort()
        for item in list2copy:
            nTotal = item[0]
            common_path = item[1]
            self.UploadToEOS(common_path, "root", 1, 1)

    def UploadRawToEOS(self, verbose=1, overwrite=0):
#        path:          EOS path to be removed
#        verbose:       print verbosed messages
#        update:        If update DB
#        irm:           If using interactive rm command
#        notverify:     Not to check the consistency of tape backup
#        force:
        if self.logger == None:
            FORMAT = '[%(asctime)-19.19s %(levelname)-8s] %(message)s'
            logging.basicConfig(format=FORMAT)
#            self.logger =  Logger.Logger(inspect.stack()[0][3]).getLogger(level_log=logging.DEBUG)
            self.logger =  logging.getLogger(inspect.stack()[0][3])
            self.logger.setLevel(logging.getLevelName('DEBUG'))
        sql = "SELECT PATH FROM AMSDES.DATAFILES WHERE PATH LIKE '%%/Data/AMS02/2011A/RawData/%%' AND (EOSTIME<=0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) ='%s/') AND (substr(type,1,instr(type,' ')-1) = 'SCI' OR substr(type,1,instr(type,' ')-1) = 'CAL')" %(self.cta_prefix)

        self.__log_debug(sql)
        self.__log_info("Processing RawData ...")
        ret_files=self.sqlserver.Query(sql)
        inconsistent = []
        for ntuple in ret_files:
            path = ntuple[0]
            self.CopyToEOS(path, overwrite)



    def UploadToEOS(self, path, ftype, verbose=1, overwrite=0):
#        path:          EOS path to be removed
#        verbose:       print verbosed messages
#        update:        If update DB
#        irm:           If using interactive rm command
...
#            self.logger =  Logger.Logger(inspect.stack()[0][3]).getLogger(level_log=logging.DEBUG)
            self.logger =  logging.getLogger(inspect.stack()[0][3])
            self.logger.setLevel(logging.getLevelName('DEBUG'))
        common_path = self.getCommonPartOfPath(self.eosLink2Real(path))
        self.__log_debug(common_path)
        sql_get_ntuples ="SELECT PATH FROM AMSDES.NTUPLES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME<=0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '%s/')" %(common_path, self.cta_prefix)
        sql_get_datafiles ="SELECT PATH FROM AMSDES.DATAFILES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME<=0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '%s/')" %(common_path, self.cta_prefix)
        if (ftype == 'root'):
            sql = sql_get_ntuples
        elif (ftype == 'raw'):
            sql = sql_get_datafiles
        elif (ftype == 'both'):
            sql = "%s UNION %s" %(sql_get_ntuples, sql_get_datafiles)
        else:
            self.__log_critical("Please specify data type(s) to remove.")
            return 1

        self.__log_debug(sql)
        self.__log_info("Processing %s : %s" %(path, ftype))
        ret_files=self.sqlserver.Query(sql)
        inconsistent = []
        for ntuple in ret_files:
            path = ntuple[0]
...
#            self.logger =  Logger.Logger(inspect.stack()[0][3]).getLogger(level_log=logging.DEBUG)
            ...
            self.logger.setLevel(logging.getLevelName('DEBUG'))
        paths2remove = []
        self.__log_info("Starting ...")

        sql_ds2remove = "SELECT DISTINCT DATASET,DATASETS.DATAMC,PRODUCTIONSET.NAME,JOBNAME FROM AMSDES.DATASETSDESC LEFT JOIN AMSDES.DATASETS ON DATASETSDESC.DID=DATASETS.DID LEFT JOIN AMSDES.PRODUCTIONSET ON DATASETS.VERSION=PRODUCTIONSET.VDB WHERE KEEPONEOS=0"
        ret_ds2remove = self.sqlserver.Query(sql_ds2remove)
        for ds in ret_ds2remove:
            dataset = ds[0]
            prodset = ds[2]
            if (ds[1] == 1):
                datamc = 'Data'
                ...
                datamc = 'MC'
            template = re.sub('\.job$', '', ds[3])
            paths2remove.append("/%s/%s/%s/%s/" %(datamc, prodset, dataset, template))

        list2remove = list()
        for common_path in paths2remove:
            sql_file2remove = "SELECT PATH FROM AMSDES.NTUPLES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/') UNION SELECT PATH FROM AMSDES.DATAFILES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/')" %(common_path, common_path)
            ret_file2remove = self.sqlserver.Query(sql_file2remove)
            sql_file_wo_backup = "SELECT PATH FROM AMSDES.NTUPLES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/') AND CTATIME <= 0 UNION SELECT PATH FROM AMSDES.DATAFILES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/') AND CTATIME <= 0" %(common_path, common_path)
            ret_file_wo_backup = self.sqlserver.Query(sql_file_wo_backup)
            if (len(ret_file2remove) > 0): # and len(ret_file_wo_backup) > 0):
                self.__log_info("%s %d %d" %(common_path, len(ret_file2remove), len(ret_file_wo_backup)))
                list2remove.append((len(ret_file2remove), len(ret_file_wo_backup), common_path))
        ...
        for item in list2remove:
            ...
            nNoBackup = item[1]
            common_path = item[2]
            p = subprocess.Popen([self.eosselect, 'ls', '-d', "%s%s" %(self.eos_prefix, common_path)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out, err = p.communicate()
            if (p.returncode != 0):
                sql_update_ntuples = "UPDATE AMSDES.NTUPLES SET PATH=REPLACE(PATH, '/eosams/', '%s/'), EOSTIME=0 WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/')" %(self.cta_prefix, common_path)
                sql_update_datafiles = "UPDATE AMSDES.DATAFILES SET PATH=REPLACE(PATH, '/eosams/', '%s/'), EOSTIME=0 WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/')" %(self.cta_prefix, common_path)
                self.__log_warning("/eos/ams%s does not seem to be exist" %(common_path))
                self.__log_warning("IF YOU CONFIRM THAT, TRY TO SYNC DB BY:\n%s\n%s" %(sql_update_ntuples, sql_update_datafiles))
            elif (nNoBackup < 400):
                self.RemoveFromEOS("/eos/ams%s" %(common_path), "both", 1, 1, 0, 0, 1, 1, 1, 0, 1)

    def RemoveFromEOS(self, path, ftype, verbose=1, update=1, irm=0, notverify=0, removeempty=0, update_tape_time=0, copy_tape=0, force=0, ignoremissingoneos=1):
#        path:          EOS path to be removed
#        verbose:       print verbosed messages
#        update:        If update DB
#        irm:           If using interactive rm command
#        notverify:     Not to check the consistency of tape backup
#        force:
#        ignoremissingoneos: Ignore if file is missing on EOS
        if self.logger == None:
            self.logger =  Logger.Logger(inspect.stack()[0][3]).getLogger(level_log=logging.DEBUG, level_console=logging.INFO)
        if (not re.match(r"^%s/" %(self.eos_prefix), self.eosLink2Real(path))):
            self.__log_critical("Path must start with /eosams or /eos/ams.")
            exit -1
        common_path = self.getCommonPartOfPath(self.eosLink2Real(path))
        sql_get_ntuples ="SELECT PATH FROM AMSDES.NTUPLES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/') AND ADLER32 IS NULL" %(common_path)
        sql_get_datafiles ="SELECT PATH FROM AMSDES.DATAFILES WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/') AND ADLER32 IS NULL" %(common_path)
        sql_update_ntuples = "UPDATE AMSDES.NTUPLES SET ADLER32 = :adler32 WHERE REGEXP_SUBSTR(path,'/(Data|MC)/.*') = :commpath"
        sql_update_datafiles = "UPDATE AMSDES.DATAFILES SET ADLER32 = :adler32 WHERE REGEXP_SUBSTR(path,'/(Data|MC)/.*') = :commpath"

        if (ftype == 'root'):
            sql_without_backup = "%s AND CTATIME<=0" %(sql_get_ntuples)
            sql = sql_get_ntuples
        elif (ftype == 'raw'):
            sql_without_backup = "%s AND CTATIME<=0" %(sql_get_datafiles)
            sql = sql_get_datafiles
        elif (ftype == 'both'):
            sql_without_backup = "%s AND CTATIME<=0 UNION %s AND CTATIME<=0" %(sql_get_ntuples, sql_get_datafiles)
            sql = "%s UNION %s" %(sql_get_ntuples, sql_get_datafiles)
        else:
            self.__log_critical("Please specify data type(s) to remove.")
            return 1
        self.__log_info("Processing %s : %s" %(path, ftype))
        ret_files_without_backup = self.sqlserver.Query(sql_without_backup)
        if (len(ret_files_without_backup) > 0):
            self.__log_warning("There are %d paths without backups on tape." %(len(ret_files_without_backup)))
            nProcessed = 0
            for path in ret_files_without_backup:
                common_p = self.getCommonPartOfPath(path[0])
                if (self.processInconsistentFilesTapeEOS(common_p, removeempty, update_tape_time, copy_tape) == 0):
                    nProcessed += 1
                else:
                    self.__log_error("Unresolved inconsistency path: %s" %(common_p))
            if (nProcessed != len(ret_files_without_backup)):
                return 2
        ret_files=self.sqlserver.Query(sql)
        inconsistent = []
        for ntuple in ret_files:
            path = ntuple[0]
            common_p = self.getCommonPartOfPath(self.eosLink2Real(path))
            cta_p = "%s%s" %(self.cta_prefix, common_p)
            eos_p    = "%s%s" %(self.eos_prefix, common_p)
            cta_adler = self.xrdGetChecksum(cta_p)
            eos_adler = self.xrdGetChecksum(eos_p)
            if (cta_adler != eos_adler):
                self.__log_error("Inconsistent adler checksums: %s EOS %s CTA %s" %(common_p, eos_adler, cta_adler))
                if (self.processInconsistentFilesTapeEOS(common_p, removeempty, update_tape_time, copy_tape) == 0):
                    cta_adler = self.xrdGetChecksum(cta_p)
                    if (cta_adler != eos_adler):
                        if (ignoremissingoneos == 1 and eos_adler == ""):
                            self.__log_info("Ignore missing file on EOS %s" %(common_p))
                        else:
                            inconsistent.append(common_p)
                    else:
                        self.__log_info("Fixed inconsistency: %s %s %s" %(common_p, eos_adler, cta_adler))
            elif (eos_adler == "" and ignoremissingoneos != 1):
                self.__log_warning("Get ADLER failed: %s" %(common_p))
                inconsistent.append(common_p)
            else:
                self.__log_debug("Updating DB: ADLER32 %s for path %s" %(common_p, cta_adler))
                if re.match(r".*\.root$", common_p):
                    self.sqlserver.sql(sql_update_ntuples, adler32=cta_adler, commpath=common_p)
                    self.sqlserver.sql('COMMIT');
                else:
                    self.sqlserver.sql(sql_update_datafiles, adler32=cta_adler, commpath=common_p)
                    self.sqlserver.sql('COMMIT');
        if (len(inconsistent) > 0):
            self.__log_error("The following paths have different ADLER checksum on tape and EOS:")
            for common_p in inconsistent:
                self.__log_error("Inconsistent adler checksums for path: %s" %(common_p))
            return 3 

        if (len(ret_files) >= 0):
            self.__log_info("Update DB ...")
            sql_update_ntuples = "UPDATE AMSDES.NTUPLES SET PATH=REPLACE(PATH, '/eosams/', '%s/'), EOSTIME=0 WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/')" %(self.cta_prefix, common_path)
            sql_update_datafiles = "UPDATE AMSDES.DATAFILES SET PATH=REPLACE(PATH, '/eosams/', '%s/'), EOSTIME=0 WHERE REGEXP_SUBSTR(path, '/(Data|MC)/(.*?/){4}') = '%s' AND (EOSTIME>0 OR REGEXP_SUBSTR(path, '(.*/?)(Data|MC)/',1,1,'c',1) = '/eosams/')" %(self.cta_prefix, common_path)
            if (ftype == 'root'):
                self.sqlserver.Update(sql_update_ntuples)
                self.__log_debug(sql_update_ntuples)
            elif (ftype == 'raw'):
                self.sqlserver.Update(sql_update_datafiles)
                self.__log_debug(sql_update_datafiles)
            elif (ftype == 'both'):
                self.__log_debug(sql_update_ntuples)
                self.__log_debug(sql_update_datafiles)
                self.sqlserver.Update(sql_update_ntuples)
        ...
        self.__log_info("Consistency checking finished, now removing EOS files ...")
        p = subprocess.Popen([self.eosselect, '-r', '0', '0', 'rm', '-rF', "%s%s" %(self.eos_prefix, common_path)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = p.communicate()
        if (p.returncode == 0):
            self.__log_debug(out+err)
            self.__log_info("Revmoving %s finished, now COMMIT DB changes ..." %(common_path))
            if (len(ret_files) >= 0):
                self.sqlserver.Commit(1)
        else:
            self.__log_debug(out+err)
            self.__log_error("Revmoving failed, now ROLLBACK DB changes ...")
            if (len(ret_files) > 0):
                self.sqlserver.Commit(0)

    def processInconsistentFilesTapeEOS(self, common_p, removeempty, update_tape_time, copy_tape):
        retcode = -1
        (mtime, size) = self.xrdGetStatTimeSize("%s%s" %(self.eos_prefix, common_p))
        if re.match(r".*\.root$", common_p):
            table = 'NTUPLES'
        else:
            table = 'DATAFILES'
        if (size == 0):
            if (removeempty):
                self.__log_info("Removing %s from DB ..." %(common_p))
                sql_delete = "DELETE FROM %s WHERE REGEXP_SUBSTR(path,'/(Data|MC)/.*') = '%s'" %(table, common_p)
                self.sqlserver.Update(sql_delete)
                self.sqlserver.Commit(1)
                retcode = 0
            else:
                self.__log_debug("%s : %d" %(common_p, size))
        elif (size == -1):
            (mtime, size_cta) = self.xrdGetStatTimeSize("%s%s" %(self.cta_prefix, common_p))
            sql_getpath = "SELECT PATH, EOSTIME FROM %s WHERE REGEXP_SUBSTR(path,'/(Data|MC)/.*') = '%s'" %(table, common_p)
            ret_path = self.sqlserver.Query(sql_getpath)
            if (size_cta > 0):
                self.__log_debug("%s exists on tape " %(common_p))
                return 0
            else:
                if (not re.match(r"^/eosams/.*", ret_path[0][0]) and not re.match(r"^%s/.*" %(self.cta_prefix), ret_path[0][0])):
                    self.__log_debug("%s is not on EOS, please consider copy from disk." %(ret_path[0][0]))
                else:
                    self.__log_debug("%s%s is not found, please consider to remove." %(self.cta_prefix, common_p))
        else:
            (mtime, size_cta) = self.xrdGetStatTimeSize("%s%s" %(self.cta_prefix, common_p)) 
            sql_getcrc = "select crc from %s where REGEXP_SUBSTR(path,'/(Data|MC)/.*') = '%s'" %(table, common_p)
            ret_crc = self.sqlserver.Query(sql_getcrc)
            if ( self.compareCRC("%s%s" %(self.eos_prefix, common_p), ret_crc[0][0]) != 1):
                self.__log_debug("Staging %s%s" %(self.cta_prefix, common_p))
                self.__stager("%s%s" %(self.cta_prefix, common_p))
                self.__log_error("Bad CRC checksum found for %s%s" %(self.eos_prefix, common_p))
                return -1
            elif (copy_tape):
                for i in [ 1, 2, 3, 4, 5 ]:
                    self.__log_debug("xrdcp -f -v root://eosams.cern.ch/%s%s root://%s/%s%s" %(self.eos_prefix, common_p, self.cta_xrd_server, self.cta_prefix, common_p))
                    p = subprocess.Popen(['xrdcp', '-f', '-v', "root://eosams.cern.ch/%s%s" %(self.eos_prefix, common_p), "root://%s/%s%s" %(self.cta_xrd_server, self.cta_prefix, common_p)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                    out, err = p.communicate()
                    self.__log_debug(out+err)
                    if (p.returncode == 0):
                        retcode = 0
                        (mtime, size_cta) = self.xrdGetStatTimeSize("%s%s" %(self.cta_prefix, common_p)) 
                        break
            if (size_cta == size and update_tape_time and size_cta > 0):
                self.__log_info("Update CTATIME to %d for %s" %(mtime, common_p))
                sql_update_cta_time = "UPDATE %s SET CTATIME=%d WHERE REGEXP_SUBSTR(path,'/(Data|MC)/.*') = '%s'" %(table, mtime, common_p)
                self.sqlserver.Update(sql_update_cta_time)
                self.sqlserver.Commit(1)
                retcode = 0
            else:
                self.__log_warning("%s : %d" %(common_p, size))
        return retcode

    def getCommonPartOfPath(self, path):
        path_parts = path.split("/")
        if re.match(r"^%s/" %(self.cta_prefix), path):
            i = 5
        elif re.match(r"^%s/" %(self.eos_prefix), path):
            i = 3
        elif re.match(r"^/Data", path) or re.match(r"^/MC", path):
            i = 1
        elif re.match(r"^Data", path) or re.match(r"^MC", path):
            i = 0
        else:
        ...
        while (i < len(path_parts)):
            common_path += "/" + path_parts[i]
            i += 1
        return common_path

    def xrdGetServerByPath(self, path):
        server = None
        if re.match(r"^%s/.*" %(self.eos_prefix), path):
            server = self.eos_xrd_server
        elif re.match(r"^%s/.*" %(self.cta_prefix), path):
            server = self.cta_xrd_server
        return server

    def xrdGetChecksum(self, path):
        server = self.xrdGetServerByPath(path)
        if (server is None):
            p = subprocess.Popen(['xrdadler32', path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out, err = p.communicate()
            if (p.returncode != 0):
                self.__log_warning(out+err)
                return ""
            out_arr = out.split()
            try:
                checksum = re.sub("^0*", "", out_arr[0])
            except:
                self.__log_warning(out+err)
                return ""
            return checksum
        p = subprocess.Popen(['xrdfs', server, 'query', 'checksum', path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        cmd = "%s %s %s %s %s" %('xrdfs', server, 'query', 'checksum', path)
        self.__log_debug(cmd)
        out, err = p.communicate()
        if (p.returncode != 0):
            self.__log_warning(out+err)
    ...
    def xrdGetStatTimeSize(self, path):
        server = self.xrdGetServerByPath(path)
        if (server is None):
            return 0
        p = subprocess.Popen(['xrdfs', server, 'stat', path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = p.communicate()
        if (p.returncode != 0):
            self.__log_warning(out+err)
            return -1, -1
        ...
        for line in out_arr:
            if (line.find("Size:") == 0):
                xrdSize = int(line.split()[1])
            elif (line.find("MTime:") == 0):
                datestring = line.split()[1]
                timestring = line.split()[2]
                xrdTime = int(time.mktime(datetime.datetime.strptime("%s %s" %(datestring, timestring), "%Y-%m-%d %H:%M:%S").timetuple()))
        return xrdTime, xrdSize

    def MuxDataFiles(self):
        if self.logger == None:
            FORMAT = '[%(asctime)-19.19s %(levelname)-8s] %(message)s'
            logging.basicConfig(format=FORMAT)
            self.logger =  logging.getLogger(inspect.stack()[0][3])
        ...
        paths2move = []
        for rawType in ['CMD']:
            sql_paths2move = "SELECT PATH, PATHS FROM AMSDES.DATAFILES WHERE TYPE LIKE '%s%%' AND PATH NOT LIKE '%%/RawData/' || substr(TYPE, 0, 3) || '/%%' AND EOSTIME>0" %(rawType)
            ret_paths2move = self.sqlserver.Query(sql_paths2move)
            update_sql = "UPDATE AMSDES.DATAFILES SET PATH=:newpath WHERE PATH=:oldpath"
            for datafile in ret_paths2move:
                path = datafile[0]
                link = datafile[1]
                if re.match(r"^/eosams/.*", path):
                    eospath = self.eosLink2Real(path)
                    ctapath = eospath.replace(self.eos_prefix, self.cta_prefix)
                elif re.match(r"^%s/.*" %(self.cta_prefix), path):
                    ctapath = path
                    eospath = ctapath.replace(self.cta_prefix, self.eos_prefix)
                else:
                    continue
                cta_newpath = ctapath.replace('/RawData/', "/%s/" %(rawType))
                eos_newpath = eospath.replace('/RawData/', "/RawData/%s/" %(rawType))
                eos_move_cmd = "eos file rename %s %s" %(eospath, eos_newpath)
                db_path = path.replace('/RawData/', "/RawData/%s/" %(rawType))
                relink_cmd = "ln -sf %s %s" %(db_path, link)
                if os.system(eos_move_cmd) == 0:
                    if self.__file_rename(ctapath, cta_newpath) == 0:
                        if os.system(relink_cmd) == 0:
                            self.sqlserver.sql(update_sql, oldpath=path, newpath=db_path)
                            self.sqlserver.Commit()
                            self.__log_info("Updating db of the new path: %s" %(db_path))
                        else:
                            self.__log_warning("Relinking failed, reversing EOS and tape renaming")
                            eos_move_cmd = "eos file rename %s %s" %(eos_newpath, eospath)
                            os.system(eos_move_cmd)
                            self.__file_rename(cta_newpath, ctapath)
                    else:
                        eos_move_cmd = "eos file rename %s %s" %(eos_newpath, eospath)
                        os.system(eos_move_cmd)
                        self.__log_warning("CTA renaming failed, reversing EOS renaming")
                else:
                    self.__log_warning("EOS renaming failed, giving up for %s" %(path))

    def RevalidateDataSet(self,run2p,dataset,u,v,f,donly,datamc,buildno,mt):
        self.update=u
        self.verbose=v
        self.run2p=run2p
        self.force=f
        ...
        if(datamc!=0):
            ...
            if(run2p/10000000000>0):
                ...
                runn=" and ntuples.run<%d " %(run2p)
                runnd=" and ntuples_deleted.run<%d " %(run2p)
        sql="select path, nevents, levent, run from ntuples where path like '%%%s/%%' and datamc=%d %s %s %s" %(dataset,datamc%10,runn,runbuild,arun2p) 
        df=0
        files=self.sqlserver.Query(sql)
        datapath=dataset
        ds1=""
        ...
        for file in files:
            ...
            run = file[3]
            if (self.mt):
                while threading.activeCount() > int(self.mt):
                    time.sleep(1)
                try:
                    validateThread = self.threadRevalidateNtuple(self, path, nevents, levent, run)
                    validateThread.start()
                    threads.append(validateThread)
                except:
                    print "EXCEPTION in starting new thread";
            else:
                self.revalidateNtuple(path, nevents, levent, run)
        for t in threads:
            t.join()
        self.__log_info("Good %d, broken %d, uncheck %d.\n" %(len(self.GoodRuns), len(self.FailedRuns), len(self.CheckedRuns)))
        for r in self.FailedRuns:
            self.__log_info("%d," %(r))

    class threadRevalidateNtuple(threading.Thread):
        def __init__(self, caller, path, nevents, levent, run):
            threading.Thread.__init__(self)
            self.caller = caller
            self.path = path
            ...
            self.runid = run 
        def run(self):
            RemoteClient.revalidateNtuple(self.caller, self.path, self.nevents, self.levent, self.runid)

    def revalidateNtuple(self, path, nevents, levent, run):
        (ret,i)=self.validateDST(path, nevents, 'RootFile', levent)
        self.__log_info("validateDST(%s, %d, %s, %d, %d) : Status : %d : Ret : %d\n" %(path, nevents, 'RootFile', levent, run, i, ret))
        if( i == 0xff00 or (i & 0xff)):
            unchecked_runs.append(run)
        else:
            i= i>>8 
            if(i==134):
                self.CheckedRuns.append(run)
            elif(i/128):
                self.FailedRuns.append(run)
            else:
                self.GoodRuns.append(run)
