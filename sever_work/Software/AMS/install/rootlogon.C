...
for(int im=0;im<2;im++){
  for(int ic=0;ic<nc;ic++){
    ...
    bool newmake[2]={0}; 
    if(!umake.Contains("GLIBCXX_USE_CXX11_ABI")){
      TString flagc=TString(fc[ic])+" -D_GLIBCXX_USE_CXX11_ABI=0";
      umake.ReplaceAll(fc[ic],flagc);
      newmake[0]=1;
    }
    if(!flagopt.Contains("GLIBCXX_USE_CXX11_ABI")){
      flagopt.Append(" -D_GLIBCXX_USE_CXX11_ABI=0");
      newmake[1]=1;
    }
    if(!umake.Contains("-std=c++")){
      TString flagc=TString(fc[ic])+" -std=c++11";
      umake.ReplaceAll(fc[ic],flagc);
      newmake[0]=1;
    }
    if(ic<=1&&intelds.Contains("oneAPI")){
      ...
      if(umake.Contains("MIC-AVX-512")){
        umake.ReplaceAll("MIC-AVX-512","CORE-AVX512");
      }
      if(flagopt.Contains("MIC-AVX-512")){
        flagopt.ReplaceAll("MIC-AVX-512","CORE-AVX512");
        newmake[1]=1;
      }
