...
do
  ...
  if [ -n "$maketool" ] ; then
    ...
    ./${mycc}  "$ifile" "$tmpofile" "$runopt1"
    substat=$?
  elif [ -n "$runopt1" ] ; then
    root -b -q ${mycc}.C+'("'$ifile'","'$tmpofile'","'$runopt1'")'
  else 
    root -b -q ${mycc}.C+'("'$ifile'","'$tmpofile'")'
  fi
#---cp
  if  [ -f "$tmpofile" ] ; then
    tmpofile1=$tmpofile
#    tmpofile1=`echo "$tmpofile" | sed 's/.root//'`
    ...
    do
      if echo "$ofile" | grep castor ; then
        rfcp ${tmpofile1}* ${odir}
      elif echo "$ofile" | grep eos ; then
