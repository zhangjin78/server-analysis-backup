...
#ifndef G4INCLXXInterfaceQ_hh
#define G4INCLXXInterfaceQ_hh 1
 ...
 */
class G4INCLXXInterfaceQ : public G4VIntraNuclearTransportModel {
public:
  G4INCLXXInterfaceQ(G4VPreCompoundModel * const aPreCompound = 0);
  ~G4INCLXXInterfaceQ(); // Destructor

  G4int operator==(G4INCLXXInterfaceQ& right) {
    return (this == &right);
  }
  ...
  void SetEexcFactor(G4double useexc);
...
private:
  ...
  G4INCLXXInterfaceQ(const G4INCLXXInterfaceQ &rhs);

  /// \brief Dummy assignment operator to shut up Coverity warnings
  G4INCLXXInterfaceQ &operator=(G4INCLXXInterfaceQ const &rhs);
  ...
  G4BinaryLightIonReactionQ *theBackupModel;
  ...
  G4double fEexc;      //Ecited energy bias
};


inline void G4INCLXXInterfaceQ::SetEexcFactor(G4double useexc){
  fEexc=useexc;
  theBackupModel->SetEexcFactor(useexc);
}


inline void G4INCLXXInterfaceQ::DeleteAllSecondary(){
