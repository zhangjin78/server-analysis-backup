...
*KEEP,NSHMAK.
!$OMP THREADPRIVATE (/HADTHR/)
      COMMON /NSHMAK/ NNSHMA,NPSHMA,NTSHMA,NSHMAC,NSHMA2
*KEEP,REJEC.
!$OMP THREADPRIVATE (/NSHMAK/)
...
!$OMP THREADPRIVATE (/PTSAMP/)
      COMMON /STARS/ISTAR2,ISTAR3
C!$OMP THREADPRIVATE (/STARS/)
...
!$OMP THREADPRIVATE (ECM0,RMIN,B,PPTT,DPPTT)
      ECM=CMENER
      PTTHR=2.5+0.12*(LOG10(CMENER/50.))**3
      PTCUT=PTTHR      
      CALL SIGSHD(ECM)
...
C        IOUTPA=IOUTPO
      ELSEIF ( MODE.EQ.1 ) THEN
        IF( IOUTPA.GE.1 )WRITE(6,9010)MODE,ISAMPT,PTCUT,SIGS,DSIGH,B
     * ,C,SIGSOF,SIGHAR,RMIN
        PTT   =LOG(1.0-RNDMDPM(V)*(1.0-RMIN))/(B+0.00001D0)
        PT=SQRT(PTT)
        IIPT=PT*50./PTCUT+1.
        IIPT=MIN( IIPT,50 )
...
!$OMP THREADPRIVATE (/NUCIMP/)
 ...
 1001 FORMAT(' HADHAD 1:',
     ...
     +I3,2E12.3,3F7.3,3I4)
      ...
      SIGTOT=SIGHT + SIGHTE
      IF (SIGTOT*RNDMDPM(BB).LE.SIGHTE)IELINE=1
...
C 
      ...
      INTEGER INIT
      DATA INIT/0/
      SAVE PT,YPT,DPT,INIT
!$OMP THREADPRIVATE(   PT,YPT,DPT,INIT)
      if(INIT.EQ.0) THEN
      DPT=0.1
      DO  I=1,10
        DO  J=1,50
