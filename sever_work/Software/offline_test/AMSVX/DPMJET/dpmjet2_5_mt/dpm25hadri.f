...
!$OMP THREADPRIVATE (A, P, SIG, SEG, ITT, SITO, PLAB, ALP, BET)
!$OMP THREADPRIVATE (STOT, REA)
...
*
     ...
     &     TSIG , RELTO
!$OMP THREADPRIVATE (EKIHN, EKIHA, AMASS, SEPIMP, SEPIPP)
!$OMP THREADPRIVATE (SEPP, SENP, SENA, SEPA,TSIG , RELTO)
...
*-- Author :
   ...
   10 X=RNDMDPM(A)
      ...
      CFE=(XX-YY)/XY
      SFE=2.*X*Y/XY
      IF(RNDMDPM(C).LT.0.5D0)                                   GO TO 20
      RETURN
   20 SFE=-SFE
      RETURN
      END
