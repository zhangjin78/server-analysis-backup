...
!$OMP THREADPRIVATE (/USER2/)
      COMMON /COLLAP/ S2,i1,i2,p1,p2,i3,i4,i5
*!$OMP THREADPRIVATE (/COLLAP/)
...
!$OMP THREADPRIVATE (/COLLIS/)
      COMMON /STRUFU/ISTRUM,ISTRUT
!$OMP THREADPRIVATE (/STRUFU/)
      COMMON /PTLARG/XSMAX
!$OMP THREADPRIVATE (/PTLARG/)
      COMMON /HAXSUM/XSHMX
C     COMMON /POMENE/POEN(20),POEN1(20),POEN2(20),NESTEP
!$OMP THREADPRIVATE (/HAXSUM/)
      COMMON /POMENE/POEN(28),POEN1(28),POEN2(28),NESTEP
C     COMMON /LAPENEN/PTTHRZ(20),PTTHZ2(20),INDENE
C!$OMP THREADPRIVATE (/POMENE/)
      COMMON /LAPENEN/PTTHRZ(28),PTTHZ2(28),INDENE
C
C                              Fill COLLAP COMMON Block
!$OMP THREADPRIVATE (/LAPENEN/)
      S2 = S0
      I1 = IJPROJ0
      I2  = IJTAR0
      P1  = PTTHR0
...
!$OMP THREADPRIVATE (/USER1/)
      COMMON /USER2/CMENER0,SDFRAC0,PTLAR0,ISTRUF0,ISINGD0,IDUBLD0
!$OMP THREADPRIVATE (/USER2/)
C      COMMON /COLLAP/S,IJPROJ,IJTAR,PTTHR,PTTHR2,IOPHRD,IJPRLU,
C     * IJTALU
C!$OMP THREADPRIVATE (/COLLAP/)
      COMMON/COLLIS/ S0, IJPROJ0, IJTAR0, PTTHR0, PTTHR20, IOPHRD0,
     *               IJPRLU0, IJTALU0
*
C     COMMON /USER/TITLE,PROJTY,TARGTY,CMENER,ISTRUF
...
C                                              OF POMERTONS IS CREATED       
      IF ( NPDCOR.EQ.1     .AND.
     &     IHARD .GT.1     .AND.
     &     (1.-X1S)*(1.-X2S).LT.RNDMDPM(AI)*(1.-AA*IHARD)**2 ) GOTO 5
 301  CONTINUE 
C
C  end of loop
C
...
50    CONTINUE
      IF(IHARD.LT.1) THEN
        ...
        call fortran_thread_signalled(mpi)
	if(mpi.ne.1)then
         STOP
        else
         call sleep(864000)
...
!$OMP THREADPRIVATE (/HAXIK /)
     ...
     +        ,IEOTRI,IEVENT,ISWIT(10),IFINIT(20),NEVENT,NRNDM(2)
      COMMON /AMSFFKEY/is,ir,jo,id,cpl  
      real t0,t1,cpl
      data t0/0/
      save t0
      data mx12/0/
      ...
      data mpi/0/
      save mpi
!$OMP THREADPRIVATE (iendr,mpi)
      if((IEOTRI.eq.1.and.IEORUN.eq.1).or.ifinit(20).eq.-700)then
        if(iendr.eq.0)then
            call fortran_throw_ex('HAX1X2-IEOTRISET')
!$OMP CRITICAL (IENDRC)
             write(*,*)'HAX1X2 ENDRUN DETECTED ',mx12,mx13,cpl
             call timex(t0)
             iendr=1
             mx13=0
             call fortran_thread_signalled(mpi)
!$OMP END CRITICAL (IENDRC)
        endif
         call timex(t1)
         cplm=cpl
         if(cplm.lt.600)cplm=600
         if(mpi.ne.1.and.t1-t0.gt.cplm.and.mx13.ne.0)then
          write(*,*)'HAX1X2 RAISING INT ',t1-t0,mx13,cpl
          call fortran_raise_int()
        endif
      else
      if(iendr.eq.1)iendr=0
      endif
      IERR=0
      SA=SQRT(A)
      is12=0
12    continue
c--------------------------------- sample z=x*y
      z=a*Exp(RNDMDPM(aa23)*Log(zmax/a))
      xm=xrest
      ym=yrest
      if(xm.lt.yrest) then
        xm=yrest
      ...
      if(RNDMDPM(aa23).gt.ww) then
       if(is12.lt.1000)then
          is12=is12+1
          goto 12
      else 
        if(mx12.lt.1000.and.mx12.ge.0)
     +  write(*,*)'HAX1X2 12 sampling exceeding ',ww
        mx12=mx12+1
        IERR=1
      endif                         
      endif                         
c--------------------------------- sample u=x+y
      ...
      is13=0
13    continue
      c=Exp(RNDMDPM(aaa23)*Log((umax+Sqrt(cc))/umin))
      cmax=Exp(Log((umax+Sqrt(cc))/umin))
      uu=umin*(c**2+1.)/2./c
      uuma=umin*(cmax**2+1.)/2./cmax
      uumi=umin
      if(uumi.gt.2.*ym.and.uumi.le.ym+z/ym .and.
     +   uuma.gt.2.*ym.and.uuma.le.ym+z/ym)then
              uu=ym+z/ym+1e-6
         if(mx13.lt.1000.and.mx13.ge.0)
     +   write(*,*)'HAX1X2 range error detected ',
     +   2.*ym,uumi,uuma,ym+z/ym,mx13
         mx13=mx13+1
       endif
      if(uu.gt.2.*ym.and.uu.lt.ym+z/ym) then
        if(is13.lt.1000)then
         is13=is13+1
         goto 13
        else
         iz=0
         call fortran_throw_ex('HAX1X2 sampling exceeded')  
         if(mx13.lt.1000.and.mx13.ge.0)
     +    write(*,*)'HAX1X2 13 sampling exceeding ',
     +    uu,2*ym,ym+z/ym,z,
     +    umax,umin,cc,c,Log((umax+Sqrt(cc))/umin),a,uu**2-4.*z,mx13
          mx13=mx13+1
          uu=ym+z/ym+1e-6 
        endif
      endif
...
!$OMP THREADPRIVATE (RM)
      ...
      IF     ( M.EQ.1 ) THEN
10      CALL HAX1X2(IERR)
        V  =-0.5*W1/(W1+RNDMDPM(AI)*W)
        U  =-1.-V
        R  = (1.+W)*2.25*(V*V*(3.-U*V-V/(U*U))-U)
        RMAX=RM(1)*WEMAX*(1.+WEMAX)
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(1)*RNDMDPM(AI)  .AND. IERR.eq.0) GOTO 10
        IF(WIK.LT.RNDMDPM(AI) .AND. IERR.eq.0) GOTO 10
        IF ( RNDMDPM(AJ).LE.0.5D0 ) V = U
      ELSEIF ( M.EQ.2 .OR. M.EQ.4 ) THEN
20      CALL HAX1X2(IERR)
        WL = LOG(W1)
        V  =-EXP(-0.6931472+RNDMDPM(AI)*WL)
        U  =-1.-V
        R  = (U*U+V*V)*((16./27.)/U-(4./3.)*V)*(WL/W)*AXX
        IF ( R*W.LT.RM(M)*RNDMDPM(AI) .AND. IERR.eq.0 ) GOTO 20
        IF ( RNDMDPM(AJ).LE.0.5D0 ) V = U
      ELSEIF ( M.EQ.3 ) THEN
30      CALL HAX1X2(IERR)
        V  =-0.5*W1/(W1+RNDMDPM(AI)*W)
        ...
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(3)*RNDMDPM(AI)  .AND. IERR.eq.0) GOTO 30
        IF(WIK.LT.RNDMDPM(AI)  .AND. IERR.eq.0)GOTO 30
      ELSEIF ( M.EQ.5 ) THEN
50      CALL HAX1X2(IERR)
        V  =-0.5*AXX/(W1+2.*RNDMDPM(AI)*W)
        U  =-1.-V
        R  = (4./9.)*(1.+U*U+V*V*(U*U+V*V))-(8./27.)*U*U*V
        RMAX=RM(5)*WEMAX
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(5)*RNDMDPM(AI) .AND. IERR.eq.0 ) GOTO 50
        IF(WIK.LT.RNDMDPM(AI) .AND. IERR.eq.0) GOTO 50
      ELSEIF ( M.EQ.6 ) THEN
60      CALL HAX1X2(IERR)
        V  =-0.5*(1.+W)+RNDMDPM(AI)*W
        U  =-1.-V
        R  = (4./9.)*(U*U+V*V)*AXX
        IF ( R*W.LT.RM(6)*RNDMDPM(AI) .AND. IERR.eq.0 ) GOTO 60
      ELSEIF ( M.EQ.7 ) THEN
70      CALL HAX1X2(IERR)
        V  =-0.5*W1/(W1+RNDMDPM(AI)*W)
        U  =-1.-V
        ...
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(7)*RNDMDPM(AI) .AND. IERR.eq.0 ) GOTO 70
        IF(WIK.LT.RNDMDPM(AI) .AND. IERR.eq.0) GOTO 70
        IF ( RNDMDPM(AJ).LE.0.5D0 ) V = U
      ELSEIF ( M.EQ.8 ) THEN
80      CALL HAX1X2(IERR)
        V  =-0.5*AXX/(W1+2.*RNDMDPM(AI)*W)
        ...
        WIK=R*W/RMAX
        IF(WIK.GT.1.D0) WRITE(6,*) ' HARKIN : WIK > 1 : ',M,R
C        IF ( R*W.LT.RM(8)*RNDMDPM(AI) .AND. IERR.eq.0 ) GOTO 80
        IF(WIK.LT.RNDMDPM(AI)  .AND. IERR.eq.0) GOTO 80
      ELSEIF ( M.EQ.-1 ) THEN
90      CALL HAX1X2(IERR)
        WL = LOG(W1)
        V  =-EXP(-0.6931472+RNDMDPM(AI)*WL)
        ...
        IF ( R*W.LT.RM(-1)*RNDMDPM(AI) .AND. IERR.eq.0) GOTO 90
      ENDIF
C      PARAMETER ( TINY= 1.D-30, ONE=1.D0 ,TINY6 =1.D-06)
      ...
      if(IERR.ne.0)then
        write(*,*)'HARKIN-E-M',M,v,u,pt,etac,etad
      endif
      RETURN
      END
...
10    CONTINUE
      ...
      IREJEV = IREJEV+1
C  find subprocess
      B      = RNDMDPM(AI)*XSECT(2,0,INDENE)
C     B      = RNDMDPM(AI)*XSECT(2,0)
      MSPR   =-2
      SUM    = 0.0
20    MSPR   = MSPR+1
...
C     WEIGHT = F/XSECT(2,MSPR)
      IF ( WEIGHT.LT.RNDMDPM(AI) ) GOTO 10
C-------------------------------------------------------------------
...
C  find final partons
      ...
      ELSEIF ( MSPR.EQ.4 ) THEN
        IC = INT(FLOAT(NF+NF)*RNDMDPM(AI))+1
        IF ( IC.GT.NF ) IC = NF-IC
        ID =-IC
      ELSEIF ( MSPR.EQ.6 ) THEN
        IC = INT(FLOAT(NF+NF-2)*RNDMDPM(AI))+1
...
30    A1     = RNDMDPM(AI)
      ...
      SINPHI = SIGN(((A1*A2)+(A1*A2))/((A1*A1)+(A2*A2)),RNDMDPM(AI)-0.5)
C
      IF ( RNDMDPM(AI)*PDA(IA).GT.PDA(-IA) ) IA = SIGN(ABS(IA)+10,IA)
      IF ( RNDMDPM(AJ)*PDB(IB).GT.PDB(-IB) ) IB = SIGN(ABS(IB)+10,IB)
C  fill event record
      LINE          = LINE+1
      PREC(1,LINE)  = 0.0
      PREC(2,LINE)  = 0.0
...
C
C$OMP THREADPRIVATE (Q0S)
        if(iopt.ne.0)then
          call harini_copy(iopt)
          return
        endif
