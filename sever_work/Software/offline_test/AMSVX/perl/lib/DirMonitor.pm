...
#        dst_dir=>'/fcdat0/scratch/MC/afsbackup',
#        dst_dir2=>'/fcdat0/scratch/MC/afsbackup',
...
sub run{
  ...
  $self->{error_counter}=0;
	...
	foreach my $dir (@{$self->{src_dirs}}){
	        ...
	        my $num=-1;
        	if($ans==1){
#                	Changed by Baosong, to avoid "Argument list too long"
#                	my @files=`/bin/ls -ctr $dir/* | grep $dir | grep -v :\$ 2>/dev/null`;
                	my @files=`/bin/ls -ctr $dir/ | grep -v :\$ 2>/dev/null`;
	                $num=$#files+1;
