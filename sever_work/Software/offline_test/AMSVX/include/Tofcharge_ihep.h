//  $Id: Tofcharge_ihep.h,v 1.24 2022/02/12 17:10:36 qyan Exp $
...
class TofChargePar: public TObject{
  ...
  */
  ...
  public:
    TofChargePar(){Beta=Layer=Bar=-1; QLA=QLD=QLARaw=QLDRaw=0; IsGoodPath=0; DefaultQDAOpt=22; ProbZ.clear();}
 ...
 */
   ...
   */
  int  ReFit(float fbeta=0,int opt=TofChargeHR::DefaultQOptIonW,float frig=0);
/**@}*/
...
#endif
  static TofPDFH *GetHead();
  static TofPDFH *pHead;
#ifndef __PPC64
#pragma omp threadprivate (pHead)   
#endif
  ...
  int Init();
  /// ReBuild For BetaH-Charge
  int ReBuildI(BetaHR *betah,TofChargeHR &tofch);
  /// Fill ZProb to TofChargePar
  int FillProbV(vector<TofChargePar> &cpar);
  /// Calculate Likelihood and Prob for different Charge
  void LikelihoodCal(const vector<TofChargePar> &cpars,vector<TofLikelihoodPar> &like);
  /// Calculate Likelihood for index IZ
  number GetLikelihood(int IZ,const vector<TofChargePar> &cpars);
  /// LikelihoodQ Estimator opt=0 QMean-QRMS opt=1 LikelihoodQ
  number GetLikelihoodQ(vector<TofChargePar> &cpars,number &MeanQ,number &RMSQ,int opt=0);
  ...
  int SelectM(int pattern,const vector<TofChargePar> &cpar,int fTrTrack,vector<TofChargePar> &cpars);
  /// ProbZ from PDF
   number GetProbZ(int ilay,int ibar,number Z,number QLA,number QLD,number QLARaw,number QLDRaw,number betah);
  /// ProbZI from PDF
   number GetProbZI(int ilay,int ibar,int ZI,number QL,number betah,int isanode);
  /// Choose Anode or Dynode From PDF
   int ChooseDA(int ilay,int ibar,number Z,number QLA,number QLD, number QLARaw,number QLDRaw,number &QL);
