//  $Id: EcalChi2CY.h,v 1.23 2018/07/07 20:38:11 bbeische Exp $
...
public:
    ...
    */
    ...
    \param[in] logErg   Natural logarithm of Ecal Shower Deposition (GeV)
    \param[in] type     Currently only provide type 0
    \param[out] mean    if successful, mean>=0; if database has not been loaded, mean=-1
    */
    float get_mean(int flayer, float coo, float logErg, int type=0)    ;
    ...
    */
    float get_rms (int flayer, float coo, float logErg, int type=0)    ;
    ///Get hit occupance
    /*!
    \param[in] flayer   Layer number. e.g 0~17
    \param[in] coo      Coordinate w.r.t the shower axis
    ...
    */
    float get_prob(int flayer, float coo, float logErg, int type=0)    ;
