//  $Id: trrec.h,v 1.131 2019/02/19 06:44:24 choutko Exp $
#ifndef _PGTRACK_
...
void VerySimpleFit(AMSPoint err);
 static void _Start(){_Time=AMSgettime());time(&__Time);}
static geant _CheckTime(){return AMSgettime()-_Time;}
...
public:
 ...
 /// Default constructor
  ...
  /// Copy constructor
  AMSTrCluster(const TrClusterR& orig):TrClusterR(orig),AMSlink(0,0){}
  /// Destructor
  ~AMSTrCluster(){}
...
public:
 /// Default constructor
  ...
  /// Copy constructor
  AMSTrRecHit(const TrRecHitR& orig):TrRecHitR(orig),AMSlink(0,0){}
...
public:
  ...
  public:
  integer operator < (AMSlink & o)const {
    AMSTrTrack *to =(AMSTrTrack*)(&o);
    int prim=(_ReconType/10000)%10;//primary tag
    int primo=(to->_ReconType/10000)%10;
    int genid=(_ReconType/100)%100;//generate id
    int genido=(to->_ReconType/100)%100;
    if     (prim!=primo)return (prim>primo);//primary first
    else if(genid!=genido) return (genid<genido);//then reconstruction sequence
    else return (_quality>to->_quality); //then quality
  }
