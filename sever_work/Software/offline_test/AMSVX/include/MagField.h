//  $Id: MagField.h,v 1.22 2022/02/12 17:10:36 qyan Exp $
#ifndef __MagField__
#define __MagField__
#include "typedefs.h"
#include <cstdlib>
...
public:
  ...
  int    GetScaleFlag  () const { return _flag; }
  //! returns the magnet temperature
  float  GetMagTemp(int slot) const { return mag_temp[slot];}
  //! returns the magnet temperature flag
  int  GetMagTempFlag() const { return _flag;}
  //! Sets the temp for the Correction of the B fiels (18.5 degrees = no correction 
  void   SetMagTemp(float temp1,float temp2=0, int flag=1 );  
...
class BCorr : public TObject {
protected:
  std::map <unsigned int,float> _m;
  static BCorr *_h;
public:
  typedef std::map<unsigned int,float>::iterator It;
  typedef std::map<unsigned int,float>::const_iterator Ic;

  static BCorr *GetHead() { if(!_h) _h = new BCorr; return _h; }
  static BCorr *Load(const char *fname);
  static BCorr *Load(TFile *f);

  void  Set(unsigned int t, float v);
  float Get(unsigned int t) const;
  int  GetN(void) const { return _m.size(); }
  unsigned int First(void) const { Ic i = _m.begin(); return i->first; }
  unsigned int Last (void) const { Ic i = _m.end();
                            if (i != _m.begin()) i--; return i->first; }
