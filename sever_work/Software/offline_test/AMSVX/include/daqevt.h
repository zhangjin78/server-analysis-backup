//  $Id: daqevt.h,v 1.109 2016/05/24 11:21:12 choutko Exp $
// V. Choutko 15/6/97
//
...
protected:
  ...
  unsigned int _JLength[24];
  unsigned int _JLengthU[24];
  unsigned int _SubLength[7];   //  tracker trd tof rich ecal lvl1 lvl3
  unsigned int _SubLengthU[7];   //  tracker trd tof rich ecal lvl1 lvl3
  unsigned int _SubCount[7];   //  tracker trd tof rich ecal lvl1 lvl3
  unsigned int _JRoomError;
...
public:
  ...
  integer getjlength(unsigned int i) const {return i<sizeof(_JLength)/sizeof(_JLength[0])?_JLength[i]*sizeof(_pData[0]):0;}
  integer getjlengthU(unsigned int i) const {return i<sizeof(_JLengthU)/sizeof(_JLengthU[0])?_JLengthU[i]*sizeof(_pData[0]):0;}
  integer getsublength(unsigned int i) const {return i<sizeof(_SubLength)/sizeof(_SubLength[0])?_SubLength[i]*sizeof(_pData[0]):0;}
  integer getsublength() {
    int length=0;
    for (unsigned int i=0;i<sizeof(_SubLength)/sizeof(_SubLength[0]);i++)
    length+=getsublength(i);
    return length;
  }
  integer getsublengthU(unsigned int i) const {return i<sizeof(_SubLengthU)/sizeof(_SubLengthU[0])?_SubLengthU[i]*sizeof(_pData[0]):0;}
  integer getsublengthU() {
    int length=0;
    for (unsigned int i=0;i<sizeof(_SubLengthU)/sizeof(_SubLengthU[0]);i++)
    length+=getsublengthU(i);
    return length;
  }
  unsigned int getjroomerror() const {return _JRoomError;}
  unsigned int getsubroomerror() {
    unsigned int error=0;
    for (unsigned int i=0;i<sizeof(_SubLengthU)/sizeof(_SubLengthU[0]);i++)
      error|=((getsublengthU(i)!=getsublength(i))<<i);
    return error;
  }
