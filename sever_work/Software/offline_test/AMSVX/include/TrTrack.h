//  $Id: TrTrack.h,v 1.173 2022/06/16 12:46:31 qyan Exp $
#ifndef __TrTrackR__
#define __TrTrackR__
...
///\ingroup tkrec
///
/// TrTrackPar contains a set of track parameters correponding one 
...
/// Track finding/Reconstruction parts are implemented in TrRecon and TrReconQ.
///
...
#define DEF_NTKZRIG 4
  static double RigPZ[DEF_NTKZRIG];
  /// Track Rigidity at 0 Z=195cm(TOI), 1 Z=0(Middle TkInner), 2 Z=-70cm(RICH), 3: Z=-136cm(L9)
  /*!
   * Note: Rigidity position dependence only supported by Kalman*/
  Double32_t PZRigidity[DEF_NTKZRIG];
  ...
  /// Returns the X residual on selected layer (J-scheme),999 means failed
  float GetResidualX_LayJ(int layJ) const {
    int lay=TkDBc::Head->GetLayerFromJ(layJ);
    return Residual[lay-1][0];
  }
  /// Returns the Y residual on selected layer (J-scheme),999 means failed
  ...
  /// DEPRECATED Returns the X residual on selected layer (OLD scheme),999 means failed
  float GetResidualX_Lay(int lay) const {
    return Residual[lay-1][0];
  }
  /// DEPRECATED Returns the Y residual on selected layer (OLD scheme),999 means failed
  ...
  TrTrackPar()
    ...
    for (int iz = 0; iz < DEF_NTKZRIG; iz++)PZRigidity[iz]=0;
...
public:
  ...
  enum EFitMethods { 
    ...
    kChoutko       = 1,
    /// GEANE DAF    filter      : implemented by Q.Yan
    kGEANE_DAF     = 2,
    /// GEANE Kalman filter      : implemented by Q.Yan
    kGEANE_Kalman  = 3, 
    /// Juan algo
    kAlcaraz       = 4,
    ...
    kSimple        = 12,
    /// General broken lines fitter (see NIM A 673 (2012) 107–110) AMS: by Q.Yan
    kGBLFITTERAMS  = 13,
    /// General broken lines fitter original: by Q.Yan
    kGBLFITTER     = 14
  };
  ...
  enum EFitOptions { 
    ...
    kMC  = 0x1000000,
    /// Energy Loss switch
    kEnergyLoss = 0x2000000,
    /// New V6 alignment 2022 
    kAlignV6 =0x4000000
  };
  ///_ReconType/10%10: new algorithm rec-steps(0-3); _ReconType/100%100: Track serial-number;  _ReconType/10000%10: primary Track tag
  short int _ReconType;

#define DEF_ADVFIT_NUM 4
  static int DefaultAdvancedFitFlags[DEF_ADVFIT_NUM];

#define DEF_ADVFIT_NUM2 7
  static int DefaultAdvancedFitFlags2[DEF_ADVFIT_NUM2];

  /// Advanced fit bits
  static int AdvancedFitBits;
  /// Advanced fit bits for Half-Span
  static int AdvancedFitBitsHalf;
  /// Advanced fit bits for External Alignment, otherwise only PG was kept
  static int AdvancedFitBitsExtAll;
  //==== Static members ===

  /// the charge object(implemented in 2018 by Hu LIU)
  TrTrackChargeH trkcharge; //!
...
protected:
  ...
  /// Track 0 old only, 1  new-old whole track share, 2  old share one hit, 3  old share end hits.
  /// Track pattern ID
  short int _Pattern;
  /// Number of hits
  short int _Nhits;
...
//  TrFit _TrFit;  //!
  ...
  //! Track Status word
  int Status;
  //! the mass used for advanced fit
  float advancedfitmass;
  //! the charge used for advanced fit
  float advancedfitcharge;
...
public:
  ...
  //! returns the Number of Hits in Inner layers (L2J-L7J)
  int GetNhitsI  (int ixy=1) const { 
    return ixy==1? _Nhits-((_bit_pattern&0x080)?1:0)-((_bit_pattern&0x100)?1:0) : _NhitsXY-((_bit_patternX&0x080)?1:0)-((_bit_patternX&0x100)?1:0);
  }
  //! returns the Number of Inner Planes (L2J-L7J)
  int GetNPlanesI (int ixy=1) const{
    int pbit=0, np=0;
    int bitpatt=(ixy==1)? GetBitPatternJ(): GetBitPatternXYJ();
    for(int ilay=0+1;ilay<9-1;ilay++){
      if(bitpatt&(1<<ilay))pbit|=(1<<(ilay/2));
    }
    for(int ip=0;ip<4;ip++){if(pbit&(1<<ip))np++;}
    return np;
  }
  //! Number of  X hits
  ...
  /// Get the pointer of the tracker layer geometry (New V6 version, new alignment 2022)
  TkLayerN *GetLayer(int layJ) const {return TkTrackN::GetHead()->GetLayer(layJ-1);}
  /// Get the hit coordinate used for the fitting (New V6 version, new alignment 2022)
  /*!
   * \param[in] layJ          tracker layer, J-scheme 1-9
   ...
   * \param[in] chrg          particle charge, 0 means the stored particle charge from latest track fitting
   * \param[in] dxdzg         particle incoming angle dx/dz in global coordinate system, -2 means the stored dx/dz from latest track fitting
   * \param[in] dydzg         particle incoming angle dy/dz in global coordinate system, -2 means the stored dy/dz from latest track fitting
   * \param[in] updatecls     1 update particle charge and incoming angle to the TrCluster, 0 no update 
   * \return    local coordinate in sensor (getM) and global coordinate in tracker (getMGlobal)
   */ 
  AMSPlaneM GetHitCooLJN(int layJ, int ualign=111, bool dzshiftondxy=1, float chrg=0, float dxdzg=-2, float dydzg=-2, bool updatecls=0) const;
  ...
  int SetRecType( int rtype  ){_ReconType=rtype; return rtype;}
  /// Return the mass used for advanced fit
  float GetAdvancedFitMass() const {return advancedfitmass;}
  /// Return the charge used for advanced fit
  float GetAdvancedFitCharge() const {return advancedfitcharge;}
  double ReFitHitStat(float chrg=0,int layJ=-1,int id = 0,bool forceV6=0);
  ...
  /**@{*/
   ...
   //! It gives you the integer number (fit code) to be used to access the fit results (TrTrackPar obj)Advanced TrTrackPar accessor
   /*! \param[out]  _TrFit Fitting manager, _TrFit only filled when refit>=2
   */
  int   iTrTrackPar(TrFit &_TrFit, int algo=0, int pattern=0, int refit=0, float mass = DefaultMass, float chrg = DefaultCharge, float beta=999,float fixrig=0.);
  ...
  /*!
    ...
    \li 5 GEANE DAF filter (introduced by Q.Yan)
    \li 6 GEANE Kalman filter (introduced by Q.Yan)
    \li 7 General broken lines (Gbl) fitter (see NIM A 673 (2012) 107–110) AMS: by Q.Yan
    \li 8 General broken lines (Gbl) fitter original: by Q.Yan
    \li +10 mscattering off
    \li +30 energy_loss off
    \li +40 energy_loss off and mscattering off
    ...
    \li +30  New V6 version Alignment (new alignment 2022), tracker geometry managed by TkTrackN
    \li  30  do not refit 
    \li  31  refit if does not exist 
    \li  32  force refit 
    \li  33  same as 32 
    \li  XYZ034 (only for experts) X=1(0)/Y=1(0)/Z=1(0) with(without) V6 layer/ladder/sensor alignment (e.g. 1034 with only sensor alignment)
    \li  DEPRECATED OPT (kept for compatibility)
    \li  4  same as 3
    \li  5  same as 13
    \li  6  perform MC clusters fit
    \li 16  perform MC clusters fit trying using only primaty one first  
    ...
    If not loaded elsewhere the program try load the file AMSDATADIR.amsdatadir/MagneticFieldMapPermanent_NEW.bin
  ...
  !*/
  const TrTrackPar&  gTrTrackPar(int fit_type);
  ...
  /// Get fitted Rigidity from TrTrackPar corresponding to id and iz (Z postion)
  /*! 
   * \param[in] iz: -1 default, 0 Z=195cm(TOI), 1 Z=0(Middle TkInner), 2 Z=-70cm(RICH), 3: Z=-136cm(L9)
   * \return    Rigidity at Z postion, =0 if interpolation failed
   */
  double   GetRigidity (int id= 0, int iz= -1) const;
  ...
  //! Get p-value of fitting
  /*! \param[in]  ixy: 0 X, 1 Y, 2 (X+Y) 
   */
  double GetPVal      (int ixy=2, int id= 0) const;
  ...
  //! Return time-dependent corrected rigidity (valid only for PM6 alignment)
  /*!
   * \param[in]  version:  0 old rigidity-scale, 1 5years new rigidity-scale(QY), 3 7years new rigidity-scale(QY)
   * \param[in] iz: -1 default, 0 Z=195cm(TOI), 1 Z=0(Middle TkInner), 2 Z=-70cm(RICH), 3: Z=-136cm(L9)
   */
  double GetCorrectedRigidity (int id= 0, int version= 3, int iz= -1 ) const;
  ...
  /// Return an AMSPoint with (X,Y) residual at layer ilay J-scheme (1-9) Z is the layer avarage one and should be not tusted too much, X(Y)=999 means failed
  ...
  /// Return an AMSPoint with (X,Y) residual at layer ilay Old-scheme (1-9) Z is the layer avarage one and should be not tusted too much, X(Y)=999 means failed
  /// \parameter id  Fit id for the residuals
  /// \return The obtained quantity can be:
  /// \li The true fit residual => In the case the track has an hit on the layer and that hit is included in the fit 
  /// \li The distance between the hit and the track fit => in the case the track has an hit on the layer that  is NOT used in the fit
...
//  TrFit *GetTrFit() { return &_TrFit; }
  ...
  /// Interpolation onto Z=const. plane. Please use TrFit.InterpolateKalman for GEANE Kalman (DAF) algorithm and TrFit.InterpolateGbl for Gbl algorithm
  /*!
   * \param[in]  zpl  Plane position (Z=zpl)
   * \param[out] pnt  Track position  at Z=zpl
   * \param[out] dir  Track direction at Z=zpl
  ...
  ///  Interpolation onto AMSPlane. Please use TrFit.InterpolateKalman for GEANE Kalman (DAF) algorithm and TrFit.InterpolateGbl for Gbl algorithm
  /*!
   * \param[in,out] pl        Destination Plane, call pl.getP() and pl.getD() to get track position and direction (intersection point with the Plane) in global coordinate system, or call pl.getPLocal() and pl.getDLocal() to get them in local (Plane) coordinate system
   * \param[out]    rigidity  Track rigidity at the Plane
   * \param[in]     id        Fitting method ID
   * \param[in]     mass      Paritcle mass
   * \param[in]     chrg      Particle charge
   * \param[in]     eloss     0(1) rigidity without(with) energy loss correction
   * \param[in]     direction 1(-1) downward (upward) going particle
   * \return                  Path length between Z=P0z(usually 0) and the Plane, <=-1000 interpolation failed
   */
  double Interpolate(AMSPlane &pl, double &rigidity, int id = 0, float mass = DefaultMass, float chrg = DefaultCharge, int eloss=0, int direction=1) const;

  ///  Interpolation onto Tracker layer J-Scheme (New V6 version, new alignment 2022)
  /*!
   * \param[out]    length    Path length between Z=P0z(usually 0) and the Plane, <=-1000 interpolation failed
   * \param[out]    rigidity  Track rigidity at the Plane
   * \param[in]     layJ      Tracker layer, J-scheme 1-9
   * \param[in]     id        Fitting method ID
   * \param[in]     mass      Paritcle mass
   ...
   * \param[in]     eloss     0(1) rigidity without(with) energy loss correction
   * \param[in]     direction 1(-1) downward (upward) going particle
   * \return                  Measurement on sensor plane together with track interpolation if hit exists, otherwise the closest sensor plane with track interpolation
   */
  AMSPlaneM InterpolateLayerJN(double &length, double &rigidity, int layJ, int id = 0, float mass = DefaultMass, float chrg = DefaultCharge, int eloss=0, int direction=1) const;
  ...
  static int DefaultChargeVersion; 
#pragma omp threadprivate(DefaultChargeVersion)

  /** @name Track charge estimation (best available estimators shortcuts) */
  /**@{*/
  //! Get the full tracker floating point charge estimator 
  mean_t  GetQ_all(float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);
  //! Get the inner tracker floating point charge estimator (all calculated information)
  mean_t  GetInnerQ_all(float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);
  //! Get floating point charge estimator of a hit
  float   GetLayerJQ(int jlayer, float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);
  ...
  float   GetQ(float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);
  //! Get the number of points used in the full tracker floating point charge estimator (better to use GetQ_all)
  int     GetQ_NPoints(float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);
  //! Get the RMS of the full tracker floating point charge estimator (better to use GetQ_all)
  float   GetQ_RMS(float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);
  //! Get the inner tracker floating point charge estimator (better to use GetInnerQ_all)
  float   GetInnerQ(float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);
  //! Get the number of points used in the inner tracker floating point charge estimator (better to use GetInnerQ_all)
  int     GetInnerQ_NPoints(float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);
  //! Get the RMS of the inner tracker floating point charge estimator (better to use GetInnerQ_all) 
  float   GetInnerQ_RMS(float beta = 1, int fit_id = -1, float mass_on_z = 0, int version = TrTrackR::DefaultChargeVersion);

  /// Get charge estimation by YJ (all calculated information)
  TrQYJTrack GetQYJ_all(int iside = 2, float beta = 1, int fit_id = -1, int qopt=TrChargeYJ::DefaultCorOpt|TrChargeYJ::kPow);
  /// Get floating charge estimation by YJ
  float   GetInnerQYJ(float &innerq_rms, int &innerq_patt, int iside = 2, float beta = 1, int fit_id = -1, int qopt=TrChargeYJ::DefaultCorOpt|TrChargeYJ::kPow);
  /// Get charge estimator of a hit by YJ (all calculated information)
  TrQYJHit GetLayerQYJ_all(int jlayer, int iside = 2, float beta = 1, int fit_id = -1, int qopt=TrChargeYJ::DefaultCorOpt);
  /// Get floating point charge estimator of a hit by YJ
  float    GetLayerQYJ(int jlayer, int iside = 2, float beta = 1, int fit_id = -1, int qopt=TrChargeYJ::DefaultCorOpt);

    //! Get the full tracker floating point charge estimator implemented by Hu LIU. X: iside=0; Y: iside=1; X&Y combined: iside=2 (Z0 is the seed charge, qopt=-1 use the default charge option)
  mean_t  GetQH_all(int iside=2, float beta = 1, int fit_id = -1, int qopt=-1, float Z0=-1);
  //! Get the full tracker floating point charge estimator (better to use GetQH_all), implemented by Hu LIU. X: iside=0; Y: iside=1; X&Y combined: iside=2 (Z0 is the seed charge, qopt=-1 use the default charge option)
  float   GetQH(int iside=2, float beta = 1, int fit_id = -1, int qopt=-1, float Z0=-1){
     return GetQH_all(iside,beta,fit_id,qopt,Z0).Mean;
  }
  //! Get the inner tracker floating point charge estimator (all calculated information) implemented by Hu LIU. X: iside=0; Y: iside=1; X&Y combined: iside=2 (Z0 is the seed charge, qopt=-1 use the default charge option)
  mean_t  GetInnerQH_all(int iside=2, float beta = 1, int fit_id = -1, int qopt=-1, float Z0=-1);
  //! Get the inner tracker floating point charge estimator (better to use GetInnerQH_all), implemented by Hu LIU. X: iside=0; Y: iside=1; X&Y combined: iside=2 (Z0 is the seed charge, qopt=-1 use the default charge option)
  float   GetInnerQH(int iside=2, float beta = 1, int fit_id = -1, int qopt=-1, float Z0=-1){
     return GetInnerQH_all(iside,beta,fit_id,qopt,Z0).Mean;
  }
  //! Get floating point charge estimator of a hit, implemented by Hu LIU. X: iside=0; Y: iside=1; X&Y combined: iside=2 (Z0 is the seed charge, qopt<0 use the default charge option(qopt=-2 is the unbiased charge, qopt=-1 is the track associated charge, the only difference is whether use Ztrk to calculate eta))
  float   GetLayerJQH(int jlayer,int iside = 2, float beta = 1, int fit_id = -1, int qopt=-2, float Z0=-1, int res_mult=-1, float dxdz=-2, float dydz=-2);
  /**@}*/

  /** @name Track mass estimation */
  /**@{*/
  //! Get the likelihood value for the assumption for nucleus(z,a) with beta
  float GetMassLikelihood(int z, int a, float beta);
  /**@}*/
  ...
  /// Clear _TrackPar
  void ClearPar() { _TrackPar.clear(); }

  /// remove TrTrackPar with id
  int RemovePar(int id);
  ...
  geant Chi2FastFitf() const {return GetChisq();}

  geant HChi2f(int half) const {
      int iid=trdefaultfit&(~(kFitLayer8|kFitLayer9)); 
      return (half==0)?GetChisq(iid|kUpperHalf):GetChisq(iid|kLowerHalf);
  }

  geant FChi2MSf() const {
     int iid=trdefaultfit&(~(kFitLayer8|kFitLayer9|kMultScat));
     return  GetChisq(iid);
  }

  geant HRigidityf(int half) const {
     int iid=trdefaultfit&(~(kFitLayer8|kFitLayer9));
     return (half==0)?GetRigidity(iid|kUpperHalf):GetRigidity(iid|kLowerHalf);
  }
  ...
  /// Keep the actual coordinates at LayerJ=i+1 used in the last FitT
  static AMSPoint FitCoo[trconst::maxlay];
#pragma omp threadprivate(FitCoo)

  /// Perform 3D fitting with the method specified by ID
  /*! \param[out]  _TrFit Fitting manager
   */
  float FitT(TrFit &_TrFit, int id = 0,
             int layer = -1, bool update = true, const float *err = 0,
             float mass = DefaultMass, float chrg = DefaultCharge, float beta=999.,float fixrig=0., int ualign=111);
...
/*! \brief Performs again the existing fits with a different particle evaluation
  ...
  int AdvancedFitDone(int add_flags=0, bool primary=true) const;

  /// Do advanced fits specified in the default list and add in OR the add_flags
  int DoAdvancedFit(int add_flags=0, float mass=0, float chrg=0, bool primary=true);
  ...
  void RecalcHitCoordinates(int id = 0,bool mc=false, bool primary=false, float chrg=0, int ualign=111);
  ...
  */
  ...
  /// Find BetaHR which associates with this TrTrack
  BetaHR *pBetaH();

  /// Get beta from pBetaH
  float GetBeta();
  ...
  int SetResolvedMultiplicityLJ(int jlay,int mult);
  int UpdateHitCoo();
  ClassDef(TrTrackR, 7); 
