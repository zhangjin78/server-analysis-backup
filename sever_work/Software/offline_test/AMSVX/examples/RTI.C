...
// -----------------------------------------------------------
//        Created:       2013-Jan-26  Q.Yan  qyan@cern.ch (User Guide)
//        Run:           root -b -q RTI.C+
// -----------------------------------------------------------
...
int RTI(){
   ...
   AMSChain a;
   a.Add("root://eosams.cern.ch//eos/ams/Data/AMS02/2022/ISS.B1236/pass8/1355240756.00000001.root");
   a.Add("root://eosams.cern.ch//eos/ams/Data/AMS02/2022/ISS.B1236/pass8/1355242081.00000001.root");
   int Tcalopt=1;//0 by Time; 1 by Processed Runs (suggested); 2 by Run Id
...
//--Time Calculation
  switch(Tcalopt){
   ...
   case 1:  //---Calculate Time method 2  //by Processed Runs from Recording AMSEventR:fRunList (recommended)
...
bool TPreSelect(AMSSetupR::RTI &a){
  ...
  bool isphorun=(a.run>=1620025528&&a.run<1635856717);//run with photon polarization trigger
  bool cut[10]={0};
...
//-----
  AMSPoint nexl[2];
  AMSEventR::GetRTInL1L9(nexl,a.utime,90);//get total number of events having an inner track and external-layer hits within 90 secs 
  cut[9]=(nexl[0][0]>700&&nexl[1][0]>500);
//----
...
//----Process Event Select Helium Using Max-TkQ BetaH
   if(pev->nParticle()==0)return false;
   BetaHR *betah=pev->pParticle(0)->pBetaH();
   if(!betah||betah->GetBuildType()!=1)return false;
...
//---RIT+Event Cutoff-Cut
   float rig=betah->pTrTrack()->GetRigidity();//no selection on Track pattern and charge
//--Find Cutoff
   double ucutr=1.2*a.cfi[1][1]; //30deg IGRF Cutoff
...
//--Time Cal
     ...
     double ucutr=1.2*a.cfi[1][1];//30deg IGRF Cutoff Recommended
