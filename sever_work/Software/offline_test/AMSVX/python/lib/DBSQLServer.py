...
class DBSQLServer:
    ...
    def __connect(self, connstring):
        ...
        if self.logger is not None:
            self.logger.info("oldsql(Connect) to %s", self.dbfile)

        try:
            self.dbhandler = cx_Oracle.connect(connstring)
    ...
    def Query(self,string):
        if self.logger is not None:
            self.logger.info("oldsql(Query)='%s', host=%s", string, self.dbhandler.dsn)
    ...
    def QuerySafe(self,string):
        if self.logger is not None:
            self.logger.info("oldsql(QuerySafe)='%s', host=%s", string, self.dbhandler.dsn)
    ...
    def Update(self,string):
        if self.logger is not None:
            self.logger.info("oldsql(Update)='%s', host=%s", string, self.dbhandler.dsn)
    ...
    def Commit(self,commit=1):
        if(commit==1):
            if self.logger is not None:
                self.logger.info("sql='COMMIT', host=%s", self.dbhandler.dsn)
            return self.dbhandler.commit()
        else:
            if self.logger is not None:
                self.logger.info("sql='ROLLBACK', host=%s", self.dbhandler.dsn)
    ...
    def sql(self, sql, **kwargs):
        ...
        if commit.match(sql):
            if self.logger is not None:
                self.logger.info("sql='%s', args=%s, host=%s", sql, kwargs, self.dbhandler.dsn)
        else:
            edit = re.compile('^\s*(UPDATE |DELETE |INSERT )')
            if edit.match(sql):
                if self.logger is not None:
                    self.logger.info("sql='%s', args=%s, host=%s, rows=%d", sql, kwargs, self.dbhandler.dsn, self.dbcursor.rowcount)
            else:
