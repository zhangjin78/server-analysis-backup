...
class Logger:
    ...
    log_count = 50
    log_file_with_date = True

    def __init__(self, app_name, log_dir = None, log_file_with_date = True):
        self.app_name = app_name
        self.log_file_with_date = log_file_with_date
        if log_dir is None:
            logpath_pair = commands.getstatusoutput("/afs/cern.ch/ams/local/bin/get_log_path.sh")
            if (logpath_pair[0] == 0):
                self.log_dir  = logpath_pair[1]
        else:
    ...
    def getLogger(self, level_log=None, level_console=None):
        ...
        # create file handler
        if self.log_file_with_date == True:
            log_file = "%s/%s.%s.log" % (
                self.log_dir,
                self.app_name,
        ...
        mail_h = logging.handlers.SMTPHandler('localhost', '%s@example.com' % self.app_name, ['oleg.demakov@cern.ch'], self.app_name)
