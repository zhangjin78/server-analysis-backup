//  $Id: main.cxx,v 1.72 2015/12/23 11:48:25 choutko Exp $
#include <TASImage.h>
#include <TRegexp.h>
...
void OpenChain(AMSChain & chain, char * filenam){
  ...
  else if(a.Contains(gn) || a.Contains(hn)){
  strcpy(filename,filenam);
/*
      bool again=true;
...
again:
    ...
    if(i){
     ...
     if(a.Contains(gn)){ 
     strcpy(filename,"root://eosams.cern.ch///eos/ams/Data/AMS02/2014/ISS.B950/pass6/");
     strcat(filename,filenam+5);
     strcat(filename,".00000001.root");
     }
     else abort();
    }
    else{
      ...
      if(ftxt.good() && !ftxt.eof()){
       ...
       int found = line.find_last_not_of(" ");
       if(found>=0)line.resize(found+1);        
       for(int pos=0;pos<line.length();pos=line.find(".root ",pos)){
        if(pos>0)line.insert(pos+5,"?svcClass=amsuser");
        pos=0;
       }           
       strcpy(filename,line.c_str());
       }
      else{
       cerr<<"main-E-UnableToReadFile "<<tmp<<endl; 
       abort();
     }
      ftxt.close();
      unlink(tmp);
