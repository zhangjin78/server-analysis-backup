...
namespace genfit {
  ...
  public:
     ...
     */
    GblFitter() : AbsFitter(), m_gblInternalIterations(""), m_enableScatterers(true), m_enableIntermediateScatterer(true), scatEpsilon(2.e-8), m_segmentController(nullptr), useAMSTune(0), m_recalcJacobians(0), m_externalIterations(1), traj_(nullptr){;}
     ...
     * @param rads returned: total radition length distribution (QY)
    ...
    ///equivalent thin scatter developed by Q.Yan, asked Q.Yan (qyan@cern.ch) for details 
    double getAMSScatterermMat(double& length,
                               double& theta, double& s, double& ds, double &rads,
                               const double p, const double mass, const double charge,
                               const double dirz,double len,double zm[2]) const;
    /**
    ...
    /// scanning materials by Q.Yan 
    double scanMat(Track* trk, const AbsTrackRep* rep,double rad[20],double fds[20][2],double ftheta[20][2],double fscatmd[20][2]);   
    ...
    gbl::GblTrajectory* getGblInfo() const {return traj_.get();}

  private:
    std::unique_ptr<gbl::GblTrajectory> traj_; // Ownership    
