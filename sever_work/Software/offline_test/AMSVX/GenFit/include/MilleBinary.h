...
class MilleEvent: public TObject{
public:
   MilleEvent(bool doublePrec = false,unsigned int aSize = 2000);
   virtual ~MilleEvent(){}
   bool isdoublePrec(){return (doubleBuffer.size()>=1&&floatBuffer.size()==0);}
   ...
   void read(double *bufferDouble, float *bufferFloat, long int *bufferInt, long int *lengthBuffers, long int *errorFlag);
#else
   void read(double *bufferDouble, float *bufferFloat, int *bufferInt, int *lengthBuffers, int *errorFlag);
#endif
   int lenData();
   int Clear();
public:
...
public:
  MilleBinary(const std::string fileName = "milleBinaryISN.dat",
		bool doublePrec = false, unsigned int aSize = 2000, int usegz=0);
  MilleBinary(TFile *fo,bool doublePrec = false, unsigned int aSize = 2000);
  virtual ~MilleBinary();
  int bookTree();
  MilleEvent *getEvent(){return ev;}
  TChain     *getChain(){return ch;}
  Long64_t    getpEntry(){return entry;}
  int resetChain();
  void addData(double aMeas, double aPrec,
		const std::vector<unsigned int> &indLocal,
		const std::vector<double> &derLocal,
