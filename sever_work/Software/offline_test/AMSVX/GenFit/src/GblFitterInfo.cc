...
namespace genfit {
  ...
  gbl::GblPoint GblFitterInfo::constructGblPoint() {
    ...
    // Derivatives      
    if(hasMeasurements()){//adding alignment module
      ICalibrationParametersDerivatives* globals =dynamic_cast<ICalibrationParametersDerivatives*>(trackPoint_->getRawMeasurement(0));
      if(globals&&globals->isValidCalib()){
        std::vector<int> &labels=globals->globalLabels();
        TMatrixD derivs = globals->globalDerivatives(&sop);
