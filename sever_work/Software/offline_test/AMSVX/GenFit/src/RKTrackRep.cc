...
RKTrackRep::RKTrackRep(double mass, double charge, char propDir) :
  AbsTrackRep(mass,charge,propDir),
  lastStartState_(this),
  lastEndState_(this),
  RKStepsFXStart_(0),
  RKStepsFXStop_(0),
...
double RKTrackRep::RKPropagate(M1x7& state7,
  ...
  TrProp::GuFld(r[0], r[1], r[2], bf);
  for(int ic=0;ic<3;ic++)H0[ic]=bf[ic];//kGauss
#else
...
#ifdef _PGTRACK_
    TrProp::GuFld(r[0], r[1], r[2], bf);
    for(int ic=0;ic<3;ic++)H1[ic]=bf[ic];//kGauss
#else
...
#ifdef _PGTRACK_
    TrProp::GuFld(r[0], r[1], r[2], bf);
    for(int ic=0;ic<3;ic++)H2[ic]=bf[ic];//kGauss
#else
...
double RKTrackRep::estimateStep(const M1x7& state7,
  ...
  if(limits.getLowestLimitVal() > MINSTEP){ // only call stepper if step estimation big enough
    ...
    if(getPDG()==0)MaterialEffects::getInstance()->setParticleMassCharge(getPDGMass(),getPDGCharge());
...
bool RKTrackRep::isSame(const AbsTrackRep* other) {
  ...
  if(getPDG()==0){
    if(getPDGMass() != other->getPDGMass() || getPDGCharge() != other->getPDGCharge() )return false;
  }
