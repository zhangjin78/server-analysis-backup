...
AntiRecoPG::AntiRecoPG(int version, int Istolerant){
    cout << "## AntiRecoPG Initiate Parameters" << endl;
    InitPar(version,Istolerant);
    cout << "## AntiRecoPG version= " << version << ", Ist= " << Istolerant << endl;
}
...
AntiRecoPG* AntiRecoPG::gethead(int version, int Istolerant) {
  ...
  //  if (version!=12) cerr << "## AntiRecoPG::gethead()-E- " << version << " change not permitted! " <<endl;
  }
  return head;
}
...
int AntiRecoPG::LoadHistoCaliFile( int VERNUM, int iforce){ 
  ...
  if (VERNUM>=0){
    ...
    const char* dbPath= getenv("AntiPGDataDir");
    if (dbPath==NULL) {
	dbPath = Form("%sAntiPG", AMSDATADIR.amsdatadir);
    }
...
int AntiRecoPG::LoadCaliFile( int VERNUM, int iforce){ 
  ...
  if (VERNUM>=0){
	...
	if(stringa == "version" || stringa == "submitted") {
	  cout << "AntiRecoPG::LoadCaliFile " << stringa << " " << stringa2 << endl; ifind++;}
	if(stringa == "isMC") {
	  ifind++;
