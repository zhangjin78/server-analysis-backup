...
//     TXNetFile g("");
//     TCastorFile h("");
int rootread(char * fname, int nev, int iver,  int & lastevent,bool jou,bool sfast){
...
//if(! rfile->Get("TrInnerDzDB"))return -1;
#ifdef __ICC
if(TStreamerInfo::fgWarn)return -1;
#endif
 if(jou){
   TObjString s("");
   s.Read("DataCards");
...
if(TStreamerInfo::fgWarn)return -1;
  ...
  //rfile->Close();
  if(nevread!=nev){
        if(iver)cerr<<" nevread!=nev "<<nevread<<" "<<nev<<endl; 
        return -2;
}
