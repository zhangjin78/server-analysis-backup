/// $Id: TrRawCluster.C,v 1.19 2018/01/23 11:37:08 choutko Exp $ 
...
TrRawClusterR::TrRawClusterR(int tkid, int clsaddwrd, int clslenwrd, short int* adc) {
  ...
  if (adc) for (int i = 0; i <((_lengthword&0x7f)+1) ; i++) _signal.push_back(MCConvert(adc[i],true));
}
...
#include "job.h"
#include "tkdcards.h"
float TrRawClusterR::MCConvert(float a, bool back){
#ifdef _PGTRACK_
 a/=8;
 if(AMSJob::gethead()->isRealData() || !TRMCFFKEY.DisableOverflow || a<=TRMCFFKEY.TPar[0])return a;
 if(!back){
  float b=(TRMCFFKEY.TPar[0]+(a-TRMCFFKEY.TPar[0])/TRMCFFKEY.TPar[1]);
   b*=8;
  if(b>32767){
   static int prt=0;
   if(prt++<100)cerr<<"TrRawClusterR::MCConvert-W-OverFlow "<<a<<" "<<b<<endl;
   b=32767.; 
  }
  return b;
 }
  ...
  float b=TRMCFFKEY.TPar[0]+(a-TRMCFFKEY.TPar[0])*TRMCFFKEY.TPar[1];
//  cout<<"TrRawClusterR::MCConvert-I- "<<a<<" "<<b<<endl;
  return b;
 }
#else
return 0;
