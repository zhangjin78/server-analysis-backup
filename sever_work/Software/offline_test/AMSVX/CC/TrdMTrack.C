...
void TrdMTrack::Init_Base(){
  ...
  sprintf(fname, "%sMTrack_lookup_files.txt", AMSDATADIR.amsdatadir);//the directory has been changed to v6.00
...
void TrdMTrack::update_alignment_vectors(unsigned long evttime){
  ...
  TString name;
  name=TString(AMSDATADIR.amsdatadir)+"TRD/Modul_alignment_"+TString(number)+".root";
...
void TrdMTrack::Init_PDFs(){
  ...
  if(TrdPdfType==0){
    ...
    // TString name=TString(AMSDATADIR.amsdatadir)+"TRD/TrdScalibPdfs_v23.root";
    TString name=TString(AMSDATADIR.amsdatadir)+"TRD/acroot/data/TrdQt_pdf_v11.root";
