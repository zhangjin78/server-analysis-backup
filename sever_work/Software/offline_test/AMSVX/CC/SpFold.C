...
Double_t SpFold::PDFS(Double_t *xp, Double_t *par)
{
  Double_t pr[13];
  for (Int_t i = 0; i < 13; i++) pr[i] = (i == 1) ? 0 : par[i];
  ...
  for (Int_t i = 0; i < nd; i++) {
    ...
    Double_t rg = TMath::Sqrt(r1*r2);
    Double_t dr = (r2-r1)*((fXtype == RINV) ? 1 : 1/rrec/rrec);
    Double_t x  = 1/rrec-1/rg -par[1];
    ss += Flux(&rg, pf)*(*fRPDF)(&x, pr)*dr;
  }
...
TF1 *SpReso::Fit(TH1 *hist, Option_t *option, Double_t range)
{
  ...
  if (range < 1) {
    func->SetParameter(Nx+2, 0);
    func->SetParameter(Nx+3, 0);
  }
