...
TMinuit *TrdKCluster::gMinuit_RigidityEstimate = NULL;
...
float TrdKCluster::DefaultMCGainCalibration=1.113;
...
int TrdKCluster::DoAddEmptyTube=0;
...
void TrdKCluster::Init(AMSEventR *evt){
    ...
    QTRDHitCollection.reserve(NTRDHit);
    QTRDHitCollectionRefit.reserve(NTRDHit);
    QTRDHitCollectionNuclei.reserve(NTRDHit);
    QTRDHitCollectionDeltaRay.reserve(NTRDHit);
    QTRDHitCollectionNucleiUpper.reserve(NTRDHit);
    QTRDHitCollectionNucleiLower.reserve(NTRDHit);

    QTRDHitCollection.clear();
    QTRDHitCollectionRefit.clear();
    QTRDHitCollectionNuclei.clear();
    QTRDHitCollectionDeltaRay.clear();
    QTRDHitCollectionNucleiUpper.clear();
    QTRDHitCollectionNucleiLower.clear();

    DoHitPreselection(MinimumDistance); //=======Select +/- MinimumDistance (cm) volumn around TrTrack=============
    if(DoAddEmptyTube)AddEmptyTubes(MinimumDistance);  //===========Add empty tubes to complete the geometry==========
    ...
    if(IsReadCalibOK && ForceReadCalibration>0){
        ...
        LastProcessedRun_Calibration=Time;
		 ...
		 IsReadCalibOK=DoGainCalibrationMC();
		 LastProcessedRun_Calibration=0;
    }
...
int TrdKCluster::Build(){
    ...
    if(IsMC){
        ...
        TrdKCluster::SetDefaultMCGainCalibration(1.113);
    } else if(IsTB){
...
int TrdKCluster::FindBestMatch_FromECAL(){
    ...
    for(Int_t i=0; i<ntrk; i++){
        ...
        if(fitid<0)fitid=0;//you can also continue to abandon this event
...
void TrdKCluster::Init_Base(){
    ...
    if(TRDTubeCollection.size()!=TrdHCalibR::n_tubes)Constrcut_TRDTube();
...
int TrdKCluster::DoGainCalibration(){
    ...
    if(read==0){
        static int ic=0;
        if(ic++<10)cout<<"~~~~~~~WARNING, TrdKCluster~~~~~~~Can Not Read DBs for Gain Calibration, Returning 0"<<endl;
        return 0;
...
int TrdKCluster::DoGainCalibrationMC(){
    for(int i=0;i<NHits();i++){
        GetHit(i)->DoGainCalibration(DefaultMCGainCalibration);
    }
    return 1;
}
/////////////////////////////////////////////////////////////////////
...
int TrdKCluster::DoAlignment(int &readplane, int &readglobal){
    ...
    if(read==0){
        static int ic=0;
        if(ic++<10)cout<<"~~~~~~~WARNING, TrdKCluster~~~~~~~Can Not Read DBs for Alignment, Returning 0"<<endl;
        return 0;
    }else if(read==2){
        for(int i=0;i<NHits();i++){
...
void TrdKCluster::AddEmptyTubes(float cut_distance){
    ...
    selected_tube_collection.reserve(500);
...
void TrdKCluster::SetTrTrack(TrTrackR* track, int fitcode){
    ...
    if(!track->ParExists(fitcode)) {
        ...
        static int ic=0;
        if(ic++<10)cout<<"~~WARNING~~TrdKCluster::SetTrTrack : Input fitcode do not exist, use default fitcode : "<<fitcode<<endl;
    }
...
int TrdKCluster::GetLikelihoodRatio_TRDRefit(float threshold, double* LLR, double* L, int &nhits,float &total_pathlength, float &total_amp , int fitmethod, int particle_hypothesis,int flag_debug, float ECAL_Energy_Hypothesis){
    if(!HasTRDTrack && !fitmethod)fitmethod=1;
    if(HasTRDTrack==0 || fitmethod>0)   FitTRDTrack(fitmethod,particle_hypothesis);
    if(flag_debug<0)return GetLikelihoodRatio(threshold,LLR,L,nhits,total_pathlength,total_amp,&TRDtrack_extrapolated_P0,&TRDtrack_extrapolated_Dir,ECAL_Energy_Hypothesis);
...
int TrdKCluster::GetLikelihoodRatio_TrTrack(float threshold, double* LLR, double* L, int &nhits, float &total_pathlength, float &total_amp,int flag_debug, float ECAL_Energy_Hypothesis){
  if(flag_debug<0)return GetLikelihoodRatio(threshold,LLR,L,nhits,total_pathlength,total_amp,&track_extrapolated_P0,&track_extrapolated_Dir,ECAL_Energy_Hypothesis);
...
void TrdKCluster::InitXePressure(){
    ...
    TString name=TString(AMSDATADIR.amsdatadir)+"trd_pressures.root";//the directory has been changed to v6.00
...
////////// For Rigidity Estimation///////////////////

void TrdKCluster::fcn_RigidityEstimate(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag){
    TrdKCluster* cluster=( TrdKCluster*)gMinuit_RigidityEstimate->GetObjectFit();
    f=cluster->EstimateRigidityFromLikelihood(par);
    return;
}

double TrdKCluster::EstimateRigidityFromLikelihood(Double_t *par){

    double f=0;
    float rig=par[0];

    for(int i=0;i<v_amp.size();i++){
        float path_length=v_path.at(i);
        int Layer=v_layer.at(i);
        float Amp=v_amp.at(i);
        float Xe = v_xe.at(i);
        kpdf_p->GetPar_Smooth(fabs(rig),path_length,Layer,Xe/1000.);
        double kp=kpdf_p->GetLikelihood(Amp);
        f+=-2*log(kp);
    }
    return f;
...
int TrdKCluster::GetRigidityEstimate(float threshold, float rig_init, double& rig, double& rig_err){


    if(!gMinuit_RigidityEstimate)  gMinuit_RigidityEstimate=new TMinuit(1);
    ...
    for(int i=0;i<NHits();i++){
        ...
        float path_length=hit->Tube_Track_3DLength(&track_extrapolated_P0,&track_extrapolated_Dir);
        int Layer=hit->TRDHit_Layer;
        float Amp=hit->TRDHit_Amp;
        if(Amp>threshold && path_length>0.3){
            Track_nhits++;
            v_amp.push_back(Amp);
            v_path.push_back(path_length);
            v_layer.push_back(Layer);
            v_xe.push_back(Pressure_Xe);
        }
    }

    gMinuit_RigidityEstimate->SetFCN(fcn_RigidityEstimate);

    gMinuit_RigidityEstimate->SetObjectFit(this);

    float init_p=0;
    if(rig_init!=0)init_p=rig_init;
    else init_p = fabs(Track_Rigidity);
    if(!init_p) init_p = 100;

    double arglist[10];
    arglist[0] = -1;
    gMinuit_RigidityEstimate->SetPrintLevel(-1);
    int ierr=0;
    gMinuit_RigidityEstimate->mnparm(0,  "p",  init_p, 0.5*init_p, 1 , 10000,ierr);
    arglist[0]=0;
    gMinuit_RigidityEstimate->mnexcm("MIGRAD", arglist, 0,ierr);

    double bnd1, bnd2;
    int ivar;
    TString name;
    gMinuit_RigidityEstimate->mnpout(0, name, rig, rig_err, bnd1, bnd2, ivar);

//    cout<<"TRD Rigidity Estimate: " <<rig<<", "<<rig_err<<",  FromTrTrac: " <<Track_Rigidity<<endl;
