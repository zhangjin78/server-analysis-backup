// $Id: GM_SubLibrary.C,v 1.12 2022/06/16 10:30:27 qyan Exp $
//--------------------------------------------------------
//    
...
void GM_ScanIGRF(GMtype_Data *G10, GMtype_Data *G11, GMtype_Data *H11, GMtype_Data *G20, GMtype_Data *G21, GMtype_Data *H21, GMtype_Data *G22, GMtype_Data *H22)
{
	...
	int i;
	double temp;
	char buffer[256]; 		/*check buffer!*/
	FILE *IGRF;
        char pathFile[2048];
        sprintf( pathFile,"%s%s", AMSDATADIR.amsdatadir, "IGRF13.tab" );//the directory has been changed to v6.00
	...
	for( i = 0; i < 4; i++)
	{
		fgets(buffer, 256, IGRF);		/*first 4 lines...*/
	}
