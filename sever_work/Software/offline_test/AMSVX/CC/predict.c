...
#pragma omp threadprivate  	(tsince, jul_epoch, jul_utc, eclipse_depth)
#pragma omp threadprivate        (sat_azi, sat_ele, sat_range, sat_range_rate)
#pragma omp threadprivate	(sat_lat, sat_lon, sat_alt, sat_vel, phase)
#pragma omp threadprivate (sun_azi, sun_ele, daynum, fm, fk, age, aostime)
#pragma omp threadprivate (lostime, ax, ay, az, rx, ry, rz, squint, alat, alon)
#pragma omp threadprivate (sun_ra, sun_dec, sun_lat, sun_lon, sun_range, sun_range_rate)
#pragma omp threadprivate (moon_az, moon_el, moon_dx, moon_ra, moon_dec, moon_gha, moon_dv)
...
#pragma omp threadprivate	(qthfile, tlefile, dbfile, temp, output)
#pragma omp threadprivate	(serial_port, resave, reload_tle, netport)
#pragma omp threadprivate	(once_per_second, ephem, sat_sun_status, findsun)
#pragma omp threadprivate	(calc_squint, database, xterm, io_lat, io_lon)


int	indx, antfd, iaz, iel, ma256, isplat, isplong, socket_flag=0,
	Flags=0;

#pragma omp threadprivate (indx, antfd, iaz, iel, ma256, isplat, isplong, socket_flag,Flags)

long	rv, irk;
#pragma omp threadprivate	(rv, irk)

unsigned char checksum_table[256];
bool checksum_table_initialized = false;
...
void initialize_checksum_table() {

  int x;
  for (x=0; x<=255; checksum_table[x]=0, x++);
  for (x='0'; x<='9'; checksum_table[x]=x-'0', x++);
  checksum_table['-']=1;
  checksum_table_initialized = true;
}
...
char KepCheck(const char *line1,const char *line2)
{
	...
	unsigned sum1, sum2;
        ...
        if (!checksum_table_initialized)
          initialize_checksum_table();

	/* Compute checksum for each line */
	for (x=0, sum1=0, sum2=0; x<=67; sum1+=checksum_table[(int)line1[x]], sum2+=checksum_table[(int)line2[x]], x++);

	/* Perform a "torture test" on the data */

	x=(checksum_table[(int)line1[68]]^(sum1%10)) | (checksum_table[(int)line2[68]]^(sum2%10)) |
...
void Data2TLE(int x)
{
	...
	for (i=0, sum=0; i<=67; sum+=checksum_table[(int)line1[i]], i++);

	line1[68]=(sum%10)+'0';

	for (i=0, sum=0; i<=67; sum+=checksum_table[(int)line2[i]], i++);
...
int ISSLoad(const char *name,const char *line1, const char *line2){


			if (strstr(name,"ISS") && KepCheck(line1,line2)) {

				strncpy(sat[0].name,name,24);
				strncpy(sat[0].line1,line1,69);
				strncpy(sat[0].line2,line2,69);
