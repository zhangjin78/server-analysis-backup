...
double TrSimSensor::GetDiffusionWidth(double y0, int charge) {
  // E. Belau et al. NIM A (1983) 253-260. 
  double k_B = 1.3806503e-23; //! Boltzmann constant (m^2kg/s^2/K)  
  double e = 1.602176565e-19; //! Elementary charge (C)
  double T = 273.15; // K 
  double Vd = 35; //! depletion voltage is 35 +- 8 V (N. Dinu, E. Fiandrini, Proc. 20th IEEE Instr. & Meas. Tech. Conf., Vail, Colorado, USA, 2003.)
  double Vb = 80; //! bias is 80 V
  double d = SUBSTRATEWIDTH*1e-6; // m
  if (y0>d) y0 = d;
  if (y0<0) y0 = 0; 
  double fact = TRMCFFKEY.BelauFactDiffusion;
  if ( (fact<0)&&(charge!=0) ) fact = TMath::Max(2.,2.+(abs(charge)-2)*0.125); // hard-coded diffusion dependence on particle charge
  return TMath::Max(TRMCFFKEY.BelauMinDiffusion*1e-6,fact*d*sqrt(-(k_B*T/e)*log(1-(2*Vd/(Vd+Vb))*((d-y0)/d))/Vd));
}


TrSimCluster TrSimSensor::MakeImplantCluster(double senscoo, double sensangle, double dzstep, double sensheight, double hallproj, int charge) {
  ...
  double diffusion  = GetDiffusionRadius()/GetImplantPitch();                      // diffusion for model 4 [implant pitch]
  double sign       = (sensangle>0) ? -1 : 1;                                      // angle [rad]
  int    seedind    = (int) ceil(coordimpl-0.5);                                   // seed strip address
  // tmp cluster vars
  double check = 0.;
  ...
  for (int istrip=seedind; istrip<GetNImplantStrips(); istrip++) {
    double x1 = (istrip-0.5)*GetImplantPitch();
    double x2 = (istrip+0.5)*GetImplantPitch();
    double weight = 0.;
    switch (GetDiffusionType()) {
      ...
      case 4:
        ...
        break;
      case 5:
        weight = GetWeightBelau(senscoo*1e4,sensheight,fabs(dzstep/cos(sensangle)),sensangle,x1,x2,hallproj,charge);
        break;
      default:
...
double TrSimSensor::GetWeightBelau(double senscoo, double sensheight, double sensstep, double sensangle, double x1, double x2, double hallproj, int charge) {
  if (VERBOSE) {
    printf("TrSimSensor::GetWeightBelau-V-Begin coo(um):%7.1f height(um):%7.1f step(um):%7.1f angle:%7.4f x1(um):%7.1f x2(um):%7.1f Z:%2d\n",
      senscoo,sensheight,sensstep,sensangle,x1,x2,charge
    );
  }
  // sensor shift with angle (rought approx.) 
  double shift = 0;
  if      ( (GetSensorType()!=kS)&&((TRMCFFKEY.BelauShift%10)!=0)&&(charge!=0) ) {
    double A[6] = {10,-4.36147e+00,4.00352e+00,-3.20887e+00,1.98220e-01,-8.12993e-01};
    double X = fabs(charge);
    if (X<A[0]) shift = A[1]+A[2]*X+A[3]*X*X+A[4]*X*X*X;
    else shift = A[1]+A[2]*A[0]+A[3]*A[0]*A[0]+A[4]*A[0]*A[0]*A[0]-A[5]*A[0]+A[5]*X;
    shift *= sensangle;
  }
  else if ( (GetSensorType()==kS)&&((int(TRMCFFKEY.BelauShift/10)%10)!=0)&&(charge!=0) ) {
    double A[6] = {10,-3.13740e+00,1.23269e+00,4.59550e-01,-2.06200e-02,5.99525e-01};
    double X = fabs(charge);
    if (X<A[0]) shift = A[1]+A[2]*X+A[3]*X*X+A[4]*X*X*X;
    else shift = A[1]+A[2]*A[0]+A[3]*A[0]*A[0]+A[4]*A[0]*A[0]*A[0]-A[5]*A[0]+A[5]*X; 
    shift *= sensangle;
  ...
  // substep evaluation 
  int nsteps = int(ceil(sensstep/TRMCFFKEY.BelauSubStep)); 
  if (nsteps>100) nsteps = 100; // not too many steps 
  if (nsteps<  1) nsteps =   1; // not 0 steps 
  double dstep  = sensstep/nsteps; // um
  // substepping limits
  double zmin   = sensheight - (sensstep/2)*cos(sensangle); // um
  double zmax   = sensheight + (sensstep/2)*cos(sensangle); // um
  double zstep  = (zmax-zmin)/nsteps; // um
  double xmin   = senscoo - (sensstep/2)*sin(sensangle); // um
  double xmax   = senscoo + (sensstep/2)*sin(sensangle); // um
  double xstep  = (xmax-xmin)/nsteps; // um
  if (VERBOSE) {
    printf("TrSimSensor::GetWeightBelau-V-Limits zmax(um):%7.1f zmin(um):%7.1f dz(um):%7.1f xmin(um):%7.1f xmax(um):%7.1f dx(um):%7.1f step(um):%7.2f substep(um):%7.2f nsubsteps:%2d\n",
      zmax,zmin,zmax-zmin,xmax,xmin,xmax-xmin,sensstep,dstep,nsteps
    );
  }
  // every sub-step should have this area (normalization)
  double weight = 1./nsteps;
  // sub-steps loop
  double value = 0;
  for (int isub=0; isub<nsteps; isub++) {
    double x = xmin + xstep*(isub+0.5); // um 
    double z = zmin + zstep*(isub+0.5); // um
    double y0 = (GetSensorType()==kS) ? 150-z : z+150; 
    // lorentz angle effect (proj*B*mu*rH)
    double dhall = y0*hallproj*TRMCFFKEY.BelauHallMobility[(GetSensorType()==kS)?1:0]*1e-4; // um 
    double mu = x + dhall + shift; // position (step, hall, shift)
    double sigma = 1e6*GetDiffusionWidth(fabs(y0*1e-6),charge); // diffusion 
    // gaussian integration using erf
    double X1 = (x1-mu)/(sigma*sqrt(2));
    double X2 = (x2-mu)/(sigma*sqrt(2));
    double integral = 0.5*fabs(TMath::Erf(X2)-TMath::Erf(X1));
    // substep fraction weighting 
    integral *= weight;
    // strip value update 
    value += integral;
    if (VERBOSE) {
      printf("TrSimSensor::GetWeightBelau-V-SubStep(%2d/%2d) type:%1d x(um):%7.1f z(um):%7.1f mu(um):%7.1f y0(um):%7.4f diff(um):%7.4f X1:%7.2f X2:%7.2f w:%7.5f int:%8.6f val:%8.6f\n",
        isub+1,nsteps,GetSensorType(),x,z,mu,y0,sigma,X1,X2,weight,integral,value
      );
    }
  }
  if (VERBOSE) {
    printf("TrSimSensor::GetWeightBelau-V-Ending coo(strip):%10.4f x1(strip):%7.2f x2(strip):%7.2f val:%8.6f\n",
      senscoo/GetImplantPitch(),x1/GetImplantPitch(),x2/GetImplantPitch(),value
    );
  }
  return value;
}
...
TrSimCluster TrSimSensor::MakeCluster(double senscoo, double sensangle, int nsens, double dzstep, double sensheight, double hallproj, int charge) { 
  TrSimCluster implclus = MakeImplantCluster(senscoo,sensangle,dzstep,sensheight,hallproj,charge); 
