//  $Id: trigger102.C,v 1.120 2018/06/22 04:56:57 choutko Exp $
...
//
Trigger2LVL1 *pl1= new Trigger2LVL1(PhysBPatt,JMembPatt,auxtrpat,toftrcode1,toftrcode2,tofpatt1,tofpatt2,antipatt,ectrigfl,ectrpatt,ectrsum,livetime,rates,trtime);//create lvl1trig-object
        AMSEvent::gethead()->addnext(AMSID("TriggerLVL1",0), pl1);
...
#pragma omp threadprivate(count)
    ...
    float d = 0.0f;
    Level1R l1r(pl1);
    int ph=l1r.RebuildTrigPatt(a,b,AMSEvent::gethead()->gettime());
    if((++count%100)==1 || ph){
       AMSEvent::gethead()->addnext(AMSID("TrigPhysLVL1",0), pl1);
       if(RNDM(d)<IOPA.Portion)AMSEvent::gethead()->addnext(AMSID("TrigPhysLVL1PS",0), pl1);
     }
   } 
