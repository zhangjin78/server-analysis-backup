//  $Id: trigger302.C,v 1.59 2016/05/24 11:21:11 choutko Exp $
...
void TriggerAuxLVL302::addnoisetk(integer crate){
  ...
  if(!AMSJob::gethead()->isReconstruction()){
	...
	if(strip >= TkDBc::Head->NStripsDrp(i))strip=TkDBc::Head->NStripsDrp(i)-1;
	TrLadCal * idd= TrCalDB::Head->FindCal_HwId(crate*100+drp);
	if(!idd) continue;
	int strip2=(i==0)?strip+640:strip;
	if(idd->Status(strip2)!=0){ 
	  if(maxtr-_ltr > 4){
	    _ptr[_ltr]=1;   // length + max strp
	    _ptr[_ltr+1]=strip2*10000+crate*100+drp;//idd.gethaddr();
	    _ptr[_ltr+2]=1000;
	    _ptr[_ltr+3]=1000;
	    _ltr+=4;
	  }    
...
void TriggerAuxLVL302::filltk(integer crate){
  ...
  while (ptr){
    _ltr+=ptr->lvl3format(_ptr+_ltr,maxtr-_ltr,LVL3FFKEY.SeedThr);
    ptr=ptr->next();
  }
...
integer TriggerLVL302::_TrackerAux[NTRHDRP][trconst::ncrt];
...
// geant TriggerLVL302::_TrackerCoo[NTRHDRP][trconst::ncrt][3];
// geant TriggerLVL302::_TrackerDir[NTRHDRP][trconst::ncrt];
// geant TriggerLVL302::_TrackerCooZ[maxlay];
// integer TriggerLVL302::_TrackerDRP2Layer[NTRHDRP][trconst::ncrt];
// number TriggerLVL302::_stripsize=0.0055;
 integer TriggerLVL302::_TrackerOtherTDR[NTRHDRP][trconst::ncrt];
//
...
TriggerLVL302::TriggerLVL302(bool tofin, bool trdin ):
  ...
  VZERO(_TrackerAux,trconst::ncrt*trconst::ntdr);
...
void TriggerLVL302::init(){
  ...
  _ECcrz3=TkDBc::Head->GetZlayer(8);//z-pos last trk-layer(X)(~ cm)
  _ECcrz4=TkDBc::Head->GetZlayer(8);//z-pos last trk-layer(Y)(~ cm)
  //-----------------   
...
//   // Tracker
  ...
  for(i=0;i<(trconst::ncrt*trconst::ntdr);i++){
    _TrackerStatus[i]=0;
  }


//   for (i=0;i<TkDBc::Head->nlay();i++)
//     _TrackerCooZ[i]=TkDBc::Head->GetZlayer(i+1);

//  //  for(i=0;i<trconst::ntdr;i++){
//     for( int k=0;k<trconst::ncrt;k++){
//       TrLadCal * id= TrCalDB::Head->FindCal_HwId(k*100+i);
...
// 	_TrackerDir[i][k]=0;
// 	int nfs=0;
// 	//	int half=TkDBc::Head->  id.gethalf(); 
// 	int a= half==0?0:TkDBc::Head->nhalf(id.getlayer(),id.getdrp());
// 	int b= half==0?TkDBc::Head->nhalf(id.getlayer(),id.getdrp()):
// 	  TkDBc::Head->nsen(id.getlayer(),id.getdrp());
// 	/*
// 	  if(a>=b){
// 	  // warning force dead
...
void TriggerLVL302::preparetracker(){
  ...
  for(int icase=0;icase<2;icase++){       
	...
	geant coo=coodown+(cooup-coodown)/(_TOFCoo[toftop[icase]][0][2]-_TOFCoo[tofbot[icase]][0][2])*
	  (TkDBc::Head->GetZlayer(i+1)-_TOFCoo[tofbot[icase]][0][2]);
	geant addmagfield=(icase==0?0.1:1)*LVL3FFKEY.TrTOFSearchReg*(1-fabs(TkDBc::Head->GetZlayer(i+1)-z12)/dz12);
	geant addtof=addtofup+(_TOFCoo[toftop[icase]][0][2]-TkDBc::Head->GetZlayer(i+1))/dz12/2*(addtofdown-addtofup);
	...
	TRDAux._Par[icase][2]=z;
      ...
      for(int i=0;i<TkDBc::Head->nlay();i++){
	geant coo=TRDAux._Par[icase][0]*(TkDBc::Head->GetZlayer(i+1)-TRDAux._Par[icase][2])+TRDAux._Par[icase][1];
	geant addtrd=fabs(3*TRDAux._Par[icase][3]*(TkDBc::Head->GetZlayer(i+1)-TRDAux._Par[icase][2]));
	geant addmagfield=(icase==0?0.2:2)*LVL3FFKEY.TrTOFSearchReg*
	  fabs((TkDBc::Head->GetZlayer(i+1)-TkDBc::Head->GetZlayer(1))/
	       (TkDBc::Head->GetZlayer(TkDBc::Head->nlay())-TkDBc::Head->GetZlayer(1)));
...
#pragma omp critical (hf1)
{
	...
	while(ptr){
	  ...
	  if(side != 0 && (LVL3FFKEY.NoK || _TrackerAux[drp][crate])){
	    ...
	    int tkid= TkDBc::Head->HwId2Tkid(crate*100+drp);
	    AMSPoint coo=TkCoo::GetGlobalAC(tkid,0,ss+0.5);

	    //       cout << "Trigger3  s/n*8 "<<((*((int16u*)ptr)>>6)&63)<<" lay "<<layer<<" strip "<<strip<<" coo "<<coo<<endl;
	    if(AMSFFKEY.Debug){
	      cout << "Trigger3  s/n*8 "<<((*((int16u*)ptr)>>6)&63)<<" lay "<<layer<<" strip "<<strip<<" coo "<<coo[1]<<endl;
	    }
	    if (coo[1] > plvl3->getlowlimitY(layer) && coo[1] < plvl3->getupperlimitY(layer)){
	      //           cout << "added  y ok "<<endl;
	      if((coo[0]-0.1 <0 && plvl3->getlowlimitX(layer) < 0) ||
		 (coo[0]+0.1>0 && plvl3->getupperlimitX(layer) > 0) ){
		//           cout << "added ok "<<endl;
		if(!(plvl3->addnewhit(coo[1],amp,layer,drp+crate*NTRHDRP))){
...
void TriggerLVL302::fit(integer idum){
  ...
  if(_NTrHits < 3)ic=TkDBc::Head->nlay()-2;
  else if (_NTrHits > TkDBc::Head->nlay())ic=0;
  ...
  if( _NPatFound  == 0){
    int nht=0;
    for(int i=0;i<TkDBc::Head->nlay();i++)if(_nhits[i])nht++;
    if(nht>TkDBc::Head->nlay()/2 - (ula-lla==TkDBc::Head->nlay()-1?1:0))_TrackerTrigger=5;
