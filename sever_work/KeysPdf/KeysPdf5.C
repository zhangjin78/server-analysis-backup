...
int KeysPdf5(const string fver, const int dstver=62, const int run_charge=8, const int StoreRig=0, const int QVer=1, const bool bTest=false)
{
	...
	string sHomeDir;
	#ifdef ATLAS
	sHomeDir = "/atlas/users/chenya/result";
	#elif defined BAOBAB
	sHomeDir = "/home/chenyao/result";
	#elif defined LXPLUS
	sHomeDir = "/afs/cern.ch/user/c/chenya/result"; //XXX
	#endif
	string outdir=Form("%s/analysis6/amsd%dn_B1130_%s/total%s", sHomeDir.c_str(), dstver, fver.c_str(), ((bTest)?"_test":""));
	cout << outdir << endl;
	...
	for (int ixy=XY0; ixy<nXY; ixy++)
	{
		...
		rrvQ[ixy] = new RooRealVar(histn.c_str(), histn.c_str(), 0, 35);
	} //ixy
		...
		//for (int ir=1; ir<3; ir++)
		{
			...
			for (int ich=Charge; ich<=35; ich++)
			{
				for (int ixy=XY0; ixy<nXY; ixy++)
				{
					...
					//--plot pdf
					rkpLQTemp->plotOn(rpQFrame, RooFit::DrawOption("PL"));
