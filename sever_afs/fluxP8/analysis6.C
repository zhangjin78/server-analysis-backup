...
int analysis(TChain &ch, string odir, string ofile, long int nrun=-1, const int iGeoSafety=1)
{
	...
	if (!ev->isreal)
	{
		...
		histn = Form("/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/amsd68n_MC_TOTALGENEV/amsd%dn_%s%d/total_raw_EvenLogBin.root", dstver, sElementShort[MCQ-1].c_str(), MCMass);
		// histn = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/amsd68n_MC_TOTALGENEV/amsd68n_F19/total_raw_EvenLogBin.root";
		// histn = "/afs/cern.ch/work/j/jzhang2/AMS/analisiP8/amsd68n_MC_TOTALGENEV/amsd68n_Si28/total_raw_EvenLogBin.root";
		...
		for (int ir=0; ir<iSpanU; ir++) //2022.06.26
		{
			...
			else
			{
				...
				if( (ev->mch >= 1 && ev->mch <= 20) || (ev->mch == 26)){
					// histn = "/afs/cern.ch/work/j/jzhang2/AMS/Flux/hzbin/fluxfit_amsd68n_q14_20230304";
					string fluxmodle_dir = "/afs/cern.ch/work/j/jzhang2/AMS/plot/SurvivalProb/FluxCalculation/Input/FluxFit/SplineFit";
		...
		for (it=viQSel.begin(); it!=viQSel.end(); it++)
		{
			...
			for (int ir=2; ir<3; ir++){
				...
				for (int iperiod=0; iperiod<3; iperiod++) {//iperiod=2: sum of two trigger periods
					...
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), iNbin, pBins, i1DBin/10, iQDL, iQDU));
					histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALmip_RigCut0", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), iNbin, pBins, i1DBin/10, iQDL, iQDU));
					histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip_RigCut0", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), iNbin, pBins, i1DBin/10, iQDL, iQDU));
					histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), iNbin, pBins, i1DBin/10, iQDL, iQDU));
					histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_ECALmip_RigCut0_ChisqCut_Sel", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), iNbin, pBins, i1DBin/10, iQDL, iQDU));
					...
					histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip_purity", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), 140, -35, 35, 140, -35, 35));
					histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), 140, -35, 35, 140, -35, 35));
					//----
					histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALmip_poscut", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), iNbin, pBins, i1DBin/10, iQDL, iQDU));
					histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip_poscut", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), iNbin, pBins, i1DBin/10, iQDL, iQDU));
					histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALmip_poscut", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), 140, -35, 35, 140, -35, 35));
					histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip_purity_poscut", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), 140, -35, 35, 140, -35, 35));
					histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip_poscut", ir, *it, iperiod);
					hmanQ[iQVer].Add(new TH2F(histn.c_str(), histn.c_str(), 140, -35, 35, 140, -35, 35));
	...
	// (int ievent=2500000; ievent<nrun2; ievent++)
	{
		...
		#ifdef MCSUR /// geometrical factor
		for (int iQVer=QVeri; iQVer<nQVer; iQVer++){
			if (ev->isreal) break;
			...
			for (int ir=1; ir<3; ir++){
				for (it=viQSel.begin(); it!=viQSel.end(); it++){
					...
					histn = Form("rig_r%d_q%d_GeoFactor_Trrige%d_Sam_nopre_gen",  ir, *it, mctrigger);
					GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch);
					histn = Form("rig_r%d_q%d_GeoFactor_Trrige%d_Sam_nopre_MCreweight_gen",  ir, *it, mctrigger);
		...
		for (int ir=1; ir<2; ir++){
			...
			for (it=viQSel.begin(); it!=viQSel.end(); it++){
				...
				for (int iQVer=QVeri; iQVer<nQVer; iQVer++){
					...
					dInnerQ = ev->GetTkInQNew(2, iQVer);
					double dUTofQ = ev->GetSubDetQ(1, iQVer);
					double dTOFbeta = ev->tof_betah;
					...
					for (int iperiod=0; iperiod<2; iperiod++){
						...
						for (int itofqcut=0; itofqcut<4; itofqcut++){
							...
							if(itofqcut > 0) continue; // 2024.05.06
							...
							for (int isel=-1; isel<iNTighten; isel++){
								...
								bool bL1QSel=true;
								if (isel==-1){
									if (!ev->isreal) bL1QSel = (dL1Q>*it-1.5);
									else bL1QSel = (dL1Q>*it-0.5) && (dL1Q<*it+0.5);
								}
								...
								if(bL1QSel && !ev->isreal && ev->mtrpri[0]){
								// if(bL1QSel && !ev->isreal){
									if(ev->mtrpri[1]){
										histn = Form("rig_r%d_q%d_L1L2Sur_noSel_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
										histn = Form("rig_r%d_q%d_L1L2Sur_noSel_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
										histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noSel_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
										histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noSel_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);

										bool bSurSel = false;

										bSurSel = bRun && bRTI && bRTIdL1L9;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bRun_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bRun_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bRun_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bRun_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && bTrig;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);

											if(dInnerQ == 0){
												// cout << "\t====> Find Events with Inner charge 0" << endl;
												// cout << "\tRun: " << ev->run << "\tEvent No." << ev->event << endl;
												histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_InnerqQ0_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
												...
												histn = Form("UTOFQvsRig_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_InnerqQ0_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dUTofQ, ww[ir]);
												histn = Form("InnerHitvsRig_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_InnerqQ0_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInNHit(), ww[ir]);
												histn = Form("InnerHitvsRig_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_InnerqQ0_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetTkInNHit(), ww[ir]);
												for(int il = 2; il <8+1; il++){
													histn = Form("L%dPosXY_r%d_q%d_L1L2Sur_bTrig_noL1L2Frag_InnerqQ0_period%d_tofqcut%d%s", il, ir, *it, iperiod, itofqcut, sTighten.c_str());
													GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrcoo[il-1][0], ev->mtrcoo[il-1][1], ww[ir]);
												}
											}
										}

										bSurSel = bSurSel && bPart;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bPart_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bPart_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bPart_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bPart_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && bBeta;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bBeta_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bBeta_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bBeta_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bBeta_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && (ev->tk_exqls[0]&0x10013D)==0;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bL1Qstatus_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bL1Qstatus_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bL1Qstatus_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bL1Qstatus_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && (ev->GetUBExtQTrkCor(0, 0, iQVer)>0 && ev->GetUBExtQTrkCor(0, 1, iQVer)>0);
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bL1Qxy_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bL1Qxy_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bL1Qxy_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bL1Qxy_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
										
										int mnhitf=0;
										for(int il=0+1;il<9-1;il++) {
											if(ev->IsPassTkL(il)){mnhitf++;}
										}
										bSurSel = bSurSel && (mnhitf>=5);
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bmnhitf5_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bmnhitf5_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bmnhitf5_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bmnhitf5_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
										
										bool geomcut = false;
										geomcut = ev->IsPassTkL(0) && ev->IsPassTkL(1);
										geomcut = geomcut && (ev->IsPassTkL(2) || ev->IsPassTkL(3));
										geomcut = geomcut && (ev->IsPassTkL(4) || ev->IsPassTkL(5));
										geomcut = geomcut && (ev->IsPassTkL(6) || ev->IsPassTkL(7));
										bSurSel = bSurSel && geomcut;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bgeomcut_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bgeomcut_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bgeomcut_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bgeomcut_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && (ev->GetTkInNHit()>=5);
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerHits_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerHits_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerHits_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerHits_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
										...
										bool innerHitYP = false;
										innerHitYP = l2H && l34H && l56H && l78H;
										bSurSel = bSurSel && innerHitYP;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerHitYP_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerHitYP_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerHitYP_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerHitYP_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
										
										bSurSel = bSurSel && (ev->GetChis(0,1)<10);
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerChisq_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerChisq_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerChisq_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerChisq_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
											cout << "\t====> Find Events with ALL selections" << endl;
											cout << "\tRun: " << ev->run << "\tEvent No." << ev->event << endl;
										}
									}
									else{
										histn = Form("rig_r%d_q%d_L1L2Sur_noSel_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
										histn = Form("rig_r%d_q%d_L1L2Sur_noSel_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
										histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noSel_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
										histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noSel_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										...
										bSurSel = bRun && bRTI && bRTIdL1L9;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bRun_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bRun_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bRun_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bRun_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && bTrig;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bTrig_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bTrig_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bTrig_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bTrig_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && bPart;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bPart_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bPart_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bPart_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bPart_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && bBeta;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bBeta_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bBeta_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bBeta_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bBeta_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && (ev->tk_exqls[0]&0x10013D)==0;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bL1Qstatus_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bL1Qstatus_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bL1Qstatus_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bL1Qstatus_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && (ev->GetUBExtQTrkCor(0, 0, iQVer)>0 && ev->GetUBExtQTrkCor(0, 1, iQVer)>0);
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bL1Qxy_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bL1Qxy_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bL1Qxy_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bL1Qxy_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
										...
										bSurSel = bSurSel && (mnhitf>=5);
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bmnhitf5_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bmnhitf5_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bmnhitf5_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bmnhitf5_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
										...
										bSurSel = bSurSel && geomcut;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_bgeomcut_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_bgeomcut_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bgeomcut_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_bgeomcut_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}

										bSurSel = bSurSel && (ev->GetTkInNHit()>=5);
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerHits_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerHits_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerHits_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerHits_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
										...
										bSurSel = bSurSel && innerHitYP;
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerHitYP_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerHitYP_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerHitYP_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerHitYP_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
										
										bSurSel = bSurSel && (ev->GetChis(0,1)<10);
										if(bSurSel){
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerChisq_L1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
											histn = Form("rig_r%d_q%d_L1L2Sur_binnerChisq_L1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerChisq_L1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, ww[ir]);
											histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_binnerChisq_L1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, ww[ir]);
										}
									}
								}
		...
		// if (bTrig && bRun && bRTI && bRTIdL1L9) ///// 2023.02.26 test Trig for MC
		if (bRun && bRTI && bRTIdL1L9)
		{
			...
			if (bPart && bBeta)
			{
				...
				for (int ir=1; ir<2; ir++)
				{
					...
					// if (ev->Select_Rig(1, icffv, iReferSpan) && bTkgeom[ir] && (ev->tk_exqls[0]&0x10013D)==0) //delete Inner chisqY cut #2025.06.20
					// if (bRig[ir][0] && ev->Select_TrTrack(iReferSpan) && bTkgeom[ir] && (ev->tk_exqls[0]&0x10013D)==0) //2024.04.16
					#else
					iReferSpan=ir;
					...
					#endif //L1L2SURNEW
					{
						...
						for (it=viQSel.begin(); it!=viQSel.end(); it++)
						{
							...
							for (int iQVer=QVeri; iQVer<nQVer; iQVer++)
							{
								...
								#ifdef L1L2SURNEW
								///Beta Cor
								// dL1Q=ev->GetUBExtQBetaCor(0, 2, iQVer); /// 2024.03.17
								// if (!(ev->GetUBExtQBetaCor(0, 0, iQVer)>0 && ev->GetUBExtQBetaCor(0, 1, iQVer)>0)) continue; //L1XY
								// if(ev->tk_l1qvs < 1.5) continue; // 2024.04.01
								// if(ev->tk_q[1] < 1.5) continue; // 2024.04.01
								///Trk Cor
								dL1Q=ev->GetUBExtQTrkCor(0, 2, iQVer);
								...
								//// L1 Pick Up 2024.04.07
								// bool bL1PU = ev->GetChis(1, 1, iReferSpan) < 10 && (ev->HasTkLHitXY(0)==2) && ((ev->tk_qls[0]&0x10013D)==0);
								// bool bL1PU = (ev->tk_lid[0] == ev->tk_exlid[0]) && (ev->tk_cad[0] == ev->tk_excad[0]);
								// bool bL1PU = (ev->tk_lid[0] == ev->tk_exlid[0]);
								...
								for (int iperiod=0; iperiod<2; iperiod++)
								{
									...
									#ifdef SAVETREE
									// if(ev->GetRigidity(iReferSpan) > 18 && ev->GetRigidity(iReferSpan) < 22.8){ //Rig 20
									// if(ev->GetRigidity(iReferSpan) > 2.15 && ev->GetRigidity(iReferSpan) < 2.67){  //Rig 6
									// 	SelVarTree(hmanQ[iQVer], *ev, outfile, *it, iQVer, ww0);
									// }
									SelVarTree(hmanQ[iQVer], *ev, outfile, *it, iQVer, ww0);
									continue;
									#endif //SAVETREE
									...
									for (int itofqcut=0; itofqcut<4; itofqcut++)
									{
										...
										//detet InnerQ chisq cut #2025.06.20
										bool bTracker = false;
										bTracker = ev->GetTkInNHit()>=5;
										bool l2H = (ev->HasTkLHitXY(1)>0);
										bool l34H = ((ev->HasTkLHitXY(2)>0)||(ev->HasTkLHitXY(3)>0));
										bool l56H = ((ev->HasTkLHitXY(4)>0)||(ev->HasTkLHitXY(5)>0));
										bool l78H = ((ev->HasTkLHitXY(6)>0)||(ev->HasTkLHitXY(7)>0));
										bool innerHitYP = false;
										innerHitYP = l2H && l34H && l56H && l78H;
										bTracker = bTracker && innerHitYP;
										if(!bTracker) continue; //2025.06.20
										...
										GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
										histn = Form("Beta_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										GetHist(hmanQ[iQVer], histn.c_str())->Fill(dTOFbeta, ww[ir]);
										histn = Form("Beta_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_NoReweight", ir, *it, iperiod, itofqcut);
										GetHist(hmanQ[iQVer], histn.c_str())->Fill(dTOFbeta);
										
										//----L1Q distribution vs Rig
										histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										if(ev->tk_lid[0]!=+811) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]); //2024.05.12

										//--- L1 non-fragemented
										histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_noAboveL1BK_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										if(ev->PrimaryUpToTrLayer(1)) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]); //2025.01.13

										histn = Form("L1QvsBeta_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dTOFbeta, dL1Q, ww[ir]);

										histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_BetaCor_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										if((ev->GetUBExtQBetaCor(0, 0, iQVer)>0 && ev->GetUBExtQBetaCor(0, 1, iQVer)>0) && ev->tk_l1qvs > 1.5) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetUBExtQBetaCor(0, 2, iQVer), ww[ir]);

										///--- Different selection on L1 Q
										/// 3. only for *it for L1Inner
										bool bSel1 = (ev->ntrack==1 || !ev->Select_Tk2ndRecon());
										bool bSel2 = true; //LTOF odd charge
										bool bSel3 = true; //TOF s0-s1
										bool bSel4 = true; //Inner Charge
										bool bSel5 = true; //UTOF Charge
										bool bSel6 = ev->tk_lid[0]!=+811; ///2024.05.06
										if(*it%2==1 || *it>20){
											if(*it%2==1) bSel2 = bSel2 && (fabs(ev->GetTOFUDQ(1)-*it)<0.5);
											else bSel2 = bSel2 && ev->GetTOFUDQ(1)>*it-0.5 && ev->GetTOFUDQ(1)<*it+1.5;
											bSel2 = bSel2 && (fabs(ev->GetTofLQ(2)-ev->GetTofLQ(3))<fabs(*it-ev->GetQSelRange(0, *it, 1, iQVer)));
											bSel2 = bSel2 && ev->IsGoodTofQUD(0) && ev->IsGoodTofQUD(1);
										}
										// bSel3 = bSel3 && (fabs(ev->GetTofLQ(2)-ev->GetTofLQ(2+1))<fabs(*it-ev->GetQSelRange(0, *it, 1, iQVer)));
										bSel3 = bSel3 && (fabs(ev->GetTofLQ(0)-ev->GetTofLQ(0+1))<fabs(*it-ev->GetQSelRange(0, *it, 1, iQVer)));
										bSel4 = bSel4 && (fabs(ev->GetSubDetQ(0,iQVer)-*it)<0.5);
										bSel5 = bSel5 && (fabs(ev->GetTOFUDQ(0)-*it)<0.5);
										// if(*it%2==1) bSel6 = bSel6 && (fabs(ev->GetTOFUDQ(1)-*it)<0.5);
										// else bSel6 = bSel6 && ev->GetTOFUDQ(1)>*it-0.5 && ev->GetTOFUDQ(1)<*it+1.5;
										// bSel7 = bSel7 && (fabs(ev->GetSubDetQ(5,iQVer)-*it)<0.5);

										// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_Sel1", ir, *it, iperiod, itofqcut);
										// if(bSel1) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_Sel2", ir, *it, iperiod, itofqcut);
										// if(bSel2) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_Sel3", ir, *it, iperiod, itofqcut);
										// if(bSel3) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_Sel4", ir, *it, iperiod, itofqcut);
										// if(bSel4) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_Sel5", ir, *it, iperiod, itofqcut);
										// if(bSel5) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_Sel6", ir, *it, iperiod, itofqcut);
										// if(bSel6) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_Sel7", ir, *it, iperiod, itofqcut);
										// if(bSel7) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_TotalSel", ir, *it, iperiod, itofqcut);
										// if(bSel1 && bSel2 && bSel3 && bSel4 && bSel5 && bSel6) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										...
										//----L1Q vs. InnerQ
										double arig1 = ev->GetRigidity(iReferSpan);
										// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// if(arig1 > 2.4 && arig1 < 2.67) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
										// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_4p43T4p88", ir, *it, iperiod, itofqcut);
										// if(arig1 > 4.43 && arig1 < 4.88) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
										// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_38p9T41p9", ir, *it, iperiod, itofqcut);
										// if(arig1 > 38.9 && arig1 < 41.9) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);

										//---for L1Q Beta Cor 2024.03.31
										// if(dL1Q < 0.1 && dInnerQ > 0.1){
										// 	histn = Form("TK_l1qvs_rig_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// 	GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_l1qvs, ww[ir]);
										// 	histn = Form("TK_l1qvr_rig_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// 	GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_l1qvr, ww[ir]);
										// 	histn = Form("TK_l1qvs_rig_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_NoMCreweight", ir, *it, iperiod, itofqcut);
										// 	GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_l1qvs);
										// 	histn = Form("TK_l1qvr_rig_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_NoMCreweight", ir, *it, iperiod, itofqcut);
										// 	GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_l1qvr);
										// 	if(arig1 > 2.4 && arig1 < 2.67){
										// 		histn = Form("TK_l1qvsvsInnerQ_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dInnerQ, ev->tk_l1qvs, ww[ir]);
										// 		histn = Form("TK_l1qvrvsInnerQ_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dInnerQ, ev->tk_l1qvr, ww[ir]);
										// 		histn = Form("TK_l1qvsvsL1QTrkCor_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetUBExtQTrkCor(0, 2, iQVer), ev->tk_l1qvs, ww[ir]);
										// 		histn = Form("TK_l1qvrvsL1QTrkCor_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetUBExtQTrkCor(0, 2, iQVer), ev->tk_l1qvr, ww[ir]);
										// 		histn = Form("TK_posL1_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_pos[0][0], ev->tk_pos[0][1], ww[ir]);
										// 		histn = Form("TK_pos1sL1_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_pos1s[0][0], ev->tk_pos1s[0][1], ww[ir]);
										// 		for(int ilt = 0; ilt < 4; ilt++){
										// 			histn = Form("TOF_posS%d_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ilt, ir, *it, iperiod, itofqcut);
										// 			GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tof_pos[ilt][0], ev->tk_pos[ilt][1], ww[ir]);
										// 			histn = Form("TOF_oqMaxvsInnerQ_S%d_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ilt, ir, *it, iperiod, itofqcut);
										// 			GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dInnerQ, ev->tof_oq[ilt][0], ww[ir]);
										// 			histn = Form("TOF_oqMaxvsInnerQ_S%d_r%d_q%d_L1L2Sur_UBL1QBetaCor0_period%d_tofqcut%d_2p4T2p67", ilt, ir, *it, iperiod, itofqcut);
										// 			GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dInnerQ, ev->tof_oq[ilt][1], ww[ir]);
										// 		}
										// 	}
										// }
										// if(dL1Q > 0.1 && dInnerQ < 0.1){
										// 	if(arig1 > 2.4 && arig1 < 2.67){
										// 		histn = Form("TK_l1qvsvsTKQ_r%d_q%d_L1L2Sur_InnerQ0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_q[1], ev->tk_l1qvs, ww[ir]);
										// 		histn = Form("TK_l1qvsvsL1QBetaCor_r%d_q%d_L1L2Sur_InnerQ0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetUBExtQBetaCor(0, 2, iQVer), ev->tk_l1qvs, ww[ir]);
										// 		histn = Form("TK_posL1_r%d_q%d_L1L2Sur_InnerQ0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_pos[0][0], ev->tk_pos[0][1], ww[ir]);
										// 		histn = Form("TK_pos1sL1_r%d_q%d_L1L2Sur_InnerQ0_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_pos1s[0][0], ev->tk_pos1s[0][1], ww[ir]);
										...
										//---for L1Q distribution 2024.04.07
										// if(dInnerQ > (*it-0.5) && dInnerQ < (*it+0.5)){
										// 	histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_NoFrag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// 	if(bL1PU){
										// 		histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_NoFrag_L1PU_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// 	}else{
										// 		histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_NoFrag_noL1PU_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// 	}
										// }
										// if(dInnerQ < (*it-0.5)){
										// 	histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// 	if(bL1PU){
										// 		histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_Frag_L1PU_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// 	}else{
										// 		histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_Frag_noL1PU_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
										// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										// 	}
										// }

										//--- Addd Inner cut for Mn
										if(dInnerQ > (*it-0.5) && dInnerQ < (*it+0.5)){
											histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_InnerQcut_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										}
										if(dInnerQ > (*it-0.25) && dInnerQ < (*it+0.25)){
											histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_InnerQcut2_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
										}
										...
										if (!ev->isreal)
										{
											...
											//----L1Q vs. InnerQ
											double arig1 = ev->mmom/ev->mch;
											// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_2p4T2p67", ir, *it, iperiod, itofqcut);
											// if(arig1 > 2.4 && arig1 < 2.67) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
											// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_4p43T4p88", ir, *it, iperiod, itofqcut);
											// if(arig1 > 4.43 && arig1 < 4.88) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
											// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_38p9T41p9", ir, *it, iperiod, itofqcut);
											// if(arig1 > 38.9 && arig1 < 41.9) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);

											//---for MC truth in fragmentation 2025.02.04
											// if(!ev->PrimaryUpToTrLayer(1)){
											if(!ev->mtrpri[0]){  //#2025.02.21
												int dL1Z = ev->GetTrMCLZ(1);
												// histn = Form("L1ZvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1Frag", ir, *it, iperiod, itofqcut);
												// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Z, ww[ir]);
												// histn = Form("L1ZvsRig_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag", ir, *it, iperiod, itofqcut);
												// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dL1Z, ww[ir]);
												if(arig1 >  2.4 && arig1 < 2.67){
													// histn = Form("L1PaticleIDvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, ev->mtrpar[0], ww[ir]);
													// histn = Form("L1QvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// histn = Form("L1QvsL1PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[0], dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// if(dL1Q>*it-1.5){
													// 	histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1Frag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1Frag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// }
												}
												if(arig1 > 4.43 && arig1 < 4.88){
													// histn = Form("L1PaticleIDvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, ev->mtrpar[0], ww[ir]);
													// histn = Form("L1QvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// histn = Form("L1QvsL1PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[0], dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// if(dL1Q>*it-1.5){
													// 	histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1Frag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1Frag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// }
												}
												if(arig1 > 38.9 && arig1 < 41.9){
													// histn = Form("L1PaticleIDvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, ev->mtrpar[0], ww[ir]);
													// histn = Form("L1QvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// histn = Form("L1QvsL1PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[0], dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1Frag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// if(dL1Q>*it-1.5){
													// 	histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1Frag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1Frag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// }
												}

												if(ev->mtrpar[0] == 77){
													// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1Frag_L1PID77", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
													// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_GenRig_noL1Qcut_period%d_tofqcut%d_AboveL1Frag_L1PID77", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dL1Q, ww[ir]);
													// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1Frag_L1PID77", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_GenRig_noL1Qcut_period%d_tofqcut%d_AboveL1Frag_L1PID77", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetTkInQNew(2, iQVer), ww[ir]);
													// if(ev->mtrpar[1] == 76){
													// 	histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1Frag_L1PID77_L2PID76", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
													// 	histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_GenRig_noL1Qcut_period%d_tofqcut%d_AboveL1Frag_L1PID77_L2PID76", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dL1Q, ww[ir]);
													// 	histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1Frag_L1PID77_L2PID76", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_GenRig_noL1Qcut_period%d_tofqcut%d_AboveL1Frag_L1PID77_L2PID76", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	if(dL1Q>*it-1.5){
													// 		histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_period%d_tofqcut%d_AboveL1Frag_L1PID77_L2PID76", ir, *it, iperiod, itofqcut);
													// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 		histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1Frag_L1PID77_L2PID76", ir, *it, iperiod, itofqcut);
													// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	}
													// }
												}
												...
												int dL1Z = ev->GetTrMCLZ(1);
												// histn = Form("L1ZvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1NoFrag", ir, *it, iperiod, itofqcut);
												// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Z, ww[ir]);
												// histn = Form("L1ZvsRig_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag", ir, *it, iperiod, itofqcut);
												// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dL1Z, ww[ir]);
												if(arig1 >  2.4 && arig1 < 2.67){
													// histn = Form("L1PaticleIDvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, ev->mtrpar[0], ww[ir]);
													// histn = Form("L1QvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// histn = Form("L1QvsL1PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[0], dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// if(dL1Q>*it-1.5){
													// 	histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// }
													if(ev->mtrpar[0] == 77 && (ev->mtrpar[1] == 77) ){
														// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_L1PID77_L2PID77_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
														// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
													}
													if(ev->mtrpar[0] == 77 && (ev->mtrpar[1] > 11600 && ev->mtrpar[1] < 11700) ){
														// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_L1PID77_L2IOS16_AboveL1NoFrag_2p4T2p67", ir, *it, iperiod, itofqcut);
														// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
													}
												}
												if(arig1 > 4.43 && arig1 < 4.88){
													// histn = Form("L1PaticleIDvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, ev->mtrpar[0], ww[ir]);
													// histn = Form("L1QvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// histn = Form("L1QvsL1PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[0], dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// if(dL1Q>*it-1.5){
													// 	histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// }
													if(ev->mtrpar[0] == 77 && (ev->mtrpar[1] == 77) ){
														// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_L1PID77_L2PID77_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
														// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
													}
													if(ev->mtrpar[0] == 77 && (ev->mtrpar[1] > 11600 && ev->mtrpar[1] < 11700) ){
														// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_L1PID77_L2IOS16_AboveL1NoFrag_4p43T4p88", ir, *it, iperiod, itofqcut);
														// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
													}
												}
												if(arig1 > 38.9 && arig1 < 41.9){
													// histn = Form("L1PaticleIDvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, ev->mtrpar[0], ww[ir]);
													// histn = Form("L1QvsL1Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dL1Z, dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// histn = Form("L1QvsL1PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[0], dL1Q, ww[ir]);
													// histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_noL1Qcut_GenRig_period%d_tofqcut%d_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// if(dL1Q>*it-1.5){
													// 	histn = Form("InnerQvsL2PID_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mtrpar[1], ev->GetTkInQNew(2, iQVer), ww[ir]);
													// 	histn = Form("InnerQvsL2Z_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
													// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTrMCLZ(2), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// }
													if(ev->mtrpar[0] == 77 && (ev->mtrpar[1] == 77) ){
														// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_L1PID77_L2PID77_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
														// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
													}
													if(ev->mtrpar[0] == 77 && (ev->mtrpar[1] > 11600 && ev->mtrpar[1] < 11700) ){
														// histn = Form("L1QvsInnerQ_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_L1PID77_L2IOS16_AboveL1NoFrag_38p9T41p9", ir, *it, iperiod, itofqcut);
														// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetTkInQNew(2, iQVer), dL1Q, ww[ir]);
													}
												}

												if(ev->mtrpar[0] == 77){
													// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1NoFrag_L1PID77", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
													// histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_GenRig_noL1Qcut_period%d_tofqcut%d_AboveL1NoFrag_L1PID77", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dL1Q, ww[ir]);
													// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1NoFrag_L1PID77", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
													// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_GenRig_noL1Qcut_period%d_tofqcut%d_AboveL1NoFrag_L1PID77", ir, *it, iperiod, itofqcut);
													// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetTkInQNew(2, iQVer), ww[ir]);
													if(ev->mtrpar[1] == 47){
														// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1NoFrag_L1PID77_L2PID47", ir, *it, iperiod, itofqcut);
														// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
														if(dL1Q>*it-1.5){
															// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_period%d_tofqcut%d_AboveL1NoFrag_L1PID77_L2PID47", ir, *it, iperiod, itofqcut);
															// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
														}
													}
													if(ev->mtrpar[1] == 49){
														// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1NoFrag_L1PID77_L2PID49", ir, *it, iperiod, itofqcut);
														// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
														if(dL1Q>*it-1.5){
															// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_period%d_tofqcut%d_AboveL1NoFrag_L1PID77_L2PID49", ir, *it, iperiod, itofqcut);
															// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
														}
													}
													for(int iInZ = 61; iInZ < 121; iInZ++){
													...
													for(int iZ = 2; iZ <= 16; iZ++){
														int ipid = 10000 + iZ *100;
														int ipid2 = 10000 + (iZ+1) *100;
														if((ev->mtrpar[1] > ipid && ev->mtrpar[1] < ipid2) ){
															// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noL1Qcut_period%d_tofqcut%d_AboveL1NoFrag_L1PID77_totISO%d", ir, *it, iperiod, itofqcut, iZ);
															// GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
															if(dL1Q>*it-1.5){
																// histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_period%d_tofqcut%d_AboveL1NoFrag_L1PID77_totISO%d", ir, *it, iperiod, itofqcut, iZ);
												...
												bool bL1PID = false;
												bool bL2PID = false;
												int ipid = 10000 + *it *100;
												int ipid2 = 10000 + (*it+1) *100;
												int L1PID_truth = ev->mtrpar[0];
												int L2PID_truth = ev->mtrpar[1];
												if(*it == 10){
													if(L1PID_truth == 71 || (L1PID_truth > ipid && L1PID_truth < ipid2)){
														bL1PID = true;
													}
													if(L2PID_truth == 71 || (L2PID_truth > ipid && L2PID_truth < ipid2)){
														bL2PID = true;
													}
												}
												else if(*it == 12){
													if(L1PID_truth == 73 || (L1PID_truth > ipid && L1PID_truth < ipid2)){
														bL1PID = true;
													}
													if(L2PID_truth == 73 || (L2PID_truth > ipid && L2PID_truth < ipid2)){
														bL2PID = true;
													}
												}
												else if(*it == 14){
													if(L1PID_truth == 75 || (L1PID_truth > ipid && L1PID_truth < ipid2)){
														bL1PID = true;
													}
													if(L2PID_truth == 75 || (L2PID_truth > ipid && L2PID_truth < ipid2)){
														bL2PID = true;
													}
												}
												else if(*it == 16){
													if(L1PID_truth == 77 || (L1PID_truth > ipid && L1PID_truth < ipid2)){
														bL1PID = true;
													}
													if(L2PID_truth == 77 || (L2PID_truth > ipid && L2PID_truth < ipid2)){
														bL2PID = true;
													}
												}
												if(bL1PID){
											...
											//---for L1Q distribution 2024.04.07
											// if(ev->PrimaryUpToTrLayer(2)){
											// 	histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_NoFrag_MCTruth_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
											// 	if(bL1PU){
											// 		histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_NoFrag_MCTruth_L1PU_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
											// 	}else{
											// 		histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_NoFrag_MCTruth_noL1PU_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
											// 	}
											// }
											// if(ev->PrimaryUpToTrLayer(1) && !ev->PrimaryUpToTrLayer(2)){
											// 	histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_Frag_MCTruth_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											// 	GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
											// 	if(bL1PU){
											// 		histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_Frag_MCTruth_L1PU_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
											// 	}else{
											// 		histn = Form("L1QvsRig_r%d_q%d_L1L2Sur_noL1Qcut_Frag_MCTruth_noL1PU_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
											// 		GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dL1Q, ww[ir]);
											// 	}
											// }
										}
										...
										for (int isel=-1; isel<iNTighten; isel++)
										{
											...
											// if (isel==-1) bL1QSel = (dL1Q>*it-1.5); // no up limit for MC 2024.03.26
											if (isel==-1){
												// if (!ev->isreal) bL1QSel = (dL1Q>(14-1.5)); //-- 2024.07.06
												// if (!ev->isreal) bL1QSel = (dL1Q>*it-0.5) && (dL1Q<*it+0.5); // 2024.07.23
												if (!ev->isreal) bL1QSel = (dL1Q>*it-1.5);
												// if (!ev->isreal) bL1QSel = (dL1Q>16-1.5) && (dL1Q<16+1.5); //-- 2025.01.13
												// if (!ev->isreal) bL1QSel = (dL1Q>16-0.5) && (dL1Q<16+0.5); //-- 2025.01.14
												// if (!ev->isreal) bL1QSel = (dL1Q>*it-1.); /// 2024.09.26
												// if (!ev->isreal) bL1QSel = (dL1Q>*it-0.25); /// 2024.09.26
												// if (!ev->isreal) bL1QSel = (dL1Q>*it-0.5); /// 2024.09.18
												else bL1QSel = (dL1Q>*it-0.5) && (dL1Q<*it+0.5);
												// else bL1QSel = (dL1Q>*it-0.3) && (dL1Q<*it+0.3); //2024.09.12
											}
											else if (isel==0) bL1QSel = (dL1Q>*it-0.25) && (dL1Q<*it+0.5);
											else if (isel==1) bL1QSel = (dL1Q>*it-0.5) && (dL1Q<*it+0.25);
											else if (isel==2) bL1QSel = (dL1Q>*it-0.25) && (dL1Q<*it+0.25);
											...
											if (bL1QSel)
											{
												...
												GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
												histn = Form("rig_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s_NoReweight", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan));
												histn = Form("Beta_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist(hmanQ[iQVer], histn.c_str())->Fill(dTOFbeta);
												histn = Form("Beta_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s_NoReweight", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist(hmanQ[iQVer], histn.c_str())->Fill(dTOFbeta);
												...
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
												histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s_NoReweight", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer));
												histn = Form("InnerQvsBeta_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dTOFbeta, ev->GetTkInQNew(2, iQVer), ww[ir]);
												histn = Form("InnerQvsBeta_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s_NoReweight", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(dTOFbeta, ev->GetTkInQNew(2, iQVer));
												histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s_NoAboveL1BK", ir, *it, iperiod, itofqcut, sTighten.c_str());
												if(ev->PrimaryUpToTrLayer(1)) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
												
												//----UTofQ distribution vs Rig
												histn = Form("UTofQvsRig_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUTofQ, ww[ir]);
												histn = Form("UTofQvsRig_r%d_q%d_L1L2Sur_period%d_tofqcut%d%s_NoReweight", ir, *it, iperiod, itofqcut, sTighten.c_str());
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUTofQ);

												//----InnerQ distribution vs Rig For non-Fragmentation #2024.09.22
												bool bNoFrag = true;
												bNoFrag = bNoFrag && (ev->ntrack==1 || !ev->Select_Tk2ndRecon());
												if(*it%2==1 || *it>20){
													if(*it%2==1) bNoFrag = bNoFrag && (fabs(ev->GetTOFUDQ(1)-*it)<0.5);
													else bNoFrag = bNoFrag && ev->GetTOFUDQ(1)>*it-0.5 && ev->GetTOFUDQ(1)<*it+1.5;
													bNoFrag = bNoFrag && (fabs(ev->GetTofLQ(2)-ev->GetTofLQ(3))<fabs(*it-ev->GetQSelRange(0, *it, 1, iQVer)));
													bNoFrag = bNoFrag && ev->IsGoodTofQUD(0) && ev->IsGoodTofQUD(1);
												}
												bNoFrag = (fabs(ev->GetTofLQ(2)-ev->GetTofLQ(2+1))<fabs(*it-ev->GetQSelRange(0, *it, 1, iQVer)));
												bNoFrag = (fabs(ev->GetTofLQ(0)-ev->GetTofLQ(0+1))<fabs(*it-ev->GetQSelRange(0, *it, 1, iQVer)));
												bNoFrag = (fabs(ev->GetTOFUDQ(0)-*it)<0.5);
												histn = Form("InnerQvsRig_r%d_q%d_NoFragCut_L1L2Sur_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
												if(bNoFrag) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
												...
												if (!ev->isreal)
												{
													...
													GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetTkInQNew(2, iQVer), ww[ir]);
													histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_GenRig_period%d_tofqcut%d%s_NoReweight", ir, *it, iperiod, itofqcut, sTighten.c_str());
													GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetTkInQNew(2, iQVer));
												}
											}
										} //isel
									} //itofqcut
				...
				for (int ir=1; ir<2; ir++){
					...
					//--
					int iReferSpan=(ir==1)?0:3; //using Inner rigidity for L1Inner,InnerL9 fo FullSpan
					if (ev->Select_Rig(1, icffv, iReferSpan) && (ev->tk_exqls[0]&0x10013D)==0){
						for (it=viQSel.begin(); it!=viQSel.end(); it++){
							if (!ev->isreal && *it!=ev->mch) continue;
							for (int iQVer=QVeri; iQVer<nQVer; iQVer++){
								if (iQVer%3<ChargeVersion || iQVer%3>ChargeVersion+nQVerStore-1) continue;
									...
									for (int itofqcut=0; itofqcut<4; itofqcut++){
										...
										if (!ev->isreal && ev->mtrpri[0]){
											if(ev->mtrpri[1]){
												...
												//--Tracker selection
												histn = Form("ChisqYvsRig_r%d_q%d_L1L2Sur_noL1Qcut_noTKTg_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetChis(0,1), ww[ir]);
												histn = Form("ChisqYvsRig_r%d_q%d_L1L2Sur_noL1Qcut_noTKTg_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetChis(0,1), ww[ir]);
												histn = Form("InnerHitvsRig_r%d_q%d_L1L2Sur_noL1Qcut_noTKTg_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInNHit(), ww[ir]);
												histn = Form("InnerHitvsRig_r%d_q%d_L1L2Sur_noL1Qcut_noTKTg_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ev->GetTkInNHit(), ww[ir]);
												...
												innerHitYP = l2H && l34H && l56H && l78H;
												histn = Form("rig_r%d_q%d_L1L2Sur_bHit_noL1Qcut_noTKTg_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												if(innerHitYP) GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
												histn = Form("rig_r%d_q%d_L1L2Sur_bHit_noL1Qcut_noTKTg_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												if(innerHitYP) GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);

												//--Fiduical volume selection
												int mnhitf=0;
												for(int il=0+1;il<9-1;il++) {
													if(ev->IsPassTkL(il)){mnhitf++;}
												}
												histn = Form("FiducialHitvsRig_r%d_q%d_L1L2Sur_noL1Qcut_noTKTg_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), mnhitf, ww[ir]);
												histn = Form("FiducialHitvsRig_r%d_q%d_L1L2Sur_noL1Qcut_noTKTg_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, mnhitf, ww[ir]);
												bool geomcut = false;
												geomcut = ev->IsPassTkL(0) && ev->IsPassTkL(1);
												geomcut = geomcut && (ev->IsPassTkL(2) || ev->IsPassTkL(3));
												geomcut = geomcut && (ev->IsPassTkL(4) || ev->IsPassTkL(5));
												geomcut = geomcut && (ev->IsPassTkL(6) || ev->IsPassTkL(7));
												histn = Form("rig_r%d_q%d_L1L2Sur_bFiduical_noL1Qcut_noTKTg_noL1L2Frag_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												if(geomcut) GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
												histn = Form("rig_r%d_q%d_L1L2Sur_bFiduical_noL1Qcut_noTKTg_noL1L2Frag_GenRig_period%d_tofqcut%d", ir, *it, iperiod, itofqcut);
												if(geomcut) GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
												...
												for (int isel=-1; isel<iNTighten; isel++){
													...
													if (bL1QSel && !ev->isreal){
														if(isel != -1) continue;
														histn = Form("rig_r%d_q%d_L1L2Sur_noTKTg_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
														GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ww[ir]);
														histn = Form("rig_r%d_q%d_L1L2Sur_noTKTg_noL1L2Frag_GenRig_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
														GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, ww[ir]);
														//----InnerQ distribution vs Rig
														histn = Form("InnerQvsRig_r%d_q%d_L1L2Sur_noTKTg_noL1L2Frag_period%d_tofqcut%d%s", ir, *it, iperiod, itofqcut, sTighten.c_str());
														GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetTkInQNew(2, iQVer), ww[ir]);
				...
				for(int ir=2; ir<3; ir++){
					if (ev->isreal && !bTrig) break;
					int iReferSpan = (ir==2)?1:-1; //L1Inner;
					if (ev->Select_Rig(1, icffv, iReferSpan) && ev->Select_TrTrack(iReferSpan) && bTkgeom[ir]){
						for (it=viQSel.begin(); it!=viQSel.end(); it++){
							#ifndef EFFSEC
							if (!ev->isreal && *it!=ev->mch) continue;
							...
							for (int iQVer=QVeri; iQVer<nQVer; iQVer++) {
								if (iQVer%3<ChargeVersion || iQVer%3>ChargeVersion+nQVerStore-1) continue;
								const int iTrigPeriod=ev->TriggerPeriod();
								double dInnerQ = ev->GetTkInQNew(2, iQVer);
								double dTOFbeta = ev->tof_betah;
								...
								for (int iperiod=0; iperiod<2; iperiod++){
									...
									// if (ev->Select_Charge(*it, iReferSpan, iQVer)){
									if (ev->Select_Charge(*it, iReferSpan, iQVer) && ev->Select_TofQ(*it, 3, iQVer)){ // 2025.07.29
										...
										histn = Form("InnerQvsRig_r%d_q%d_period%d_L8L9Sur_EventSel", ir, *it, iperiod);
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, dWeigh);
										histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_EventSel", ir, *it, iperiod);
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
										histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_EventSel_cut0", ir, *it, iperiod);
										if(dUnbiasedL9Q != 0) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
										histn = Form("InnerQvsRig_r%d_q%d_period%d_L8L9Sur_GenRig_EventSel", ir, *it, iperiod);
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, dWeigh);
										histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_GenRig_EventSel", ir, *it, iperiod);
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dUnbiasedL9Q, dWeigh);
										histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_GenRig_EventSel_cut0", ir, *it, iperiod);
										if(dUnbiasedL9Q != 0) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dUnbiasedL9Q, dWeigh);
										histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_EventSel_NorReweight", ir, *it, iperiod);
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q);
										histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_EventSel_cut0_NorReweight", ir, *it, iperiod);
										if(dUnbiasedL9Q != 0) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q);

										// if(dUnbiasedL9Q < 1 && dUnbiasedL9Q > 0){
										// 	cout << "\t====> Find Events with Unbiased L9 Q [0,1]" << endl;
										// 	cout << "\tRun: " << ev->run << "\tEvent No." << ev->event << endl;
										// }

										if(!ev->isreal && ev->mtrpri[7]){
											if(ev->mtrpri[8]){
												histn = Form("InnerQvsRig_r%d_q%d_period%d_L8L9Sur_NoL8L9Frag", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, dWeigh);
												histn = Form("InnerQvsRig_r%d_q%d_period%d_L8L9Sur_GenRig_NoL8L9Frag", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, dWeigh);
												histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_NoL8L9Frag", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
												histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_GenRig_NoL8L9Frag", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dUnbiasedL9Q, dWeigh);
												if(dUnbiasedL9Q < 2 && dUnbiasedL9Q > 0){
													// cout << "\t====> Find Events with Unbiased L9 Q [0,1]" << endl;
													// cout << "\tRun: " << ev->run << "\tEvent No." << ev->event << endl;
												}
											}
											else{
												histn = Form("InnerQvsRig_r%d_q%d_period%d_L8L9Sur_L8L9Frag", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, dWeigh);
												histn = Form("InnerQvsRig_r%d_q%d_period%d_L8L9Sur_GenRig_L8L9Frag", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dInnerQ, dWeigh);
												histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_L8L9Frag", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
												histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_GenRig_L8L9Frag", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch, dUnbiasedL9Q, dWeigh);
												// cout << "\tRigidity(FullSpan):" << ev->GetRigidity(2) << endl;
												// cout << "\tRigidityRatio(L1Inner/FullSpan):" << ev->GetRigidity(1) / ev->GetRigidity(2) << endl;
											}
										}

										//-- ECAL information
										histn = Form("RigRatiovsRig_r%d_q%d_period%d_L8L9Sur_EventSel", ir, *it, iperiod);
										GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetRigidity(1) / ev->GetRigidity(2), dWeigh);
										if(ev->GetRigidity(2) > 0){
											histn = Form("InnerQvsRig_r%d_q%d_period%d_L8L9Sur_EventSel_RigCut", ir, *it, iperiod);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dInnerQ, dWeigh);
											histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_EventSel_RigCut", ir, *it, iperiod);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
											...
											// cout << "\t===> Find Events with Rigidity(FullSpan) = 0: Run " << ev->run << " Event " << ev->event << " itrtrack " << ev->itrtrack << " ntrack " << ev->ntrack << " R_gbl " << ev->GetRigidity(2, 1) << endl;
										}
										// if( (ev->GetRigidity(2, 0) == 0 && ev->GetRigidity(2, 1) > 0) || (ev->GetRigidity(2, 1) == 0 && ev->GetRigidity(2, 0) > 0) ){
										// 	cout << "\t===> Find Events : Run " << ev->run << " Event " << ev->event << " R_chotko " << ev->GetRigidity(2, 0) << " R_gbl " << ev->GetRigidity(2, 1) << endl;
										// }
										if(ev->IsPassEcal2(0,7,0)){
											histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALfiducial", ir, *it, iperiod);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);

											histn = Form("ECalcellmaxvsRig_r%d_q%d_period%d_L8L9Sur_Sam", ir, *it, iperiod);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->ecal_icellmax[0], dWeigh);
											if(ev->ecal_icellmax[0]>=2 && ev->ecal_icellmax[0]<=69){
												histn = Form("ECalcellmaxvsRig_r%d_q%d_period%d_L8L9Sur_Sel", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->ecal_icellmax[0], dWeigh);
											}

											histn = Form("ECalcogvsRig_r%d_q%d_period%d_L8L9Sur_EventSel", ir, *it, iperiod);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->ecal_cog[0], dWeigh);
											double dis_ecal0 = fabs(ev->ecal_cog[0]-ev->ecal_icellmax[0]);
											histn = Form("ECaldisvsRig_r%d_q%d_period%d_L8L9Sur_Sam", ir, *it, iperiod);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dis_ecal0, dWeigh);
											// if(dis_ecal0 > 60 && ev->ecal_cog[0] != -1 && ev->ecal_icellmax[0] != 9999){
											// 	cout << "\t Dis need more larger range !!" << endl;
											// 	cout << "\t" << ev->ecal_cog[0] << "\t" << ev->ecal_icellmax[0] << "\t" << dis_ecal0 << endl;
											// }
											if(dis_ecal0 < 2){
												histn = Form("ECaldisvsRig_r%d_q%d_period%d_L8L9Sur_Sel", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dis_ecal0, dWeigh);
											...
											double ratio_ecal0 = ev->ecal_S3[0]/ev->ecal_S5[0];
											histn = Form("ECals3s5vsRig_r%d_q%d_period%d_L8L9Sur_Sam", ir, *it, iperiod);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ratio_ecal0, dWeigh);
											if(ev->ecal_S5[0] > 0 && ratio_ecal0 > 0.98){
												histn = Form("ECals3s5vsRig_r%d_q%d_period%d_L8L9Sur_Sel", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ratio_ecal0, dWeigh);
											}
											...
											histn = Form("FSChisqYvsRig_r%d_q%d_period%d_L8L9Sur_ChisqCut_Sam", ir, *it, iperiod);
											GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetChis(2,1), dWeigh);
											if(ev->GetChis(2,1) < 10){
												histn = Form("FSChisqYvsRig_r%d_q%d_period%d_L8L9Sur_ChisqCut_Sel", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), ev->GetChis(2,1), dWeigh);
											}

											if(ev->ECalMIPsSel() && ev->ECalMaxEdepSel()){
												histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALmip", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
												histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALmip", ir, *it, iperiod);
												GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_pos[8][0], ev->tk_pos[8][1], dWeigh);
												histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip_purity", ir, *it, iperiod);
												if(dUnbiasedL9Q > *it-0.5) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_pos[8][0], ev->tk_pos[8][1], dWeigh);

												// cout << "\t====> Find Events with Unbiased L9" << endl;
												// for(int i = 0; i < 9; i++){
												// 	cout << i << "\t Tracker Ladder ID: " << ev->tk_lid[i] << endl;
												// }

												bool bL9QSel = ev->IsUBXYExtQTrkCor(1,iQVer) && ((ev->GetUBExtQStatusTrkCor(1) & 0x10013D)==0) && dUnbiasedL9Q>*it-1.5;
												// bool bL9QSel = ev->HasTkLHitXY(8)==2 && ev->Select_ExtQ(*it, 3, iQVer);
												bL9QSel = bL9QSel && ev->GetChis(2,1) < 10;
												if(bL9QSel){
													histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip", ir, *it, iperiod);
													if(dUnbiasedL9Q > *it-0.5) GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->tk_pos[8][0], ev->tk_pos[8][1], dWeigh);
													histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sel_ECALmip", ir, *it, iperiod);
												...
												if(ev->GetRigidity(2) > 0){
													histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALmip_RigCut0", ir, *it, iperiod);
													GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
													if(ev->GetChis(2,1) < 10 && ev->nshow < 3){
														histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_ECALmip_RigCut0_ChisqCut_Sel", ir, *it, iperiod);
														GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
													}
													if(bL9QSel){
												...
												bool bPosCut = ev->tk_pos[8][1] > 0;
												bPosCut = ev->tk_pos[8][1] < -15 && ev->tk_pos[8][1] > -24;
												bPosCut = bPosCut && ev->tk_pos[8][0] > 16 && ev->tk_pos[8][0] < 22;
												if(bPosCut){
													histn = Form("UnbiasedL9QvsRig_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALmip_poscut", ir, *it, iperiod);
													GetHist2d(hmanQ[iQVer], histn.c_str())->Fill(ev->GetRigidity(iReferSpan), dUnbiasedL9Q, dWeigh);
													histn = Form("L9PosXY_r%d_q%d_period%d_L8L9Sur_Efficiency_Sam_ECALmip_poscut", ir, *it, iperiod);
				...
				for (int ir=1; ir<3; ir++){
					...
					bool bMCSurSam = false;
					bool bMCSurSel = false;
					bool bMCSurSel2 = false;
					...
					for (it=viQSel.begin(); it!=viQSel.end(); it++){
						for (int iQVer=QVeri; iQVer<nQVer; iQVer++){
							...
							// bMCSurSam = ev->IsPassTkL(0); // layer-1
							bMCSurSam = ev->Select_GeomAccept(ir);
							bMCSurSel = ev->PrimaryUpToTrLayer(1);
							if(bMCSurSam){
								histn = Form("rig_r%d_q%d_MCSur_Trrige%d_AboveL1ToL1_Sam_gen",  ir, *it, mctrigger);
								GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch);
							...
							// bMCSurSam = ev->IsPassTkL(1); // layer-1
							bMCSurSam = ev->Select_GeomAccept(ir);
							bMCSurSel = ev->PrimaryUpToTrLayer(2);
							if(bMCSurSam){
								histn = Form("rig_r%d_q%d_MCSur_Trrige%d_AboveL1ToL2_Sam_gen",  ir, *it, mctrigger);
								GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch);
							...
							// bMCSurSam = ev->IsPassTkL(7); // layer-1
							bMCSurSam = ev->Select_GeomAccept(ir);
							bMCSurSel = ev->PrimaryUpToTrLayer(8);
							if(bMCSurSam){
								histn = Form("rig_r%d_q%d_MCSur_Trrige%d_AboveL1ToL8_Sam_gen",  ir, *it, mctrigger);
								GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch);
							...
							// bMCSurSam = ev->IsPassTkL(8); // layer-1
							bMCSurSam = ev->Select_GeomAccept(ir);
							bMCSurSel = ev->PrimaryUpToTrLayer(9);
							if(bMCSurSam){
								histn = Form("rig_r%d_q%d_MCSur_Trrige%d_AboveL1ToL9_Sam_gen",  ir, *it, mctrigger);
								GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch);
							...
							// bMCSurSam = bTkgeom[ir] && ev->PrimaryUpToTrLayer(1);
							bMCSurSam = ev->Select_GeomAccept(ir) && ev->PrimaryUpToTrLayer(1);
							bMCSurSel = ev->PrimaryUpToTrLayer(2);
							if(bMCSurSam){
								histn = Form("rig_r%d_q%d_MCSur_Trrige%d_L1ToL2_Sam_gen",  ir, *it, mctrigger);
								GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch);
							...
							// bMCSurSam = bTkgeom[ir] && ev->PrimaryUpToTrLayer(2);
							bMCSurSam = ev->Select_GeomAccept(ir) && ev->PrimaryUpToTrLayer(2);
							bMCSurSel = ev->PrimaryUpToTrLayer(8);
							if(bMCSurSam){
								histn = Form("rig_r%d_q%d_MCSur_Trrige%d_L2ToL8_Sam_gen",  ir, *it, mctrigger);
								GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch);
							...
							// bMCSurSam = bTkgeom[ir] && ev->PrimaryUpToTrLayer(8);
							bMCSurSam = ev->Select_GeomAccept(ir) && ev->PrimaryUpToTrLayer(8);
							bMCSurSel = ev->PrimaryUpToTrLayer(9);
							if(bMCSurSam){
								histn = Form("rig_r%d_q%d_MCSur_Trrige%d_L8ToL9_Sam_gen",  ir, *it, mctrigger);
								GetHist(hmanQ[iQVer], histn.c_str())->Fill(ev->mmom/ev->mch);
